...
an unlinked SQLite DB/WAL/SHM generation.  ``anika_state`` supplies only the
...
_ANIKA_EXECUTABLES = frozenset({"anika", "anika-agent", "anika-acp"})
_ANIKA_PYTHON_MODULES = frozenset({"acp_adapter", "anika_cli.main"})
_ANIKA_PYTHON_SCRIPTS = frozenset({"anika_cli/main.py", "run_agent.py"})
...
def _looks_like_anika(argv: Sequence[str]) -> bool:
    """Return whether argv identifies a supported Anika execution target."""
    if not argv:
        return False
    program = os.path.basename(argv[0]).lower().removesuffix(".exe")
    if program in _ANIKA_EXECUTABLES:
        return True
    ...
    if kind == "module":
        return normalized in _ANIKA_PYTHON_MODULES
    return any(
        normalized == script or normalized.endswith(f"/{script}")
        for script in _ANIKA_PYTHON_SCRIPTS
    )
        ...
        try:
            ...
            for pid_str in os.listdir("/proc"):
                ...
                try:
                    ...
                    if argv is not None and _looks_like_anika(argv):
                        cmdline = " ".join(argv)
