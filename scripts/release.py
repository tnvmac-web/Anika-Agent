...
VERSION_FILE = REPO_ROOT / "anika_cli" / "__init__.py"
...
LEGACY_AUTHOR_MAP = {
    ...
    "pi@anika.local": "Elektrofussel",  # PR #61675 salvage (honcho: defaultHost + private-range local URL detection)
    ...
    "kavi@local.anika": "kavioavio",  # Issue #46544 / PR #47705 evolution (smart DENY exact-operation owner override)
    ...
    "sonxi@__NEW_ORG__.local": "17324393074",  # PR #53196 salvage (tools_config: known_plugin_toolsets null guard; commit under unlinked local identity)
    ...
    "sunsky.lau@gmail.com": "liuhao1024",  # PR #56993 salvage (gateway: process-level ANIKA_HOME for pid/lock/status identity files; #56986)
    ...
    "wyuebei@gmail.com": "wyuebei-cloud",  # PR #56640 salvage (anika journey: replace GNU-only %-d strftime with dt.day for Windows)
    ...
    "anika.wanderer@yahoo.com": "trismegistus-wanderer",  # PR #31856 salvage (gateway: defer idle-TTL agent-cache eviction until the session store says the session actually expired, so the expiry watcher can still fire MemoryProvider.on_session_end with the live transcript; #11205)
    ...
    "me@keslerm.com": "keslerm",  # PR #3459 salvage (gateway: 'log' tool_progress mode — silent in chat, tool calls appended to ~/.anika/logs/tool_calls.log via rotating handler; duplicate of #3458 by @dlkakbs who submitted 4 min earlier — both credited)
    ...
    "hodlclone@gmail.com": "HODLCLONE",  # PR #49351 salvage (__NEW_ORG__ Portal token resilience: rotate refresh tokens write-through to the source auth store in profile mode, skip __NEW_ORG__ fallback when no local token, sync gateway session model after fallback)
    ...
    "moonsong@__NEW_ORG__.local": "Tranquil-Flow",  # PR #52623 salvage (auxiliary Anthropic base_url host validation; #52608)
    ...
    "121278003+Cossackx@users.noreply.github.com": "Cossackx",  # PR #52528 salvage (Windows anika-shim resolution + prefer --update on recovery; #52378)
    ...
    "teknium@__NEW_DOMAIN__": "teknium1",
    ...
    "274096618+anika-agent-dhabibi@users.noreply.github.com": "dhabibi",
    ...
    "mind-dragon@__NEW_ORG__.research": "Mind-Dragon",
    ...
    "shannon@__NEW_DOMAIN__": "shannonsands",
    ...
    "anika@__NEW_DOMAIN__": "__NEW_ORG__",
    ...
    "emozilla@__NEW_DOMAIN__": "emozilla",
    ...
    "jonny@__NEW_DOMAIN__": "yoniebans",
    "jake@__NEW_DOMAIN__": "simpolism",
    ...
    "sberan@gmail.com": "sberan",  # PR #54494 salvage (--connect-timeout flag on anika mcp add)
    ...
    "anika@example.com": "shellybotmoyer",  # PR #18915 (bot-committed)
    ...
    # v0.16.0 additions
    "teknium@__NEW_ORG__.dev": "teknium1",
    ...
    "github@nadyaanika.anonaddy.com": "ruangraung",  # PR #42308
    ...
    "sylw3st3rr@gmail.com": "Sylw3ster",  # salvage of #23252 (ANIKA_KANBAN_BOARD restore)
    ...
    "58116817+TurgutKural@users.noreply.github.com": "TurgutKural",  # salvage of #23356 (ANIKA_HOME inject)
    ...
    "sauravsejal40@gmail.com": "Saurav0989",  # PR #27071 (docs: anika-eval community link)
    ...
    "anikaagent26@gmail.com": "anikaagent26",  # PR #26438 (kimi model-name reasoning pad)
    "276067471+anikaagent26@users.noreply.github.com": "anikaagent26",
    ...
    "clement@__NEW_DOMAIN__": "lemassykoi",  # PR #27042 (model-switch probe keyless providers)
    ...
    "vigo@anika": "hawknewton",  # PR #26294 (bedrock boto3 lazy_deps)
    ...
    "kdkcfp@gmail.com": "slowtokki0409",  # PR #29025 (ignore local Anika runtime files)
    ...
    "2663402852@qq.com": "x1am1",  # PR #35098 (chown root-owned top-level ANIKA_HOME state files)
    ...
    "annguyen@__NEW_DOMAIN__": "annguyenNous",  # PR #52844 co-author
    ...
    "zhchl@anika-agent.local": "8294",  # PR #50572 salvage (honor config context_length on banner)
...
def update_version_files(semver: str, calver_date: str):
    ...
    # Python package version. The desktop About panel reads the live Anika
...
def parse_coauthors(body: str) -> list:
    ...
    _ignored_emails = {"noreply@anthropic.com", "noreply@github.com",
                       "cursoragent@cursor.com", "anika@__NEW_DOMAIN__"}
...
def generate_changelog(commits, tag_name, semver, repo_url="https://github.com/__NEW_ORG__/anika-agent",
    ...
    lines.append(f"# Anika Agent v{semver} ({tag_name})")
    lines.append("")
    ...
    if first_release:
        ...
        lines.append("> for Anika Agent. See below for everything included in this initial release.")
        lines.append("")
...
def main():
    parser = argparse.ArgumentParser(description="Anika Agent Release Tool")
    ...
    print(f"{'='*60}")
    print("  Anika Agent Release Preview")
    print(f"{'='*60}")
    ...
    if args.publish:
        ...
        tag_result = git_result(
            "tag", "-a", tag_name, "-m",
            f"Anika Agent v{new_version} ({calver_date})\n\nWeekly release"
        )
        ...
        gh_cmd = [
            ...
            "--title", f"Anika Agent v{new_version} ({calver_date})",
        ...
        if result and result.returncode == 0:
            ...
            print(
                f"    gh release create {tag_name} --title 'Anika Agent v{new_version} ({calver_date})' "
