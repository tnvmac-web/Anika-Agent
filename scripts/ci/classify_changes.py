...
# tree generates llms.txt — the index every LLM (Anika included, via the
# anika-agent skill) reads to learn what Anika can do — and
...
_MCP_CATALOG_FILES = {"anika_cli/mcp_catalog.py"}
...
# Rust crates — currently just the Tauri bootstrap installer (Anika-Setup).
...
def _py_test_only(p: str) -> bool:
    ...
    Product jobs (Desktop E2E's ``anika serve`` backend, the Docker image)
