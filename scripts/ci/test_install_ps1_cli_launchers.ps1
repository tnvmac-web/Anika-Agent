# Behavioral test for install.ps1's anika launcher staging (PR #92092,
...
#
# The test lifts the real Install-AnikaCommandLaunchers function from the
...
# the caller (Set-PathVariable passes $AnikaHome\bin -- the managed binary
...
$fn = $ast.Find({
    ...
    $n.Name -eq 'Install-AnikaCommandLaunchers'
}, $true)

if (-not $fn) {
    throw "Install-AnikaCommandLaunchers not found in $installPs1"
}
...
$caseRoot = [System.IO.Path]::GetFullPath((Join-Path $tempBase (
    'anika-cli-launcher-test-' + [guid]::NewGuid().ToString('N')
)))
...
try {
    $installRoot = Join-Path $caseRoot 'anika-agent'
    ...
    $missingThrew = $false
    try {
        Install-AnikaCommandLaunchers -Root $installRoot -Destination $binDir | Out-Null
    } catch {
        $missingThrew = $_.Exception.Message -like '*required launcher not found*'
    }
    Assert-True $missingThrew 'missing anika.exe fails the launcher stage'
    ...
    $anikaV1 = [byte[]](77, 90, 1)
    $anikaV2 = [byte[]](77, 90, 2)
    $acp = [byte[]](77, 90, 3)
    [System.IO.File]::WriteAllBytes((Join-Path $scriptsDir 'anika.exe'), $anikaV1)
    Set-Content -Path (Join-Path $installRoot 'venv\pyvenv.cfg') `
        -Value "home = X" -Encoding Ascii

    $staged = Install-AnikaCommandLaunchers -Root $installRoot -Destination $binDir
    Assert-True ($staged -eq $binDir) 'returns the destination it staged into'
    Assert-BytesEqual $anikaV1 `
        ([System.IO.File]::ReadAllBytes((Join-Path $binDir 'anika.exe'))) `
        'normal venv: exe copy lands in the destination'
    Assert-True (-not (Test-Path -LiteralPath (Join-Path $binDir 'anika-acp.exe'))) `
        'optional ACP launcher may be absent'

    [System.IO.File]::WriteAllBytes((Join-Path $scriptsDir 'anika.exe'), $anikaV2)
    [System.IO.File]::WriteAllBytes((Join-Path $scriptsDir 'anika-acp.exe'), $acp)
    Install-AnikaCommandLaunchers -Root $installRoot -Destination $binDir | Out-Null
    Assert-BytesEqual $anikaV2 `
        ([System.IO.File]::ReadAllBytes((Join-Path $binDir 'anika.exe'))) `
        'installer refreshes an existing Anika launcher'
    Assert-BytesEqual $acp `
        ([System.IO.File]::ReadAllBytes((Join-Path $binDir 'anika-acp.exe'))) `
    ...
    Install-AnikaCommandLaunchers -Root $installRoot -Destination $binDir | Out-Null
    Assert-True (Test-Path -LiteralPath (Join-Path $binDir 'anika.cmd')) `
        'relocatable venv: .cmd delegator staged'
    Assert-True (-not (Test-Path -LiteralPath (Join-Path $binDir 'anika.exe'))) `
        'relocatable venv: stale exe copy removed'
    $cmdBody = [System.IO.File]::ReadAllText((Join-Path $binDir 'anika.cmd'))
    Assert-True ($cmdBody.Contains((Join-Path $scriptsDir 'anika.exe')) -and $cmdBody.Contains('%*')) `
