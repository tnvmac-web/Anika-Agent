#!/usr/bin/env python3
"""Live test harness for Anika Agent's Tool Search feature.
...
# Force-isolate the test environment BEFORE any anika imports.
ORIGINAL_HOME = os.environ.get("ANIKA_HOME")
ORIGINAL_AUTH = Path.home() / ".anika" / "auth.json"
...
def setup_isolated_home(enabled: bool, listing: str = "off",
    ...
    """Create a fresh ~/.anika/ for one test, copying minimal credentials.

    Also reads OPENROUTER_API_KEY from the user's real ``~/.anika/.env`` so
    the agent can authenticate against OpenRouter inside the isolated home.
    """
    home_dir = Path(tempfile.mkdtemp(prefix="anika_ts_live_"))
    anika_home = home_dir / ".anika"
    anika_home.mkdir(parents=True)

    if ORIGINAL_AUTH.exists():
        shutil.copy(ORIGINAL_AUTH, anika_home / "auth.json")

    # Copy .env so OPENROUTER_API_KEY (or others) are visible to the agent
    # running inside the isolated home.
    real_env_file = Path.home() / ".anika" / ".env"
    if real_env_file.exists():
        shutil.copy(real_env_file, anika_home / ".env")
        ...
        load_anika_dotenv(anika_home=str(Path.home() / ".anika"))
    ...
    (anika_home / "config.yaml").write_text(_yaml_dump(cfg), encoding="utf-8")
    return anika_home
...
def reset_module_state():
    """Drop cached modules so the new ANIKA_HOME takes effect."""
    keys = [k for k in sys.modules.keys()
            if k.startswith(("tools.", "model_tools", "toolsets",
                             "anika_cli", "agent.", "run_agent"))]
    for k in keys:
        del sys.modules[k]
...
def run_one_scenario(scenario: Dict[str, Any], enabled: bool, out_dir: Path) -> Dict[str, Any]:
    ...
    os.environ["ANIKA_HOME"] = str(home)
...
def main():
    ...
    # Restore original ANIKA_HOME
    if ORIGINAL_HOME is not None:
        os.environ["ANIKA_HOME"] = ORIGINAL_HOME
    else:
