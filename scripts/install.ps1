# ============================================================================
# Anika Agent Installer for Windows
# ============================================================================
...
param(
    ...
    [switch]$ForceCommit,
    [string]$Tag = "",
    [string]$AnikaHome = $(if ($env:ANIKA_HOME) { $env:ANIKA_HOME } else { "$env:LOCALAPPDATA\anika" }),
    [string]$InstallDir = $(if ($env:ANIKA_HOME) { "$env:ANIKA_HOME\anika-agent" } else { "$env:LOCALAPPDATA\anika\anika-agent" }),
    ...
    # builds apps/desktop into a launchable Anika.exe.
    #
    # Why opt-in:
    #   * Anika-Setup.exe (the signed Tauri bootstrap installer) passes
    ...
    #     from inside an already-launched Anika.exe; if THAT recursively
    #     built apps/desktop it would try to overwrite the live Anika.exe
    ...
    #     `anika desktop` already builds on demand.
...
# ANIKA_HOME and InstallDir -- in that short form:
...
function Write-PathDiag {
    ...
    [Console]::Error.WriteLine("[anika] $Message")
}
...
function ConvertTo-LongPath {
    ...
    try {
        if (-not ([System.Management.Automation.PSTypeName]'AnikaInstall.LongPath').Type) {
            Add-Type -Namespace 'AnikaInstall' -Name 'LongPath' -MemberDefinition @'
...
'@
        ...
        $length = [AnikaInstall.LongPath]::GetLongPathNameW($Path, $buffer, $buffer.Capacity)
...
# long. An explicitly passed -AnikaHome / -InstallDir is normalized in place
...
if ($PSBoundParameters.ContainsKey('AnikaHome')) {
    $AnikaHome = ConvertTo-LongPath $AnikaHome
} else {
    $AnikaHome = ConvertTo-LongPath $(
        if ($env:ANIKA_HOME) { $env:ANIKA_HOME } else { "$env:LOCALAPPDATA\anika" }
    )
}
if ($PSBoundParameters.ContainsKey('InstallDir')) {
    ...
    $InstallDir = ConvertTo-LongPath $(
        if ($env:ANIKA_HOME) { "$env:ANIKA_HOME\anika-agent" } else { "$env:LOCALAPPDATA\anika\anika-agent" }
    )
}
if ($script:NormalizedProfilePaths) {
    ...
    Write-PathDiag "resolved install paths: AnikaHome=$AnikaHome InstallDir=$InstallDir"
}
...
$RepoUrlSsh = "git@github.com:__NEW_ORG__/anika-agent.git"
$RepoUrlHttps = "https://github.com/__NEW_ORG__/anika-agent.git"
$PythonVersion = "3.11"
...
function Write-Banner {
    ...
    Write-Host "|             * Anika Agent Installer                    |" -ForegroundColor Magenta
    Write-Host "+---------------------------------------------------------+" -ForegroundColor Magenta
    Write-Host "|  An open source AI agent by __NEW_ORG__.              |" -ForegroundColor Magenta
    Write-Host "+---------------------------------------------------------+" -ForegroundColor Magenta
    Write-Host ""
}
...
function Write-BrowserEnv {
    ...
    if (-not (Test-Path $AnikaHome)) {
        New-Item -ItemType Directory -Force -Path $AnikaHome | Out-Null
    }
    $envFile = Join-Path $AnikaHome ".env"
...
function Install-AgentBrowser {
    ...
    # which every consumer (tools/browser_tool.py, `anika update`'s npx
    ...
    $prefixDir = Join-Path $AnikaHome "node"
...
function Install-Uv {
    # Anika owns its own uv at $AnikaHome\bin\uv.exe.  Always install there --
    # no PATH probing, no conda guards, no multi-location resolution chains.
    # The runtime update path (anika_cli/managed_uv.py) looks in the same
    # place, so install.ps1 and `anika update` stay in sync.
    $managedUv = Join-Path $AnikaHome "bin\uv.exe"
    ...
    Write-Info "Installing managed uv into $AnikaHome\bin ..."
    New-Item -ItemType Directory -Path (Join-Path $AnikaHome "bin") -Force | Out-Null

    # UV_INSTALL_DIR tells the astral installer to place the binary
    # directly into $AnikaHome\bin instead of ~/.local/bin.
    $prevEAP = $ErrorActionPreference
    try {
        $ErrorActionPreference = "Continue"
        $env:UV_INSTALL_DIR = Join-Path $AnikaHome "bin"
        ...
        # (anika_cli/managed_uv.py looks only at $AnikaHome\bin\uv.exe).
...
# Put the Anika-managed Node dir at the FRONT of the persisted User PATH.
#
...
# environment (a standalone anika-setup.exe run, a user typing `npm`) silently
...
# Upgrade the Anika-managed Node tree's bundled npm into $NpmRange when
...
function Update-ManagedNpm {
    ...
    if (Test-ManagedNodeInUse $NodeDir) {
        Write-Warn "Anika-managed Node.js is in use by a running app; skipping the bundled npm upgrade (applies on a later update with the app closed)."
        return $false
    }

    Write-Info "Upgrading bundled npm to satisfy $range ..."

    $tmpCwd = Join-Path $env:TEMP ("anika-npm-upgrade-" + [Guid]::NewGuid().ToString("N"))
...
function Resolve-UvCmd {
    ...
    # sibling process and ANIKA_HOME wasn't propagated).
...
function Initialize-ManagedPythonEnvironment {
    # Python used by Anika belongs to the checkout, never to another
    # application or a user-level uv configuration. Keep this aligned with
    # anika_cli.managed_uv.managed_python_env(), which owns the update path.
    foreach ($name in @(
    ...
    $managedRoot = Join-Path $InstallDir ".anika-runtime\python"
...
function Resolve-AvailablePythonVersion {
    # Return the path and minor version of the first Anika-managed interpreter
    ...
    #
    # Under Anika-Setup.exe each stage runs in a fresh powershell.exe. The
    ...
    foreach ($ver in $candidates) {
        ...
        try {
            ...
            throw "Failed to resolve Anika-managed Python $ver`: $_"
        } finally {
...
function New-GitBashAslrFailureReason {
    ...
    return @(
        ...
        "Then rerun Anika setup. If the override is blocked or later re-applied, ask your Windows administrator to allow this per-program exception."
...
function Install-Git {
    ...
    which Anika uses to run shell commands.
    ...
    package manager):
      ...
      2. Download **PortableGit** from the official git-for-windows GitHub
         ...
         ``%LOCALAPPDATA%\anika\git`` -- never touches system Git, never
    ...
    **Why PortableGit, not MinGit:**  MinGit is the minimal-automation
    ...
    Anika needs ``bash.exe`` to run shell commands.  PortableGit is the
    ...
    blocked).  Owning the Anika copy of Git ourselves is predictable and
    recoverable: if it ever breaks, ``Remove-Item %LOCALAPPDATA%\anika\git``
    and re-running this installer fully recovers.

    After install we locate ``bash.exe`` and persist the path in
    ``ANIKA_GIT_BASH_PATH`` (User scope) so Anika can find it in a fresh
    ...
    if (Get-Command git -ErrorAction SilentlyContinue) {
        ...
        Write-Info "Trying a Anika-managed PortableGit install instead..."
    }

    # Download PortableGit into $AnikaHome\git.  Always works as long as
    # we can reach github.com -- no admin, no winget, no reliance on the
    # user's possibly-broken system Git install.
    Write-Info "Git not found -- downloading PortableGit to $AnikaHome\git\ ..."
    ...
    try {
        ...
        if ($arch -eq "32-bit-mingit") {
            Write-Warn "32-bit Windows detected -- PortableGit is 64-bit only.  Installing MinGit 32-bit as a last resort; bash-dependent Anika features (terminal tool, agent-browser) will not work on this machine."
        ...
        Write-Info "then re-run this installer.  Anika needs Git Bash on Windows to run"
...
function Set-GitBashEnvVar {
    ...
    ``ANIKA_GIT_BASH_PATH`` (User env scope) so Anika can find it even before
    ...
    #
    # Layouts:
    #   PortableGit (our default): $AnikaHome\git\bin\bash.exe
    #   MinGit (32-bit fallback):  $AnikaHome\git\usr\bin\bash.exe
    $candidates += "$AnikaHome\git\bin\bash.exe"       # PortableGit layout (primary)
    $candidates += "$AnikaHome\git\usr\bin\bash.exe"   # MinGit / PortableGit usr\bin fallback
    ...
    foreach ($candidate in $candidates) {
        if ($candidate -and (Test-Path $candidate)) {
            [Environment]::SetEnvironmentVariable("ANIKA_GIT_BASH_PATH", $candidate, "User")
            $env:ANIKA_GIT_BASH_PATH = $candidate
            $script:GitBashPath = $candidate
            Write-Info "Set ANIKA_GIT_BASH_PATH=$candidate"
            return
        }
    }

    Write-Warn "Could not locate bash.exe -- Anika may not find Git Bash."
    Write-Info "If needed, set ANIKA_GIT_BASH_PATH manually to your bash.exe path."
}
...
# range used to provision the Anika-managed tree. Keeping this probe separate
...
function Test-SystemNodeReady {
    ...
    if (Test-NodeVersionOk $version) {
        ...
        Write-Warn "Node.js $version is unsupported (Anika requires Node 22.22+, 24.11+, or 26+)"
        return $false
    }
    ...
    if ($npmVersion) {
        Write-Warn "Node.js $version uses npm $npmVersion, which does not satisfy Anika requirement $npmRange"
    } else {
...
function Test-Node {
    ...
    Write-Info "Using a Anika-managed Node.js installation instead..."

    # Prefer a Anika-managed Node from a previous run over a too-old system one.
    $managedNode = "$AnikaHome\node\node.exe"
    if ((Test-Path $managedNode) -and (Test-NodeVersionOk (& $managedNode --version))) {
        $version = & $managedNode --version
        $env:Path = "$AnikaHome\node;$env:Path"
        Set-ManagedNodeFirstOnUserPath "$AnikaHome\node"
        Write-Success "Node.js $version found (Anika-managed)"
        ...
        Update-ManagedNpm "$AnikaHome\node" | Out-Null
        $script:HasNode = $true
        return $true
    }

    Write-Info "Installing Anika-managed Node.js $NodeVersion LTS..."
    ...
    # The portable zip path drops node.exe + npm into $AnikaHome\node\
    ...
    Write-Info "Downloading portable Node.js $NodeVersion to $AnikaHome\node\ ..."
    ...
    try {
        ...
        if ($zipName) {
            ...
            $tmpDir = "$env:TEMP\anika-node-extract"
            ...
            if ($extractedDir) {
                ...
                Get-ChildItem "$AnikaHome" -Directory -Filter "node.old-*" -ErrorAction SilentlyContinue |
                    Where-Object { $_.LastWriteTime -lt (Get-Date).AddMinutes(-10) } |
                    Remove-Item -Recurse -Force -ErrorAction SilentlyContinue
                Get-ChildItem "$AnikaHome" -Directory -Filter "node.new-*" -ErrorAction SilentlyContinue |
                    Where-Object { $_.LastWriteTime -lt (Get-Date).AddMinutes(-10) } |
                    Remove-Item -Recurse -Force -ErrorAction SilentlyContinue
                ...
                if (Test-Path "$AnikaHome\node") {
                    try {
                        Rename-Item "$AnikaHome\node" $backup -ErrorAction Stop
                    } catch {
                        Write-Warn "Anika-managed Node.js is in use by a running app; deferring its upgrade. Close the app and re-run the update."
                ...
                $version = & "$AnikaHome\node\node.exe" --version
                Write-Success "Node.js $version installed to $AnikaHome\node\ (portable, user-scoped)"
...
function Install-SystemPackages {
    ...
    if ($hasWinget) {
        ...
        foreach ($pkg in $wingetPkgs) {
            $log = "$env:TEMP\anika-winget-$($pkg -replace '[^A-Za-z0-9]','_')-$(Get-Random).log"
            $pkgLogs[$pkg] = $log
...
function Install-Repository {
        ...
        if ($repoValid) {
            ...
            try {
                ...
                if (-not [string]::IsNullOrWhiteSpace(($statusOut -join "`n"))) {
                    ...
                    # the index conflict state is dropped. Mirrors the `anika
                    # update` path (#4735).
                    ...
                    $stashName = "anika-install-autostash-" + (Get-Date -Format "yyyyMMdd-HHmmss")
                ...
                if ($Commit) {
                    ...
                    # BACKWARDS. anika-setup.exe bakes its build-time commit
                    ...
                    # ff-only pull cannot succeed -- mirror ``anika update`` and
                ...
                if ($autostashRef) {
                    ...
                    if ($restoreNow) {
                        ...
                        if (($restoreExit -eq 0) -and ($conflictedFiles.Count -eq 0)) {
                            ...
                            Write-Warn "Review git diff / git status if Anika behaves unexpectedly."
                        } else {
            ...
            try {
                ...
                Write-Info "terminals, running anika processes) and try again."
                throw
            }
        }
    }
    ...
    if (-not $didUpdate) {
        ...
        if (-not $cloneSuccess) {
            ...
            try {
                ...
                if ($Commit) {
                    $zipUrl = "https://github.com/__NEW_ORG__/anika-agent/archive/$Commit.zip"
                    $zipLabel = $Commit
                } elseif ($Tag) {
                    $zipUrl = "https://github.com/__NEW_ORG__/anika-agent/archive/refs/tags/$Tag.zip"
                    $zipLabel = $Tag
                } else {
                    $zipUrl = "https://github.com/__NEW_ORG__/anika-agent/archive/refs/heads/$Branch.zip"
                    $zipLabel = $Branch
                }
                $zipPath = "$env:TEMP\anika-agent-$zipLabel.zip"
                $extractPath = "$env:TEMP\anika-agent-extract"
                ...
                if ($extractedDir) {
                    ...
                    # `anika update` (see the notes at the shared clone-path
    ...
    # next `anika update` checkout aborts on a "dirty" tree the user never
...
function Install-Venv {
    ...
    # Re-resolve the interpreter before creating the venv.  Under Anika-Setup.exe
    ...
    if (-not $resolvedPython) {
        throw "Anika-managed Python is unavailable. Run install.ps1 -Stage python first."
    }
    ...
    if (Test-Path -LiteralPath "venv") {
        ...
        # speedups.pyd) are loaded as DLLs by any running anika process.
        ...
        if ($env:OS -eq "Windows_NT") {
            $myPid = $PID
            Write-Info "Stopping any running anika processes before recreating venv..."
            ...
            try {
                schtasks /Query /FO CSV 2>$null | ConvertFrom-Csv | Where-Object { $_.TaskName -like '*Anika_Gateway*' } | ForEach-Object {
                    $tn = $_.TaskName
            ...
            # The launcher CLI (anika.exe) plus its child tree.
            & taskkill /F /T /IM anika.exe /FI "PID ne $myPid" 2>$null | Out-Null
            # taskkill /IM anika.exe is NOT enough: the gateway/agent that a
            # scheduled task or watchdog autostarts runs as
            # `pythonw.exe -m anika_cli.main gateway run` straight out of
            # venv\Scripts\, so its image name is python/pythonw, not anika.exe.
            ...
            # Anika children re-exec through .anika-runtime, so killing only
            ...
            # user (see anika_cli/gateway_windows.py), so the installer always
        ...
        try {
            ...
            throw (
                ...
                "A process still has the install directory open (often a non-Anika " +
    ...
    $env:UV_PYTHON = $venvPythonExe
        ...
        # the task fires normally on next logon and `anika update` / the
...
function Restore-VenvBackup {
    ...
    # venv so Anika (and the venv-blocker probe) stay usable (#83149).
    $backupName = Get-PendingVenvBackup
    if (-not $backupName) { return }
    try {
...
function Install-Dependencies {
    ...
    if (Test-Path "uv.lock") {
        ...
        # empty and producing the broken state where `anika.exe` exists
...
"@ 2>$null
    ...
    if (-not $installed) {
        throw "Failed to install anika-agent package even with no extras. Inspect the uv pip install output above."
    }
    ...
    # isn't set, leaving venv\ empty and anika.exe broken with
    ...
    # here, not 30 seconds later when the user runs `anika`.
    if (-not $NoVenv) {
        $venvPython = "$InstallDir\venv\Scripts\python.exe"
        if (-not (Test-Path $venvPython)) {
            throw "Install reported success but $venvPython does not exist. The dependency sync likely landed in a sibling .venv\ directory. Re-run the installer; if it persists, close Anika processes and preserve existing venv directories before retrying. Do not delete venv in place."
        }
        ...
        if ($importExitCode -ne 0) {
            ...
            $hint = if (Test-Path $sibling) {
                "Detected sibling .venv\ at $sibling -- uv synced there instead of venv\. Close Anika processes, preserve the existing venv, and rerun the installer so the transactional recovery path can move directories safely."
            } else {
    ...
    Complete-VenvTransaction
        ...
        # a failed update leaves Anika and its blocker probe usable.
        Restore-VenvBackup
        Pop-Location
        throw
    }
    ...
    if (-not $NoVenv) {
        # uv on Windows can register anika.exe in dist-info/RECORD but fail to
        ...
        # `anika` command while anika-agent.exe / anika-acp.exe exist
...
"@ 2>$null
            if ($LASTEXITCODE -eq 0 -and $scriptNames) {
                ...
                if ($missing.Count -gt 0) {
                    ...
                    if ($stillMissing.Count -gt 0) {
                        ...
                        Write-Info "Workaround: `"$pythonExe`" -m anika_cli.main <command>"
                    } else {
    ...
    # users hit and lazy-import errors from `anika dashboard` are confusing.
    ...
    if (Test-Path $pythonExe) {
        ...
        try {
            & $pythonExe -m py_compile "$InstallDir\anika_cli\web_server.py" 2>&1 | Out-Null
            if ($LASTEXITCODE -eq 0) { $webServerSyntaxOk = $true }
        } catch { }
        $ErrorActionPreference = $prevEAP
        if (-not $webOk) {
            Write-Warn "fastapi/uvicorn not importable -- `anika dashboard` will not work."
            Write-Info "Attempting targeted install of [web] extra as last resort..."
            & $UvCmd pip install -e ".[web]"
            if ($LASTEXITCODE -eq 0) {
                Write-Success "[web] extra installed; `anika dashboard` should now work."
            } else {
        ...
        if (-not $webServerSyntaxOk) {
            throw "dashboard backend source failed syntax check: anika_cli/web_server.py"
        }
    }
...
function Install-AnikaCommandLaunchers {
    param(
        [Parameter(Mandatory=$true)] [string]$Root,
        [Parameter(Mandatory=$true)] [string]$Destination
    )

    # Expose ONLY the anika launchers on PATH -- never the whole
    # venv\Scripts directory, which contains python.exe / pip.exe and
    # silently hijacks the `python` command in every terminal (#83797).
    # Requiring anika.exe before creating the destination keeps the PATH
    # stage from reporting success with an unusable command (PR #92092).
    $scriptsDir = Join-Path $Root "venv\Scripts"
    $requiredSource = Join-Path $scriptsDir "anika.exe"
    if (-not (Test-Path -LiteralPath $requiredSource -PathType Leaf)) {
        throw "Cannot set up the anika command: required launcher not found: $requiredSource"
    }

    New-Item -ItemType Directory -Force -Path $Destination | Out-Null

    # Launcher form depends on the venv (keep in lockstep with
    # anika_cli/_install_repair.py): a normal venv's exe trampoline
    ...
    foreach ($launcher in @("anika", "anika-acp")) {
    ...
    $requiredExe = Join-Path $Destination "anika.exe"
    $requiredCmd = Join-Path $Destination "anika.cmd"
    if (-not ((Test-Path -LiteralPath $requiredExe -PathType Leaf) -or
              (Test-Path -LiteralPath $requiredCmd -PathType Leaf))) {
        throw "Cannot set up the anika command: launcher was not installed: $requiredExe"
    }
    return $Destination
}

function Set-PathVariable {
    Write-Info "Setting up anika command..."
    
    if ($NoVenv) {
        $anikaBin = "$InstallDir"
    } else {
        # $AnikaHome\bin is the managed binary dir (shared with the managed
        # uv), OUTSIDE the git checkout: `anika update`'s autostash
        ...
        # installer staged under anika-agent\bin. No git operation can ever
        # touch this dir. Staging and verification live in
        # Install-AnikaCommandLaunchers, which throws BEFORE any PATH
        # mutation when the launchers cannot be staged.
        $anikaBin = "$AnikaHome\bin"
        Install-AnikaCommandLaunchers -Root $InstallDir -Destination $anikaBin | Out-Null
    }
    ...
    #   anika-agent\bin -- lived inside the git checkout, where the update
    #                       autostash could sweep the launchers off disk
    # The anika-agent\bin FILES are left in place on purpose: editor/ACP
    ...
    if (-not $NoVenv) {
        ...
        if ($cleaned.Count -ne $items.Count) {
            ...
            Write-Info "Removed legacy launcher entries from user PATH (kept anika via $anikaBin)"
        }
    }
    
    if ($currentPath -notlike "*$anikaBin*") {
        [Environment]::SetEnvironmentVariable(
            "Path",
            "$anikaBin;$currentPath",
            "User"
        )
        Write-Success "Added to user PATH: $anikaBin"
    } else {
        Write-Info "PATH already configured"
    }
    
    # Set ANIKA_HOME so the Python code finds config/data in the right place.
    # Only needed on Windows where we install to %LOCALAPPDATA%\anika instead
    # of the Unix default ~/.anika
    $currentAnikaHome = [Environment]::GetEnvironmentVariable("ANIKA_HOME", "User")
    if (-not $currentAnikaHome -or $currentAnikaHome -ne $AnikaHome) {
        [Environment]::SetEnvironmentVariable("ANIKA_HOME", $AnikaHome, "User")
        Write-Success "Set ANIKA_HOME=$AnikaHome"
    }
    $env:ANIKA_HOME = $AnikaHome
    
    # Update current session
    $env:Path = "$anikaBin;$env:Path"
    
    Write-Success "anika command ready"
}

function Write-BootstrapMarker {
    # Writes $InstallDir\.anika-bootstrap-complete which tells the Anika
    ...
    $markerPath = Join-Path $InstallDir ".anika-bootstrap-complete"
    $marker = [ordered]@{
        schemaVersion = 1
...
function Copy-ConfigTemplates {
    ...
    # Create the ANIKA_HOME directory structure ($AnikaHome, default %LOCALAPPDATA%\anika)
    New-Item -ItemType Directory -Force -Path "$AnikaHome\cron" | Out-Null
    New-Item -ItemType Directory -Force -Path "$AnikaHome\sessions" | Out-Null
    New-Item -ItemType Directory -Force -Path "$AnikaHome\logs" | Out-Null
    New-Item -ItemType Directory -Force -Path "$AnikaHome\pairing" | Out-Null
    New-Item -ItemType Directory -Force -Path "$AnikaHome\hooks" | Out-Null
    New-Item -ItemType Directory -Force -Path "$AnikaHome\image_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$AnikaHome\audio_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$AnikaHome\memories" | Out-Null
    New-Item -ItemType Directory -Force -Path "$AnikaHome\skills" | Out-Null

    
    # Create .env
    $envPath = "$AnikaHome\.env"
    ...
    # Create config.yaml
    $configPath = "$AnikaHome\config.yaml"
    ...
    # (the default PS5 behaviour), and Anika's prompt-injection scanner
    ...
    # PowerShell version.
    $soulPath = "$AnikaHome\SOUL.md"
    if (-not (Test-Path $soulPath)) {
        # MUST match DEFAULT_SOUL_MD in anika_cli/default_soul.py. The runtime
...
"@
    ...
    Write-Success "Configuration directory ready: $AnikaHome"
    
    # Seed bundled skills into $AnikaHome\skills (manifest-based, one-time per skill)
    Write-Info "Syncing bundled skills to $AnikaHome\skills ..."
    ...
    if (Test-Path $pythonExe) {
        try {
            ...
            Write-Success "Skills synced to $AnikaHome\skills"
        } catch {
            # Fallback: simple directory copy
            $bundledSkills = "$InstallDir\skills"
            $userSkills = "$AnikaHome\skills"
            if ((Test-Path $bundledSkills) -and -not (Get-ChildItem $userSkills -Exclude '.bundled_manifest' -ErrorAction SilentlyContinue)) {
                Copy-Item -Path "$bundledSkills\*" -Destination $userSkills -Recurse -Force -ErrorAction SilentlyContinue
                Write-Success "Skills copied to $AnikaHome\skills"
            }
        }
    }
}
...
function Install-NodeDeps {
    if (-not $HasNode) {
        # Cross-process driver mode (Anika-Setup.exe runs each -Stage NAME
    ...
    if (-not $npmCmd) {
        ...
        Write-Info "Open a new PowerShell window and re-run 'anika setup tools' later."
        return
    }
    $npmExe = $npmCmd.Source
    if ($npmExe -like "*.ps1") {
    ...
    if (Test-Path "$InstallDir\package.json") {
        ...
        $browserLog = "$env:TEMP\anika-npm-browser-$(Get-Random).log"
        ...
        if ($browserNpmOk) {
            ...
            if (-not $npxExe) {
                ...
                $pwLog = "$env:TEMP\anika-playwright-install-$(Get-Random).log"
                Push-Location $InstallDir
    ...
    if (Test-Path "$tuiDir\package.json") {
        ...
        $tuiLog = "$env:TEMP\anika-npm-tui-$(Get-Random).log"
...
# failure is non-fatal (browser_exec can still run via uvx, and `anika tools`
...
function Install-BrowserUseCli {
    ...
    $managedBin = Join-Path $AnikaHome "bin"
    $managedBu = Join-Path $managedBin "browser-use.exe"
    # MANAGED-FIRST: only Anika' managed copy short-circuits. A browser-use
    ...
    $ErrorActionPreference = "Continue"
    try {
        # UV_TOOL_BIN_DIR keeps the binary inside Anika' managed bin dir,
        ...
        if ($LASTEXITCODE -eq 0) {
            ...
            Write-Info "Install later with: uv tool install browser-use  (or via 'anika tools')"
        }
    } catch {
...
# Provision it at install time so enabling the tool later -- via `anika
...
# paths still lazy-install via install_cua_driver() (anika_cli/tools_config)
...
function Install-CuaDriver {
    ...
    $ErrorActionPreference = "Continue"
    try {
        # Same upstream installer `anika computer-use install` runs. Bounded
        ...
        # Anika' _CUA_INSTALLER_TIMEOUT (660s).
        ...
        if (Wait-Job $job -Timeout 660) {
            ...
            if ($installedCuaDriver -and (Test-CuaDriverRuntimeContract -DriverPath $installedCuaDriver.Source)) {
                Write-Success "Computer Use driver installed (enable via 'anika tools' -> Computer Use)"
            } else {
                Write-Warn "Computer Use driver install did not produce a compatible runtime -- repair it before enabling the tool."
                Write-Info "Install later with: anika computer-use install"
            }
        } else {
...
# the final `electron` -> `Anika` rename dies with ENOENT and every re-run
...
function Install-Desktop {
    # Build apps/desktop into a launchable Anika.exe. Only called from
    ...
    #
    # The Tauri bootstrap installer's launch_anika_desktop command
    # resolves apps/desktop/release/win-unpacked/Anika.exe directly,
    ...
    # enforces the supported Node lines and prepends the Anika-managed Node to
    ...
    # The --dir mode produces an unpacked Anika.exe in
    ...
    # are NOT trying to work around). The Anika icon + product name are
    # stamped onto Anika.exe by our own rcedit step (Set-DesktopExeIdentity)
    ...
    $buildLog = "$env:TEMP\anika-desktop-build-$(Get-Random).log"
    ...
    $exeCandidates = @(
        "$desktopDir\release\win-unpacked\Anika.exe",
        "$desktopDir\release\win-arm64-unpacked\Anika.exe"
    )
    $found = $false
    $desktopExe = $null
    ...
    if (-not $found) {
        throw "Desktop build completed but no Anika.exe was found under $desktopDir\release\*-unpacked\"
    }

    # 3b. The Anika icon + identity are stamped onto Anika.exe by the
    ...
    #     SIDs under %LOCALAPPDATA% (electron/electron#51761, anika-agent#38216).
    ...
    #    Anika.exe. We deliberately do NOT point them at `anika desktop`: that
...
function New-DesktopShortcuts {
    ...
    try {
        ...
        $targets = @(
            (Join-Path ([Environment]::GetFolderPath('Programs')) 'Anika.lnk'),
            (Join-Path ([Environment]::GetFolderPath('Desktop')) 'Anika.lnk')
        )
        ...
        # with the Anika icon, but without this the shortcut can keep drawing
...
function Install-PlatformSdks {
    ...
    # ~/.anika/.env are importable.  Two problems this solves:
    #
...
function Invoke-SetupWizard {
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Skipping setup wizard (non-interactive). Configure via the GUI or 'anika setup'."
        return
    }
    ...
    # Run anika setup using the venv Python directly (no activation needed)
    if (-not $NoVenv) {
        & ".\venv\Scripts\python.exe" -m anika_cli.main setup
    } else {
        python -m anika_cli.main setup
    }
...
function Start-GatewayIfConfigured {
    ...
    $anikaCmd = "$InstallDir\venv\Scripts\anika.exe"
    if (-not (Test-Path $anikaCmd)) {
        $anikaCmd = "anika"
    }

    # If WhatsApp is enabled but not yet paired, run foreground for QR scan
    $whatsappEnabled = $content | Where-Object { $_ -match "^WHATSAPP_ENABLED=true" }
    $whatsappSession = "$AnikaHome\whatsapp\session\creds.json"
    if ($whatsappEnabled -and -not (Test-Path $whatsappSession)) {
        Write-Host ""
        Write-Info "WhatsApp is enabled but not yet paired."
        Write-Info "Running 'anika whatsapp' to pair via QR code..."
        Write-Host ""
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Start the gateway later with: anika gateway"
        return
    }
    ...
    if ($response -eq "" -or $response -match "^[Yy]") {
        ...
        try {
            $logFile = "$AnikaHome\logs\gateway.log"
            Start-Process -FilePath $anikaCmd -ArgumentList "gateway" `
                -RedirectStandardOutput $logFile `
                -RedirectStandardError "$AnikaHome\logs\gateway-error.log" `
                -WindowStyle Hidden
            Write-Success "Gateway started! Your bot is now online."
            Write-Info "Logs: $logFile"
            Write-Info "To stop: close the gateway process from Task Manager"
        } catch {
            Write-Warn "Failed to start gateway. Run manually: anika gateway"
        }
    } else {
        Write-Info "Skipped. Start the gateway later with: anika gateway"
    }
}
...
function Write-Completion {
    ...
    Write-Host "$AnikaHome\config.yaml"
    Write-Host "   API Keys:  " -NoNewline -ForegroundColor Yellow
    Write-Host "$AnikaHome\.env"
    Write-Host "   Data:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$AnikaHome\cron\, sessions\, logs\"
    Write-Host "   Code:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$AnikaHome\anika-agent\"
    Write-Host ""
    ...
    Write-Host ""
    Write-Host "   anika              " -NoNewline -ForegroundColor Green
    Write-Host "Start chatting"
    Write-Host "   anika setup        " -NoNewline -ForegroundColor Green
    Write-Host "Configure API keys & settings"
    Write-Host "   anika config       " -NoNewline -ForegroundColor Green
    Write-Host "View/edit configuration"
    Write-Host "   anika config edit  " -NoNewline -ForegroundColor Green
    Write-Host "Open config in editor"
    Write-Host "   anika gateway      " -NoNewline -ForegroundColor Green
    Write-Host "Start messaging gateway (Telegram, Discord, etc.)"
    Write-Host "   anika update       " -NoNewline -ForegroundColor Green
...
$InstallStages = @(
    ...
    @{ Name = "repository";       Title = "Cloning Anika repository";            Category = "install";      NeedsUserInput = $false; Worker = "Stage-Repository" }
    # Managed Python lives under $InstallDir\.anika-runtime, so the checkout
...
if ($IncludeDesktop) {
    ...
    # (Anika-Setup.exe), never via the irm|iex CLI one-liner.
    $InstallStages += @{ Name = "desktop"; Title = "Building desktop app"; Category = "install"; NeedsUserInput = $false; Worker = "Stage-Desktop" }
}
$InstallStages += @(
    @{ Name = "path";             Title = "Adding Anika to PATH";                Category = "finalize";     NeedsUserInput = $false; Worker = "Stage-Path" }
...
try {
    ...
    Write-Host "  Invoke-WebRequest -Uri 'https://anika-agent.__NEW_DOMAIN__/install.ps1' -OutFile install.ps1" -ForegroundColor Yellow
