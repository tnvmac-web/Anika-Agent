#!/usr/bin/env python3
"""Boot-time re-seed of a terminally-dead __NEW_ORG__ bootstrap session.

Background
----------
A __NEW_ORG__ bootstrap session (client_id ``anika-cli-vps``) can take a terminal
``invalid_grant`` and be quarantined locally — the refresh path clears the dead
tokens from ``auth.json`` and stamps
``providers.__NEW_ORG__.last_auth_error.relogin_required = true``. From then on every
...
``stage2-hook.sh`` seeds ``auth.json`` from ``ANIKA_AUTH_JSON_BOOTSTRAP`` only
...
``ANIKA_AUTH_JSON_REBOOTSTRAP`` (plus a restart). On boot we re-seed the Nous
...
Design constraints
------------------
- Pure stdlib, no anika_cli imports: runs early in the boot hook, before the
...
# from ANIKA_AUTH_JSON_BOOTSTRAP (create-only, blank-volume seed) so the two
# paths can never be confused: BOOTSTRAP seeds a fresh volume; REBOOTSTRAP
# overwrites a terminally-dead __NEW_ORG__ entry on an existing volume.
REBOOTSTRAP_ENV = "ANIKA_AUTH_JSON_REBOOTSTRAP"
BOOTSTRAP_CLIENT_ID = "anika-cli-vps"


def _nous_entry_is_terminal(nous_state: Any) -> bool:
    """True iff the on-disk __NEW_ORG__ provider entry is in the terminal/quarantined
    state AND holds no usable credential.

    Mirrors the ``terminal`` predicate in ``anika_cli.auth.get_nous_session_validity``:
...
def _extract_nous_from_seed(seed_raw: str) -> Optional[dict]:
    """Pull the ``providers.nous`` block out of a ANIKA_AUTH_JSON_REBOOTSTRAP
    payload. The payload is a full auth.json document (same shape as
    ANIKA_AUTH_JSON_BOOTSTRAP). Returns None unless it carries the expected VPS
    ...
    if not isinstance(nous, dict) or not nous:
        return None
    if __NEW_ORG__.get("client_id") != BOOTSTRAP_CLIENT_ID:
        return None
    if not (
        isinstance(__NEW_ORG__.get("access_token"), str)
        and nous["access_token"].strip()
        and isinstance(__NEW_ORG__.get("refresh_token"), str)
...
def _seed_is_newer(local_nous: Any, seed_nous: dict) -> bool:
    ...
    if not isinstance(local_nous, dict):
        return False
    local_obtained = _parse_timestamp(local___NEW_ORG__.get("obtained_at"))
    seed_obtained = _parse_timestamp(seed___NEW_ORG__.get("obtained_at"))
    return bool(
...
def reseed_if_terminal(auth_path: str, seed_raw: str) -> str:
    """Core logic. Returns a short status string for logging/testing:
      ...
      - "no_auth_file"     — auth.json absent (blank volume → let the normal
                             ANIKA_AUTH_JSON_BOOTSTRAP path handle it)
...
def main() -> int:
    ...
    if result == "reseeded":
        print("[rebootstrap] __NEW_ORG__ bootstrap session was terminal; re-seeded auth.json from "
              f"{REBOOTSTRAP_ENV}")
    elif result == "reseeded_newer":
        print("[rebootstrap] Applied newer orchestrator-issued __NEW_ORG__ bootstrap session from "
              f"{REBOOTSTRAP_ENV}")
    else:
