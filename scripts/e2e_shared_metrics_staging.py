...
ANIKA_HOME so the operator's own telemetry state is untouched.
...
STAGING = "https://telemetry.staging-__NEW_DOMAIN__/v1/telemetry"


def main() -> int:
    scratch = Path(tempfile.mkdtemp(prefix="anika-telemetry-e2e-"))
    os.environ["ANIKA_HOME"] = str(scratch)
    ...
    from anika_cli.observability.shared_metrics_send_config import (
        resolve_send_config,
    )
    ...
    from anika_cli.observability.shared_metrics_sender import (
    ...
    for index, metric_count in ((0, 3), (1, 140)):
        ...
        payload = {
            "schema_version": "anika.shared_metrics.v2",
    ...
    print(f"scratch ANIKA_HOME : {scratch}")
    ...
    print("  aws s3 ls --recursive "
          "s3://anika-agent-telemetry-staging-767397871023-us-west-2-an/raw/ "
          "| tail -20")
    return 0
