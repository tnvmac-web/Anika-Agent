...
measured in the production session DB) through `anika chat` twice — once per
...
Environment:
  ...
  ABEVAL_HOME    ANIKA_HOME for runs   (default: $ABEVAL_ROOT/home)
                 Must be a configured Anika home with credentials for the
...
(tracker: __NEW_ORG__/anika-agent#77056).
"""
import json
import os
import re
...
""".strip(),
            ...
            env.update({
                ...
                "ANIKA_HOME": str(HOME),
                "ANIKA_NEMO_RELAY_PLUGINS_TOML": str(relay_config),
            })
            q = TASKS[name].replace("{WORK}", str(work))
            t0 = time.time()
            try:
                p = subprocess.run(
                    [sys.executable, "-m", "anika_cli.main", "chat", "--query", q,
