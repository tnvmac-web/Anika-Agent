...
batch (tracker: [#77056](https://github.com/__NEW_ORG__/anika-agent/issues/77056)).
...
- **Two arms, one variable.** `baseline` and `fixes` runs differ ONLY by
  ...
  Anika home, same model, same tasks, same reps.
...
1. Create a dedicated Anika home with credentials for the models under test:
   ...
   integration at it; no Anika observability plugin needs to be enabled.
