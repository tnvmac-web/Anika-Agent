...
When Anika runs in TUI mode, the gateway child process communicates with
...
# ``get_anika_home() / "plugins"`` (user) and ``./.anika/plugins/`` (project,
# gated behind ``ANIKA_ENABLE_PROJECT_PLUGINS``) — see
# ``anika_cli/plugins.py:10-12``.  The guard only checked the bundled
...
# this script runs as a standalone subprocess — ``anika_constants`` isn't
...
def main() -> int:
    ...
    # Add repo root to sys.path so we can import anika_constants (this script
    ...
    # plugins in get_anika_home()/plugins/ and project plugins in
    # ./.anika/plugins/, where code like ori/hooks.py can spawn
    # subprocesses with inherited stdin — #67639).
    plugin_roots: list[Path] = [get_anika_home() / "plugins"]
    if os.environ.get("ANIKA_ENABLE_PROJECT_PLUGINS"):
        plugin_roots.append(Path.cwd() / ".anika" / "plugins")
