#!/usr/bin/env bash
# Canonical test runner for anika-agent. Run this instead of calling
...
#   * Proper venv activation (probes .venv, venv, then ~/.anika/...)
#
# Usage:
...
# (ANIKA_PYTHON is exported by the devShell hook and ships [dev] extras:
# pytest, pytest-asyncio, pytest-timeout, ruff, ty).
#
# A candidate must have pytest INSTALLED, not merely exist. The release venv
# at ~/.anika/anika-agent/venv has bin/activate but no pytest, so an
...
VENV_PYTHON=""
SKIPPED_VENVS=""
for candidate in "$REPO_ROOT/.venv" "$REPO_ROOT/venv" "$HOME/.anika/anika-agent/venv"; do
...
if [ -n "$VENV" ]; then
  PYTHON="$VENV_PYTHON"
elif [ -n "${ANIKA_PYTHON:-}" ] && [ -x "$ANIKA_PYTHON" ] \
    && "$ANIKA_PYTHON" -c 'import pytest' 2>/dev/null; then
  # Guard with an import check: ANIKA_PYTHON may point at the RELEASE
  # venv (no pytest) when inherited from a wrapped `anika` binary rather
  # than the devShell hook.
  PYTHON="$ANIKA_PYTHON"
  echo "▶ no local venv — using Nix dev venv via ANIKA_PYTHON: $PYTHON"
else
  echo "error: no virtualenv with pytest found in $REPO_ROOT/.venv or $REPO_ROOT/venv," >&2
  echo "       and ANIKA_PYTHON is not a python with pytest (enter the Nix devShell or create a venv)" >&2
  if [ -n "$SKIPPED_VENVS" ]; then
...
if [ -f "$HOME/.anika/pytest_live_guard.py" ]; then
  EXTRA_PYTHONPATH="$HOME/.anika"
...
#
#   * ANIKA_TEST_WORKERS / PATHS / FILE_TIMEOUT / FILE_RETRIES / SLICE are
#     read by run_tests_parallel.py at argparse-default time — inside the
#     stripped environment.
#   * ANIKA_TEST_IMAGE is read by tests/docker/conftest.py to skip its
...
#     ANIKA_GIT_SHA build-arg the workflow bakes in).
#
# These are test-infrastructure knobs, not credentials — same class as the
# ANIKA_RUN_SLOW_PET_TESTS / ANIKA_E2E_BROWSER opt-ins already forwarded.
# Keep this an explicit allowlist (no ANIKA_TEST_* glob) so the "no
# credential can leak" property stays auditable at a glance.
TEST_ENV=()
for _test_var in ANIKA_TEST_IMAGE ANIKA_TEST_WORKERS ANIKA_TEST_PATHS \
  ANIKA_TEST_FILE_TIMEOUT ANIKA_TEST_FILE_RETRIES ANIKA_TEST_SLICE; do
  if [ -n "${!_test_var:-}" ]; then
...
exec env -i \
  ...
  PYTHONHASHSEED=0 \
  PYTHONUTF8=1 \
  ${ANIKA_RUN_SLOW_PET_TESTS:+ANIKA_RUN_SLOW_PET_TESTS="$ANIKA_RUN_SLOW_PET_TESTS"} \
  ${ANIKA_E2E_BROWSER:+ANIKA_E2E_BROWSER="$ANIKA_E2E_BROWSER"} \
