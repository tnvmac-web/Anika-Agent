...
function Invoke-Normalization {
    ...
    try {
        ...
        # its diagnostics are all `[anika] `-prefixed, so the two separate
        ...
        $stderr = ($raw | Where-Object { $_ -like '`[anika`]*' }) -join "`n"
        $stdout = ($raw | Where-Object { $_ -notlike '`[anika`]*' }) -join "`n"
    } finally {
    ...
    return @{
        ...
        AnikaHome = $(if ($paths) { $paths.anika_home } else { $null })
...
$expectedInstallDir = "$($longRoot)${sep}AppData${sep}Local" + '\anika\anika-agent'
...
$result = Invoke-Normalization -Environment @{ TEMP = $shortTemp; TMP = $shortTemp } `
    -ExtraArgs @('-InstallDir', (Join-Path $shortProfile 'custom-anika'))
Assert-Equal -Expected (Join-Path $longRoot 'custom-anika') -Actual $result.InstallDir -Label "explicit -InstallDir keeps the caller's directory, on the long root"
