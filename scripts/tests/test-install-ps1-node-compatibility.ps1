...
# changing PATH, installing software, or touching the user's Anika home.

$repoRoot = Split-Path -Parent (Split-Path -Parent (Split-Path -Parent $MyInvocation.MyCommand.Path))
$installScript = Join-Path $repoRoot 'scripts\install.ps1'
$testRoot = Join-Path $env:TEMP ("anika-node-compatibility-test-" + [Guid]::NewGuid().ToString('N'))
$AnikaHome = Join-Path $testRoot 'home'
$InstallDir = Join-Path $testRoot 'missing-checkout'
. $installScript -AnikaHome $AnikaHome -InstallDir $InstallDir
