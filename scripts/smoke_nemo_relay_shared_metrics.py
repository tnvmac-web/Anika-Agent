"""Run a real Anika CLI turn and validate the Relay shared-metrics output."""
...
def _resolve_anika_executable(anika_repo: Path) -> Path:
    for relative_path in (
        Path(".venv") / "bin" / "anika",
        Path(".venv") / "Scripts" / "anika.exe",
    ):
        candidate = anika_repo / relative_path
        if candidate.is_file():
            return candidate
    discovered = shutil.which("anika")
    if discovered:
        return Path(discovered)
    raise SystemExit(
        "Anika executable not found in the repository virtual environment "
        "or on PATH"
    )
...
def _arguments() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=Path.cwd(),
        help="Anika source checkout containing .venv/bin/anika",
    )
    parser.add_argument(
        "--relay-python",
        type=Path,
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Directory for the isolated ANIKA_HOME and captured output",
    )
...
def _validate_store(database_path: Path) -> list[dict[str, Any]]:
    ...
    if by_name["anika.client.active"] != [
        {
        ...
        raise AssertionError(
            f"Unexpected client-active counter: {by_name['anika.client.active']}"
        )
    [model] = by_name["anika.model_route.count"]
    expected_model = {
        "name": "anika.model_route.count",
        "dimensions": {
            "model": MODEL_CANARY,
            "provider": "custom",
        },
    ...
    if model != expected_model:
        raise AssertionError(
            f"Unexpected model counter: {by_name['anika.model_route.count']}"
        )
    expected_start = {
    ...
    if by_name["anika.task_run.started"] != [expected_start]:
        raise AssertionError(
            f"Unexpected task start: {by_name['anika.task_run.started']}"
        )
    [terminal] = by_name["anika.task_run.finished"]
    ...
    [tool] = by_name["anika.tool_call.count"]
    ...
    lifecycle = by_name["anika.skill.lifecycle.count"]
    expected_actions = {
        "archived",
        "created",
        "edited",
    ...
    loads = by_name["anika.skill.load.count"]
...
def _validate_packages(
    ...
    try:
        ...
        raise RuntimeError(
            "The Anika development environment requires jsonschema"
        ) from exc
    ...
    if metrics["anika.client.active"] != [
        {
            "name": "anika.client.active",
            "type": "counter",
            "dimensions": {},
        ...
        raise AssertionError(
            f"Unexpected client-active metric: {metrics['anika.client.active']}"
        )
    [model] = metrics["anika.model_route.count"]
    if model["dimensions"] != {
        "model": MODEL_CANARY,
        "provider": "custom",
    } or model["value"] != 2:
        raise AssertionError(
            f"Unexpected model metric: {metrics['anika.model_route.count']}"
        )
    [terminal] = metrics["anika.task_run.finished"]
    ...
    [tool] = metrics["anika.tool_call.count"]
    if (
        tool["dimensions"]
        != {
            "approval_outcome": "not_required",
    ...
    lifecycle = metrics["anika.skill.lifecycle.count"]
    ...
    loads = metrics["anika.skill.load.count"]
    if {
        (
...
def main() -> int:
    args = _arguments()
    anika_repo = args.anika_repo.resolve()
    relay_python = args.relay_python.resolve() if args.relay_python else None
    anika = _resolve_anika_executable(anika_repo)
    ...
    if args.output_dir:
        ...
        root = Path(tempfile.mkdtemp(prefix="anika-relay-shared-metrics-"))
    home = root / "anika-home"
    ...
    try:
        ...
        env = os.environ.copy()
        env["ANIKA_HOME"] = str(home)
        python_paths = [str(anika_repo)]
    ...
    (root / "anika.stdout.txt").write_text(result.stdout, encoding="utf-8")
    (root / "anika.stderr.txt").write_text(result.stderr, encoding="utf-8")
    if result.returncode != 0:
        raise AssertionError(
            f"Anika exited with {result.returncode}\n"
            f"stdout:\n{result.stdout}\nstderr:\n{result.stderr}"
        )
    ...
    if PROMPT_CANARY not in json.dumps(request.get("messages", [])):
        raise AssertionError("Anika model request did not contain the prompt canary")
    ...
    if TOOL_CALL_CANARY not in follow_up or TOOL_RESULT_CANARY not in follow_up:
        raise AssertionError("Anika did not return the tool result to the model")
    if RESPONSE_CANARY not in result.stdout:
        raise AssertionError("Anika did not print the mock model response")
    ...
    package_paths, packages = _validate_packages(
        ...
        / "observability"
        / "schemas"
        / "anika.shared_metrics.v2.schema.json",
    )

    print("Anika -> NeMo Relay shared-metrics smoke test passed")
