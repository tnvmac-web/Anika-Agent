...
/// read side does not see EOF until the last of them closes it. `anika update`
...
/// stranded with it.
///
/// Same bound `Invoke-AnikaStep` grew in `scripts/desktop-update/windows.ps1`
...
///
/// `anika_home_override` propagates to the child as $ANIKA_HOME so the
...
pub async fn run_script(
    ...
    if let Some(cwd) = stable_script_cwd(script_path, anika_home_override) {
        cmd.current_dir(cwd);
    }

    if let Some(home) = anika_home_override {
        cmd.env("ANIKA_HOME", home);
    }
...
fn stable_script_cwd<'a>(script_path: &'a Path, anika_home_override: Option<&'a str>) -> Option<&'a Path> {
    if let Some(home) = anika_home_override {
...
"#;
    ...
    #[test]
    fn stable_script_cwd_prefers_existing_anika_home() {
    ...
    /// child. `anika update` is this shape -- the Electron build alone is
    /// megabytes.
    #[cfg(unix)]
    #[tokio::test]
