...
//!
//! Mirrors `anika_constants.get_anika_home()` from the Python CLI:
//!   Windows: %LOCALAPPDATA%\anika
//!   macOS:   ~/.anika
//!   Linux:   ~/.anika  (override via $ANIKA_HOME)
//!
//! NOTE (macOS): Python's get_anika_home(), scripts/install.sh, and the
//! Electron desktop's resolveAnikaHome() ALL use ~/.anika on macOS — there
...
/// Returns the canonical Anika home directory, respecting $ANIKA_HOME if set.
pub fn anika_home() -> PathBuf {
    if let Ok(override_path) = std::env::var("ANIKA_HOME") {
    ...
    {
        // %LOCALAPPDATA%\anika — matches scripts/install.ps1's $AnikaHome.
        if let Some(local_app_data) = dirs::data_local_dir() {
            return local_app_data.join("anika");
        }
    }

    // macOS + Linux + fallback: ~/.anika (matches Python get_anika_home(),
    // install.sh, and the Electron desktop's resolveAnikaHome()).
    if let Some(home) = dirs::home_dir() {
        return home.join(".anika");
    }
...
pub fn bootstrap_cache_dir() -> PathBuf {
    anika_home().join("bootstrap-cache")
}
...
/// ANIKA_HOME so it survives repo checkout deletion (unlike anything under
/// anika-agent/).
///
/// On Windows this is `%LOCALAPPDATA%\anika\anika-setup.exe`; on other
...
/// mid-update re-locks the venv shim and triggers `force_kill_other_anika`,
/// which then kills that legitimate backend in a respawn loop (#50238).
///
/// Lives directly under ANIKA_HOME (same rationale as `installer_dest`) so the
/// Electron desktop — which resolves ANIKA_HOME identically and pins it into
/// the updater's env — agrees on the exact path.
pub fn update_in_progress_marker() -> PathBuf {
    anika_home().join(".anika-update-in-progress")
}
...
pub fn copy_self_to_anika_home() -> std::io::Result<()> {
    ...
    tracing::info!(?src, ?dest, "copied installer to ANIKA_HOME");
    Ok(())
}
...
/// We don't always know ACTIVE_ANIKA_ROOT until install.ps1 reports it, so
/// this is a probe helper, not a definitive path.
pub fn likely_bootstrap_marker(install_root: &Path) -> PathBuf {
    install_root.join(".anika-bootstrap-complete")
}

/// Initializes tracing to bootstrap-installer.log under ANIKA_HOME/logs/.
...
pub fn init_logging() -> Option<WorkerGuard> {
    ...
    if let Err(err) = std::fs::create_dir_all(&dir) {
        ...
        eprintln!("[anika-setup] could not create log dir {dir:?}: {err}");
        return None;
    }
    ...
    let env_filter = tracing_subscriber::EnvFilter::try_from_env("ANIKA_BOOTSTRAP_LOG")
...
#[tauri::command]
pub fn get_anika_home() -> String {
    anika_home().to_string_lossy().into_owned()
}
