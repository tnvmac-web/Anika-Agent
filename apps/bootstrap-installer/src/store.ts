...
export const $anikaHome = atom<string | null>(null)
...
export async function initialize(): Promise<void> {
  ...
  if (fake) {
    unlisten = () => {}
    $logPath.set('~/.anika/logs/bootstrap-installer.log')
    $anikaHome.set('~/.anika')
  ...
  try {
    const [logPath, anikaHome, mode] = await Promise.all([
      invoke<string>('get_log_path'),
      invoke<string>('get_anika_home'),
      invoke<AppMode>('get_mode')
    ])

    $logPath.set(logPath)
    $anikaHome.set(anikaHome)
    $mode.set(mode)
  } catch (err) {
  ...
  unlisten = await listen<BootstrapEvent>('bootstrap', (event) => {
    ...
    switch (payload.type) {
      ...
      case 'complete':
        ...
        // Install: show the "launch Anika" success screen. Update: this is a
...
export async function startUpdate(): Promise<void> {
  ...
  // Update is driven by the desktop handing off (Anika-Setup.exe --update);
...
export async function launchAnikaDesktop(): Promise<void> {
  ...
  await invoke('launch_anika_desktop', { installRoot })
}
...
const FAKE_INSTALL_STAGES: FakeStage[] = [
  ...
  { name: 'repo', title: 'Anika repository' },
