 ...
 *     `X-Anika-Session-Token` header; WS uses `?token=`.
...
// anika_cli/dashboard_auth/cookies.py.
//
// Two cookies are in play (see that module):
//   - anika_session_at: the OAuth access token. Short-lived (~15 min); its
//     Max-Age tracks the access-token TTL, so the cookie jar drops it the
//     instant the AT expires.
//   - anika_session_rt: the OAuth refresh token. Long-lived (24h rotating,
//     reuse-detected — Portal NAS #293 / anika #37247). When the AT cookie
...
const AT_COOKIE_VARIANTS = ['__Host-anika_session_at', '__Secure-anika_session_at', 'anika_session_at']
const RT_COOKIE_VARIANTS = ['__Host-anika_session_rt', '__Secure-anika_session_rt', 'anika_session_rt']

// The __NEW_ORG__ portal (NAS) does NOT use Anika gateway session cookies — it is a
...
// Keep this aligned with anika_cli.profiles.validate_profile_name(). `default`
// is the built-in root alias; these names cannot be created as profiles.
const RESERVED_REMOTE_PROFILES = new Set(['anika', 'test', 'tmp', 'root', 'sudo'])
...
/** Which Anika profile the remote SSH dashboard should actually run as.
...
// Anika Cloud connection (cloud-auto-discovery Q3/Q6): it carries a
...
function normalizeSshConfig(entry) {
  ...
  const remoteAnikaPath = String(entry.remoteAnikaPath || '').trim()

  if (remoteAnikaPath) {
    out.remoteAnikaPath = remoteAnikaPath
  }

  // A Desktop profile can be a local routing label rather than the profile
  // name used by the remote Anika installation. Preserve an explicit mapping
  // when it is a valid Anika profile identifier; otherwise fall back to the
...
function localPrimaryRequestScope(opts: ProfileRouteOptions): boolean | null {
  ...
  // spawned them: `_spawn_anika_action` registers the (often dynamic, e.g.
 ...
 *     `--profile`, so its `ANIKA_HOME` scopes it.
 *
...
function resolveProfileBackendRoute(profile, opts: ProfileRouteOptions = {}): ProfileBackendRoute {
  ...
  if (scopedProfile === primaryProfile) {
    ...
    // the wire: the dashboard's process ANIKA_HOME can belong to a different
 ...
 * `anika:api` handler — both routes reach a backend whose namespace is the
...
function translateSelfProfileQuery(path, profile, backendProfile) {
  ...
  try {
    parsed = new URL(rawPath, 'http://anika.local')
  } catch {
    return path
  }
...
/**
 * Resolve the two decisions made by the `anika:api` IPC handler from the same
...
/**
 * True if any cookie in `cookies` is a anika session ACCESS-token cookie
...
/**
 * True if the cookie jar holds a live __NEW_ORG__ PORTAL (Privy) session — a non-empty
 ...
 * the Anika gateway session cookies, so cloud sign-in / discovery liveness
