...
import { buildDesktopBackendEnv, anikaManagedNodePathEntries, normalizeAnikaHomeRoot } from './backend-env'
import {
...
import { isAnikaOwnedVenvDaemon } from './venv-holder-select'
...
import {
  ...
  resolveVenvAnikaCommand
} from './windows-anika-path'
import {
...
const USER_DATA_OVERRIDE = process.env.ANIKA_DESKTOP_USER_DATA_DIR
...
const DEV_SERVER = process.env.ANIKA_DESKTOP_DEV_SERVER
const IS_PACKAGED = app.isPackaged || Boolean(process.env.ANIKA_DESKTOP_IS_PACKAGED)
...
// switches only apply pre-launch. Override with ANIKA_DESKTOP_DISABLE_GPU
...
if (REMOTE_DISPLAY_REASON) {
  ...
  console.log(
    `[anika] remote display detected (${REMOTE_DISPLAY_REASON}); disabling GPU hardware acceleration to prevent flicker`
  )
}
...
if (DEV_CDP.port) {
  ...
  console.log(
    `[anika] renderer debugging on http://127.0.0.1:${DEV_CDP.port} — anything that can reach it ` +
      'can run code in the renderer. ANIKA_DESKTOP_CDP_PORT=off to disable.'
  )
} else {
  const why = describeDevCdpDecision(DEV_CDP)

  if (why) {
    console.warn(`[anika] ${why}`)
  }
}
...
if (IS_WSL && !REMOTE_DISPLAY_REASON && fs.existsSync('/dev/dxg')) {
  ...
  console.log('[anika] WSL GPU passthrough (/dev/dxg) detected; enabling GPU acceleration')
}
...
// it). The value arrives via ANIKA_DESKTOP_PASSWORD_STORE, bridged by the
// `anika desktop` launcher from detection or `desktop.password_store` in
...
if (PASSWORD_STORE.warning) {
  console.warn(`[anika] ${PASSWORD_STORE.warning}`)
}

if (PASSWORD_STORE.store) {
  app.commandLine.appendSwitch('password-store', PASSWORD_STORE.store)
  console.log(`[anika] using password-store backend: ${PASSWORD_STORE.store}`)
}
...
// never go through `anika desktop`; it is version-scoped so an app update
...
if (IS_WINDOWS) {
  ...
  // expose Anika sessions/config to every packaged app on the machine.
  if (shouldAttemptAclRepair(priorMarker)) {
    const exeDir = path.dirname(process.execPath)
    const acl = grantAllApplicationPackagesAcl(exeDir, { execFileSync })

    if (acl.ok) {
      console.log(`[anika] granted ALL APPLICATION PACKAGES RX on ${exeDir} (#38216)`)
    } else if (acl.error && acl.error !== 'missing-target-or-exec') {
      console.warn(`[anika] AppContainer ACL grant failed on ${exeDir}: ${acl.error}`)
    }
  }
  ...
  if (sandboxDecision.enable && sandboxDecision.reason !== 'already-enabled') {
    ...
    console.log(
      `[anika] Windows sandbox fallback enabled (${sandboxDecision.reason}); launching with --no-sandbox (#38216)`
    )
  }
  ...
  app.on('child-process-gone', (_event, details) => {
    ...
    console.warn(
      `[anika] Windows GPU sandbox crashed (exit=${details?.exitCode}); relaunching once with --no-sandbox (#38216)`
    )
    ...
    try {
      ...
      console.error(`[anika] --no-sandbox relaunch failed: ${error?.message || error}`)
    }
  })
}

ipcMain.handle('anika:get-remote-display-reason', () => REMOTE_DISPLAY_REASON)
...
// always-on timers. A completely idle, minimized Anika burned ~20% CPU
    ...
    try {
      ...
      if (parsed && typeof parsed === 'object' && typeof parsed.commit === 'string' && parsed.commit.length >= 7) {
        if (parsed.schemaVersion !== INSTALL_STAMP_SCHEMA_VERSION) {
          console.warn(
            `[anika] install-stamp.json schemaVersion ${parsed.schemaVersion} != expected ${INSTALL_STAMP_SCHEMA_VERSION}; ignoring`
          )
      ...
      console.warn(`[anika] install-stamp.json found at ${p} , but parsing failed with ${e}`)
...
if (INSTALL_STAMP) {
  console.log(
    `[anika] install stamp: ${INSTALL_STAMP.commit.slice(0, 12)}${INSTALL_STAMP.branch ? ` (${INSTALL_STAMP.branch})` : ''}${INSTALL_STAMP.dirty ? ' [DIRTY]' : ''} from ${INSTALL_STAMP.source || 'unknown'}`
  )
} else if (IS_PACKAGED) {
  ...
  console.error(
    '[anika] WARNING: no install-stamp.json found in packaged build. First-launch bootstrap will not have a pinned ref to install.'
  )
}

// ANIKA_HOME — the user-facing root for everything Anika-related. Mirrors
// scripts/install.ps1's $AnikaHome and scripts/install.sh's $ANIKA_HOME.
//
// Defaults:
//   Windows: %LOCALAPPDATA%\anika (matches install.ps1)
//   macOS / Linux: ~/.anika (matches install.sh)
//
// Special case for Windows: if the user has a legacy ~/.anika directory
// (e.g., from a prior pip install or a manual setup) AND no
// %LOCALAPPDATA%\anika yet, prefer the legacy path so we don't orphan their
// existing config / sessions / .env. New installs go to %LOCALAPPDATA%.
//
// ANIKA_DESKTOP_USER_DATA_DIR (used by test:desktop:fresh) puts the sandbox
// ANIKA_HOME beneath the throwaway userData dir so a fresh-install run never
// touches the user's real ~/.anika / %LOCALAPPDATA%\anika.
function resolveAnikaHome() {
  if (process.env.ANIKA_HOME) {
    return normalizeAnikaHomeRoot(process.env.ANIKA_HOME)
  }

  if (USER_DATA_OVERRIDE) {
    return path.join(path.resolve(USER_DATA_OVERRIDE), 'anika-home')
  }

  if (IS_WINDOWS) {
    // A GUI app launched from Explorer inherits the environment block captured
    // at login, so a ANIKA_HOME set via `setx` AFTER login is invisible in
    // process.env even though the CLI (a fresh shell) sees it. Without this the
    // backend silently falls back to %LOCALAPPDATA%\anika and reports "No
    // inference provider configured" despite a valid configured home (#45471).
    // Consult the live User-scoped registry value before the default below.
    const fromRegistry = readWindowsUserEnvVar('ANIKA_HOME')

    if (fromRegistry) {
      return normalizeAnikaHomeRoot(fromRegistry)
    }
  }

  if (IS_WINDOWS && process.env.LOCALAPPDATA) {
    const localappdata = path.join(process.env.LOCALAPPDATA, 'anika')
    const legacy = path.join(app.getPath('home'), '.anika')

    // Migrate transparently to LOCALAPPDATA, but honour an existing legacy
    // ~/.anika setup (no LOCALAPPDATA install yet) so users don't lose state.
  ...
  return path.join(app.getPath('home'), '.anika')
}

const ANIKA_HOME = resolveAnikaHome()

function pathWithAnikaManagedNode(...entries) {
  const managed = anikaManagedNodePathEntries(ANIKA_HOME).filter(directoryExists)

  return [...managed, ...entries, process.env.PATH].filter(Boolean).join(path.delimiter)
}

// ACTIVE_ANIKA_ROOT — the canonical mutable Anika install. Same path
// install.ps1 / install.sh use, so a desktop-only user and a CLI-only user end
// up with identical layouts and can share one install.
const ACTIVE_ANIKA_ROOT = path.join(ANIKA_HOME, 'anika-agent')
// VENV_ROOT — venv lives inside the repo, exactly like install.ps1 does it.
const VENV_ROOT = path.join(ACTIVE_ANIKA_ROOT, 'venv')
...
// boots, going straight to `resolveAnikaBackend()`. Missing or stale marker
...
//
// We deliberately put the marker INSIDE ACTIVE_ANIKA_ROOT (not alongside)
...
const BOOTSTRAP_COMPLETE_MARKER = path.join(ACTIVE_ANIKA_ROOT, '.anika-bootstrap-complete')
...
// active-profile.json records which Anika profile the desktop launches its
// local backend as. When set, startAnika() passes `anika --profile <name>
// dashboard …`, which deterministically pins ANIKA_HOME (see
// _apply_profile_override in anika_cli/main.py) and bypasses the sticky
// ~/.anika/active_profile file. Unset (null) preserves the legacy behavior:
// no --profile flag, so the backend honors active_profile / default.
const DESKTOP_PROFILE_CONFIG_PATH = path.join(app.getPath('userData'), 'active-profile.json')
// Mirrors anika_cli.profiles._PROFILE_ID_RE so we never hand the backend a
// value its profile resolver would reject and exit on.
const PROFILE_NAME_RE = /^[a-z0-9][a-z0-9_-]{0,63}$/
// Branch we track for self-update. The GUI work has merged to main, so this
// tracks main. User can also override at runtime via
// anikaDesktop.updates.setBranch().
const DEFAULT_UPDATE_BRANCH = 'main'
// desktop.log lives under ANIKA_HOME/logs/ so it sits next to agent.log,
// errors.log, gateway.log produced by anika_logging.setup_logging — one log
// directory per user, regardless of which UI surface produced the line.
const DESKTOP_LOG_PATH = path.join(ANIKA_HOME, 'logs', 'desktop.log')
...
//
// Mirror the Python logs (anika_logging.py RotatingFileHandler, maxBytes x
...
const BOOT_FAKE_MODE = process.env.ANIKA_DESKTOP_BOOT_FAKE === '1'
const BOOT_FAKE_ERROR = process.env.ANIKA_DESKTOP_BOOT_FAKE_ERROR || ''
...
const SKIP_QUIT_CONFIRM = process.env.ANIKA_DESKTOP_SKIP_QUIT_CONFIRM === '1'

const BOOT_FAKE_STEP_MS = (() => {
  const raw = Number.parseInt(String(process.env.ANIKA_DESKTOP_BOOT_FAKE_STEP_MS || ''), 10)
...
const APP_NAME = process.env.ANIKA_DESKTOP_APP_NAME || 'Anika'
...
// 'anika:native-theme' ('dark' | 'light' | 'system'); we pin
...
// electron-builder derives from the build `appId` (com.__NEW_ORG__.anika) —
// keep this string in sync with package.json `build.appId`. macOS/Linux don't
// need this, so gate it on Windows. (Fixes: desktop approval/turn notifications
// never firing on Windows.)
if (IS_WINDOWS) {
  app.setAppUserModelId('com.__NEW_ORG__.anika')
}

// Seed the native About panel with the live Anika version. This is refreshed
// on every open via the explicit "About" menu handler (refreshAboutPanel), so
// an in-place `anika update` mid-session is reflected without an app restart;
// the seed here just covers the first open and any non-menu invocation path.
app.setAboutPanelOptions({
  applicationName: APP_NAME,
  applicationVersion: resolveAnikaVersion(),
  copyright: 'Copyright © 2026 __NEW_ORG__'
})
...
// startAnika(); this pool only holds EXTRA profile
...
// The legacy ANIKA_DESKTOP_POOL_* env vars remain the initial-value fallback
...
function readPersistedPoolLimits() {
  try {
    ...
    const fromEnv = clampPoolLimits({
      maxBackends: Number(process.env.ANIKA_DESKTOP_POOL_MAX) || undefined,
      idleMs: Number(process.env.ANIKA_DESKTOP_POOL_IDLE_MS) || undefined
    })
...
function logPoolSpawnFailure(label: string, error: unknown): void {
  if (isBackgroundSlotWaitTimeout(error)) {
    ...
    rememberLog(
      `Anika backend for profile ${label} failed to start: ${error instanceof Error ? error.message : String(error)}`
    )
  }
}
...
//   * WSL2 IPC stall  = the renderer's `anika:backend:touch` roundtrips
...
const POOL_KEEPALIVE_FRESH_MS = Math.max(
  120_000,
  Number(process.env.ANIKA_DESKTOP_POOL_KEEPALIVE_FRESH_MS) || 4 * 60_000
)
...
// onto the error so subsequent startAnika() calls (e.g. the renderer's
// ensureGatewayOpen retrying after the WS won't open) return the same error
// instead of re-running install.ps1 in a hot loop. Cleared explicitly by
// the renderer's "Reload and retry" path or by quitting the app.
let bootstrapFailure = null
// Latched non-bootstrap backend spawn failure — stops getConnection() from
// respawning anika serve backend children in a tight loop while boot is broken.
...
let bootProgressState = {
  ...
  message: 'Waiting to start Anika backend',
  phase: 'idle',
  progress: 0,
  retryable: false,
  running: false,
...
function rememberLog(chunk) {
  ...
  if (anikaLog.length > 300) {
    anikaLog.splice(0, anikaLog.length - 300)
  }
...
function ensureWslWindowsFonts() {
  ...
  try {
    ...
    const confPath = path.join(confDir, '99-anika-wsl-windows-fonts.conf')
    let existing = ''
...
function broadcastBootProgress() {
  ...
  webContents.send('anika:boot-progress', bootProgressState)
}
...
//
// The snapshot is queryable via the anika:bootstrap:get IPC handler so a
...
function broadcastBootstrapEvent(ev) {
  ...
  webContents.send('anika:bootstrap:event', ev)
}
...
function promptFirstRunSetupChoice(backend) {
  broadcastBootstrapEvent({
    ...
    activeRoot: backend.activeRoot || ACTIVE_ANIKA_ROOT
  })
}
...
// The Tauri updater writes ANIKA_HOME/.anika-update-in-progress for the whole
...
// (`force_kill_other_anika`, taskkill /IM anika.exe) kills it, the launch
...
function updateGateDeps() {
  return {
    hasLiveMarker: () => Boolean(readLiveUpdateMarker(ANIKA_HOME)),
...
const BUNDLE_SWAP_RELAUNCH_FLAG = '--anika-bundle-swap-relaunched'
...
// The detached updater swaps the packaged bundle on disk AFTER `anika update`
...
async function waitForUpdateToFinish() {
  ...
  const outcome = await waitForUpdateClearance(updateGateDeps(), {
    onWaitTick: async reason => {
      ...
      await advanceBootProgress(
        'backend.update-wait',
        'An update is finishing — Anika will start automatically when it completes…',
        12
      )
    },
  ...
  // "nothing happened").
  try {
    const result = readAndConsumeHandoffResult(ANIKA_HOME)
    ...
    if (result && result.ok && result.manual) {
      ...
      dialog.showErrorBox(
        'Anika update did not finish',
        `${result.message}\n\nDetails: ${path.join(ANIKA_HOME, 'logs', 'desktop-update-handoff.log')}`
      )
    }
  } catch (err) {
  ...
  if (outcome === 'timeout') {
    ...
    await advanceBootProgress('backend.update-restart', 'Restarting Anika to load the updated app…', 14)
...
function findOnPath(command) {
  ...
  // shell-script shim named `anika` — must not shadow `anika.cmd`/`anika.exe`.
...
function unwrapWindowsVenvAnikaCommand(command, backendArgs) {
  return resolveVenvAnikaCommand(command, backendArgs, {
    isWindows: IS_WINDOWS,
    isCommandScript,
    fileExists,
    directoryExists,
...
// spawns `anika serve`; runtimes older than serve only have `dashboard`. We
...
// (covers a bare `anika` resolved from PATH with no known source root). Result
...
function backendSupportsServe(backend) {
  ...
  if (backend.root) {
    try {
      const src = fs.readFileSync(path.join(backend.root, 'anika_cli', 'subcommands', 'dashboard.py'), 'utf8')
  ...
  if (supported === null) {
    try {
      ...
      // `anika --version` (~10.5s measured cold), and a false negative here
      ...
      execProbeSync(backend.command, [...prefix, 'serve', '--help'], {
        ...
        env: { ...process.env, ANIKA_HOME, ...(backend.env || {}) },
        timeout: PROBE_TIMEOUT_MS,
        stdio: 'ignore',
        // `.cmd`/`.bat` shim backends carry shell: true in their descriptor
        // (see resolveAnikaBackend step 4); execFileSync of a .cmd without
...
function isAnikaSourceRoot(root) {
  return directoryExists(root) && fileExists(path.join(root, 'anika_cli', 'main.py'))
}

function findPythonForRoot(root) {
  const override = process.env.ANIKA_DESKTOP_PYTHON
...
function findSystemPython() {
  ...
  // CPython but several Anika deps (notably pywinpty's Rust-built
      ...
      try {
        const out = execFileSync(
          ...
          hiddenWindowsChildOptions({
            ...
            // Bare interpreter startup — much lighter than the anika-import
...
// findGitBash — locate bash.exe on Windows. Resolves ANIKA_GIT_BASH_PATH
...
// normal ANIKA_DASHBOARD_READY stdout line and no ready-file side channel is
// needed.
...
// install only has PortableGit under %LOCALAPPDATA%\anika\git (never on
...
function resolveGitBinary() {
  ...
  if (localAppData) {
    candidates.push(path.join(localAppData, 'anika', 'git', 'cmd', 'git.exe'))
    candidates.push(path.join(localAppData, 'anika', 'git', 'bin', 'git.exe'))
  }
...
function recentAnikaLog() {
  return anikaLog.slice(-20).join('\n')
}

// ─── Self-update (git-pull against the running backend's anika root) ──────
...
// Dev → SOURCE_REPO_ROOT. Packaged/CLI install → ACTIVE_ANIKA_ROOT.
// ANIKA_DESKTOP_ANIKA_ROOT always wins so devs can pin a worktree.
function resolveUpdateRoot() {
  const candidates = [
    process.env.ANIKA_DESKTOP_ANIKA_ROOT && path.resolve(process.env.ANIKA_DESKTOP_ANIKA_ROOT),
    !IS_PACKAGED && isAnikaSourceRoot(SOURCE_REPO_ROOT) ? SOURCE_REPO_ROOT : null,
    isAnikaSourceRoot(ACTIVE_ANIKA_ROOT) ? ACTIVE_ANIKA_ROOT : null
  ].filter(Boolean)

  return candidates.find(c => directoryExists(path.join(c, '.git'))) || candidates[0] || ACTIVE_ANIKA_ROOT
}
...
function emitUpdateProgress(payload) {
  ...
  for (const window of BrowserWindow.getAllWindows()) {
    window.webContents.send('anika:updates:progress', merged)
  }
}
      ...
      const req = https.get(
        url,
        {
          headers: {
            ...
            'User-Agent': 'anika-desktop-update-check'
          },
          timeout: 10_000
        },
        res => {
...
// Windows that binary owns ALL repo mutation — running `anika update` +
...
function resolveUpdaterBinary() {
  return resolveStagedUpdaterBinary(ANIKA_HOME, { fileExists, isWindows: IS_WINDOWS })
}
...
// Path to the venv shim whose lock decides whether `anika update` can write
// fresh entry points. On Windows this is the file the running backend
// `anika.exe` holds open; on POSIX it's never mandatory-locked.
function venvAnikaShimPath(updateRoot) {
  return IS_WINDOWS
    ? path.join(updateRoot, 'venv', 'Scripts', 'anika.exe')
    : path.join(updateRoot, 'venv', 'bin', 'anika')
}
...
// Kill only Anika-OWNED venv daemons (the memory plugin's hindsight daemon:
...
// venv files mapped. External holders (a user terminal running `anika`,
...
function killAnikaOwnedVenvDaemons(updateRoot) {
  if (!IS_WINDOWS) {
    return
  }
  ...
  try {
    ...
    holders = (Array.isArray(parsed) ? parsed : [parsed]).filter(p =>
      isAnikaOwnedVenvDaemon(p?.ExecutablePath, p?.CommandLine, scriptsDir)
    )
  } catch {
  ...
  for (const holder of holders) {
    ...
    if (Number.isInteger(pid) && pid > 0) {
      rememberLog(`[updates] stopping Anika-owned venv daemon (hindsight) PID ${pid} before hand-off`)
...
// only signals the direct child, so on Windows a backend `anika.exe` that
// spawned its own grandchildren (a `anika` REPL, a pty terminal session, the
...
async function claimBackendChild(child, command, profile, nonce, outputTail: BackendOutputTail | null = null) {
  ...
  if (decision.action === 'fail') {
    ...
    throw new Error(
      `Anika backend (PID ${child.pid}) died before its identity could be recorded: ${decision.reason}${outputTail?.describe() ?? ''}`
    )
  }
  ...
  if (decision.action === 'degrade') {
    ...
    rememberLog(
      `WARNING: process start marker probe failed for live Anika backend PID ${child.pid}; ` +
  ...
  try {
    ...
    child.anikaBackendIdentity = identity
    ...
    await waitForBackendExit(child)
    throw new Error(
      `Could not persist ownership for the Anika backend: ${error.message}${outputTail?.describe() ?? ''}`
    )
  }
}

function releaseBackendChild(child) {
  const identity = child?.anikaBackendIdentity
...
// `anikaProcess.kill('SIGTERM')` + `app.quit()` fire-and-forget: SIGTERM on
// Windows doesn't reap the backend's grandchildren, and quit didn't wait for
// teardown, so the updater raced a still-locked `anika.exe`, the quarantine
...
// problem: the desktop's backend (and the grandchildren IT spawned — a anika
// REPL, a pty terminal, the gateway) keep `anika.exe` and other files in the
...
async function releaseBackendLock(updateRoot, tag) {
  ...
  const anikaProcess = backendConnectionState.getProcess()
  ...
  // unlock — the shim probe only covers venv\Scripts\anika.exe, but the
  // backend is `python.exe -m anika_cli.main serve`, which need not hold
  // the shim at all (#74805 first-attempt race).
  const initialPids = []

  if (anikaProcess && Number.isInteger(anikaProcess.pid)) {
    initialPids.push(anikaProcess.pid)
  }
  ...
  stopBackendTreesForUpdate(anikaProcess, {
  ...
  // `anika gateway stop --all`: the CLI discovers every profile's gateway
  ...
  stopGatewayBeforeUpdate(venvAnikaShimPath(updateRoot), ANIKA_HOME)

  // Reap Anika-OWNED venv daemons the tree-kill above cannot reach: the
  ...
  killAnikaOwnedVenvDaemons(updateRoot)

  const shim = venvAnikaShimPath(updateRoot)
  ...
  const gate = await waitForBackendRelease(
    initialPids,
    {
      ...
      collectStragglerPids: () => {
        ...
        const currentAnikaProcess = backendConnectionState.getProcess()

        if (currentAnikaProcess && Number.isInteger(currentAnikaProcess.pid)) {
          stragglers.push(currentAnikaProcess.pid)
        }
...
// `anika update`). Instead we spawn the staged Anika-Setup binary with
// --update and quit, so it can run `anika update` (which refuses while we
...
async function applyUpdates(opts: { stopSafeBlockers?: boolean } = {}) {
  ...
  try {
    ...
    if (!updater && !IS_WINDOWS) {
      ...
      // Windows (quit → detached orchestrator → `anika update` → relaunch),
      ...
      // in: the ANIKA_DESKTOP_CHILD_PID reaper-exclusion dance (#37532),
      ...
      // get the manual `anika update` card once; their next update pulls it.
      return await applyUpdatesPosixHandoff(opts)
    }

    if (!updater) {
      // No staged updater binary — this is a CLI-installed user (they ran
      // `anika desktop`, never the Tauri installer that self-copies
      // anika-setup.exe into ANIKA_HOME). On Windows the repo hand-off
      ...
      if (!resolveUpdateScriptHandoff(updateRoot)) {
        // They DO have a working `anika` on PATH / in the venv, so the
        ...
        try {
          ...
          if (head.code === 0 && current && current !== 'HEAD') {
            ...
            if (branch !== 'main') {
              command = `anika update --branch ${branch}`
            }
          }
        } catch {
          // Best-effort: fall back to bare `anika update` if branch detection fails.
        }

        rememberLog(`[updates] no staged updater; surfacing manual \`${command}\` for CLI install at ${updateRoot}`)
        emitUpdateProgress({ stage: 'manual', message: command, percent: null })

        return { ok: true, manual: true, command, anikaRoot: updateRoot }
      }

      rememberLog('[updates] no staged updater; using repo hand-off script for CLI install')
    }

    const handoffConflict = updateHandoffConflict(ANIKA_HOME)
    ...
    emitUpdateProgress({
      stage: 'restart',
      message:
        'Updating Anika — this window will close and the updater will open. Don’t reopen Anika yourself; it restarts automatically when the update finishes.',
      percent: 100
    })
    ...
    preflightStateDb(ANIKA_HOME, rememberLog)

    // Stop our own backend(s) and wait for the venv shim to unlock BEFORE we
    // spawn the updater. Without this the updater races a still-locked
    // anika.exe (held by the backend child / its grandchildren) and the update
    ...
    if (!lock.unlocked) {
      ...
      // terminal running anika, an unkillable child). Handing off anyway
      ...
      const message =
        'Update aborted: another process is holding the Anika install open ' +
        '(a second Anika window or a terminal running anika?). Close it and retry.'
      ...
      if (IS_WINDOWS) {
        ...
        startGatewaysAfterUpdateAbort(venvAnikaShimPath(updateRoot))
      }
    ...
    // Anika processes running from this venv.  The updater normally refuses
    ...
    // `anika update` will run (the venv shim is locked while we live).
    //
    ...
    // checkout, so each `anika update` refreshes the code that drives the
    ...
    if (scriptHandoff) {
      ...
      child = spawnUpdaterProcess(wrapped.command, wrapped.args, {
        ...
        env: {
          ...process.env,
          ANIKA_HOME,
          ANIKA_UPDATE_STARTED_AT: String(updateStartedAt),
          PATH: pathWithAnikaManagedNode(venvBin)
        },
        detached: true,
        stdio: 'ignore'
      })
      ...
      // The `anika update` child adopts the SCRIPT's claim via
      // update_lock.py's process-ancestry rule; no mtime heuristics needed.
      if (Number.isInteger(child.pid)) {
        writeUpdateMarker(ANIKA_HOME, child.pid, { startedAt: updateStartedAt })
      }
      ...
      // abort with "Another Anika update is already running (PID <itself>)" —
      ...
      if (Number.isInteger(child.pid) && stagedUpdaterSupportsPrewrittenMarker(updater)) {
        writeUpdateMarker(ANIKA_HOME, child.pid)
    ...
    if (!handoffOutcome.ok) {
      const message = `Update failed to start: ${handoffOutcome.message}. Anika will keep running — try again, or run \`anika update\` from a terminal.`
...
async function handOffWindowsBootstrapRecovery(reason) {
  ...
  const venvBin = path.join(updateRoot, 'venv', IS_WINDOWS ? 'Scripts' : 'bin')
  const venvAnika = path.join(venvBin, IS_WINDOWS ? 'anika.exe' : 'anika')
  const venvPython = path.join(venvBin, IS_WINDOWS ? 'python.exe' : 'python')

  // The updater invokes the venv's Anika launcher, which in turn requires the
  ...
  // marker-only install through --update dead-ends at "Could not find the anika
  ...
  const updaterArgs = chooseUpdaterArgs(
    {
      hasBootstrapMarker: fileExists(path.join(updateRoot, '.anika-bootstrap-complete')),
      hasVenvAnika: fileExists(venvAnika),
...
// Python-level pre-update snapshot inside `anika update`.
function preflightStateDb(anikaHome, rememberLog) {
  const stateDbPath = path.join(anikaHome, 'state.db')
  ...
  try {
    ...
    if (stat.size > 100) {
      ...
      const emergencyPath = path.join(anikaHome, `state.db.pre-update-emergency-${ts}.bak`)
      ...
      try {
        ...
        try {
          const homeDir = fs.readdirSync(anikaHome)
          ...
          for (const old of backups.slice(2)) {
            try {
              fs.unlinkSync(path.join(anikaHome, old))
            } catch {
              void 0
            }
          }
...
// out, runs `anika update`, swaps/relaunches the app bundle, and writes
// .anika-update-result.json for the relaunched Desktop to surface. It shows
...
async function applyUpdatesPosixHandoff(opts: any) {
  ...
  if (!handoff) {
    emitUpdateProgress({ stage: 'manual', message: 'anika update', percent: null })

    return { ok: true, manual: true, command: 'anika update', anikaRoot: updateRoot }
  }
  ...
  const child = spawnUpdaterProcess(handoff.command, args, {
    ...
    env: {
      ...
      ANIKA_HOME,
      ANIKA_UPDATE_STARTED_AT: String(updateStartedAt),
      PATH: pathWithAnikaManagedNode(path.join(updateRoot, 'venv', 'bin'))
    },
    detached: true,
    stdio: 'ignore'
  })
  ...
  emitUpdateProgress({
    stage: 'restart',
    message:
      'Updating Anika — this window will close. Don’t reopen Anika yourself; it restarts automatically when the update finishes.',
    percent: 100
  })
...
// Marker-independent: is the canonical install at ACTIVE_ANIKA_ROOT actually
...
function isActiveRuntimeUsable() {
  ...
  return (
    isAnikaSourceRoot(ACTIVE_ANIKA_ROOT) &&
    fileExists(venvPython) &&
    canImportAnikaCli(venvPython, {
      env: {
        PYTHONPATH: [ACTIVE_ANIKA_ROOT, process.env.PYTHONPATH].filter(Boolean).join(path.delimiter)
      }
    })
  )
}
...
function activeRuntimeState() {
  ...
  // pinned commit -- users update via the in-app update path or `anika
...
function resolveWebDist() {
  const override = process.env.ANIKA_DESKTOP_WEB_DIST
  ...
  if (IS_PACKAGED && /app\.asar(?=$|[\\/])/.test(fallback) && !directoryExists(fallback)) {
    rememberLog(
      `[web-dist] dashboard frontend dir resolved to an asar-internal path that ` +
        ...
        `Ensure dist/** is unpacked (asarUnpack) or set ANIKA_DESKTOP_WEB_DIST.`
    )
  }
...
function resolveRendererIndex() {
  ...
  if (present.length > 0) {
    ...
    rememberLog(
      `[renderer] every renderer bundle is incomplete (${present.join(', ')}). ` +
        ...
        `Repair with: anika desktop --force-build`
    )
  ...
  rememberLog(
    `[renderer] index.html not found — the desktop app was packaged without a ` +
      ...
      `Rebuild with: anika desktop --force-build`
  )
...
function resolveAnikaCwd() {
  // In a packaged build, `process.cwd()` resolves to the install root (e.g.
  // `…/win-unpacked` on Windows or `/Applications/Anika.app/Contents/...`
...
function sanitizeWorkspaceCwd(cwd) {
  ...
  if (!trimmed || isPackagedInstallPath(trimmed)) {
    return { cwd: resolveAnikaCwd(), sanitized: Boolean(trimmed) }
  }
...
function createPythonBackend(root, label, backendArgs, options: any = {}) {
  ...
  return {
    ...
    label,
    command,
    args: ['-m', 'anika_cli.main', ...backendArgs],
...
// createActiveBackend — build a backend pointing at ACTIVE_ANIKA_ROOT, the
...
function createActiveBackend(backendArgs) {
  ...
  return {
    kind: 'python',
    label: `Anika at ${ACTIVE_ANIKA_ROOT}`,
    command,
    args: ['-m', 'anika_cli.main', ...backendArgs],
    env: buildDesktopBackendEnv({
      anikaHome: ANIKA_HOME,
      pythonPathEntries: [ACTIVE_ANIKA_ROOT, ...getVenvSitePackagesEntries(VENV_ROOT)],
      venvRoot: VENV_ROOT
    }),
    root: ACTIVE_ANIKA_ROOT,
    bootstrap: true,
    shell: false
  }
}

function resolveAnikaBackend(backendArgs) {
  // 1. Explicit override -- ANIKA_DESKTOP_ANIKA_ROOT points at a developer
  //    checkout. Honour it as-is (no bootstrap; the user is driving).
  const overrideRoot = process.env.ANIKA_DESKTOP_ANIKA_ROOT && path.resolve(process.env.ANIKA_DESKTOP_ANIKA_ROOT)

  if (overrideRoot && isAnikaSourceRoot(overrideRoot)) {
    const backend = createPythonBackend(overrideRoot, `Anika source at ${overrideRoot}`, backendArgs)
  ...
  //    installed `anika` on PATH so local Python edits are actually exercised.
  //    (In dev with no checkout, SOURCE_REPO_ROOT won't pass isAnikaSourceRoot.)
  if (!IS_PACKAGED && isAnikaSourceRoot(SOURCE_REPO_ROOT)) {
    const backend = createPythonBackend(SOURCE_REPO_ROOT, `Anika source at ${SOURCE_REPO_ROOT}`, backendArgs)
  ...
  // 3. ACTIVE_ANIKA_ROOT — the canonical install at
  //    %LOCALAPPDATA%\\anika\\anika-agent (Windows) or ~/.anika/anika-agent.
  ...
  if (activeRuntime.shouldUseActiveRuntime && !bootstrapRepairRequested) {
    if (!activeRuntime.hasValidMarker) {
      rememberLog(
        `[bootstrap] Active Anika runtime at ${ACTIVE_ANIKA_ROOT} is usable but the bootstrap marker is missing or stale; skipping first-run bootstrap.`
      )
    }
  ...
  // 4. Existing `anika` on PATH -- installed via install.ps1 / install.sh from
  //    a previous tool-only setup, or pip-installed system-wide. Use it but
  //    do NOT write a bootstrap marker; the user did this themselves and we
  //    don't want to take ownership of an install we didn't perform.
  //    ANIKA_DESKTOP_IGNORE_EXISTING=1 forces the bootstrap path for testing.
  if (process.env.ANIKA_DESKTOP_IGNORE_EXISTING !== '1') {
    let anikaCommand = null
    const anikaOverride = process.env.ANIKA_DESKTOP_ANIKA

    if (anikaOverride) {
      const resolvedOverride = findOnPath(anikaOverride)

      if (resolvedOverride) {
        anikaCommand = resolvedOverride
      } else if (!isWindowsBinaryPathInWsl(anikaOverride, { isWsl: IS_WSL })) {
        anikaCommand = anikaOverride
      } else {
        rememberLog(`Ignoring Windows Anika override under WSL: ${anikaOverride}`)
      }
    } else {
      anikaCommand = findOnPath('anika')
    }

    if (anikaCommand) {
      if (looksLikeDesktopAppBinary(anikaCommand)) {
        rememberLog(`Ignoring desktop app executable on PATH while resolving Anika CLI: ${anikaCommand}`)
        anikaCommand = null
      }
    }

    if (anikaCommand) {
      const unwrapped = unwrapWindowsVenvAnikaCommand(anikaCommand, backendArgs)

      if (unwrapped) {
        return unwrapped
      }

      // Smoke-test the candidate before trusting it. A `anika` shim
      ...
      const shellForProbe = isCommandScript(anikaCommand)

      // ANIKA_DESKTOP_ANIKA is an explicit deployment override (used by
      ...
      if (shouldTrustAnikaOverride(anikaOverride) || verifyAnikaCli(anikaCommand, { shell: shellForProbe })) {
        ...
        // unwrapWindowsVenvAnikaCommand here: the second call repeats the
        // same un-memoized import probe, costing up to another full probe
        // timeout on the boot path for an answer we already have.
        return {
          label: `existing Anika CLI at ${anikaCommand}`,
          command: anikaCommand,
          args: backendArgs,
          bootstrap: false,
          env: {},
          kind: 'command',
      ...
      rememberLog(
        `Ignoring existing Anika CLI at ${anikaCommand}: --version probe failed; falling through to bootstrap.`
      )
    }
  }

  // 5. Last-ditch: pip-installed anika_cli module via system Python.
  ...
  if (python) {
    ...
    // having anika_cli installed -- common on dev boxes that have
    ...
    // a uv-managed 3.11 into %LOCALAPPDATA%\anika\anika-agent\venv.
    if (canImportAnikaCli(python)) {
      return {
        kind: 'python',
        label: `installed anika_cli module via ${python}`,
        command: python,
        args: ['-m', 'anika_cli.main', ...backendArgs],
        bootstrap: false,
        env: {},
    ...
    rememberLog(`Ignoring system Python ${python}: anika_cli is not importable; falling through to bootstrap.`)
  }
  ...
  //    resolveAnikaBackend was the old "no payload" path and forced the
  //    user into a dead end. With the bootstrap protocol, "no install yet"
  //    is a recoverable state the GUI can drive through.
  return {
    kind: 'bootstrap-needed',
    label: 'Anika Agent not installed yet; bootstrap required',
    command: null,
    args: backendArgs,
    bootstrap: true,
    env: {},
...
async function ensureRuntime(backend) {
  ...
  // backend.kind === 'bootstrap-needed' means resolveAnikaBackend couldn't
  ...
  if (backend.kind === 'bootstrap-needed') {
    rememberLog('[bootstrap] no Anika install found; starting first-launch bootstrap')

    if (await handOffWindowsBootstrapRecovery('bootstrap-needed')) {
      const handoffError: Error & { isBootstrapFailure?: boolean; bootstrapHandedOff?: boolean } = new Error(
        'Anika recovery was handed off to Anika Setup. The desktop will restart when recovery completes.'
      )
    ...
    const bootstrapResult = await runBootstrap({
      ...
      anikaHome: ANIKA_HOME,
      logRoot: path.join(ANIKA_HOME, 'logs'),
    ...
    if (bootstrapResult.cancelled) {
      const cancelledError = new Error('Anika install was cancelled.') as any
    ...
    if (!bootstrapResult.ok) {
      const bootstrapError = new Error(
        `Anika bootstrap failed${bootstrapResult.failedStage ? ` at stage '${bootstrapResult.failedStage}'` : ''}: ` +
          `${bootstrapResult.error || 'unknown error'}. ` +
          `Check ${path.join(ANIKA_HOME, 'logs', 'desktop.log')} for the full transcript.`
      ) as any

      bootstrapError.isBootstrapFailure = true
      bootstrapError.failedStage = bootstrapResult.failedStage || null
      // Latch the failure so subsequent startAnika() calls return this
      // same error without re-running install.ps1.  Cleared by the
      // anika:bootstrap:reset IPC (renderer's "Reload and retry").
    ...
    return ensureRuntime(resolveAnikaBackend(backend.args))
  }
  ...
  if (!isAnikaSourceRoot(ACTIVE_ANIKA_ROOT)) {
    throw new Error(
      `Anika install at ${ACTIVE_ANIKA_ROOT} is missing or incomplete. ` +
        'Reinstall via the desktop installer or scripts/install.ps1.'
    )
  }

  // On Windows, preflight Git Bash. Anika' terminal tool calls bash.exe
  ...
  // %LOCALAPPDATA%\anika\git\, which findGitBash() picks up, so for any
  // user who completed the bootstrap this is a no-op. For users who got
  // here via an external `anika` on PATH, this check still helps.
  if (IS_WINDOWS && !findGitBash()) {
    throw new Error(
      'Git for Windows is required for Anika on Windows (provides Git Bash, ' +
  ...
  if (!fileExists(venvPython)) {
    ...
    // normal flow because activeRuntimeState() requires isAnikaSourceRoot()
    // plus an importable anika_cli before it hands back the active runtime.
    // If we hit this, the user (or a deleted venv) broke the invariant; tell
    // them to re-run the install.
    throw new Error(
      `Anika venv missing at ${VENV_ROOT}. Re-run the desktop installer or ` + '`scripts/install.ps1` to rebuild it.'
    )
  }

  backend.command = getVenvPython(VENV_ROOT)
  backend.label = `Anika at ${ACTIVE_ANIKA_ROOT} (venv: ${VENV_ROOT})`
  updateBootProgress({
    phase: 'runtime.ready',
    message: 'Anika runtime is ready',
    progress: 82,
    running: true,
    error: null
  })
...
function multipartBody(upload) {
  const boundary = `----anika-${crypto.randomBytes(12).toString('hex')}`
    ...
    (requestState: any) =>
      new Promise((resolve, reject) => {
        ...
        if (parsed.protocol !== 'http:' && parsed.protocol !== 'https:') {
          reject(new Error(`Unsupported Anika backend URL protocol: ${parsed.protocol}`))
                ...
                reject(
                  new Error(
                    `Expected JSON from ${url} but got HTML (status ${res.statusCode}). ` +
                      'The endpoint is likely missing on the Anika backend.'
                  )
                )
        ...
        req.setTimeout(timeoutMs, () => {
          req.destroy(new Error(`Timed out connecting to Anika backend after ${timeoutMs}ms`))
        })
  ...
  return new Promise((resolve, reject) => {
    ...
    const req = client.request(
      parsed,
      {
        agent,
        method: 'GET',
        headers: options.bearer ? { Authorization: `Bearer ${options.bearer}` } : { 'X-Anika-Session-Token': token }
      },
      res => {
...
function fetchPublicJson(url, options: any = {}) {
  ...
  // NO ``X-Anika-Session-Token`` header — used by the auth-mode probe before
...
function getLinkTitleSession() {
  ...
  linkTitleSession = session.fromPartition('anika:link-titles', { cache: false })
...
async function previewFileTarget(rawTarget, baseDir) {
  ...
  const base = baseDir ? path.resolve(expandUserPath(baseDir)) : resolveAnikaCwd()
...
function sendPreviewFileChanged(payload) {
  ...
  webContents.send('anika:preview-file-changed', payload)
}
...
// waitForAnikaReady how to read a 401 — rejected session vs gated route.
...
async function waitForAnika(baseUrl, token, signal?, authMode?, headers = {}) {
  const { probeHealth, probeIsCredentialed } = await buildReadinessHealthProbe(baseUrl, authMode, token)

  return waitForAnikaReady(baseUrl, {
    token,
    signal,
    fetchPublicJson,
...
function sendBackendExit(payload) {
  ...
  webContents.send('anika:backend-exit', payload)
}
...
function sendClosePreviewRequested() {
  ...
  webContents.send('anika:close-preview-requested')
}
...
function sendPreviewNavCommand(command: 'back' | 'forward' | 'reload') {
  ...
  webContents.send('anika:preview-nav', command)
}
...
function sendOpenFolderRequested() {
  ...
  webContents.send('anika:open-folder-requested')
}

// Tell the renderer the machine just woke. Sleep silently drops the
// renderer's WebSocket to the local backend; the renderer reconnects on this
// signal so the chat composer doesn't stay stuck on "Starting Anika...".
function sendPowerResume() {
  ...
  webContents.send('anika:power-resume')
}
...
// 'anika:power-battery' push below.
ipcMain.handle('anika:power-battery:get', () => onBatteryPower === true)
...
function broadcastBatteryState(next: boolean) {
  ...
  for (const win of BrowserWindow.getAllWindows()) {
    ...
    if (webContents && !webContents.isDestroyed()) {
      webContents.send('anika:power-battery', next)
    }
  }
}
...
async function showPluginCompatNoticeOnce() {
  ...
  try {
    notice = pendingPluginCompatNotice(ANIKA_HOME, app.getPath('userData'))
  } catch (err) {
...
function sendOpenUpdatesRequested() {
  ...
  webContents.send('anika:open-updates')
...
function sendWindowStateChanged(nextIsFullscreen?: boolean, target = mainWindow) {
  ...
  webContents.send('anika:window-state-changed', state)
}
...
function installContextMenuBridge(window: BrowserWindow) {
  window.webContents.on('context-menu', (_event, params) => {
    ...
    if (params.isEditable && params.misspelledWord) {
      window.webContents.send('anika:context-menu-spellcheck', {
...
//
// Hosted Anika gateways gate the dashboard behind an OAuth provider (e.g.
// __NEW_ORG__) instead of a static session token. The auth model is
// fundamentally different from the token path:
//
//   * REST is authed by HttpOnly session cookies (``anika_session_at``),
...
//   * __NEW_ORG__ Portal now issues a 24h ROTATING, reuse-detected refresh token
//     alongside the ~15-min access token (Portal NAS #293 / anika #37247).
//     Both are set as HttpOnly cookies (``anika_session_at`` ~15 min,
//     ``anika_session_rt`` 24h). When the AT cookie lapses but the RT cookie
...
// "not signed in", which on the initial boot path (startAnika → the renderer's
// single-shot boot() with no retry) surfaced as the "Anika couldn't start"
...
async function hasLiveOauthSession(baseUrl) {
  ...
  // for a signed-in user — the exact race that produced the transient "Anika
  ...
  return new Promise((resolve, reject) => {
    ...
    try {
      win = new BrowserWindow({
        width: 520,
        height: 720,
        title: silent ? 'Connecting to Anika Cloud agent…' : 'Sign in to Anika gateway',
...
function fetchJsonViaOauthSession(url, options: any = {}) {
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      ...
      reject(new Error(`Timed out connecting to Anika backend after ${timeoutMs}ms`))
    }, timeoutMs)
...
// feature; the server half lives in anika_cli/dashboard_auth/native_flow.py.
// ---------------------------------------------------------------------------
...
// --- Anika Cloud discovery + silent per-agent sign-in (cloud-auto-discovery
// Phase 3) ---------------------------------------------------------------
//
// The "cloud" connection mode lets a user sign in to the __NEW_ORG__ portal ONCE in
...
// portal session cookie living in `persist:anika-remote-oauth`:
...
// Canonical __NEW_ORG__ portal base URL, overridable for staging/dev. Mirrors the CLI
// convention (anika_cli/auth.py DEFAULT_NOUS_PORTAL_URL + the same env names)
// so a single override flips every Anika surface to the same portal.
const DEFAULT_NOUS_PORTAL_URL = 'https://portal.__NEW_DOMAIN__'

function resolvePortalBaseUrl() {
  const raw = process.env.ANIKA_PORTAL_BASE_URL || process.env.NOUS_PORTAL_BASE_URL || DEFAULT_NOUS_PORTAL_URL

  return String(raw).trim().replace(/\/+$/, '')
}

// Whether the OAuth partition currently holds a live __NEW_ORG__ portal session — the
// credential that powers both discovery and the silent cascade. The portal
// authenticates via PRIVY, not the Anika gateway session cookies, so this
// checks for the `privy-token` cookie on the portal host (NOT
// hasLiveOauthSession, which looks for anika_session_at/rt that the portal
    ...
    return await new Promise<boolean>(resolve => {
      ...
      try {
        win = new BrowserWindow({
          ...
          height: 720,
          show: false,
          title: 'Renewing Anika Cloud session…',
          autoHideMenuBar: true,
          webPreferences: {
            contextIsolation: true,
            nodeIntegration: false,
...
function openPortalLoginWindow() {
  ...
  return new Promise((resolve, reject) => {
    if (!app.isReady()) {
      reject(new Error('Desktop is not ready to start a Anika Cloud sign-in.'))
...
// Discover the hosted (Anika Cloud) agents the signed-in user can see. Calls
...
async function discoverCloudAgents(org?: string) {
  ...
  if (!(await hasLivePortalSession())) {
    const err = new Error(
      'You are not signed in to Anika Cloud. Open Settings → Gateway, choose Anika Cloud, and sign in.'
    ) as any
  ...
  try {
    ...
    if (body === undefined) {
      ...
      if (error && error.statusCode === 401) {
        const err = new Error(
          'Your Anika Cloud session has expired. Open Settings → Gateway and sign in again.'
        ) as any
...
async function cloudAgentSilentSignIn(dashboardUrl) {
  ...
  if (!(await hasLivePortalSession())) {
    const err = new Error('Your Anika Cloud session has expired. Sign in to Anika Cloud again.') as any
    err.needsCloudLogin = true
    throw err
  }
...
function sanitizeConnectionProfiles(raw: Record<string, any>) {
  ...
  for (const [name, entry] of Object.entries(raw)) {
    ...
    // Preserve the Anika Cloud org tag on cloud-mode entries so Settings can
...
function readDesktopConnectionsRegistry() {
  ...
  if (mtime === null) {
    ...
    try {
      ...
      // hard-failing every anika:connections:* call.
 ...
 * editor doesn't carry — cloud `org`, ssh `remoteAnikaPath`/`remoteProfile` —
...
// a valid stored value (pins the root ANIKA_HOME explicitly); null means "no
...
// contains "anika", avoiding false positives from stale gateway.pid files
// whose PID was recycled by the OS to an unrelated process.
function isAnikaProcess(pid) {
  try {
...
//   1. Legacy ~/.anika/active_profile (explicit CLI choice via anika profile use)
//   2. Running gateway (gateway.pid with verified liveness + anika identity)
...
function migrateActiveProfileIfMissing() {
  migrateActiveProfileIfMissingPure(DESKTOP_PROFILE_CONFIG_PATH, {
    legacyActivePath: path.join(ANIKA_HOME, 'active_profile'),
    anikaHome: ANIKA_HOME,
    profilesRoot: path.join(ANIKA_HOME, 'profiles'),
...
async function sanitizeDesktopConnectionConfig(config = readDesktopConnectionConfig(), profile = null) {
  ...
  const envOverride = key ? false : Boolean(process.env.ANIKA_DESKTOP_REMOTE_URL)
  ...
  const remoteUrl = envOverride ? String(process.env.ANIKA_DESKTOP_REMOTE_URL || '') : String(block.url || '')
  ...
  return {
    ...
    remoteUrl,
    // The persisted Anika Cloud org (slug/id) for a cloud connection, or '' for
    ...
    sshRemoteAnikaPath: (ssh || savedSsh)?.remoteAnikaPath || '',
    sshRemoteProfile: (ssh || savedSsh)?.remoteProfile || '',
    // The env override only forces the global/primary connection; a per-profile
    // scope is never overridden by ANIKA_DESKTOP_REMOTE_URL.
    envOverride
  }
}
...
// `org` (optional) is the Anika Cloud org slug/id the instance was discovered
...
function coerceDesktopConnectionConfig(input: any = {}, existing = readDesktopConnectionConfig(), options: any = {}) {
  ...
  // describe a discovered Anika Cloud instance, NOT a user-owned remote gateway,
...
function buildSshBlock(input: any, existingBlock: any = {}) {
  ...
  const merged = normalizeSshConfig({
    ...
    remoteAnikaPath: input.sshRemoteAnikaPath ?? existingBlock.remoteAnikaPath,
...
async function buildRemoteConnection(
  ...
  if (authMode === 'oauth') {
    ...
    // refresh token (anika #37247), and the gateway middleware transparently
    ...
    try {
      ...
      throw gatewayTicketFailure(
        ...
        'Could not reach the remote Anika gateway while refreshing its WebSocket ticket. Try reconnecting.'
      )
    }
  ...
  if (!token) {
    throw new Error(
      'Remote Anika gateway is selected, but no session token is saved. ' +
...
function activeSshTerminalTarget(webContentsId?: number) {
  ...
  const route = resolveDesktopRemoteRoute({
    config,
    env: {
      token: process.env.ANIKA_DESKTOP_REMOTE_TOKEN,
      url: process.env.ANIKA_DESKTOP_REMOTE_URL
    },
    profile,
    registry: readDesktopConnectionsRegistry()
  })
...
async function rollbackSshBootstrapResult(ssh, result, profile, sshConfig, boundaryError) {
  ...
  try {
    const expected = {
      ...
      anikaPath: result.anikaPath,
      anikaHome: result.anikaHome,
    ...
    if (result.platform?.os === 'Windows') {
      await terminateOwnedWindowsDashboardForUpdate(
        ssh,
        { anikaPath: result.anikaPath, anikaHome: result.anikaHome, python: result.pythonPath },
        expected
      )
...
async function bootstrapSshConnectionInner(profile, sshConfig, reuseToken, source, metadata, fingerprint, lease) {
  ...
  try {
    ...
    const platform = await detectRemotePlatform(ssh, sshConfig.remoteAnikaPath || '')
    const lifecycle = platform.os === 'Windows' ? connectWindowsRemote : remoteLifecycle.connect
    result = await lifecycle({
      ssh,
      profile: resolveRemoteSshDashboardProfile(sshConfig.remoteProfile, profile),
      remoteAnikaPath: sshConfig.remoteAnikaPath || '',
      ownershipId: sshOwnershipKey(profile),
      ...
      pickLocalPort,
      waitForAnika: (baseUrl, token) => waitForAnika(baseUrl, token, lease.signal, 'token'),
  ...
  await fenceManagedSshBootstrapPublication({
    ...
    publish: () => {
      ...
      sshConnections.set(scope, {
        ...
        host: sshConfig.host,
        hostLabel,
        anikaVersion: result.anikaVersion || '',
        ...
        // per-profile pools also use bare keys. Only startAnika' explicit call
  ...
  sshRememberLog(
    `[ssh] connection ${result.reused ? 'REUSED' : 'spawned'} dashboard: ` +
      `${result.anikaVersion || 'anika (version unknown)'} at ${result.anikaPath || '?'}`
  )
  ...
  return {
    ...connection,
    remoteAnikaVersion: result.anikaVersion || '',
    ssh: {
      effectiveConfigFingerprint: sshConfig.effectiveConfigFingerprint,
      host: sshConfig.host,
...
//   2. env override (ANIKA_DESKTOP_REMOTE_URL/_TOKEN) — applies app-wide
...
function globalRemoteActive() {
  if (process.env.ANIKA_DESKTOP_REMOTE_URL) {
    return true
  }
...
// env → global) so a startAnika() failure can be classified as remote (never
...
async function probeRemoteAuthMode(rawUrl) {
  ...
  // sending any credentials. ``/api/status`` is public on every Anika
  ...
  if (authRequired) {
    ...
    // button can read "Sign in with __NEW_ORG__" instead of a generic
...
async function testDesktopConnectionConfig(input: any = {}) {
  if (input.mode === 'ssh') {
    const sshConfig = normalizeSshConfig({
      ...
      remoteAnikaPath: input.sshRemoteAnikaPath
    })
    ...
    try {
      ...
      for (;;) {
        try {
          await ssh.open()
          const platform: any = await detectRemotePlatform(ssh, sshConfig.remoteAnikaPath || '')
          let anikaPath
          let anikaVersion
          let supported
          ...
          if (platform.os === 'Windows') {
            ...
            anikaPath = runtime.anikaPath
            const inspection = await helper(ssh, runtime, 'inspect', [runtime.anikaPath])
            anikaVersion = inspection.version
            supported = inspection.supported
          } else {
            anikaPath = await remoteLifecycle.locateAnika(ssh, sshConfig.remoteAnikaPath || '')
            anikaVersion = await remoteLifecycle.probeAnikaVersion(ssh, anikaPath)
            supported = await remoteLifecycle.remoteSupportsSshOwnership(ssh, anikaPath)
          }
          ...
          if (!supported) {
            return {
              ...
              error: 'Update Anika on the remote host before connecting with Desktop SSH.'
            }
          }
          ...
          return {
            ...
            remoteAnikaPath: anikaPath,
            remoteAnikaVersion: anikaVersion,
  ...
  if (wantRemote && block?.url) {
    ...
    const remote = (await resolveRemoteBackend(key)) || (await startAnika())
  ...
  // connect to Anika gateway". Mirror the renderer's connect here so the test
...
function resetAnikaConnection({ soft = false } = {}) {
  backendStartFailure = null
  remoteReauthFailure = null
  remoteLiveness.clear()
  const anikaProcess = backendConnectionState.invalidate()
  stopBackendChild(anikaProcess)
...
// startAnika() spawns fresh instead of racing the dying one. Shared by the
// connection-config and profile switch flows.
async function teardownPrimaryBackendAndWait({ soft = false } = {}) {
  // Capture the reference before resetAnikaConnection() invalidates it.
  const anikaProcess = backendConnectionState.getProcess()
  const dying = anikaProcess && !anikaProcess.killed ? anikaProcess : null
...
function sendConnectionApplied() {
  ...
  webContents.send('anika:connection:applied')
}
...
function broadcastConnectionsChanged(payload: { connectionId: string; reason: 'removed' | 'saved' | 'updated' }) {
  for (const win of BrowserWindow.getAllWindows()) {
    ...
    if (webContents && !webContents.isDestroyed()) {
      webContents.send('anika:connections:changed', payload)
    }
  }
}
...
async function ensureBackend(profile, opts: { spawnPriority?: LocalBackendSpawnPriority } = {}) {
  ...
  if (route.backend === 'primary') {
    const connection = await startAnika()
  ...
  // spawning, so two children never share one profile's ANIKA_HOME.
  ...
  const resolveRegistrySshConfig = () => {
    ...
    if (!resolvedRegistrySshConfig) {
      resolvedRegistrySshConfig = normalizeSshConfig({
        ...
        remoteAnikaPath: source.remoteAnikaPath,
...
async function connectRegistryBackend(
  ...
  if (source.kind === 'ssh') {
    ...
    return {
      ...
      // is only the routing label. anika:api uses it to translate explicit
      // self-profile query filters into the backend's namespace.
      remoteProfile: sshConfig.remoteProfile || '',
      logs: anikaLog.slice(-80),
      ...getWindowState()
    }
  }
  ...
  await waitForAnika(connection.baseUrl, connection.token, undefined, connection.authMode, connection.headers)
  poolEntry.remoteBaseUrl = connection.baseUrl
...
function remoteUpdateTargetFromState(state): RemoteUpdateTarget {
  if (!state?.ssh || !state?.anikaPath || !state?.anikaHome) {
  ...
  return {
    ...
    anikaPath: state.anikaPath,
    anikaHome: state.anikaHome,
...
async function openManagedSshUpdateTransport(
  ...
  try {
    const platform: any = await detectRemotePlatform(ssh, config.remoteAnikaPath || '')

    if (platform.os === 'Windows') {
      const runtime = platform.anikaPath ? platform : await probeWindowsRemote(ssh, config.remoteAnikaPath || '')
      ...
      return {
        ...
        target: {
          ...
          platform: 'Windows',
          anikaPath: runtime.anikaPath,
          anikaHome: runtime.anikaHome,
          pythonPath: runtime.python
        }
      }
    }

    const anikaPath = await remoteLifecycle.locateAnika(ssh, config.remoteAnikaPath || '')
    const anikaHome = await remoteLifecycle.probeRemoteAnikaHome(ssh)

    return {
      close: () => ssh.close(),
      target: { ssh, platform: platform.os, anikaPath, anikaHome }
    }
  } catch (error) {
    await ssh.close()
    throw error
...
async function drainManagedSshScope(scope) {
  ...
  try {
    ...
    if (state.remotePlatform === 'Windows') {
      await terminateOwnedWindowsDashboardForUpdate(
        state.ssh,
        { anikaPath: state.anikaPath, anikaHome: state.anikaHome, python: state.pythonPath },
        expected
      )
    ...
    if (state && state.localPort && state.remotePort && scope.reuseToken) {
      try {
        ...
        if (!forwardClosed) {
          await waitForAnika(`http://127.0.0.1:${state.localPort}`, scope.reuseToken, undefined, 'token')
        } else {
...
// local-spawn portion of startAnika() but without the boot-progress UI,
...
async function spawnPoolBackend(profile, entry, opts: { forceLocal?: boolean; poolKey?: string } = {}) {
  ...
  if (remote) {
    await waitForAnika(remote.baseUrl, remote.token, undefined, remote.authMode, remote.headers)
  ...
  // --profile wins over the inherited ANIKA_HOME env (see _apply_profile_override
  // step 3 in anika_cli/main.py), so the child re-homes to this profile.
  // --port 0: the OS assigns an ephemeral port; the child announces it on stdout.
  const backendArgs = ['--profile', profile, 'serve', '--host', '127.0.0.1', '--port', '0']
  const backend = await ensureRuntime(resolveAnikaBackend(backendArgs))
  // Route old runtimes (no `serve`) through the legacy `dashboard --no-open`.
  backend.args = getBackendArgsForRuntime(backend)
  const anikaCwd = resolveAnikaCwd()
  ...
  assertLocalProfileCanStart(profile, profileDeletionGate, key =>
    directoryExists(path.join(ANIKA_HOME, 'profiles', key))
  )
  rememberLog(`Starting Anika backend for profile "${profile}" via ${backend.label}`)
  ...
  const child = spawn(
    ...
    hiddenWindowsChildOptions({
      ...
      env: {
        ...
        TERMINAL_CWD: anikaCwd,
        ANIKA_DASHBOARD_SESSION_TOKEN: token,
        ...
        ...parentIdentityEnv,
        ANIKA_WEB_DIST: webDist,
        ...(readyFile ? { ANIKA_DESKTOP_READY_FILE: readyFile } : {})
      },
      shell: backend.shell,
      stdio: ['ignore', 'pipe', 'pipe']
    })
  ...
  child.once('error', error => {
    rememberLog(`Anika backend for profile "${profile}" failed to start: ${error.message}`)
    void teardownFailedLocalBackend(poolKey, entry).catch(cleanupError => {
      rememberLog(
        `Anika backend for profile "${profile}" cleanup failed: ${cleanupError instanceof Error ? cleanupError.message : String(cleanupError)}`
      )
    })
    rejectStart?.(error)
  })
  child.once('exit', (code, signal) => {
    rememberLog(`Anika backend for profile "${profile}" exited (${signal || code})`)
    releaseLocalBackendSlot(entry)
    releaseBackendChild(child)
    ...
    if (!ready) {
      rejectStart?.(
        new Error(
          `Anika backend for profile "${profile}" exited before it became ready (${signal || code}).${outputTail.describe()}`
        )
      )
    }
  })
  ...
  await Promise.race([waitForAnika(baseUrl, token), startFailed])
  ready = true

  const authToken = await adoptServedDashboardToken(baseUrl, token, {
    childAlive: () => child.exitCode === null && !child.killed,
    label: `Anika backend for profile "${profile}"`,
    rememberLog
  })
  ...
  if (!wsProbe.ok) {
    throw new Error(
      `Anika backend for profile "${profile}" is HTTP-reachable but the WebSocket (/api/ws) rejected the session token: ${wsProbe.reason}`
    )
  }
...
// backend whose ensure_anika_home() recreates the deleted profile directory.
//
...
async function startAnika() {
  ...
  if (!isPrimaryInstance) {
    ...
    throw new Error('Anika Desktop is already running in another window.')
  }

  await reapOrphanedBackendsOnce()

  // Latched-failure short-circuit: once bootstrap has failed in this
  // process, every subsequent startAnika() call re-throws the same error
  ...
  if (BOOT_FAKE_ERROR) {
    await advanceBootProgress('backend.resolve', 'Resolving Anika backend', 8)
  ...
  const connectionPromise = (async () => {
    const connectRemote = async remote => {
      ...
      if (!backendConnectionState.isCurrentAttempt(connectionAttempt)) {
        throw new Error('Anika backend start was superseded by a newer connection attempt.')
      }

      await advanceBootProgress('backend.remote', `Connecting to remote Anika backend at ${remote.baseUrl}`, 24)
      await waitForAnika(remote.baseUrl, remote.token, undefined, remote.authMode, remote.headers)
      ...
      updateBootProgress({
        phase: 'backend.ready',
        message: 'Remote Anika backend is ready',
        progress: 94,
        running: true,
        error: null
      })

      return createPrimaryRemoteConnection(remote, anikaLog.slice(-80), getWindowState())
    }
    ...
    // deterministic (it wins over the sticky ~/.anika/active_profile file) and
    // resolves ANIKA_HOME the same way `anika -p <name>` does on the CLI. An
    ...
    const setup = await runPrimaryBackendStartup({
      ...
      prepareLocalBackend: async () => {
        await advanceBootProgress('backend.runtime', 'Resolving Anika runtime', 28)

        return resolveAnikaBackend(backendArgs)
      },
      resolveRemote: () => {
    ...
    await advanceBootProgress('backend.spawn', `Starting Anika backend via ${backend.label}`, 84)
    rememberLog(`Starting Anika backend via ${backend.label}`)
    ...
    const anikaProcess = spawn(
      ...
      hiddenWindowsChildOptions({
        ...
        env: {
          ...process.env,
          // Explicitly pin ANIKA_HOME for the child so Python's get_anika_home()
          // resolves to the SAME location our resolveAnikaHome() picked. Without
          // this pin, Python falls back to ~/.anika on every platform — fine on
          // mac/linux (where our default matches), but on Windows our default is
          // %LOCALAPPDATA%\anika, which differs from C:\Users\<u>\.anika.
          // Mismatch would split config / sessions / .env / logs across two
          // directories. install.ps1 sets ANIKA_HOME via setx; the desktop
    ...
    primaryOutputTail.attach(anikaProcess)
    ...
    const portAnnouncement = waitForDashboardPortAnnouncement(anikaProcess, {
    ...
    const processOwner = backendConnectionState.attachProcess(connectionAttempt, anikaProcess)

    if (!processOwner) {
      stopBackendChild(anikaProcess)
      await waitForBackendExit(anikaProcess)
      releaseBackendChild(anikaProcess)
      throw new Error('Anika backend start was superseded by a newer connection attempt.')
    }

    anikaProcess.stdout.on('data', rememberLog)
    anikaProcess.stderr.on('data', rememberLog)
    ...
    anikaProcess.once('error', error => {
      releaseBackendChild(anikaProcess)

      if (!backendConnectionState.clearForCurrentProcess(processOwner)) {
        rememberLog(`Ignoring stale Anika backend error: ${error.message}`)
        rejectBackendStart?.(new Error('Anika backend start was superseded by a newer connection attempt.'))

        return
      }

      rememberLog(`Anika backend failed to start: ${error.message}`)
      updateBootProgress(
        {
          error: error.message,
          message: `Anika backend failed to start: ${error.message}`,
          phase: 'backend.error',
          running: false
        },
        { allowDecrease: true }
    ...
    anikaProcess.once('exit', (code, signal) => {
      releaseBackendChild(anikaProcess)

      if (!backendConnectionState.clearForCurrentProcess(processOwner)) {
        rememberLog(`Ignoring stale Anika backend exit (${signal || code})`)
      ...
      rememberLog(`Anika backend exited (${signal || code})`)
      sendBackendExit({ code, signal })

      if (!backendReady) {
        const message = `Anika backend exited before it became ready (${signal || code}).${primaryOutputTail.describe()}`
        updateBootProgress(
          {
            error: message,
            message,
        ...
        rejectBackendStart?.(
          new Error(
            `Anika backend exited before it became ready (${signal || code}). Log: ${DESKTOP_LOG_PATH}\n${recentAnikaLog()}`
          )
        )
      }
    })

    await advanceBootProgress('backend.port', 'Waiting for Anika backend to launch', 86)
    ...
    const baseUrl = `http://127.0.0.1:${port}`
    await advanceBootProgress('backend.wait', 'Waiting for Anika backend to become ready', 90)
    await Promise.race([waitForAnika(baseUrl, token), backendStartFailed])
    backendReady = true
    backendStartFailure = null

    const authToken = await adoptServedDashboardToken(baseUrl, token, {
      childAlive: () => anikaProcess.exitCode === null && !anikaProcess.killed,
      rememberLog
    })
    ...
    if (!wsProbe.ok) {
      throw new Error(
        `Local Anika backend is HTTP-reachable but the WebSocket (/api/ws) rejected the session token: ${wsProbe.reason}`
      )
    }

    updateBootProgress({
      phase: 'backend.ready',
      message: 'Anika backend is ready. Finalizing desktop startup',
      progress: 94,
      running: true,
      error: null
    })
    ...
    // The backend's plugin discovery just ran and refreshed ANIKA_HOME/.plugin-compat-report.json.
...
function wireCommonWindowHandlers(win, { zoom = true }: { zoom?: boolean } = {}) {
  ...
  // links arrive via `anika:openExternal`, not here. See window-open-policy.ts.
...
function notifyBrowserPopoutClosed(tabId) {
  ...
  for (const other of BrowserWindow.getAllWindows()) {
    if (!other.isDestroyed()) {
      other.webContents.send('anika:browser-popout:closed', tabId)
    }
  }
}
...
// here so it can leave the app's bounds and stay visible while Anika is
...
// pushes pet state over IPC (anika:pet-overlay:state); the overlay just renders
// it. Control flows back (pop-in, composer submit) via anika:pet-overlay:control.
...
function spawnPetOverlayWindow(bounds) {
  const win = new BrowserWindow({
    ...
    // instead, leaving the main Anika app as the Dock/cmd-tab anchor.
    skipTaskbar: !IS_MAC,
    hasShadow: false,
    alwaysOnTop: true,
    ...
    // anchor and the Anika icon drops out of cmd/alt-tab — especially when the
    // main window is minimized. We flip this on only while the composer needs
    // the keyboard (see anika:pet-overlay:set-focusable).
    focusable: false,
    show: false,
  ...
  try {
    ...
    // which briefly hides the Dock/cmd-tab presence. Keep Anika in the normal
  ...
  win.on('closed', () => {
    ...
    if (mainWindow && !mainWindow.isDestroyed()) {
      mainWindow.webContents.send('anika:pet-overlay:control', { type: 'pop-in' })
    }
  })
...
// window showing only the composer and its scrollback, so Anika can be driven
...
function startHudCursorFeed(win: BrowserWindow) {
  ...
  const timer = setInterval(() => {
    ...
    last = key
    win.webContents.send('anika:hud:cursor', point)
  }, HUD_CURSOR_POLL_MS)
...
function startHudGameOverlayFeed(win: BrowserWindow) {
  ...
  const push = (state: { active: boolean; app: string }) => {
    if (!win.isDestroyed()) {
      win.webContents.send('anika:hud:game-overlay', state)
    }
  }
  ...
  const enumerate = async () => {
    ...
    if (!reported) {
      reported = true
      console.warn(`[anika] HUD cannot enumerate windows: ${windows.reason}`)
    }
...
function broadcastHudState(open) {
  ...
  for (const win of BrowserWindow.getAllWindows()) {
    if (!win.isDestroyed()) {
      win.webContents.send('anika:hud:changed', payload)
    }
  }
}
...
function spawnHudWindow(sessionId, profile) {
  const win = new BrowserWindow({
    ...
    // `anika:hud:set-bounds`, which flips resizable on for the call — the
  ...
  // grab, anika:hud:workspace-transfer temporarily makes the X11 window
...
function openHudWindow(sessionId, profile) {
  ...
  if (hudWindow && !hudWindow.isDestroyed()) {
    ...
    if (sessionId && sessionId !== hudSessionId) {
      ...
      hudWindow.webContents.send('anika:hud:goto', sessionId)
...
function spawnQuickEntryWindow() {
  ...
  win.webContents.on('did-finish-load', () => {
    if (!win.isDestroyed() && quickEntryLastState) {
      win.webContents.send('anika:quick-entry:state', quickEntryLastState)
    }
  })
...
function showQuickEntryWindow() {
  ...
  quickEntryWindow.webContents.send('anika:quick-entry:shown')
}
...
function createWindow() {
    ...
    callbacks: {
      ...
      onFailedLoadBudgetExhausted: details => {
        ...
        void loadRendererLoadErrorPage(mainWindow, {
          ...
          repairHint: 'anika desktop --force-build',
  ...
  startAnika().catch(error => rememberLog(error.stack || error.message))
...
ipcMain.handle('anika:connection', async (_event, profile, extra) => {
...
ipcMain.handle('anika:connection:for', async (_event, payload) => {
  ...
  // Same single-owner claim as 'anika:connection', keyed by the composite
...
ipcMain.on('anika:connection:active-route', (event, route) => {
...
// "Starting Anika…". Before the renderer's backoff loop reconnects, it asks us
// to confirm the cached PRIMARY backend is still reachable; if a remote one is
// not, we drop the cache so the next getConnection() rebuilds it. Local backends
// self-heal via their child 'exit' handler, so we never touch them here.
ipcMain.handle('anika:connection:revalidate', async () => {
  ...
  return remoteRevalidation.run(connectionPromise, async () => {
    const [result] = await Promise.all([
      revalidateRemoteConnection({
        ...
        resetConnection: () => resetAnikaConnection({ soft: true }),
...
ipcMain.handle('anika:backend:touch', async (_event, profile) => {
...
ipcMain.handle('anika:pool-limits:get', async () => ({ ...poolLimits }))
ipcMain.handle('anika:pool-limits:set', async (_event, raw) => {
...
ipcMain.handle('anika:gateway:ws-url', async (_event, profile) => {
  return gatewayWsUrlIpcResult(() => freshGatewayWsUrl(profile))
})
ipcMain.handle('anika:window:openSession', async (_event, sessionId, opts) => {
...
ipcMain.handle('anika:window:openInstance', async () => {
  createInstanceWindow()

  return { ok: true }
})
ipcMain.handle('anika:window:openBrowser', async (_event, tabId) => {
...
ipcMain.handle('anika:window:openInTerminal', async (_event, sessionId, opts) => {
  ...
  try {
    ...
    const backend = resolveAnikaBackend(tuiResumeArgs(sessionId.trim(), profile || undefined))

    if (!backend.command) {
      return { ok: false, error: 'Anika is not installed yet' }
    }
    ...
    const scriptPath = path.join(
      scriptDir,
      `anika-${crypto.randomBytes(6).toString('hex')}${terminalScriptExtension()}`
    )
    ...
    fs.writeFileSync(
      ...
      buildTerminalScript({
        ...
        cwd,
        env: terminalScriptEnv(backend.env, ANIKA_HOME)
      }),
      { mode: 0o700 }
    )
...
ipcMain.handle('anika:wake-indicator:get', () => wakeIndicatorController.getState())
ipcMain.on('anika:wake-indicator:set', (_event, state) => {
...
ipcMain.handle('anika:zoom:get', event => {
...
ipcMain.on('anika:zoom:set-percent', (event, percent) => {
  const window = BrowserWindow.fromWebContents(event.sender)
...
ipcMain.handle('anika:backend:recycle', async (_event, profile) => {
...
ipcMain.handle('anika:bootstrap:reset', async () => {
  // Renderer's "Reload and retry" path. Clear the latched failure and
  // reset connection state so the next startAnika() call restarts the
...
ipcMain.handle('anika:bootstrap:repair', async () => {
  // Forceful repair: force the next startAnika() through the full installer
  ...
  // "bypass active runtime" path inside startAnika() and fall through
...
ipcMain.handle('anika:boot-progress:get', async () => bootProgressState)
ipcMain.handle('anika:bootstrap:get', async () => getBootstrapState())
ipcMain.handle('anika:connection-config:get', async (_event, profile) =>
  sanitizeDesktopConnectionConfig(readDesktopConnectionConfig(), profile)
)
ipcMain.handle('anika:plugin-profile-routes', async (_event, rawProfileNames) => {
...
ipcMain.handle('anika:ssh-config:hosts', async () => ({ hosts: collectSshConfigHosts() }))
ipcMain.handle('anika:ssh-config:resolve', async (_event, host) => {
...
ipcMain.handle('anika:connection-config:test', async (_event, payload) => testDesktopConnectionConfig(payload))
...
ipcMain.handle('anika:secret-storage:get', async () => ({ on: secretStoragePolicy().on }))
ipcMain.handle('anika:secret-storage:set', async (_event: any, on: any) => applySecretStorageEncryption(on === true))
...
ipcMain.handle('anika:connections:list', async () => sanitizeConnectionsRegistry())
ipcMain.handle('anika:connections:save', async (_event, payload) => {
  const saved = await saveRegistryConnection(payload)

  return { ok: true, connection: saved, registry: sanitizeConnectionsRegistry() }
})
ipcMain.handle('anika:connections:remove', async (_event, id) => {
  const key = String(id || '')
...
ipcMain.handle('anika:connections:set-primary', async (_event, id) => {
...
ipcMain.handle('anika:connections:set-launch-mode', async (_event, mode) => {
  assertCanMutateManagedPrimaryRouting()
...
ipcMain.handle('anika:connections:set-last-used', async (_event, id) => {
  const registry = setLastUsedConnection(readDesktopConnectionsRegistry(), String(id || ''))
  writeDesktopConnectionsRegistry(registry)

  return { ok: true, registry: sanitizeConnectionsRegistry(registry) }
})
ipcMain.handle('anika:connections:test', async (_event, id) => {
  const registry = readDesktopConnectionsRegistry()
  ...
  if (entry.kind === 'ssh') {
    const result = await testDesktopConnectionConfig({
      ...
      sshRemoteAnikaPath: entry.remoteAnikaPath
    })
...
async function probeSshProfileInventory(connection) {
  ...
  const sshConfig = normalizeSshConfig({
    ...
    remoteAnikaPath: connection.remoteAnikaPath
  })
  ...
  try {
    await ssh.open()
    const profiles = await remoteLifecycle.listRemoteAnikaProfiles(ssh)
    ...
    registry.connections.map(async connection => {
      ...
      try {
        ...
        if (connection.kind === 'ssh') {
          ...
          // The root ANIKA_HOME is an agent too; enumerations that omit it
...
ipcMain.handle('anika:agents:roster', async () => {
  const registry = readDesktopConnectionsRegistry()
  const enumerations = await enumerateRegistryAgentSources(registry)
...
// anika:gateway:ws-url. Same single-use-ticket discipline for OAuth sources.
...
ipcMain.handle('anika:gateway:ws-url-for', async (_event, payload) => {
...
ipcMain.handle('anika:connections:update-managed', async (_event, rawId) => requestManagedSshUpdate(rawId))

// Fan out `anika update` to every eligible registered connection at once.
...
ipcMain.handle('anika:connections:update-all', async (_event, payload) => {
  const registry = readDesktopConnectionsRegistry()
    ...
    registry.connections
      ...
      .map(async connection => {
        ...
        try {
          ...
          const body: any = await postJsonForBackend(descriptor, '/api/anika/update', {}, { timeoutMs: 15_000 })
...
// analogue of the anika:api handler's own auth split: OAuth backends prefer a
...
ipcMain.handle('anika:connection-config:probe', async (_event, rawUrl) => probeRemoteAuthMode(rawUrl))
ipcMain.handle('anika:connection-config:oauth-login', async (_event, rawUrl) => {
  ...
  if (strategy === 'native') {
    try {
      ...
      // startAnika() re-dials instead of replaying the stale rejection.
      remoteReauthFailure = null
...
ipcMain.handle('anika:connection-config:oauth-logout', async (_event, rawUrl) => {
  const baseUrl = normalizeRemoteBaseUrl(rawUrl)
...
// --- Anika Cloud (cloud-auto-discovery Phase 3) ---
...
ipcMain.handle('anika:cloud:agent-sign-in', async (_event, dashboardUrl) => {
...
ipcMain.handle('anika:connection-config:save', async (_event, payload) => {
  assertCanMutateManagedPrimaryRouting()
...
ipcMain.handle('anika:connection-config:apply', async (_event, payload) => {
  assertCanMutateManagedPrimaryRouting()
...
ipcMain.handle('anika:profile:get', async () => ({ profile: readActiveDesktopProfile() }))
// Persistence-only sibling of anika:profile:set: records the profile the
...
// the gateway (#79886).
ipcMain.handle('anika:profile:remember', async (_event, name) => ({
  profile: writeActiveDesktopProfile(name)
}))
ipcMain.handle('anika:profile:set', async (_event, name) => {
  assertCanMutateManagedPrimaryRouting()
  const next = writeActiveDesktopProfile(name)

  // Switching profiles is a backend re-home: relaunch the dashboard under the
  // new ANIKA_HOME. Pool backends keep their own homes, so only the primary
  // is torn down.
  await teardownPrimaryBackendAndWait()
  mainWindow?.reload()
...
ipcMain.on('anika:previewShortcutActive', (_event, active) => {
  previewShortcutActive = Boolean(active)
})

ipcMain.handle('anika:requestMicrophoneAccess', async () => {
...
ipcMain.handle('anika:window:readBelow', async event => {
...
async function handleAnikaApiRequest(request) {
  ...
  // backend calls ensure_anika_home() which recreates the profile directory,
...
ipcMain.handle('anika:api', async (_event, request) => {
  ...
  // respawn the old-name backend and recreate its ANIKA_HOME (#45474).
  ...
  if (!mutatingProfile) {
    return handleAnikaApiRequest(request)
  }

  const releaseProfileDeletion = profileDeletionGate.acquire(mutatingProfile)

  return handleAnikaApiRequest(request).finally(releaseProfileDeletion)
})
...
ipcMain.handle('anika:ambient:claim', (_event, key) => !claimedAmbientCue(String(key ?? '')))

ipcMain.handle('anika:notify', (_event, payload) => {
  ...
  notification.on('click', () => {
    ...
    if (payload?.sessionId) {
      mainWindow.webContents.send('anika:focus-session', payload.sessionId)
    }

    // Plugin / session-less activation — serializable path (+ optional notifyId
    // for renderer callbacks). Same vocabulary as anika://index-network/….
    if (payload?.activate || payload?.notifyId) {
      mainWindow.webContents.send('anika:notification-activate', {
  ...
  notification.on('action', (_actionEvent, index) => {
    ...
    if (payload?.sessionId && !payload?.notifyId && !payload?.activate) {
      mainWindow.webContents.send('anika:notification-action', { sessionId: payload.sessionId, actionId: action.id })
...
ipcMain.handle('anika:data-url-read-max:get', () => ({
...
ipcMain.handle('anika:data-url-read-max:set', (_event, maxMb) => {
...
ipcMain.handle('anika:readFileDataUrl', async (_event, filePath) => {
...
ipcMain.handle('anika:readFileDataUrlForAttach', async (_event, filePath) => {
  return readFileDataUrlForIpc(filePath, {
...
ipcMain.handle('anika:readFileText', async (_event, filePath) => {
...
// `anika:readFileText` is the *preview* read and silently truncates at
...
ipcMain.handle('anika:readPluginSource', async (_event: unknown, filePath: unknown) => {
  const { resolvedPath, stat } = await resolveReadableFileForIpc(filePath, {
...
ipcMain.handle('anika:selectPaths', async (_event, options: any = {}) => {
...
ipcMain.handle('anika:writeClipboard', (_event, text) => {
...
ipcMain.handle('anika:selectSavePath', async (_event, options: any = {}) => {
  const result = await dialog.showSaveDialog(mainWindow, {
...
ipcMain.handle('anika:readClipboard', () => clipboard.readText())

ipcMain.handle('anika:saveGatewayFile', (_event, payload) => saveGatewayFile(payload))

ipcMain.handle('anika:saveImageFromUrl', (_event, url) => saveImageFromUrl(String(url || '')))
...
ipcMain.handle('anika:context-menu:edit', (event, command) => {
...
ipcMain.handle('anika:context-menu:copy-image', event => {
...
ipcMain.handle('anika:context-menu:spellcheck', (event, action) => {
...
ipcMain.handle('anika:context-menu:guest-add-word', (_event, payload) => {
...
ipcMain.handle('anika:capturePreview', async (_event, payload) => {
  const guest = electronWebContents.fromId(Number(payload?.webContentsId))

  return capturePreviewContents(guest, payload?.rect, payload?.viewport)
})

ipcMain.handle('anika:saveImageBuffer', async (_event, payload) => {
...
ipcMain.handle('anika:saveClipboardImage', async () => {
...
ipcMain.handle('anika:normalizePreviewTarget', (_event, target, baseDir) =>
  normalizePreviewTarget(String(target || ''), baseDir ? String(baseDir) : '')
)

ipcMain.handle('anika:watchPreviewFile', (_event, url) => watchPreviewFile(String(url || '')))

ipcMain.handle('anika:watchDirectory', (_event, dir) => watchDirectory(String(dir || '')))

ipcMain.handle('anika:stopPreviewFileWatch', (_event, id) => stopPreviewFileWatch(String(id || '')))
...
ipcMain.on('anika:active-work', (event, payload) => {
  const id = event.sender.id
...
ipcMain.on('anika:titlebar-theme', (_event, payload) => {
...
ipcMain.on('anika:native-theme', (_event, mode) => {
...
ipcMain.on('anika:translucency:support', event => {
...
ipcMain.on('anika:keep-awake', (_event, on) => {
...
ipcMain.handle('anika:quick-entry:settings:get', async () => {
...
ipcMain.handle('anika:quick-entry:settings:set', async (_event, patch) => {
...
ipcMain.on('anika:quick-entry:submit', (_event, payload) => {
  hideQuickEntryWindow()
  ...
  mainWindow.webContents.send('anika:quick-entry:submit', {
...
ipcMain.on('anika:quick-entry:state', (_event, payload) => {
  quickEntryLastState = payload ?? null

  if (quickEntryWindow && !quickEntryWindow.isDestroyed()) {
    quickEntryWindow.webContents.send('anika:quick-entry:state', payload)
  }
})

ipcMain.on('anika:quick-entry:dismiss', () => hideQuickEntryWindow())
...
ipcMain.on('anika:devtools:disable-f12', (_event, on) => {
...
ipcMain.handle('anika:openExternal', (_event, url) => {
...
// per-session secondary windows spawned via `anika:window:openSession`).
...
ipcMain.handle('anika:find-in-page', async (event, query, options) => {
  const win = BrowserWindow.fromWebContents(event.sender)
...
ipcMain.handle('anika:stop-find-in-page', event => {
  const win = BrowserWindow.fromWebContents(event.sender)
...
ipcMain.handle('anika:preview:reach', async (event, url) => reachablePreviewUrl(event.sender.id, String(url || '')))

ipcMain.handle('anika:openPreviewInBrowser', async (_event, url) => {
...
// it via writeDefaultProjectDir so resolveAnikaCwd picks it up on the next
// session spawn (no app restart needed).
ipcMain.handle('anika:setting:defaultProjectDir:get', async () => ({
  dir: readDefaultProjectDir(),
  defaultLabel: app.getPath('home'),
  resolvedCwd: resolveAnikaCwd()
}))

ipcMain.handle('anika:workspace:sanitize', async (_event, cwd) => sanitizeWorkspaceCwd(cwd))

ipcMain.handle('anika:setting:defaultProjectDir:set', async (_event, dir) => {
...
ipcMain.handle('anika:setting:defaultProjectDir:pick', async () => {
...
ipcMain.handle('anika:fetchLinkTitle', (_event, url) => fetchLinkTitle(url))

ipcMain.handle('anika:resolveFavicon', (_event, url) => resolveFaviconCached(url))

ipcMain.handle('anika:logs:reveal', async () => {
  try {
    await fs.promises.mkdir(path.dirname(DESKTOP_LOG_PATH), { recursive: true })
...
ipcMain.handle('anika:logs:recent', async () => ({ path: DESKTOP_LOG_PATH, lines: anikaLog.slice(-200) }))
...
ipcMain.on('anika:logs:renderer-error', (_event, report) => {
...
// Embedded terminal PTY host (anika:terminal:*) — see terminal-ipc.ts.
...
ipcMain.handle('anika:updates:check', async () =>
...
ipcMain.handle('anika:updates:apply', async (_event, payload) =>
...
ipcMain.handle('anika:updates:branch:get', async () => readDesktopUpdateConfig())

ipcMain.handle('anika:updates:branch:set', async (_event, name) => {
...
// Resolve the canonical Anika version (the one `release.py` bumps in
// anika_cli/__init__.py + pyproject.toml) so the desktop About panel shows the
// real Anika version instead of the Electron app's own package.json version,
// which historically drifted (stuck at 0.0.2). Falls back to app.getVersion()
// when the source tree can't be read (e.g. a packaged build without the repo).
function resolveAnikaVersion() {
  try {
    const root = resolveUpdateRoot()
    const initPath = path.join(root, 'anika_cli', '__init__.py')
...
// Renderer-bundle skew: `anika update` moves the SOURCE TREE, but the UI
...
// Re-resolve the live Anika version and push it into the native About panel
// just before showing it, so an in-place `anika update` is reflected without
  ...
  void detectRendererSkew().then(skew => {
    app.setAboutPanelOptions({
      ...
      applicationVersion: skew.outOfSync
        ? `${resolveAnikaVersion()} — app build out of date, update the desktop app`
        : resolveAnikaVersion(),
      copyright: 'Copyright © 2026 __NEW_ORG__'
    })
    app.showAboutPanel()
...
ipcMain.handle('anika:version', async () => {
  const skew = await detectRendererSkew()

  return {
    appVersion: resolveAnikaVersion(),
    electronVersion: process.versions.electron,
    nodeVersion: process.versions.node,
    platform: process.platform,
    anikaRoot: resolveUpdateRoot(),
...
// The About page's "Restart Anika" button (shown when bundleSwapPending):
...
ipcMain.handle('anika:app:relaunch', async () => {
...
// the actual removal via `anika uninstall …` so the cross-platform PATH /
// registry / service / node-symlink cleanup all lives in one place
// (anika_cli/uninstall.py + anika_cli/gui_uninstall.py).
//
...
async function getUninstallSummary() {
  ...
  const agentRoot = ACTIVE_ANIKA_ROOT
  ...
  const fallback = () => ({
    anika_home: ANIKA_HOME,
    agent_installed: isAnikaSourceRoot(agentRoot) && fileExists(py),
    gui_installed: true,
  ...
  return new Promise(resolve => {
    ...
    try {
      const child = spawn(
        py,
        ['-m', 'anika_cli.main', 'uninstall', '--gui-summary'],
        hiddenWindowsChildOptions({
          cwd: agentRoot,
          env: { ...process.env, ANIKA_HOME, NO_COLOR: '1' },
          stdio: ['ignore', 'pipe', 'ignore']
        })
      )
...
async function runDesktopUninstall(mode) {
  ...
  if (!fileExists(venvPy)) {
    return {
      ok: false,
      error: 'agent-missing',
      message: `Can't run the uninstaller: no Anika agent venv at ${VENV_ROOT}.`
    }
  }
  ...
  // Python with PYTHONPATH=<agentRoot> so `import anika_cli` resolves from
  ...
  try {
    await releaseBackendLock(ACTIVE_ANIKA_ROOT, 'uninstall')
  } catch (error) {
  ...
  try {
    if (IS_WINDOWS) {
      scriptPath = path.join(app.getPath('temp'), `anika-uninstall-${Date.now()}.cmd`)
      ...
      scriptPath = path.join(app.getPath('temp'), `anika-uninstall-${Date.now()}.sh`)
...
ipcMain.handle('anika:uninstall:summary', async () => getUninstallSummary())
ipcMain.handle('anika:uninstall:run', async (_event, payload) => {
...
ipcMain.handle('anika:vscode-theme:fetch', async (_event, id) => fetchMarketplaceThemes(String(id || '')))

// Search the Marketplace for color-theme extensions (empty query = top installs).
ipcMain.handle('anika:vscode-theme:search', async (_event, query) => searchMarketplaceThemes(String(query || ''), 20))

// ---------------------------------------------------------------------------
// anika:// deep links (e.g. anika://blueprint/morning-brief?time=08:00,
// anika://mcp/install?name=NAME&config=B64 — the vendor "Add to Anika"
// button, or anika://plugin/install?repo=owner/repo). Dev
// (`ANIKA_DESKTOP_DEV_SERVER`) registers anika-dev:// instead — bare
// Electron or a stale OS handler often owns anika:// on dev machines.
...
// ---------------------------------------------------------------------------
const ANIKA_PROTOCOL = DEV_SERVER ? 'anika-dev' : 'anika'
/** Schemes accepted when parsing inbound URLs (dev accepts both).
 *  'hermes' is kept as a legacy alias for backward compat with existing
 *  hermes:// links registered before the Anika rebrand. */
const DEEPLINK_SCHEMES = DEV_SERVER ? ['anika-dev', 'anika', 'hermes'] : ['anika', 'hermes']
...
function handleDeepLink(url) {
  ...
  // anika://blueprint/<key>?slot=val  -> host="blueprint", path="/<key>"
  ...
  try {
    ...
    mainWindow.focus()
    mainWindow.webContents.send('anika:deep-link', payload)
...
ipcMain.handle('anika:deep-link-ready', () => {
  ...
  if (_pendingDeepLink) {
    ...
    handleDeepLink(
      `${ANIKA_PROTOCOL}://${queued.kind}/${encodeURIComponent(queued.name)}` +
...
function registerDeepLinkProtocol() {
  try {
    if (process.defaultApp && process.argv.length >= 2) {
      ...
      app.setAsDefaultProtocolClient(ANIKA_PROTOCOL, process.execPath, [entry])
      // Backward-compat: register legacy 'hermes' scheme so existing
      // hermes:// links still open this app after the Anika rebrand.
      app.setAsDefaultProtocolClient('hermes', process.execPath, [entry])
    } else {
      app.setAsDefaultProtocolClient(ANIKA_PROTOCOL)
      // Backward-compat: register legacy 'hermes' scheme so existing
      // hermes:// links still open this app after the Anika rebrand.
      app.setAsDefaultProtocolClient('hermes')
    }

    rememberLog(`[deeplink] registered ${ANIKA_PROTOCOL}:// handler`)
  } catch (err) {
...
// second-instance argv. Without the lock a second `anika://` launch spawns a
...
if (!isPrimaryInstance) {
  ...
  // startup (shortcut registration, createWindow → startAnika), whose
...
app.whenReady().then(() => {
  ...
  // startAnika() finishes resolving the configured backend.
  const primaryProfile = primaryProfileKey()
  ...
  // Win/Linux cold start: the launching anika:// URL is in our own argv.
...
app.on('before-quit', event => {
  ...
  // quitting Anika never keeps another app's chord hostage.
