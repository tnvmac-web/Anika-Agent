...
function mkTmpHome() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'anika-bootstrap-test-'))
}
...
test('runBootstrap bails immediately when the signal is already aborted', async () => {
  ...
  const result = await runBootstrap({
    installStamp: null,
    activeRoot: '/tmp/anika-runner-test',
    sourceRepoRoot: null,
    anikaHome: '/tmp/anika-runner-test',
    logRoot: '/tmp/anika-runner-test',
...
test('installedAgentInstallScript resolves the installer in the agent checkout', () => {
  ...
  try {
    ...
    const scriptsDir = path.join(home, 'anika-agent', 'scripts')
...
test('existing checkout detection requires git metadata', () => {
  ...
  try {
    const activeRoot = path.join(home, 'anika-agent')
...
test('fresh bootstrap args include the packaged commit pin', () => {
  ...
  assert.deepEqual(
    buildPosixPinArgs({
      installStamp,
      activeRoot: '/tmp/anika-agent',
      anikaHome: '/tmp/anika'
    }),
    ['--dir', '/tmp/anika-agent', '--anika-home', '/tmp/anika', '--branch', 'main', '--commit', installStamp.commit]
  )
})
...
test('existing-checkout bootstrap args keep branch but skip the packaged commit pin', () => {
  ...
  assert.deepEqual(
    ...
    ['--dir', '/tmp/anika-agent', '--anika-home', '/tmp/anika', '--branch', 'main']
  )
})
...
test('fallback install stamps use an unpinned branch ref', () => {
  ...
  assert.deepEqual(
    buildPosixPinArgs({
      installStamp: stamp,
      activeRoot: '/tmp/anika',
      anikaHome: '/tmp/home'
    }),
    ['--dir', '/tmp/anika', '--anika-home', '/tmp/home', '--branch', 'main']
  )
})
