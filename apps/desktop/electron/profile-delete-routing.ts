// Profile-delete routing logic for the `anika:api` IPC handler.
//
...
// be rejected: spawning a fresh backend would call ensure_anika_home() and
...
/** Parse a `anika:api` request into the profile name a DELETE targets. */
 ...
 * still removing the profile and recreate its ANIKA_HOME.
 *
...
/**
 * Route the next `anika:api` request away from the primary/window backend
 ...
 * ensure_anika_home() recreates the directory the delete just removed.
 */
