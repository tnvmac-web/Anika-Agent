...
//   ANIKA_WIN_SSH_HOST   ssh alias/host of the Windows box
//   ANIKA_WIN_SSH_USER   remote user
//   ANIKA_WIN_SSH_ANIKA absolute path to the remote anika.exe under test
const liveHost = process.env.ANIKA_WIN_SSH_HOST || ''
const liveUser = process.env.ANIKA_WIN_SSH_USER || ''
const configuredAnika = process.env.ANIKA_WIN_SSH_ANIKA || ''
const ownershipId = '89abcdef0123456789abcdef01234567'

function fetchJson(url, token, path) {
  return fetch(`${url}${path}`, { headers: { 'X-Anika-Session-Token': token } }).then(async response => {
    if (!response.ok) {
...
test.skipIf(!liveHost || !liveUser || !configuredAnika)(
  ...
  async () => {
    ...
    try {
      ...
      const runtimeScript = `& '${configuredAnika.replace('anika.exe', 'python.exe')}' -m anika_cli.windows_ssh_runtime read-lock '${ownershipId}'`
      ...
      if (lock) {
        const python = configuredAnika.replace('anika.exe', 'python.exe')
        const terminate = `& '${python}' -m anika_cli.windows_ssh_runtime terminate '${lock.pid}' '${lock.creationTimeNs}' '${lock.anikaPath}' '${lock.spawnNonce}'`
        await ssh.exec(`powershell.exe -NoProfile -NonInteractive -Command "${terminate}"`)
        await ssh.exec(
          `powershell.exe -NoProfile -NonInteractive -Command "& '${python}' -m anika_cli.windows_ssh_runtime remove-lock '${ownershipId}'"`
        )
        await ssh.exec(
          `powershell.exe -NoProfile -NonInteractive -Command "& '${python}' -m anika_cli.windows_ssh_runtime remove-log '${ownershipId}' '${lock.spawnNonce}'"`
        )
      }
