/**
 * windows-anika-path.ts
 *
 * Pure, dependency-injected pieces of Windows `anika` resolution pulled out
 * of main.ts's findOnPath(), handOffWindowsBootstrapRecovery(), and
 * unwrapWindowsVenvAnikaCommand(). Each of the three functions here pins one
 ...
 *      FIRST, so an extensionless Git-Bash `anika` shim shadowed the real
 *      anika.cmd/anika.exe; the shim then failed the --version probe and
 ...
 *      and Anika launcher. Marker-only or partial runtimes must use --repair;
 *      only a runnable pair can use --update.
 *   3. resolveVenvAnikaCommand() — unwrapWindowsVenvAnikaCommand() returned
 ...
 * extensionless file (e.g. a Git-Bash shell-script shim named `anika`) must
 * not shadow `anika.cmd`/`anika.exe`. The empty entry is kept LAST so
 ...
 * venv Python interpreter and the Anika launcher that drives `anika update`.
...
export interface ResolveVenvAnikaCommandDeps {
  isWindows: boolean
  isCommandScript: (command: string) => boolean
  fileExists: (filePath: string) => boolean
  directoryExists: (filePath: string) => boolean
  canImportAnikaCli: (python: string, opts?: { env?: Record<string, string> }) => boolean
...
/**
 * If `command` is a Windows venv `anika`/`anika.exe` console-script shim
 * (i.e. `<venvRoot>/Scripts/anika(.exe)`), resolve it to the underlying
 * venv python invoked as `python -m anika_cli.main <backendArgs>` — but
 * ONLY after smoke-testing that interpreter with canImportAnikaCli(). A
 * venv whose update died mid-`pip install` still has python.exe + anika.exe
 ...
 *
 * Returns null when `command` is not a venv anika shim, the underlying
 * python doesn't exist, or the import probe fails. Otherwise returns the
 * resolved backend descriptor.
 */
export function resolveVenvAnikaCommand(
  command: string,
  backendArgs: string[],
  deps: ResolveVenvAnikaCommandDeps
): {
  label: string
  command: string
  args: string[]
  ...
  if (!/^anika(?:\.exe)?$/i.test(basename(resolved))) {
    return null
  }
  ...
  if (
    ...
    rememberLog?.(
      `Ignoring venv Anika at ${python}: runtime import probe failed (broken/partial venv); falling through to bootstrap.`
    )
  ...
  return {
    label: `existing Anika Python at ${python}`,
    command: python,
