 ...
 *
 * The staged Tauri `anika-setup.exe` has no self-update path, so every
 ...
 * checkout instead: every `anika update` refreshes the code that drives the
 ...
 * and QUITS, the script waits it out, runs `anika update`, swaps/relaunches
 * the app, and writes .anika-update-result.json. With the app gone before
 * the update starts, the ANIKA_DESKTOP_CHILD_PID reaper-exclusion dance is
 ...
 * `anika update` card — old checkouts pull the script on their next update).
 */
 ...
 *
 * The Tauri installer self-copies into ANIKA_HOME on *every* platform
 * (`anika-setup.exe` on Windows, `anika-setup` elsewhere — see
 * apps/bootstrap-installer `paths::installer_dest` and
 * `bootstrap::copy_self_to_anika_home`), so finding that binary on macOS or
 ...
 * than the hand-off protocol holds the update marker, spawns `anika update`,
...
export function resolveStagedUpdaterBinary(
  ...
  const candidate = path.join(anikaHome, 'anika-setup.exe')
 ...
 *
 * `copy_self_to_anika_home` deliberately no-ops during `--update`
 ...
 * "Another Anika update is already running (PID <itself>, started 1s ago)" —
 * the observed infinite "Install didn't finish" loop. Skipping the pre-write
 * for those binaries lets them acquire cleanly and run `anika update`, which
 ...
 * the installer is written to ANIKA_HOME at install/repair time, making mtime
