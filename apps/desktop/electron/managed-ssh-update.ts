 ...
 * wire protocol:
 *
 *   gate dials -> drain every captured scope -> detached `anika update`
...
function buildPosixManagedUpdateLaunch(target: RemoteUpdateTarget, correlationId: string): string {
  ...
  const home = validateRemoteValue(target.anikaHome, 'Anika home')
  const anikaPath = validateRemoteValue(target.anikaPath, 'launcher path')
  ...
  const launcherWord = expandRemotePath(anikaPath)

  const updateCommand =
    `env ANIKA_HOME=${homeWord} ` +
    `ANIKA_UPDATE_CORRELATION_ID=${shq(correlation)} ` +
    'ANIKA_UPDATE_ORIGIN_PROFILE=default ' +
    `ANIKA_UPDATE_ORIGIN_HOME=${homeWord} ` +
    `ANIKA_UPDATE_OUTPUT_PATH=${outputWord} ` +
...
function buildWindowsManagedUpdateLaunch(target: RemoteUpdateTarget, correlationId: string): string {
  ...
  const wrapper = [
    ...
    `$env:ANIKA_HOME=${psLiteral(home)}`,
    `$env:ANIKA_UPDATE_CORRELATION_ID=${psLiteral(correlation)}`,
    '$env:ANIKA_UPDATE_ORIGIN_PROFILE="default"',
    `$env:ANIKA_UPDATE_ORIGIN_HOME=${psLiteral(home)}`,
    `$env:ANIKA_UPDATE_OUTPUT_PATH=${psLiteral(outputPath)}`,
    ...
    `$env:ANIKA_UPDATE_WINDOWS_DETACHED=${psLiteral(correlation)}`,
    `$env:ANIKA_UPDATE_TAURI_OUTCOME_PATH=${psLiteral(statusPath)}`,
    `$env:ANIKA_UPDATE_TAURI_READY_PATH=${psLiteral(readyPath)}`,
    ...
    `& ${psLiteral(anikaPath)} update --yes *>> ${psLiteral(outputPath)}`,
    '$rc=$LASTEXITCODE',
...
marker_path=install_root/'.anika-update-in-progress'
...
async function runManagedSshUpdate<TScope extends ManagedSshScope>(
  ...
  return {
    ...
    message:
      outcome === 'updated'
        ? 'Remote Anika updated and every managed SSH profile is ready.'
        : restoreOk
