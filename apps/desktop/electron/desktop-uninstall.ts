 ...
 * the three user-facing uninstall modes to the `anika uninstall` CLI flags,
...
/**
 * Map an uninstall mode to the `python -m anika_cli.uninstall` argv (after the
 * python executable). Uses the dedicated lightweight module entrypoint (not
 * `anika_cli.main`) so it can run under a system Python OUTSIDE the venv that
...
function uninstallArgsForMode(mode) {
  ...
  return ['-m', 'anika_cli.uninstall', '--mode', mode]
}
 ...
 *
 *   macOS:   …/Anika.app/Contents/MacOS/Anika  → …/Anika.app
 *   Windows: …\Anika\Anika.exe                 → …\Anika  (install dir)
...
function resolveRemovableAppPath(execPath, platform, env: any = {}) {
  ...
  if (platform === 'darwin') {
    // …/Anika.app/Contents/MacOS/Anika → strip 3 segments to the .app
    const macOsDir = p.dirname(exe) // …/Contents/MacOS
    const contents = p.dirname(macOsDir) // …/Contents
    const appBundle = p.dirname(contents) // …/Anika.app
  ...
  if (platform === 'win32') {
    // NSIS per-user installs Anika.exe directly in the install dir.
    const dir = p.dirname(exe)

    if (/[\\/]Anika$/i.test(dir) || /[\\/]anika-desktop$/i.test(dir)) {
      return dir
    }
  ...
  // Unpacked electron-builder tree: …/linux-unpacked/anika
  const dir = p.dirname(exe)
 ...
 * being deleted); `pythonPath` is prepended to PYTHONPATH so `import anika_cli`
 ...
 */
function buildPosixCleanupScript({ desktopPid, pythonExe, pythonPath, agentRoot, uninstallArgs, appPath, anikaHome }) {
 ...
 * modes + `pythonPath`=agentRoot so `import anika_cli` resolves from source
...
function buildWindowsCleanupScript({
  ...
  // a problem, but Anika install paths don't use them.
  const q = s => `"${String(s).replace(/"/g, '')}"`

  const lines = [
    '@echo off',
    'setlocal enableextensions',
    `set "ANIKA_HOME=${String(anikaHome).replace(/"/g, '')}"`,
    `set "PID=${pid}"`
  ]
