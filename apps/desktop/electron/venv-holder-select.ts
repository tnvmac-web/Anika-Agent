 ...
 *
 * The pre-update handoff kills Anika-OWNED venv daemons (the memory plugin's
 * hindsight daemon) so the updater never races a mapped shim. External
 * holders (a user terminal running `anika`, unrelated scripts) must NOT be
...
/**
 * True when a process is a Anika-owned venv daemon: its exe lives under
 ...
 * spawns DETACHED — it outlives Anika and holds venv shims mapped).
 */
export function isAnikaOwnedVenvDaemon(
