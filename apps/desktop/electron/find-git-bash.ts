 ...
 *   1. ANIKA_GIT_BASH_PATH env var override
 *   2. PortableGit under %LOCALAPPDATA%\anika\git\ (install.ps1)
...
export function findGitBash(opts: GitBashOptions): string | null {
  ...
  // Respect ANIKA_GIT_BASH_PATH if set (mirrors tools/environments/local.py:_find_bash).
  const gitBashPath = env.ANIKA_GIT_BASH_PATH
  ...
  if (localAppData) {
    candidates.push(joinWin(localAppData, 'anika', 'git', 'bin', 'bash.exe'))
    candidates.push(joinWin(localAppData, 'anika', 'git', 'usr', 'bin', 'bash.exe'))
  }
