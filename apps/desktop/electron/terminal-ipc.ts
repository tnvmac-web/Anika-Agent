...
// registry, and the anika:terminal:* IPC surface. Extracted from main.ts; the
...
export function registerTerminalIpc({
  ...
  function terminalShellCommand() {
    // ANIKA_DESKTOP_SHELL is the cross-platform escape hatch (a path or a bare
    ...
    const override = (process.env.ANIKA_DESKTOP_SHELL || (isWindows ? '' : process.env.SHELL) || '').trim()
  ...
  function terminalShellEnv() {
    ...
    // /TERM=dumb; some terminals set COLORFGBG which would flip Anika' TUI into
    ...
    env.TERM_PROGRAM = 'Anika'
    env.TERM_PROGRAM_VERSION = app.getVersion()

    // Let a anika/--tui launched in this pane know it's embedded in the desktop
    // GUI (build_environment_hints surfaces this). Distinct from ANIKA_DESKTOP,
    // which marks the agent *backend* and gates cron/gateway behavior.
    env.ANIKA_DESKTOP_TERMINAL = '1'

    return env
  }

  function terminalChannel(id, suffix) {
    return `anika:terminal:${id}:${suffix}`
  }
  ...
  ipcMain.handle('anika:terminal:start', async (event, payload = {}) => {
  ...
  ipcMain.handle('anika:terminal:attach', (event, id) => {
  ...
  ipcMain.handle('anika:terminal:write', (_event, id, data) => {
    const sessionInfo = terminalSessions.get(String(id || ''))
  ...
  ipcMain.handle('anika:terminal:resize', (_event, id, size = {}) => {
    const sessionInfo = terminalSessions.get(String(id || ''))
  ...
  ipcMain.handle('anika:terminal:cwd', async (_event, id) => {
    const sessionInfo = terminalSessions.get(String(id || ''))
  ...
  ipcMain.handle('anika:terminal:dispose', (_event, id) => disposeTerminalSession(String(id || '')))
