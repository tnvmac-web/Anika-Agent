export type ParentWatchdogEnv = {
  ANIKA_PARENT_PID: string
  ANIKA_PARENT_START_MARKER?: string
  ANIKA_PARENT_NONCE?: string
  /** Spawn tag consumed by anika_cli.process_identity (`v1:<install>:<purpose>:<spawner_pid>:<spawner_create_s>`).
...
export function parentWatchdogEnv(pid: number, startMarker: string | null, nonce: string): ParentWatchdogEnv {
  ...
  const env: ParentWatchdogEnv = { ANIKA_PARENT_PID: String(pid) }
  env.ANIKA_SPAWN = spawnTag(pid, startMarker)
  ...
  env.ANIKA_PARENT_START_MARKER = startMarker
  env.ANIKA_PARENT_NONCE = nonce

  return env
}

/** Build the `ANIKA_SPAWN` tag mirrored by anika_cli/process_identity.py.
