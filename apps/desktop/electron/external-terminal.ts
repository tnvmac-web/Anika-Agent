// Launching the Anika TUI in the user's OWN terminal emulator.
//
...
// `anika --tui --resume <id>` there. Two problems have to be solved for that
// to work anywhere:
//
//  1. WHAT to run. The desktop's Anika runtime is often a venv Python invoked
//     as `python -m anika_cli.main`, not a `anika` on PATH — so the command
 ...
 * headless child) is the wrong answer for an interactive terminal. The Anika
