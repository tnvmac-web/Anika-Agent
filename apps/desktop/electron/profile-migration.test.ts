 ...
 *      top of startAnika() in main.ts, before primaryProfileKey() is read); the
...
function baseDeps(overrides: Record<string, unknown> = {}) {
  ...
  return {
    legacyActivePath: '/home/u/.anika/active_profile',
    anikaHome: '/home/u/.anika',
    profilesRoot: '/home/u/.anika/profiles',
...
test('findRunningGatewayProfiles returns [] when no pid files exist', () => {
  const fs = makeFs({ '/home/u/.anika/profiles/coder': { dir: true } })
  assert.deepEqual(
    findRunningGatewayProfiles('/home/u/.anika/profiles', ['coder'], {
      ...fs,
      isAnikaProcess: () => true
    }),
    []
...
test('findRunningGatewayProfiles drops stale (non-anika) recycled PIDs', () => {
  // Two profiles have pid files, but only coder's pid is a live anika process.
  // The recycled PID at 5678 belongs to an unrelated process (e.g. Chrome).
  const fs = makeFs({
    '/home/u/.anika/profiles/coder/gateway.pid': { content: '{"pid":1234}' },
    '/home/u/.anika/profiles/writer/gateway.pid': { content: '{"pid":5678}' }
  })

  const deps = {
    ...fs,
    isAnikaProcess: (pid: number) => pid === 1234
  }

  assert.deepEqual(findRunningGatewayProfiles('/home/u/.anika/profiles', ['coder', 'writer'], deps), ['coder'])
})

test('findRunningGatewayProfiles tolerates malformed pid files', () => {
  const fs = makeFs({
    '/home/u/.anika/profiles/coder/gateway.pid': { content: '{not json' }
  })
...
test('findRunningGatewayProfiles drops non-integer and non-positive PIDs', () => {
  const fs = makeFs({
    '/home/u/.anika/profiles/coder/gateway.pid': { content: '{"pid":-1}' },
    '/home/u/.anika/profiles/writer/gateway.pid': { content: '{"pid":1.5}' },
    '/home/u/.anika/profiles/extra/gateway.pid': { content: '{"pid":0}' }
  })

  assert.deepEqual(
    findRunningGatewayProfiles('/home/u/.anika/profiles', ['coder', 'writer', 'extra'], {
      ...fs,
      isAnikaProcess: () => true
    }),
    []
...
test('findRunningGatewayProfiles preserves order of allProfiles', () => {
  const fs = makeFs({
    '/home/u/.anika/profiles/coder/gateway.pid': { content: '{"pid":1}' },
    '/home/u/.anika/profiles/writer/gateway.pid': { content: '{"pid":2}' }
  })

  const deps = { ...fs, isAnikaProcess: () => true }
  assert.deepEqual(findRunningGatewayProfiles('/home/u/.anika/profiles', ['coder', 'writer'], deps), [
    'coder',
    'writer'
  ])
})
...
test('listProfileDirs includes default and any name passing the regex', () => {
  const fs = makeFs({
    '/home/u/.anika/profiles/default': { dir: true },
    '/home/u/.anika/profiles/coder': { dir: true },
    '/home/u/.anika/profiles/writer': { dir: true }
  })
...
test('listProfileDirs skips files (not directories) and invalid names', () => {
  const fs = makeFs({
    '/home/u/.anika/profiles/default': { dir: true },
    '/home/u/.anika/profiles/some-file.txt': { dir: false },
    '/home/u/.anika/profiles/UPPERCASE': { dir: true } // regex rejects
  })
...
test('decideMigration suppresses write when best is default (single-profile fallback)', () => {
  ...
  // default-primary and we leave it alone. Default's DB is $ANIKA_HOME/state.db,
  // not profiles/default/state.db.
  const deps = baseDeps()

  const d = decideMigration(null, [], ['default', 'coder'], deps, p => (p.endsWith('/.anika/state.db') ? 99 : 50))
...
test('migrateActiveProfileIfMissing is a no-op when a user-selected preference file exists', () => {
  ...
  const fs = makeFs({
    ...
    '/home/u/.anika/profiles/coder': { dir: true },
    '/home/u/.anika/profiles/writer': { dir: true },
    '/home/u/.anika/profiles/writer/state.db': { size: 400 * 1024 * 1024, mtime: NOW - 86_400_000 }
  })
...
test('migrateActiveProfileIfMissing writes legacy choice with no _migrated flag', () => {
  ...
  const fs = makeFs({
    '/home/u/.anika/active_profile': { content: 'coder' },
    '/home/u/.anika/profiles/coder': { dir: true },
    '/home/u/.anika/profiles/writer': { dir: true }
  })
...
test('migrateActiveProfileIfMissing writes heuristic choice with _migrated=true', () => {
  ...
  const fs = makeFs({
    '/home/u/.anika/profiles/coder/state.db': { size: 50 * 1024 * 1024, mtime: NOW - 86_400_000 },
    '/home/u/.anika/profiles/writer/state.db': { size: 200 * 1024 * 1024, mtime: NOW - 86_400_000 }
  })
...
test('migrateActiveProfileIfMissing is a no-op for single-profile (default-only) installs', () => {
  ...
  // Production default DB is ~/.anika/state.db, not profiles/default/state.db.
  let written: unknown = null

  const fs = makeFs({
    '/home/u/.anika/state.db': { size: 10 * 1024 * 1024, mtime: NOW - 86_400_000 }
  })
...
test('migrateActiveProfileIfMissing prefers a single running gateway over heuristics', () => {
  ...
  const fs = makeFs({
    '/home/u/.anika/profiles/coder/gateway.pid': { content: '{"pid":42}' },
    '/home/u/.anika/profiles/writer/state.db': { size: 500 * 1024 * 1024, mtime: NOW - 86_400_000 }
  })

  const deps = baseDeps({
    ...fs,
    isAnikaProcess: (pid: number) => pid === 42,
    writeJson: (_p: string, payload: unknown) => {
      written = payload
    }
  })
...
// ---------------------------------------------------------------------------
// Production layout: default is ~/.anika, not ~/.anika/profiles/default
// ---------------------------------------------------------------------------

test('profileStateDbPath puts default at anikaHome, named under profilesRoot', () => {
  assert.equal(profileStateDbPath('default', '/home/u/.anika', '/home/u/.anika/profiles'), '/home/u/.anika/state.db')
  assert.equal(
    profileStateDbPath('conduit', '/home/u/.anika', '/home/u/.anika/profiles'),
    '/home/u/.anika/profiles/conduit/state.db'
  )
})

test('profileGatewayPidPath puts default at anikaHome', () => {
  assert.equal(
    profileGatewayPidPath('default', '/home/u/.anika', '/home/u/.anika/profiles'),
    '/home/u/.anika/gateway.pid'
  )
  assert.equal(
    profileGatewayPidPath('coder', '/home/u/.anika', '/home/u/.anika/profiles'),
    '/home/u/.anika/profiles/coder/gateway.pid'
  )
})
...
test('findRunningGatewayProfiles sees default gateway.pid at anikaHome', () => {
  const fs = makeFs({
    '/home/u/.anika/gateway.pid': { content: '{"pid":99}' },
    '/home/u/.anika/profiles/coder/gateway.pid': { content: '{"pid":11}' }
  })

  assert.deepEqual(
    findRunningGatewayProfiles('/home/u/.anika/profiles', ['default', 'coder'], {
      ...fs,
      anikaHome: '/home/u/.anika',
      isAnikaProcess: pid => pid === 99
    }),
    ['default']
  )
})
...
test('migrateActiveProfileIfMissing does not pin a tiny named profile over a large default DB', () => {
  ...
  // ~/.anika/profiles/<name>, never scored ~/.anika/state.db, and wrote
  // { profile: named, _migrated: true }.
  let written: unknown = null

  const fs = makeFs({
    '/home/u/.anika/profiles/conduit': { dir: true },
    '/home/u/.anika/state.db': { size: 409 * 1024 * 1024, mtime: NOW - 86_400_000 },
    '/home/u/.anika/profiles/conduit/state.db': { size: 2 * 1024 * 1024, mtime: NOW - 60_000 }
  })
...
test('migrateActiveProfileIfMissing still pins a named profile that actually beats default', () => {
  ...
  const fs = makeFs({
    '/home/u/.anika/profiles/work': { dir: true },
    '/home/u/.anika/state.db': { size: 5 * 1024 * 1024, mtime: NOW - 86_400_000 },
    '/home/u/.anika/profiles/work/state.db': { size: 200 * 1024 * 1024, mtime: NOW - 86_400_000 }
  })
...
test('migrateActiveProfileIfMissing does not pin default when only default gateway is running', () => {
  ...
  const fs = makeFs({
    '/home/u/.anika/gateway.pid': { content: '{"pid":7}' },
    '/home/u/.anika/state.db': { size: 10 * 1024 * 1024, mtime: NOW - 86_400_000 }
  })

  const deps = baseDeps({
    ...fs,
    isAnikaProcess: (pid: number) => pid === 7,
    writeJson: (_p: string, payload: unknown) => {
      written = payload
    }
  })
