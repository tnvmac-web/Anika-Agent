...
function ownershipEntry(overrides: Partial<BackendIdentity> = {}) {
  return { command: 'anika serve --port 0', ...identity(overrides) }
}
...
test('backend identity check matches only serve and dashboard invocation shapes', () => {
  assert.equal(backendCommandMatches('/venv/bin/anika serve --port 0'), true)
  assert.equal(backendCommandMatches('python -m anika_cli.main dashboard --no-open'), true)
  assert.equal(backendCommandMatches('/venv/bin/anika --profile work serve --port 0'), true)
  assert.equal(backendCommandMatches('"C:\\Anika Runtime\\anika.exe" dashboard --no-open'), true)
  assert.equal(backendCommandMatches('anika chat --query serve'), false)
