...
export function registerPetOverlayIpc({
  ...
  ipcMain.handle('anika:pet-overlay:open', async (_event, request) => {
  ...
  ipcMain.handle('anika:pet-overlay:close', async () => {
    closePetOverlay()
  ...
  ipcMain.on('anika:pet-overlay:set-bounds', (_event, bounds) => {
  ...
  ipcMain.on('anika:pet-overlay:ignore-mouse', (_event, ignore) => {
    const petOverlayWindow = getPetOverlayWindow()
  ...
  ipcMain.on('anika:pet-overlay:set-focusable', (_event, focusable) => {
    const petOverlayWindow = getPetOverlayWindow()
  ...
  ipcMain.on('anika:pet-overlay:state', (_event, payload) => {
    const petOverlayWindow = getPetOverlayWindow()

    if (petOverlayWindow && !petOverlayWindow.isDestroyed()) {
      petOverlayWindow.webContents.send('anika:pet-overlay:state', payload)
    }
  })
  // Overlay → main renderer: control messages (pop back in, composer submit).
  ipcMain.on('anika:pet-overlay:control', (_event, payload) => {
    const mainWindow = getMainWindow()
    ...
    mainWindow.webContents.send('anika:pet-overlay:control', payload)
  })
}
