// Unit tests for the pure Windows `anika` resolution helpers extracted from
// main.ts's findOnPath(), handOffWindowsBootstrapRecovery(), and
// unwrapWindowsVenvAnikaCommand(). These pin the two Windows resolution bugs
// that caused desktop reinstall loops:
//   1. buildPathExtCandidates() — PATHEXT extensions must be tried BEFORE the
//      empty extension, or an extensionless Git-Bash `anika` shim shadows
//      the real anika.cmd/anika.exe.
...
//   3. resolveVenvAnikaCommand() — must probe the venv python via
//      canImportAnikaCli() before trusting it, or a broken venv gets
...
test('chooseUpdaterArgs: gentle --update when both updater runtime files exist', () => {
  assert.deepEqual(chooseUpdaterArgs({ hasBootstrapMarker: true, hasVenvAnika: true, hasVenvPython: true }, 'main'), [
    '--update',
    '--branch',
    'main'
  ])
...
test('chooseUpdaterArgs: marker-only install uses --repair when the venv is gone', () => {
  assert.deepEqual(
    chooseUpdaterArgs({ hasBootstrapMarker: true, hasVenvAnika: false, hasVenvPython: false }, 'main'),
    ['--repair', '--branch', 'main']
  )
})

test('chooseUpdaterArgs: partial updater runtimes use --repair', () => {
  assert.deepEqual(chooseUpdaterArgs({ hasBootstrapMarker: true, hasVenvAnika: false, hasVenvPython: true }, 'main'), [
    '--repair',
    '--branch',
    'main'
  ])
  assert.deepEqual(chooseUpdaterArgs({ hasBootstrapMarker: true, hasVenvAnika: true, hasVenvPython: false }, 'main'), [
    '--repair',
    '--branch',
    'main'
  ])
...
test('chooseUpdaterArgs: passes the branch through unchanged in both modes', () => {
  assert.deepEqual(
    chooseUpdaterArgs({ hasBootstrapMarker: false, hasVenvAnika: true, hasVenvPython: true }, 'release/1.2'),
    ['--update', '--branch', 'release/1.2']
  )
  assert.deepEqual(
    chooseUpdaterArgs({ hasBootstrapMarker: false, hasVenvAnika: false, hasVenvPython: false }, 'release/1.2'),
    ['--repair', '--branch', 'release/1.2']
  )
})

function makeDeps(overrides: Partial<Parameters<typeof resolveVenvAnikaCommand>[2]> = {}) {
  return {
    isWindows: true,
    isCommandScript: () => false,
    fileExists: () => true,
    directoryExists: () => false,
    canImportAnikaCli: () => true,
    getVenvPython: (venvRoot: string) => `${venvRoot}/Scripts/python.exe`,
    getVenvSitePackagesEntries: () => [],
    buildDesktopBackendEnv: () => ({ FAKE_ENV: '1' }),
    anikaHome: '/fake/anika-home',
...
test('resolveVenvAnikaCommand: returns null off Windows', () => {
  const deps = makeDeps({ isWindows: false })

  assert.equal(resolveVenvAnikaCommand('/root/venv/Scripts/anika.exe', [], deps), null)
})

test('resolveVenvAnikaCommand: returns null for a .cmd/.bat script command', () => {
  const deps = makeDeps({ isCommandScript: () => true })

  assert.equal(resolveVenvAnikaCommand('/root/venv/Scripts/anika.cmd', [], deps), null)
})

test('resolveVenvAnikaCommand: returns null when the basename is not anika/anika.exe', () => {
  const deps = makeDeps()

  assert.equal(resolveVenvAnikaCommand('/root/venv/Scripts/python.exe', [], deps), null)
})

test('resolveVenvAnikaCommand: returns null when the parent dir is not Scripts', () => {
  const deps = makeDeps()

  assert.equal(resolveVenvAnikaCommand('/root/venv/bin/anika.exe', [], deps), null)
})

test('resolveVenvAnikaCommand: returns null when the venv python does not exist on disk', () => {
  const deps = makeDeps({ fileExists: () => false })

  assert.equal(resolveVenvAnikaCommand('/root/venv/Scripts/anika.exe', [], deps), null)
})

test('resolveVenvAnikaCommand: probes the venv python before trusting it (returns null on failed probe)', () => {
  let probed = false

  const deps = makeDeps({
    canImportAnikaCli: (python: string) => {
      probed = true
  ...
  const result = resolveVenvAnikaCommand('/root/venv/Scripts/anika.exe', ['serve'], deps)
...
test('resolveVenvAnikaCommand: returns the resolved python backend descriptor when the probe passes', () => {
  const deps = makeDeps()
  const result = resolveVenvAnikaCommand('/root/venv/Scripts/anika.exe', ['serve', '--port', '0'], deps)
  ...
  assert.deepEqual(result.args, ['-m', 'anika_cli.main', 'serve', '--port', '0'])
...
test('resolveVenvAnikaCommand: is case-insensitive on anika.exe and the Scripts dir name', () => {
  const deps = makeDeps()

  assert.ok(resolveVenvAnikaCommand('/root/venv/Scripts/ANIKA.EXE', [], deps))
  assert.ok(resolveVenvAnikaCommand('/root/venv/SCRIPTS/anika.exe', [], deps))
})
