...
function createTempGitRepo() {
  const cwd = fs.mkdtempSync(path.join(os.tmpdir(), 'anika-update-count-'))
  ...
  try {
    ...
    git('config', 'user.name', 'Anika Test')
    git('config', 'user.email', 'anika@example.invalid')
...
test('compareApiUrl builds the GitHub compare URL for HTTPS origins', () => {
  assert.equal(
    compareApiUrl({
      currentSha: SHA_A,
      originUrl: 'https://github.com/__NEW_ORG__/anika-agent.git',
      targetSha: SHA_B
    }),
    `https://api.github.com/repos/__NEW_ORG__/anika-agent/compare/${SHA_A}...${SHA_B}`
  )
})
...
test('compareApiUrl handles SSH origin forms', () => {
  for (const originUrl of [
    ...
    'ssh://git@github.com/__NEW_ORG__/anika-agent.git',
