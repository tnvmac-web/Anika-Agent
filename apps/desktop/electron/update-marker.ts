...
export function markerPath(anikaHome) {
  return path.join(anikaHome, '.anika-update-in-progress')
}
...
export function readLiveUpdateMarker(
  ...
  const file = markerPath(anikaHome)
  let raw
 ...
 *
 * The Tauri-based anika-setup.exe takes several seconds to initialise its
...
export function writeUpdateMarker(
  ...
  const file = markerPath(anikaHome)
  const nowMs = now()
  const owner = readLiveUpdateMarker(anikaHome, { kill, maxAgeMs, now: () => nowMs })
 ...
 * Anika to exit…") clobbers that still-running updater's claim: the
...
export function updateHandoffConflict(
  ...
  const owner = readLiveUpdateMarker(anikaHome, opts)
