 ...
 *
 * The Python side (anika_cli/plugin_compat.py) statically scans the user's enabled external plugins on
 * every CLI/gateway/TUI start and writes ANIKA_HOME/.plugin-compat-report.json when any plugin imports a
...
export function readReport(anikaHome: string): PluginCompatReport | null {
  try {
    const raw = fs.readFileSync(path.join(anikaHome, REPORT_FILE), 'utf8')
...
export function pendingNotice(anikaHome: string, userData: string): PendingNotice | null {
  const report = readReport(anikaHome)
