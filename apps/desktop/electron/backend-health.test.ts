...
test('uses lightweight /api/health for current backends', async () => {
  ...
  await waitForAnikaReady('http://127.0.0.1:9000/', {
    token: 'secret-token',
...
test('does not fall back to heavyweight /api/status for transient health failures', async () => {
  ...
  await assert.rejects(
    waitForAnikaReady('http://127.0.0.1:9000', {
      fetchPublicJson: async url => {
        calls.push(['public', url])
        throw new Error('Timed out connecting to Anika backend after 15000ms')
      },
      fetchJson: async url => {
        calls.push(['token', url])
      },
...
test('recognizes missing-route shapes only', () => {
  ...
  assert.equal(
    isMissingHealthEndpointError(
      new Error('Expected JSON from /api/health but got HTML. The endpoint is likely missing on the Anika backend.')
    ),
    true
  )
  assert.equal(isMissingHealthEndpointError(new Error('Timed out connecting to Anika backend after 15000ms')), false)
...
test('anonymous gate-shaped 401 falls back to /api/status (backend predates /api/health)', async () => {
  ...
  await waitForAnikaReady('http://192.168.1.132:9119', {
    token: null,
    fetchPublicJson: async url => {
      calls.push(['public', url])
...
test('a credentialed 401 fails fast for reauth instead of reporting a dead session ready', async () => {
  ...
  await assert.rejects(
    waitForAnikaReady('https://gateway.example', {
      token: 'session-token',
      fetchPublicJson: async () => {
...
test('unsigned OAuth is a terminal reauth failure; needsOauthLogin alone is not', () => {
  // The unsigned-in throw must set isReauthRequired so startAnika latches.
  ...
  assert.equal(isReauthRequiredError(new Error('Could not reach the remote Anika gateway')), false)
})
...
test('a credentialed probe still uses the 404 fallback for a genuinely missing route', async () => {
  ...
  await waitForAnikaReady('https://gateway.example', {
    token: 'session-token',
    fetchPublicJson: async () => {
      throw new Error('public probe must not be used when credentialed')
    },
...
test('isNousCloudAgentUrl detects cloud agent hosts', () => {
  // Positive cases
  assert.equal(isNousCloudAgentUrl('https://ares-3009.agents.__NEW_DOMAIN__'), true)
  assert.equal(isNousCloudAgentUrl('https://ares-3009.agents.__NEW_DOMAIN__/api/health'), true)
  assert.equal(isNousCloudAgentUrl('http://test.agents.__NEW_DOMAIN__'), true)
  ...
  assert.equal(isNousCloudAgentUrl('https://__NEW_DOMAIN__'), false)
  assert.equal(isNousCloudAgentUrl('not-a-url'), false)
})

test('waitForAnikaReady surfaces actionable error for cloud agent 503', async () => {
  let attempts = 0
  const currentTime = { value: 0 }

  try {
    await waitForAnikaReady('https://ares-3009.agents.__NEW_DOMAIN__', {
      fetchPublicJson: async () => {
        attempts++
        // Always return 503
    ...
    assert.ok(error.message.includes('__NEW_ORG__ Cloud agent'), `unexpected message: ${error.message}`)
    assert.ok(error.message.includes('503'), `should mention status code: ${error.message}`)
    assert.ok(error.message.includes('portal.__NEW_DOMAIN__'), `should mention portal: ${error.message}`)
    assert.ok(error.message.includes('discord.gg/__NEW_ORG__'), `should mention Discord: ${error.message}`)
...
test('waitForAnikaReady does not cloud-wrap non-cloud 503 errors', async () => {
  const currentTime = { value: 0 }
...
test('makeNousCloudBackendDownError produces the Cloud shape and preserves cause', () => {
  const err = new Error('upstream unavailable') as any
  err.statusCode = 503
  const result = makeNousCloudBackendDownError('https://ares-3009.agents.__NEW_DOMAIN__', err)
  assert.ok(result)
  ...
  assert.ok(result?.message.includes('__NEW_ORG__ Cloud agent ares-3009.agents.__NEW_DOMAIN__ is down'))
})

test('makeNousCloudBackendDownError returns null for a Cloud 401 (routes to reauth)', () => {
  const err = new Error('Unauthorized') as any
  err.statusCode = 401
  assert.equal(makeNousCloudBackendDownError('https://ares-3009.agents.__NEW_DOMAIN__', err), null)
})
...
test('makeNousCloudBackendDownError preserves legacy string-prefix compatibility', () => {
  const result = makeNousCloudBackendDownError(
    'https://ares-3009.agents.__NEW_DOMAIN__',
