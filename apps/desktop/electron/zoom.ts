...
export const ZOOM_STORAGE_KEY = 'anika:desktop:zoomLevel'
 ...
 * but forgot to emit 'anika:zoom:changed', leaving the control stuck at 100%.
...
export function applyZoomLevel(webContents, level) {
  ...
  webContents.send('anika:zoom:changed', { level: clamped, percent: zoomLevelToPercent(clamped) })
 ...
 * only learns of changes through `anika:zoom:changed`, which never fired —
