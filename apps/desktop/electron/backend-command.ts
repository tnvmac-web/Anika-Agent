// Backend subcommand routing for the desktop-managed Anika process.
//
// The desktop app launches its own headless backend via `anika serve` — it
...
// app hasn't updated yet, or an older `anika` resolved from PATH) only knows
 ...
 * @param {string} [profile] optional Anika profile to pin via `--profile`.
 */
 ...
 * `-m anika_cli.main` and any `--profile <name>`). Returns a copy; if there is
...
/**
 * True when a runtime's `anika_cli/subcommands/dashboard.py` source registers
