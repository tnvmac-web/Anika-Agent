 ...
 * runtime, remote gateways, Anika Cloud instances, SSH hosts) that are all
 ...
 * apps/shared/src/backend-scope.ts (`@anika/shared`) — tsconfig project
...
export async function reuseMatchingPrimarySshBackend({
  ...
  if (
    ...
    String(source.remoteAnikaPath || '').trim() !== String(activeSsh.remoteAnikaPath || '').trim() ||
...
/** Turn `ls ~/.anika/profiles` output into roster names. Always includes
...
/**
 * Whether "Update all instances" may drive this connection. Anika Cloud
 * instances are platform-managed — we never run `anika update` against them.
...
export function normalizeConnectionInput(input: ConnectionInput, registry: ConnectionRegistry): RegistryConnection {
  ...
  if (kind === 'ssh') {
    const ssh = normalizeSshConfig({
      ...
      remoteAnikaPath: input.remoteAnikaPath,
  ...
  if (kind === 'remote' || kind === 'cloud') {
    ...
    // transport-/Anika-managed names; an empty result stores nothing.
 ...
 * `remoteAnikaPath`/`remoteProfile`. Only fields the payload explicitly
...
export function migrateV1ToRegistry(v1: unknown): ConnectionRegistry {
  ...
  const addRemoteLike = (block: Record<string, any>, kind: 'cloud' | 'remote'): null | RegistryConnection => {
    ...
    const label = uniqueLabel(
      hostLabelFromBaseUrl(url) || (kind === 'cloud' ? 'Anika Cloud' : 'Remote gateway'),
