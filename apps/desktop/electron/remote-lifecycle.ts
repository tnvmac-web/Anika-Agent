 ...
 * remote-lifecycle.ts
 *
 * Pure, electron-free remote Anika dashboard lifecycle over SSH for Desktop
 ...
 *
 *   - locate the Anika install on the remote (login-shell probe),
 ...
 *     `ANIKA_DASHBOARD_READY port=<n>` readiness line,
 ...
 * the real SshConnection, fetch, adoptServedDashboardToken, and waitForAnika in.
 *
 * The minted ANIKA_DASHBOARD_SESSION_TOKEN is the SPAWN credential. After
...
const READY_RE = /^ANIKA_(?:BACKEND|DASHBOARD)_READY port=(\d+)/m
const REMOTE_LOCK_DIR = '~/.anika/desktop-ssh'
...
// Resolve the remote anika executable. An EXPLICIT path is honored strictly
...
async function locateAnika(ssh, remoteAnikaPath) {
  const resolveLauncher = async (candidate: string) => {
    // Return the candidate path directly. The anika binary or wrapper script
    ...
    //     the Anika version, and
    //   - capability probing: `<python> serve --help` failed entirely.
    // See https://github.com/__NEW_ORG__/anika-agent/issues/74411
    return candidate
  }
  ...
  if (remoteAnikaPath) {
    if (await isExecutable(remoteAnikaPath)) {
      return resolveLauncher(remoteAnikaPath)
    }

    const err: any = new Error(
      `The Anika path you set is not an executable on the remote host: "${remoteAnikaPath}". ` +
        'Check the path (it must be the full path to the `anika` binary on the remote, e.g. ' +
        '~/anika-agent/.venv/bin/anika), or clear it to auto-detect.'
    )

    err.kind = 'anika-not-found'
    throw err
  }
  ...
  try {
    const found = (await ssh.exec(`bash -lc ${shq('command -v anika')}`)).trim()
  ...
  candidates.push('~/.local/bin/anika')
  candidates.push('/usr/local/bin/anika')
  candidates.push('~/.anika/anika-agent/venv/bin/anika')
  ...
  const err: any = new Error(
    'Anika is not installed on the remote host (could not find a `anika` executable). ' +
      'Install it on the remote with:  curl -fsSL https://anika-agent.__NEW_DOMAIN__/install.sh | sh  ' +
      '— or set the Anika path explicitly in the SSH connection settings.'
  )
...
// Probe the resolved binary's version string (first line of `<anika> --version`,
// e.g. "Anika Agent v0.18.2 ..."), or '' on failure. Surfaces WHICH anika a
// connection uses, so a stale/unexpected install is visible.
async function probeAnikaVersion(ssh, anikaPath) {
  try {
    const out = (await ssh.exec(`${expandRemotePath(anikaPath)} --version 2>&1`)).trim()
...
async function probeRemotePlatform(ssh) {
  ...
  if (!SUPPORTED_REMOTE_OS.has(osName)) {
    const err: any = new Error(
      `Unsupported remote platform "${osName || 'unknown'}". Anika Desktop SSH mode supports Linux, macOS, and Windows remote hosts.`
    )
...
// The ANIKA_HOME the remote dashboard will use (explicit env wins, else
// ~/.anika). Recorded in the lockfile so a future reuse can tell it's the same
// state store; best-effort.
async function probeRemoteAnikaHome(ssh) {
  try {
    const out = (await ssh.exec('echo "${ANIKA_HOME:-$HOME/.anika}"')).trim().split('\n').pop()

    return out || '~/.anika'
  } catch (cause) {
    const error: any = new Error('Could not resolve the remote Anika home.')
...
marker=home/'.anika-update-in-progress'
try:
 ...
 * bytes; it never imports or executes code from the changing Anika checkout.
 ...
 */
async function assertRemoteInstallUpdateClear(ssh, anikaHome) {
  const home = assertSafeRemoteHome(anikaHome)
  let observation = ''
  ...
  try {
    ...
    const error: any = new Error('Could not prove that the remote Anika install is clear for SSH startup.')
  ...
  const error: any = new Error(
    live
      ? `Remote Anika update process ${live[1]} is still running; SSH startup is paused.`
      : 'The remote Anika update marker is unreadable or malformed; refusing SSH startup.'
  )
...
async function listRemoteAnikaProfiles(ssh) {
  const home = assertSafeRemoteHome(await probeRemoteAnikaHome(ssh))
  ...
  try {
    ...
    const error: any = new Error('Could not list remote Anika profiles.')
    error.kind = 'transient-transport-error'
    error.cause = cause
    throw error
  }
...
function assertSafeRemoteHome(home) {
  ...
  if (!/^(\/|~\/)[A-Za-z0-9._/+-]+$/.test(value) || value.includes('..')) {
    const error: any = new Error('Unsafe remote Anika home.')
...
async function readLockfile(ssh, ownershipId) {
  ...
  for (const field of ['profile', 'anikaPath', 'anikaHome', 'logPath', 'startedAt']) {
...
async function pidIsOurDashboard(
  ...
  if (!pid || !/^[0-9a-f]{16}$/.test(String(spawnNonce || '')) || !anikaPath) {
    return false
  }
  ...
  try {
    const script =
      ...
      `pid=${Number(pid)}\n` +
      `expected=os.path.expanduser(${shq(anikaPath)})\n` +
      // The installer-facing launcher is intentionally preserved for invocation
      // (#74411), but it may `exec python <install-dir>/anika`, leaving neither
      // launcher nor ANIKA_HOME-derived entrypoint in argv. The ownership-scoped
      // token path + random nonce + exact profile below are the alternative proof.
      `anika_home=os.path.expanduser(${shq(anikaHome)}) if ${shq(anikaHome)} else ""\n` +
      'expected_entries={expected}\n' +
      'if anika_home:\n' +
      ' expected_entries.add(os.path.join(anika_home,"anika-agent","venv","bin","anika"))\n' +
...
function buildOwnedStaleTerminationCommand(lock, ownershipId) {
  ...
  const expectedPath = expandRemotePath(lock.anikaPath)
  const expectedHome = lock.anikaHome ? expandRemotePath(lock.anikaHome) : "''"
  ...
  const executableMatch = lock.anikaHome
...
function lockMatchesManagedUpdateScope(lock, expected) {
  return Boolean(
    ...
    lock.anikaPath === expected.anikaPath &&
    lock.anikaHome === expected.anikaHome
  )
}
...
expected_path=os.path.expanduser(${py(lock.anikaPath)})
anika_home=os.path.expanduser(${py(lock.anikaHome)})
expected_entries={expected_path,os.path.join(anika_home,"anika-agent","venv","bin","anika")}
...
// execing Anika. mutexPath is expandRemotePath() output — a complete shell
...
result=None
try:
 result=subprocess.run(["sh","-c",payload,"anika-update-mutex",str(fd)],pass_fds=(fd,),check=False)
finally:
 os.close(fd)
...
function buildSpawnCommand(anikaPath, profile, opts: any = {}) {
  const anika = expandRemotePath(anikaPath)
  ...
  const marker = expandRemotePath(`${remoteInstallRoot(opts.anikaHome || '~/.anika')}/.anika-update-in-progress`)

  const updateMutex = expandRemotePath(
    `${remoteInstallRoot(opts.anikaHome || '~/.anika')}/.anika-update-in-progress.mutex`
  )
  ...
  const dashCmd =
    ...
    `exec env ANIKA_DESKTOP=1 ${anika} ${profileArgs}${subCmd}`

  const detachedShell = `eval "exec $1>&-"; ${dashCmd} </dev/null >> ${logPath} 2>&1 & echo $!`
  const detachedSpawn = `child=$("$(command -v setsid || echo nohup)" sh -c ${shq(detachedShell)} anika-update-child "$1" & echo $!)`
...
async function remoteSupportsSshOwnership(ssh, anikaPath) {
  const anika = expandRemotePath(anikaPath)

  const out = await ssh.exec(
    `help="$(${anika} serve --help 2>&1)"; ` +
...
async function spawnRemoteDashboard(
  ssh,
  { anikaPath, profile, token, ownershipId, anikaHome = '~/.anika', assertInstallClear = async () => {} }
) {
  if (!(await remoteSupportsSshOwnership(ssh, anikaPath))) {
    const err: any = new Error(
      'The remote Anika install does not support --ssh-session-token-file and --ssh-owner-nonce. ' +
        'Update Anika on the remote host to continue using Desktop SSH mode.'
    )
  ...
  try {
    ...
    out = await ssh.exec(
      buildSpawnCommand(anikaPath, profile, {
        spawnNonce,
        tokenFilePath,
        logPath,
        anikaHome,
 ...
 * opened SshConnection, forward/pickLocalPort/waitForAnika, a token-gated
...
async function connect(deps) {
  ...
  const anikaHome = await probeRemoteAnikaHome(ssh)
  await assertRemoteInstallUpdateClear(ssh, anikaHome)
  const anikaPath = await locateAnika(ssh, remoteAnikaPath)
  log(`located anika at ${anikaPath}`)
  const anikaVersion = await probeAnikaVersion(ssh, anikaPath)

  if (anikaVersion) {
    log(`remote anika version: ${anikaVersion}`)
  }
  ...
  if (isLockfileSkew(lock)) {
    ...
    const error: any = new Error(
      `The remote ownership record ${lpath} does not match this Anika Desktop build (${lock.reason}). ` +
  ...
  if (lock) {
    ...
    const reusable =
      ...
      lock.anikaPath === anikaPath &&
      lock.anikaHome === anikaHome
  ...
  const spawned = await spawnRemoteDashboard(ssh, {
    ...
    ownershipId,
    anikaHome,
    assertInstallClear: () => assertRemoteInstallUpdateClear(ssh, anikaHome)
  })
  ...
  try {
    ...
    assertBootstrapNotSuperseded(signal)
    const baseUrl = `http://127.0.0.1:${localPort}`
    await waitForAnika(baseUrl, spawnToken)
    assertBootstrapNotSuperseded(signal)
