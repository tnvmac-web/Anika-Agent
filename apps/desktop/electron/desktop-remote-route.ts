...
export function resolveDesktopRemoteRoute({
  ...
  if (envUrl) {
    ...
    if (!envToken) {
      throw new Error(
        'ANIKA_DESKTOP_REMOTE_URL is set but ANIKA_DESKTOP_REMOTE_TOKEN is not. ' +
          'Both must be provided to connect to a remote Anika backend.'
      )
    }
  ...
  if (!modeIsRemoteLike(config.mode)) {
    ...
    // resolves local and spawns a loopback `anika serve` the desktop never
