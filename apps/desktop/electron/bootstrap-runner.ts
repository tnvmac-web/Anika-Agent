 ...
 * bootstrap-runner.ts
 *
 * Drives apps/desktop's first-launch install of Anika Agent by spawning
...
function readExistingPinnedCommit(activeRoot: string | null | undefined): string | null {
  ...
  try {
    const raw = fs.readFileSync(path.join(activeRoot, '.anika-bootstrap-complete'), 'utf8')
...
function bootstrapCacheDir(anikaHome) {
  return path.join(anikaHome, 'bootstrap-cache')
}

// The install.sh / install.ps1 that ships inside the already-installed agent
// checkout under ~/.anika/anika-agent. Used as a last-resort fallback when
// the pinned commit can't be fetched from GitHub (e.g. a locally-built desktop
// app stamped to an unpushed HEAD).
function installedAgentInstallScript(anikaHome) {
  if (!anikaHome) {
    return null
  }

  const candidate = path.join(anikaHome, 'anika-agent', 'scripts', installScriptName())
...
function cachedScriptPath(anikaHome, commit) {
  return path.join(bootstrapCacheDir(anikaHome), `install-${commit}.${process.platform === 'win32' ? 'ps1' : 'sh'}`)
}
...
function downloadInstallScript(ref, destPath) {
  ...
  const url = `https://raw.githubusercontent.com/__NEW_ORG__/anika-agent/${ref}/scripts/${scriptName}`
...
async function resolveInstallScript({
  ...
  const cached = cachedScriptPath(anikaHome, installRef.cacheKey)
  ...
  try {
    ...
    const installed = installedAgentInstallScript(anikaHome)
...
function spawnPowerShell(scriptPath, args, { emit, stageName, abortSignal, anikaHome }: any = {}) {
  return new Promise<any>((resolve, reject) => {
    ...
    const child = spawn(
      ...
      hiddenWindowsChildOptions({
        ...
        env: {
          ...process.env,
          // Pass ANIKA_HOME through so install.ps1 respects the caller's
          // choice rather than re-computing the default.
          ANIKA_HOME: anikaHome || process.env.ANIKA_HOME || ''
        }
      })
    )
...
function spawnBash(scriptPath, args, { emit, stageName, abortSignal, anikaHome }: any = {}) {
  return new Promise<any>((resolve, reject) => {
...
function buildPosixPinArgs({ installStamp, activeRoot, anikaHome, pinCommit = true }) {
  const args = ['--dir', activeRoot, '--anika-home', anikaHome]
...
async function fetchManifest({ scriptPath, installerKind, emit, anikaHome, activeRoot, installStamp, pinCommit }) {
  const isPosix = installerKind === 'posix'

  const args = isPosix
    ? ['--manifest', ...buildPosixPinArgs({ installStamp, activeRoot, anikaHome, pinCommit })]
...
async function runStage({
  ...
  const args = isPosix
    ? [
        ...
        '--non-interactive',
        '--json',
        ...buildPosixPinArgs({ installStamp, activeRoot, anikaHome, pinCommit })
      ]
...
async function runBootstrap(opts) {
  ...
  const runLog = openRunLog(logRoot || path.join(anikaHome, 'logs'))
  ...
  try {
    ...
    const scriptInfo = await resolveInstallScript({ installStamp, sourceRepoRoot, anikaHome, emit })
