...
test('uninstallArgsForMode maps each mode to the module-runner argv', () => {
  assert.deepEqual(uninstallArgsForMode('gui'), ['-m', 'anika_cli.uninstall', '--mode', 'gui'])
  assert.deepEqual(uninstallArgsForMode('lite'), ['-m', 'anika_cli.uninstall', '--mode', 'lite'])
  assert.deepEqual(uninstallArgsForMode('full'), ['-m', 'anika_cli.uninstall', '--mode', 'full'])
})
...
test('resolveRemovableAppPath finds the .app bundle on macOS', () => {
  assert.equal(
    resolveRemovableAppPath('/Applications/Anika.app/Contents/MacOS/Anika', 'darwin'),
    '/Applications/Anika.app'
  )
  assert.equal(
    resolveRemovableAppPath('/Users/x/Applications/Anika.app/Contents/MacOS/Anika', 'darwin'),
    '/Users/x/Applications/Anika.app'
  )
})
...
test('resolveRemovableAppPath finds the install dir on Windows', () => {
  assert.equal(
    resolveRemovableAppPath('C:\\Users\\x\\AppData\\Local\\Programs\\Anika\\Anika.exe', 'win32'),
    'C:\\Users\\x\\AppData\\Local\\Programs\\Anika'
  )
  assert.equal(
    resolveRemovableAppPath('C:\\Users\\x\\AppData\\Local\\anika-desktop\\Anika.exe', 'win32'),
    'C:\\Users\\x\\AppData\\Local\\anika-desktop'
  )
})

test('resolveRemovableAppPath returns null for an unrecognized Windows dir', () => {
  assert.equal(resolveRemovableAppPath('C:\\Temp\\foo\\Anika.exe', 'win32'), null)
})

test('resolveRemovableAppPath uses APPIMAGE on Linux when set', () => {
  assert.equal(
    resolveRemovableAppPath('/tmp/.mount_AnikaXXXX/anika', 'linux', { APPIMAGE: '/home/x/Apps/Anika.AppImage' }),
    '/home/x/Apps/Anika.AppImage'
  )
})

test('resolveRemovableAppPath finds the unpacked dir on Linux', () => {
  assert.equal(resolveRemovableAppPath('/opt/anika/linux-unpacked/anika', 'linux', {}), '/opt/anika/linux-unpacked')
  // A system-package install (/usr/bin) → null, left to apt/dnf.
  assert.equal(resolveRemovableAppPath('/usr/bin/anika', 'linux', {}), null)
})
...
test('shouldRemoveAppBundle requires packaged AND a resolved path', () => {
  assert.equal(shouldRemoveAppBundle(true, '/Applications/Anika.app'), true)
  assert.equal(shouldRemoveAppBundle(false, '/Applications/Anika.app'), false)
...
test('buildPosixCleanupScript waits for the PID, runs the uninstall module, removes bundle', () => {
  const script = buildPosixCleanupScript({
    desktopPid: 4321,
    pythonExe: '/home/x/.anika/anika-agent/venv/bin/python',
    pythonPath: null,
    agentRoot: '/home/x/.anika/anika-agent',
    uninstallArgs: ['-m', 'anika_cli.uninstall', '--mode', 'gui'],
    appPath: '/opt/anika/linux-unpacked',
    anikaHome: '/home/x/.anika'
  })
  ...
  assert.match(script, /'-m' 'anika_cli\.uninstall' '--mode' 'gui'/)
  assert.match(script, /rm -rf '\/opt\/anika\/linux-unpacked'/)
  assert.match(script, /export ANIKA_HOME='\/home\/x\/\.anika'/)
})
...
test('buildPosixCleanupScript exports PYTHONPATH when pythonPath is set (lite/full)', () => {
  const script = buildPosixCleanupScript({
    ...
    pythonPath: '/home/x/.anika/anika-agent',
    agentRoot: '/home/x/.anika/anika-agent',
    uninstallArgs: ['-m', 'anika_cli.uninstall', '--mode', 'full'],
    appPath: null,
    anikaHome: '/home/x/.anika'
  })

  // System python + source on PYTHONPATH so import anika_cli works while the
  // venv is torn down.
  assert.match(script, /export PYTHONPATH='\/home\/x\/\.anika\/anika-agent'/)
  assert.match(script, /'\/usr\/bin\/python3' '-m' 'anika_cli\.uninstall' '--mode' 'full'/)
})
...
test('buildPosixCleanupScript omits the bundle rm when appPath is null', () => {
  const script = buildPosixCleanupScript({
    ...
    pythonPath: null,
    agentRoot: '/a',
    uninstallArgs: ['-m', 'anika_cli.uninstall', '--mode', 'lite'],
    appPath: null,
    anikaHome: '/h'
  })
  ...
  assert.match(script, /'-m' 'anika_cli\.uninstall' '--mode' 'lite'/)
})
...
test('buildWindowsCleanupScript waits (bounded) for PID, runs uninstall, rmdir bundle', () => {
  const script = buildWindowsCleanupScript({
    ...
    agentRoot: 'C:\\anika',
    uninstallArgs: ['-m', 'anika_cli.uninstall', '--mode', 'full'],
    appPath: 'C:\\Users\\x\\AppData\\Local\\Programs\\Anika',
    anikaHome: 'C:\\Users\\x\\AppData\\Local\\anika'
  })

  assert.match(script, /@echo off/)
  assert.match(script, /set "PID=9988"/)
  // PYTHONPATH set so a system python can import anika_cli from source.
  assert.match(script, /set "PYTHONPATH=C:\\anika;%PYTHONPATH%"/)
  assert.match(script, /"C:\\Python313\\python.exe" "-m" "anika_cli\.uninstall" "--mode" "full"/)
  ...
  assert.match(script, /rmdir \/s \/q "C:\\Users\\x\\AppData\\Local\\Programs\\Anika" >nul 2>&1/)
