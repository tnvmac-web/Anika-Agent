 ...
 *
 * So we delegate to `anika gateway stop --all`: the CLI discovers every
 ...
 * force-kills survivors — the same logic `anika update`'s
...
export function stopGatewayBeforeUpdate(
  ...
  return runGatewayLifecycleCommand(anikaCliPath, ['gateway', 'stop', '--all'], deps)
}
 ...
 */
export function startGatewaysAfterUpdateAbort(anikaCliPath: string, deps: StopGatewayBeforeUpdateDeps = {}): boolean {
  return runGatewayLifecycleCommand(anikaCliPath, ['gateway', 'start', '--all'], deps)
}

function runGatewayLifecycleCommand(anikaCliPath: string, args: string[], deps: StopGatewayBeforeUpdateDeps): boolean {
  ...
  if (deps.spy) {
    deps.spy(anikaCliPath, args)
  }

  if (!existsSync(anikaCliPath)) {
    return false
  }

  try {
    exec(anikaCliPath, args, {
