...
export interface AnikaReadyOptions {
   ...
   * two very different meanings of a 401 (see `waitForAnikaReady`).
   */
...
export const REMOTE_UNSIGNED_OAUTH_MESSAGE =
  'Remote Anika gateway uses OAuth, but you are not signed in. ' +
...
/**
 * The one factory for the actionable __NEW_ORG__ Cloud agent-is-down error, shared by
 ...
 *  - readiness-probe exhaustion in waitForAnikaReady().
 *
 * Returns null unless the backend is a *.agents.__NEW_DOMAIN__ host AND the
...
export function makeNousCloudBackendDownError(baseUrl: string, error: unknown): Error | null {
  ...
  const err = new Error(
    `__NEW_ORG__ Cloud agent ${hostname} is down ` +
      `(HTTP ${serverError.statusCode}: server-side fault). ` +
      'Check https://portal.__NEW_DOMAIN__ for backend status, ' +
      'or switch to Local mode in Settings → Gateway. ' +
      'You can also reach out on Discord at discord.gg/__NEW_ORG__ ' +
...
/**
 * True when the backend URL points at a Nous-managed Anika Cloud instance
 * (e.g. ares-3009.agents.__NEW_DOMAIN__). These are Fly.io-hosted machines
...
export function isNousCloudAgentUrl(baseUrl: string): boolean {
  try {
    ...
    return host.endsWith('.agents.__NEW_DOMAIN__')
  } catch {
    return false
  }
}
 ...
 * `isReauthRequired` so startAnika latches; `needsOauthLogin` alone only
...
export async function waitForAnikaReady(baseUrl: string, options: AnikaReadyOptions): Promise<void> {
  ...
  throw new Error(`Anika backend did not become ready: ${detail}`)
}
