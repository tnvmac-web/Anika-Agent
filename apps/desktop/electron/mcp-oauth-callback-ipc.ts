 ...
 *   - the browser sees only a minimal "return to Anika" page.
 */
...
const DONE_HTML =
  ...
  '<p>You can close this window and return to Anika.</p>' +
...
export function registerMcpOauthCallbackIpc() {
  ...
  ipcMain.handle('anika:mcp-oauth:listen', async () => {
  ...
  ipcMain.handle('anika:mcp-oauth:wait', async (_event, id, timeoutMs) => {
  ...
  ipcMain.handle('anika:mcp-oauth:cancel', (_event, id) => {
    dispose(String(id || ''))
