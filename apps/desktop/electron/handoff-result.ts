 ...
 * console. It writes ANIKA_HOME/.anika-update-result.json on every exit
 ...
 * ever surfaces, and the user may not reopen Anika within 30 minutes.
...
export function handoffResultPath(anikaHome: string): string {
  return path.join(anikaHome, '.anika-update-result.json')
}
...
export function readAndConsumeHandoffResult(
  ...
  const file = handoffResultPath(anikaHome)
  let raw: string
