...
test('Windows relaunch gate refuses live and uncertain markers before executing the remote runtime', async () => {
  for (const observation of ['LIVE:4242', 'UNCERTAIN']) {
    ...
    const ssh = sshWith(async command => {
      ...
      if (script.includes('Get-Command anika.exe')) {
        return JSON.stringify({
          os: 'Windows',
          arch: 'AMD64',
          anikaHome: 'C:\\Users\\alice\\.anika',
          anikaPath: 'C:\\Anika\\anika.exe',
          python: 'C:\\Anika\\python.exe'
        })
      }

      if (script.includes('.anika-update-in-progress')) {
        return observation
      }
    ...
    assert.equal(
      scripts.some(script => script.includes('anika_cli.windows_ssh_runtime')),
      false
    )
  }
})
...
test('Windows relaunch gate uses strict install-wide marker parsing and fail-closed PID probing', async () => {
  ...
  await assertWindowsRemoteInstallUpdateClear(ssh, 'C:\\Users\\alice\\.anika\\profiles\\research')
  assert.match(script, /\.anika-update-in-progress/)
...
test('Windows probe validates Anika and Python topology before selection', async () => {
  let script = ''
  ...
  const fallbackJoin = script.indexOf('Join-Path $anikaHome')
  ...
  await assert.rejects(
    detectRemotePlatform(
      sshWith(async command => {
        ...
        throw new Error('Anika is not installed on the remote Windows host.')
      })
    ),
    (err: any) => err.kind === 'unsupported-platform' && /Anika is not installed/.test(err.message)
  )
})

test('helper command uses the fixed remote Python entry point and quotes path data', () => {
  const command = helperCommand({ python: "C:\\Program Files\\Anika's\\python.exe" }, 'inspect', [
    'C:\\x y\\anika.exe'
  ])

  const encoded = command.split(' ').pop()!
  const script = Buffer.from(encoded, 'base64').toString('utf16le')
  assert.match(script, /-m' 'anika_cli\.windows_ssh_runtime' 'inspect'/)
  assert.match(script, /Anika''s/)
  assert.match(script, /C:\\x y\\anika\.exe/)
})
...
test('Windows SSH reuse requires the requested remote profile to match the lock', () => {
  ...
  const runtime = { anikaPath: lock.anikaPath, anikaHome: lock.anikaHome }
