 ...
 * `iter_anika_node_dirs()` in anika_constants.py, which the Electron main
 * process cannot import.
 */
function anikaManagedNodePathEntries(
  anikaHome,
  ...
  const root = pathModule.join(anikaHome, 'node')
...
function buildDesktopBackendPath({
  ...
  const anikaNodeDirs = anikaManagedNodePathEntries(anikaHome, { platform, pathModule })
  const venvBin = venvRoot ? pathModule.join(venvRoot, platform === 'win32' ? 'Scripts' : 'bin') : null
  const saneEntries = platform === 'win32' ? [] : POSIX_SANE_PATH_ENTRIES

  return appendUniquePathEntries([anikaNodeDirs, venvBin, currentPath, saneEntries], { delimiter })
}

function normalizeAnikaHomeRoot(anikaHome, { pathModule = pathModuleForPlatform(process.platform) }: any = {}) {
  if (!anikaHome) {
    return anikaHome
  }

  const resolved = pathModule.resolve(String(anikaHome))
...
function buildDesktopBackendEnv({
  ...
  return {
    ...
    // cp1252, ...). anika_bootstrap sets this inside the child too, but only
