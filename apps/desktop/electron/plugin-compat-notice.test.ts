...
function tmp() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'anika-compat-'))
}
...
const REPORT = {
  ...
  plugins: {
    ...
    beta: [
      { file: 'a.py', line: 1, old: 'anika_cli.kanban_db.connect', new: 'anika_cli.kanban_db_connect.connect' },
      {
        file: 'a.py',
        line: 9,
        old: 'anika_cli.kanban_db.connect_closing',
        new: 'anika_cli.kanban_db_connect.connect_closing'
      }
    ]
  },
  lines: [
    '2 plugins use import paths that stop working on 2026-09-14 (10 days): alpha (1), beta (2)',
    'Details: anika plugins compat'
  ]
}
...
test('report → one notice naming plugins, date and the CLI command', () => {
  ...
  assert.match(n.detail, /anika plugins compat/)
})
