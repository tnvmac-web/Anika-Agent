...
const READY_RE = /^ANIKA_(?:BACKEND|DASHBOARD)_READY port=(\d+)/gm
...
async function probeWindowsRemote(ssh, explicitAnikaPath = '') {
  const explicit = psLiteral(explicitAnikaPath)
  ...
  const script = [
    ...
    '$anikaHome=$env:ANIKA_HOME',
    'if(-not $anikaHome){$anikaHome=Join-Path $env:LOCALAPPDATA "anika"}',
    'Assert-NoReparse $anikaHome $true',
    '$candidate=[IO.Path]::Combine($anikaHome, "anika-agent\\venv\\Scripts\\anika.exe")',
    ...
    '$profileCandidate=[IO.Path]::Combine($HOME, "anika-agent\\.venv\\Scripts\\anika.exe")',
    ...
    '$fallbackHomeCandidate=Join-Path $anikaHome "anika-agent\\venv\\Scripts\\anika.exe"',
    '$fallbackProfileCandidate=Join-Path $HOME "anika-agent\\.venv\\Scripts\\anika.exe"',
    '$candidates=@()',
    'if($explicit){$candidates+=$explicit}',
    '$cmd=Get-Command anika.exe -ErrorAction SilentlyContinue',
    ...
    '$anika=$null',
    'foreach($candidate in $candidates){Assert-NoReparse $candidate $true;$candidatePython=[IO.Path]::Combine([IO.Path]::GetDirectoryName($candidate), "python.exe");Assert-NoReparse $candidatePython $true;try{$item=Get-Item -LiteralPath $candidate -Force -ErrorAction Stop;if(($item.Attributes -band [IO.FileAttributes]::ReparsePoint) -eq 0 -and -not $item.PSIsContainer){$anika=$item.FullName;break}}catch [Management.Automation.ItemNotFoundException]{continue}}',
    'if(-not $anika){throw "Anika is not installed on the remote Windows host."}',
    'Assert-NoReparse $anika $false',
    'if($explicit -and $anika -ne $explicit){throw "The configured Anika path is not an executable file."}',
    '$python=[IO.Path]::Combine([IO.Path]::GetDirectoryName($anika), "python.exe")',
    'Assert-NoReparse $python $false',
    '[ordered]@{os="Windows";arch=$env:PROCESSOR_ARCHITECTURE;anikaHome=$anikaHome;anikaPath=$anika;python=$python}|ConvertTo-Json -Compress'
  ].join(';')

  return JSON.parse((await ssh.exec(powerShellCommand(script))).trim())
}

function windowsUpdateMarkerProbeCommand(anikaHome) {
  const script = [
    '$ErrorActionPreference="Stop"',
...
public static class AnikaMarkerNoFollow {
...
`,
    ...
    '}',
    '}',
    `$home=${psLiteral(anikaHome)}`,
    '$installRoot=$home',
    '$parent=Split-Path -Parent $home',
    'if((Split-Path -Leaf $parent) -ieq "profiles"){$installRoot=Split-Path -Parent $parent}',
    '$marker=Join-Path $installRoot ".anika-update-in-progress"',
    '$result="UNCERTAIN"',
    '$stream=$null;$memory=$null',
    'try{',
    'Assert-NoReparse $marker $true',
    'if(-not (Test-Path -LiteralPath $marker -PathType Leaf)){$result="CLEAR"}else{$stream=[AnikaMarkerNoFollow]::OpenRead($marker)',
 ...
 */
async function assertWindowsRemoteInstallUpdateClear(ssh, anikaHome) {
  let observation = ''

  try {
    observation =
      String(await ssh.exec(windowsUpdateMarkerProbeCommand(anikaHome)))
        .replace(/^\uFEFF/, '')
        .trim()
        .split(/\r?\n/)
        .pop() || ''
...
async function detectRemotePlatform(ssh, explicitAnikaPath = '') {
  try {
  ...
  try {
    return await probeWindowsRemote(ssh, explicitAnikaPath)
  } catch (cause: any) {
...
function helperCommand(runtime, operation, args = []) {
  const argv = [runtime.python, '-m', 'anika_cli.windows_ssh_runtime', operation, ...args]
...
function atomicWindowsSpawnCommand(runtime, reservation: any = {}) {
  const argv = [runtime.python, '-m', 'anika_cli.windows_ssh_runtime', 'spawn']
  const helper = operation => [runtime.python, '-m', 'anika_cli.windows_ssh_runtime', operation]

  const script = [
    '$ErrorActionPreference="Stop"',
    `$home=${psLiteral(runtime.anikaHome)}`,
    '$installRoot=$home',
    '$parent=Split-Path -Parent $home',
    'if((Split-Path -Leaf $parent) -ieq "profiles"){$installRoot=Split-Path -Parent $parent}',
    '$marker=Join-Path $installRoot ".anika-update-in-progress"',
    ...
    reservation.ownershipId
      ...
      : '  if($LASTEXITCODE -ne 0){exit $LASTEXITCODE}',
    reservation.ownershipId
      ? `  $spawned=$spawnLines[-1]|ConvertFrom-Json; $lock=[ordered]@{schemaVersion=2;protocolVersion=1;ownershipId=${psLiteral(reservation.ownershipId)};spawnNonce=${psLiteral(reservation.spawnNonce)};pid=[int]$spawned.pid;creationTimeNs=[string]$spawned.creationTimeNs;port=0;profile=${psLiteral(reservation.profile)};anikaPath=${psLiteral(reservation.anikaPath)};anikaHome=${psLiteral(reservation.anikaHome)};tokenFingerprint=${psLiteral(reservation.tokenFingerprint)};startedAt=${psLiteral(reservation.startedAt)}}|ConvertTo-Json -Compress; ` +
...
function validLock(lock, ownershipId) {
  ...
  return Boolean(
    ...
    typeof lock.anikaPath === 'string' &&
    typeof lock.anikaHome === 'string'
  )
}
...
function reusableWindowsLock(lock, state, profile, reuseToken, runtime) {
  return Boolean(
    ...
    lock.anikaPath === runtime.anikaPath &&
    lock.anikaHome === runtime.anikaHome
  )
}
...
async function connectWindowsRemote(deps) {
  ...
  assertBootstrapNotSuperseded(signal)
  const runtime = await probeWindowsRemote(ssh, remoteAnikaPath)
  await assertWindowsRemoteInstallUpdateClear(ssh, runtime.anikaHome)
  const inspection = await helper(ssh, runtime, 'inspect', [runtime.anikaPath])

  if (!inspection.supported) {
    const error: any = new Error('Update Anika on the remote Windows host before connecting with Desktop SSH.')
    error.kind = 'update-required'
    throw error
  }

  runtime.anikaPath = inspection.path
  const anikaVersion = inspection.version || ''
  rememberLog(`[ssh-lifecycle] remote platform Windows/${runtime.arch}`)
  rememberLog(`[ssh-lifecycle] located anika at ${runtime.anikaPath}`)
  ...
  try {
    ...
    spawned = await atomicWindowsSpawn(
      ssh,
      runtime,
      JSON.stringify({ ownershipId, spawnNonce, profile, anikaPath: runtime.anikaPath }),
      {
        ownershipId,
        spawnNonce,
        profile,
  ...
  try {
    ...
    const baseUrl = `http://127.0.0.1:${localPort}`
    await waitForAnika(baseUrl, token)
    assertBootstrapNotSuperseded(signal)
...
function buildWindowsInteractiveCommand(remoteCwd = '') {
  ...
  script.push('$host.UI.RawUI.WindowTitle="Anika SSH"', 'powershell.exe -NoLogo')
