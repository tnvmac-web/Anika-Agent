 ...
 * INITIATED, not completed. A dying `python.exe -m anika_cli.main serve`
 * stays in the process table while it unmaps .pyd files (AV / NTFS filter
 * drivers stretch this out), and it need not hold the venv `anika.exe` shim
...
export interface ReleaseGateDeps {
  /** Probe the venv anika.exe shim (real: O_RDWR open attempt). */
