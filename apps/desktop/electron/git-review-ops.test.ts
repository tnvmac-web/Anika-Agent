...
function makeRepo() {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'anika-desktop-git-status-'))

  tempDirs.push(dir)
  execFileSync('git', ['init', '-q'], { cwd: dir })
  execFileSync('git', ['config', 'user.email', 'anika-test@example.com'], { cwd: dir })
  execFileSync('git', ['config', 'user.name', 'Anika Test'], { cwd: dir })
