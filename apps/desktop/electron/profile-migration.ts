...
export interface MigrationDeps {
  ...
  /** Default profile home (`~/.anika`). Default's state.db and gateway.pid live here. */
  anikaHome: string
  /** Named-profile root (`~/.anika/profiles`). Does not contain `default`. */
  profilesRoot: string
  ...
  isAnikaProcess: (pid: number) => boolean
  now: () => number
...
/**
 * Production layout: default IS `anikaHome`; named profiles are children of
 * `profilesRoot`. There is no `profiles/default` directory on a normal install.
 */
export function profileStateDbPath(name: string, anikaHome: string, profilesRoot: string): string {
  return name === 'default' ? `${anikaHome}/state.db` : `${profilesRoot}/${name}/state.db`
}

export function profileGatewayPidPath(name: string, anikaHome: string, profilesRoot: string): string {
  return name === 'default' ? `${anikaHome}/gateway.pid` : `${profilesRoot}/${name}/gateway.pid`
}

function resolveAnikaHome(profilesRoot: string, anikaHome?: string): string {
  if (anikaHome) {
    return anikaHome
  }

  // Tests that predate anikaHome pass only profilesRoot.
...
/**
 * Return the profile names whose gateway.pid file points to a live anika process.
 ...
 *
 * `anikaHome` is optional so existing call sites that only pass `profilesRoot`
 * still work: it is derived as the parent of `…/profiles`.
 */
export function findRunningGatewayProfiles(
  profilesRoot: string,
  allProfiles: string[],
  deps: Pick<MigrationDeps, 'existsSync' | 'readFileSync' | 'isAnikaProcess'> & { anikaHome?: string }
): string[] {
  const anikaHome = resolveAnikaHome(profilesRoot, deps.anikaHome)
  const running: string[] = []

  for (const name of allProfiles) {
    const pidFile = profileGatewayPidPath(name, anikaHome, profilesRoot)
...
export function decideMigration(
  ...
  for (const name of candidates) {
    const s = score(profileStateDbPath(name, deps.anikaHome, deps.profilesRoot))
...
/** Default is always a candidate; it is `$ANIKA_HOME`, not `$ANIKA_HOME/profiles/default`. */
...
export function migrateActiveProfileIfMissing(desktopProfileConfigPath: string, deps: MigrationDeps): boolean {
  ...
  // launches `anika --profile default` and is worse than writing nothing
