...
test('resolveLoginStrategy returns native for native_pkce gateway with non-password provider', () => {
  const statusBody = { auth_required: true, auth_flows: ['cookie', 'native_pkce'] }
  const providers = [{ name: 'nous', displayName: '__NEW_ORG__', supportsPassword: false }]
...
test('buildNativeAuthorizeUrl omits provider when not given and preserves prefix', () => {
  const url = buildNativeAuthorizeUrl('https://gw.example.com/anika', {
    challenge: 'C',
  ...
  assert.equal(parsed.pathname, '/anika/auth/native/authorize')
...
test('nativeTokenUrl / nativeRefreshUrl build the right endpoints', () => {
  ...
  assert.equal(nativeRefreshUrl('https://gw.example.com/anika'), 'https://gw.example.com/anika/auth/native/refresh')
})
