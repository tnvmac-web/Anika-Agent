...
// hardening, ANIKA_HOME resolution, and the git binary stay injected.
...
export function registerFsIpc({
  ...
  ipcMain.handle('anika:fs:readDir', async (_event, dirPath) => readDirForIpc(dirPath))

  ipcMain.handle('anika:fs:gitRoot', async (_event, startPath) => gitRootForIpc(startPath))

  // Reveal a path in the OS file manager (Finder / Explorer / Files).
  ipcMain.handle('anika:fs:reveal', async (_event, targetPath) => {
  ...
  ipcMain.handle('anika:fs:openDir', async (_event, dirPath) => {
  ...
  // The LOCAL Desktop runtime-plugin root: `<ANIKA_HOME>/desktop-plugins`,
  // resolved from the main-process ANIKA_HOME (see resolveAnikaHome) — NOT from
  // the connected backend. A remote backend reports its own `anika_home` over
  ...
  async function localPluginsRoot(dirName: string): Promise<string> {
    ...
    // profiles/<name>/, matching the profile-scoped anika_home the backend
    // reported before this resolver existed. 'default'/unset pins the global root.
    const profile = readActiveDesktopProfile()
    const base = profile && profile !== 'default' ? path.join(anikaHome, 'profiles', profile) : anikaHome
  ...
  ipcMain.handle('anika:fs:desktopPluginsRoot', async () => localPluginsRoot('desktop-plugins'))

  // The LOCAL logs root (`<ANIKA_HOME>/logs`, profile-aware) — the error
  // card's "Open Logs" action reveals agent.log/gateway.log without the user
  // knowing where ANIKA_HOME lives. Same Electron-local resolution as the
  // plugin roots: valid in every connection mode, created on demand.
  ipcMain.handle('anika:fs:logsRoot', async () => localPluginsRoot('logs'))

  // The LOCAL agent-plugin root (`<ANIKA_HOME>/plugins`), same Electron-local
  ...
  ipcMain.handle('anika:fs:agentPluginsRoot', async () => localPluginsRoot('plugins'))

  ipcMain.handle('anika:plugin:probe', async (_event, payload) => {
  ...
  ipcMain.handle('anika:plugin:installDesktop', async (_event, payload) => {
    const identifier = String(payload?.identifier || payload?.repo || '').trim()
  ...
  ipcMain.handle('anika:fs:rename', async (_event, targetPath, newName) => {
  ...
  ipcMain.handle('anika:fs:writeText', async (_event, filePath, content) => {
  ...
  ipcMain.handle('anika:fs:trash', async (_event, targetPath) => {
    const target = String(targetPath || '').trim()
