...
export function registerHudIpc({
  ...
  ipcMain.on('anika:hud:native-drag', event => {
    event.returnValue = hudWindowing().move === 'native-drag'
  })

  ipcMain.on('anika:hud:windowing', event => {
  ...
  ipcMain.on('anika:hud:workspace-transfer', (event, transferring) => {
  ...
  ipcMain.handle('anika:hud:open', async (_event, request) => {
    openHudWindow(
  ...
  ipcMain.handle('anika:hud:frost', (_event, showing) => {
  ...
  // reaches the bar.
  ipcMain.on('anika:hud:ignore-mouse', (_event, ignore) => {
    const hudWindow = getHudWindow()
  ...
  ipcMain.on('anika:hud:begin-move', event => {
    const hudWindow = getHudWindow()
  ...
  ipcMain.on('anika:hud:end-move', event => {
    const hudWindow = getHudWindow()
  ...
  ipcMain.on('anika:hud:move-by', (event, delta) => {
    const hudWindow = getHudWindow()
  ...
  ipcMain.on('anika:hud:set-bounds', (event, bounds) => {
    const hudWindow = getHudWindow()
  ...
  ipcMain.handle('anika:hud:reset-layout', event => {
    const hudWindow = getHudWindow()
  ...
  ipcMain.on('anika:hud:session', (event, sessionId) => {
    const hudWindow = getHudWindow()
  ...
  ipcMain.handle('anika:hud:close', async () => {
    closeHudWindow()
