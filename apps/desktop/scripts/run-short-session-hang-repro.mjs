...
const UPSTREAM_URL = 'https://github.com/__NEW_ORG__/anika-agent.git'
...
const LEGACY_STORAGE_KEY = 'anika.desktop.inflightTurnJournal.v1'
const LEGACY_MIGRATION_KEY = 'anika.desktop.inflightTurnJournal.v2.migrated'
...
async function verifyInteractiveSurfaces(cdp, timed, measure, label) {
  ...
  const version = await timed(`version.ipc.${label}`, () => cdp.eval('window.anikaDesktop.getVersion()'))
...
async function withTemporarySandbox(label, body) {
  const sandbox = mkdtempSync(join(tmpdir(), `anika-short-session-${label}-`))
...
async function executeRunInSandboxAttempt(target, index, warmup, mock, output, sandbox, attempt) {
  ...
  const anikaHome = join(sandbox, 'anika-home')
  const userData = join(sandbox, 'electron-user-data')
  const desktopLog = join(anikaHome, 'logs', 'desktop.log')
  ...
  writeSandboxConfig(anikaHome, mock.url)
  ...
  const child = spawn(
    ...
    [target.targetDesktop, `--user-data-dir=${userData}`, `--remote-debugging-port=${port}`],
    {
      ...
      env: sanitizedEnv({
        ANIKA_DESKTOP_APP_NAME: `AnikaShortSession-${target.label}-${process.pid}-${index}-${warmup ? 'w' : 'm'}-${attempt}`,
        ANIKA_DESKTOP_ANIKA_ROOT: target.targetRoot,
        ANIKA_DESKTOP_IGNORE_EXISTING: '1',
        ANIKA_DESKTOP_USER_DATA_DIR: userData,
...
async function main() {
  ...
  const ephemeralRoot = mkdtempSync(join(tmpdir(), 'anika-short-session-ab-'))
  const prepared = []
