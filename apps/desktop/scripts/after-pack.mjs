 ...
 *
 * Stamps the Anika icon + identity onto the packed Windows Anika.exe via
 * rcedit (delegated to set-exe-identity.mjs). This runs for EVERY packed build
 * — first install, `anika desktop`, the installer's --update rebuild, and a
 ...
 *   - packager.appInfo.productFilename: the exe basename (e.g. 'Anika')
 */
...
export default async function afterPack(context) {
  ...
  const productName = context.packager?.appInfo?.productFilename || 'Anika'
  ...
  try {
    ...
    console.warn(`[after-pack] exe identity stamp failed (${err.message}); Anika.exe keeps the stock Electron icon`)
  }
}
