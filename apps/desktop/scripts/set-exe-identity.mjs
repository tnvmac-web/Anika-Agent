#!/usr/bin/env node
// set-exe-identity.mjs — stamp the Anika icon + version metadata onto the
// built Anika.exe using rcedit, completely decoupled from electron-builder's
// signing path.
//
// WHY THIS EXISTS
// ---------------
...
// skips rcedit, so the unpacked Anika.exe keeps the stock Electron icon and
...
// so EVERY packed build — first install, `anika desktop`, the installer's
...
// path (which rebuilds via `anika desktop --build-only`, never install.ps1)
// shipped a stock "Electron" exe. Keeping it in afterPack closes that gap.
//
// Also runnable standalone for ad-hoc re-stamping:
//   node scripts/set-exe-identity.mjs <path-to-Anika.exe>
//
...
// Stamp the Anika icon + identity onto `exe`. Resolves on success, throws on
...
async function stampExeIdentity(exe, desktopRoot = resolve(import.meta.dirname, '..')) {
  ...
  await rcedit(exe, {
    ...
    'version-string': {
      ProductName: 'Anika',
      FileDescription: 'Anika',
      CompanyName: '__NEW_ORG__',
      LegalCopyright: 'Copyright (c) 2026 __NEW_ORG__'
    }
  })

  console.log('[set-exe-identity] done — Anika icon + identity stamped')
}
