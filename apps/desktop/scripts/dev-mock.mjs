...
function createSandbox() {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), `anika-dev-mock-${Date.now()}`))
  ...
  return { root, anikaHome, userDataDir, cleanup: () => fs.rmSync(root, { recursive: true, force: true }) }
}

function writeMockConfig(anikaHome, mockUrl) {
  fs.writeFileSync(
    path.join(anikaHome, 'config.yaml'),
...
`,
  ...
  fs.writeFileSync(path.join(anikaHome, '.env'), 'MOCK_API_KEY=e2e-mock-key\n', 'utf8')
}
...
async function main() {
  ...
  writeMockConfig(sandbox.anikaHome, mock.url)
  console.log(`  ANIKA_HOME: ${sandbox.anikaHome}`)
  ...
  const env = {
    ...
    ANIKA_DESKTOP_APP_NAME: `AnikaDevMock-${Date.now()}`,
  }
