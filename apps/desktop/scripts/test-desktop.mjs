...
// no longer bundles the Anika Agent Python payload (that's fetched at first
// launch via install.ps1 / install.sh, per the Phase 1 thin-installer flow).
const APP = (() => {
  if (PLATFORM === 'darwin') {
    const appPath = path.join(RELEASE_ROOT, `mac-${ARCH}`, 'Anika.app')
    return {
      appPath,
      binary: path.join(appPath, 'Contents', 'MacOS', 'Anika'),
  ...
  if (PLATFORM === 'win32') {
    ...
    return {
      appPath: unpacked,
      binary: path.join(unpacked, 'Anika.exe'),
  ...
  return {
    appPath: unpacked,
    binary: path.join(unpacked, 'Anika'),
    resourcesPath: path.join(unpacked, 'resources'),
    asarPath: path.join(unpacked, 'resources', 'app.asar'),
    unpackedDistIndex: path.join(unpacked, 'resources', 'app.asar.unpacked', 'dist', 'index.html')
  }
...
// Default ANIKA_HOME for non-sandboxed runs -- matches main.ts's
// resolveAnikaHome(). On Windows it's %LOCALAPPDATA%\anika; elsewhere
// it's ~/.anika. The fresh-install sandbox launchFresh() sets its own
// ANIKA_HOME and never touches this.
const DEFAULT_ANIKA_HOME = (() => {
  if (PLATFORM === 'win32' && process.env.LOCALAPPDATA) {
    return path.join(process.env.LOCALAPPDATA, 'anika')
  }
  return path.join(os.homedir(), '.anika')
})()
const VENV_ROOT = path.join(DEFAULT_ANIKA_HOME, 'anika-agent', 'venv')
const FRESH_SANDBOX_ROOT = path.join(os.tmpdir(), 'anika-desktop-fresh-install')
...
function ensurePackagedApp() {
  if (process.env.ANIKA_DESKTOP_SKIP_BUILD === '1' && exists(APP.binary)) {
    return
  }
...
function resolveDmgPath() {
  if (!exists(RELEASE_ROOT)) {
    return path.join(RELEASE_ROOT, `Anika-${PACKAGE_JSON.version}-${ARCH}.dmg`)
  }

  const prefix = `Anika-${PACKAGE_JSON.version}`
  const candidates = fs
  ...
  return candidates.length > 0
    ...
    : path.join(RELEASE_ROOT, `Anika-${PACKAGE_JSON.version}-${ARCH}.dmg`)
}

function resolveNsisPath() {
  // electron-builder NSIS artifactName template is 'Anika-${version}-${os}-${arch}.${ext}'
...
function ensureDmg() {
  ...
  if (process.env.ANIKA_DESKTOP_SKIP_BUILD === '1' && exists(resolveDmgPath())) {
    return
  }
...
function ensureNsis() {
  ...
  if (process.env.ANIKA_DESKTOP_SKIP_BUILD === '1' && resolveNsisPath()) {
    return
  }
...
function launchFresh() {
  ...
  const anikaHome = path.join(sandbox, 'anika-home')
  ...
  env.ANIKA_DESKTOP_CWD = cwd
  env.ANIKA_DESKTOP_IGNORE_EXISTING = '1'
  env.ANIKA_DESKTOP_TEST_MODE = 'fresh-install'
  env.ANIKA_DESKTOP_USER_DATA_DIR = userDataDir
  env.ANIKA_HOME = anikaHome
  delete env.ANIKA_DESKTOP_ANIKA
  delete env.ANIKA_DESKTOP_ANIKA_ROOT
  ...
  console.log(`  ANIKA_HOME: ${anikaHome}`)
  console.log(`  cwd: ${cwd}`)

  return { runtimeRoot: path.join(anikaHome, 'anika-agent', 'venv') }
}

// Validate the packaged bundle matches the thin-installer architecture:
//   - The Anika Agent Python payload is NOT shipped (it's fetched at first
...
function validateBundle() {
  ...
  // present anymore. If a stray ship of anika_cli sneaks back in we want
  // to fail loudly rather than re-introduce the 400MB delta we just removed.
  const staleFactoryMarker = path.join(APP.resourcesPath, 'anika-agent', 'anika_cli', 'main.py')
...
function help() {
  console.log(`Usage:
  npm run test:desktop:existing  # build packaged app, launch with normal PATH/existing Anika
  npm run test:desktop:fresh     # build packaged app, launch with temp userData + ANIKA_HOME
...
Fast rerun (skip rebuild if the packaged app already exists):
  ANIKA_DESKTOP_SKIP_BUILD=1 npm run test:desktop:fresh
`)
}
