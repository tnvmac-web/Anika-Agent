 ...
 * product name (`Anika`). If a PREVIOUS `npm run pack` was interrupted
 ...
 *   '.../release/linux-unpacked/electron' -> '.../release/linux-unpacked/Anika'
 *
 * This is a hard failure with no obvious cause for the user — `anika desktop`
 ...
 * (the mac-unpacked Anika.app bundle) and Windows (win-unpacked), so we
 ...
 * Anika.exe that Windows can't load (truncated PE from a corrupt cached
 * Electron zip, wrong arch), the updater's integrity gate in
 * `anika desktop --build-only` (anika_cli/main.py
 ...
 */
export function preserveRollbackBackup(appOutDir, productExeName = 'Anika.exe') {
...
export default async function beforePack(context) {
  ...
  try {
    ...
    // rename fails.
    const productExe = `${(context && context.packager?.appInfo?.productFilename) || 'Anika'}.exe`
