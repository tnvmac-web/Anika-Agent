...
async function setupSeededMockBackend(): Promise<MockBackendFixture> {
  ...
  fs.appendFileSync(
    path.join(sandbox.anikaHome, 'config.yaml'),
    '\ncompression:\n  threshold_tokens: 1\n',
    'utf8',
  )
  writeEnvFile(sandbox.anikaHome)
  const builder = await RealSessionBuilder.start(sandbox.anikaHome)
...
test('live verify-on-stop continuations stay out of the transcript', async ({}, testInfo) => {
  ...
  writeMockProviderConfig(sandbox.anikaHome, mock.url)
  fs.appendFileSync(path.join(sandbox.anikaHome, 'config.yaml'), '\nagent:\n  verify_on_stop: true\n', 'utf8')
