 ...
 * AIAgent loop, using the E2E mock provider configured in `anikaHome`.
 *
...
export class RealSessionBuilder {
  ...
  private constructor(anikaHome: string) {
  ...
  static async start(anikaHome: string): Promise<RealSessionBuilder> {
    const builder = new RealSessionBuilder(anikaHome)
