...
async function seedBot(anikaHome: string, mockUrl: string, name: string): Promise<void> {
  const dir = path.join(anikaHome, 'profiles', name)
  fs.mkdirSync(dir, { recursive: true })
...
test.beforeAll(async () => {
  ...
  for (const name of ['alpha', 'beta', 'gamma']) {
    await seedBot(sandbox.anikaHome, mock.url, name)
  }
...
test('file bots into user sections by menu and drag; rename; delete returns them to Unassigned', async () => {
  ...
  const alphaProfile = path.join(fixture!.sandbox.anikaHome, 'profiles', 'alpha', 'profile.yaml')
