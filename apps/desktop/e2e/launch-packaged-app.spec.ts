...
/**
 * E2E smoke tests for the packaged Anika desktop app.
 *
 ...
 * (credential stripping, isolated ANIKA_HOME + userData, unique app name).
 *
...
test('window opens with the Anika title', async () => {
...
test('HUD composer remains fully inside the transparent window', async () => {
  ...
  await fixture!.page.evaluate(() =>
    (window as typeof window & {
      anikaDesktop?: { hud?: { open: (options: { sessionId: null }) => Promise<void> } }
    }).anikaDesktop?.hud?.open({ sessionId: null })
  )
