 ...
 * packaged binary) with a real `anika serve` backend pointed at a mock
 * inference server. The full chain is exercised:
 *
 *   electron → anika serve (python) → mock provider → renderer
 *
...
test.describe('dev-mode boot with mock backend', () => {
  test('window opens with Anika title', async () => {
    const title = await fixture!.page.title()
    expect(title).toContain('Anika')
  })
  ...
  test('the preload bridge reaches the renderer', async () => {
    const bridge = await fixture!.page.evaluate(() => {
      const desktop = (window as unknown as { anikaDesktop?: Record<string, unknown> }).anikaDesktop
  ...
  test('backend boots and app becomes ready', async () => {
    ...
    // electron starts → anika serve is spawned → WS connects → config
