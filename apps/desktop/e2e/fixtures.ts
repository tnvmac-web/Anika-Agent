/**
 * Shared E2E fixtures for the Anika desktop Playwright suite.
 *
 * Two fixture modes:
 *
 ...
 *     (electron → anika serve → provider → inference → renderer) is
 ...
 * `electron-builder --dir` step and matches `anika desktop --source`. The
...
export function createSandbox(prefix: string): Sandbox {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), `anika-e2e-${prefix}-${Math.random()}`))
  const anikaHome = path.join(root, 'anika-home')
  const userDataDir = path.join(root, 'electron-user-data')

  fs.mkdirSync(anikaHome, { recursive: true })
...
export function writeMockProviderConfig(
  ...
  const configPath = path.join(anikaHome, 'config.yaml')
 ...
 */
export function writeEnvFile(anikaHome: string, apiKey = 'e2e-mock-key'): void {
  const envPath = path.join(anikaHome, '.env')
 ...
 */
function writeEmptyConfig(anikaHome: string): void {
  const configPath = path.join(anikaHome, 'config.yaml')
 ...
 *
 * Key env vars:
 *  - ANIKA_HOME → sandbox anika-home (isolated config/sessions)
 *  - ANIKA_DESKTOP_USER_DATA_DIR → sandbox electron-user-data
 *  - ANIKA_DESKTOP_IGNORE_EXISTING=1 → don't pick up `anika` from PATH
 *    (we want the dev checkout at REPO_ROOT)
 *  - ANIKA_DESKTOP_ANIKA_ROOT → REPO_ROOT (dev checkout resolution)
 *  - ANIKA_DESKTOP_APP_NAME → unique-ish per test (avoids single-instance lock)
...
export function buildAppEnv(sandbox: Sandbox, extra: Record<string, string> = {}): Record<string, string> {
  ...
  return {
    ...clean,
    ANIKA_HOME: sandbox.anikaHome,
    ANIKA_DESKTOP_USER_DATA_DIR: sandbox.userDataDir,
    ANIKA_DESKTOP_IGNORE_EXISTING: '1',
    ANIKA_DESKTOP_ANIKA_ROOT: REPO_ROOT,
    ANIKA_DESKTOP_APP_NAME: `AnikaE2E-${Date.now()}`,
    ...
    ANIKA_DESKTOP_SKIP_QUIT_CONFIRM: '1',
 ...
 *
 * @param sandbox  - isolated ANIKA_HOME + userData
 * @param env      - the process environment (already has ANIKA_HOME etc.)
...
export async function setupMockBackend(options: MockBackendOptions = {}): Promise<MockBackendFixture> {
  ...
  writeEnvFile(sandbox.anikaHome)
...
export async function setupNoProvider(): Promise<NoProviderFixture> {
  ...
  writeEmptyConfig(sandbox.anikaHome)
...
export interface DeadBackendOptions {
  /**
   * When true, inject a fake boot error via ANIKA_DESKTOP_BOOT_FAKE_ERROR
 ...
 * nothing listens on). By default the backend still boots (`anika serve`
...
export async function setupDeadBackend(options: DeadBackendOptions = {}): Promise<DeadBackendFixture> {
  ...
  const configPath = path.join(sandbox.anikaHome, 'config.yaml')
  fs.writeFileSync(
    configPath,
...
`,
  ...
  const env = buildAppEnv(sandbox, options.fakeError ? { ANIKA_DESKTOP_BOOT_FAKE_ERROR: 'Failed to connect to Anika backend: connection refused' } : {})
  const { app, page } = await launchDesktop(env)
...
function resolvePackagedBinaryPath(): string {
  if (process.platform === 'win32') {
    return path.join(RELEASE_ROOT, 'win-unpacked', 'Anika.exe')
  }

  if (process.platform === 'darwin') {
    const arch = process.arch === 'arm64' ? 'arm64' : 'x64'

    return path.join(RELEASE_ROOT, `mac-${arch}`, 'Anika.app', 'Contents', 'MacOS', 'Anika')
  }

  return path.join(RELEASE_ROOT, 'linux-unpacked', 'anika')
}
 ...
 * progress without spawning a real Anika backend.
 *
 * Uses the same sandbox isolation (credential stripping, isolated
 * ANIKA_HOME + userData, unique app name) as the dev-mode fixtures.
 *
...
export async function setupPackagedApp(): Promise<PackagedAppFixture> {
  ...
  const env = buildAppEnv(sandbox, {
    ...
    ANIKA_DESKTOP_BOOT_FAKE: '1',
    ANIKA_DESKTOP_BOOT_FAKE_STEP_MS: '120',
  })

  // Clear dev-server + anika-root overrides — the packaged binary
  // should use its own bundled renderer, not the dev checkout.
  delete (env as Record<string, string | undefined>).ANIKA_DESKTOP_DEV_SERVER
  delete (env as Record<string, string | undefined>).ANIKA_DESKTOP_ANIKA
  delete (env as Record<string, string | undefined>).ANIKA_DESKTOP_ANIKA_ROOT
