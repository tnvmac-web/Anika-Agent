...
function createGitRepo(root: string): string {
  ...
  execFileSync('git', ['config', 'user.name', 'Anika E2E'], { cwd: repo })
...
function configureRepoCwd(anikaHome: string, mockUrl: string, repo: string): void {
  writeMockProviderConfig(anikaHome, mockUrl)
  fs.appendFileSync(path.join(anikaHome, 'config.yaml'), `\nterminal:\n  cwd: ${repo}\n`, 'utf8')
  writeEnvFile(anikaHome)
}
...
test.beforeAll(async () => {
  ...
  configureRepoCwd(sandbox.anikaHome, mock.url, repo)
