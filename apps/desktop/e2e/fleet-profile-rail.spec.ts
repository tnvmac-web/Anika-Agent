 ...
 * second gateway, "Homelab", is a REAL second `anika serve` this spec spawns
 * with its own ANIKA_HOME, profiles and session token, registered in the v2
...
function findAnikaBinary(): string {
  const venv = path.join(REPO_ROOT, '.venv', 'bin', 'anika')

  if (fs.existsSync(venv)) {
    return venv
  }

  const result = spawnSync('which', ['anika'], { encoding: 'utf8' })

  if (result.status === 0 && result.stdout.trim()) {
    return result.stdout.trim()
  }

  throw new Error('anika binary not found: create the repo venv (uv sync) or put anika on PATH')
}
...
/**
 * Spawn a second, fully real `anika serve` as the remote gateway. Its
 * session token is pinned through ANIKA_DASHBOARD_SESSION_TOKEN so the
...
async function startRemoteGateway(root: string, mockUrl: string, profiles: string[]): Promise<RemoteGateway> {
  ...
  const child: ChildProcess = spawn(
    ...
    ['serve', '--host', '127.0.0.1', '--port', String(port), '--skip-build'],
    {
      ...
      env: {
        ...process.env,
        ANIKA_HOME: home,
        ANIKA_DASHBOARD_SESSION_TOKEN: REMOTE_TOKEN,
      },
  ...
  while (Date.now() < deadline) {
    if (child.exitCode !== null) {
      throw new Error(`remote anika serve exited early (${child.exitCode}):\n${log}`)
    }

    try {
      const response = await fetch(`${url}/api/status`, {
        headers: { 'X-Anika-Session-Token': REMOTE_TOKEN },
      })
  ...
  if (Date.now() >= deadline) {
    throw new Error(`remote anika serve never became ready:\n${log}`)
  }
...
test.describe('fleet profile rail — two registered gateways', () => {
  ...
  test.beforeAll(async () => {
    ...
    writeMockProviderConfig(sandbox.anikaHome, mock.url)
    ...
    seedProfiles(sandbox.anikaHome, ['research'])
