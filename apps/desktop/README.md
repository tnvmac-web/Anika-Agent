...
<p align="center">
  <a href="https://github.com/__NEW_ORG__/anika-agent/releases"><img src="https://img.shields.io/badge/Download-macOS%20%C2%B7%20Windows%20%C2%B7%20Linux-FFD700?style=for-the-badge" alt="Download"></a>
...
**The native desktop app for [Anika Agent](../../README.md) — the self-improving AI agent from [__NEW_ORG__](https://__NEW_DOMAIN__).** Same agent, same skills, same memory as the CLI and gateway, in a polished native window — chat with streaming tool output, side-by-side previews, a file browser, voice, and settings, no terminal required. Available for **macOS, Windows, and Linux**.

<table>
<tr><td><b>Chat with the full agent</b></td><td>Streaming responses, live tool activity, structured tool summaries, and the same conversation history as every other Anika surface.</td></tr>
<tr><td><b>Side-by-side previews</b></td><td>Render web pages, files, and tool outputs in a right-hand pane while you keep chatting.</td></tr>
<tr><td><b>File browser</b></td><td>Explore and preview the working directory without leaving the app.</td></tr>
<tr><td><b>Voice</b></td><td>Talk to Anika and hear it back.</td></tr>
...
### Install with Anika (recommended)

Already have the Anika CLI? Just run:

```bash
anika desktop
```

It builds and launches the GUI against your existing install — same config, keys, sessions, and skills. If Desktop cannot find a usable runtime or saved remote connection, first launch lets you connect to an existing Anika gateway or install Anika locally. Local onboarding then walks you through choosing a provider and model.

### Prebuilt installers

Prebuilt installers are built and distributed via [the Anika Desktop website.](https://anika-agent.__NEW_DOMAIN__/).
...
```bash
# throwaway ANIKA_HOME, separate Electron userData, distinct app name to avoid the single-instance lock
...
first launch it can install the Anika Agent runtime into `ANIKA_HOME`
(`~/.anika`, or `%LOCALAPPDATA%\anika` on Windows), using the same layout as a
CLI install.
...
- **Anika Agent** runs as a headless `anika serve` process and exposes the
...
1. `ANIKA_DESKTOP_ANIKA_ROOT`
2. the current source checkout during development
3. a completed managed install
4. `ANIKA_DESKTOP_ANIKA`, or `anika` on `PATH`
5. a system Python that can import the Anika runtime
...
Desktop supports a managed local backend, explicit remote gateways, and Anika
...
screen offers **Connect to existing Anika** before starting the local installer.
...
terminal commands, and file operations run against the remote Anika host, not
...
transport- or Anika-managed header names such as `Authorization`, `Cookie`,
`Host`, `Origin`, `Referer`, and `X-Anika-Session-Token`.
...
Boot logs land in `ANIKA_HOME/logs/desktop.log` (includes backend output and recent Python tracebacks) — check it first if the app reports a boot failure.
...
```powershell
# Force a clean first-launch setup
Remove-Item "$env:LOCALAPPDATA\anika\anika-agent\.anika-bootstrap-complete"
# Rebuild a broken Python venv
Remove-Item -Recurse -Force "$env:LOCALAPPDATA\anika\anika-agent\venv"
```

> The default Anika home on Windows is `%LOCALAPPDATA%\anika`. Set the `ANIKA_HOME` env var if you've relocated it.
...
- 📖 [Documentation](https://anika-agent.__NEW_DOMAIN__/docs/)
