...
export function watchInPage(doc: Document, holder: WatchHolder, stage: WatchStage, label?: string): void {
  const win = doc.defaultView as (Window & { __anikaWatch?: Record<string, unknown> }) | null
  ...
  const busy = 25
  const state = win.__anikaWatch || (win.__anikaWatch = {})
  ...
  const build = () => {
    ...
    const stale = doc.querySelectorAll('anika-watch')

    for (let i = 0; i < stale.length; i++) {
      stale[i].remove()
    }

    const host = doc.createElement('anika-watch')
  ...
  // this up" in dev.
  const stamp = (win as unknown as { __anikaWatchTag?: number }).__anikaWatchTag
