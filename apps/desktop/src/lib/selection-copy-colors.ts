...
// Anika is rendering, and when the two are opposite schemes it takes over
...
function renderedMode(doc: Document): RenderedMode {
  const attr = doc.documentElement.dataset.anikaMode
 ...
 * the only carriers of Anika' palette — are dropped so the paste target's
