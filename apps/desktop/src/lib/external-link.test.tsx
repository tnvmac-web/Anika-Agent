...
function installDesktopBridge(partial: Partial<Window['anikaDesktop']> = {}) {
...
function installTitleBridge(title: string) {
  ...
  installDesktopBridge({ fetchLinkTitle: bridge as unknown as Window['anikaDesktop']['fetchLinkTitle'] })
...
describe('external link helpers', () => {
  ...
  it('opens a web link in the in-app browser', async () => {
    ...
    installDesktopBridge({ openExternal: openExternal as unknown as Window['anikaDesktop']['openExternal'] })
  ...
  it('prefixes a pretty link to a known host with its brand glyph', () => {
    ...
    const url = 'https://github.com/__NEW_ORG__/anika-agent/pull/123'
