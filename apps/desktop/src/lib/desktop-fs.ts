import { anikaApi } from '@/api/client'
import type {
  AnikaConnection,
  AnikaReadDirResult,
  AnikaReadFileTextResult,
  AnikaSelectPathsOptions
} from '@/global'
...
function connectionCacheKey(connection: AnikaConnection | null) {
  if (!connection) {
    return 'local:'
  }
...
export function desktopFsCacheKey(connection: AnikaConnection | null = $connection.get()) {
...
export async function readDesktopDir(path: string): Promise<AnikaReadDirResult> {
  if (!isDesktopFsRemoteMode()) {
    return bridge().readDir(path)
  }

  return remoteFsApi<AnikaReadDirResult>(fsPath('list', path))
}

export async function readDesktopFileText(path: string): Promise<AnikaReadFileTextResult> {
  if (!isDesktopFsRemoteMode()) {
    return bridge().readFileText(path)
  }

  return remoteFsApi<AnikaReadFileTextResult>(fsPath('read-text', path))
}
...
export async function readDesktopFileDataUrlLocalFirst(path: string): Promise<string> {
  try {
    const local = await window.anikaDesktop?.readFileDataUrl?.(path)
...
export async function selectDesktopPaths(options?: AnikaSelectPathsOptions): Promise<string[]> {
  const desktop = bridge()
