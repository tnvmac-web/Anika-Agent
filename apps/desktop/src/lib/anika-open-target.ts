 ...
 * `anika://` deep links and `host.navigate('/path?…')`.
 *
 ...
 *  - `anika://index-network/intent/1` → `/index-network/intent/1` (plugin-scoped)
 *  - `anika://open/my-page?item=x` → `/my-page?item=x` (generic open)
 *  - `/my-page?item=x` / `#/my-page?item=x` (hash-router paths)
 */

export type AnikaOpenTarget = string | { href: string } | { path: string; params?: Record<string, string> }

const ANIKA_PROTOCOL = 'anika:'
...
export function normalizeAnikaOpenString(raw: string): string | null {
  ...
  if (trimmed.startsWith('anika://') || trimmed.startsWith(`${ANIKA_PROTOCOL}//`)) {
    try {
      const url = new URL(trimmed)
      const host = url.hostname || ''
      const rest = decodeURIComponent((url.pathname || '').replace(/^\//, ''))

      // anika://open/<path>?… → /<path>?…
      if (host === 'open') {
        if (!rest) {
          return null
        }
      ...
      // anika://index-network/intent/1 → /index-network/intent/1
...
export function resolveAnikaOpenPath(target: AnikaOpenTarget | null | undefined): string | null {
  if (target == null) {
    return null
  }

  if (typeof target === 'string') {
    return normalizeAnikaOpenString(target)
  }
  ...
  if ('href' in target && typeof target.href === 'string') {
    return normalizeAnikaOpenString(target.href)
  }

  if ('path' in target && typeof target.path === 'string') {
    const base = normalizeAnikaOpenString(target.path)
 ...
 * (`anika://<kind>/<name>?…` → kind/name/params).
 */
export function pathFromAnikaDeepLink(kind: string, name: string, params: Record<string, string> = {}): string | null {
  if (!kind || !name) {
    return null
  }

  if (kind === 'open') {
    return resolveAnikaOpenPath({ path: `/${name.replace(/^\//, '')}`, params })
  }
  ...
  return resolveAnikaOpenPath({ path: `/${kind}/${name.replace(/^\//, '')}`, params })
}

/** Convenience for `anika://open/<name>?…` payloads. */
export function pathFromOpenDeepLink(name: string, params: Record<string, string> = {}): string | null {
  return pathFromAnikaDeepLink('open', name, params)
}
