...
//
// When someone is in a hands-free "Hey Anika" voice chat, the natural way to
...
// phrase (optionally addressed to Anika), so a real turn that merely contains
...
// Optional address prefixes so "anika stop" / "ok stop" / "hey anika, stop"
// still count. Stripped before matching the core phrase.
const ADDRESS_PREFIXES: readonly string[] = ['hey anika', 'hey anika,', 'anika', 'anika,', 'ok', 'okay', 'hey']
...
function stripAddress(text: string): string {
  for (const prefix of ADDRESS_PREFIXES) {
    if (text === prefix) {
      // Bare address ("anika") is not a stop command on its own.
      continue
    }
 ...
 * to Anika). Returns false for anything that merely contains "stop" as part of
