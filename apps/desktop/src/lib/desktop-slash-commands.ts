...
/** Why a known Anika command has no desktop UI surface. */
...
const DESKTOP_COMMAND_SPECS: readonly DesktopCommandSpec[] = [
  ...
  { name: '/profile', description: 'Switch the active Anika profile', surface: action('profile') },
  {
    name: '/skin',
...
function isKnownAnikaSlashCommand(command: string): boolean {
  const normalized = normalizeCommand(command)
 ...
 * Anika' built-in slash commands — i.e. skill commands (`/gif-search`,
...
export function isDesktopSlashExtensionCommand(command: string): boolean {
  ...
  return !isKnownAnikaSlashCommand(normalized)
}
 ...
 * activity — the ones that ship with Anika and were never asked for. It is
