/**
 * Routines: the Anika cron jobs scoped to the bot you're chatting with — the
...
// anika -p <bot> chat delegation wrapper so the run reaches that bot's
...
export function routinePrompt(
  ...
  return (
    ...
    `anika -p ${shellQuote(bot)} chat -c ${shellQuote(`Routine: ${title}`)} -q ${shellQuote(`[Scheduled routine] ${instruction}`)}\n\n` +
...
// Anika-native schedule string; Advanced exposes it raw.
...
/** Compose the Anika schedule string from picker state. */
