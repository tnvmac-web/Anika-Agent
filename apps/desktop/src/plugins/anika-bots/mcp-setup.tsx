...
import { Button, host, Input, useI18n } from '@anika/plugin-sdk'
...
// and the row falls back to the "run anika mcp / Settings" hint. profile is
...
export function McpSetupButton({ profile, entry, onDone, ensureProfile }: McpSetupButtonProps) {
  ...
  const beginOAuth = async () => {
    ...
    const mcpOauthBridge =
      typeof window !== 'undefined' && window.anikaDesktop && window.anikaDesktop.mcpOauth
        ? window.anikaDesktop.mcpOauth
        : null
    ...
    try {
      ...
      } else if (typeof window !== 'undefined' && window.anikaDesktop && window.anikaDesktop.openExternal) {
        window.anikaDesktop.openExternal(authUrl)
      } else {
