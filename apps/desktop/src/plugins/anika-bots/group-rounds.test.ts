...
describe('routing', () => {
  ...
  it('resolves @anika to the default member', async () => {
    const { rounds } = await loadRoom()
    ...
    const parsed = rounds.parseGroupChatMentions('@anika take a look', members)
...
describe('turn prompt', () => {
  it('addresses the default profile as @anika', async () => {
    const { rounds } = await loadRoom()
    ...
    expect(own).toMatch(/You are @anika,/)
    ...
    expect(peer).toMatch(/group chat with @anika/)
  })
