...
export function displayName(bot: Partial<RosterRow>, meta?: BotMeta | null): string {
  ...
  // "Anika". Annotated active rows carry sourceScoped too, and keying this
  ...
  // Core-profile display name (profile.yaml, set via `anika profile rename
  ...
  // never generic "Anika" or a hostname-derived label.
  if (alias) {
  ...
  // that reads like nobody bothered. Present it as Anika (the agent it is)
