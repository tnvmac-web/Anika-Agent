...
import { atom, host } from '@anika/plugin-sdk'
...
const GROUP_CHAT_SYNC_META_KEY = 'anika-bots-groups'
...
/** The v3 envelope stored under the default profile's `anika-bots-groups`
 ...
 *  clients (Anika Go, headless backends) see rooms regardless of which
 ...
 *  renamed bot never shows up as its raw profile id or a stale "Anika"
 *  (community report, Aug 21 2026: renamed default still read "Anika is
 *  thinking…" in group rooms). The untitled primary profile is literally
 *  named "default" — render it as Anika (matching displayName and the
 *  @anika handle) so the main agent never loses its name in rooms. */
export function groupSpeakerLabel(name?: null | string) {
  ...
  // Core profile display_name (`anika profile rename …` / dashboard) from
  ...
  return trimmed.toLowerCase() === 'default' ? 'Anika' : trimmed
}
