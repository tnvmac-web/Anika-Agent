...
import { host, LruCache } from '@anika/plugin-sdk'
...
// to (local, remote URL, SSH, Anika Cloud, docker) must be able to find
...
// anika_cli/config_defaults.py and tui_gateway/methods_bot_relay.py and fails
  ...
  try {
    ...
    return profiles
      .map(profile => ({
        ...
        title: String(profile?.ui_meta?.['anika-bots']?.title || profile?.display_name || ''),
