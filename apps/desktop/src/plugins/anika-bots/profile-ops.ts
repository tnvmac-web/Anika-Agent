...
import { forgetSessionUnread, host, queryClient } from '@anika/plugin-sdk'
...
export function mergeServerMeta(roster: RosterRow[], fetchedAt = 0) {
  ...
  for (const bot of roster) {
    const server = bot.ui_meta?.['anika-bots']
...
/** Permanently delete a bot's Anika profile, then remove plugin-local state
 ...
 * directory (anika-agent#52279). That is the "can't delete a bot" error. */
