...
Drop a `<name>/plugin.{ts,tsx}` here that default-exports a `AnikaPlugin` and
...
`$ANIKA_HOME/desktop-plugins/<name>/plugin.js` (the disk door) — see the
`anika-desktop-plugins` skill.
