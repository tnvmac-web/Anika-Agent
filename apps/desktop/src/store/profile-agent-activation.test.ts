...
const agentConn = (over: Partial<AnikaConnection> = {}): AnikaConnection =>
  ({ baseUrl: 'https://homelab.invalid', mode: 'remote', profile: 'research', ...over }) as AnikaConnection

const localConn = (over: Partial<AnikaConnection> = {}): AnikaConnection =>
  ({ baseUrl: '', mode: 'local', profile: 'default', ...over }) as AnikaConnection

const getConnection = vi.fn<(profile?: string | null) => Promise<AnikaConnection>>()

const getConnectionFor =
  vi.fn<(payload: { connectionId?: null | string; profile?: null | string }) => Promise<AnikaConnection>>()
...
beforeEach(() => {
  ...
  vi.stubGlobal('window', { anikaDesktop: { getConnection, getConnectionFor }, localStorage })
...
describe('ensureGatewayAgent commit hook (beforeActivate) — the Sessions switcher door (#93937)', () => {
  ...
  it('a descriptor lookup that never answers is bounded, fails open, and releases the mutex', async () => {
    ...
    try {
      getConnectionFor.mockImplementationOnce(() => new Promise<AnikaConnection>(() => undefined))
