...
import type { AnikaGitWorktree, AnikaRepoStatus } from '@/global'
...
const EMPTY_WORKTREES: AnikaGitWorktree[] = []
...
export const $repoStatusByCwd = atom<Record<string, AnikaRepoStatus | null>>({})
export const $repoWorktreesByCwd = atom<Record<string, AnikaGitWorktree[]>>({})
...
export const $repoStatus: ReadableAtom<AnikaRepoStatus | null> = computed(
...
export const $repoWorktrees: ReadableAtom<AnikaGitWorktree[]> = computed(
...
const statusAtomByCwd = new Map<string, ReadableAtom<AnikaRepoStatus | null>>()
const worktreesAtomByCwd = new Map<string, ReadableAtom<AnikaGitWorktree[]>>()
const $noRepoStatus = atom<AnikaRepoStatus | null>(null)
const $noWorktrees = atom<AnikaGitWorktree[]>(EMPTY_WORKTREES)

/** Reactive status for one repo cwd (a tile's worktree). Stable per cwd. */
export function repoStatusForCwd(cwd?: null | string): ReadableAtom<AnikaRepoStatus | null> {
...
export function repoWorktreesForCwd(cwd?: null | string): ReadableAtom<AnikaGitWorktree[]> {
  const key = normalizeCwd(cwd)
...
function setRepoStatusEntry(target: string, status: AnikaRepoStatus | null): void {
...
function setRepoWorktreesEntry(target: string, worktrees: AnikaGitWorktree[]): void {
...
interface RepoStatusRefreshRequest {
  probe: (cwd: string) => Promise<AnikaRepoStatus | null>
  seq: number
  target: string
}
