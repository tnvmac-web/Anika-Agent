...
import type { AnikaGitBaseBranch, AnikaGitBranch } from '@/global'
import { getAnikaConfig, anikaApi, type AnikaGateway } from '@/anika'
...
import type { ProjectInfo, ProjectsPayload } from '@/types/anika'
...
const PROJECT_SCOPE_KEY = 'anika.desktop.projectScope'
...
async function gatewayRequest<T>(method: string, params: Record<string, unknown> = {}): Promise<T> {
  ...
  if (!gateway) {
    throw new Error('Anika gateway is not connected')
  }
...
async function activeProjectsContext(): Promise<ActiveProjectsContext> {
  ...
  if (!gateway || gateway !== activeGateway() || profile !== projectProfile()) {
    throw new Error('Active Anika profile changed while connecting')
  }
...
async function refreshProjectTreeAcrossProfiles(): Promise<void> {
  ...
  try {
    const res = await anikaApi<ProjectTreePayload>({
...
const repoScanStates = new WeakMap<AnikaGateway, RepoScanState>()
const scanningGatewayGenerations = new WeakMap<AnikaGateway, number>()
...
export async function scanAndRecordRepos(force = false): Promise<void> {
  if (isDesktopFsRemoteMode()) {
    ...
    // with `scan: true` — added in #81723) so repos with zero Anika
  ...
  try {
    const policy = repoDiscoveryPolicyFromConfig(await getAnikaConfig(context.profile))
...
// repo, returning where Anika should start working. Git is the source of
...
export async function listRepoBranches(repoPath: string): Promise<AnikaGitBranch[]> {
  const git = desktopGit()
...
export async function listBaseBranches(repoPath: string): Promise<AnikaGitBaseBranch[]> {
  const git = desktopGit()
...
export async function revealPath(path: null | string): Promise<void> {
  if (path) {
    await window.anikaDesktop?.revealPath?.(path)
  }
}
...
export async function copyPath(path: null | string): Promise<void> {
  if (path) {
    await window.anikaDesktop?.writeClipboard?.(path)
  }
}
