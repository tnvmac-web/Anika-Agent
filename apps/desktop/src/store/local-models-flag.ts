 ...
 *
 * Local models ship on main behind `--local` (either `anika desktop --local`
 * or the flag on Anika.exe itself). The flag is strict: without it the GUI
...
export const $localModelsEnabled = atom<boolean>(
  typeof window !== 'undefined' && window.anikaDesktop?.localModelsEnabled === true
)
