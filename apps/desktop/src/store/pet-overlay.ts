 ...
 * app's bounds and stays visible while Anika is minimized. That window carries
...
const OVERLAY_ACTIVE_KEY = 'anika.desktop.pet-overlay-active.v1'
const OVERLAY_BOUNDS_KEY = 'anika.desktop.pet-overlay-bounds.v1'
...
function pushNow(): void {
  window.anikaDesktop?.petOverlay?.pushState(currentPayload())
}
...
function openOverlay(request: PetOverlayOpenRequest): void {
  const api = window.anikaDesktop?.petOverlay
...
export function restorePetOverlay(): void {
  if (!window.anikaDesktop?.petOverlay || !$petOverlayActive.get() || stateUnsubs.length) {
    return
  }
...
export function popInPet(): void {
  ...
  stateUnsubs = []
  $petOverlayActive.set(false)
  void window.anikaDesktop?.petOverlay?.close()
}
