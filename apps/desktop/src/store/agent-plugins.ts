...
/**
 * Feature store for backend (agent) plugins — the native Anika plugins plus
 ...
 * primitives `anika plugins` and the dashboard use, so all surfaces agree on
...
export interface AgentPluginRow {
  ...
  /** Agent Plugins v1 package (portable skills/MCP format) vs native Anika. */
  portable?: boolean
}
 ...
 *  ANIKA_HOME. Always refetches (it's a cheap local disk scan on the
