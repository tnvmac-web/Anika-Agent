 ...
 *
 * The archive is the CLI's own `anika profile export` tar.gz (config, skills,
...
import { exportProfileArchive, importProfileArchive } from '@/anika'
...
import type { ProfileDesktopOverlay } from '@/types/anika'
...
const ARCHIVE_FILTERS = [{ extensions: ['tar.gz', 'tgz'], name: 'Anika profile' }]
...
export async function runExportProfileFlow(profile?: string): Promise<null | string> {
  ...
  const pick = window.anikaDesktop?.selectSavePath
...
export async function runImportProfileFlow(): Promise<null | string> {
  const paths = await window.anikaDesktop?.selectPaths?.({
