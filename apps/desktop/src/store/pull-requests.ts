...
import type { AnikaBranchPullRequest } from '@/global'
import { scanSessionPullRequests, type SessionInfo } from '@/anika'
...
export const $pullRequestsByBranch = atom<Record<string, AnikaBranchPullRequest>>({})
...
export const $prBranchBySession = persistentAtom<Record<string, string>>(
  'anika.desktop.prBranchBySession',
  {},
  Codecs.stringRecord
)
...
const $prScannedSessions = persistentAtom<string[]>('anika.desktop.prScannedSessions', [], Codecs.stringArray)
...
export function pullRequestBucket(pr: AnikaBranchPullRequest | undefined): PullRequestBucket {
  if (!pr) {
    return 'none'
  }
