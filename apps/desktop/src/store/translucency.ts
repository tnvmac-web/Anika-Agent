 ...
 * `@anika/shared/translucency`, which owns the mapping both this store and the
 * main process read.
 *
 ...
 * material underneath the web contents to read (see the `[data-anika-glass]`
 * block in styles.css).
 */
...
} from '@anika/shared/translucency'
 ...
 * sets `anikaDesktop.glassSupported` from `os.release()` so Win10 cannot
...
export const GLASS_SUPPORTED =
  typeof window !== 'undefined' && typeof window.anikaDesktop?.glassSupported === 'boolean'
    ? window.anikaDesktop.glassSupported
...
export const TRANSLUCENCY_SUPPORTED =
  typeof window !== 'undefined' && typeof window.anikaDesktop?.translucencySupported === 'boolean'
    ? window.anikaDesktop.translucencySupported
    : isMacPlatform() || isWindowsPlatform()
...
/* Peek: while the user is actively adjusting translucency from Settings, the
   ...
   [data-anika-translucency-peek] rules in styles.css). A counter rather
   than a boolean: a held slider drag and a timed pulse from a picker click
   can overlap. */
const PEEK_ATTR = 'data-anika-translucency-peek'
...
const applyGlassSurfaces = ({ intensity, mode, scope }: TranslucencyState): void => {
  ...
  root.toggleAttribute('data-anika-glass-on', glassLive)
  root.toggleAttribute('data-anika-glass', glassOn)
  root.toggleAttribute('data-anika-clear', clearOn)
  ...
  if (glassOn) {
    root.setAttribute('data-anika-glass-scope', scope)
  } else {
    root.removeAttribute('data-anika-glass-scope')
  }
...
if (typeof window !== 'undefined') {
  ...
  $translucency.subscribe(state => {
    ...
    window.anikaDesktop?.setTranslucency?.(state)
  })
