...
export function setZoomPercent(percent: number): void {
  window.anikaDesktop?.zoom?.setPercent(percent)
}

if (typeof window !== 'undefined' && window.anikaDesktop?.zoom) {
  void window.anikaDesktop.zoom.get().then(({ percent }) => $zoomPercent.set(percent))
  window.anikaDesktop.zoom.onChanged(({ percent }) => $zoomPercent.set(percent))
}
