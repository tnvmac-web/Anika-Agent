import { type ConnectionState, type GatewayEvent, registryBackendScopeKey, resolveGatewayWsUrl } from '@anika/shared'
import { atom } from 'nanostores'

import type { AnikaConnection } from '@/global'
import { AnikaGateway, setApiRequestConnection } from '@/anika'
...
function dialProfile(
  desktop: NonNullable<typeof window.anikaDesktop>,
  profile: string,
...
const isOpen = (gateway: AnikaGateway | null): boolean => gateway?.connectionState === 'open'
...
interface RegistryConfig {
  ...
  onActiveConnectionChanged?: (connection: AnikaConnection) => void
  /**
...
interface Secondary {
  ...
  connection: AnikaConnection | null
  gateway: AnikaGateway
...
interface GatewayRegistryState {
  ...
  primaryGateway: AnikaGateway | null
  ...
  $gateway: ReturnType<typeof atom<AnikaGateway | null>>
  $activeProfile: ReturnType<typeof atom<string>>
}

const STATE_KEY = Symbol.for('anika.desktop.gatewayRegistryState')
...
function createRegistryState(): GatewayRegistryState {
  return {
    ...
    $gateway: atom<AnikaGateway | null>(null),
...
export function setPrimaryGateway(gateway: AnikaGateway | null, profile = 'default'): void {
...
export function setPrimaryGatewayConnection(connection: Pick<AnikaConnection, 'connectionId'> | null): void {
...
export function activeGateway(): AnikaGateway | null {
  if (g.activeKey === g.primaryProfile) {
...
function applyActive(profile: string, activationEpoch: number): boolean {
  ...
  // Push the active scope's registry connection into the anika module (null
...
function publishActiveConnection(connection: AnikaConnection): void {
...
function createSecondary(profile: string, connectionId: null | string = null): Secondary {
  const gateway = new AnikaGateway()
...
async function gatewayForProfile(
  ...
  leaseRequest = false,
  spawnPriority: SpawnPriority = 'background'
): Promise<{ gateway: AnikaGateway | null; key: string; release: () => void; scopeProfile: boolean }> {
  const key = normKey(profile)
...
export async function requestGatewayForProfile<T>(
  ...
  try {
    if (!route.gateway) {
      throw new Error(`Anika gateway unavailable for profile "${route.key}"`)
    }
...
export async function requestGatewayForAgent<T>(
  ...
  if (!window.anikaDesktop?.getConnectionFor) {
    throw new Error('This Desktop build cannot dial registry connections. Update Anika Desktop.')
  }
...
export async function ensureActiveGatewayOpen(): Promise<AnikaGateway | null> {
  if (g.activeKey === g.primaryProfile) {
    return g.primaryGateway
  }
  ...
  if (!isOpen(entry.gateway)) {
    ...
    // "Anika gateway is not connected" on the Sessions `+` action (#88880).
