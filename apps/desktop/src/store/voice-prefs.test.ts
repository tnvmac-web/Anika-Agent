...
vi.mock('@/anika', () => ({
  getAnikaConfigRecord: vi.fn(async () => ({})),
  saveAnikaConfig: vi.fn(async () => undefined)
}))
...
describe('applyVoiceStopPhraseFromConfig', () => {
  ...
  it('uses the first configured phrase so a custom phrase renders correctly', () => {
    applyVoiceStopPhraseFromConfig({ voice: { stop_phrases: ['goodbye anika', 'stop'] } })
    expect($voiceStopPhrase.get()).toBe('goodbye anika')
  })
