...
describe('composer model persistence scope', () => {
  ...
  it('keeps inferred local-primary connections on the historical bare keys', () => {
    ...
    window.localStorage.setItem('anika.desktop.composer.model', 'legacy-model')
    window.localStorage.setItem('anika.desktop.composer.provider', 'legacy-provider')
    ...
    expect(window.localStorage.getItem('anika.desktop.composer.model')).toBe('next-model')
    expect(window.localStorage.getItem('anika.desktop.composer.model.registry.local.default')).toBeNull()
  })
...
describe('session owner hints', () => {
  ...
  it('ignores malformed persisted entries and never throws on hydrate', () => {
    window.localStorage.setItem(
      'anika.desktop.sessionOwnerHints.v1',
      JSON.stringify([
        'junk',
        ['no-route', null],
...
describe('workspaceCwdForNewSession', () => {
  afterEach(() => {
    ...
    window.localStorage.removeItem('anika.desktop.workspace-cwd')
    window.localStorage.removeItem('anika.desktop.workspace-cwd.remote.http%3A%2F%2Fbackend-a.default')
    window.localStorage.removeItem('anika.desktop.workspace-cwd.remote.http%3A%2F%2Fbackend-b.default')
    delete (window as { anikaDesktop?: unknown }).anikaDesktop
  })
  ...
  it('does not publish a delayed configured default after ownership is lost', async () => {
    ...
    ;(window as { anikaDesktop?: unknown }).anikaDesktop = {
  ...
  it('prefers the configured default over the sticky remembered workspace', () => {
    window.localStorage.setItem('anika.desktop.workspace-cwd', '/home/user/sticky')
  ...
  it('keeps remote workspace memory separate from local and other remotes', () => {
    window.localStorage.setItem('anika.desktop.workspace-cwd', '/local/project')
    $currentCwd.set('/live/session/path')
...
describe('remembered session id (per profile)', () => {
  ...
  it('discards legacy unsuffixed keys on first read (zero-migration, refuse-to-guess)', () => {
    ...
    localStorage.setItem('anika.desktop.lastSessionId', 'legacy-session')
    ...
    expect(localStorage.getItem('anika.desktop.lastSessionId')).toBeNull()
  })
  ...
  it('uses encodeURIComponent so profile names with reserved chars are isolated', () => {
    ...
    expect(localStorage.getItem('anika.desktop.lastSessionId.profile.research%2Fops')).toBe('ops-session')
...
describe('remembered route (per profile)', () => {
  ...
  it('discards legacy unsuffixed keys on first read (zero-migration, refuse-to-guess)', () => {
    localStorage.setItem('anika.desktop.lastRoute', '/skills')
    ...
    expect(localStorage.getItem('anika.desktop.lastRoute')).toBeNull()
  })
  ...
  it('uses encodeURIComponent so profile names with reserved chars are isolated', () => {
    ...
    expect(localStorage.getItem('anika.desktop.lastRoute.profile.research%2Fops')).toBe('/cron')
    expect(getRememberedRoute('research')).toBeNull()
  })
