...
import { getAnikaConfigRecord, saveAnikaConfig } from '@/anika'
...
export async function setAutoSpeakReplies(enabled: boolean): Promise<void> {
  ...
  try {
    const record = await getAnikaConfigRecord()
    const voice = record.voice && typeof record.voice === 'object' ? (record.voice as Record<string, unknown>) : {}

    await saveAnikaConfig({ ...record, voice: { ...voice, auto_tts: enabled } })
  } catch (error) {
