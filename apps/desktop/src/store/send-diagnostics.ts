...
// view link plus the support handoff (GitHub Issues · __NEW_ORG__ Portal Support ·
// Discord).
//
// Consent is per-upload and explicit — no "always allow", mirroring the CLI's
// `anika debug share --nous` confirmation contract. On a remote connection
...
async function collectLocalExtras(): Promise<Record<string, string>> {
  try {
    const logs = await window.anikaDesktop?.getRecentLogs?.()
