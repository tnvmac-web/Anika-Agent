import { LOCAL_CONNECTION_ID } from '@anika/shared'
import { atom, batch, computed } from 'nanostores'

import type { AnikaConnection } from '@/global'
import { getProfiles, anikaApi, setApiRequestProfile, STARTUP_REQUEST_TIMEOUT_MS } from '@/anika'
...
import type { ProfileInfo } from '@/types/anika'
...
// /api/profiles/active `current`). "default" is the root ~/.anika. This is the
...
const PROFILE_ORDER_STORAGE_KEY = 'anika.desktop.profileOrder'
...
const PROFILE_COLORS_STORAGE_KEY = 'anika.desktop.profileColors'
...
export async function refreshActiveProfile(): Promise<void> {
  ...
  try {
    const res = await anikaApi<ActiveProfileResponse>({
...
// Persist the choice and relaunch the backend under the new ANIKA_HOME. The
...
export async function switchProfile(name: string): Promise<void> {
  ...
  await window.anikaDesktop.profile.set(name)
}
...
async function resolveConnectionForProfile(profile: string): Promise<AnikaConnection | null> {
  const getConnection = window.anikaDesktop?.getConnection
...
// previous descriptor.
async function resolveConnectionForAgent(connectionId: string, profile: string): Promise<AnikaConnection | null> {
  const getConnectionFor = window.anikaDesktop?.getConnectionFor
...
const SHOW_ALL_PROFILES_STORAGE_KEY = 'anika.desktop.showAllProfiles'
...
export function selectProfile(name: string): void {
  ...
  void Promise.all([activateOnCurrentSource(target), shouldRememberStartupProfile])
    .then(([, shouldRemember]) => {
      if (shouldRemember) {
        return window.anikaDesktop?.profile?.remember(target)
      }
...
async function isLocalDesktopProfile(target: string): Promise<boolean> {
  const getConnectionConfig = window.anikaDesktop?.getConnectionConfig
...
// Switch to the default (root ~/.anika) profile — bound to ⌘1.
...
export function touchActiveGatewayBackend(): void {
  ...
  void window.anikaDesktop?.touchBackend?.(target).catch(() => undefined)
}
