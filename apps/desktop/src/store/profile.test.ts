...
const { getProfiles } = await import('@/anika')
...
const remoteConn = (over: Partial<AnikaConnection> = {}): AnikaConnection =>
  ({ baseUrl: 'https://anika-roy.tail.ts.net', mode: 'remote', profile: 'vps-remote', ...over }) as AnikaConnection
...
beforeEach(() => {
  ...
  $profiles.set([])
  vi.stubGlobal('window', { anikaDesktop: { getConnection } })
