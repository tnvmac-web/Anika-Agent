 ...
 *  Working in `~/www/anika-agent`, the draft says "anika-agent" constantly
 *  — a "Use skill: anika-agent" pill on every mention is noise (the reported
 *  annoyance that prompted this guard). Boundary containment, not exact
 *  segment equality, so worktree dirs (`anika-agent-suggest`) suppress too. */
 ...
 *  segment of a qualified form (`github/anika-agent-dev`, `plugin:skill`). */
