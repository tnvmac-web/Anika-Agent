import type { ConnectionState } from '@anika/shared'
...
import type { SessionInfo, UsageStats } from '@/types/anika'
...
const COMPOSER_MODEL_KEY = 'anika.desktop.composer.model'
const COMPOSER_PROVIDER_KEY = 'anika.desktop.composer.provider'
const COMPOSER_MODEL_SOURCE_KEY = 'anika.desktop.composer.model-source'
const COMPOSER_EFFORT_KEY = 'anika.desktop.composer.reasoning-effort'
const COMPOSER_FAST_KEY = 'anika.desktop.composer.fast'
...
function composerScopeForConnection(connection: AnikaConnection | null): string | null {
  if (!connection) {
    return null
  }
...
const LAST_SESSION_KEY = 'anika.desktop.lastSessionId'
const LAST_ROUTE_KEY = 'anika.desktop.lastRoute'
...
function workspaceCwdKey(connection: AnikaConnection | null = $connection.get()): string {
...
export async function syncConfiguredDefaultProjectDir(shouldPublish: () => boolean = () => true): Promise<string> {
  const settings = window.anikaDesktop?.settings?.getDefaultProjectDir
...
export async function ensureDefaultWorkspaceCwd(shouldPublish: () => boolean = () => true): Promise<void> {
  const sanitize = window.anikaDesktop?.sanitizeWorkspaceCwd
...
export const $connection = atom<AnikaConnection | null>(null)
...
const SESSION_OWNER_HINTS_KEY = 'anika.desktop.sessionOwnerHints.v1'
 ...
 * AnikaConnection lookup is unavailable. */
...
export const setConnection = (next: Updater<AnikaConnection | null>) => {
...
// until config loads, and re-seeded on every profile switch by useAnikaConfig.
