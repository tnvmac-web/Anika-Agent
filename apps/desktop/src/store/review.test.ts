...
// reaches the gateway); everything else routes through window.anikaDesktop.git,
...
function file(path: string, over: Partial<AnikaReviewFile> = {}): AnikaReviewFile {
  return { path, status: 'modified', staged: false, added: 1, removed: 0, ...over } as AnikaReviewFile
}

type ReviewStub = Record<string, ReturnType<typeof vi.fn>>

// Install a review bridge on window.anikaDesktop. Any op not supplied defaults
...
describe('ship flow', () => {
  ...
  it('createOrOpenPr opens the existing PR without creating a new one', async () => {
    const review = stubReview()
    $reviewShipInfo.set({ ghReady: true, pr: { url: 'https://example.com/pr/9' } } as AnikaReviewShipInfo)

    await createOrOpenPr()

    expect(review.createPr).not.toHaveBeenCalled()
    expect(
      (window.anikaDesktop as unknown as { openExternal: ReturnType<typeof vi.fn> }).openExternal
...
describe('refreshShipInfo', () => {
  it('populates ship info from the bridge', async () => {
    const info: AnikaReviewShipInfo = {
      ghReady: true,
      pr: { url: 'https://example.com/pr/3' }
    } as AnikaReviewShipInfo
  ...
  it('resets ship info when there is no bridge', async () => {
    delete (window as unknown as { anikaDesktop?: unknown }).anikaDesktop
    $reviewShipInfo.set({ ghReady: true, pr: { url: 'x' } } as AnikaReviewShipInfo)
