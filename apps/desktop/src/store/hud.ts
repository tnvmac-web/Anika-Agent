 ...
 * composer with the reply scrolling above it, so Anika can be driven while
...
export const canUseHud = (): boolean =>
  typeof window !== 'undefined' && typeof window.anikaDesktop?.hud?.open === 'function'

export function openHud(sessionId?: null | string): void {
  const api = window.anikaDesktop?.hud
...
export function resetHudLayout(): void {
  void window.anikaDesktop?.hud?.resetLayout?.()
}
...
export const reportHudSession = (sessionId: null | string): void => window.anikaDesktop?.hud?.setSession?.(sessionId)
...
export function watchHudState(onClosed?: (sessionId: null | string) => void): () => void {
  const off = window.anikaDesktop?.hud?.onChanged?.(({ open, sessionId }) => {
    $hudActive.set(open)
