...
import type { AnikaReviewFile, AnikaReviewShipInfo } from '@/global'
...
//
// Scope is always "uncommitted" — Anika' flow is agent edits you review BEFORE
...
const OPEN_KEY = 'anika.desktop.reviewOpen'
const COMMIT_DEFAULT_KEY = 'anika.desktop.reviewCommitDefault'
const TREE_MODE_KEY = 'anika.desktop.reviewTreeMode'
const SELECTED_KEY = 'anika.desktop.reviewSelectedPath'
...
export const $reviewFiles = atom<AnikaReviewFile[]>([])
...
export const $reviewShipInfo = atom<AnikaReviewShipInfo>({ ghReady: false, pr: null })
...
type ReviewBridge = NonNullable<NonNullable<NonNullable<Window['anikaDesktop']>['git']>['review']>
...
export async function selectReviewFile(file: AnikaReviewFile): Promise<void> {
...
function matchReviewFile(files: readonly AnikaReviewFile[], path: string): AnikaReviewFile | undefined {
...
export async function createOrOpenPr(): Promise<void> {
  ...
  if (existing?.url) {
    void window.anikaDesktop?.openExternal?.(existing.url)
  ...
  await runShip(async () => {
    ...
    if (url) {
      void window.anikaDesktop?.openExternal?.(url)
    }
