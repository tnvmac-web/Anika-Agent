/**
 * @anika/plugin-sdk — THE plugin language. The vscode-module model: plugin
 ...
 *    object as `window.__ANIKA_PLUGIN_SDK__` and maps the import to it, so a
...
import { deleteProfile, getLogs, getStatus, anikaApi, type AnikaGateway } from '@/anika'
import {
  $gateway,
...
import type { PaginatedSessions, UsageStats } from '@/types/anika'
...
export interface PluginProfileRoute {
  ...
  profile: string
  /** Backend Anika profile served by that route. */
...
async function requestPluginProfile<T>(
  ...
  const getAgentRoster = window.anikaDesktop?.getAgentRoster
...
async function pluginRouteStillRegistered(route: PluginProfileRoute): Promise<boolean> {
  const getProfileRoutes = window.anikaDesktop?.getProfileRoutes
...
//
// ensureGatewayProfile awaits the store's dial, and AnikaGateway.connect() has
...
export const host = {
  ...
  deleteProfile: async (profile: string | PluginProfileRoute): Promise<void> => {
    ...
    // restores on relaunch and re-creates the deleted profile (anika-agent#94235).
    dropTilesForProfile(
  ...
  connections: async () => {
    const bridge = window.anikaDesktop?.connections

    if (!bridge) {
      throw new Error('This Desktop build has no connection registry. Update Anika Desktop.')
    }
  ...
  agents: async () => {
    const roster = window.anikaDesktop?.getAgentRoster

    if (!roster) {
      throw new Error('This Desktop build cannot enumerate multi-source agents. Update Anika Desktop.')
    }
  ...
  newChat: (profile?: null | string | PluginProfileRoute, options: PluginNewChatOptions = {}): void => {
    if (options.workspaceMode === 'bots') {
      ...
      if (!openTab) {
        notify({ kind: 'error', message: 'Update Anika Desktop to open another Bot chat.' })
   ...
   *
   *  `isStaleTile` (anika-agent#90102): the caller's reconciliation probe
  ...
  profileRoutes: async () => {
    ...
    if (!getProfileRoutes) {
      throw new Error('Anika Desktop connection routing unavailable')
    }
  ...
  listPersistedSessions: async (
    ...
    return anikaApi<PaginatedSessions>({
  ...
  setPersistedSessionHidden: async (
    ...
    return anikaApi<{ ok: boolean; hidden: boolean }>({
      ...(route ? { connectionId: route.connectionId } : {}),
  ...
  request: async <T>(method: string, params: Record<string, unknown> = {}): Promise<T> => {
    ...
    if (!gateway) {
      throw new Error('Anika gateway unavailable')
    }
 ...
 *  "ANIKA AGENT" with it, a `chat.empty` contribution writes its own name. */
...
export type { AnikaGateway } from '@/anika'
...
export type { AnikaOpenTarget } from '@/lib/anika-open-target'
...
/** Anika' reasoning levels + their compact labels, so a plugin surfacing a
...
export type { RpcEvent, StatusResponse } from '@/types/anika'
