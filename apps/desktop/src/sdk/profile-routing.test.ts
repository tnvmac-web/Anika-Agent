...
vi.mock('@/anika', () => ({ deleteProfile: vi.fn(), getLogs: vi.fn(), getStatus: vi.fn(), anikaApi: vi.fn() }))
...
const { deleteProfile, anikaApi } = await import('@/anika')
...
describe('connection-aware plugin host APIs', () => {
  it('retires a profile gateway before deleting it', async () => {
    ...
    // profile's backend, re-creating its ANIKA_HOME (#94235).
  ...
  it('refreshes the profile inventory before asking Electron for routes', async () => {
    ...
    ;(window as unknown as { anikaDesktop: unknown }).anikaDesktop = { getProfileRoutes }
  ...
  it('reads and hides persisted sessions through the source primary without activating the profile', async () => {
    ...
    expect(anikaApi).toHaveBeenNthCalledWith(
      1,
    ...
    expect(anikaApi).toHaveBeenNthCalledWith(2, {
      connectionId: 'source-a',
      path: '/api/sessions/bot-chat',
      method: 'PATCH',
      body: { hidden: true, profile: 'backend-worker' }
    })
    expect(vi.mocked(anikaApi).mock.calls.every(([request]) => !('profile' in request))).toBe(true)
