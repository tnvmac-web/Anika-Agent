import type { GatewayWsUrlResult } from '@anika/shared'
import type { TranslucencyState } from '@anika/shared/translucency'
...
declare global {
  interface Window {
    anikaDesktop: {
      ...
      // Resume this session in the user's own terminal emulator (`anika --tui
      ...
      // Anika Cloud: one portal login powers discovery + silent per-agent
      ...
      profile: {
        ...
        // backend under the new ANIKA_HOME (reloads the window). Pass null to
        // clear the preference.
        set: (name: string | null) => Promise<DesktopActiveProfile>
      }
      api: <T>(request: AnikaApiRequest) => Promise<T>
      notify: (payload: AnikaNotification) => Promise<boolean>
      ...
      readFileText: (filePath: string) => Promise<AnikaReadFileTextResult>
      ...
      readPluginSource?: (filePath: string) => Promise<AnikaReadFileTextResult>
      selectPaths: (options?: AnikaSelectPathsOptions) => Promise<string[]>
      ...
      normalizePreviewTarget: (target: string, baseDir?: string) => Promise<AnikaPreviewTarget | null>
      watchPreviewFile: (url: string) => Promise<AnikaPreviewWatch>
      ...
      watchDirectory?: (dir: string) => Promise<AnikaPreviewWatch>
      stopPreviewFileWatch: (id: string) => Promise<boolean>
      setActiveWork?: (payload: AnikaActiveWork) => void
      setTitleBarTheme?: (payload: AnikaTitleBarTheme) => void
      ...
      readDir: (path: string) => Promise<AnikaReadDirResult>
      ...
      // Local Desktop runtime-plugin root (<ANIKA_HOME>/desktop-plugins),
      ...
      /** LOCAL `<ANIKA_HOME>/logs` (profile-aware) — error card "Open Logs". */
      logsRoot?: () => Promise<string>
      // Local AGENT-plugin root (<ANIKA_HOME>/plugins), same Electron-local
      ...
      git?: {
        worktreeList: (repoPath: string) => Promise<AnikaGitWorktree[]>
        worktreeAdd: (
          repoPath: string,
        ...
        branchList: (repoPath: string) => Promise<AnikaGitBranch[]>
        ...
        baseBranchList: (repoPath: string) => Promise<AnikaGitBaseBranch[]>
        ...
        repoStatus: (repoPath: string) => Promise<AnikaRepoStatus | null>
        ...
        review: {
          list: (repoPath: string, scope: AnikaReviewScope, baseRef?: null | string) => Promise<AnikaReviewList>
          diff: (
            repoPath: string,
            filePath: string,
            scope: AnikaReviewScope,
            baseRef?: null | string,
            staged?: boolean
          ) => Promise<string>
          ...
          shipInfo: (repoPath: string) => Promise<AnikaReviewShipInfo>
          ...
          prList: (repoPath: string, branches: string[], numbers?: number[]) => Promise<AnikaRepoPullRequests>
          ...
          fetchPrComment: (repoPath: string, url: string) => Promise<AnikaPrComment | null>
      ...
      terminal: {
        ...
        onExit: (id: string, callback: (payload: AnikaTerminalExit) => void) => () => void
        resize: (id: string, size: { cols: number; rows: number }) => Promise<boolean>
        start: (options?: { cols?: number; cwd?: string; rows?: number }) => Promise<AnikaTerminalSession>
      ...
      onWindowStateChanged?: (callback: (payload: AnikaWindowState) => void) => () => void
      ...
      onPreviewFileChanged: (callback: (payload: AnikaPreviewFileChanged) => void) => () => void
...
export interface AnikaTerminalSession {
  cwd: string
  id: string
  shell: string
}

export interface AnikaTerminalExit {
  code: number | null
  signal: string | null
}
...
export interface DesktopUpdateApplyResult {
   ...
   *  `anika update` themselves. `command` is the exact line to run. */
  manual?: boolean
  command?: string
  anikaRoot?: string
...
export interface AnikaConnection {
  baseUrl: string
  darwinMajor?: number
...
export interface AnikaTitleBarTheme {
  background: string
  foreground: string
}

/** Turns in flight, so the main process can confirm before a quit kills them. */
export interface AnikaActiveWork {
  count: number
  titles: string[]
}

export interface AnikaWindowState {
  darwinMajor?: number
  isFullscreen: boolean
  isMinimized?: boolean
  isVisible?: boolean
...
export interface DesktopConnectionConfig {
  envOverride: boolean
  // The saved connection mode. 'cloud' is a Anika Cloud connection: it carries
  ...
  remoteUrl: string
  // For a 'cloud' connection: the persisted Anika Cloud org (slug or id) the
...
export interface DesktopConnectionConfigInput {
  ...
  remoteUrl?: string
  // For a 'cloud' connection: the selected Anika Cloud org (slug or id) to
...
export interface DesktopCloudStatus {
  ...
  // Whether the OAuth partition holds a live __NEW_ORG__ portal (Privy) session — the
  // portal authenticates via Privy, so this reflects the privy-token cookie, NOT
  // the anika gateway session cookies. See cookiesHavePrivySession.
  signedIn: boolean
}

// A discovered Anika Cloud agent — the trimmed DTO from NAS GET /api/agents.
...
export interface DesktopBootProgress {
  error: string | null
  fakeMode: boolean
  /** True when the boot failure is a __NEW_ORG__ Cloud agent that is down (HTTP 502/503/504). */
...
export interface AnikaApiRequest {
  path: string
  method?: string
  body?: unknown
...
export interface AnikaNotification {
  title?: string
  body?: string
  silent?: boolean
  kind?: string
...
export interface AnikaPreviewTarget {
  binary?: boolean
  byteSize?: number
  kind: 'file' | 'url'
  label: string
...
export interface AnikaReadFileTextResult {
  binary?: boolean
  byteSize?: number
  language?: string
  mimeType?: string
...
export interface AnikaPreviewWatch {
  id: string
  path: string
}
...
export interface AnikaGitWorktree {
  path: string
  branch: null | string
  isMain: boolean
  detached: boolean
...
export interface AnikaGitBranch {
  name: string
  checkedOut: boolean
  isDefault: boolean
  isRemote: boolean
...
// preselect it.
export interface AnikaGitBaseBranch {
  name: string
  isRemote: boolean
  isDefault: boolean
}
...
export interface AnikaRepoStatusFile {
  path: string
  staged: boolean
  unstaged: boolean
  untracked: boolean
...
export interface AnikaRepoStatus {
  branch: null | string
...
// turn began.
export type AnikaReviewScope = 'branch' | 'lastTurn' | 'uncommitted'

// One changed file in the review pane (status letter, +/- lines, staged flag).
export interface AnikaReviewFile {
  path: string
  added: number
  removed: number
...
export interface AnikaReviewList {
  files: AnikaReviewFile[]
...
export interface AnikaReviewPr {
  url: string
  state: string
  number: number
}
...
export interface AnikaBranchPullRequest {
  branch: string
  draft: boolean
  number: number
...
export interface AnikaRepoPullRequests {
  ghReady: boolean
  prs: AnikaBranchPullRequest[]
}
...
export interface AnikaPrComment {
  author: string
  body: string
  diffHunk: string
  kind: 'issue' | 'review'
...
export interface AnikaReviewShipInfo {
  ghReady: boolean
  pr: AnikaReviewPr | null
}

export interface AnikaReadDirEntry {
  name: string
  path: string
  isDirectory: boolean
}

export interface AnikaReadDirResult {
  entries: AnikaReadDirEntry[]
  error?: string
}

export interface AnikaPreviewFileChanged {
  id: string
  path: string
  url: string
}

export interface AnikaSelectPathsOptions {
  title?: string
  defaultPath?: string
  directories?: boolean
  multiple?: boolean
