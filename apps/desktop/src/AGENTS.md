...
## The desktop is its own chat surface on a `anika serve` backend
...
(`@anika/shared`: `JsonRpcGatewayClient` + WS URL helpers), which the web dashboard also consumes.
The desktop has **no build/runtime dependency on the dashboard frontend**: it spawns a headless
`anika serve` (`headless_backend=True` → `cmd_dashboard` skips `_build_web_ui` and exports
`ANIKA_SERVE_HEADLESS=1` so `mount_spa()` disables the SPA even if a stray `web_dist/` exists).
`dashboard` and `serve` share `cmd_dashboard`/`start_server` but neither launches the other. It does
NOT embed `anika --tui` — own composer, transcript, slash pipeline.
...
and ONLY when it does not (older managed install / PATH `anika` not yet updated) rewrites argv to
...
## Bot Mode (`src/plugins/anika-bots/`) — one bot = ONE canonical forever-chat, identified by NAME

Each bot is a Anika **profile** with a persistent identity. This invariant regressed repeatedly,
...
**There is NO session-id pin.** The old design stored a pointer in `ui_meta['anika-bots'].chat`;
...
in `src/plugins/anika-bots/`: `canonical-chat-registry.test.ts` (tripwire: the open path never
