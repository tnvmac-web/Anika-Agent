...
export function BootFailureOverlay() {
  ...
  const retry = async () => {
    setBusy('retry')
    await window.anikaDesktop?.resetBootstrap().catch(() => undefined)
    window.location.reload()
  }

  const repair = async () => {
    setBusy('repair')
    await window.anikaDesktop?.repairBootstrap().catch(() => undefined)
    window.location.reload()
  }
  ...
  const switchToLocalGateway = async () => {
    ...
    await window.anikaDesktop?.applyConnectionConfig({ mode: 'local' }).catch(() => undefined)
    setBusy(null)
  }

  // Clear this gateway's stale auth first, then re-establish it through the
  // connection's owning login flow. Anika Cloud must reuse its portal session
  ...
  const openLogs = () => void window.anikaDesktop?.revealLogs().catch(() => undefined)
  ...
  let hint: string
  // The electron boot path flags a __NEW_ORG__ Cloud backend-down (502/503/504) with
  ...
  if (remoteReauth) {
    ...
    // A __NEW_ORG__ Cloud agent is down — the user cannot restart the managed
    ...
    actions = [
      {
        ...
        onClick: () => openExternalLink('https://portal.__NEW_DOMAIN__'),
        icon: <ExternalLink />
      },
      localAction,
      { ...retryAction, variant: 'secondary' },
      {
        ...
        onClick: () => openExternalLink('https://discord.gg/__NEW_ORG__'),
        variant: 'ghost'
      },
