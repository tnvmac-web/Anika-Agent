...
function sessionsBotsTree() {
  ...
  registerPane('anika-bots:pane', { placement: 'left', hideOnly: true })
  registerPane('workspace', { placement: 'main', uncloseable: true })
  $layoutTree.set(
    split('row', [
      group(['sessions', 'anika-bots:pane'], { active: 'sessions', id: 'g-side' }),
...
describe('hide-only strip tabs', () => {
  it('hides and shows a chrome tab, keeping the pane in the tree', () => {
    ...
    expect(setStripTabHidden('anika-bots:pane', true)).toBe(true)
    expect($hiddenTreePanes.get()).toContain('anika-bots:pane')
    expect($hiddenStripTabs.get()).toContain('anika-bots:pane')
    // Hidden, not dismissed: the pane stays in the layout tree.
    expect(allPaneIds($layoutTree.get()!)).toContain('anika-bots:pane')

    expect(setStripTabHidden('anika-bots:pane', false)).toBe(true)
    expect($hiddenTreePanes.get()).not.toContain('anika-bots:pane')
    expect($hiddenStripTabs.get()).not.toContain('anika-bots:pane')
  })

  it('refuses to hide the zone last visible tab', () => {
    sessionsBotsTree()
    setStripTabHidden('anika-bots:pane', true)
  ...
  it('persists hides and clears them on reveal', () => {
    ...
    const persisted = JSON.parse(window.localStorage.getItem('anika.desktop.hiddenStripTabs.v1') ?? '[]')

    expect(persisted).toContain('anika-bots:pane')
    ...
    revealTreePane('anika-bots:pane')
    expect($hiddenTreePanes.get()).not.toContain('anika-bots:pane')
    expect(window.localStorage.getItem('anika.desktop.hiddenStripTabs.v1')).toBeNull()
  })
  ...
  it('lists the zone hide-only tabs with live hidden state for the menu', () => {
    ...
    expect(hideOnlyZoneTabs('g-side')).toEqual([
      ...
      { hidden: true, id: 'anika-bots:pane', title: 'anika-bots:pane' }
    ])
  ...
  it('excludes hide-only tabs from every close verb', () => {
    ...
    $layoutTree.set(group(['sessions', 'anika-bots:pane', 'session-tile:x'], { active: 'sessions', id: 'g-mixed' }))
