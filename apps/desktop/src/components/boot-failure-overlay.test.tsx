...
function failBoot() {
  $desktopBoot.set({
    error: 'Could not connect to Anika gateway',
    fakeMode: false,
    message: 'boot failed',
...
function stubDesktop(config: Record<string, unknown>, overrides: Record<string, unknown> = {}) {
  const original = window.anikaDesktop
  Object.defineProperty(window, 'anikaDesktop', {
    configurable: true,
    value: { getRecentLogs: async () => ({ lines: [] }), getConnectionConfig: async () => config, ...overrides }
  })

  return () => Object.defineProperty(window, 'anikaDesktop', { configurable: true, value: original })
}
...
describe('BootFailureOverlay', () => {
  ...
  it('recovers a cloud connection through the portal cascade instead of native OAuth', async () => {
    const gatewayUrl = 'https://agent-1.agents.__NEW_DOMAIN__'
    const logout = vi.fn().mockResolvedValue({ ok: true, connected: false })
    const nativeLogin = vi.fn().mockResolvedValue({ ok: true, connected: false })
    const cloudStatus = vi.fn().mockResolvedValue({ portalBaseUrl: 'https://portal.__NEW_DOMAIN__', signedIn: false })

    const cloudLogin = vi.fn().mockResolvedValue({
      ok: true,
      portalBaseUrl: 'https://portal.__NEW_DOMAIN__',
      signedIn: true
    })
  ...
  it('shows the __NEW_ORG__ Cloud down recovery when the backend flags isCloudBackendDown', async () => {
    const restore = stubDesktop(remoteToken)
    $desktopBoot.set({
      error: '__NEW_ORG__ Cloud agent ares-3009.agents.__NEW_DOMAIN__ is down (HTTP 503: server-side fault).',
      fakeMode: false,
    ...
    try {
      ...
      // remote-failure copy.
      expect(await screen.findByText(/__NEW_ORG__ Cloud agent is down/i)).toBeTruthy()
      ...
      expect(screen.getByText(/ares-3009\.agents\.__NEW_ORG__\.com/i)).toBeTruthy()
    } finally {
      restore()
    }
  })
