...
import { getGlobalModelOptions } from '@/anika'
...
import type { ModelOptionProvider, OAuthProvider } from '@/types/anika'
...
// __NEW_ORG__ Portal OAuth), ahead of OpenRouter and the rest of the key catalog.
const API_KEY_OPTIONS: ApiKeyOption[] = [
  ...
  },
  {
    ...
    docsUrl: 'https://github.com/__NEW_ORG__/anika-agent#bring-your-own-endpoint',
    placeholder: 'http://127.0.0.1:8000/v1'
  }
]

// Build the FULL API-key provider catalog from the backend model options so the
// onboarding / Providers key form lists every `api_key` provider `anika model`
...
const SHOW_ALL_KEY = 'anika-onboarding-show-all-v1'
...
export function Picker({ ctx }: { ctx: OnboardingContext }) {
  ...
  // Collapse the secondary providers behind a disclosure whenever __NEW_ORG__ Portal
        ...
        {showRest ? (
          <>
            {/* Fireworks leads the expanded list, matching CANONICAL_PROVIDERS
                (__NEW_ORG__ → Fireworks), but stays hidden until the user opens it. */}
