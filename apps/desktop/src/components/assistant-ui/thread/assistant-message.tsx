...
const ErrorRecoveryActions: FC = () => {
  ...
  // Open Logs reveals the LOCAL Electron profile's ANIKA_HOME/logs. On a
  ...
  const openLogs = useCallback(async () => {
    try {
      const root = await window.anikaDesktop?.logsRoot?.()
      ...
      const result = await window.anikaDesktop?.openDir?.(root)
