...
const ANIKA_REF_TYPES = WIRE_REFERENCE_KINDS
type AnikaRefType = (typeof ANIKA_REF_TYPES)[number]
...
const ANIKA_DIRECTIVE_RE = referenceRe()
...
export const anikaDirectiveFormatter: Unstable_DirectiveFormatter = {
...
function parseDirectiveText(text: string): Unstable_DirectiveSegment[] {
  const matches = [
    ...
    ...Array.from(text.matchAll(ANIKA_DIRECTIVE_RE)).map(match => {
...
function safeDirectiveSegments(text: string): Unstable_DirectiveSegment[] {
  try {
    return [...anikaDirectiveFormatter.parse(text)]
  } catch {
...
/**
 * Renders text containing Anika directives (`@file:...`, `@image:...`) as
...
const DirectiveImage: FC<{ id: string; label: string }> = ({ id, label }) => {
  ...
  useEffect(() => {
    ...
    const load =
      window.anikaDesktop && isRemoteGateway() ? gatewayMediaDataUrl(id) : window.anikaDesktop?.readFileDataUrl(id)
