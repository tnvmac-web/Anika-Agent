...
describe('buildToolView web-search query', () => {
  it('keeps the query separate from structured search results', () => {
    const view = buildToolView(
      part({
        args: { query: 'Anika Agent Desktop tool calls' },
        result: { web: [{ snippet: 'Desktop docs', title: 'Anika docs', url: 'https://example.com/docs' }] },
        toolName: 'web_search'
      }),
      ''
    )

    expect(view.searchQuery).toBe('Anika Agent Desktop tool calls')
    expect(view.searchHits).toEqual([
      { snippet: 'Desktop docs', title: 'Anika docs', url: 'https://example.com/docs' }
    ])
  })
})
  ...
  it('shows failed title when navigate returns success=false', () => {
    const view = buildToolView(
      part({
        ...
        args: { url: 'https://anika-agent.__NEW_DOMAIN__/docs' },
    ...
    expect(view.title).toBe('Failed to open anika-agent.__NEW_DOMAIN__/docs')
  })
  ...
  it('shows opened title on success', () => {
    const view = buildToolView(
      part({
        toolName: 'browser_navigate',
        args: { url: 'https://anika-agent.__NEW_DOMAIN__/docs' },
        result: { success: true, url: 'https://anika-agent.__NEW_DOMAIN__/docs', title: 'Docs' }
      }),
      ''
    )

    expect(view.status).toBe('success')
    expect(view.title).toBe('Opened anika-agent.__NEW_DOMAIN__/docs')
  })
})
