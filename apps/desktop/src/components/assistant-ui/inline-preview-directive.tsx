 ...
 * own design.
 *
 * WIDGETS TALK BACK OFF-SCREEN. `window.anika.send(prompt)` (or declarative
 * `data-anika-send` on any clickable element) routes the prompt through the
...
const SIZE_MESSAGE_TYPE = 'anika-inline-preview-size'
const INTENT_MESSAGE_TYPE = 'anika-inline-preview-intent'
...
/** The script that gives the widget its ONE voice: `anika.send(prompt)`.
 ...
 *  wires `data-anika-send` so declarative HTML works with zero script:
 *  `<button data-anika-send="get-price eth">ETH</button>`. */
export function intentScript(token: string): string {
  return (
    ...
    ')},"*");return true}' +
    'window.anika={send:send};' +
    'addEventListener("click",function(e){var el=e.target&&e.target.closest?' +
    'e.target.closest("[data-anika-send]"):null;' +
    'if(el)send(el.getAttribute("data-anika-send")||"")},true)})()</script>'
  )
}
...
export function InlinePreviewDirective({
  ...
  // URL connection — including a same-machine `anika serve` — renders live.)
