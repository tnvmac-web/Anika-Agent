import { JsonRpcGatewayClient } from '@anika/shared'

import type { AnikaApiRequest } from '@/global'
...
// "Timed out connecting to Anika backend" that hangs the UI (#48504).
//
...
export class AnikaGateway extends JsonRpcGatewayClient {
  constructor() {
    super({
      closedErrorMessage: 'Anika gateway connection closed',
      connectErrorMessage: 'Could not connect to Anika gateway',
      createRequestId: nextId => nextId,
      notConnectedErrorMessage: 'Anika gateway is not connected',
...
// process to route them to that host (anika:api's registry branch). Null
...
export function anikaApi<T>(request: AnikaApiRequest): Promise<T> {
  return window.anikaDesktop.api<T>({ ...connectionScoped(), ...request })
}
