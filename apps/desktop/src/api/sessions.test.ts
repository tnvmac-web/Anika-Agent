...
const anikaApi = vi.mocked(client.anikaApi)
...
describe('deleteSession profile scoping', () => {
  it('scopes the DELETE to the owning profile in the URL (object owner)', async () => {
    ...
    anikaApi.mockResolvedValue({ ok: true } as never)
    // Mirrors the real capabilityScoped for an object owner (remote-stamped row).
    vi.mocked(client.capabilityScoped).mockReturnValue({ profile: 'tommy', connectionId: 'anika-pi' })

    await deleteSession('sess-1', { connectionId: 'anika-pi', profile: 'tommy' })

    expect(anikaApi.mock.calls[0][0]).toMatchObject({
      method: 'DELETE',
      path: '/api/sessions/sess-1?profile=tommy',
      connectionId: 'anika-pi',
      profile: 'tommy'
    })
  })
  ...
  it('omits the profile query when no owner is known', async () => {
    ...
    expect((anikaApi.mock.calls[0][0] as { path: string }).path).not.toContain('profile=')
  })
...
describe('setSessionArchived profile scoping', () => {
  ...
  it('omits the profile from the body when none is given', async () => {
    ...
    const req = anikaApi.mock.calls[0][0] as { body: Record<string, unknown> }
