...
export interface OAuthProvider {
   ...
   *  embedded terminal. Null when Anika doesn't know how to remove it. */
...
export interface EnvVarInfo {
  ...
  // in anika_cli/provider_catalog.py). When present, the Keys tab groups by
  // this provider identity — the SAME one `anika model` uses — instead of
...
export type AnikaConfigRecord = Record<string, unknown>
...
export interface ModelOptionProvider {
   ...
   *  the `anika model` CLI / onboarding OAuth flow. */
  auth_type?: string
  ...
  /** __NEW_ORG__ only: whether the current account is on the free tier. */
  free_tier?: boolean
  /** __NEW_ORG__ only: paid models a free-tier user cannot select (shown disabled). */
...
// non-desktop) Anika simply carries none of it. See store/profile-share.ts.
export interface ProfileDesktopOverlay {
  ...
  /** Layout tree (anika.desktop.layoutTree.v2 shape). */
  layoutTree?: unknown
...
// folders. Mirrors anika_cli/projects_db.Project.to_dict().
...
export interface SkillInfo {
  ...
  usage?: number
  /** 'agent' = learned/local (editable), 'bundled' = ships with Anika, 'hub' = installed. */
  provenance?: 'agent' | 'bundled' | 'hub'
}

/** One entry of the built-in optional-skills catalog (optional-skills/ in the
 *  repo) — official skills that ship with Anika but install on demand. */
...
export interface ToolProvider {
   ...
   *  CLI `anika tools` active-provider detection). */
  is_active: boolean
  /** Honest readiness computed server-side (keys ∧ __NEW_ORG__ entitlement ∧
 ...
 *  not Anika); elsewhere, driver health from `cua-driver doctor`. `null`
...
export interface ActionStatusResponse {
  ...
  pid: number | null
  running: boolean
  /** anika-update only: durable completion identity recovered from update.log. */
  action_id?: string
  /** anika-update only: summary of the durable update receipt (#91277 bullet 3) —
...
/** Shape of `GET /api/anika/update/check` — the backend's own update state.
...
export interface ModelAssignmentResponse {
  ...
  base_url?: string
  /** Toolset keys auto-routed through the __NEW_ORG__ Tool Gateway as a result of
   *  switching the main provider to __NEW_ORG__. Empty unless provider === 'nous'
