...
function isMissingIpcHandler(error: unknown): boolean {
  ...
  return message.includes("No handler registered for 'anika:saveImageFromUrl'")
}
...
export function useImageDownload(src?: string) {
  ...
  const download = useCallback(async () => {
    ...
    try {
      if (window.anikaDesktop?.saveImageFromUrl) {
        if (await window.anikaDesktop.saveImageFromUrl(src)) {
