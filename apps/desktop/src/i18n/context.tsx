...
import { getAnikaConfigRecord, type AnikaConfigRecord, saveAnikaConfig } from '@/anika'
...
export interface I18nConfigClient {
  getConfig: () => Promise<AnikaConfigRecord>
  saveConfig: (config: AnikaConfigRecord) => Promise<{ ok: boolean }>
}

const defaultConfigClient: I18nConfigClient = {
  getConfig: () => {
    if (typeof window === 'undefined' || !window.anikaDesktop?.api) {
      return Promise.resolve({})
    }

    return getAnikaConfigRecord()
  },
  saveConfig: config => {
    if (typeof window === 'undefined' || !window.anikaDesktop?.api) {
...
export function getConfigDisplayLanguage(config: AnikaConfigRecord): unknown {
  return isRecord(config.display) ? config.display.language : undefined
}

export function withConfigDisplayLanguage(config: AnikaConfigRecord, locale: Locale): AnikaConfigRecord {
