...
import type { SessionInfo } from '@/anika'
...
// zero Anika sessions, and those arrive continuously — so once the user dragged
...
export function useRepoWorktreeMap(
  repoPaths: string[],
  enabled: boolean
): [Record<string, AnikaGitWorktree[]>, boolean] {
  const [map, setMap] = useState<Record<string, AnikaGitWorktree[]>>({})
