...
function buildTourScript(action: TourAction): string {
  return `(function () {
  var w = window;
  if (!w.__anikaTourEngine) {
    ${driverIife}
    w.__anikaTourHolder = {};
    w.__anikaTourCollect = (${collectTourTargets.toString()});
    w.__anikaTourEngine = (${runTourEngine.toString()});
  }
  if (!document.getElementById('__anika-tour-style')) {
    var style = document.createElement('style');
    style.id = '__anika-tour-style';
    style.textContent = ${JSON.stringify(driverCss)};
    (document.head || document.documentElement).appendChild(style);
  }
  return JSON.stringify(w.__anikaTourEngine(
    w.driver.js.driver,
    w.__anikaTourHolder,
    ${JSON.stringify(action)},
    w.__anikaTourCollect,
    document
  ));
})()`
}
