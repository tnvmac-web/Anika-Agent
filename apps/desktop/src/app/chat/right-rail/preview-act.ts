...
const preamble = () => `  var w = window;
  ...
  w.__anikaActHolder = w.__anikaActHolder || {};
  w.__anikaAct = ${actEngineSource()};
  w.__anikaWatch_fn = (${watchInPage.toString()});
  w.__anikaWatchTag = ${WATCH_TAG};
  var holder = w.__anikaActHolder;
  var act = function (a) { return w.__anikaAct(document, holder, a); };
  var watch = function (stage, label) {
    try { w.__anikaWatch_fn(document, holder, stage, label); } catch (err) {}
  };
...
${preamble()}
  ...
  document.addEventListener('pointerdown', function (e) {
    w.__anikaHit = { tag: e.target ? e.target.tagName : '?', trusted: e.isTrusted === true };
...
function hitNote(hit?: { tag: string; trusted: boolean } | null): string | undefined {
  return hit && hit.tag === 'ANIKA-WATCH' ? 'The action overlay intercepted the click instead of the page.' : undefined
}
