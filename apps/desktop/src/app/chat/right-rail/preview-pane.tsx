        ...
        <>
          <a
            ...
            onClick={event => {
              ...
              void window.anikaDesktop?.openExternal(error.url)
            }}
          >
...
export function PreviewPane({ embedded = false, onRestartServer, reloadRequest = 0, tabId, target }: PreviewPaneProps) {
  ...
  const annotateGuest = useCallback((): null | PreviewAnnotateGuest => {
    ...
    return {
      capture: async rect => {
        ...
        const dataUrl = await window.anikaDesktop.capturePreview?.({ rect, viewport, webContentsId })
  ...
  const restartServer = useCallback(async () => {
    ...
    // "Ask Anika to restart the server" looked like it did nothing —
  ...
  useEffect(() => {
    if (
      ...
      !window.anikaDesktop?.watchPreviewFile ||
      !window.anikaDesktop?.onPreviewFileChanged
    ) {
      return
    }
    ...
    const unsubscribe = window.anikaDesktop.onPreviewFileChanged(payload => {
    ...
    void window.anikaDesktop
      ...
      .then(watch => {
        if (!active) {
          void window.anikaDesktop?.stopPreviewFileWatch?.(watch.id)
    ...
    return () => {
      ...
      if (watchId) {
        void window.anikaDesktop?.stopPreviewFileWatch?.(watchId)
      }
    }
  ...
  useEffect(() => {
    ...
    webview.setAttribute('partition', 'persist:anika-preview')
    ...
    const onGuestContextMenu = (event: Event) => {
      ...
      const zoom = window.anikaDesktop?.zoom?.factor?.() || 1
      ...
      openGuestContextMenu(
        ...
        },
        {
          addToDictionary: (word: string) => {
            ...
            if (typeof webContentsId === 'number') {
              void window.anikaDesktop?.contextMenuGuestAddWord?.({ webContentsId, word })
            }
          },
          copyImage: () => void window.anikaDesktop?.contextMenuCopyImage?.(),
          ...
          <PreviewBrowserBar
            ...
            onOpenExternal={
              !isBrowserWindow() && !canOpenBrowserWindow()
                ? () => void window.anikaDesktop?.openExternal(currentUrl)
                : undefined
            }
