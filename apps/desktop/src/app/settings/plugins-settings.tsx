...
// User-installed plugins first — mirrors `anika plugins list --user`.
...
function reveal(file: string) {
  void window.anikaDesktop?.revealPath?.(file)?.catch(() => undefined)
}

async function revealPluginsDir() {
  try {
    // Electron owns the local plugin root — deriving it from the backend's
    // anika_home breaks against a remote backend (#66899).
    const dir = await window.anikaDesktop?.desktopPluginsRoot?.()
    ...
    const result = await window.anikaDesktop?.openDir?.(dir)
...
// Agent plugins live under the BACKEND's anika home (profile-aware), so the
// path comes from the gateway — not from the renderer's local ANIKA_HOME.
...
async function revealAgentPluginsDir(request: GatewayRequest) {
  try {
    ...
    const opened = await window.anikaDesktop?.openDir?.(`${home}/plugins`)
            ...
            <SelectContent>
              {profiles.map(profile => (
                <SelectItem key={profile.name} value={profile.name}>
                  {profile.is_default ? 'Anika (default)' : profile.name}
                </SelectItem>
              ))}
            </SelectContent>
          </Select>
