...
import type { ToolsetConfig } from '@/types/anika'
...
const getAnikaConfigSchema = vi.fn()
...
vi.mock('@/anika', () => ({
  ...
  getAnikaConfigSchema: () => getAnikaConfigSchema(),
...
beforeEach(() => {
  ...
  getAnikaConfigRecord.mockResolvedValue({
    tts: {
      provider: 'edge',
  ...
  getAnikaConfigSchema.mockResolvedValue({ fields: {}, category_order: [] })
...
describe('ToolsetConfigPanel', () => {
  it('renders inline voice/model fields for a TTS provider row carrying tts_provider', async () => {
    ...
    await waitFor(() => expect(saveAnikaConfig).toHaveBeenCalled(), { timeout: 3000 })
    const saved = saveAnikaConfig.mock.calls.at(-1)?.[0] as Record<string, Record<string, Record<string, string>>>
  ...
  describe('readiness pills', () => {
    it('renders the server status instead of assuming keyless rows are Ready', async () => {
      // The false-Ready bug: a logged-out __NEW_ORG__ Subscription row and a
  ...
  describe('managed __NEW_ORG__ provider activation', () => {
    const nousBrowserConfig = () =>
      config({
        ...
        providers: [
          {
            name: '__NEW_ORG__ Subscription (Browser Use cloud)',
            badge: 'subscription',
    ...
    it('surfaces a sign-in notice when the PUT reports needs_nous_auth', async () => {
      ...
      selectToolsetProvider.mockResolvedValue({
        ok: true,
        name: 'browser',
        provider: '__NEW_ORG__ Subscription (Browser Use cloud)',
      ...
      // The single __NEW_ORG__ row auto-expands; activate via the explicit button.
      await screen.findByRole('button', { name: /__NEW_ORG__ Subscription/ })
      fireEvent.click(await screen.findByRole('button', { name: /Use this backend/ }))

      await waitFor(() =>
        expect(selectToolsetProvider).toHaveBeenCalledWith('browser', '__NEW_ORG__ Subscription (Browser Use cloud)')
      )
      await waitFor(() =>
    ...
    it('drives the existing __NEW_ORG__ OAuth device-code flow from the sign-in action and refetches', async () => {
      const { notify } = await import('@/store/notifications')
      ...
      startOAuthLogin.mockResolvedValue({
        ...
        session_id: 'sess-1',
        user_code: 'NOUS-1234',
        verification_url: 'https://portal.__NEW_DOMAIN__/device?user_code=NOUS-1234',
        poll_interval: 5,
        expires_in: 600
      })
