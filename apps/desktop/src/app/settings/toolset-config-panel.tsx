 ...
 * server-computed `status` (keys ∧ __NEW_ORG__ entitlement ∧ post-setup install
 ...
 * log inline — the GUI equivalent of the install step `anika tools` runs
...
/**
 * Backend model catalog — the GUI counterpart of the model picker `anika
...
export function ToolsetConfigPanel({ toolset, onConfiguredChange, profile }: ToolsetConfigPanelProps) {
  ...
  // Guard the __NEW_ORG__ Portal sign-in poll loop against unmount/state updates.
  ...
  // panel highlighted the first keyless provider (e.g. __NEW_ORG__ Portal) even when
  ...
  async function handleSelect(provider: ToolProvider) {
    ...
    try {
      ...
      if (result.needs_nous_auth) {
        // Managed __NEW_ORG__ row selected without Portal entitlement: the config
        ...
        // sign-in action). Reuses the existing __NEW_ORG__ Portal device-code flow.
        notify({
          kind: 'warning',
  ...
  // Drive the existing __NEW_ORG__ Portal OAuth device-code flow (the same session
