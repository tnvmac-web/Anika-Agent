...
import { type ProfileScope, saveAnikaConfigRecord } from '@/anika'
 ...
 * the default browser's profile (cookies/logins) into a Anika-owned copy;
...
export function BrowserRealProfilePanel({ profile }: BrowserRealProfilePanelProps) {
  ...
  const { data: config } = useAnikaConfigRecord(profile)
  ...
  const toggle = useCallback(
    async (on: boolean) => {
      ...
      try {
        await saveAnikaConfigRecord(next, profile)
        notify({
          kind: 'info',
