 ...
 * agent sources (local runtime + any number of remote gateways / Anika Cloud
...
export function ConnectionsRegistrySection() {
  ...
  useEffect(() => {
    if (!editorWantsOauth || !editorUrl || !window.anikaDesktop?.probeConnectionConfig) {
      setAuthProbe(null)
  ...
  const signInOauth = useCallback(async () => {
    ...
    try {
      const result = await window.anikaDesktop.oauthLoginConnectionConfig(editorUrl)
  ...
  // Fan out `anika update` to every eligible source; per-connection results
