...
import { getElevenLabsVoices, getAnikaConfigSchema, saveAnikaConfig } from '@/anika'
...
import type { ConfigFieldSchema, AnikaConfigRecord } from '@/types/anika'

import { anikaConfigCacheWriter, useAnikaConfigRecord } from '../hooks/use-config-record'
...
function ConfigSettingsInner({
  ...
  const [config, setConfig] = useState<AnikaConfigRecord | null>(null)
  const { data: loadedConfig, isError: configLoadFailed, refetch: refetchConfig } = useAnikaConfigRecord(scopeProfile)
  ...
  const writeConfigCache = useMemo(() => anikaConfigCacheWriter(scopeProfile), [scopeProfile])
  ...
  const {
    ...
    queryKey:
      scopeProfile == null ? ['anika-config-schema'] : ['anika-config-schema', normalizeProfileKey(scopeProfile)],
    queryFn: () => getAnikaConfigSchema(scopeProfile),
    staleTime: 5 * 60 * 1000
  })
  ...
  // never touched — possibly changed out-of-band by `anika config set`
  // while this page sat open — is never resent with its stale value.
  const configBaselineRef = useRef<AnikaConfigRecord | null>(null)
    ...
    const t = window.setTimeout(() => {
      ...
      saveQueueRef.current = saveQueueRef.current.then(async () => {
        try {
          ...
          const result = await saveAnikaConfig(patch, scopeProfile)
  ...
  const applyConfig = (next: AnikaConfigRecord) => {
    saveVersionRef.current += 1
    setConfig(next)
    setSaveVersion(saveVersionRef.current)
  }

  const updateConfig = (next: AnikaConfigRecord) => {
