...
import { disconnectOAuthProvider, listOAuthProviders } from '@/anika'
...
import type { EnvVarInfo, OAuthProvider } from '@/types/anika'
...
const canRunInTerminal = () => typeof window !== 'undefined' && Boolean(window.anikaDesktop?.terminal)
...
//      in anika_cli/provider_catalog.py) — the SAME provider identity
//      `anika model` uses. This is authoritative: a provider tagged by the
...
function ConnectedProviderRow({
  ...
  // Anika can clear this provider's creds via the API.
  const canDisconnect = provider.disconnectable ?? provider.flow !== 'external'
  // External (CLI-managed) provider Anika can't clear via the API, but ships a
...
export function ProvidersSettings({
  ...
  // Anika never deletes creds another tool owns behind a silent API call.
