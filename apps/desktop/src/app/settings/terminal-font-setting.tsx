...
import { saveAnikaConfig } from '@/anika'
import { useI18n } from '@/i18n'
import { notifyError } from '@/store/notifications'
import type { AnikaConfigRecord } from '@/types/anika'

import { setAnikaConfigCache, useAnikaConfigRecord } from '../hooks/use-config-record'
...
function fontFamilyFromConfig(config: AnikaConfigRecord): string {
...
export function TerminalFontSetting() {
  ...
  const { data: loadedConfig } = useAnikaConfigRecord()
  ...
  const [staleConfig, setStaleConfig] = useState<AnikaConfigRecord | null>(null)
  ...
  useEffect(() => {
    ...
    // holds — successful saves write it back via setAnikaConfigCache, so
        ...
        <div className="mt-3 space-y-2">
          <div className="flex items-center gap-3">
            <Input
              ...
              className="flex-1"
              disabled={draft === null}
              list="anika-terminal-font-families"
          ...
          <datalist id="anika-terminal-font-families">
