...
vi.mock('@/anika', () => ({
  ...
  saveAnikaConfig: (config: unknown, profile?: string) => saveAnikaConfig(config, profile),
...
beforeEach(() => {
  ...
  getAnikaConfigSchema.mockResolvedValue({ fields: {} })
...
describe('ConfigSettings autosave', () => {
  it('sends a later revert instead of diffing it away against the stale page-load baseline', async () => {
    getAnikaConfigRecord.mockResolvedValue({ checkpoints: { enabled: false }, other: 'untouched' })
    ...
    try {
      ...
      await waitFor(() => expect(saveAnikaConfig).toHaveBeenCalledTimes(1))
      expect(saveAnikaConfig.mock.calls[0][0]).toEqual({ checkpoints: { enabled: true } })

      // Revert: flip it back to its original value and let autosave fire again.
      toggle.click()
      await vi.advanceTimersByTimeAsync(700)

      await waitFor(() => expect(saveAnikaConfig).toHaveBeenCalledTimes(2))
      ...
      expect(saveAnikaConfig.mock.calls[1][0]).toEqual({ checkpoints: { enabled: false } })
    } finally {
      vi.useRealTimers()
    }
  })
