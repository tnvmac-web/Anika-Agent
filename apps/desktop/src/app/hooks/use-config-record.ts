...
import { getAnikaConfigRecord, type ProfileScope, profileScopeKey } from '@/anika'
...
//
// Distinct from session/hooks/use-anika-config.ts, which is side-effecting —
// it pushes personality/cwd/voice/… into the session stores for live chat.
export const ANIKA_CONFIG_KEY = ['anika-config-record'] as const
...
// share a cache row.
export const anikaConfigKey = (profile?: ProfileScope) =>
  profile == null ? ANIKA_CONFIG_KEY : ([...ANIKA_CONFIG_KEY, profileScopeKey(profile)] as const)
...
export const useAnikaConfigRecord = (profile?: ProfileScope) =>
  useQuery({
    queryKey: anikaConfigKey(profile),
    ...
    queryFn: () => getAnikaConfigRecord(profile ?? undefined),
    staleTime: 0
  })

// setAnikaConfigCache writes the app-wide (base-key) record. Pass a profile to
...
export const setAnikaConfigCache = writeCache<AnikaConfigRecord>(ANIKA_CONFIG_KEY)
export const anikaConfigCacheWriter = (profile?: ProfileScope) =>
  writeCache<AnikaConfigRecord>(anikaConfigKey(profile))

export const invalidateAnikaConfig = (profile?: ProfileScope) =>
  queryClient.invalidateQueries({ queryKey: anikaConfigKey(profile) })
