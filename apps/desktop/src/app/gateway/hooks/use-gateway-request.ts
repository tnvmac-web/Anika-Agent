import { isGatewayReauthRequired, resolveGatewayWsUrl } from '@anika/shared'
...
export function useGatewayRequest() {
  ...
  const gateway = useStore($gateway) as AnikaGateway | null
  const gatewayRef = useRef<AnikaGateway | null>(null)

  const connectionRef = useRef<Awaited<ReturnType<NonNullable<typeof window.anikaDesktop>['getConnection']>> | null>(
    null
  )

  const gatewayStateRef = useRef(gatewayState)
  const reconnectingRef = useRef<Promise<AnikaGateway | null> | null>(null)
  ...
  useEffect(
    () =>
      $gateway.subscribe(gateway => {
        gatewayRef.current = gateway as AnikaGateway | null
      }),
    []
  )
