...
vi.mock('@/anika', async importOriginal => {
  const actual = await importOriginal<typeof AnikaModule>()

  class FakeAnikaGateway {
  ...
  return { ...actual, AnikaGateway: FakeAnikaGateway }
})

import type * as AnikaModule from '@/anika'
import type { AnikaGateway } from '@/anika'
import {
  $gateway,
...
const fakeGateway = { connectionState: 'open' } as unknown as AnikaGateway
...
async function activateRemoteGateway() {
  ...
  setPrimaryGateway(primary as unknown as AnikaGateway, 'default')
  $gateway.set(primary as unknown as AnikaGateway)
...
describe('useGatewayRequest', () => {
  ...
  it('surfaces a missing optional scoped mint bridge without falling back to a stale ticket or local lookup', async () => {
    const { desktop, gateway } = await activateRemoteGateway()
    Reflect.deleteProperty(window.anikaDesktop, 'getGatewayWsUrlFor')
  ...
  it('rejects instead of hanging forever when the reconnect getConnection() wedges (#93454)', async () => {
    ...
    } as unknown as AnikaGateway

    const getConnection = vi.fn(() => new Promise(() => undefined))

    ;(window as unknown as { anikaDesktop: unknown }).anikaDesktop = { getConnection }
    $gateway.set(dropped)
