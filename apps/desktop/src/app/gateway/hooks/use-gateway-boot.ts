import { isGatewayReauthRequired, JsonRpcGatewayError, resolveGatewayWsUrl } from '@anika/shared'
import { useEffect, useRef } from 'react'

import { shouldApplyPostBootProgressError } from '@/components/boot-failure-reauth'
import type { AnikaConnection } from '@/global'
import { AnikaGateway } from '@/anika'
...
import type { RpcEvent } from '@/types/anika'
...
export function primaryRuntimeConnectionId(connection: Pick<AnikaConnection, 'connectionId' | 'mode'>): null | string {
...
interface GatewayBootOptions {
  ...
  onConnectionReady: (
    connection: Awaited<ReturnType<NonNullable<typeof window.anikaDesktop>['getConnection']>> | null
  ) => void
  onGatewayReady: (gateway: AnikaGateway | null) => void
  refreshAnikaConfig: (force?: boolean, shouldPublish?: () => boolean) => Promise<void>
...
export function useGatewayBoot({
  ...
  useEffect(() => {
    ...
    const publish = (next: AnikaConnection | null) => {
    ...
    // composer stayed disabled forever on "Starting Anika...". Once the
    ...
    const attemptReconnect = async () => {
      ...
      try {
        ...
        // "Starting Anika…". The probe is a no-op for a healthy or local backend.
        ...
        const conn = await withTimeout(
          ...
          RECONNECT_ATTEMPT_TIMEOUT_MS,
          'Timed out reconnecting to Anika backend'
        )
        ...
        // as an opaque "Could not connect to Anika gateway". resolveGatewayWsUrl
        ...
        await callbacksRef.current.refreshAnikaConfig().catch(() => undefined)
    ...
    const softSwitch = async () => {
      ...
      try {
        ...
        await Promise.all([
          ...
          callbacksRef.current.refreshAnikaConfig(false, ownsSwitch).catch(() => undefined),
    ...
    const offBootProgress = desktop.onBootProgress(payload => {
      // Soft switch / post-boot startAnika re-emits progress — ignore so the
    ...
    const gateway = adoptedFromHmr ? survivor!.gateway : new AnikaGateway()
    ...
    async function boot() {
      try {
        ...
        // hanging "Starting Anika…" forever.
        const wsUrl = await withTimeout(
          resolveGatewayWsUrl(desktop, conn),
          RECONNECT_ATTEMPT_TIMEOUT_MS,
        ...
        await Promise.all([
          ...
          callbacksRef.current.refreshAnikaConfig(),
          ...
          // retry it), not brick boot behind the "Anika couldn't start"
