...
import type { AnikaReadDirEntry, AnikaReadDirResult } from '@/global'
...
export type ProjectTreeEntry = AnikaReadDirEntry
...
function ignoredBy(rules: GitignoreRule[], entry: AnikaReadDirEntry) {
...
async function filterIgnored(entries: AnikaReadDirEntry[], rootPath: string, dirPath: string) {
...
export async function readProjectDir(dirPath: string, rootPath = dirPath): Promise<AnikaReadDirResult> {
  if (!window.anikaDesktop) {
