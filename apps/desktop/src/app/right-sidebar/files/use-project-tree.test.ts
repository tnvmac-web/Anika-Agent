...
beforeEach(() => {
  ...
  readDir.mockReset()
  ;(window as unknown as { anikaDesktop: { readDir: typeof readDir } }).anikaDesktop = { readDir }
})
...
function ok(entries: { name: string; path: string; isDirectory: boolean }[]): AnikaReadDirResult {
  return { entries }
}
...
describe('useProjectTree', () => {
  ...
  it('reads gitignore from the real path while caching per connection', async () => {
    ...
    ;(window as unknown as { anikaDesktop: unknown }).anikaDesktop = { gitRoot, readDir, readFileDataUrl }
  ...
  it('dedupes concurrent loadChildren calls for the same id', async () => {
    ...
    let resolveChildren: ((value: AnikaReadDirResult) => void) | undefined
    readDir.mockImplementationOnce(
      () =>
        new Promise<AnikaReadDirResult>(resolve => {
  ...
  it('discards a stale live refresh after the active registered connection changes', async () => {
    let resolveRefreshFromA: ((result: AnikaReadDirResult) => void) | undefined
  ...
  it('discards a stale child read after the active registered connection changes', async () => {
    let resolveChildFromA: ((result: AnikaReadDirResult) => void) | undefined
  ...
  it('discards a stale root read after the active registered connection changes', async () => {
    let resolveFirst: ((result: AnikaReadDirResult) => void) | undefined
    readDir.mockImplementationOnce(
      () =>
        new Promise<AnikaReadDirResult>(resolve => {
  ...
  it('returns no-bridge gracefully when window.anikaDesktop is missing', async () => {
    delete (window as unknown as { anikaDesktop?: unknown }).anikaDesktop
  ...
  it('keeps an unreadable root on screen while it re-probes', async () => {
    ...
    let releaseProbe: ((value: AnikaReadDirResult) => void) | undefined
  ...
  it('keeps loaded rows on screen while the root refreshes', async () => {
    ...
    let releaseRefresh: ((value: AnikaReadDirResult) => void) | undefined
  ...
  it('clears the rows when the same path is re-read from another backend', async () => {
    ...
    let releaseFromB: ((value: AnikaReadDirResult) => void) | undefined
