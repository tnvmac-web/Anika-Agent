import { registryBackendScopeKey } from '@anika/shared'
...
export function useGatewayEventHandler(deps: GatewayEventDeps) {
  const { activeSessionIdRef, compactedTurnRef, refreshAnikaConfig, sessionStateByRuntimeIdRef } = deps
  ...
  // refreshAnikaConfig — two REST calls (config + defaults) per event, per
