import type { BillingBlock } from '@anika/shared'
 ...
 * billing-specific toast — never the generic "Anika error" — with a smart CTA
 * (__NEW_ORG__ → in-app Settings → Billing, other providers → their billing page).
 */
  ...
  if (event.type === 'message.complete') {
    ...
    const failure =
      payload?.status === 'error'
        ? {
            error: coerceGatewayText(payload.error).trim() || finalText || 'Anika reported an error',
