...
import { getAnikaConfig, getAnikaConfigDefaults } from '@/anika'
...
interface AnikaConfigOptions {
  activeSessionIdRef: MutableRefObject<string | null>
}

export function useAnikaConfig({ activeSessionIdRef }: AnikaConfigOptions) {
  const [voiceMaxRecordingSeconds, setVoiceMaxRecordingSeconds] = useState(DEFAULT_VOICE_SECONDS)
  const [sttEnabled, setSttEnabled] = useState(true)
  const profileRefreshEpochRef = useRef(0)

  const refreshAnikaConfig = useCallback(
    async (force = false, shouldPublish: () => boolean = () => true) => {
      ...
      try {
        const [config, defaults] = await Promise.all([getAnikaConfig(), getAnikaConfigDefaults().catch(() => ({}))])
        ...
        // rendering/applying Anika' built-in medium over the user's config.
        if (!canPublish()) {
          return
        }
  ...
  return { refreshAnikaConfig, sttEnabled, voiceMaxRecordingSeconds }
}
