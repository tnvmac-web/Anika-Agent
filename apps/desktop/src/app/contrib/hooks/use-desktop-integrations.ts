...
import { pathFromAnikaDeepLink, resolveAnikaOpenPath } from '@/lib/anika-open-target'
...
export function useDesktopIntegrations({
  ...
  useEffect(() => {
    ...
    // "About Anika" — it is the OS-standard affordance for updating THIS app,
    ...
    const unsubscribe = window.anikaDesktop?.onOpenUpdatesRequested?.(() => openUpdatesWindow('client'))
  ...
  useEffect(() => {
    window.anikaDesktop?.setPreviewShortcutActive?.(true)
  }, [])
  ...
  useEffect(() => {
    const unsubscribe = window.anikaDesktop?.onFocusSession?.(sessionId => {
      if (sessionId) {
  ...
  useEffect(() => {
    const unsubscribe = window.anikaDesktop?.onNotificationAction?.(({ actionId, sessionId }) => {
  ...
  // anika://index-network/intent/1 vocabulary with deep links.
  useEffect(() => {
    const unsubscribe = window.anikaDesktop?.onNotificationActivate?.(payload => {
      if (!payload) {
        return
      }
      ...
      if (payload.activate) {
        ...
        // the pre-IPC validation — any future anikaDesktop.notify caller gets
        // funneled through the same resolver.
        const path = resolveAnikaOpenPath(payload.activate)
  ...
  useEffect(() => {
    const unsubscribe = window.anikaDesktop?.onDeepLink?.(payload => {
      if (!payload?.kind) {
        return
      }
      ...
      // link (anika://index-network/intent/1, anika://open/…). The resolver
      // rejects reserved kinds and unsafe paths.
      const path = pathFromAnikaDeepLink(payload.kind, payload.name || '', payload.params || {})
    ...
    void window.anikaDesktop?.signalDeepLinkReady?.()
  ...
  useEffect(() => {
    const unsubscribe = window.anikaDesktop?.onClosePreviewRequested?.(
  ...
  useEffect(() => {
    const unsubscribe = window.anikaDesktop?.onPreviewNav?.(command => {
  ...
  useEffect(() => {
    const unsubscribe = window.anikaDesktop?.onOpenFolderRequested?.(() => void openFolderAsProject())
