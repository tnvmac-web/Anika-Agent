...
import { getAnikaConfigRecord, saveMcpServers } from '@/anika'
...
import { setAnikaConfigCache } from '../hooks/use-config-record'

/**
 * Explicit-confirm gate for `anika://mcp/install` deep links. The payload is
 ...
 * confirming lets Anika spawn that local process. An existing server name is
...
export function McpInstallDeepLinkDialog() {
  ...
  const confirm = async () => {
    ...
    try {
      ...
      const current = getServers(await getAnikaConfigRecord())
      ...
      setAnikaConfigCache(previous => (previous ? { ...previous, mcp_servers: nextServers } : previous))
