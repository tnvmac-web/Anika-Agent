...
import { getLatestSessionMessages } from '@/anika'
...
import { useAnikaConfigRecord } from '../hooks/use-config-record'
...
import { useAnikaConfig } from '../session/hooks/use-anika-config'
...
export function ContribWiring({ children }: { children: ReactNode }) {
  ...
  const { refreshAnikaConfig, sttEnabled, voiceMaxRecordingSeconds } = useAnikaConfig({ activeSessionIdRef })
  ...
  useEffect(() => {
    ...
    window.addEventListener('anika:open-keybinds', onOpenKeybinds)

    return () => window.removeEventListener('anika:open-keybinds', onOpenKeybinds)
  }, [navigate])
  ...
  useEffect(() => {
    ...
    void refreshAnikaConfig(true)
    void refreshActiveProfile()
    resetProjectTreeState()
  }, [gatewayScope, refreshCurrentModel, refreshAnikaConfig])
  ...
  const configRecord = useAnikaConfigRecord()
