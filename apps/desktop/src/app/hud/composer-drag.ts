...
function setWorkspaceTransfer(transferring: boolean): void {
  window.anikaDesktop?.hud?.setWorkspaceTransfer?.(transferring)
}

function moveHud(state: PressState): void {
  window.anikaDesktop?.hud?.moveBy?.({
    width: state.originW,
...
function armGrab(state: PressState, workspaceTransfer: boolean): void {
  ...
  window.anikaDesktop?.hud?.beginMove?.()
}
 ...
 * (see anika:hud:move-by — a transparent frameless window drifts wider on
...
export function useHudComposerDrag(
  ...
  const reset = useCallback(() => {
    ...
    if (state) {
      if (state.armed) {
        window.anikaDesktop?.hud?.endMove?.()
      }
