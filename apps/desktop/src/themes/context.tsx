...
const SKIN_KEY = 'anika-desktop-theme-v2'
const MODE_KEY = 'anika-desktop-mode-v1'
...
const PROFILE_SKINS_KEY = 'anika-desktop-profile-themes-v1'
const PROFILE_MODES_KEY = 'anika-desktop-profile-modes-v1'
// Last active profile, recorded so the boot-time paint can pick that profile's
// theme before the gateway reports which profile actually launched.
const LAST_PROFILE_KEY = 'anika-desktop-active-profile-v1'
...
function applyTheme(theme: DesktopTheme, mode: 'light' | 'dark') {
  ...
  root.dataset.anikaTheme = skinName
  root.dataset.anikaMode = rendered
  ...
  window.anikaDesktop?.setTitleBarTheme?.({
    background: chromeBg,
  ...
  try {
    window.localStorage.setItem('anika-boot-background', chromeBg)
    window.localStorage.setItem('anika-boot-color-scheme', rendered)
  } catch {
  ...
  if (typo.fontUrl && !INJECTED_FONT_URLS.has(typo.fontUrl)) {
    ...
    link.dataset.anikaThemeFont = 'true'
...
const syncNativeTheme = (pref: ThemeMode, rendered: 'light' | 'dark') =>
  window.anikaDesktop?.setNativeTheme?.(pref === 'system' ? 'system' : rendered)
...
export function ThemeProvider({ children }: { children: ReactNode }) {
  ...
  // Drain a backend-driven skin switch (Anika authoring/activating a skin from a
