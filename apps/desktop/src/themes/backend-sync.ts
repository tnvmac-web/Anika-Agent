/**
 * Live skin sync from the Anika backend.
 *
 * The backend resolves the active skin (built-in or `$ANIKA_HOME/skins/*.yaml`)
 ...
 * stomps the user's persisted desktop theme; only a genuine name change (Anika
 * authoring/activating a skin from a prompt, or `/skin` elsewhere) repaints.
 */

import type { AnikaSkin } from '@anika/shared/skin'
...
const BACKEND_THEMES_KEY = 'anika-desktop-backend-themes-v1'
 ...
 */
export function ingestBackendSkin(skin: AnikaSkin | undefined | null, { apply }: { apply: boolean }): void {
  ...
  if (name !== 'default' && !BUILTIN_THEMES[name]) {
    const theme = skinToDesktopTheme(skin as AnikaSkin)
