...
// The pre-GitHub __NEW_ORG__ palette stays available as nous-alt; the default name
...
describe('nous-alt is the retired Nous, not the default', () => {
  it('is registered under its own name and leaves nous as the default', () => {
    ...
    expect(BUILTIN_THEMES.__NEW_ORG__.darkColors?.background).not.toBe(nousAltTheme.darkColors?.background)
  })
})
