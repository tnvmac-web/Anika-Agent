"""Timezone-aware clock for Anika.

``now()`` returns a tz-aware datetime in the user's configured IANA timezone. Resolution order:
``ANIKA_TIMEZONE`` env var, then ``timezone`` in ``~/.anika/config.yaml``, else server-local
...
# ANIKA_HOME, so one unkeyed global would leak the first profile's timezone into later
...
def _timezone_cache_identity() -> Tuple[str, str]:
    tz_env = os.getenv("ANIKA_TIMEZONE", "").strip()
