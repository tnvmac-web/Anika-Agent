#!/usr/bin/env bash
# GitHub environment detection helper for Anika Agent skills.
#
...
if command -v gh &>/dev/null && gh auth status &>/dev/null 2>&1; then
    ...
    GH_AUTH_METHOD="curl"
elif _anika_env="${ANIKA_HOME:-$HOME/.anika}/.env"; [ -f "$_anika_env" ] && grep -q "^GITHUB_TOKEN=" "$_anika_env" 2>/dev/null; then
