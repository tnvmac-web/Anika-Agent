...
Edit with `anika config edit` or `anika config set section.key value`.
Full reference: https://anika-agent.__NEW_DOMAIN__/docs/user-guide/configuration
...
`anika config check` reports sections missing from an older config.

### Toolsets

Enable/disable via `anika tools` (interactive) or `anika tools enable/disable NAME`.
Full enumeration: `TOOLSETS` dict in `toolsets.py` (`_ANIKA_CORE_TOOLS` is the default bundle most platforms inherit).
