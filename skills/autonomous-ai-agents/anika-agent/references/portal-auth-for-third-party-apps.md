# __NEW_ORG__ Portal — authenticating third-party apps against the subscription

Recurring user question: "Can app X (Karakeep, OpenWebUI, LibreChat, OpenViking,
LangChain pipeline, n8n flow, etc.) use my __NEW_ORG__ Portal subscription without me
...
## Layer 1 — Is this thing a Anika plugin, or a separate app?
...
|---|---|---|
| **OpenViking memory plugin** (`plugins/memory/openviking/`) | Code that runs **inside the Anika process**. Its LLM calls go through Anika's already-configured provider. | Already uses Portal if user's Anika is configured for Portal. Nothing extra needed. `OPENVIKING_API_KEY` is the OpenViking *server's* own auth, not LLM auth. |
...
| **Karakeep, n8n, LibreChat, OpenWebUI, any self-hosted app** | Different process, often different machine. Makes its own HTTPS calls to `inference-api.__NEW_DOMAIN__`. | Layer 2/3 below. |

**Pitfall to avoid**: do not pitch "OAuth into Portal" as the solution for a
plugin that already runs inside Anika. That LLM call is already authenticated
via Anika's provider config. The plugin's own server auth (e.g.
...
Portal at `https://inference-api.__NEW_DOMAIN__/v1` is an OpenAI-compatible
inference endpoint. It accepts **bearer-token authentication only**: either

1. **A static API key** from `portal.__NEW_DOMAIN__ → API Keys`, or
...
"Sign in with __NEW_ORG__ Portal" SSO that third-party apps can register as clients
against. There is no shared cookie or session that browser-Portal-login
extends to other apps on the same machine.

What Anika Agent has that *feels* like OAuth — `anika login --provider nous`
opening a browser, user signs in, token lands in `~/.anika/auth.json` — is a
**Anika-specific browser flow**. Under the hood it produces a credential
Anika uses as a bearer. It is not a public OAuth provider that Karakeep et al.
...
1. Read Anika's existing Portal credential out of `~/.anika/auth.json`.
2. Expose a local OpenAI-compatible endpoint at `http://localhost:NNNN/v1`.
3. Forward incoming requests to `inference-api.__NEW_DOMAIN__/v1` with that
   bearer attached.
...
rides on the credential Anika already holds.

Where this could live in Anika:
...
Only when the consumer is *another first-party __NEW_ORG__ thing* (a future SDK, a
...
- **Per-key rate limits** so a noisy app can be capped without eating the
  user's headroom for Anika itself.
...
1. First decide: Anika plugin (runs inside Anika) or separate app? If plugin,
   it already uses Portal via Anika's provider config — done.
2. If separate app: today, paste the static API key from Portal → API Keys.
   Base URL `https://inference-api.__NEW_DOMAIN__/v1`. Rate limits are
...
4. Mixed setup ("Portal for some things, OpenRouter/Ollama Cloud for the
   Anika agent itself") is fully supported. Anika treats agent
...
pitch in the Tool Gateway announcement is specifically about Anika Agent's
*tools* (web search, browser, image gen, TTS) flowing through the Portal
subscription when Anika is configured to use Portal as its provider. It is
