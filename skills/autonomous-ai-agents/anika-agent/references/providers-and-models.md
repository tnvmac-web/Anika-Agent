...
Set via `anika model` (picker) or `anika setup`. 35+ provider profiles ship as
plugins under `plugins/model-providers/`; user plugins of the same name override.
Full docs: https://anika-agent.__NEW_DOMAIN__/docs/integrations/providers
...
| nous | OAuth device code | `anika auth add nous` (or `NOUS_API_KEY`) |
| openai-codex | OAuth | `anika auth add openai-codex` |
| qwen-oauth | OAuth | `anika auth add qwen-oauth` |
| minimax-oauth | OAuth | `anika auth add minimax-oauth` |
...
Multiple credentials per provider pool and rotate automatically (`anika auth`).
Fallback chain when the primary fails: `anika fallback add|remove|list`.

### User-defined model aliases

Work with `/model <name>` in CLI and every gateway platform. Resolved by
`anika_cli/model_switch.py::resolve_alias()`; user aliases are checked BEFORE
...
#   anika config set model.aliases.fav openrouter/anthropic/claude-sonnet-4.6
model:
  aliases:
