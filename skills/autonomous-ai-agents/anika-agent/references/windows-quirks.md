...
Anika runs natively on Windows (PowerShell, cmd, Windows Terminal, git-bash
...
`anika config edit` writes correctly.

### `execute_code` / Sandbox

**WinError 10106** from the sandbox child process — it can't create an
`AF_INET` socket. Root cause is usually Anika's env scrubber dropping
...
Anika-installed `venv/Scripts/` has no pip/pytest (stripped for size).
...
every Anika tool and most Windows APIs. Prefer forward slashes in code
