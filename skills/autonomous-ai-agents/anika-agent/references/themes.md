# Themes / Skins — Author a Anika Color Theme

Author a Anika **skin** — one YAML file that themes the CLI, the TUI, and the
desktop GUI at once. The skin engine (`anika_cli/skin_engine.py`) resolves the
active skin and the gateway pushes it to every surface, so a file dropped in
`~/.anika/skins/` is the theme analogue of a plugin: no code, all surfaces. This
...
- The user asks for a custom look ("make me a synthwave theme", "dark forest
  vibes", "match my brand colors") for Anika itself.
...
- Write access to the Anika home dir — `~/.anika` by default, or `$ANIKA_HOME`
  / the active profile's dir. Skins live in `<anika-home>/skins/`.
- Native tools: `write_file` (create the YAML), `read_file` / `search_files`
  (inspect existing skins), `terminal` (activate via `anika config set`).
...
3. `write_file` it to `<anika-home>/skins/<name>.yaml`.
...
4. **Confirm the new look landed** and tell the user how to revert: run
   `anika config set display.skin default` (or they can `/skin default`).
...
```
anika skin set <key> <hex>      # e.g. anika skin set ui_tool "#00FFFF"
```
...
palette and resets the background. `anika skin set background "#08201f"` changes
only the background; `anika skin use <name>` / `anika skin list` switch and
enumerate.

## Pitfalls

- **Don't hardcode `~/.anika`** when a profile is active — resolve the real home
  from `$ANIKA_HOME` first, falling back to `~/.anika`.
...
- **Never hand-edit `config.yaml` to activate.** Use `anika config set
...
- **You apply it, not the user.** `anika config set display.skin <name>` is
...
- `read_file` the written `<anika-home>/skins/<name>.yaml` and confirm valid
  YAML with the intended `name` and `colors`.
- Run `anika config get display.skin` and confirm it reports `<name>`.
