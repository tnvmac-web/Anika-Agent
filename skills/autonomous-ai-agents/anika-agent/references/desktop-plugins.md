...
Write plugins for the Anika desktop app: statusbar items, layout panes,
...
system (`~/.anika/plugins/`) is otherwise documented separately.

There are TWO on-disk doors, same contract and hot reload:

- `$ANIKA_HOME/desktop-plugins/<id>/plugin.js` — standalone desktop plugin.
  Loads enabled by default.
- `$ANIKA_HOME/plugins/<id>/desktop/plugin.js` — the desktop HALF of a
...
- The Anika desktop app (it loads plugins; the CLI/gateway alone does not).
- Write access to `$ANIKA_HOME/desktop-plugins/` (usually
  `~/.anika/desktop-plugins/`).

## How to Run

1. Create `$ANIKA_HOME/desktop-plugins/<name>/plugin.js` from
   `templates/plugin.js` (in this skill directory) — that's
   `~/.anika/...` by default, or `~/.anika/profiles/<profile>/...` under a
...
The ONLY import surface is `@anika/plugin-sdk` (plus `react` /
...
- `ctx.os` — the curated OS door, attributed to your plugin:
  ...
  posts a native OS notification. Fires only while the user is away from Anika
  ...
  (`anika://index-network/intent/1`), a hash path (`/index-network/intent/1`),
...
- Backend: if the plugin ships a Python `plugin_api.py` (under
  `~/.anika/plugins/<id>/dashboard/`, manifest `"api": "plugin_api.py"`), reach
...
- Do not import anything except `@anika/plugin-sdk`, `react`, and
