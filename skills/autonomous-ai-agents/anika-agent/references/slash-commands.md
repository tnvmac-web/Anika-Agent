...
Registry of record: `anika_cli/commands.py` (`COMMAND_REGISTRY`) — every
...
/snapshot [sub]          Create/restore Anika config+state snapshots (CLI)
...
/subscription (/upgrade) __NEW_ORG__ plan info (CLI)
/topup                   __NEW_ORG__ balance / billing
/copy [N]                Copy last response to clipboard (CLI)
/paste                   Attach clipboard image (CLI)
/image <path>            Attach a local image file (CLI)
/update                  Update Anika to latest
