...
For occasional contributors and PR authors. Full developer docs: https://anika-agent.__NEW_DOMAIN__/docs/developer-guide/
...
Config: `~/.anika/config.yaml` (settings), `~/.anika/.env` (API keys) — both under `$ANIKA_HOME` when it is set.
...
`_ANIKA_CORE_TOOLS` (every platform) or to a specific toolset.

All handlers must return JSON strings. Use `get_anika_home()` for paths,
never hardcode `~/.anika`. For custom/local-only tools, write a plugin in
`~/.anika/plugins/` instead of editing core — see the developer docs.

### Adding a Slash Command

1. Add `CommandDef` to `COMMAND_REGISTRY` in `anika_cli/commands.py`
...
- Tests auto-redirect `ANIKA_HOME` to temp dirs — never touch real `~/.anika/`.
...
- POSIX file modes (0o600, etc.) → `@pytest.mark.skipif(sys.platform.startswith("win"), reason="POSIX mode bits not enforced on Windows")` (see `tests/anika_cli/test_auth_toctou_file_modes.py`)
...
- Use `get_anika_home()` from `anika_constants` for all paths (profile-safe)
