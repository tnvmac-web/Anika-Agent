...
Author widget apps for the Anika TUI (`anika --tui`): glanceable ambient
...
- The TUI must be in use (`anika --tui`). Widgets do not render in the
...
1. Use `write_file` to create `~/.anika/tui-widgets/<name>.mjs` (see
...
- A thrown `register()` is logged and skipped; check
  `~/.anika/logs/tui_gateway_crash.log` if a widget never appears.
