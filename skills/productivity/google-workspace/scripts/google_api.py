#!/usr/bin/env python3
"""Google Workspace API CLI for Anika Agent.

Uses the Google Workspace CLI (`gws`) when available, but preserves the
existing Anika-facing JSON contract and falls back to the Python client
...
from _anika_home import get_anika_home
...
def _gws_binary() -> str | None:
    override = os.getenv("ANIKA_GWS_BIN")
    if override:
        return override
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace API for Anika Agent")
