...
Ledger path resolution (first wins):
  --ledger PATH
  $ANIKA_CITATION_LEDGER
  $ANIKA_HOME/cache/citations/ledger.json
"""
...
from _anika_home import get_anika_home  # noqa: E402
...
def resolve_ledger_path(explicit: str | None = None) -> Path:
    ...
    env = os.environ.get("ANIKA_CITATION_LEDGER", "").strip()
    if env:
        return Path(env).expanduser()
    return get_anika_home() / "cache" / "citations" / "ledger.json"
