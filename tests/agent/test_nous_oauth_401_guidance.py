"""Tests for the __NEW_ORG__ OAuth 401 actionable-guidance branch in
...
Regression context: ashh hit a __NEW_ORG__ 401 (OAuth token expired / portal said
account out of credits) plus a model slug ``deepseek/deepseek-v4-flash:free``
that's OpenRouter syntax, not a __NEW_ORG__ catalog name. The previous guidance
branch only covered ``openai-codex`` and ``xai-oauth``; ``nous`` fell through
to a generic "Your API key was rejected... run anika setup" message, which is
...
def test_nous_401_guidance_strings_present():
    """User-facing remediation strings for __NEW_ORG__ OAuth 401s must exist."""
    source = inspect.getsource(turn_recovery._print_nonretryable_auth_guidance)

    # Must tell the user it's an OAuth token problem, NOT an API key problem
    # (__NEW_ORG__ Portal has no API key path — auth_type=oauth_device_code only).
    assert "__NEW_ORG__ Portal OAuth token was rejected" in source

    # Must give a concrete re-auth command, not a generic "anika setup".
    assert "anika portal" in source

    # Must point at the portal so users can check account/credit status.
    assert "portal.__NEW_DOMAIN__" in source
