...
def test_reconcile_absorbed_into_beats_everything_else(curator_env):
    ...
    out = curator_env._reconcile_classification(
        ...
        destinations={"anika-agent-dev"},
        absorbed_declarations={
            "pr-review-format": {"into": "anika-agent-dev", "declared": True},
        },
    )
...
# `anika curator status`). The full data has always been in REPORT.md on
...
# ---------------------------------------------------------------------------
# Pin hint — surfaces `anika curator pin <umbrella>` in the rename block so
