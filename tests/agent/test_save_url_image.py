...
def http_server(tmp_path, monkeypatch):
    """Spin up a localhost HTTP server and isolate ANIKA_HOME under tmp_path."""
    monkeypatch.setenv("ANIKA_HOME", str(tmp_path / ".anika"))
    (tmp_path / ".anika").mkdir()

    # Force the constants/image cache helpers to re-read ANIKA_HOME.
    import sys
    for mod in list(sys.modules):
        if mod.startswith("anika_constants") or mod.startswith("agent.image_gen_provider"):
...
class TestSaveUrlImage:
    def test_writes_real_bytes_to_anika_home_cache(self, http_server):
        base, _ = http_server
        ...
        # The cache directory must be under ANIKA_HOME — gateway cleanup
