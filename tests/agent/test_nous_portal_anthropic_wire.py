"""__NEW_ORG__ Portal ``anthropic/*`` models route on the native Messages wire.

Portal serves its ``anthropic/*`` catalog at
``https://inference-api.__NEW_DOMAIN__/v1/messages`` alongside the
...
PORTAL_URL = "https://inference-api.__NEW_DOMAIN__/v1"
# Staging / preview hosts used via NOUS_INFERENCE_BASE_URL — not the prod
# hostname, so Portal behaviour must key off provider=__NEW_ORG__.
...
class TestApiModeRouting:
    ...
    def test_determine_api_mode_honors_the_model_for_nous(self):
        ...
        empty-mode path) must still land Claude on Messages — the Anika
        overlay alone advertises openai_chat for every __NEW_ORG__ model."""
        ...
        assert (
            determine_api_mode("nous", PORTAL_URL, model="anika-4-405b")
            == "chat_completions"
        )
...
class TestRuntimeResolution:
    ...
    def test_target_model_wins_over_the_persisted_default(self, monkeypatch):
        ...
        monkeypatch.setattr(
            rp,
            "_get_model_config",
            lambda: {"provider": "nous", "default": "anika-4-405b"},
        )
...
class TestClientShape:
    ...
    def test_lookalike_host_does_not_get_portal_treatment(self):
        ...
        spoofed = "https://inference-api.__NEW_DOMAIN__.attacker.test/v1"
    ...
    def test_portal_bearer_does_not_also_send_env_anthropic_api_key(
        ...
        constructor omits it. Anika loads that env from ~/.anika/.env, so
...
class TestPortalBodyFields:
    ...
    They are produced by the __NEW_ORG__ provider profile, which only the OpenAI-wire
    ...
    def test_portal_tags_reach_the_messages_request(self):
        from agent.portal_tags import anika_client_tag

        tags = self._build()["extra_body"]["tags"]

        assert "product=anika-agent" in tags
        assert anika_client_tag() in tags
    ...
    def test_non_anthropic_catalog_model_stays_on_chat_completions(self):
        ...
        with (
            patch(
                ...
                return_value=(plain, "anika-4-405b"),
            ),
            patch(
            ...
            client, model = resolve_provider_client("nous", "anika-4-405b")

        assert model == "anika-4-405b"
