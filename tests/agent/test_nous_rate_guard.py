"""Tests for agent/nous_rate_guard.py — cross-session __NEW_ORG__ Portal rate limit guard."""
...
def rate_guard_env(tmp_path, monkeypatch):
    ...
    anika_home = str(tmp_path / ".anika")
    os.makedirs(anika_home, exist_ok=True)
...
class TestAuxiliaryClientIntegration:
    ...
    def test_try_nous_skips_when_rate_limited(self, rate_guard_env, monkeypatch):
        ...
        monkeypatch.setattr(aux, "_read_nous_auth", lambda: {
            ...
            "inference_base_url": "https://api.__NEW_ORG__.test/v1",
        })
...
class TestIsGenuineNousRateLimit:
    ...
    __NEW_ORG__ Portal multiplexes upstreams (DeepSeek, Kimi, MiMo, Anika).
