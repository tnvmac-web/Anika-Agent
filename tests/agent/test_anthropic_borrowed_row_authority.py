...
def anika_home(tmp_path, monkeypatch):
    """Real on-disk ANIKA_HOME so ``load_pool()`` re-reads what it persisted."""
    home = tmp_path / "anika"
...
def test_persisted_claude_code_row_carries_no_token_material(
    ...
    rows = [r for r in _persisted_rows(anika_home) if r.get("source") == "claude_code"]
    assert len(rows) == 1
...
def test_pool_store_sync_never_adopts_a_borrowed_row(anika_home, claude_credentials):
...
def test_empty_oauth_entry_is_never_leased(anika_home, claude_credentials):
...
def test_anika_pkce_row_still_syncs_from_the_pool_store(monkeypatch):
    """The borrowed-source refusal must not disable pool-owned adoption.

    ``anika_pkce`` *is* pool-owned, so its persisted row keeps its tokens and
