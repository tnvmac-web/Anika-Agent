...
class TestNamedProfileHintIntegration:
    ...
    ``TestNamedProfileHint`` mocks ``get_anika_home``,
    ``get_default_anika_root`` and ``_resolve_active_profile_name``, so it
    ...
    ``ANIKA_HOME`` and no resolver mocks.
    """

    def test_real_anika_home_under_profiles_renders_correct_paths(
        ...
        assert get_default_anika_root() == root

        agent = _make_agent(valid_tool_names=["read_file"])
        with patch("agent.coding_context._coding_mode", return_value="off"):
            prompt = "\n\n".join(_prompt_parts(agent).values())

        assert "Active Anika profile: coder." in prompt
    ...
    def test_real_default_home_renders_default_branch(self, tmp_path, monkeypatch):
        """ANIKA_HOME at the root resolves to the default profile, unchanged."""
        root = tmp_path / ".anika"
        ...
        assert "Active Anika profile: default." in prompt
...
def test_coding_prompt_preserves_legacy_workspace_order(monkeypatch):
    ...
    monkeypatch.setattr(system_prompt, "ANIKA_AGENT_HELP_GUIDANCE", "HELP")
    monkeypatch.setattr(system_prompt, "ANIKA_AGENT_HELP_GUIDANCE_NO_SKILLS", "HELP")
    monkeypatch.setattr(system_prompt, "STEER_CHANNEL_NOTE", "STEER")
    monkeypatch.setattr(system_prompt, "get_anika_home", lambda: Path("/anika"))

    # Production renders this as str(get_anika_home()) + "/profiles/<name>/",
    # and str(Path("/anika")) is platform-dependent (backslash on Windows) —
    # build the expectation the same way instead of hardcoding "/anika".
    _home_str = str(Path("/anika"))
    expected_profile = (
    ...
    with (
        ...
        patch("anika_time.now", return_value=datetime(2026, 1, 2)),
    ):
...
class TestTelegramRichMessagesHint:
    ...
    def test_base_hint_without_rich_messages(self, monkeypatch):
        ...
        with patch("anika_cli.config.load_config_readonly") as mock_cfg:
    ...
    def test_gateway_rich_messages_integration_via_real_config(self, tmp_path, monkeypatch):
        ...
        under ANIKA_HOME with ``gateway.platforms.telegram.extra.rich_messages``
        ...
        from anika_cli import config as _cfgmod
...
def test_conversation_start_uses_session_start_not_build_time(monkeypatch):
    ...
    with (
        ...
        patch("anika_time.now", return_value=datetime(2026, 1, 2, 9, 0)),
    ):
        prompt = build_system_prompt(agent, system_message="SYSTEM_MESSAGE")
...
class TestConversationStartedTwoLine:
    ...
    def test_same_day_session_keeps_single_line(self):
        from anika_time import now as anika_now
        sid = anika_now().strftime("%Y%m%d_%H%M%S_fresh")
