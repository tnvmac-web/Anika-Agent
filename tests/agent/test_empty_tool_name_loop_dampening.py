...
@pytest.fixture()
def agent_env():
    """Spin up the mock provider + an isolated ANIKA_HOME, yield (agent, helpers)."""
    ...
    test_home = tempfile.mkdtemp(prefix="anika_e2e_47967_")
    os.makedirs(os.path.join(test_home, ".anika"))
    prev_home = os.environ.get("ANIKA_HOME")
    os.environ["ANIKA_HOME"] = os.path.join(test_home, ".anika")
    ...
    for mod in list(sys.modules):
        if mod == "run_agent" or mod.startswith("agent.") or mod.startswith("tools.") or mod.startswith("anika_"):
            del sys.modules[mod]
    ...
    try:
        ...
        # references into anika_cli.*/tools.* and their monkeypatches would otherwise
        ...
        if prev_home is None:
            ...
            os.environ["ANIKA_HOME"] = prev_home
