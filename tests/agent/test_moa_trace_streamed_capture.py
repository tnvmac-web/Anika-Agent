...
path via ``anika chat --query``) couldn't see what the aggregator actually
produced without joining to the session DB by hand.

These exercise the real ``consume_and_save_trace`` → ``save_moa_turn`` path
with real file I/O against a temp ANIKA_HOME — no mocks on the write path.
"""
...
def _enable_traces(tmp_path, monkeypatch):
    """Point ANIKA_HOME at a temp dir and turn moa.save_traces on."""
    anika_home = tmp_path / ".anika"
    anika_home.mkdir()
    monkeypatch.setenv("ANIKA_HOME", str(anika_home))

    # save_moa_turn reads config via anika_cli.config.load_config; stub it to
