...
def sample_repo(tmp_path: Path) -> Path:
    ...
    repo.mkdir()
    _git(repo, "init")
    _git(repo, "config", "user.name", "Anika Tests")
...
def test_binary_reference_block_maps_host_attachment_to_container_path(tmp_path: Path, monkeypatch):
    ...
    attachments = anika_home / "attachments"
    ...
    assert result.expanded
    # Default container base for the docker backend is /root/.anika.
    assert "/root/.anika/attachments/archive.zip" in result.message
...
def test_oversized_text_reference_maps_host_attachment_to_container_path(
    ...
    assert "/root/.anika/attachments/large.txt" in attached_context
    assert "too large to inline safely" in result.message
...
def test_binary_reference_block_keeps_host_path_on_local_backend(tmp_path: Path, monkeypatch):
    ...
    assert result.expanded
    assert str(payload) in result.message
    assert "/root/.anika/attachments/" not in result.message
...
async def test_blocks_canonical_read_denylist_credential_stores(tmp_path: Path, monkeypatch):
    ...
    a chat peer could otherwise attach `@file:~/.anika/auth.json` and read the
    ...
    oauth = anika_home / ".anthropic_oauth.json"
    oauth.write_text('{"access_token": "OAUTH-SECRET"}\n', encoding="utf-8")

    mcp_token = anika_home / "mcp-tokens" / "github.json"
    ...
    result = await preprocess_context_references_async(
        "inspect @file:.anika/auth.json and @file:.anika/.anthropic_oauth.json "
        "and @file:.anika/mcp-tokens/github.json and @file:project/.env",
        cwd=tmp_path,
...
@pytest.mark.parametrize(
    ...
    [
        "/tmp/plain.png",
        "/Users/me/Library/Application Support/Anika/composer-images/a.png",
