...
def isolated_home(monkeypatch):
    """Temp ANIKA_HOME with config + clean credential env vars."""
    test_home = tempfile.mkdtemp(prefix="anika_test_31179_")
    anika_home = os.path.join(test_home, ".anika")
...
def _fresh_modules():
    """Drop cached anika modules so each test reloads against current env."""
    _drop_reload_targets()
