...
binary download are exercised manually by `anika secrets bitwarden
...
def anika_home(tmp_path, monkeypatch):
    """Point Anika at an isolated home directory."""
    home = tmp_path / ".anika"
    home.mkdir()
    monkeypatch.setenv("ANIKA_HOME", str(home))
    # Some modules cache get_anika_home; clear if needed.
    import anika_constants
    if hasattr(anika_constants, "_ANIKA_HOME_CACHE"):
        anika_constants._ANIKA_HOME_CACHE = None  # type: ignore[attr-defined]
    return home
...
def test_install_bws_happy_path(anika_home, monkeypatch):
...
def test_env_loader_calls_bsm_when_enabled(tmp_path, monkeypatch):
    ...
    from anika_cli.env_loader import _apply_external_secret_sources
