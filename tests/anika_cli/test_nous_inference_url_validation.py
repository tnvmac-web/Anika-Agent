"""Regression tests for __NEW_ORG__ Portal inference_base_url host-allowlist validation.
...
class TestValidatorRules:
    ...
    def test_attacker_host_rejected(self, caplog):
        with caplog.at_level(logging.WARNING, logger="anika_cli.auth"):
            assert (
    ...
    def test_default_inference_url_is_in_allowlist(self):
        ...
        ``inference-api.__NEW_DOMAIN__``, they MUST update the allowlist
...
class TestCallSiteWiring:
    ...
    def _read_auth_source(self):
        # The __NEW_ORG__ refresh sites live in auth___NEW_ORG__.py (split out of auth.py);
        # read both so the guard tolerates relocation but still fires on deletion.
        import anika_cli.auth as _auth_mod
        import anika_cli.auth_nous as _nous_mod
    ...
    def test_proxy_adapter_also_validates(self):
        """The __NEW_ORG__ proxy adapter applies the validator as defense-in-depth
        ...
        boundary."""
        from pathlib import Path
        import anika_cli.proxy.adapters.nous_portal as _nous_adapter
...
class TestHealsPoisonedStoredValue:
    ...
    stale ``stg-inference-api.__NEW_DOMAIN__`` persisted before the
    ...
    def test_refresh_resets_rejected_url_to_default(self, monkeypatch):
        ...
        import anika_cli.auth_nous as anika_cli_auth_nous

        poisoned = "https://stg-inference-api.__NEW_DOMAIN__/v1"
        state = {
            "access_token": "tok",
            "refresh_token": "rtok",
            "client_id": "anika-cli",
        ...
        monkeypatch.setattr(anika_cli_auth_nous, "_nous_invoke_jwt_status", lambda *a, **k: "needs_refresh")
        monkeypatch.setattr(
            auth,
        ...
        monkeypatch.setattr(anika_cli_auth_nous, "_assert_nous_inference_jwt_usable", lambda *a, **k: None)
        monkeypatch.setattr(auth, "_select_nous_invoke_jwt", lambda *a, **k: None)
        monkeypatch.setattr(anika_cli_auth_nous, "_select_nous_invoke_jwt", lambda *a, **k: None)
...
class TestEnvOverrideWins:
    ...
    STAGING = "https://stg-inference-api.__NEW_DOMAIN__/v1"
    ...
    def _patch_no_refresh(self, monkeypatch, auth, state):
        ...
        monkeypatch.setattr(anika_cli_auth_nous, "_nous_invoke_jwt_status", lambda *a, **k: None)
        monkeypatch.setattr(
        ...
        monkeypatch.setattr(anika_cli_auth_nous, "_write_shared_nous_state", lambda *a, **k: None)
        monkeypatch.setattr(auth, "_sync_nous_pool_from_auth_store", lambda *a, **k: None)
        monkeypatch.setattr(anika_cli_auth_nous, "_sync_nous_pool_from_auth_store", lambda *a, **k: None)
...
class TestProxyAdapterEnvOverride:
    """The __NEW_ORG__ proxy adapter is the second chokepoint: it re-validates the
