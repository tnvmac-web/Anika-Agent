...
When ``ANIKA_HOME`` points to a named profile, ``read_credential_pool()``
...
def profile_env(tmp_path, monkeypatch):
    """Set up a global root + an active profile under Path.home()/.anika/profiles/coder.

    * Path.home() -> tmp_path
    * Global root -> tmp_path/.anika            (has its own auth.json fixture)
    * Profile     -> tmp_path/.anika/profiles/coder   (active, ANIKA_HOME points here)
...
# ``_load_provider_state`` itself, those helpers raise ``"Anika is not
# logged into __NEW_ORG__ Portal"`` even though the user has a valid global Nous
...
def classic_env(tmp_path, monkeypatch):
    """Classic single-root layout (ANIKA_HOME != ~/.anika, no profiles)."""
    fake_home = tmp_path / "home"
    fake_home.mkdir()
    monkeypatch.setattr(Path, "home", lambda: fake_home)
    anika_home = tmp_path / "classic"
    anika_home.mkdir()
