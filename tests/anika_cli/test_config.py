"""Tests for anika_cli configuration management."""
    ...
    def test_default_path(self):
        with patch.dict(os.environ, {}, clear=False):
            ...
            if sys.platform == "win32":
                # Windows default is %LOCALAPPDATA%\anika — see
                # anika_constants._get_platform_default_anika_home.
                ...
                assert home == base / "anika"
            else:
                assert home == Path.home() / ".anika"


class TestEnsureAnikaHome:
    ...
    def test_upgrades_legacy_template_soul_md(self, tmp_path):
        ...
        from anika_cli.default_soul import DEFAULT_SOUL_MD, _LEGACY_TEMPLATE_SOULS
    ...
    _PRE_REWRITE_DEFAULT_SOUL = (
        "You are Anika Agent, an intelligent AI assistant created by __NEW_ORG__ "
...
class TestLoadConfigParseFailure:
    ...
    their config.yaml was being ignored. The helper must:
      * log at WARNING (so ``anika logs`` surfaces it)
    ...
    def test_corrupt_config_is_backed_up(self, tmp_path, capsys):
        ...
        """
        from anika_cli import config as cfg_mod
...
class TestSaveAndLoadRoundtrip:
    ...
    def test_atomic_config_write_refuses_unparseable_existing_config(self, tmp_path):
        ...
        from anika_cli.config import atomic_config_write
...
class TestSaveEnvValueSecure:
    ...
    def test_save_env_value_quotes_values_containing_hash(self, tmp_path):
        ...
        with patch.dict(os.environ, {"ANIKA_HOME": str(tmp_path)}, clear=False):
...
class TestRemoveEnvValue:
    def test_removes_key_from_env_file(self, tmp_path):
        ...
        with patch.dict(os.environ, {"ANIKA_HOME": str(tmp_path), "KEY_B": "value_b"}):
    ...
    def test_clears_os_environ_even_when_not_in_file(self, tmp_path):
        ...
        with patch.dict(os.environ, {"ANIKA_HOME": str(tmp_path), "ORPHAN_KEY": "orphan"}):
    ...
    def test_remove_env_value_preserves_existing_file_mode_on_posix(self, tmp_path):
        ...
        with patch.dict(os.environ, {"ANIKA_HOME": str(tmp_path), "DROP": "gone"}):
...
class TestSanitizeEnvLines:
    ...
    def test_migrate_reports_normalized_line_formatting(self, capsys):
        ...
        with (
            patch("anika_cli.config.sanitize_env_file", return_value=2),
            patch(
                "anika_cli.config.check_config_version",
                return_value=(latest_version, latest_version),
            ),
            patch("anika_cli.config.read_raw_config", return_value={}),
            patch("anika_cli.config.get_missing_env_vars", return_value=[]),
            patch("anika_cli.config.get_missing_config_fields", return_value=[]),
            patch("anika_cli.config.get_missing_skill_config_vars", return_value=[]),
        ):
...
class TestOptionalEnvVarsRegistry:
    ...
    def test_tavily_in_env_vars_by_version(self):
        ...
        from anika_cli.config import ENV_VARS_BY_VERSION
        all_vars = []
    ...
    def test_max_iterations_not_offered_as_env_var(self):
        """ANIKA_MAX_ITERATIONS must NOT be in OPTIONAL_ENV_VARS (issue #17534).

        Offering it as an editable env var (dashboard, `anika setup`) lets a
        ...
        via config.yaml; ANIKA_MAX_ITERATIONS remains a read-only backward-compat
        fallback in the gateway/CLI, never a promoted write target.
        """
        from anika_cli.config import OPTIONAL_ENV_VARS
        assert "ANIKA_MAX_ITERATIONS" not in OPTIONAL_ENV_VARS
...
class TestMemoryProviderEnvVarsRegistry:
    ...
    and `anika setup` surface it (previously only Honcho was listed, leaving
...
class TestConfigSupportFloor:
    ...
    def test_v11_config_is_refused_and_untouched(self, tmp_path, capsys):
        ...
        assert "run `anika setup` to regenerate" in captured.out
    ...
    def test_floor_message_uses_display_anika_home(self):
        from anika_cli.config_migrations import support_floor_message
        from anika_constants import display_anika_home

        msg = support_floor_message()
        assert f"{display_anika_home()}/config.yaml" in msg
...
class TestCustomProviderCompatibility:
    ...
    def _run_ladder(current_ver: int):
        from anika_cli.config_migrations import run_migrations
...
class TestEnvWriteDenylist:
    ...
    ``PATH``, ``EDITOR``, etc. — or selected Anika runtime/security controls.
    ...
    ``LD_PRELOAD=/tmp/evil.so`` in ``.env`` and own the next Anika
    process on next startup via the dotenv → ``os.environ`` chain in
    ``anika_cli/env_loader.py``.
    ...
    def _anika_home(self, tmp_path, monkeypatch):
    ...
    @pytest.mark.parametrize(
        "allowed_key",
        [
            "ANIKA_LANGFUSE_PUBLIC_KEY",
            "ANIKA_SPOTIFY_CLIENT_ID",
            "ANIKA_QWEN_BASE_URL",
            "ANIKA_MAX_ITERATIONS",
        ],
    )
    def test_anika_integration_keys_still_writable(self, allowed_key):
        """``ANIKA_*`` overall is NOT blocked.
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            "ANIKA_ENV_PATH",
            "ANIKA_OPTIONAL_MCPS",
            "ANIKA_COPILOT_ACP_COMMAND",
            "ANIKA_COPILOT_ACP_ARGS",
            "ANIKA_YOLO_MODE",
            "ANIKA_ACCEPT_HOOKS",
            ...
            "ANIKA_CRON_SESSION",
            "ANIKA_SINGLE_QUERY_SESSION",
            "ANIKA_SESSION_KEY",
            "ANIKA_SESSION_PLATFORM",
        ],
    )
    def test_anika_security_control_keys_are_not_writable(self, protected_key):
    ...
    def test_preexisting_optional_mcps_override_still_loads(self, tmp_path):
        ...
        from anika_cli.config import invalidate_env_cache

        catalog = tmp_path / "custom-mcp-catalog"
        (tmp_path / ".env").write_text(
            f"ANIKA_OPTIONAL_MCPS={catalog}\n",
            encoding="utf-8",
        )
        invalidate_env_cache()

        assert load_env()["ANIKA_OPTIONAL_MCPS"] == str(catalog)

    @pytest.mark.parametrize(
        ("key", "expected"),
        [
            ("Path", "PATH"),
            ("Anika_Yolo_Mode", "ANIKA_YOLO_MODE"),
            ("Anika_Optional_Mcps", "ANIKA_OPTIONAL_MCPS"),
            ("Anika_Copilot_Acp_Command", "ANIKA_COPILOT_ACP_COMMAND"),
            ("Anika_Copilot_Acp_Args", "ANIKA_COPILOT_ACP_ARGS"),
        ],
    )
    def test_windows_policy_names_are_case_insensitive(self, key, expected):
        from anika_cli.config import _env_var_policy_name
    ...
    def test_windows_env_assignment_matching_is_case_insensitive(self, prefix):
        from anika_cli.config import _env_line_defines_key
...
"""
    ...
    def test_persist_migration_writes_full_read_raw_config(self, tmp_path):
        from anika_cli.config import _persist_migration, read_raw_config
    ...
    def test_preserves_existing_codex_app_server_auto_value(self, tmp_path):
        with patch.dict(os.environ, {"ANIKA_HOME": str(tmp_path)}):
            self._write(
                ...
                "_config_version: 31\n"
                "compression:\n"
                "  codex_app_server_auto: anika\n",
            )

            migrate_config(interactive=False, quiet=True)

            raw = yaml.safe_load((tmp_path / "config.yaml").read_text())
            assert raw["compression"]["codex_app_server_auto"] == "anika"
...
def test_default_config_has_no_duplicate_top_level_keys():
    ...
    import ast
    import anika_cli.config as cfg_mod
...
class TestConfigCommandFailClosedSurface:
    """`anika config set/unset` must exit cleanly (no traceback) when the
