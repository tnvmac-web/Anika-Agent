...
from anika_cli.models_pricing import compute_sale_discount, fetch_models_with_pricing
...
def test_free_model_gets_flat_100_percent_discount():
    ...
    # "0.0000000000" strings (__NEW_ORG__ portal shape) count as free too.
...
def test_fetch_models_with_pricing_copies_nested_original(monkeypatch):
    ...
    # __NEW_ORG__ Portal opts in via include_sale_original=True.
...
def test_resolve_nous_pricing_credentials_honors_inference_env_override(monkeypatch):
    ...
    monkeypatch.setenv(
        ...
        "https://stg-inference-api.__NEW_DOMAIN__/v1",
    )
    ...
    assert base_url == "https://stg-inference-api.__NEW_DOMAIN__"
...
def test_resolve_nous_pricing_credentials_normalizes_either_suffix(monkeypatch):
    ...
    monkeypatch.setattr(
        "anika_cli.auth.resolve_nous_runtime_credentials", lambda: None
    )
    for override in (
        "https://stg-inference-api.__NEW_DOMAIN__",
        "https://stg-inference-api.__NEW_DOMAIN__/",
        "https://stg-inference-api.__NEW_DOMAIN__/v1",
        "https://stg-inference-api.__NEW_DOMAIN__/v1/",
    ):
