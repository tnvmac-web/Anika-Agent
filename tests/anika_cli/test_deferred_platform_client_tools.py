...
loader so ``anika chat`` doesn't import ~20 gateway SDKs. The a2a plugin ships
...
``anika tools`` checklist. The same tools worked in gateway/web processes only
...
def _write_platform_plugin(
    ...
    *,
    ...
    """
    from anika_cli.plugins import PluginManifest
...
def clean_registry():
    ...
    ``probeplat`` name, so the imported ``anika_plugins.*`` modules have to go
    ...
    for name in set(sys.modules) - before_modules:
        if name.startswith("anika_plugins."):
...
class TestA2AClientToolsInCliProcess:
    ...
    def test_a2a_appears_in_the_anika_tools_checklist(self):
    ...
    def test_platform_bundle_includes_the_client_tools(self):
        ...
        assert A2A_CLIENT_TOOLS.issubset(set(resolve_toolset("anika-a2a")))
...
class TestDeferredPlatformToolPreregistration:
    ...
    def test_tools_stay_attributed_after_materialization(
        ...
        """`anika plugins list` must still credit the pre-registered tools.
    ...
    def test_partially_registered_tools_are_still_attributed(
        ...
        `anika plugins list` under-report what the process is carrying, and
    ...
    def test_failed_materialization_tears_down_pre_registered_tools(
        ...
        adapter. Attribution and the registry therefore agree at zero: `anika
