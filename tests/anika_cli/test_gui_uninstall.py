"""Tests for anika_cli.gui_uninstall — GUI-only uninstall + install discovery.
...
import anika_cli.gui_uninstall as gu


def _make_agent(anika_home: Path) -> Path:
    """Create a fake agent install: source package + venv."""
    agent_root = anika_home / "anika-agent"
    (agent_root / "anika_cli").mkdir(parents=True)
    (agent_root / "anika_cli" / "__init__.py").write_text("")
    (agent_root / "venv" / "bin").mkdir(parents=True)
    return agent_root


def _make_gui_build(anika_home: Path) -> None:
    """Create the source-built GUI artifacts a `anika desktop` run produces."""
    desktop = anika_home / "anika-agent" / "apps" / "desktop"
    ...
    (anika_home / "anika-agent" / "node_modules").mkdir(parents=True)
    (anika_home / "desktop-build-stamp.json").write_text("{}")


def _make_user_data(anika_home: Path) -> None:
    (anika_home / "config.yaml").write_text("x: 1\n")
    (anika_home / ".env").write_text("KEY=secret\n")
    (anika_home / "sessions").mkdir()
...
def test_gui_install_summary_shape(tmp_path, monkeypatch):
    ...
    _make_agent(anika_home)
    _make_gui_build(anika_home)
    monkeypatch.setattr(gu, "packaged_gui_app_paths", lambda: [])
    monkeypatch.setattr(gu, "desktop_userdata_dir", lambda: tmp_path / "none")

    summary = gu.gui_install_summary(anika_home)
    ...
    assert summary["anika_home"] == str(anika_home)
...
def test_linux_discovery_includes_launcher_entry(tmp_path, monkeypatch):
    """The launcher entry that `anika desktop` installs is removable."""
...
def test_uninstall_removes_launcher_entry_and_refreshes_cache(tmp_path, monkeypatch):
    ...
    anika_home = tmp_path / ".anika"
    _make_agent(anika_home)
    icon = lde.icon_path(anika_home / "anika-agent")
    icon.parent.mkdir(parents=True, exist_ok=True)
    icon.write_bytes(b"\x89PNG")
    monkeypatch.setattr(gu, "desktop_userdata_dir", lambda: tmp_path / "none")

    removed = gu.uninstall_gui(anika_home)
    ...
    assert lde.icon_path(anika_home / "anika-agent").exists()
    # The agent itself survives a GUI uninstall.
    assert (anika_home / "anika-agent" / "anika_cli").is_dir()
...
def test_uninstall_skips_cache_refresh_when_no_launcher_entry(tmp_path, monkeypatch):
    ...
    gu.uninstall_gui(tmp_path / ".anika")
...
def test_uninstall_args_namespace_mode_mapping():
    ...
    import anika_cli.uninstall as uninstall
