"""Tests for anika_cli.relaunch — unified self-relaunch utility."""

import sys

import pytest

from anika_cli import relaunch as relaunch_mod


class TestResolveAnikaBin:
    def test_prefers_absolute_argv0_when_executable(self, monkeypatch):
        fake = "/nix/store/abc/bin/anika"
        ...
        assert relaunch_mod.resolve_anika_bin() == fake
    ...
    def test_resolves_relative_argv0(self, monkeypatch, tmp_path):
        ...
        # Ensure we don't accidentally match a real 'anika' on PATH
        monkeypatch.setattr(relaunch_mod.shutil, "which", lambda _name: None)
        assert relaunch_mod.resolve_anika_bin() == str(fake)

    def test_falls_back_to_path_which(self, monkeypatch):
        monkeypatch.setattr(sys, "argv", ["-c"])  # not a real path
        monkeypatch.setattr(
            relaunch_mod.shutil, "which", lambda name: "/usr/bin/anika" if name == "anika" else None
        )
        assert relaunch_mod.resolve_anika_bin() == "/usr/bin/anika"
...
class TestBuildRelaunchArgv:
    def test_uses_bin_when_available(self, monkeypatch):
        monkeypatch.setattr(relaunch_mod, "resolve_anika_bin", lambda: "/usr/bin/anika")
        argv = relaunch_mod.build_relaunch_argv(["--resume", "abc"])
        assert argv[0] == "/usr/bin/anika"
    ...
    def test_can_disable_preserve(self, monkeypatch):
        ...
        assert argv == ["/usr/bin/anika", "--resume", "abc"]
...
class TestRelaunch:
    def test_calls_execvp(self, monkeypatch):
        ...
        assert calls == [("/usr/bin/anika", ["/usr/bin/anika", "--resume", "abc"])]
    ...
    def test_windows_uses_subprocess_not_execvp(self, monkeypatch):
        ...
        anika).  relaunch() must detect win32 and use subprocess.run +
        sys.exit instead.
        ...
        that motivated it.
        """
        monkeypatch.setattr(relaunch_mod, "resolve_anika_bin", lambda: r"C:\Users\test\anika.exe")
        ...
        monkeypatch.setattr(relaunch_mod.sys, "argv", [r"C:\Users\test\anika.exe"])
        ...
        assert execvp_calls == []
        assert captured_argv == [[r"C:\Users\test\anika.exe", "chat"]]
    ...
    def test_windows_propagates_child_exit_code(self, monkeypatch):
        ...
        monkeypatch.setattr(relaunch_mod, "resolve_anika_bin", lambda: r"C:\anika.exe")
...
class TestResolveAnikaBinWindowsPyGuard:
    """On Windows, resolve_anika_bin MUST NOT return a .py path.
    ...
    def test_windows_rejects_py_argv0_falls_through_to_path(self, monkeypatch, tmp_path):
        ...
        # Force PATH lookup to return a anika.exe so the test doesn't
        # exercise the None-fallback path (that's a separate test).
        monkeypatch.setattr(
            relaunch_mod.shutil, "which",
            lambda name: r"C:\venv\Scripts\anika.exe" if name == "anika" else None,
        )

        bin_path = relaunch_mod.resolve_anika_bin()
        # Must NOT be the .py — must be the anika.exe PATH entry.
        assert bin_path == r"C:\venv\Scripts\anika.exe"
    ...
    def test_posix_still_accepts_py_argv0(self, monkeypatch, tmp_path):
        ...
        script = tmp_path / "anika"
        script.write_text("#!/usr/bin/env python3\n")
        script.chmod(0o755)
        monkeypatch.setattr(relaunch_mod.sys, "argv", [str(script), "chat"])
        assert relaunch_mod.resolve_anika_bin() == str(script)

    @pytest.mark.windows_only
    def test_windows_py_argv0_with_no_anika_on_path_returns_none(self, monkeypatch, tmp_path):
        """Bulletproof fallback: if argv0 is .py on Windows AND anika.exe
        ...
        assert relaunch_mod.resolve_anika_bin() is None
