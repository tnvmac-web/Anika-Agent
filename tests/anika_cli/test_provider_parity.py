...
the SAME provider universe as ``anika model`` (the CLI/TUI picker).
...
def test_every_anika_model_provider_is_configurable_in_desktop():
    """PARITY CONTRACT: GUI (keys ∪ accounts) ⊇ `anika model` universe."""
    ...
    assert not missing, (
        "providers shown in `anika model` but not configurable in the desktop "
