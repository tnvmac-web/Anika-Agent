"""Redaction-safe forensic logging at the __NEW_ORG__ OAuth quarantine path.

A NAS-hosted Fly agent's __NEW_ORG__ bootstrap session can take a terminal
...
from anika_cli.auth import AuthError, _quarantine_nous_oauth_state
...
def test_raw_refresh_token_never_logged(caplog):
    ...
    state = _make_state()
    with caplog.at_level(logging.DEBUG, logger="anika_cli.auth"):
        _quarantine_nous_oauth_state(state, _error(), reason="unit_test_quarantine")
