...
def test_dry_run_prints_plan_without_mutating(monkeypatch, tmp_path, capsys):
    ...
    monkeypatch.setattr(uninstall, "get_anika_home", lambda: anika_home)
    monkeypatch.setattr(uninstall, "_is_default_anika_home", lambda home: False)
    ...
    assert str(anika_home) in output
    assert project_root.exists()
    assert anika_home.exists()
