"""Tests for the per-profile Projects store (anika_cli/projects_db)."""
...
def test_create_get_list(conn):
    pid = pdb.create_project(conn, name="Anika Agent", folders=["/tmp/anika"])
    proj = pdb.get_project(conn, pid)

    assert proj is not None
    assert proj.slug == "anika-agent"
    assert proj.name == "Anika Agent"
    # First folder becomes primary.
    assert proj.primary_path == "/tmp/anika"
    assert [f.path for f in proj.folders] == ["/tmp/anika"]
    assert proj.folders[0].is_primary is True

    # Lookup by slug too.
    assert pdb.get_project(conn, "anika-agent").id == pid
...
def test_per_profile_isolation(tmp_path):
    # Two distinct DB paths stand in for two profiles' ANIKA_HOME.
