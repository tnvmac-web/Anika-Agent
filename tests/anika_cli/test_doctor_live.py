"""Tests for ``anika doctor --live`` — opt-in bounded real-call tool-backend probes.
...
from anika_cli import doctor_live
from anika_cli.doctor_live import (
    ProbeResult,
...
class TestBrowserAvailableNpxRung:
    ...
    with. It must fall through to the same cascade `anika doctor` uses."""

    def _block_path_and_node_modules_checks(self, monkeypatch, tmp_path):
        monkeypatch.setattr("shutil.which", lambda *a, **k: None)
        monkeypatch.setattr("anika_cli.doctor.ANIKA_HOME", tmp_path / "home")
        monkeypatch.setattr("anika_cli.doctor.PROJECT_ROOT", tmp_path / "root")
