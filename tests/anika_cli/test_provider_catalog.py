"""Tests for the unified provider catalog (anika_cli.provider_catalog).

These are invariant tests, not snapshots: they assert the parity *contract*
between what ``anika model`` shows (``CANONICAL_PROVIDERS``) and what the
...
from anika_cli.provider_catalog import (
    ProviderDescriptor,
    provider_catalog,
