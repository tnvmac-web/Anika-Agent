...
which broke ``anika --tui`` because ``ui-tui/`` had vanished.
...
from anika_cli.update_cmd import _atomic_replace_dir
...
def test_atomic_replace_swaps_content_on_success(tmp_path: Path) -> None:
    ...
    assert not (dst.parent / "ui-tui.anika-update-staging").exists()
    assert not (dst.parent / "ui-tui.anika-update-old").exists()
