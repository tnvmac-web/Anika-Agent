...
``_load_auth_store`` and the Codex/__NEW_ORG__ shared-store readers previously called
...
import anika_cli.auth as auth
import anika_cli.auth_codex as auth_codex
...
def anika_home(tmp_path, monkeypatch):
    """Point ANIKA_HOME at a tmp dir so we never touch the real auth store.

    Required because ``_auth_file_path()`` has a seat belt that refuses to
    resolve to the real user's ~/.anika/auth.json under pytest.
    """
...
class TestAuthStoreEncodingRoundTrip:
    def test_load_reads_utf8_with_non_ascii_label(self, anika_home, windows_default_encoding):
    ...
    def test_load_does_not_corrupt_store_on_non_ascii(self, anika_home, windows_default_encoding):
        ...
        assert not (anika_home / "auth.json.corrupt").exists()
    ...
    def test_load_handles_utf8_with_bom(self, anika_home):
...
class TestExplicitEncodingPassed:
    ...
    def test_load_auth_store_passes_utf8_encoding(self, anika_home):
        auth_path = anika_home / "auth.json"
...
# --- sibling readers of the same ~/.anika/auth.json in other modules -------
#
# _load_auth_store lives in anika_cli/auth.py, but several other modules read
# the same ~/.anika/auth.json directly. They had the same UTF-8-vs-cp1252
# asymmetry on Windows — these tests pin the sibling reads too.

class TestAuthJsonSiblingReaders:
    def test_has_xai_credentials_reads_non_ascii_store(self, anika_home, windows_default_encoding, monkeypatch):
        ...
        _write_utf8(anika_home / "auth.json", store)

        from tools.xai_http import has_xai_credentials

        assert has_xai_credentials() is True

    def test_auxiliary_nous_provider_reads_non_ascii_store(self, anika_home, windows_default_encoding, monkeypatch):
        """agent/auxiliary_client's Nous-provider lookup reads the same store.

        The lookup returns None on any read failure, silently disabling __NEW_ORG__ as
        ...
        # ANIKA_HOME env from the fixture doesn't reach it — point it at
        # the tmp store explicitly.
        monkeypatch.setattr(aux, "_AUTH_JSON_PATH", anika_home / "auth.json")
    ...
    def test_read_shared_nous_state_reads_non_ascii_store(
        ...
        """anika_cli.auth._read_shared_nous_state must read a non-ASCII store.

        The shared __NEW_ORG__ store (``nous_auth.json``) is written as UTF-8. A
    ...
    def test_has_any_provider_configured_reads_non_ascii_auth_store(
        self, anika_home, monkeypatch, windows_default_encoding
    ):
        """anika_cli.main._has_any_provider_configured reads auth.json.
        ...
        # anika_cli.auth; patch it on the source module so the local import
        # sees the fake.
    ...
    def test_managed_tool_gateway_reads_non_ascii_nous_state(
        self, anika_home, windows_default_encoding
    ):
        """tools.managed_tool_gateway._read_nous_provider_state reads auth.json.

        The __NEW_ORG__ provider entry can carry a non-ASCII label. Under the
        ...
        assert __NEW_ORG__.get("agent_key") == "k"
        # The non-ASCII label round-trips intact.
        assert __NEW_ORG__.get("label") == "工作账号"
