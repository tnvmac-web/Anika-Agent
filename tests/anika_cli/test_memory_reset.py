"""Tests for the `anika memory reset` CLI command.
...
- Profile-scoped reset (uses ANIKA_HOME)
"""
...
def memory_env(tmp_path, monkeypatch):
    """Set up a fake ANIKA_HOME with memory files."""
    anika_home = tmp_path / ".anika"
    memories = anika_home / "memories"
    memories.mkdir(parents=True)
    monkeypatch.setenv("ANIKA_HOME", str(anika_home))

    # Create sample memory files
    (memories / "MEMORY.md").write_text(
        "§\nAnika repo is at ~/.anika/anika-agent\n§\nUser prefers dark themes",
        encoding="utf-8",
    )
...
def _run_memory_reset(target="all", yes=False, monkeypatch=None, confirm_input="no"):
    ...
    Simulates what happens when `anika memory reset` is run.
    """
...
class TestMemoryReset:
    ...
    def test_reset_no_files_exist(self, tmp_path, monkeypatch):
        ...
        anika_home = tmp_path / ".anika"
        (anika_home / "memories").mkdir(parents=True)
        monkeypatch.setenv("ANIKA_HOME", str(anika_home))
