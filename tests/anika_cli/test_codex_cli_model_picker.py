...
Covers:
 - Normal path (tokens already in Anika auth store)
 - Claude Code fallback (tokens only in ~/.claude/.credentials.json)
 - Negative case (no credentials anywhere)

Note: auto-import from ~/.codex/auth.json was removed in #12360 — Anika
now owns its own openai-codex auth state, and users explicitly adopt
existing Codex CLI tokens via `anika auth openai-codex`. The old
...
@pytest.fixture()
def anika_auth_only_env(tmp_path, monkeypatch):
    """Tokens already in Anika auth store (no Codex CLI needed)."""
...
def test_normal_path_still_works(anika_auth_only_env):
    """openai-codex appears when tokens are already in Anika auth store."""
...
def claude_code_only_env(tmp_path, monkeypatch):
    ...
    ~/.claude/.credentials.json (Claude Code) — not in env vars or Anika
    auth store."""
    anika_home = tmp_path / ".anika"
    anika_home.mkdir()
