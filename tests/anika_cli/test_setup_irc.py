"""Tests for IRC gateway configuration via `anika setup gateway` UI.

Covers the full plugin-platform discovery → status → configure flow so that
a fresh Anika install (no state, no env vars) can set up IRC through the
...
class TestIRCFreshInstallDiscovery:
    """IRC appears in the setup menu on a brand-new Anika install."""
    ...
    def test_irc_status_configured_when_env_set(self, monkeypatch):
        ...
        try:
            ...
            monkeypatch.setenv("IRC_CHANNEL", "#anika")
            monkeypatch.setenv("IRC_NICKNAME", "anika-bot")
...
class TestIRCGatewaySetupFreshInstall:
    """Simulate the full `anika setup gateway` experience with IRC present."""
