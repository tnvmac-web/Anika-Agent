...
profile switcher can target any profile's ANIKA_HOME. These tests pin:
reads/writes land in the REQUESTED profile, the dashboard's own profile
stays untouched, and the chat PTY env is scoped via ANIKA_HOME.
"""
import json
...
import anika_cli.web_server_profiles as _web_server_profiles


@pytest.fixture
def isolated_profiles(tmp_path, monkeypatch, _isolate_anika_home):
    ...
    monkeypatch.setattr(profiles, "_get_default_anika_home", lambda: default_home)
...
class TestProfileScopedMcp:
    ...
    def test_mcp_test_oauth_server_without_token_is_not_ok(
        ...
        import anika_cli.mcp_config as mcp_config
...
class TestProfileScopedModel:
    ...
    def test_model_options_uses_config_only_scope_for_selected_profile(
        ...
        monkeypatch.setattr(
            "anika_cli.inventory.load_picker_context", lambda: object()
        )
        monkeypatch.setattr(
            "anika_cli.inventory.build_model_options_payload",
...
class TestProfileScopedPostSetup:
    def test_post_setup_spawns_with_profile_flag(
        ...
        config / write per-profile state see the same ANIKA_HOME the rest
        ...
        monkeypatch.setattr(
            "anika_cli.tools_config.valid_post_setup_keys",
...
class TestProfileScopedGateway:
    ...
    def test_status_reads_requested_profile_home(
        ...
        def fake_get_running_pid(*args, **kwargs):
            ...
            seen_homes.append(str(get_anika_home()))
            return None
        ...
        assert resp.json()["anika_home"] == str(isolated_profiles["worker_beta"])
...
class TestProfileScopedChatPty:
    def test_chat_argv_scopes_anika_home(self, isolated_profiles, monkeypatch):
        import anika_cli.web_server as web_server

        monkeypatch.setattr(
            "anika_cli.main_tui_launch._make_tui_argv",
            lambda root, tui_dev=False: (["cat"], None),
            raising=False,
        )
        argv, cwd, env = _web_server_chat._resolve_chat_argv(profile="worker_beta")
        assert env is not None
        assert env["ANIKA_HOME"] == str(isolated_profiles["worker_beta"])
        # Scoped chat must NOT attach to the dashboard's in-memory gateway.
        assert "ANIKA_TUI_GATEWAY_URL" not in env
