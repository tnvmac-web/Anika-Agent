...
class TestUnifiedDashboardRouting:
    ...
    def test_profile_launch_reexecs_machine_dashboard(self, main_mod, monkeypatch):
        ...
        monkeypatch.setattr(
            "anika_cli.profiles.get_active_profile_name", lambda: "worker_x"
        )
        ...
        # ANIKA_HOME.  For a standard install (ANIKA_HOME unset) that root is
        # the platform-native default (~/.anika), NOT dropped — see the Docker
        # test below for why we resolve explicitly instead of popping.
        from anika_constants import get_default_anika_root
        assert env.get("ANIKA_HOME") == str(get_default_anika_root())


    def test_desktop_profile_backend_skips_machine_dashboard_reroute(self, main_mod, monkeypatch):
        """A desktop-spawned named-profile backend (ANIKA_DESKTOP=1) must NOT
