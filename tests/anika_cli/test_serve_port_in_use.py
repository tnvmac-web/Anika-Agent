...
``anika serve``, the gateway, anything) → ``anika serve`` printed only
...
* conflict → single stdout sentinel ``BACKEND_PORT_IN_USE port=<port>`` +
  ...
  transient-condition convention) — and NO ``ANIKA_BACKEND_READY``.
* free explicit port → boots and announces ``ANIKA_BACKEND_READY`` exactly
...
def test_probe_detects_held_socket():
    from anika_cli.web_server_lifecycle import _port_bind_conflict
...
def test_addr_in_use_error_classification():
    ...
    from anika_cli.web_server_lifecycle import _is_addr_in_use_error
...
def test_exit_code_is_distinct_tempfail():
    from anika_cli.web_server_lifecycle import PORT_IN_USE_EXIT_CODE
...
# ---------------------------------------------------------------------------
# E2E: real start_server in a subprocess (temp ANIKA_HOME)
# ---------------------------------------------------------------------------
...
def _spawn_serve(port: int, tmp_path: Path, merge_stderr: bool = True) -> subprocess.Popen:
    ...
    env.update(
        ANIKA_HOME=str(home),
        ANIKA_SERVE_HEADLESS="1",
        PYTHONUNBUFFERED="1",
    )
    # Ensure a stray desktop-parent env can't arm the watchdog/reaper paths.
    for k in ("ANIKA_DESKTOP", "ANIKA_PARENT_PID", "ANIKA_PARENT_START_MARKER",
              "ANIKA_PARENT_NONCE"):
        env.pop(k, None)
    code = (
...
def test_conflict_emits_sentinel_and_exit_75(tmp_path):
    ...
    assert "ANIKA_BACKEND_READY" not in out  # never claimed ready
...
def test_free_port_boots_and_announces_ready(tmp_path):
    ...
    proc = _spawn_serve(port, tmp_path)
    try:
        ready, lines = _read_until(proc, "ANIKA_BACKEND_READY")
        out = "".join(lines)
        assert ready, f"no READY sentinel; output:\n{out}"
        assert f"ANIKA_BACKEND_READY port={port}" in out
...
def test_ephemeral_port_zero_unaffected(tmp_path):
    ...
    try:
        ...
        ready_line = next(l for l in lines if "ANIKA_BACKEND_READY" in l)
