"""Regression tests for the __NEW_ORG__ Portal env-override bypassing the host
...
``ANIKA_PORTAL_BASE_URL=https://portal.staging-__NEW_DOMAIN__`` in its
...
class TestPortalEnvOverrideHelper:
    def test_none_when_unset(self, monkeypatch):
        monkeypatch.delenv("ANIKA_PORTAL_BASE_URL", raising=False)
    ...
    def test_env_override_not_gated_by_allowlist(self, monkeypatch):
        ...
        monkeypatch.setenv(
            "ANIKA_PORTAL_BASE_URL", "https://portal.staging-__NEW_DOMAIN__"
        )
        assert "portal.staging-__NEW_DOMAIN__" not in _NOUS_PORTAL_ALLOWED_HOSTS
        assert (
            _nous_portal_env_override() == "https://portal.staging-__NEW_DOMAIN__"
        )
...
class TestResolveAccessTokenEnvOverrideWins:
    ...
    def test_env_override_wins_even_with_staging_state_stored(
        ...
        a prior ANIKA_AUTH_JSON_BOOTSTRAP seed), and the env var is set to
        ...
        staging_portal = "https://portal.staging-__NEW_DOMAIN__"
        monkeypatch.setenv("ANIKA_HOME", str(tmp_path))
        monkeypatch.setenv("ANIKA_PORTAL_BASE_URL", staging_portal)
