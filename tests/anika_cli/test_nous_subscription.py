"""Tests for __NEW_ORG__ subscription feature detection."""

import shutil
import sys

from anika_cli.nous_account import NousPortalAccountInfo, NousToolAccessInfo
from anika_cli import nous_subscription as ns
...
def _capture_checklist(monkeypatch, *, selected_idx):
    ...
    import anika_cli.setup as setup_mod

    monkeypatch.setattr(setup_mod, "prompt_checklist", _fake_checklist, raising=False)
    monkeypatch.setattr(
        "anika_cli.config.save_config", lambda cfg: None, raising=False
    )
    return captured
...
def test_logged_in_entitled_account_yields_a_state_for_every_feature(monkeypatch):
    ...
    crashing `anika status`, `anika tools`, and the dashboard toolsets API."""
...
def test_get_gateway_eligible_tools_treats_explicit_backend_as_configured(monkeypatch):
    ...
    single Enter during `anika model` overwrote it to `web.backend: nous`.
    """
...
def test_get_gateway_eligible_tools_not_entitled_returns_four_empty_lists(monkeypatch):
    """A logged-in __NEW_ORG__ account with no paid access and no free tool pool
...
def test_prompt_enable_tool_gateway_not_entitled_does_not_crash(monkeypatch):
    ...
    not raise when a __NEW_ORG__ account is logged in but not entitled to the Tool
...
def test_prompt_enable_tool_gateway_persists_decline(monkeypatch):
    ...
    captured = _capture_checklist(monkeypatch, selected_idx=[])
    monkeypatch.setattr(
        "anika_cli.config.save_config", lambda cfg: saved.append(dict(cfg)), raising=False
    )
...
def test_apply_nous_managed_defaults_writes_video_gen_config(monkeypatch):
    ...
    when a __NEW_ORG__ subscriber selects video_gen without a direct FAL_KEY."""
...
# ---------------------------------------------------------------------------
# ensure_nous_portal_access — inline login gate for `anika tools`
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# STT — managed-by-__NEW_ORG__ detection (Phase 4 follow-up)
# ---------------------------------------------------------------------------
...
def _block_legacy_agent_browser_checks(monkeypatch):
    ...
    monkeypatch.setattr("anika_constants.agent_browser_runnable", lambda path: False)
...
def test_has_agent_browser_import_failure_falls_back_to_path_check(monkeypatch):
    ...
    monkeypatch.setattr(
        "anika_constants.agent_browser_runnable",
...
def test_has_agent_browser_import_failure_falls_back_to_anika_managed_node_path(
    monkeypatch, tmp_path
):
    """If tools.browser_tool_install cannot be imported, the managed-Node rung must
    still find a runnable agent-browser under the Anika Node dir even when
    ...
    monkeypatch.setattr(
        "anika_constants.with_anika_node_path", lambda: {"PATH": str(managed_dir)}
    )
    monkeypatch.setattr(
        "anika_constants.agent_browser_runnable",
