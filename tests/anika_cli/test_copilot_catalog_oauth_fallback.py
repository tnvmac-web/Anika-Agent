...
``anika auth add copilot`` or by ``_seed_from_env`` when the env var
is set in ``~/.anika/.env`` — the picker was silently dropping back to
...
from anika_cli.models import _resolve_copilot_catalog_api_key
