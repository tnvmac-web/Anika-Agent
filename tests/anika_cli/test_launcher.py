"""Tests for the top-level `./anika` launcher script."""
...
def test_launcher_delegates_to_argparse_entrypoint(monkeypatch):
    """`./anika` should use `anika_cli.main`, not the legacy Fire wrapper."""
    launcher_path = Path(__file__).resolve().parents[2] / "anika"
    called = []

    fake_main_module = types.ModuleType("anika_cli.main")

    def fake_main():
        called.append("anika_cli.main")

    fake_main_module.main = fake_main
    monkeypatch.setitem(sys.modules, "anika_cli.main", fake_main_module)
