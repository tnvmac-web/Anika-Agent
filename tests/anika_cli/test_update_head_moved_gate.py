"""Tests for the post-pull HEAD-movement gate in ``anika update``.
...
Before this guard ``anika update`` printed "✓ Code updated!" and
...
def _patch_update_deps(monkeypatch, tmp_path, run_side_effect):
    """Patch the anika_cli.main helpers ``_cmd_update_impl`` touches.

    ``_m()`` in update_cmd.py lazily returns anika_cli.main, so patching
    attributes on that module is the canonical test surface (matches
    tests/anika_cli/test_cmd_update.py).
    """
    ...
    monkeypatch.setattr(
        anika_main, "_run_pre_update_backup", lambda *a, **k: None
    )
    monkeypatch.setattr(
    ...
    monkeypatch.setattr(
        anika_gateway, "find_gateway_pids", lambda all_profiles=False: []
    )
    monkeypatch.setattr(
        anika_gateway, "supports_systemd_services", lambda: False
    )
    monkeypatch.setattr(
...
def test_update_success_when_head_moves(monkeypatch, tmp_path, capsys):
    ...
    anika_main.cmd_update(args)  # completes normally (no SystemExit)
