"""Tests for anika backup and import commands."""
...
def _no_real_gateway_service(monkeypatch):
    ...
    assert the wiring."""
    import anika_cli.gateway as gateway_mod
...
def _advance_backup_clock(seconds: float = 1.1) -> None:
    """Skew anika_cli.backup's datetime forward instead of sleeping.
    ...
    import anika_cli.backup as _backup

    shim = getattr(_backup.datetime, "_anika_test_shim", None)
    if shim is None:
        class _ShimDatetime(_dt.datetime):
            _anika_test_shim = True
...
def _make_anika_tree(root: Path) -> None:
    """Create a realistic ~/.anika directory structure for testing."""
    (root / "config.yaml").write_text("model:\n  provider: openrouter\n")
    (root / ".env").write_text("OPENROUTER_API_KEY=sk-test-123\n")
    for db_name in ("memory_store.db", "anika_state.db"):
    ...
    # anika-agent repo (should be EXCLUDED)
    (root / "anika-agent").mkdir(exist_ok=True)
    (root / "anika-agent" / "run_agent.py").write_text("# big file\n")
    (root / "anika-agent" / ".git").mkdir()
    (root / "anika-agent" / ".git" / "HEAD").write_text("ref: refs/heads/main\n")
...
class TestShouldExclude:
    def test_excludes_anika_agent(self):
        from anika_cli.backup import _should_exclude
        assert _should_exclude(Path("anika-agent/run_agent.py"))
        assert _should_exclude(Path("anika-agent/.git/HEAD"))
    ...
    def test_excludes_state_snapshots_dir(self):
        ...
        from anika_cli.backup import _EXCLUDED_DIRS, _QUICK_SNAPSHOTS_DIR, _should_exclude
    ...
    def test_excludes_desktop_emergency_state_db_baks(self):
        ...
        state.db.pre-update-emergency-*.bak files at the ANIKA_HOME root —
...
class TestIterBackupFiles:
    def test_manual_and_automatic_paths_share_one_walk(self, tmp_path):
        ...
        ``anika-agent`` at ANY depth, silently dropping nested skill dirs
        like ``skills/autonomous-ai-agents/anika-agent/`` that the manual
        path preserved. One shared iterator makes that drift impossible;
        this test pins the contract."""
        from anika_cli.backup import _iter_backup_files

        root = tmp_path / ".anika"
        root.mkdir()
        _make_anika_tree(root)

        # The case the old automatic walk got wrong: a nested dir named
        # anika-agent holding real skill content.
        nested = root / "skills" / "autonomous-ai-agents" / "anika-agent"
        ...
        rel_nested = str(Path("skills/autonomous-ai-agents/anika-agent/SKILL.md"))
        assert rel_nested in selected
        assert str(Path("models/big.gguf")) not in selected
        assert not any(s.startswith("anika-agent") for s in selected)
...
class TestBackup:
    ...
    def test_db_snapshots_staged_beside_output_zip(self, tmp_path, monkeypatch):
        ...
        import anika_cli.backup as backup_mod
        staged_dirs = []
    ...
    def test_pre_update_db_snapshots_staged_beside_output_zip(self, tmp_path, monkeypatch):
        ...
        out_zip = anika_home / "backups" / "pre-update-test.zip"
        ...
        monkeypatch.setattr(backup_mod.tempfile, "NamedTemporaryFile", _spy)
        result = backup_mod._write_full_zip_backup(out_zip, anika_home)
    ...
    def test_skips_symlinked_files(self, tmp_path, monkeypatch):
        """Backup must not dereference symlinks and leak files outside ANIKA_HOME."""
        anika_home = tmp_path / ".anika"
        anika_home.mkdir()
        _make_anika_tree(anika_home)
        outside = tmp_path / "outside-secret.txt"
        outside.write_text("outside secret\n")
        _symlink_file_or_skip(anika_home / "skills" / "outside-link.txt", outside)
        ...
        from anika_cli.backup import run_backup
        run_backup(args)
    ...
    def test_state_snapshots_not_nested_into_backup(self, tmp_path, monkeypatch):
        ...
        anika_home.mkdir()
        _make_anika_tree(anika_home)
        with sqlite3.connect(anika_home / "state.db") as conn:
            conn.execute("CREATE TABLE sessions (id TEXT PRIMARY KEY)")
            conn.execute("INSERT INTO sessions VALUES ('s1')")

        from anika_cli.backup import _QUICK_SNAPSHOTS_DIR, create_quick_snapshot, run_backup

        # Real producer, so the layout under state-snapshots/ is whatever the
        # code actually writes (manifest.json + state.db copy + ...).
        snap_id = create_quick_snapshot(anika_home=anika_home)
        assert snap_id and (anika_home / _QUICK_SNAPSHOTS_DIR / snap_id / "state.db").exists()
...
class TestValidateBackupZip:
    ...
    def test_state_db_passes(self, tmp_path):
        """A zip containing state.db is accepted as a valid Anika backup."""
        from anika_cli.backup import _validate_backup_zip
...
class TestImport:
    ...
    def test_import_auto_installs_gateway_service(self, tmp_path, monkeypatch):
        ...
        from anika_cli.backup import run_import
    ...
    def test_import_survives_service_layer_import_failure(self, tmp_path, monkeypatch, capsys):
        ...
        assert "Done. Your Anika configuration has been restored." in out
        assert "anika gateway install" in out
    ...
    def test_preserves_per_profile_gateway_state(self, tmp_path, monkeypatch):
        ...
        anika_home = tmp_path / ".anika"
        (anika_home / "profiles" / "coder").mkdir(parents=True)
        monkeypatch.setenv("ANIKA_HOME", str(anika_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)

        live_state = '{"gateway_state": "running"}'
        (anika_home / "profiles" / "coder" / "gateway_state.json").write_text(live_state)
        ...
        assert (anika_home / "profiles" / "coder" / "config.yaml").read_text() == "model: anthropic\n"
        assert (
            anika_home / "profiles" / "coder" / "gateway_state.json"
    ...
    def test_preserves_runtime_pid_and_process_files(self, tmp_path, monkeypatch):
        ...
        (anika_home / "gateway.pid").write_text("4242")
        (anika_home / "processes.json").write_text('{"live": true}')
        ...
        assert (anika_home / "gateway.pid").read_text() == "4242"
        assert (anika_home / "processes.json").read_text() == '{"live": true}'
        # cron.pid / gateway.lock had no live copy and were not seeded.
        assert not (anika_home / "cron.pid").exists()
        assert not (anika_home / "gateway.lock").exists()
    ...
    def test_restores_secret_files_with_0600_perms(self, tmp_path, monkeypatch):
        ...
        for rel in (".env", "auth.json", "state.db", "profiles/coder/.env"):
            mode = (anika_home / rel).stat().st_mode & 0o777
...
class TestRoundTrip:
    def test_backup_then_import(self, tmp_path, monkeypatch):
        ...
        # Source
        src_home = tmp_path / "source" / ".anika"
        src_home.mkdir(parents=True)
        _make_anika_tree(src_home)

        monkeypatch.setenv("ANIKA_HOME", str(src_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "source")

        # Backup
        out_zip = tmp_path / "roundtrip.zip"
        from anika_cli.backup import run_backup, run_import

        run_backup(Namespace(output=str(out_zip)))
        assert out_zip.exists()

        # Import into a different location
        dst_home = tmp_path / "dest" / ".anika"
        dst_home.mkdir(parents=True)
        monkeypatch.setenv("ANIKA_HOME", str(dst_home))
        ...
        # anika-agent should NOT be present
        assert not (dst_home / "anika-agent").exists()
...
class TestFormatSize:
    def test_bytes(self):
        from anika_cli.sizefmt import format_bytes as _format_size
...
class TestValidation:
    ...
    def test_detect_prefix_only_dirs(self):
        ...
        import io
        from anika_cli.backup import _detect_prefix

        buf = io.BytesIO()
        with zipfile.ZipFile(buf, "w") as zf:
            # Only directory entries (trailing slash)
            zf.writestr(".anika/", "")
            zf.writestr(".anika/skills/", "")
        buf.seek(0)
        with zipfile.ZipFile(buf, "r") as zf:
...
class TestBackupEdgeCases:
    ...
    def test_empty_anika_home(self, tmp_path, monkeypatch):
        """Backup handles empty anika home (no files to back up)."""
        anika_home = tmp_path / ".anika"
        anika_home.mkdir()
        # Only excluded dirs, no actual files
        (anika_home / "__pycache__").mkdir()
        (anika_home / "__pycache__" / "foo.pyc").write_bytes(b"\x00")
    ...
    def test_pre1980_timestamp_skipped(self, tmp_path, monkeypatch):
        ...
        anika_home = tmp_path / ".anika"
        anika_home.mkdir()
        (anika_home / "config.yaml").write_text("model: test\n")

        # Create a file with epoch timestamp (1970-01-01)
        old_file = anika_home / "ancient.txt"
...
class TestImportEdgeCases:
    ...
    def test_eof_during_confirmation(self, tmp_path, monkeypatch):
        ...
        anika_home = tmp_path / ".anika"
        anika_home.mkdir()
        (anika_home / "config.yaml").write_text("existing\n")
        monkeypatch.setenv("ANIKA_HOME", str(anika_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
    ...
    def test_progress_with_many_files(self, tmp_path, monkeypatch):
        ...
        assert (anika_home / "config.yaml").exists()
        assert (anika_home / "sessions" / "s0599.json").exists()
...
class TestImportAtomicWrites:
    """`anika import` must never leave a user's file truncated.
    ...
    tests pin the invariant for both restore branches: the ANIKA_HOME branch
    ...
    def test_failed_member_leaves_existing_file_intact(self, tmp_path, monkeypatch):
        ...
        (anika_home / "config.yaml").write_text(original)
        monkeypatch.setenv("ANIKA_HOME", str(anika_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        ...
        assert (anika_home / "config.yaml").read_text() == original
        # And the aborted write must not litter the directory it staged in.
        assert list(anika_home.glob(".config.yaml.*")) == []

    def test_failed_external_member_leaves_existing_file_intact(self, tmp_path, monkeypatch):
        """Same invariant on the `_external/` branch, which writes outside ANIKA_HOME."""
        dst_home = tmp_path / "dst"
        dst_home.mkdir()
        anika_home = dst_home / ".anika"
        anika_home.mkdir()
    ...
    def test_restore_preserves_existing_file_owner(self, tmp_path, monkeypatch):
        ...
        ``sudo anika import`` onto a user-owned (or Docker/NAS volume-owned)
        ANIKA_HOME would hand every restored file to root. The uid/gid is
        ...
        monkeypatch.setattr(
            "anika_cli.backup._preserve_file_owner",
    ...
    def test_restore_does_not_carry_setuid_onto_archive_content(
        ...
        re-serialize content this process produced; ``anika import`` is the
...
class TestSafeCopyDb:
    def test_copies_valid_database(self, tmp_path):
        from anika_cli.backup import _safe_copy_db
    ...
    def test_aborts_when_source_remains_busy_past_deadline(
        ...
        from anika_cli import backup as backup_mod
    ...
    def test_is_zeroed_sqlite_file_detects_nul_header(self, tmp_path):
        from anika_cli.backup import is_zeroed_sqlite_file
        p = tmp_path / "state.db"
...
class TestQuickSnapshot:
    @pytest.fixture
    def anika_home(self, tmp_path):
        """Create a fake ANIKA_HOME with critical state files."""
        home = tmp_path / ".anika"
        home.mkdir()
    ...
    def test_state_db_safely_copied(self, anika_home):
        from anika_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(anika_home=anika_home)
        db_copy = anika_home / "state-snapshots" / snap_id / "state.db"
    ...
    def test_failed_state_db_copy_is_loud(self, anika_home, monkeypatch, capsys):
    ...
    def test_restore_refused_db_is_not_counted(self, anika_home, monkeypatch):
        ...
        restore_quick_snapshot(snap_id, anika_home=anika_home)

        manifest = json.loads(
            (backup_mod._quick_snapshot_root(anika_home) / snap_id / "manifest.json").read_text()
        )
    ...
    def test_restore_state_db_live_connection(self, anika_home):
        ...
        live_conn = sqlite3.connect(str(anika_home / "state.db"))
        ...
        result = restore_quick_snapshot(snap_id, anika_home=anika_home)
        assert result is True
    ...
    def test_snapshot_includes_pairing_directories(self, anika_home):
        ...
        (anika_home / "platforms" / "pairing").mkdir(parents=True)
        (anika_home / "platforms" / "pairing" / "telegram-approved.json").write_text(
            '{"12345": {"user_name": "alice"}}'
        )
        (anika_home / "platforms" / "pairing" / "discord-approved.json").write_text(
            '{"67890": {"user_name": "bob"}}'
        )
        # Legacy pairing store (old location)
        (anika_home / "pairing").mkdir()
        (anika_home / "pairing" / "matrix-approved.json").write_text(
            '{"@charlie:server": {"user_name": "charlie"}}'
        )
        # Feishu's separate JSON
        (anika_home / "feishu_comment_pairing.json").write_text(
        ...
        snap_dir = anika_home / "state-snapshots" / snap_id
...
# ---------------------------------------------------------------------------
# Pre-update backup (anika update safety net)
# ---------------------------------------------------------------------------
    ...
    def test_oversized_db_suppresses_pruning(self, anika_home, capsys):
        ...
        """
        import json
        from anika_cli.backup import create_quick_snapshot, list_quick_snapshots

        # First snapshot: complete (state.db is small, under any cap)
        first_id = create_quick_snapshot(label="complete", anika_home=anika_home)
        assert first_id is not None
        first_dir = anika_home / "state-snapshots" / first_id
        ...
        second_id = create_quick_snapshot(
            label="oversized", anika_home=anika_home, max_file_size=1024, keep=1
        )
        assert second_id is not None
        second_dir = anika_home / "state-snapshots" / second_id
        ...
        all_snaps = list_quick_snapshots(limit=100, anika_home=anika_home)
...
class TestQuickSnapshotProjectsKanban:
    ...
    def test_non_default_kanban_board_snapshotted(self, anika_home):
        ...
        board_dir = anika_home / "kanban" / "boards" / "work"
        ...
        snap_id = create_quick_snapshot(anika_home=anika_home)
        copy = (
            anika_home / "state-snapshots" / snap_id
        ...
        assert restore_quick_snapshot(snap_id, anika_home=anika_home) is True
        conn = sqlite3.connect(str(board_dir / "kanban.db"))
    ...
    def test_board_db_copied_wal_safely(self, anika_home, monkeypatch):
        ...
        import anika_cli.backup as bk
        from anika_cli.backup import create_quick_snapshot

        board = anika_home / "kanban" / "boards" / "work"
        ...
        copy = anika_home / "state-snapshots" / snap_id / "kanban" / "boards" / "work" / "kanban.db"
        rows = sqlite3.connect(str(copy)).execute("SELECT * FROM tasks").fetchall()
        assert rows == [("w1", "ship")]


class TestPreUpdateBackup:
    """Tests for create_pre_update_backup — the auto-backup ``anika update``
    ...
    def test_backup_contents_match_full_backup(self, anika_home):
        """Pre-update backup should include the same user data that
        ``anika backup`` would, and should exclude the same directories."""
        from anika_cli.backup import create_pre_update_backup
        out = create_pre_update_backup(anika_home=anika_home)
        ...
        # anika-agent repo excluded
        assert not any(n.startswith("anika-agent/") for n in names)
        # __pycache__ excluded
        assert not any("__pycache__" in n for n in names)
        # pid files excluded
        assert "gateway.pid" not in names

    def test_pre_update_zip_does_not_nest_the_pre_update_snapshot(self, anika_home):
        """``anika update`` in ``full`` mode takes the quick snapshot *before*
        ...
        snap_id = create_quick_snapshot(label="pre-update", anika_home=anika_home)
        assert snap_id and (anika_home / _QUICK_SNAPSHOTS_DIR / snap_id / "state.db").exists()
    ...
    def test_rotation_keeps_only_n(self, anika_home):
        ...
        created = []
        for _ in range(5):
            out = create_pre_update_backup(anika_home=anika_home, keep=3)
            created.append(out)
            _advance_backup_clock()

        remaining = sorted(
            p.name for p in (anika_home / "backups").iterdir()
    ...
    def test_skips_symlinked_files(self, anika_home, tmp_path):
        """Pre-update backups must not dereference symlinks outside ANIKA_HOME."""
        from anika_cli.backup import create_pre_update_backup
...
class TestRunPreUpdateBackup:
    ...
    @pytest.fixture
    def anika_home(self, tmp_path, monkeypatch):
        root = tmp_path / ".anika"
        root.mkdir()
        _make_anika_tree(root)
        # Point ANIKA_HOME at the temp dir so config + backup paths resolve here
        monkeypatch.setenv("ANIKA_HOME", str(root))
        # Make Path.home() point at tmp_path for anything that uses it
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        # Config reads resolve ANIKA_HOME dynamically and their caches are
    ...
    @staticmethod
    def _set_mode(anika_home, value):
        import yaml
        (anika_home / "config.yaml").write_text(yaml.safe_dump({
            "_config_version": 22,
            "updates": {"pre_update_backup": value},
        }))

    @staticmethod
    def _zips(anika_home):
        d = anika_home / "backups"
        return list(d.glob("pre-update-*.zip")) if d.exists() else []

    @staticmethod
    def _snaps(anika_home):
        d = anika_home / "state-snapshots"
        return [p for p in d.iterdir() if p.is_dir()] if d.exists() else []




    def test_config_off_disables_everything_silently(self, anika_home, capsys):
        ...
        self._set_mode(anika_home, "off")
        from anika_cli.update_cmd import _run_pre_update_backup
        snap_id = _run_pre_update_backup(Namespace(no_backup=False, backup=False))
        out = capsys.readouterr().out
        assert snap_id is None
        assert out == ""
        assert not self._snaps(anika_home)
        assert not self._zips(anika_home)



    def test_config_full_mode(self, anika_home, capsys):
        self._set_mode(anika_home, "full")
        from anika_cli.update_cmd import _run_pre_update_backup
        snap_id = _run_pre_update_backup(Namespace(no_backup=False, backup=False))
        out = capsys.readouterr().out
        assert snap_id is not None
        ...
        assert len(self._zips(anika_home)) == 1




# ---------------------------------------------------------------------------
# Pre-migration backup (anika claw migrate safety net)
# ---------------------------------------------------------------------------

class TestPreMigrationBackup:
    """Tests for create_pre_migration_backup — the auto-backup
    ``anika claw migrate`` runs before mutating ~/.anika/."""
    ...
    def test_restorable_with_anika_import(self, anika_home, tmp_path):
        """The zip produced by pre-migration backup must be a valid Anika
        backup — `anika import` should accept it."""
        from anika_cli.backup import create_pre_migration_backup, _validate_backup_zip
        out = create_pre_migration_backup(anika_home=anika_home)
        assert out is not None
        with zipfile.ZipFile(out) as zf:
    ...
    def test_does_not_touch_pre_update_backups(self, anika_home):
        """Pre-migration rotation must only prune pre-migration-*.zip files,
        leaving pre-update-*.zip backups untouched."""
        from anika_cli.backup import create_pre_update_backup, create_pre_migration_backup
        update_backup = create_pre_update_backup(anika_home=anika_home, keep=5)
        assert update_backup is not None and update_backup.exists()
        # Spin up a lot of migration backups with keep=1
        for _ in range(3):
            out = create_pre_migration_backup(anika_home=anika_home, keep=1)
            assert out is not None
            _advance_backup_clock()
...
class TestRestoreCronJobsIfEmptied:
    """`anika update` config migration can leave cron/jobs.json valid-but-empty,
    ...
    def _make_snapshot(self, anika_home: Path, label="pre-update"):
        from anika_cli.backup import create_quick_snapshot
        return create_quick_snapshot(label=label, anika_home=anika_home, keep=5)

    def test_restores_when_emptied_after_migration(self, tmp_path):
        from anika_cli.backup import restore_cron_jobs_if_emptied
        anika_home = tmp_path / ".anika"
        jobs_path = anika_home / "cron" / "jobs.json"
        # Pre-update: 3 real jobs.
        self._seed_jobs(jobs_path, [{"id": "a"}, {"id": "b"}, {"id": "c"}])
        snap_id = self._make_snapshot(anika_home)
        assert snap_id

        # Migration silently empties the file (valid JSON, zero jobs).
        jobs_path.write_text(json.dumps({"jobs": []}))

        result = restore_cron_jobs_if_emptied(snap_id, anika_home=anika_home)
        assert result is not None
...
class TestRestoreConfigModelSettingsIfRewritten:
    ...
    def _seed(self, anika_home: Path) -> Path:
        anika_home.mkdir(parents=True, exist_ok=True)
        cfg = anika_home / "config.yaml"
        cfg.write_text(self.USER_CONFIG, encoding="utf-8")
        return cfg

    def test_restores_rewritten_provider_and_dropped_moa(self, tmp_path):
        import yaml
        from anika_cli.backup import restore_config_model_settings_if_rewritten

        anika_home = tmp_path / ".anika"
        cfg = self._seed(anika_home)
        snap_id = self._make_snapshot(anika_home)
        assert snap_id
...
# location, NOT under ANIKA_HOME. (backup/import cycle data-loss fix)
# ---------------------------------------------------------------------------

class TestMemoryProviderExternalPaths:
    def _make_min_tree(self, anika_home: Path) -> None:
        anika_home.mkdir(parents=True, exist_ok=True)
        (anika_home / "config.yaml").write_text("model:\n  provider: openrouter\n")
        (anika_home / ".env").write_text("OPENROUTER_API_KEY=sk-test\n")
        (anika_home / "state.db").write_bytes(b"x")
    ...
    def test_backup_skips_external_paths_outside_home(self, tmp_path, monkeypatch):
        ...
        anika_home = tmp_path / ".anika"
        self._make_min_tree(anika_home)
    ...
    def test_import_restores_external_to_home_relative_location(self, tmp_path, monkeypatch):
        """_external/ members restore to ~/<relpath>, not under ANIKA_HOME,
        ...
        # External state did NOT leak into ANIKA_HOME.
        assert not (anika_home / "_external").exists()


# ---------------------------------------------------------------------------
# run_import: ANIKA_HOME override handling (issue #99839)
# ---------------------------------------------------------------------------


class TestImportHonorsAnikaHomeOverride:
    """`anika import` must restore into the home the command runs under.

    Resolving the target through get_default_anika_root() maps a profile
    ...
    def test_import_targets_named_profile_home(self, tmp_path, monkeypatch):
        """ANIKA_HOME=<root>/profiles/<name> must restore INTO the profile,
        not into <root> (which would clobber the live root config)."""
        root = tmp_path / "anika-root"
        profile = root / "profiles" / "coder"
        profile.mkdir(parents=True)
        # Live root config that must survive untouched.
        (root / "config.yaml").write_text("model:\n  provider: openai\n")

        monkeypatch.setenv("ANIKA_HOME", str(profile))
        from anika_constants import get_anika_home

        assert get_anika_home() == profile
    ...
    def test_import_skips_gateway_install_for_non_default_home(
        ...
        monkeypatch.setenv("ANIKA_HOME", str(sandbox))
        ...
        monkeypatch.setattr(
            backup_mod,
            "_get_platform_default_anika_home",
            lambda: native_default,
        )

        calls = []
        monkeypatch.setattr(
            "anika_cli.gateway.ensure_gateway_service",
            lambda *a, **kw: calls.append(kw),
        )
        monkeypatch.setattr(
            "anika_cli.gateway._is_service_running",
            lambda: False,
        )
    ...
    def test_import_installs_gateway_when_default_home_is_target(
        ...
        native_default = tmp_path / "native-default"
        native_default.mkdir()
        monkeypatch.setenv("ANIKA_HOME", str(native_default))
...
def _write_session_db(path: Path, sessions: int, messages_per_session: int) -> None:
    """Create a minimal Anika-shaped session database at *path*."""
...
class TestImportLiveSessionDatabase:
    """`anika import` must not swap the inode of a database Anika holds open.
