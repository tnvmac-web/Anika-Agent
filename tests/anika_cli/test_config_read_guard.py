...
Canonical owners:
  ...
  * ``anika_cli/config.py`` — ``load_config()`` / ``load_config_readonly()``
...
ALLOWLIST = {
    ...
    # gateway.run._anika_home (delegates to read_raw_config otherwise).
    "gateway/run.py",
    # Reads the MANAGED-scope config.yaml (/etc/anika/...), not the user's —
    # it IS the overlay source; the canonical loaders call into it.
    "anika_cli/managed_scope.py",
