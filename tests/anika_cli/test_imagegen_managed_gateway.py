...
the user's personal FAL_KEY instead of the __NEW_ORG__ Tool Gateway (real incident:
...
the managed "__NEW_ORG__ Subscription" row, ``image_gen.provider: fal`` for the
...
def _quiet(monkeypatch):
    import anika_cli.tools_config_providers as tc
...
def test_reconfigure_managed_fal_row_keeps_managed_selection(monkeypatch):
    ...
    branch wrote the managed one — a __NEW_ORG__ Subscription user re-entering the
