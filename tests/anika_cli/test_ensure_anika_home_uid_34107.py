"""Regression tests for #34107 — Docker UID/GID handling in ensure_anika_home.

When Anika runs in Docker with ``ANIKA_UID=1000`` / ``ANIKA_GID=911``,
the entrypoint chowns the top-level ``ANIKA_HOME`` once at startup. But
subdirectories created at runtime by ``ensure_anika_home()`` — especially
...
The fix is a ``_chown_to_anika_uid`` helper that reads the env vars and
...
# ---------------------------------------------------------------------------
# _resolve_anika_uid_gid
# ---------------------------------------------------------------------------


class TestResolveAnikaUidGid:
    def test_returns_parsed_values_when_both_set(self, monkeypatch):
        monkeypatch.setenv("ANIKA_UID", "1000")
        monkeypatch.setenv("ANIKA_GID", "911")
        from anika_cli.config import _resolve_anika_uid_gid
...
class TestChownToAnikaUid:
    ...
    def test_attributeerror_swallowed_for_windows_compat(self, tmp_path, monkeypatch):
        ...
        with patch.object(cfg.os, "chown", side_effect=AttributeError("no chown on this platform")):
            cfg._chown_to_anika_uid(d)  # must not raise
...
class TestSecureDirChown:
    ...
    def test_secure_dir_no_chown_when_env_unset(self, tmp_path, monkeypatch):
        monkeypatch.delenv("ANIKA_UID", raising=False)
        monkeypatch.delenv("ANIKA_GID", raising=False)
        from anika_cli import config as cfg
