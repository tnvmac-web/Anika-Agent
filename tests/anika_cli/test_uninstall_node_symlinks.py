"""Tests for anika_cli.uninstall.remove_node_symlinks.

Regression for #34536: the POSIX installer drops node/npm/npx symlinks in
~/.local/bin pointing into $ANIKA_HOME/node and prepends ~/.local/bin to
PATH, shadowing an existing nvm. Uninstall must remove those symlinks, but
only when they still resolve into the Anika-managed node dir.
"""
...
def _make_anika_node(anika_home: Path) -> Path:
    """Create a fake $ANIKA_HOME/node/bin/{node,npm,npx} tree."""
    node_bin = anika_home / "node" / "bin"
...
def test_leaves_unrelated_symlinks_untouched(fake_home):
    ...
    anika_home = fake_home / ".anika"
    _make_anika_node(anika_home)
    ...
    removed = uninstall.remove_node_symlinks(anika_home)
...
def test_removes_fhs_symlinks_in_usr_local_bin(fake_home, tmp_path, monkeypatch):
    ...
    """
    anika_home = fake_home / ".anika"
    node_bin = _make_anika_node(anika_home)
