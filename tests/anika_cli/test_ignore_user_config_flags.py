"""Tests for --ignore-user-config and --ignore-rules flags on `anika chat`.
...
files. In Anika the equivalent isolation is:

* ``--ignore-user-config`` → skip ``~/.anika/config.yaml`` in ``load_cli_config()``
...
argparse → cmd_chat → cli.main() → AnikaCLI → AIAgent call chain.
"""
...
def _clean_env(monkeypatch):
    ...
    """
    for var in ("ANIKA_IGNORE_USER_CONFIG", "ANIKA_IGNORE_RULES"):
...
class TestIgnoreUserConfigEnvGate:
    """``load_cli_config()`` must honour ``ANIKA_IGNORE_USER_CONFIG=1``.

    When the env var is set, user config at ``<anika_home>/config.yaml`` is
    ...
    def _write_user_config(self, tmp_path, model_default):
        ...
        # config. With ANIKA_IGNORE_USER_CONFIG=1, load_cli_config() falls
    ...
    def _reload_cli(self, monkeypatch, tmp_path):
        """Point cli._anika_home at tmp_path and return a fresh load_cli_config."""
        import cli
    ...
    def test_flag_ignored_when_set_to_other_value(self, tmp_path, monkeypatch):
        ...
        self._write_user_config(tmp_path, "test-vendor/ignore-user-config-sentinel")
        monkeypatch.setenv("ANIKA_IGNORE_USER_CONFIG", "true")  # not "1"
...
class TestIgnoreRulesEnvGate:
    """The constructor / env var must propagate to ``AnikaCLI.ignore_rules``
    so ``AIAgent`` is built with ``skip_context_files=True`` and
    ``skip_memory=True``.
    """

    def test_env_var_enables_ignore_rules(self, monkeypatch):
        """Setting ANIKA_IGNORE_RULES=1 flips AnikaCLI.ignore_rules True."""
        monkeypatch.setenv("ANIKA_IGNORE_RULES", "1")

        # Import AnikaCLI lazily — cli.py has heavy module-init side effects
        # that we don't want to run at test collection time.
        import cli
        importlib.reload(cli)

        # Build only enough of AnikaCLI to reach the ignore_rules assignment.
        ...
        obj = object.__new__(cli.AnikaCLI)
        # Replicate the exact logic from cli.py AnikaCLI.__init__:
        ignore_rules = False  # constructor default
        obj.ignore_rules = ignore_rules or os.environ.get("ANIKA_IGNORE_RULES") == "1"

        assert obj.ignore_rules is True


class TestCmdChatWiring:
    """The wiring inside ``cmd_chat()`` in ``anika_cli/main.py`` must set
    ...
    def test_both_flags_set_both_env_vars(self, monkeypatch):
        monkeypatch.delenv("ANIKA_IGNORE_USER_CONFIG", raising=False)
        monkeypatch.delenv("ANIKA_IGNORE_RULES", raising=False)
        ...
        assert os.environ.get("ANIKA_IGNORE_USER_CONFIG") == "1"
        assert os.environ.get("ANIKA_IGNORE_RULES") == "1"
    ...
    def test_flags_absent_sets_nothing(self, monkeypatch):
        ...
        assert "ANIKA_IGNORE_USER_CONFIG" not in os.environ
        assert "ANIKA_IGNORE_RULES" not in os.environ
