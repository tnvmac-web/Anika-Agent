...
2. Spawn: _default_spawn sets the ANIKA_KANBAN_GOAL_MODE env vars only
...
# ---------------------------------------------------------------------------
# CLI judge gate tests (anika kanban complete bypass fix)
# ---------------------------------------------------------------------------

class TestCLIJudgeGate:
    """anika kanban complete must apply the same goal_mode judge gate as the
    ...
    def _run(self, monkeypatch, *, goal_mode=True, judge_available=True,
        ...
        from anika_cli.kanban import _cmd_complete
        ...
        monkeypatch.setattr("anika_cli.kanban.kb.get_task", lambda conn, tid: fake_task)
        monkeypatch.setattr("anika_cli.kanban.kb.complete_task", fake_complete_task)
        monkeypatch.setattr("anika_cli.kanban.kbc.connect_closing", fake_connect_closing)
        monkeypatch.setattr("anika_cli.kanban._worker_run_id_for", lambda _: None)
