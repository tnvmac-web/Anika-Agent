"""``anika`` must survive git operations on the checkout (launcher layout).

The Windows ``anika`` command is a launcher derived from the venv console
script. Its canonical home is the managed binary dir ``ANIKA_HOME\\bin`` —
OUTSIDE the git checkout — because the earlier in-checkout home
(``anika-agent\\bin``) was swept by ``anika update``'s autostash
(``git stash push --include-untracked``) and, with the desktop updater's
``--keep-stash``, never restored: ``anika`` stopped resolving in every new
...
existing install's PATH to the canonical layout from the ``anika update``
tail. Platform verdict, PATH values, and registry I/O are injected
parameters (same pattern as ``anika_constants.venv_bin_dir``), so these
...
from anika_cli._install_repair import (
...
def _make_managed(tmp_path, monkeypatch, *, relocatable: bool = False):
    """Fake managed layout: ANIKA_HOME/anika-agent/venv/Scripts + launchers."""
    home = tmp_path / "anika"
    root = home / "anika-agent"
...
def test_source_checkout_untouched(tmp_path, monkeypatch):
    """A checkout NOT under ANIKA_HOME gains nothing anywhere."""
    home = tmp_path / "anika-home"
    home.mkdir()
    monkeypatch.setenv("ANIKA_HOME", str(home))
    root = tmp_path / "src" / "anika-agent"
    scripts = root / "venv" / "Scripts"
    scripts.mkdir(parents=True)
    for name in _WINDOWS_BIN_LAUNCHERS:
...
def test_profile_session_still_heals_the_shared_bin(tmp_path, monkeypatch):
    """Under ``anika -p <name>`` ANIKA_HOME points inside profiles/<name>;
    ...
    monkeypatch.setenv("ANIKA_HOME", str(home / "profiles" / "work"))
...
# ---------------------------------------------------------------------------
# migrate_windows_bin_path — the `anika update` tail migration
# ---------------------------------------------------------------------------
...
def test_migration_moves_path_to_home_bin_and_strips_legacy(managed_install):
    ...
    (root / "bin").mkdir()
    (root / "bin" / "anika.exe").write_bytes(b"legacy copy")
    ...
    assert (root / "bin" / "anika.exe").read_bytes() == b"legacy copy"
...
def test_normalize_windows_path_equivalences():
    assert (
        _normalize_windows_path(r"C:\Users\Me\AppData\Local\anika\bin")
        == _normalize_windows_path("c:/users/me/appdata/local/ANIKA/BIN/")
    )
...
def test_repo_gitignores_the_legacy_bin_dir():
    ...
    launchers at ``<checkout>/bin``. ``anika update`` autostashes with
    ...
    result = subprocess.run(
        ["git", "-C", str(repo_root), "check-ignore", "-q", "bin/anika.exe"],
        capture_output=True,
    )
    assert result.returncode == 0, (
        "bin/anika.exe is not gitignored — anika update's autostash "
