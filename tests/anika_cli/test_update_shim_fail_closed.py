...
The bug class: on Windows, when `anika.exe`/sibling shims could not be
...
def _make_shims(scripts_dir: Path, names=("anika", "anika-gateway")) -> list[Path]:
...
def test_strict_quarantine_refuses_before_install(windows, tmp_path, monkeypatch):
    ...
    # anika.exe cannot be renamed; anika-gateway.exe can
    real_rename = Path.rename

    def deny_anika(self, target):
        if self.name == "anika.exe":
            raise PermissionError(13, "held open")
        return real_rename(self, target)

    monkeypatch.setattr(Path, "rename", deny_anika)
    monkeypatch.setattr(main_install_repair, "_anika_exe_shims", lambda d: shims)
    ...
    assert install_ran == []
    assert "anika.exe" in exc_info.value.failed_shims
    # The successful rename (anika-gateway.exe) was rolled back.
    assert (scripts / "anika-gateway.exe").exists()
    assert not list(scripts.glob("anika-gateway.exe.old.*"))


def test_non_strict_keeps_warn_and_try(windows, tmp_path, monkeypatch):
    scripts = tmp_path / "venv" / "Scripts"
    shims = _make_shims(scripts, names=("anika",))
    monkeypatch.setattr(
        Path, "rename",
...
def test_recovery_install_cmd_fail_closed(windows, tmp_path, monkeypatch):
    ...
    _make_shims(scripts, names=("anika",))
...
def test_refusal_writes_marker_and_exits_2(monkeypatch, capsys):
    ...
    exc = main_install_repair.ShimQuarantineError(["anika.exe"])
    with pytest.raises(SystemExit) as exit_info:
        update_cmd._refuse_update_for_contended_shims(exc)
    assert exit_info.value.code == 2
    assert wrote == [1]
    out = capsys.readouterr().out
    assert "anika.exe" in out
