...
def test_presets_stale_when_a_staged_model_has_no_section(anika_home):
    from anika_cli.local_runtime.bootstrap import _presets_stale

    _stage(anika_home, "model-a")
    _stage(anika_home, "model-b")
    _write_presets(anika_home, "model-a")
    assert _presets_stale() is True


def test_presets_current_when_every_staged_model_is_covered(anika_home):
    from anika_cli.local_runtime.bootstrap import _presets_stale
...
def test_no_models_is_never_stale(anika_home):
    from anika_cli.local_runtime.bootstrap import _presets_stale
...
def test_boot_replaces_incumbent_with_stale_presets(anika_home, monkeypatch):
    ...
    import anika_cli.local_runtime.bootstrap as boot
    ...
    stopped = {}
    monkeypatch.setattr(
        "anika_cli.local_runtime.endpoint._state_endpoint",
    ...
    monkeypatch.setattr(
        "anika_cli.local_runtime.binaries.ensure_runtime_installed", fake_boot)
...
def test_refresh_bounces_an_adopted_server(anika_home, monkeypatch):
...
def test_refresh_no_server_anywhere_is_a_noop(anika_home, monkeypatch):
    import anika_cli.local_runtime.bootstrap as boot

    monkeypatch.setattr(boot, "_SUPERVISOR", None)
    monkeypatch.setattr(
        "anika_cli.local_runtime.endpoint._state_endpoint", lambda: None)
