"""Tests for anika_cli.web_server and related config utilities."""
...
import anika_cli.config as _cfg_mod
import anika_cli.web_routers.chat_ws as _rt_chat_ws
import anika_cli.web_routers.status as _rt_status
import anika_cli.web_server_chat as _web_server_chat
import anika_cli.web_server_config as _web_server_config
import anika_cli.web_server_dashboard as _web_server_dashboard
import anika_cli.web_server_files as _web_server_files
import anika_cli.web_server_gateway as _web_server_gateway
import anika_cli.web_server_lifecycle as _web_server_lifecycle
import anika_cli.web_server_memory as _web_server_memory
import anika_cli.web_server_messaging as _web_server_messaging
import anika_cli.web_server_sessions as _web_server_sessions
...
@pytest.fixture
def _install_example_plugin(_isolate_anika_home):
    """Drop the example-dashboard fixture into the per-test ANIKA_HOME
    ...
    ``ANIKA_BUNDLED_PLUGINS`` override because the bundled dir is
    resolved per-call (other tests in the suite implicitly rely on the
    real bundled plugins — kanban, anika-achievements, model providers
    ...
    from anika_cli import web_server

    user_plugins_dir = get_anika_home() / "plugins"
    ...
    # plugin in exactly as a real operator would with `anika plugins
    # enable example`.
    from anika_cli.config import load_config, save_config
    _cfg = load_config()
    ...
    #      and start serving requests against a torn-down ANIKA_HOME.
    app = web_server.app
...
class TestReloadEnv:
    ...
    def test_removes_deleted_known_vars(self, tmp_path):
        """reload_env() removes known Anika vars not present in .env."""
        env_file = tmp_path / ".env"
...
class TestSessionTokenInjection:
    """The desktop shell mints ANIKA_DASHBOARD_SESSION_TOKEN and signs its
    ...
    def test_honors_injected_token(self, monkeypatch):
        import anika_cli.web_server as ws

        original_app = ws.app
        original_token = ws._SESSION_TOKEN
        monkeypatch.setenv("ANIKA_DASHBOARD_SESSION_TOKEN", "desktop-seeded-token")
    ...
    def test_falls_back_to_random_token(self, monkeypatch):
        ...
        monkeypatch.delenv("ANIKA_DASHBOARD_SESSION_TOKEN", raising=False)
        with patch.object(
...
class TestWebServerEndpoints:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_anika_home):
        """Create a TestClient and isolate the state DB under the test ANIKA_HOME."""
        try:
            from starlette.testclient import TestClient
        except ImportError:
        ...
        import anika_state
        from anika_constants import get_anika_home
        from anika_cli.web_server import app, _SESSION_HEADER_NAME, _SESSION_TOKEN

        monkeypatch.setattr(anika_state, "DEFAULT_DB_PATH", get_anika_home() / "state.db")
    ...
    def test_get_sessions_poll_preserves_pending_wal(self):
        ...
        db_path = get_anika_home() / "state.db"
    ...
    def test_get_status_loads_gateway_config_off_event_loop(self, monkeypatch):
        ...
        import anika_cli.web_server as web_server
    ...
    def test_profiles_sidebar_heals_stale_schema_store(self):
        ...
        array — the desktop rendered "No sessions yet" after `anika update`
    ...
    def test_startup_eager_reconcile_heals_stale_store(self):
        ...
        #79531/#80037: after `anika update` an old-schema state.db used to
    ...
    def test_startup_eager_reconcile_never_raises(self, monkeypatch):
        ...
        monkeypatch.setattr(anika_state, "SessionDB", boom)
    ...
    def test_heal_gives_up_when_reconcile_cannot_fix_the_store(self, monkeypatch):
        ...
        original_init = anika_state.SessionDB.__init__
        ...
        # the class on anika_state covers every open the helper makes.
        monkeypatch.setattr(anika_state.SessionDB, "__init__", counting_init)
    ...
    def test_generic_corruption_does_not_trigger_writable_heal(
        ...
        monkeypatch.setattr(anika_state, "SessionDB", corrupt_open)
    ...
    def test_decode_error_triggers_writable_heal(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(anika_state, "SessionDB", scripted_open)
    ...
    def test_messaging_platforms_profile_scopes_gateway_reads(self, monkeypatch):
        ...
        ANIKA_HOME contextvar override (#56986), so /api/messaging/platforms
        ...
        """
        import anika_cli.web_server as web_server
        from anika_cli import profiles as profiles_mod
    ...
    def test_declared_surface_put_writes_config_and_secret(self):
        from anika_constants import get_anika_home
        from anika_cli.config import load_env
        ...
        config_path = get_anika_home() / "hindsight" / "config.json"
    ...
    def test_put_memory_provider_config_writes_config_and_secret(self):
        from anika_constants import get_anika_home
        from anika_cli.config import load_config, load_env
    ...
    def _isolate_honcho_config(self):
        # Honcho tests write the suite-wide ANIKA_HOME honcho.json; snapshot and
        # restore it so provider status/config state never leaks across tests.
        from anika_constants import get_anika_home

        path = get_anika_home() / "honcho.json"
    ...
    def test_put_honcho_writes_host_block_root_and_secret(self, monkeypatch, tmp_path):
        ...
        cfg = json.loads((get_anika_home() / "honcho.json").read_text(encoding="utf-8"))
        # baseUrl is root-scoped; the rest live in the active host block.
        assert cfg["baseUrl"] == "https://honcho.example.dev"
        assert cfg["hosts"]["anika"]["workspace"] == "myws"
        assert cfg["hosts"]["anika"]["peerName"] == "eri"
        assert cfg["hosts"]["anika"]["environment"] == "local"
        assert cfg["hosts"]["anika"]["sessionStrategy"] == "per-repo"
        # The key lands where the client reads first; GET keeps it write-only.
        assert cfg["hosts"]["anika"]["apiKey"] == "hch-test-key"
    ...
    def test_get_media_requires_auth(self):
        from anika_cli.web_server import _SESSION_HEADER_NAME
    ...
    def test_update_anika_returns_docker_guidance_without_spawning(self, monkeypatch):
        import anika_cli.web_server as web_server
        ...
        def fail_spawn(*_args, **_kwargs):
            nonlocal spawned
            spawned = True
            raise AssertionError("docker update guard should not spawn anika update")
        ...
        # method through anika_cli.config directly.
        monkeypatch.setattr(
            "anika_cli.config.detect_install_method", lambda *_a, **_k: "docker"
        )
        monkeypatch.setattr(_cfg_mod, "detect_install_method", lambda _root: "docker")
        monkeypatch.setattr(_web_server_gateway, "_spawn_anika_action", fail_spawn)
        _web_server_gateway._ACTION_PROCS.pop("anika-update", None)
        _web_server_gateway._ACTION_RESULTS.pop("anika-update", None)

        resp = self.client.post("/api/anika/update")

        assert resp.status_code == 200
        data = resp.json()
        assert data["ok"] is False
        assert data["name"] == "anika-update"
        assert data["pid"] is None
        assert data["error"] == "docker_update_unsupported"
        assert "docker pull __NEW_ORG__/anika-agent:latest" in data["message"]
        assert spawned is False

        status = self.client.get("/api/actions/anika-update/status")
        assert status.status_code == 200
        status_data = status.json()
        assert status_data["running"] is False
        assert status_data["exit_code"] == 1
        assert status_data["pid"] is None
        assert any("docker pull __NEW_ORG__/anika-agent:latest" in line for line in status_data["lines"])

    def test_update_anika_returns_apt_guidance_without_spawning(self, monkeypatch):
        import anika_cli.web_server as web_server
        ...
        def fail_spawn(*_args, **_kwargs):
            nonlocal spawned
            spawned = True
            raise AssertionError("APT-managed update guard should not spawn anika update")

        monkeypatch.setattr(_web_server_files, "_dashboard_local_update_managed_externally", lambda: False)
        # The shared admission gate (#91277 Phase 3) resolves the install
        # method through anika_cli.config directly, so patch it there (the
        # web_server module alias only feeds the /update/check endpoint).
        monkeypatch.setattr(
            "anika_cli.config.detect_install_method", lambda *_a, **_k: "apt"
        )
        ...
        assert data["update_command"] == "pkg upgrade anika-agent"
        assert spawned is False

        check = self.client.get("/api/anika/update/check")
        assert check.status_code == 200
        check_data = check.json()
        assert check_data["install_method"] == "apt"
        assert check_data["can_apply"] is False
        assert check_data["update_command"] == "pkg upgrade anika-agent"
    ...
    def test_update_status_recovers_completed_result_after_dashboard_restart(self, monkeypatch, tmp_path):
        ...
        action_id = "c" * 32
        (tmp_path / "anika-update.log").write_text(
            "=== anika-update started 2026-08-17 11:19:34 ===\n"
            "pulling updates...\n",
            encoding="utf-8",
        )
        (tmp_path / "update.log").write_text(
            "=== anika update started 2026-08-17T11:19:35 ===\n"
            "✓ Update complete!\n"
            f"=== anika-update completed {action_id} ===\n",
            encoding="utf-8",
        )
        ...
        status = self.client.get("/api/actions/anika-update/status?lines=2000")
        ...
        assert f"=== anika-update completed {action_id} ===" in data["lines"]

    def test_update_anika_spawns_with_action_id(self, monkeypatch):
        import anika_cli.web_server as web_server
        ...
        monkeypatch.setattr(_web_server_gateway, "_spawn_anika_action", fake_spawn)
        _web_server_gateway._ACTION_PROCS.pop("anika-update", None)
        _web_server_gateway._ACTION_RESULTS.pop("anika-update", None)
        ...
        assert calls == [
            (["update"], "anika-update", {"ANIKA_ACTION_ID": "a" * 32})
        ]

    def test_update_anika_reuses_running_action(self, monkeypatch):
        import anika_cli.web_server as web_server
        ...
        _web_server_gateway._ACTION_PROCS["anika-update"] = Proc()
        _web_server_gateway._ACTION_IDS["anika-update"] = "b" * 32

        try:
            resp = self.client.post("/api/anika/update")
        finally:
            _web_server_gateway._ACTION_PROCS.pop("anika-update", None)
            _web_server_gateway._ACTION_IDS.pop("anika-update", None)
    ...
    def test_platform_scoped_messaging_env_vars_are_channel_managed(self):
        ...
        tests/anika_cli/test_setup_hidden_env.py.
        """
        from anika_cli.web_server_messaging import _MESSAGING_KEYS_PAGE_KEYS, _build_catalog_entry, _channel_managed_env_keys
    ...
    def test_model_set_maps_unknown_vendor_to_aggregator(self, monkeypatch):
        ...
        a Anika provider — keep the user's aggregator instead of writing a
        provider that can never resolve credentials."""
        monkeypatch.setattr(
            "anika_cli.model_cost_guard.expensive_model_warning",
    ...
    def test_reveal_env_var(self, tmp_path):
        ...
        from anika_cli.config import save_env_value
        from anika_cli.web_server import _SESSION_HEADER_NAME, _SESSION_TOKEN
    ...
    def test_reveal_env_var_legacy_authorization_header_still_works(self, tmp_path):
        ...
        from anika_cli.config import save_env_value
        from anika_cli.web_server import _SESSION_TOKEN
    ...
    def test_telegram_onboarding_apply_reports_restart_failure_after_save(
        ...
        def fake_request(method, path, *, body=None, bearer_token=None):
            if method == "POST":
                return {
                    ...
                    "suggested_username": "anika_pair_restart_fails_bot",
                    "deep_link": "https://t.me/newbot/AnikaSetupBot/anika_pair_restart_fails_bot",
                    "qr_payload": "https://t.me/newbot/AnikaSetupBot/anika_pair_restart_fails_bot",
            ...
            return {
                "status": "ready",
                "bot_username": "anika_pair_restart_fails_bot",
        ...
        monkeypatch.setattr(_web_server_gateway, "_spawn_anika_action", fail_spawn_action)
    ...
    def test_unauthenticated_api_blocked(self):
        ...
        from starlette.testclient import TestClient
        from anika_cli.web_server import app
    ...
    def test_parse_model_ids_handles_openai_and_bare_shapes(self):
        ...
        from anika_cli.web_server_profiles import _parse_model_ids
    ...
    def test_set_model_main_custom_persists_api_key_and_registers_provider(self):
        ...
        ``anika model`` custom flow. Regression for the desktop loop where a
    ...
    def test_deleting_the_active_custom_endpoint_clears_its_model_mirror(self):
        ...
        """
        from anika_cli.config import custom_endpoint_key_env, get_env_value, load_config
    ...
    def test_numeric_yaml_provider_key_can_be_activated_and_deleted(self):
        ...
        """
        from anika_cli.config import get_config_path, load_config
    ...
    def test_custom_endpoint_save_scopes_to_the_requested_profile(self):
        ...
        """
        from anika_cli import profiles as profiles_mod
        from anika_cli.config import custom_endpoint_key_env
        from anika_constants import get_anika_home

        default_home = get_anika_home()
        worker_home = profiles_mod.get_profile_dir("worker")
        worker_home.mkdir(parents=True)
    ...
    def test_custom_endpoint_save_leaves_a_hand_written_env_ref_alone(self, monkeypatch):
        ...
        from anika_cli.config import custom_endpoint_key_env, get_config_path, get_env_value
    ...
    def test_two_endpoints_on_one_host_keep_separate_credentials(self):
        ...
        the first's key.
        """
        from anika_cli.config import custom_endpoint_key_env, get_env_value
...
class TestBuildSchemaFromConfig:
    ...
    def test_overrides_applied(self):
        from anika_cli.web_server_config import CONFIG_SCHEMA
    ...
    def test_timezone_field_is_searchable_select(self):
        ...
        """
        from anika_cli.web_server_config import CONFIG_SCHEMA, _timezone_options
...
class TestConfigRoundTrip:
    ...
    def test_round_trip_preserves_schema_invisible_nested_keys(self):
        ...
        from anika_cli.config import load_config, read_raw_config, save_config
...
class TestNewEndpoints:
    ...
    @pytest.fixture(autouse=True)
    def _setup(self, monkeypatch, _isolate_anika_home):
        try:
            from starlette.testclient import TestClient
        except ImportError:
            pytest.skip("fastapi/starlette not installed")
    ...
    def test_profiles_create_builder_mcp_auth_is_profile_scoped(
        ...
        from anika_constants import get_anika_home
        import anika_cli.profiles as profiles_mod
    ...
    def test_toolsets_resolve_subscription_features_once(self, monkeypatch):
        import anika_cli.tools_config as tools_config
        from anika_cli.nous_subscription import NousSubscriptionFeatures
    ...
    def test_get_toolset_config_reports_truthful_provider_status(self, monkeypatch):
        ...
        logged-out __NEW_ORG__ Subscription rows, xAI TTS without Grok OAuth, and
        never-installed KittenTTS/Piper. The endpoint now reports the honest
        state so keyless ≠ ready.
        """
        import anika_cli.tools_config as tools_config
        from anika_cli.nous_account import NousPortalAccountInfo

        # Logged out of __NEW_ORG__ Portal → managed subscription rows need sign-in.
        monkeypatch.setattr(
            "anika_cli.nous_subscription.get_nous_portal_account_info",
        ...
        import anika_cli.tools_config_post_setup as tools_config_post_setup
        ...
        assert by_name["__NEW_ORG__ Subscription"]["status"] == "needs_auth"
    ...
    def test_select_managed_nous_provider_reports_needs_nous_auth(self, monkeypatch):
        """Selecting a managed __NEW_ORG__ row while logged out flags needs_nous_auth.
        ...
        gap so the client can drive the existing __NEW_ORG__ OAuth flow.
        """
        from anika_cli.nous_account import NousPortalAccountInfo
        ...
        resp = self.client.put(
            ...
            json={"provider": "__NEW_ORG__ Subscription (Browser Use cloud)"},
        )
        assert resp.status_code == 200
        data = resp.json()
        assert data["ok"] is True
        ...
        # maps it to the Browser Use cloud through the __NEW_ORG__ Tool Gateway).
        from anika_cli.config import load_config
        cfg = load_config()
    ...
    def test_terminal_ssh_probe_ready_when_configured(self, monkeypatch):
        ...
        config["terminal"]["ssh_user"] = "anika"
        save_config(config)
        ...
        assert "anika@devbox.example.com" in ssh["detail"]
...
class TestDesktopLoopbackAuthExemption:
    ...
    def test_exempt_with_desktop_env_and_session_token_on_loopback(self, monkeypatch):
        ...
        monkeypatch.setenv("ANIKA_DESKTOP", "1")
        monkeypatch.setenv("ANIKA_DASHBOARD_SESSION_TOKEN", "desktop-minted")
    ...
    def test_not_exempt_without_desktop_env(self, monkeypatch):
        ...
        monkeypatch.delenv("ANIKA_DESKTOP", raising=False)
        monkeypatch.setenv("ANIKA_DASHBOARD_SESSION_TOKEN", "tok")
    ...
    def test_not_exempt_without_any_credential(self, monkeypatch):
        ...
        # ANIKA_DESKTOP=1 alone is not enough: a plain serve with the env var
...
class TestModelContextLength:
    ...
    def test_normalize_extracts_context_length_from_dict(self):
        ...
        from anika_cli.web_server_config import _normalize_config_for_web
    ...
    def test_denormalize_writes_context_length_into_model_dict(self):
        ...
        from anika_cli.web_server_config import _denormalize_config_from_web
        from anika_cli.config import save_config
...
class TestStatusInstallId:
    ...
    Anika home (not the profile home), survives a fresh process-cache read,
    ...
    def test_status_reports_persistent_install_id(self, monkeypatch):
        ...
        id_file = get_default_anika_root() / "install_id"
    ...
    def test_all_profiles_of_one_install_share_the_id(self, monkeypatch, tmp_path):
        """ANIKA_HOME=<root> and ANIKA_HOME=<root>/profiles/<name> resolve to
        ...
        monkeypatch.setenv("ANIKA_HOME", str(profile_home))
        monkeypatch.setattr(ws, "_INSTALL_ID_CACHE", {"value": None})
...
class TestNormaliseThemeDefinition:
    ...
    def test_rejects_non_dict(self):
        from anika_cli.web_server_dashboard import _normalise_theme_definition
...
class TestDiscoverUserThemes:
    """Tests for _discover_user_themes() — scans ~/.anika/dashboard-themes/."""

    def test_returns_empty_when_dir_missing(self, tmp_path, monkeypatch):
        monkeypatch.setenv("ANIKA_HOME", str(tmp_path))
        from anika_cli import web_server
    ...
    def test_ignores_transient_profile_override(self, tmp_path, monkeypatch):
        ...
        from anika_constants import (
            reset_anika_home_override,
            set_anika_home_override,
        )
        from anika_cli import web_server

        token = set_anika_home_override(str(other))
        try:
            results = _web_server_dashboard._discover_user_themes()
        finally:
...
class TestThemeBootstrapCSS:
    ...
    @staticmethod
    def _write_theme(anika_home, name="ocean"):
        themes_dir = anika_home / "dashboard-themes"
    ...
    def test_user_theme_renders_bundle_vars(self, tmp_path, monkeypatch):
        ...
        assert css.startswith('<style id="anika-theme-bootstrap">')
...
class TestDeleteSessionEndpoint:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_anika_home):
        ...
        monkeypatch.setattr(
            anika_state, "DEFAULT_DB_PATH", get_anika_home() / "state.db"
        )
...
class TestBulkDeleteSessionsEndpoint:
    ...
    1. Route-ordering: ``/api/sessions/bulk-delete`` must shadow the
       ...
       the block comment in ``anika_cli/web_server.py``).
    ...
    def test_route_order_not_shadowed_by_session_id(self):
        ...
        assert "deleted" in body, (
            ...
            "registration order in anika_cli/web_server.py."
        )
...
class TestDeleteEmptySessionsEndpoint:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_anika_home):
        ...
        # Pin the SessionDB to the isolated ANIKA_HOME so each test
...
class TestPluginAPIAuth:
    ...
    @pytest.fixture(autouse=True)
    def _setup_test_client(self, monkeypatch, _isolate_anika_home, _install_example_plugin):
        ...
        installs it into the per-test ``ANIKA_HOME``.
        """
        try:
            from starlette.testclient import TestClient
        except ImportError:
    ...
    def test_plugin_route_allows_auth(self):
        ...
        test fixture (installed into ANIKA_HOME by the class-level
    ...
    def test_non_kanban_plugin_route_requires_auth(self):
        ...
        so any plugin's API surface — kanban, anika-achievements, future
        plugins — must require the session token. Hit a non-kanban plugin
        path to lock that in.
        """
        # Real plugin path (anika-achievements is loaded by default).
        resp = self.client.get("/api/plugins/anika-achievements/overview")
        assert resp.status_code == 401
...
class TestDashboardPluginManifestExtensions:
    ...
    def test_user_plugins_ignore_profile_home_override(self, tmp_path, monkeypatch):
        ...
        ANIKA_HOME override scopes a request to another profile."""
        from anika_constants import (
            reset_anika_home_override,
            set_anika_home_override,
        )
        ...
        monkeypatch.setenv("ANIKA_HOME", str(launch_home))
        from anika_cli import web_server
        token = set_anika_home_override(str(other))
        try:
    ...
    def test_user_plugins_found_under_profile_scoped_process(self, tmp_path, monkeypatch):
        ...
        sets ANIKA_HOME=<root>/profiles/<name>) must still discover user
        plugins installed in the anika root's plugins/ directory."""
        root = tmp_path / "anika-root"
...
# or ``sh -c 'printf …'``) instead of the real ``anika --tui`` binary.  The
# endpoint resolves its argv through ``_resolve_chat_argv``, so tests
# monkeypatch that hook.
# ---------------------------------------------------------------------------

import sys
from anika_cli import main_tui_launch
...
class TestPtyWebSocket:
    ...
    def test_tui_python_command_uses_child_path(self, tmp_path):
        ...
        import anika_cli.main as main_mod

        command = f"anika-review-python{Path(sys.executable).suffix}"
        ...
        env = {
            "ANIKA_CWD": str(tmp_path),
            "ANIKA_PYTHON": command,
            "PATH": str(bin_dir),
        }

        main_tui_launch._apply_tui_python_env(env)

        assert env["ANIKA_PYTHON"] == command
    ...
    def test_unavailable_platform_closes_with_message(self, monkeypatch):
        from anika_cli.pty_bridge import PtyUnavailableError
    ...
    def test_pub_broadcasts_to_events_subscribers(self):
        ...
        """
        import asyncio
        from anika_cli import web_server as ws_mod
...
def test_resolve_chat_argv_injects_gateway_ws_url(monkeypatch):
    import anika_cli.main_tui_launch as tui_launch
    import anika_cli.web_server as ws
    ...
    gateway_url = env.get("ANIKA_TUI_GATEWAY_URL", "")
...
class TestDashboardPluginStaticAssetAllowlist:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_anika_home, _install_example_plugin):
        ...
        the per-test ``ANIKA_HOME`` user-plugins dir.
        """
        try:
            from starlette.testclient import TestClient
        except ImportError:
...
class TestDesktopCronTicker:
    ...
    def test_ticker_runs_when_desktop(self, monkeypatch, _isolate_anika_home):
        import threading
        ...
        with self._client():
            assert called.wait(3.0), "expected cron tick under ANIKA_DESKTOP=1"
...
class TestServeIndexMissingIndex:
    ...
    def _client_with_dist(tmp_path, monkeypatch, *, write_index: bool):
        ...
        monkeypatch.setattr(ws, "WEB_DIST", dist)
        monkeypatch.delenv("ANIKA_SERVE_HEADLESS", raising=False)
        spa_app = FastAPI()
        _web_server_dashboard.mount_spa(spa_app)
    ...
    def test_index_uses_ssh_token_applied_after_spa_mount(
        ...
        assert resp.status_code == 200
        assert 'window.__ANIKA_SESSION_TOKEN__="after-mount"' in resp.text


class TestHeadlessServeTokenPage:
    """Headless `anika serve` must serve the Desktop token handshake page
    at `/` when the dashboard auth gate is off (#94227).

    The Electron renderer boots by fetching `/` and extracting
    ``window.__ANIKA_SESSION_TOKEN__`` for WebSocket auth. Headless serve
    ...
    def _headless_client(monkeypatch, *, gated: bool):
        ...
        monkeypatch.setenv("ANIKA_SERVE_HEADLESS", "1")
        spa_app = FastAPI()
    ...
    def test_root_serves_token_page_when_not_gated(self, monkeypatch):
        ...
        match = re.search(
            r'window\.__ANIKA_SESSION_TOKEN__\s*=\s*("(?:\\.|[^"\\])*")',
            resp.text,
        )
        ...
        assert "window.__ANIKA_AUTH_REQUIRED__=false" in resp.text
...
class TestHashedAssetCacheHeaders:
    ...
    def test_serve_css_is_immutable_and_keeps_prefix_rewrites(
        ...
        prefixed = client.get(
            ...
            headers={"X-Forwarded-Prefix": "/anika"},
        )
        assert prefixed.status_code == 200
        assert prefixed.headers["cache-control"] == self._IMMUTABLE
        assert "url(/anika/ds-assets/bg.png)" in prefixed.text
        assert "url(/anika/fonts-terminal/x.woff2)" in prefixed.text
