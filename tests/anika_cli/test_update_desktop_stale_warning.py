...
#88251: ``anika update`` treated a failed desktop pack as non-fatal, printed
...
import anika_cli.update_cmd_maint as update_cmd_maint
...
def desktop_env(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "anika_constants.with_anika_node_path", lambda: {}, raising=False
    )
    monkeypatch.setattr(
        "anika_constants.display_anika_home", lambda: str(tmp_path), raising=False
    )
...
def test_gateway_exit_code_file_tracks_desktop_rebuild(tmp_path, monkeypatch):
    monkeypatch.setattr(update_cmd, "get_anika_home", lambda: tmp_path)
