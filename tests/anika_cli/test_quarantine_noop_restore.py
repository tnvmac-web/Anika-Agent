...
``anika*.exe`` shims aside (``anika.exe.old.<ms>``) before invoking the
...
quarantined shims stayed renamed aside and ``anika`` vanished from PATH
...
def _make_scripts_dir(tmp_path: Path) -> Path:
    ...
    for name in ("anika", "anika-agent", "anika-acp", "anika-gateway"):
...
def test_main_noop_success_restores_shims(tmp_path):
    ...
    assert "anika.exe" in names, "anika.exe must be restored after a no-op install"
    assert "anika-acp.exe" in names
    assert "anika-gateway.exe" in names
    assert (scripts / "anika.exe").read_bytes() == b"MZ-old-anika"
...
def test_main_rewriting_success_keeps_fresh_shims(tmp_path):
    ...
    assert (scripts / "anika.exe").read_bytes() == b"MZ-new-anika"
...
# ---------------------------------------------------------------------------
# anika_cli._install_repair._run_install_cmd (the deferred-recovery path)
# ---------------------------------------------------------------------------
...
def test_repair_noop_success_restores_shims(tmp_path):
    ...
    names = _shim_names(scripts)
    assert "anika.exe" in names, "anika.exe must be restored after a no-op recovery install"
    assert (scripts / "anika.exe").read_bytes() == b"MZ-old-anika"
...
def test_repair_rewriting_success_keeps_fresh_shims(tmp_path):
    ...
    def fake_run(cmd, cwd=None, check=None, env=None):
        (scripts / "anika.exe").write_bytes(b"MZ-new-anika")
