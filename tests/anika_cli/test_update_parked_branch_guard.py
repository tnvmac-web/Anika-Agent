"""Regression tests for the parked-branch guard in ``anika update``.
...
memory-limit``, days behind main) left there by earlier tooling. ``anika
...
def _no_config(monkeypatch):
    ...
    import anika_cli.config as anika_config

    monkeypatch.setattr(anika_config, "load_config", lambda: {})
...
def test_skip_warning_names_branch_behind_count_and_commands(repo_pair, capsys):
    ...
    assert f"git -C {repo_pair} checkout main && anika update" in out
...
def test_print_update_completion_carries_branch_and_sha(
    ...
    monkeypatch.setattr(anika_main, "PROJECT_ROOT", repo_pair)
...
def _patch_update_flow(monkeypatch, repo, run_real_git=True):
    ...
    """
    monkeypatch.setattr(anika_main, "PROJECT_ROOT", repo)
    ...
    monkeypatch.setattr(anika_main, "_record_bytecode_fingerprint", lambda *a, **k: None)
    ...
    monkeypatch.setattr(anika_main, "_pause_windows_gateways_for_update", lambda: None)
    monkeypatch.setattr(
        anika_main, "_resume_windows_gateways_after_update", lambda *a, **k: None
    )
    monkeypatch.setattr(anika_main, "_capture_active_lazy_features", lambda: [])
    monkeypatch.setattr(anika_main, "_capture_active_tool_dependencies", lambda: [])
