...
substring check, missing case variants and matching lookalike URLs). Anika
had the same class at several sites: keyless-endpoint detection, /model
catalog routing, local-endpoint detection, and __NEW_ORG__ Portal cache-layout
...
def test_nous_portal_host_detection():
    ...
    assert base_url_host_matches("https://inference-api.__NEW_DOMAIN__/v1", "__NEW_DOMAIN__")
    assert base_url_host_matches("https://portal.__NEW_DOMAIN__", "__NEW_DOMAIN__")
    assert not base_url_host_matches("https://__NEW_DOMAIN__.evil.io/v1", "__NEW_DOMAIN__")
    assert not base_url_host_matches("https://proxy.example/__NEW_DOMAIN__/v1", "__NEW_DOMAIN__")
