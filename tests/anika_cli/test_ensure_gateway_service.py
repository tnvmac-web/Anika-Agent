"""Tests for anika_cli.gateway.ensure_gateway_service — the zero-prompt
service install/start path used by `anika setup` and `anika import`.
...
def _patch_host(monkeypatch, *, container=False, systemd=True, macos=False, windows=False):
    monkeypatch.setattr("anika_constants.is_container", lambda: container)
