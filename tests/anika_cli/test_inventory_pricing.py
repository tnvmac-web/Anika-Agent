...
columns + Free/Pro badges and gate paid models on free __NEW_ORG__ accounts, the
same way the `anika model` CLI picker does.
"""

from threading import Event
from time import monotonic

import anika_cli.inventory as inv
...
def test_prewarm_preserves_context_and_runs_once_per_profile(tmp_path, monkeypatch):
    ...
    def capture_context(_rows):
        ...
        observed[label] = (anika_home_key(), dict(scope))
        started[label].set()
    ...
    try:
        ...
        assert observed["a"] == (
            anika_home_key(tmp_path / "a"),
            {"PROFILE_MARKER": "a"},
        )
        assert observed["b"] == (
            anika_home_key(tmp_path / "b"),
            {"PROFILE_MARKER": "b"},
        )
    finally:
        release.set()
...
def test_prewarm_endpoint_rotation_starts_a_new_worker(tmp_path, monkeypatch):
    ...
    token = set_anika_home_override(str(tmp_path / "profile"))
    threads = []
    try:
        threads.append(
...
def test_prewarm_nous_rotation_when_another_provider_is_current(tmp_path, monkeypatch):
    """__NEW_ORG__ endpoint identity must not depend on __NEW_ORG__ being selected."""
    from anika_constants import (
        reset_anika_home_override,
        set_anika_home_override,
    )
