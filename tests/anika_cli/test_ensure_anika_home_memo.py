"""ensure_anika_home is memoized per home path (perf: it runs on every
...
def test_distinct_home_paths_each_get_the_skeleton(tmp_path, monkeypatch):
    first = tmp_path / "a" / ".anika"
    second = tmp_path / "b" / ".anika"

    monkeypatch.setenv("ANIKA_HOME", str(first))
    cfg.ensure_anika_home()

    # Profile switch: ANIKA_HOME moves → the new path is ensured too.
