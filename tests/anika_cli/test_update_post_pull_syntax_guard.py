"""Tests for the post-pull syntax guard in ``anika update``.

When a bad commit lands on ``main`` with a syntax error in a critical file
(e.g. orphan merge-conflict markers in ``anika_cli/config.py``), the CLI
becomes unbootable — every ``anika`` invocation imports those files at
...
markers in ``anika_cli/config.py``; users who ran ``anika update`` in
the 7-minute window before #28458 landed could not run any ``anika``
command afterward.
"""
...
def test_capture_head_sha_returns_stripped_sha(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(anika_main.subprocess, "run", fake_run)
...
def test_validate_critical_files_syntax_tolerates_missing_files(tmp_path):
    ...
    # Populate everything except anika_constants.py
    for relpath in update_cmd._UPDATE_CRITICAL_FILES:
        if relpath == "anika_constants.py":
            continue
...
# release; if a future ``anika update`` would brick users, this test fails
# in CI first.
# ---------------------------------------------------------------------------
