...
The notice prints the most recent curator run summary on `anika update`,
...
session start) so users normally never see the rename map. `anika update`
...
def curator_env(tmp_path, monkeypatch, capsys):
    ...
    from anika_cli import update_cmd_maint
