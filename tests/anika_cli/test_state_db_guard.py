...
from anika_cli.backup import verify_sqlite_integrity
...
class TestPreUpdateBackupIntegrityGuard:
    ...
    ANIKA_HOME whose state.db is corrupted mid-flight (#68474)."""
    ...
    def anika_home(self, tmp_path, monkeypatch):
        ...
        for mod in list(sys.modules.keys()):
            if mod.startswith("anika_cli.config") or mod == "anika_constants":
                del sys.modules[mod]
        return root

    def test_healthy_db_stays_quiet(self, anika_home, capsys):
    ...
    def test_zeroed_db_after_snapshot_is_loud(self, anika_home, capsys, monkeypatch):
