"""Tests for kanban board export / import (``anika_cli.kanban_transfer``).
...
from anika_cli import kanban_transfer as kt
from anika_cli.archive_safe import normalize_archive_parts, safe_extract_targz
...
def kanban_root(tmp_path, monkeypatch):
    ...
    def _use(name: str) -> Path:
        ...
        monkeypatch.setenv("ANIKA_KANBAN_HOME", str(root))
        for var in ("ANIKA_KANBAN_DB", "ANIKA_KANBAN_WORKSPACES_ROOT",
                    "ANIKA_KANBAN_ATTACHMENTS_ROOT", "ANIKA_KANBAN_BOARD"):
...
def test_import_rejects_a_future_format_version(kanban_root, tmp_path):
    ...
    kanban_root("target")
    with pytest.raises(ValueError, match="newer than this Anika"):
