...
Covers:
- Defense 1: gateway stop/restart refuse when _ANIKA_GATEWAY=1
...
from anika_cli.cron import cron_command
...
class TestGatewayLifecyclePattern:
    ...
    @pytest.mark.parametrize("text", [
        ...
        "anika gateway stop",
        "anika gateway uninstall",
        "anika  gateway  restart",         # double spaces
        "Hermez Gateway Restart".lower().replace("z", "s"),  # case handled
        "ANIKA GATEWAY RESTART",           # uppercase
    ])
    def test_anika_gateway_commands(self, text):
    ...
    @pytest.mark.parametrize("text", [
        ...
        "launchctl submit -l ai.anika.gateway-hard-restart-no-photon-notice -- /bin/sh ~/.anika/scripts/hard_restart_gateway_no_photon_notice.sh",
        "launchctl submit -l anika-gateway-restart-helper -- /bin/sh helper.sh",
        # bootstrap loads an arbitrary plist — same laundering shape.
        "launchctl bootstrap gui/501 ~/Library/LaunchAgents/ai.anika.gateway.restart-once.plist",
        "launchctl bootout gui/501/ai.anika.gateway",
        ...
        (
            "launchctl submit \\\n"
            "  -l ai.anika.gateway-hard-restart-no-photon-notice \\\n"
            "  -- /bin/sh ~/.anika/scripts/hard_restart_gateway_no_photon_notice.sh"
        ),
    ])
    ...
    def test_label_defined_before_verb_is_caught(self):
        ...
        cmd = (
            "uid=$(id -u); for item in 'ai.anika.gateway-apollo:/a.plist' "
            "'ai.anika.gateway-cronus:/c.plist' 'ai.anika.gateway-plutus:/p.plist' "
            "'ai.anika.gateway:/Users/botuser/Library/LaunchAgents/ai.anika.gateway.plist'; "
    ...
    def test_line_continuation_does_not_bridge_unrelated_lines(self):
        ...
        text = (
            ...
            "unrelated anika note on the next line"
        )
    ...
    @pytest.mark.parametrize("text", [
        ...
        # `launchctl kick"start" ... ai.anika.gateway` executes exactly as
        ...
        'launchctl kick"start" -k gui/501/ai.anika.gateway',
        "launchctl kick'start' -k gui/501/ai.anika.gateway",
        "launchctl kick\\start -k gui/501/ai.anika.gateway",
        'launchctl "kickstart" -k gui/501/ai.anika.gateway',
        # Splices on the newer/legacy unload spellings this PR added.
        'launchctl boot"out" gui/501/ai.anika.gateway',
        "launchctl dis\\able gui/501/ai.anika.gateway",
        # The gateway identifier itself can be spliced just as easily.
        'launchctl bootout gui/501/ai.anika."gateway"',
        # Same class on the systemctl and anika-CLI branches.
        'systemctl re"start" anika-gateway',
        'anika gateway re"start"',
    ])
    ...
    def test_spliced_verb_inside_shell_c_payload_is_blocked(self):
        ...
        command = 'sh -c \'launchctl kick"start" -k gui/501/ai.anika.gateway\''
    ...
    @pytest.mark.parametrize("text", [
        ...
        'launchctl kick"start" -k gui/501/ai.anika.update-checker',
        'systemctl re"start" anika-meta.service',
    ...
    @pytest.mark.parametrize("text", [
        ...
        "anika cron list",
        "anika update",
        "anika config set model claude",
        "echo 'just a normal cron job'",
        "run the backup script",
        "gateway is running fine",
        # `anika gateway start` is benign — starting a gateway from inside a
        ...
        "anika gateway start",
        "anika gateway start --all",
        # Tightened launchctl/systemctl branches: ops on NON-gateway anika
        # services must not be falsely blocked (the old `.*anika` matched any
        # anika token).
        "launchctl unload ai.anika.update-checker.plist",
        "launchctl restart ai.anika.daemon",
        ...
        "systemctl restart anika-meta.service",
        "systemctl restart anika-cron-helper",
        ...
        "echo after the anika gateway restarted cleanly",
        "the anika gateway stopped responding, please investigate",
        # #92372 Branch D: `p?kill` without a leading \b matched the "kill"
        # tail of "skill".
        "anika skill view gateway-notes && echo anika gateway docs",
        # #77173/#77536: a file path with embedded spaces containing the
        # lifecycle words must not match — `anika` is a path component
        # there, not a command.
        "cat '/docs/anika gateway restart-notes.md'",
        "less /home/user/notes/anika gateway restart runbook.txt",
    ])
    ...
    @pytest.mark.parametrize("text", [
        ...
        "anika gateway restart",
        "anika gateway restart; echo done",
        "anika gateway stop && echo stopped",
        # #77173 command-position anchor must not weaken separator/subshell
        # forms either.
        "true;anika gateway restart",
        "true && anika gateway stop",
        "echo $(anika gateway restart)",
        "echo `anika gateway restart`",
    ])
    ...
    @pytest.mark.parametrize("text", [
        ...
        'import subprocess\nsubprocess.run(["launchctl", "bootout", "gui/501/ai.anika.gateway"])',
        'subprocess.run(["anika", "gateway", "restart"])',
        'os.system("launchctl kickstart -k gui/501/ai.anika.gateway")',
    ])
    ...
    @pytest.mark.parametrize("text", [
        ...
        'data = ["anika", "notes"]  # unrelated list',
    ])
    ...
    def test_inert_heredoc_body_prose_not_blocked(self):
        ...
        text = (
            ...
            "If the box is wedged, a human can run: anika gateway restart\n"
            "EOF"
        )
        assert not _contains_gateway_lifecycle_command(text), f"Should NOT match: {text!r}"
...
class TestProfileFlagGatewayLifecycle:
    """#78028: `anika -p <profile> gateway restart|stop` bypasses Branch A's
    ...
    def _pin_profile_identity(self, monkeypatch):
        # The ambient test env may carry ANIKA_HOME/ANIKA_PROFILE; pin the
        # profile identity explicitly so every assertion is deterministic.
        monkeypatch.setenv("ANIKA_PROFILE", "zeus")
        monkeypatch.delenv("ANIKA_PROFILE_NAME", raising=False)

    @pytest.mark.parametrize("text", [
        "anika -p zeus gateway stop",
        "anika -p zeus gateway restart",
        "anika --profile zeus gateway restart",
        "anika --profile zeus gateway stop",
        "anika --profile=zeus gateway restart",
        # Global flags before/after the selector must not hide the shape.
        "anika -v -p zeus gateway restart",
        "anika -p zeus -v gateway restart",
        "anika --debug --profile zeus gateway stop",
    ...
    @pytest.mark.parametrize("text", [
        "anika -p venus gateway stop",
        "anika -p venus gateway restart",
        "anika --profile venus gateway restart",
        "anika --profile=venus gateway stop",
        "anika -p venus -v gateway restart",
    ])
    def test_sibling_allowed(self, text):
        assert not _contains_gateway_lifecycle_command(text), f"Should allow: {text!r}"

    @pytest.mark.parametrize("text", [
        "anika -p zeus gateway start",
        "anika -p zeus gateway start --all",
    ])
    ...
    def test_adjacent_form_still_blocked(self):
        ...
        assert _contains_gateway_lifecycle_command("anika gateway restart")
        assert _contains_gateway_lifecycle_command("anika gateway stop")

    def test_anika_home_derived_profile(self, monkeypatch):
        # Without ANIKA_PROFILE the guard falls back to the ANIKA_HOME-
        # derived profile identity (get_active_profile_name) — the signal the
        # gateway process itself carries.
        monkeypatch.delenv("ANIKA_PROFILE", raising=False)
        monkeypatch.delenv("ANIKA_PROFILE_NAME", raising=False)
        import anika_cli.profiles as profiles_mod

        monkeypatch.setattr(profiles_mod, "get_active_profile_name", lambda: "zeus")
        assert _contains_gateway_lifecycle_command("anika -p zeus gateway restart")
        assert not _contains_gateway_lifecycle_command("anika -p venus gateway restart")
    ...
    def test_no_profile_context_conservative_allow(self, monkeypatch):
        ...
        assert not _contains_gateway_lifecycle_command("anika -p zeus gateway restart")
        assert _contains_gateway_lifecycle_command("anika gateway restart")
...
class TestCronCreateLifecycleBlock:
    ...
    def test_block_anika_gateway_restart(self, capsys):
        args = Namespace(
            cron_command="create",
            schedule="30m",
            prompt="Upgrade anika then run anika gateway restart",
            name=None,
            deliver=None,
            repeat=None,
            skill=None,
    ...
    def test_block_script_with_lifecycle_command(self, tmp_path, capsys, monkeypatch):
        ...
        # restart_anika_gateway_once.sh). The script must live under
        # ANIKA_HOME/scripts so the scheduler — and the guard — resolve it.
        monkeypatch.setenv("ANIKA_HOME", str(tmp_path / ".anika"))
        scripts_dir = tmp_path / ".anika" / "scripts"
...
class TestGatewaySelfTargetingGuard:
    ...
    def test_stop_allows_outside_gateway(self, monkeypatch):
        ...
        monkeypatch.delenv("_ANIKA_GATEWAY", raising=False)
...
class TestTerminalToolGatewayLifecycleGuard:
    """terminal_tool must refuse gateway lifecycle commands when _ANIKA_GATEWAY=1.

    Issue #37453: systemctl --user restart anika-gateway runs as a child of the
    ...
    @pytest.mark.parametrize("cmd", [
        "systemctl restart anika-gateway",
        "systemctl --user restart anika-gateway",
        "systemctl stop anika-gateway.service",
        "anika gateway restart",
        "anika gateway uninstall",
        "launchctl kickstart gui/501/ai.anika.gateway",
        "launchctl bootout gui/501/ai.anika.gateway",
        # #62891 exact reported shape and its bootstrap sibling.
        "launchctl submit -l ai.anika.gateway-hard-restart-no-photon-notice -- /bin/sh ~/.anika/scripts/hard_restart_gateway_no_photon_notice.sh",
        "launchctl submit -l com.foo -- /path/gateway",
        "launchctl bootstrap gui/501 ~/Library/LaunchAgents/ai.anika.gateway.restart-once.plist",
        "pkill -f anika.*gateway",
    ])
    ...
    def test_force_true_cannot_bypass_block(self, monkeypatch):
        ...
        result = json.loads(tt.terminal_tool(
            command="systemctl restart anika-gateway", force=True
        ))
    ...
    def test_blocks_launchctl_submit_inside_gateway(self, monkeypatch, tmp_path):
        ...
        result = json.loads(tt.terminal_tool(
            command=(
                "launchctl submit -l ai.anika.delayed-ops -- "
                f"/bin/bash {script}"
            )
        ))
    ...
    @pytest.mark.parametrize("command", [
        # Neutral, non-anika label: label-independent detection is the point
        # (#62891 second reproduction used `ai.anika.svc-reload-tmp`).
        "launchctl submit -l com.foo -- /path/gateway",
        "launchctl submit -l ai.anika.svc-reload-tmp -- /bin/sh /tmp/h-svc-reload.sh",
    ...
    def test_cli_agent_session_not_blocked_by_inherited_env(
        ...
        """#92560: CLI/TUI agent sessions inherit _ANIKA_GATEWAY=1 from the
        ...
        # Simulate a CLI agent session: _ANIKA_GATEWAY=1 is in the
        ...
        self._patch_env(monkeypatch, _FakeEnv(), inside_gateway=False)
        monkeypatch.setenv("_ANIKA_GATEWAY", "1")
        monkeypatch.setattr(
            tt, "_check_all_guards", lambda cmd, env, **kwargs: {"approved": True}
        )

        result = json.loads(tt.terminal_tool(command="anika gateway restart"))

        assert result["exit_code"] == 0
        assert calls == ["anika gateway restart"]
    ...
    def test_blocks_launchctl_submit_hidden_in_referenced_script(
        ...
        script.write_text(
            "#!/bin/bash\nlaunchctl submit -l ai.anika.loop -- /bin/true\n"
        )
        self._patch_env(monkeypatch, self._make_fake_env(), inside_gateway=True)
    ...
    def test_launchctl_submit_parser_handles_shell_quoting(self, monkeypatch):
        ...
        self._patch_env(monkeypatch, self._make_fake_env(), inside_gateway=True)
        result = json.loads(tt.terminal_tool(
            command="launchctl sub\"\"mit -l ai.anika.loop -- /bin/true"
        ))
    ...
    def test_safe_systemctl_commands_pass_through(self, monkeypatch):
        """Non-anika systemctl commands must not be blocked by this guard."""
        import tools.terminal_tool as tt
...
class TestLifecycleGuardModule:
    ...
    def test_prompt_with_command_raises(self):
        ...
        with pytest.raises(GatewayLifecycleBlocked) as exc:
            check_gateway_lifecycle("please run anika gateway restart", None)
    ...
    def test_split_across_prompt_and_script_still_blocks(self, tmp_path):
        ...
        script.write_text("anika gateway stop\n", encoding="utf-8")
        with pytest.raises(GatewayLifecycleBlocked):
    ...
    def test_binary_script_does_not_silently_bypass(self, tmp_path):
        ...
        script.write_bytes(b"\xfeanika gateway restart\xff")
        with pytest.raises(GatewayLifecycleBlocked):
            check_gateway_lifecycle("", str(script))


    def test_relative_script_resolved_under_scripts_dir(self, tmp_path, monkeypatch):
        """A bare/relative script name resolves under ANIKA_HOME/scripts (the
    ...
    def test_python_script_with_literal_lifecycle_command_still_blocked(
        ...
        script.write_text('import os\nos.system("anika gateway restart")\n', encoding="utf-8")
        with pytest.raises(GatewayLifecycleBlocked):
            check_gateway_lifecycle("clean prompt", str(script))
    ...
    def test_read_referenced_script_tolerates_nul_in_path(self):
        ...
        text, unsafe = _read_referenced_script(Path("/tmp/anika\x00binary"))
        assert text is None
    ...
    def test_binary_from_remote_callback_never_false_positives(self):
        ...
        def _remote_read(_path: str):
            return "MZ\x00\x00\x90\x00 anika gateway restart \x00\x00junk"
    ...
    def test_cron_guard_total_when_home_unresolvable(self, monkeypatch):
        """`get_anika_home()` falls back to Path.home(), which raises
        RuntimeError when neither ANIKA_HOME nor HOME resolves
...
class TestRelativePathDoesNotDisableDataExemption:
    ...
    @pytest.mark.parametrize("command", [
        "grep -r 'systemctl restart anika-gateway' .",
        "grep -rn 'anika gateway restart' ./logs",
        "rg 'anika gateway restart' ../archive",
        "grep -c 'systemctl stop anika-gateway' ./var/log/syslog",
        "sqlite3 ./stats.db \"SELECT restart_reason FROM anika_gateway_restarts\"",
    ])
    ...
    @pytest.mark.parametrize("command", [
        ...
        'sqlite3 ./db ".shell anika gateway restart"',
        'sqlite3 ./db ".system systemctl restart anika-gateway"',
        'psql ./x -c "\\! systemctl restart anika-gateway"',
        "grep -r 'anika gateway restart' . | sh",
        "grep -r 'anika gateway restart' ./logs | bash",
        "grep -r 'anika gateway restart' . | sudo sh",
        "grep -r 'x' . ; anika gateway restart",
        "grep -r 'x' . && systemctl restart anika-gateway",
        'grep -r "$(anika gateway restart)" .',
        "rg 'x' ./logs | xargs systemctl restart anika-gateway",
    ])
    ...
    @pytest.mark.parametrize("command", [
        ...
        'sqlite3 db ".shell anika gateway restart"',
        'sqlite3 db ".system systemctl restart anika-gateway"',
        'psql -c "\\! systemctl restart anika-gateway"',
    ])
...
class TestCreateJobBlocksLifecycleCommands:
    ...
    anika_cli.cron.cron_create. Enforcing at create_job covers both."""
    ...
    def test_create_job_blocks_prompt_command(self):
        ...
        with pytest.raises(GatewayLifecycleBlocked):
            create_job(prompt="then run anika gateway restart", schedule="30m")
    ...
    def test_cronjob_tool_surfaces_block_as_error(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("ANIKA_HOME", str(tmp_path / ".anika"))
        (tmp_path / ".anika").mkdir(parents=True)
        from tools.cronjob_tools import cronjob
        result = json.loads(cronjob(
            action="create", schedule="0 9 * * *",
            prompt="please run anika gateway restart nightly",
        ))
...
class TestLifecycleGuardDataArgumentExemption:
    ...
    @pytest.mark.parametrize("command", [
        ...
        "'systemctl stop anika-gateway'\"",
        # grep/rg pattern arguments hunting for the lifecycle string.
        "grep -c 'systemctl restart anika-gateway' /var/log/syslog",
        "rg 'anika gateway restart' /home/user/.anika/logs/",
        "journalctl -u anika-gateway --grep 'systemctl restart anika-gateway'",
        # SQL with stop/restart column/value words but no command shape.
        'sqlite3 stats.db "SELECT stop_time, restart_reason FROM '
        'anika_gateway_restarts"',
    ...
    @pytest.mark.parametrize("command", [
        ...
        'sqlite3 db ".shell anika gateway restart"',
        'psql -c "\\! systemctl restart anika-gateway"',
        "grep 'systemctl restart anika-gateway' cmds.txt | sh",
        "grep gateway f | xargs systemctl restart anika-gateway",
        'grep "$(systemctl restart anika-gateway)" f',
        "grep 'restart' log; systemctl restart anika-gateway",
        'sqlite3 db "SELECT 1"; anika gateway stop',
        # Plain lifecycle commands are unaffected by the exemption.
        "anika gateway restart",
        "sudo systemctl stop anika-gateway",
    ])
    ...
    def test_python_script_branch_gets_the_same_exemption(self, tmp_path):
        ...
        prompt = (
            ...
            "WHERE msg LIKE '%systemctl restart anika-gateway%'\""
        )
