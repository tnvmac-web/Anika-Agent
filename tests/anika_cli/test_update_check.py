"""Tests for the update check mechanism in anika_cli.banner."""
...
def test_check_for_updates_uses_cache(tmp_path, monkeypatch):
    ...
    with patch("anika_cli.banner.subprocess.run") as mock_run:
...
def test_check_via_local_git_fetch_failure_returns_none(tmp_path, monkeypatch):
    ...
    def mock_git_stdout(args, *, cwd, timeout=5):
        if args[:2] == ["remote", "get-url"]:
            return "https://github.com/__NEW_ORG__/anika-agent.git"
...
def test_check_for_updates_does_not_cache_none(tmp_path, monkeypatch):
    ...
    cache_file = tmp_path / ".update_check"
    monkeypatch.setenv("ANIKA_HOME", str(tmp_path))
    monkeypatch.delenv("ANIKA_REVISION", raising=False)
    ...
    monkeypatch.setattr(
        "anika_cli.config.detect_install_method", lambda root: "git"
    )
    monkeypatch.setattr(
        "anika_cli.config.get_project_root", lambda: repo_dir
    )
    ...
    # Simpler: just patch the get_anika_home and the repo_dir resolution
    # by making check_for_updates find our fake repo via anika_home fallback.
    # The code checks Path(__file__).parent.parent/.git first, then falls
    # back to anika_home / "anika-agent". We ensure the fallback hits.
    # To do this, we make Path(__file__).parent.parent.resolve() return
    # a path without .git, so it falls through to anika_home / "anika-agent".
    real_resolve = Path.resolve

    def fake_resolve(self, *args, **kwargs):
        s = str(self)
        if "banner.py" in s or s.endswith("anika_cli"):
