...
anika_cli.config._load_config_impl, so the managed-scope merge has to be
applied in BOTH places or the interactive CLI/TUI surface (skin, display prefs)
silently ignores administrator-pinned values while `anika config`/`doctor`
...
def _load_cli_config(home):
    ...
    cli.py binds ``_anika_home = get_anika_home()`` at import time (module
    singleton), so monkeypatching ANIKA_HOME after import doesn't move it.
    ...
    this only matters for tests that swap ANIKA_HOME.)
    """
    import cli

    cli._anika_home = home
