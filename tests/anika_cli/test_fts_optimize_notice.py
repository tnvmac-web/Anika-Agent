...
def test_update_notice_offers_v1_trigram_tool_calls_rebuild(tmp_path, monkeypatch, capsys):
    ...
    class FakeSessionDB:
        ...
        _db_needs_fts_storage_upgrade = staticmethod(
            anika_state.SessionDB._db_needs_fts_storage_upgrade
        )

    monkeypatch.setattr(anika_constants, "get_anika_home", lambda: tmp_path)
    monkeypatch.setattr(anika_state, "SessionDB", FakeSessionDB)
    ...
    assert "anika sessions optimize-storage" in capsys.readouterr().out
    conn.close()
