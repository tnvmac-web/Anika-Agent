...
#
# Installs a real, earlier Anika the way a user does, applies ONE update route,
# and requires the checkout to land on this commit with a working `anika`.
#
...
# Keep sandbox state out of the default .anika-sandbox so a run never clobbers
...
SANDBOX_DIR_NAME=".anika-sandbox-e2e-$ROUTE"
export ANIKA_DEV_SANDBOX_DIR="$SANDBOX_DIR_NAME"

SANDBOX_ROOT="$REPO_ROOT/$SANDBOX_DIR_NAME"
INSTALL_DIR="/home/anika/.anika/anika-agent"   # user-level layout (sandbox default)
FAKE_REMOTE="/work/repos/anika-agent.git"
...
LOG_DIR="${ANIKA_E2E_LOG_DIR:-$(mktemp -d -t anika-install-e2e-logs.XXXXXX)}"
...
# ── helpers ────────────────────────────────────────────────────────────────
# Does the INSTALLED anika accept FLAG on `anika update`?
#
...
# that silently rots. `anika update --help` is the same surface a user meets,
# and argparse prints every option it accepts.
update_supports() {
  local flag="$1"
  in_sandbox "anika update --help 2>&1" | grep -qF -- "$flag"
}
...
require_anika_works() {
  local when="$1" out
  out="$(in_sandbox "anika --version" 2>&1)" \
    || { printf '%s\n' "$out" >&2; fail "anika --version failed $when"; }
  printf '%s\n' "$out" | sed 's/^/    /'
  ok "anika runs $when"
}

# ── install the earlier Anika ─────────────────────────────────────────────
...
require_anika_works 'after install'

# ── apply exactly one update route ─────────────────────────────────────────
case "$ROUTE" in
  update)
    step 'ROUTE: anika update'
    ...
    # anika which it accepts; older ones read the prompt from stdin, so close it.
    if update_supports --yes; then
      update_cmd="anika update --yes"
    else
      update_cmd="anika update </dev/null"
    fi
    if ! in_sandbox "cd $INSTALL_DIR && $update_cmd"; then
      collect_sandbox_logs update
      fail "anika update failed ($update_cmd)"
    fi
    require_landed_on_target 'anika update'
    require_anika_works 'after anika update'
    ;;
  installer)
    ...
    require_anika_works 'after installer re-run'
    ;;
esac
