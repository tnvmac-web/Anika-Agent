...
def test_root_session_no_compression(db):
    ...
    assert prov["currentAnikaSessionId"] == "root1"
    assert prov["rootAnikaSessionId"] == "root1"
    assert prov["parentAnikaSessionId"] is None
...
def test_compression_split_continuation(db):
    ...
    prov = build_session_provenance(
        db, "acp-1", "new", previous_anika_session_id="old"
    )
    assert prov["sessionKind"] == "continuation"
    assert prov["parentAnikaSessionId"] == "old"
    assert prov["rootAnikaSessionId"] == "old"
    assert prov["compressionDepth"] == 1
    assert prov["previousAnikaSessionId"] == "old"
...
def test_non_compression_parent_is_root_not_continuation(db):
    ...
    assert prov["sessionKind"] == "root"
    assert prov["compressionDepth"] == 0
    assert prov["rootAnikaSessionId"] == "p"  # lineage root still walked
...
def test_meta_wrapper_shape(db):
    ...
    assert set(meta.keys()) == {"anika"}
    assert "sessionProvenance" in meta["anika"]
    assert meta["anika"]["sessionProvenance"]["currentAnikaSessionId"] == "root1"
