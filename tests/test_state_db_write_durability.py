...
The defect: anika_state already knows macOS `fsync()` does not guarantee
...
import anika_state
from anika_state import repair_state_db_schema
from anika_state_repair import _connect_repair_durable
...
def test_repair_path_has_no_bare_connects() -> None:
    ...
    """
    # The repair/probe helpers live in anika_state_repair; anika_state only
    # re-imports them.
    import anika_state_repair

    source = Path(anika_state_repair.__file__).read_text(encoding="utf-8")
    tree = ast.parse(source, filename=str(anika_state_repair.__file__))
