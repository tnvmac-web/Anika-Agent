...
Override the image with ``ANIKA_TEST_IMAGE`` env var to point at a pre-built
...
IMAGE_TAG = os.environ.get("ANIKA_TEST_IMAGE", "anika-agent-harness:latest")
...
def built_image() -> str:
    ...
    Override with ``ANIKA_TEST_IMAGE`` env var to point at a pre-built
    image (faster local iteration).
    """
    if os.environ.get("ANIKA_TEST_IMAGE"):
        return IMAGE_TAG
...
# ---------------------------------------------------------------------------
# docker_exec — default to the unprivileged anika user
# ---------------------------------------------------------------------------
#
# Background: every Anika runtime path inside the container drops to UID
# 10000 (the ``anika`` user) via ``s6-setuidgid anika``. ``docker exec``
...
# fails (PermissionError swallowed) for anika, so a test that ran as root
...
# helpers below run every probe under ``-u anika`` unless a specific
...
def docker_exec(
    ...
    *args: str,
    ...
    """Run a command inside ``container`` as ``user`` (default: anika).
