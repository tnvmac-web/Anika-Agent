"""Runtime smoke test for Docker $ANIKA_HOME/logs/gateways seeding.

Build the real image and verify logs/ and logs/gateways/ exist and are
owned by the anika user after container boot.
...
registered later runs its log service as the dropped anika user and
...
def test_logs_gateways_seeded_and_anika_owned(
    built_image: str, container_name: str,
) -> None:
    """logs/ and logs/gateways/ must exist and be owned by anika after boot."""
    ...
    assert "logs=anika" in r.stdout, (
        f"logs/ not owned by anika: {r.stdout}"
    )
    assert "gateways=anika" in r.stdout, (
        f"logs/gateways/ not owned by anika: {r.stdout}"
    )
