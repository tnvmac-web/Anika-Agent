...
Build the real image and verify at runtime:
  ...
  1. /opt/anika is not writable by the anika user (immutable install tree)
  2. PYTHONDONTWRITEBYTECODE and ANIKA_DISABLE_LAZY_INSTALLS are set
  3. /opt/anika/.install_method contains "docker" (code-scoped stamp)
  4. $ANIKA_HOME/.install_method is NOT stamped as "docker" by stage2
  5. A stale "docker" stamp in $ANIKA_HOME is healed (removed) on boot
"""
...
def test_install_tree_not_writable_by_anika(
    built_image: str, container_name: str,
) -> None:
    """The anika user must not be able to modify /opt/anika.
    ...
    r = docker_exec_sh(
        container_name,
        # Try to create a file under /opt/anika as the anika user
        "touch /opt/anika/test_write 2>&1 && "
        "echo WRITE_SUCCEEDED || echo WRITE_FAILED",
        timeout=10,
    )
    assert "WRITE_FAILED" in r.stdout, (
        f"anika user can write to /opt/anika (install tree not immutable): "
        f"{r.stdout}"
    )
    ...
    r = docker_exec_sh(
        container_name,
        "touch /opt/anika/.venv/test_write 2>&1 && "
        "echo WRITE_SUCCEEDED || echo WRITE_FAILED",
        timeout=10,
    )
    assert "WRITE_FAILED" in r.stdout, (
        f"anika user can write to /opt/anika/.venv: {r.stdout}"
    )


def test_anika_disable_lazy_installs_and_dont_write_bytecode(
    built_image: str, container_name: str,
) -> None:
    """The container must set PYTHONDONTWRITEBYTECODE and
    ANIKA_DISABLE_LAZY_INSTALLS=1 so no .pyc files are written to the
    ...
    r = docker_exec_sh(
        ...
        'test "$ANIKA_DISABLE_LAZY_INSTALLS" = "1" && '
        'echo ENV_OK || echo ENV_MISSING',
        timeout=10,
    )
    assert "ENV_OK" in r.stdout, (
        f"expected PYTHONDONTWRITEBYTECODE=1 and "
        f"ANIKA_DISABLE_LAZY_INSTALLS=1, got: {r.stdout} stderr={r.stderr}"
    )
...
def test_stale_docker_stamp_in_home_is_healed_on_boot(
    ...
    """A stale 'docker' stamp left in $ANIKA_HOME by an older image
    ...
    assert "HEALED" in r.stdout or r.stdout.strip() != "docker", (
        f"stale 'docker' stamp in $ANIKA_HOME was not healed on boot: "
        f"{r.stdout}"
    )
