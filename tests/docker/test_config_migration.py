...
Build the real image and verify: a config.yaml present in $ANIKA_HOME
is migrated by docker_config_migrate.py on boot, running as the anika
user.
"""
...
def test_config_migration_runs_on_boot(
    ...
    """A config.yaml in $ANIKA_HOME must be migrated on boot by
    docker_config_migrate.py, running as the anika user."""
    # Start container
    ...
    assert "EXISTS" in r.stdout, (
        f"config.yaml not found in $ANIKA_HOME: {r.stdout}"
    )

    # Verify the migration script exists in the image
    r = docker_exec_sh(
        container_name,
        "test -f /opt/anika/scripts/docker_config_migrate.py && "
    ...
    # Verify config.yaml is owned by anika (migration ran as anika)
    r = docker_exec_sh(
        container_name,
        'stat -c "%U" /opt/data/config.yaml',
        timeout=10,
    )
    assert r.stdout.strip() == "anika", (
        f"config.yaml not owned by anika (migration may have run as root): "
        f"{r.stdout.strip()}"
    )
