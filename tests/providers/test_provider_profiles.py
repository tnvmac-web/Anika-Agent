...
class TestNousProfile:
    ...
    def test_extra_body_ignores_provider_preferences(self):
        """__NEW_ORG__ Portal rejects caller-supplied provider routing prefs (HTTP 400)."""
        p = get_provider_profile("nous")
        body = p.build_extra_body(
...
class TestAlibabaRegionalAndTokenPlanProfiles:
    ...
    def test_cn_variants_resolve_in_auth_registry(self, monkeypatch):
        ...
        anika_cli/auth.py:461-490) and raised
        "Unknown provider 'alibaba-coding-plan-cn'" (anika_cli/auth.py:1937)
