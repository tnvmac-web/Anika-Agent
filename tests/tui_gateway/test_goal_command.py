...
def anika_home(tmp_path, monkeypatch):
    ...
    # Bust the goal-module DB cache so it re-resolves ANIKA_HOME.
...
@pytest.fixture()
def server(anika_home, monkeypatch):
    ...
    # Pin config resolution to the isolated ANIKA_HOME. Sibling test
    ...
    # module-level ``_anika_home = get_anika_home()`` snapshot freezes
    ...
    monkeypatch.setattr(mod, "_anika_home", anika_home)
...
def test_moa_bare_returns_usage(server, session, anika_home):
    _write_moa_config(anika_home, """
moa:
