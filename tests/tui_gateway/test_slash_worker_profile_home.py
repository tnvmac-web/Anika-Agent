...
def test_slash_worker_accepts_profile_home():
    ...
    # anika_state evaluates get_anika_home() / "state.db" at import time, so
    # the mock must return a Path (a bare str raises TypeError under per-file
    # subprocess isolation).
    with patch.dict("sys.modules", {
        "anika_constants": MagicMock(
            get_anika_home=MagicMock(return_value=Path("/tmp/anika_test")),
        ),
    }):
        with patch("subprocess.Popen") as mock_popen:
            ...
            worker = _SlashWorker(
                ...
                model="test-model",
                profile_home="/home/luke/.anika/profiles/work"
            )

            # Verify Popen was called
            assert mock_popen.called

            # Check that ANIKA_HOME was set in the environment
            call_kwargs = mock_popen.call_args[1]
            assert "env" in call_kwargs
            assert call_kwargs["env"]["ANIKA_HOME"] == "/home/luke/.anika/profiles/work"
