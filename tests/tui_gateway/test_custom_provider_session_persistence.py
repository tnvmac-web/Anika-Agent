...
"No LLM provider configured. Run `anika model`..." (resume failed) or
...
def _make_agent_with_override(override, monkeypatch, config, model_cfg=None):
    ...
    # Keep credential-pool resolution off the developer's real ANIKA home.
...
# found" (the b200/anika-ultra-sft report). The stored MODEL NAME is the one
...
class TestModelNameRecoversEntryIdentity:
    def test_identity_by_model_from_providers_dict_models_list(self, monkeypatch):
        ...
        assert (
            rp.find_custom_provider_identity_by_model("anika-ultra-sft")
            == "custom:anika-ultra"
        )
...
# what left room bots stuck on a stale provider (e.g. "out of __NEW_ORG__ credits"
...
#
# Regression: GH #89497 (room bots hang then report "out of __NEW_ORG__ credits").
...
# "out of __NEW_ORG__ credits" after the profile was switched to ollama-cloud) while
