    ...
    with patch.dict(
        "sys.modules",
        {
            "anika_constants": MagicMock(
                get_anika_home=MagicMock(return_value="/tmp/anika_test_review_summary")
            ),
...
def test_init_session_attaches_background_review_callback(server, monkeypatch):
    ...
    cb("💾 Self-improvement review: Skill 'anika-release' patched")
    ...
    assert sid == "sid-abc"
    assert payload == {
        "text": "💾 Self-improvement review: Skill 'anika-release' patched"
    }
