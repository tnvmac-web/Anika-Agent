...
guard is covered in ``tests/anika_cli/test_picker_prewarm.py``):

- ``main()`` invokes ``anika_cli.model_switch_providers.prewarm_picker_cache_async``
...
import anika_cli.model_switch_providers
...
def _run_main(monkeypatch, events, *, prewarm=None):
    ...
    # entry.main() imports the helper lazily from anika_cli.model_switch,
