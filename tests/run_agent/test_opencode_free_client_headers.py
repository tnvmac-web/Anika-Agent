...
@patch("agent.process_bootstrap.OpenAI")
def test_opencode_free_sends_anika_attribution(mock_openai):
    """Keyless requests still identify as Anika (attribution headers match
    ...
    headers = _zen_call_headers(mock_openai)
    assert headers.get("X-Title") == "Anika Agent"
    assert str(headers.get("User-Agent", "")).startswith("AnikaAgent/")
