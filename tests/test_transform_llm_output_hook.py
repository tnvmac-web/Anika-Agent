...
from anika_cli.plugins import PluginManager, VALID_HOOKS


def _make_enabled_plugin(anika_home: Path, name: str, register_body: str) -> Path:
    """Create a plugin under <anika_home>/plugins/<name> and opt it in."""
    plugin_dir = anika_home / "plugins" / name
...
def test_no_plugins_returns_empty_results(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("ANIKA_HOME", str(tmp_path / "anika_empty"))
