"""Tests for anika_bootstrap — Windows UTF-8 stdio shim.

The bootstrap module is imported at the top of every Anika entry point
(anika, anika-agent, anika-acp, gateway, batch_runner, cli.py).  It
...
Key invariants covered by these tests:
  ...
  5. Load order: every Anika entry point imports anika_bootstrap as its
...
def _fresh_import():
    """Return a freshly-imported anika_bootstrap module.

    Drops any cached copy from sys.modules first so module-level code
    runs again and the platform check re-evaluates.
    """
    sys.modules.pop("anika_bootstrap", None)
    import anika_bootstrap  # noqa: WPS433
    return anika_bootstrap
...
class TestWindowsBehavior:
    ...
    def test_stdout_reconfigured_to_utf8_on_windows(self):
        # The live process's stdout should now be UTF-8 (the Anika CLI
...
class TestEntryPointsImportBootstrap:
    """Every Anika entry point must import anika_bootstrap as its
    ...
    # Entry points that invoke Anika as a process.  Each one must
    # import anika_bootstrap before doing any file I/O or stdout writes.
    ENTRY_POINTS = [
        "anika_cli/main.py",   # anika CLI (console_script)
        "run_agent.py",          # anika-agent (console_script)
        "acp_adapter/entry.py",  # anika-acp (console_script)
    ...
    def test_entry_point_imports_bootstrap(self, path):
        """The file must contain 'import anika_bootstrap' and that
        ...
        half-finished ``anika update`` (git-reset landed new code but
        ``uv pip install -e .`` didn't finish re-registering
        ``anika_bootstrap`` as a top-level module) leaves anika
        ...
        """
        # Resolve relative to the anika-agent repo root.  Tests live
        # at tests/test_anika_bootstrap.py, so go up one dir.
        import pathlib
        ...
        for node in ast.iter_child_nodes(tree):
            ...
            # anika start even when anika_bootstrap hasn't been
        ...
        assert first_import_name == "anika_bootstrap", (
            f"{path}: first top-level import is {first_import_name!r}, "
            f"but it must be 'anika_bootstrap' so UTF-8 stdio is "
            f"configured before anything else initializes.  Move the "
            f"'import anika_bootstrap' line to be the first import."
        )


class TestHardenImportPath:
    """harden_import_path() must keep a same-named package in the launch
    directory from shadowing Anika's own top-level modules — covering both
    the relative ('' / '.') and absolute-path forms the cwd can take on
    sys.path (issue #51286)."""

    def _run(self, hb, path_seed, env=None):
        original = sys.path[:]
        original_env = os.environ.get("ANIKA_PYTHON_SRC_ROOT")
        try:
            sys.path[:] = path_seed
            if env is not None:
                os.environ["ANIKA_PYTHON_SRC_ROOT"] = env
            elif "ANIKA_PYTHON_SRC_ROOT" in os.environ:
                del os.environ["ANIKA_PYTHON_SRC_ROOT"]
            hb.harden_import_path(src_root="/opt/anika")
            return sys.path[:]
        finally:
            sys.path[:] = original
            if original_env is None:
                os.environ.pop("ANIKA_PYTHON_SRC_ROOT", None)
            else:
                os.environ["ANIKA_PYTHON_SRC_ROOT"] = original_env

    def test_relative_cwd_forms_removed(self):
        hb = _fresh_import()
        result = self._run(hb, ["", ".", "/opt/anika", "/usr/lib/python"])
    ...
    def test_absolute_cwd_path_loses_to_src_root(self):
        ...
        # Anika root.  The guard must relocate Anika to the front.
        hb = _fresh_import()
        result = self._run(hb, ["/home/user/tg-ws-proxy", "/opt/anika"])
        assert result[0] == "/opt/anika"
        # The cwd absolute path may still appear (it can hold legit deps),
        # but only AFTER the Anika root.
        assert result.index("/opt/anika") < result.index("/home/user/tg-ws-proxy")
    ...
    def test_env_var_used_when_no_arg(self):
        ...
        try:
            ...
            os.environ["ANIKA_PYTHON_SRC_ROOT"] = "/env/anika"
            hb.harden_import_path()
            assert sys.path[0] == "/env/anika"
        finally:
            sys.path[:] = original
            if original_env is None:
                os.environ.pop("ANIKA_PYTHON_SRC_ROOT", None)
