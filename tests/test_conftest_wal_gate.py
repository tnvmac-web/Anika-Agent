"""The conftest WAL gate must agree with anika_state, and must not import it.

``tests/conftest.py::_wal_is_usable`` duplicates the SQLite WAL-reset version
predicate instead of importing ``anika_state``. That is deliberate: importing
``anika_state`` during collection caches ``DEFAULT_DB_PATH`` from the real
``~/.anika`` before the per-test ``ANIKA_HOME`` redirect, which makes tests
...
def test_conftest_gate_agrees_with_anika_state(version_info, monkeypatch):
    ...
    assert _wal_is_usable() is not is_sqlite_wal_reset_vulnerable(version_info), (
        f"conftest gate and anika_state disagree for SQLite {version_info}"
    )


def test_conftest_does_not_import_anika_state_at_collection():
    """The gate must stay import-free of anika_state.

    Importing it during collection caches DEFAULT_DB_PATH from the real
    ~/.anika, so tests read live production sessions instead of a tempdir.
    Reading the source is not an option here (banned), so assert on behavior:
    the gate must work with ``anika_state`` absent from ``sys.modules`` and
    ...
    def guard(name, *args, **kwargs):
        if name == "anika_state" or name.startswith("anika_state."):
            blocked.append(name)
            raise AssertionError(
                "conftest._wal_is_usable imported anika_state — this caches "
                "DEFAULT_DB_PATH from the real ~/.anika during collection"
            )
        return real_import(name, *args, **kwargs)

    saved = sys.modules.pop("anika_state", None)
