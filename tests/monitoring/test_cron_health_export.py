...
def test_registered_observable_metric_names_cover_snapshot_metrics(monkeypatch):
    ...
    gateway_snapshot = type("S", (), {"metrics": [
        _M("anika.gateway.up"), _M("anika.gateway.active_agents"),
        _M("anika.gateway.busy"), _M("anika.gateway.drainable"),
        _M("anika.gateway.restart_requested"),
        _M("anika.platform.up"), _M("anika.platform.degraded"),
    ]})()
    cron_snapshot = type("S", (), {"metrics": [
        _M("anika.cron.scheduler.heartbeat_age_seconds"),
        _M("anika.cron.scheduler.last_success_age_seconds"),
        _M("anika.cron.scheduler.catch_up_occurrences"),
        _M("anika.cron.jobs.enabled"), _M("anika.cron.jobs.running"),
        _M("anika.cron.jobs.overdue"),
    ]})()
...
def test_monitoring_docs_distinguish_relay_health_scope_and_terminal_flush():
    ...
    assert "Anika Agent-owned Relay transport health" in text
