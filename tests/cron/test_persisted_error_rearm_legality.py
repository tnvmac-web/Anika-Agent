...
def cron_store(tmp_path, monkeypatch):
    ...
    (anika_home / "cron").mkdir(parents=True)
    monkeypatch.setenv("ANIKA_HOME", str(anika_home))
    monkeypatch.setattr(J, "ANIKA_DIR", anika_home)
    monkeypatch.setattr(J, "CRON_DIR", anika_home / "cron")
    monkeypatch.setattr(J, "JOBS_FILE", anika_home / "cron" / "jobs.json")
    monkeypatch.setattr(J, "OUTPUT_DIR", anika_home / "cron" / "output")
...
class TestCronRearmRespectsScheduleLegality:
    def test_weekday_job_errored_friday_does_not_fire_on_saturday(self, cron_store):
        ...
        with mock.patch.object(J, "_anika_now", lambda: now):
            due = J.get_due_jobs()
