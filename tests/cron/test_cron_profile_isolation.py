...
profile's ANIKA_HOME and EXECUTE under it.

- Storage: a job created under profile ``coder`` writes to
  ``~/.anika/profiles/coder/cron/jobs.json`` — NOT the shared default root.
- Execution: the profile-scoped gateway's in-process ticker resolves the
  active ANIKA_HOME (profile home) at call time, so jobs run with that
...
one store and runs them under whatever ANIKA_HOME the ticker happens to have —
...
def _set_profile_env(monkeypatch, root: Path, profile_home: Path) -> None:
    ...
    ANIKA_HOME is a profile under it (``<root>/profiles/<name>``)."""
    import anika_constants

    monkeypatch.setattr(
        anika_constants, "_get_platform_default_anika_home", lambda: root
    )
    monkeypatch.setenv("ANIKA_HOME", str(profile_home))


def test_cron_storage_anchors_at_profile_home(tmp_path, monkeypatch):
    """Under a profile ANIKA_HOME (<root>/profiles/<name>), the cron store
    ...
    assert anika_constants.get_anika_home().resolve() == profile_home.resolve()
    assert anika_constants.get_default_anika_root().resolve() == root.resolve()

    # cron/jobs.py computes ANIKA_DIR from get_anika_home() at import, so a
    # fresh import under this env anchors the store at <profile>/cron.
    import cron.jobs as jobs

    importlib.reload(jobs)
    try:
        assert jobs.ANIKA_DIR.resolve() == profile_home.resolve()
        assert (
