...
Older versions of ``install.sh`` created ``$command_link_dir/anika`` as a
symlink to the pip-generated entry point at ``$ANIKA_BIN`` (i.e.
``venv/bin/anika``). When ``setup_path()`` later switched to writing a bash
shim with ``cat > "$command_link_dir/anika" <<EOF``, the redirect followed
the existing symlink and overwrote the pip entry point with the shim. The
shim's ``exec "$ANIKA_BIN" "$@"`` then self-recursed and ``anika`` hung on
every invocation.

These tests pin the fix: ``setup_path()`` must remove ``$command_link_dir/anika``
...
def _extract_setup_path_shim_block() -> str:
    ...
    match = re.search(
        r"(?P<block>mkdir -p \"\$command_link_dir\".*?chmod \+x \"\$command_link_dir/anika\")",
        text,
        re.DOTALL,
    )
...
def test_setup_path_shim_block_removes_old_link_before_writing() -> None:
    ...
    rm_idx = block.find('rm -f "$command_link_dir/anika"')
    cat_idx = block.find('cat > "$command_link_dir/anika" <<EOF')
    assert rm_idx != -1, (
        "setup_path() must `rm -f` $command_link_dir/anika before the "
...
def test_re_running_setup_path_block_preserves_pip_entry_point(tmp_path: Path) -> None:
    ...
    Layout mirrors a real install:
        ...
        tmp/
          venv/bin/anika        <- pip entry point (the one we must preserve)
          local_bin/anika       <- symlink → ../venv/bin/anika  (old install)

    Then we run the exact shim-write block from setup_path() with
    ``ANIKA_BIN`` and ``command_link_dir`` pointed at this fixture. The fix
    requires that, after the run:

      * ``venv/bin/anika`` still contains its original pip-script body
      * ``local_bin/anika`` is a regular file (not a symlink) holding the shim
    """
    venv_bin = tmp_path / "venv" / "bin"
    venv_bin.mkdir(parents=True)
    pip_entry = venv_bin / "anika"
    ...
    shim_path = command_link_dir / "anika"
    ...
    assert pip_entry.read_text() == pip_marker, (
        "venv/bin/anika was overwritten by setup_path() — symlink-stomp "
        "regression (#21454)."
    )
    ...
    assert not shim_path.is_symlink(), (
        "command_link_dir/anika must be replaced with a regular file, not "
