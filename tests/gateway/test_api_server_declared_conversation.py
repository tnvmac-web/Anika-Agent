...
``X-Anika-Session-Key``.  Every conversation-affinity hint Anika sends is
derived from that physical id — ``prompt_cache_key`` on both OpenAI-wire
transports, the OpenRouter/__NEW_ORG__ sticky ``session_id``, and xAI's
...
def _headers(session_key=None):
    ...
    if session_key:
        h["X-Anika-Session-Key"] = session_key
    return h
