...
async def test_run_agent_binds_api_session_context_for_tool_env(adapter, monkeypatch):
    ...
    monkeypatch.setenv("ANIKA_SESSION_ID", "stale-session")
    observed = {}
    ...
    class FakeAgent:
        ...
        def run_conversation(self, user_message, conversation_history, task_id):
            ...
            observed["context_session_id"] = get_session_env("ANIKA_SESSION_ID")
            observed["context_platform"] = get_session_env("ANIKA_SESSION_PLATFORM")
            observed["context_session_key"] = get_session_env("ANIKA_SESSION_KEY")
            observed["child_session_id"] = _make_run_env({}).get("ANIKA_SESSION_ID")
...
async def test_session_chat_treats_pre_existing_poisoned_row_as_no_model(session_db):
    ...
    virtual model alias (e.g. "anika-agent") persisted literally as its
...
async def test_session_model_lock_endpoint_then_chat_reuses_persisted_lock_and_provider_credentials(
    ...
    assert captured["base_url"] == "https://__NEW_ORG__.example/v1"
