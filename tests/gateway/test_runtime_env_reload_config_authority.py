...
ANIKA_MAX_ITERATIONS, but a later per-turn load_dotenv(..., override=True)
can restore a stale .env ANIKA_MAX_ITERATIONS value before the next turn.
"""
...
def test_reload_runtime_env_preserves_config_terminal_backend(
    ...
    ``anika config get terminal.backend`` still says local.
    """
    anika_home = tmp_path / ".anika"
    anika_home.mkdir()
    (anika_home / "config.yaml").write_text(
    ...
    (anika_home / ".env").write_text("TERMINAL_ENV=docker\n", encoding="utf-8")
