...
class TestDMTopicFallbackReplyToMode:
    ...
    Regression tests for https://github.com/__NEW_ORG__/anika-agent/issues/23994:
    reply_to_mode 'off' was ignored when sending via Anika-created DM topic
...
class TestDMTopicSyntheticSendRouting:
    ...
    Regression tests for https://github.com/__NEW_ORG__/anika-agent/issues/87051:
    ...
    response in a different chat lane than the Anika topic the session runs
    in. The no-anchor branch must prefer the Anika topic's
    ``message_thread_id`` whenever it resolves.
    """

    def test_no_anchor_prefers_anika_topic_thread_id(self):
