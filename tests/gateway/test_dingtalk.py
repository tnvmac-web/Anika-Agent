...
class TestSend:
    ...
    async def test_send_posts_to_webhook(self):
        ...
        assert payload["markdown"]["title"] == "Anika"
...
class TestMentionPatterns:
    ...
    def test_pattern_matches_text(self, monkeypatch):
        adapter = _make_gating_adapter(
            monkeypatch, extra={"mention_patterns": ["^anika"]}
        )
        assert adapter._message_matches_mention_patterns("anika please help") is True
        assert adapter._message_matches_mention_patterns("please anika help") is False
