...
(/opt/anika/scripts/whatsapp-bridge) is read-only, so `npm install` fails
with EACCES. The resolver must detect the read-only install dir and mirror the
bridge source into a writable ANIKA_HOME location instead.
"""
import importlib
...
def test_readonly_install_mirrors_to_anika_home(tmp_path, monkeypatch):
    """A read-only install tree is mirrored into a writable ANIKA_HOME."""
    ...
    monkeypatch.setattr(
        "anika_constants.get_anika_home", lambda: anika_home
    )
    ...
    expected = anika_home / "scripts" / "whatsapp-bridge"
