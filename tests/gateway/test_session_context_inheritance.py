...
variant that the ``_UNSET``-strip guard does NOT catch:
    ...
    *concurrent* message A had ALREADY called ``set_session_vars``, B inherits
    ...
    ``ANIKA_SESSION_*`` identity through the subprocess-env bridge. The bridge's
...
def _spawn_view():
    ...
    env = _make_run_env({})
    return {
        "ANIKA_SESSION_CHAT_ID": env.get("ANIKA_SESSION_CHAT_ID"),
        "ANIKA_SESSION_THREAD_ID": env.get("ANIKA_SESSION_THREAD_ID"),
        "ANIKA_SESSION_KEY": env.get("ANIKA_SESSION_KEY"),
    }
...
def test_reset_session_vars_closes_inheritance_leak():
    ...
    for var in ("ANIKA_SESSION_CHAT_ID", "ANIKA_SESSION_THREAD_ID", "ANIKA_SESSION_KEY"):
    ...
    assert captured["bound"]["ANIKA_SESSION_CHAT_ID"] == "FOREIGN_CHAT"
    assert captured["bound"]["ANIKA_SESSION_KEY"] == FOREIGN["session_key"]
...
# flag read via ``async_delivery_supported()``, not a string ``ANIKA_SESSION_*``
