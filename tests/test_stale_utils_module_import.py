...
Real incident (gateway session 1518671026962174144)::
    ...
    cannot import name 'env_float' from 'utils' (~/.anika/anika-agent/utils.py)
...
2. ``anika update`` ran ``git pull``, updating ``utils.py`` (now defining
...
``anika_cli/main.py`` (the ``anika update`` flow, ~line 9326) already
acknowledges this exact hazard -- "source files on disk are newer than cached
Python modules in this process" -- and reloads ``anika_constants`` after the
