...
class TestStatusBarSegment:
    ...
    def test_text_renderer_includes_the_badge_at_every_width_tier(self, width):
        ...
        host = AnikaCLI.__new__(AnikaCLI)
        host.model = "opus"
        ...
        with patch.object(AnikaCLI, "_get_status_bar_snapshot", return_value=snapshot), \
             patch.object(AnikaCLI, "_is_session_yolo_active", return_value=False):
            text = AnikaCLI._build_status_bar_text(host, width=width)
