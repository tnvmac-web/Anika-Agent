...
def test_frontmatter_required_fields():
    ...
    anika = fm["metadata"]["anika"]
    assert anika["tags"]
    assert "related_skills" in anika
...
def test_author_credits_human_first():
    fm, _ = _frontmatter_and_body()
    assert not fm["author"].startswith("Anika Agent"), "human contributor must be credited first"
...
def test_related_skills_resolve_in_repo():
    ...
    for name in fm["metadata"]["anika"]["related_skills"]:
        hits = (
