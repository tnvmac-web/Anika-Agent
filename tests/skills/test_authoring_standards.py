...
def test_required_frontmatter_fields(p):
    ...
    anika = (fm.get("metadata") or {}).get("anika") or {}
    if not (anika.get("tags") or fm.get("tags")) and not _grandfathered(p, "tags"):
        pytest.fail(f"{_rel(p)}: no tags (metadata.anika.tags or top-level tags)")
...
def test_related_skills_resolve(p):
    ...
    anika = (fm.get("metadata") or {}).get("anika") or {}
    dangling = [
        rs for rs in (anika.get("related_skills") or []) if rs not in _all_names()
    ]
