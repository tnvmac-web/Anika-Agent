"""The `anika-agent` skill is what a running Anika knows about itself.

`website/` is never packaged, so an installed Anika has no local copy of the
user guide; skills ARE synced into `$ANIKA_HOME/skills/`. The skill therefore
...
shipped feature is invisible and the agent answers "Anika can't do that."
"""
...
SKILL_DIR = REPO / "skills" / "autonomous-ai-agents" / "anika-agent"
