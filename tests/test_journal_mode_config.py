...
def _reset_configured_delete_override_warned_paths():
    ...
    anika_state_wal._delete_overridden_warned_paths.clear()
    yield
    anika_state_wal._delete_overridden_warned_paths.clear()
...
def test_resolve_journal_mode_uses_real_database_config(monkeypatch, tmp_path):
    from anika_state_wal import resolve_journal_mode

    _configure_mode(monkeypatch, tmp_path, "DELETE")
    assert resolve_journal_mode() == "delete"


def test_new_nonsecret_anika_env_override_is_not_exposed(monkeypatch, tmp_path):
    from anika_state_wal import resolve_journal_mode

    _configure_mode(monkeypatch, tmp_path, "wal")
    monkeypatch.setenv("ANIKA_JOURNAL_MODE", "delete")
...
def test_real_db_openers_honor_configured_delete(monkeypatch, tmp_path):
    ...
    from anika_cli import kanban_db, projects_db
    ...
    discord = DiscordRecoveryStore(anika_home=tmp_path)
