...
- ``anika verify`` records into the evidence ledger (pass and fail),
- a passing run satisfies the verify-on-stop guard,
- the verify-on-stop nudge names ``anika verify --json`` when the workspace
...
def _workspace(tmp_path, *, scripts=None, manifest_recipe=None):
    ...
    if manifest_recipe is not None:
        anika_dir = project / ".anika"
        anika_dir.mkdir()
        (anika_dir / "environment.json").write_text(
...
def test_record_verify_run_marks_workspace_passed(anika_home):
    project = _workspace(anika_home)
    ...
    assert status["evidence"]["canonical_command"] == "anika verify"


def test_record_verify_run_records_failure(anika_home):
    project = _workspace(anika_home)
...
def test_cli_passing_run_writes_ledger_evidence(anika_home, capsys):
    project = _workspace(anika_home, manifest_recipe={"name": "Fake", "test": ["echo ok"]})
...
def test_cli_failing_run_writes_failed_evidence(anika_home, capsys):
    project = _workspace(anika_home, manifest_recipe={"name": "Fake", "test": ["false"]})
    code = run_verify_command(make_args(project))
    assert code == 1
    status = verification_status(session_id=None, cwd=project)
    assert status["status"] == "failed"
...
def test_cli_partial_run_records_targeted_scope(anika_home, capsys):
...
def test_cli_run_uses_anika_session_id_env(anika_home, capsys, monkeypatch):
    monkeypatch.setenv("ANIKA_SESSION_ID", "sess-42")
    project = _workspace(anika_home, manifest_recipe={"name": "Fake", "test": ["echo ok"]})
...
# ---------------------------------------------------------------------------
# closed loop: edit -> stop guard nudge -> anika verify -> guard satisfied
# ---------------------------------------------------------------------------


def test_passing_verify_run_satisfies_stop_guard(anika_home, capsys):
    project = _workspace(anika_home, manifest_recipe={"name": "Fake", "test": ["echo ok"]})
...
# ---------------------------------------------------------------------------
# nudge wording: recipe-aware `anika verify --json` suggestion
# ---------------------------------------------------------------------------


def test_nudge_mentions_anika_verify_when_recipe_has_start(anika_home):
    project = _workspace(anika_home, scripts={"test": "vitest", "dev": "vite"})
    changed = str(project / "src" / "app.ts")
    ...
    assert "anika verify --json" in nudge
    # The cheap verify commands are still listed first.
    assert "npm run test" in nudge


def test_nudge_mentions_anika_verify_when_manifest_exists(anika_home):
    # No start script, but a saved .anika/environment.json qualifies.
    project = _workspace(
        anika_home,
        scripts={"test": "vitest"},
...
def test_nudge_keeps_plain_wording_without_recipe_start(anika_home):
    # Verify commands but no start script and no manifest: today's wording.
    project = _workspace(anika_home, scripts={"test": "vitest"})
    changed = str(project / "src" / "app.ts")
    mark_workspace_edited(session_id="s1", cwd=project, paths=[changed])
    nudge = build_verify_on_stop_nudge(session_id="s1", changed_paths=[changed])
    assert nudge is not None
    assert "anika verify" not in nudge


def test_nudge_recipe_detection_failure_is_silent(anika_home, monkeypatch):
...
def test_detect_path_merges_project_facts_commands(anika_home, capsys):
    project = _workspace(anika_home)  # package.json with no scripts
...
def test_manifest_recipe_is_not_merged(anika_home, capsys):
...
def test_merge_skips_commands_recipe_already_has(anika_home, capsys):
    project = _workspace(anika_home, scripts={"test": "vitest"})
    code = run_verify_command(make_args(project, detect_only=True))
    assert code == 0
    payload = json.loads(capsys.readouterr().out)
