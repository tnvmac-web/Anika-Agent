"""Windows installer regression for Anika children outside the venv.

The venv sweep deliberately selects process roots by executable path so it
does not kill unrelated Python processes. A selected Anika process can spawn
...
def test_venv_sweep_stops_managed_runtime_children_but_not_unrelated_processes(
    ...
    runtime_dir = install_dir / ".anika-runtime" / "python" / "generation-test"
    ...
    # Keep the test away from real gateway tasks and real Anika launchers
    ...
    wrapper.write_text(
        ...
        f"-InstallDir '{install_dir}' -AnikaHome '{anika_home}'\n"
        "exit $LASTEXITCODE\n",
        encoding="utf-8",
    )
