"""Shared fixtures for the anika-agent test suite.
...
2. **Isolated ANIKA_HOME.** ANIKA_HOME points to a per-test tempdir so
   code reading ``~/.anika/*`` via ``get_anika_home()`` can't see the
   real one. (We do NOT also redirect HOME — that broke subprocesses in
   CI. Code using ``Path.home() / ".anika"`` instead of the canonical
   ``get_anika_home()`` is a bug to fix at the callsite.)
3. **Deterministic runtime.** TZ=UTC, LANG=C.UTF-8, PYTHONHASHSEED=0.
4. **No ANIKA_SESSION_* inheritance** — the agent's current gateway
...
# ── Sandbox ANIKA_HOME before ANY test module is imported ──────────────────
# `anika_cli/main.py` calls `setup_logging()` at MODULE level, which resolves
# `get_anika_home()` and attaches rotating file handlers to the ROOT logger.
...
# real `~/.anika/logs/agent.log` and `errors.log`.
#
# The `_isolate_env` fixture below also sandboxes ANIKA_HOME, but fixtures run
...
# the REAL Anika root — capture it BEFORE the sandbox rewires ANIKA_HOME,
# otherwise the deny-list would point at the throwaway tempdir and the guard
# would silently stop protecting the operator's actual ~/.anika (#69385).
_PRE_SANDBOX_KANBAN_OVERRIDE = os.environ.get("ANIKA_KANBAN_HOME", "").strip()
_PRE_SANDBOX_ANIKA_HOME = os.environ.get("ANIKA_HOME", "")


def _anika_home_points_at_production(value: str) -> bool:
    """True when a pre-set ANIKA_HOME resolves to the real production root.

    Gateway-launched shells (and developer shells that ``export
    ANIKA_HOME=~/.anika``) hand pytest the PRODUCTION home. Historically
    the session sandbox below honored any pre-set value, so collection-time
    imports (logging handlers, ``anika_state.DEFAULT_DB_PATH``) froze paths
    inside the real ``~/.anika`` — the escape vector that landed pytest
    ...
    ANIKA_HOME is honored now.
    """
    if not value:
        return True
    try:
        resolved = Path(value).expanduser().resolve()
        real_root = (Path.home() / ".anika").resolve()
    except Exception:
        return True
...
if _anika_home_points_at_production(os.environ.get("ANIKA_HOME", "")):
    _SESSION_ANIKA_HOME = tempfile.mkdtemp(prefix="anika-test-home-")
    os.environ["ANIKA_HOME"] = _SESSION_ANIKA_HOME
    atexit.register(shutil.rmtree, _SESSION_ANIKA_HOME, True)
...
# like a real CLI") — which used to disarm anika_state's live-DB guard in
# the child at the same moment the child lost the ANIKA_HOME redirect.
# ANIKA_TEST_ISOLATION is OUR marker: exported here (before any test module
# imports), inherited by every child by default, and honored by
# anika_state_guard._running_under_pytest() as a test-context signal. A child
...
# open a real DB must export ANIKA_STATE_DB_GUARD_BYPASS=1 in that child's
# env instead of stripping markers.
os.environ["ANIKA_TEST_ISOLATION"] = os.environ.get("ANIKA_HOME", "") or "1"

#: ANIKA_HOME as it stood when conftest was imported - i.e. before any test
...
ANIKA_HOME_AT_CONFTEST_IMPORT = os.environ.get("ANIKA_HOME", "")
...
# ANIKA_* vars that change test behavior by being set. Unset all of these
# unconditionally — individual tests that need them set do so explicitly.
_ANIKA_BEHAVIORAL_VARS = frozenset({
    ...
    # final response text to ``anika_cli.voice.speak_text`` — real synthesis,
    ...
    # any test run launched FROM a Anika agent session inherits them and
    # anika_constants home-resolution helpers prefer them over monkeypatched
    ...
    "ANIKA_MAX_ITERATIONS",
    "ANIKA_SESSION_PLATFORM",
    "ANIKA_SESSION_CHAT_ID",
    "ANIKA_SESSION_CHAT_NAME",
    "ANIKA_SESSION_CHAT_TYPE",
    "ANIKA_SESSION_THREAD_ID",
    "ANIKA_SESSION_SOURCE",
    "ANIKA_SESSION_KEY",
    "ANIKA_GATEWAY_SESSION",
    "ANIKA_CRON_SESSION",
    "_ANIKA_GATEWAY",
    "ANIKA_PLATFORM",
    "ANIKA_MODEL",
    "ANIKA_INFERENCE_MODEL",
    "ANIKA_INFERENCE_PROVIDER",
    "ANIKA_TUI_PROVIDER",
    "ANIKA_MANAGED",
    "ANIKA_MANAGED_DIR",
    "ANIKA_DEV",
    "ANIKA_CONTAINER",
    "ANIKA_EPHEMERAL_SYSTEM_PROMPT",
    "ANIKA_TIMEZONE",
    "ANIKA_REDACT_SECRETS",
    "ANIKA_BACKGROUND_NOTIFICATIONS",
    "ANIKA_EXEC_ASK",
    "ANIKA_HOME_MODE",
    "ANIKA_AGENT_USE_LEGACY_SESSION_KEYS",
    # Kanban path/board pins must never leak from a developer shell or
    # dispatched worker into tests; otherwise tests can write fake tasks to
    # the real ~/.anika/kanban.db instead of the per-test ANIKA_HOME.
    "ANIKA_KANBAN_DB",
    "ANIKA_KANBAN_BOARD",
    "ANIKA_KANBAN_HOME",
    "ANIKA_KANBAN_WORKSPACES_ROOT",
    "ANIKA_KANBAN_LOGS_ROOT",
    "ANIKA_KANBAN_TASK",
    "ANIKA_KANBAN_WORKSPACE",
    "ANIKA_KANBAN_RUN_ID",
    "ANIKA_KANBAN_CLAIM_LOCK",
    "ANIKA_KANBAN_DISPATCH_IN_GATEWAY",
    ...
    # otherwise-unrelated config tests away from the default "anika" host.
    "ANIKA_HONCHO_HOST",
    ...
    "ANIKA_DASHBOARD_OAUTH_CLIENT_ID",
    "ANIKA_DASHBOARD_PORTAL_URL",
    "TERMINAL_CWD",
    "TERMINAL_ENV",
...
def _hermetic_environment(tmp_path, monkeypatch):
    ...
    Also redirects HOME and ANIKA_HOME to per-test tempdirs so code that
    reads ``~/.anika/*`` can't touch the real one, and pins TZ/LANG so
    ...
    # 2. Blank behavioral ANIKA_* vars that could change test semantics.
    for name in _ANIKA_BEHAVIORAL_VARS:
        monkeypatch.delenv(name, raising=False)
    ...
    monkeypatch.setenv("ANIKA_HONCHO_HOST", "anika")

    # 3. Redirect ANIKA_HOME to a per-test tempdir. Code that reads
    #    ``~/.anika/*`` via ``get_anika_home()`` now gets the tempdir.
    #
    #    NOTE: We do NOT also redirect HOME. Doing so broke CI because
    #    some tests (and their transitive deps) spawn subprocesses that
    #    inherit HOME and expect it to be stable. If a test genuinely
    #    needs HOME isolated, it should set it explicitly in its own
    #    fixture. Any code in the codebase reading ``~/.anika/*`` via
    #    ``Path.home() / ".anika"`` instead of ``get_anika_home()``
    #    is a bug to fix at the callsite.
    fake_anika_home = tmp_path / "anika_test"
    fake_anika_home.mkdir()
    (fake_anika_home / "sessions").mkdir()
    (fake_anika_home / "cron").mkdir()
    (fake_anika_home / "memories").mkdir()
    (fake_anika_home / "skills").mkdir()
    monkeypatch.setenv("ANIKA_HOME", str(fake_anika_home))
    ...
    # anika_state's live-DB guard stays armed in them even when the test
    # strips pytest's own PYTEST_* vars from the child env.
    monkeypatch.setenv("ANIKA_TEST_ISOLATION", str(fake_anika_home))
    # And never let a developer-shell (or leaked child) bypass disarm the
    # guard for in-process code under test.
    monkeypatch.delenv("ANIKA_STATE_DB_GUARD_BYPASS", raising=False)

    # 3b. anika_state computes ``DEFAULT_DB_PATH = get_anika_home() / "state.db"``
    #     at import time. When the module is first imported at collection (any
    #     test file with a top-level ``from anika_state import ...``) that
    ...
    anika_state_mod = sys.modules.get("anika_state")
    if anika_state_mod is not None and hasattr(anika_state_mod, "DEFAULT_DB_PATH"):
        monkeypatch.setattr(
            anika_state_mod, "DEFAULT_DB_PATH", fake_anika_home / "state.db"
        )
    ...
    monkeypatch.setenv("ANIKA_DISABLE_LAZY_INSTALLS", "1")

    # 5. Reset plugin singleton so tests don't leak plugins from
    #    ~/.anika/plugins/ (which, per step 3, is now empty — but the
    #    singleton might still be cached from a previous test).
    try:
        import anika_cli.plugins as _plugins_mod
        ...
        # previous test's tmp_path ANIKA_HOME can't leak forward. Paths
...
@pytest.fixture(autouse=True)
def _isolate_anika_home(_hermetic_environment):
...
def _neutralize_kanban_memory_guard(request, monkeypatch):
    ...
    if request.node.get_closest_marker("real_memory_guard"):
        return
    try:
        from anika_cli import kanban_db_dispatch as _kbd_mod
    except Exception:
        return
...
# invocation), kanban writes silently pollute the real ~/.anika. This autouse
...
# allow-list because test-level fixtures legitimately move ANIKA_HOME to
...
def _capture_real_kanban_root() -> Path:
    ...
    file (before the session ANIKA_HOME sandbox rewired the env), so the
    deny-list keeps pointing at the operator's actual root. Mirrors
    ``kanban_db.kanban_home()`` resolution order:
    1. ``ANIKA_KANBAN_HOME`` env var when set and non-empty
    2. the real (pre-sandbox) Anika root otherwise
    """
    if _PRE_SANDBOX_KANBAN_OVERRIDE:
        return Path(_PRE_SANDBOX_KANBAN_OVERRIDE).expanduser().resolve()
    if _PRE_SANDBOX_ANIKA_HOME and not _anika_home_points_at_production(
        _PRE_SANDBOX_ANIKA_HOME
    ):
        # ANIKA_HOME was genuinely set to a CUSTOM root before the sandbox
        ...
        return get_default_anika_root().resolve()
    # No pre-existing ANIKA_HOME: the real root is the platform default,
    # NOT the sandbox tempdir now sitting in the env.
    return (Path.home() / ".anika").resolve()
...
def _kanban_write_guard(_hermetic_environment, monkeypatch):
    ...
    ``~/.anika`` captured at import time. Hermetic tests that legitimately
    move ANIKA_HOME to sibling tempdirs are unaffected.

    Only patches when ``anika_cli.kanban_db_connect`` is *already imported*
    ...
    """
    _kdb = sys.modules.get("anika_cli.kanban_db")
    _kdbc = sys.modules.get("anika_cli.kanban_db_connect")
    if _kdb is None or _kdbc is None:
        return

    # The sys.modules probe can observe the module MID-IMPORT: a fixture
    # boundary firing while another test's lazy `import anika_cli.kanban_db`
...
# ``anika_state._ensure_test_isolation`` (the single choke point every
# ``SessionDB()`` construction goes through) refuses, under pytest, any DB
# path that resolves inside the REAL Anika root. This fixture wires the
# test-side knobs:
...
#     installs where ANIKA_HOME is not ~/.anika) into the guard's
...
# so subprocess children that import anika_state directly are covered even
# without this fixture.


@pytest.fixture(autouse=True)
def _state_db_write_guard(request, monkeypatch):
    _hs = sys.modules.get("anika_state")
    ...
    if _PRE_SANDBOX_ANIKA_HOME and not _anika_home_points_at_production(
        ...
        extra_roots.append(
            Path(_PRE_SANDBOX_ANIKA_HOME).expanduser().resolve()
        )
    monkeypatch.setattr(
...
def _reset_tui_gateway_server_state():
    ...
    # A leaked context-local Anika home override redirects every later
    # ``get_anika_home()`` call (active-session registry, config paths)
    # to a stale per-test tmpdir. Force the main-thread ContextVar back
    # to its default.
    try:
        from anika_constants import get_anika_home_override, set_anika_home_override

        if get_anika_home_override() is not None:
            set_anika_home_override(None)
    except Exception:
        pass
...
@pytest.fixture()
def mock_config():
    """Return a minimal anika config dict suitable for unit tests."""
    return {
...
# environment and finds the developer's live ``anika-gateway`` process
...
# correlated with a ``tests/anika_cli/`` pytest run starting up.
#
...
#    ``check_output`` reject any ``systemctl ... <verb> anika-gateway``
...
def _wal_is_usable() -> bool:
    """True when Anika will actually put a database into WAL mode here.

    Anika refuses journal_mode=WAL on SQLite builds carrying the upstream
    ...
    running Anika can link DIFFERENT SQLite versions: a repo ``.venv`` on
    3.50.4 (vulnerable → DELETE) alongside a Anika managed runtime on 3.53.1
    (fixed → WAL). The same test then passes in one and fails in the other.

    IMPORTANT: this must NOT import ``anika_state``. That module computes
    ``DEFAULT_DB_PATH`` from ``get_anika_home()`` at import time, so importing
    it during collection — before the per-test ``_isolate_anika_home`` fixture
    redirects ``ANIKA_HOME`` — permanently caches the DEVELOPER'S REAL
    ``~/.anika/state.db`` for the whole session. Tests then read live
    production sessions instead of a tempdir. The version predicate is
    duplicated from ``anika_state._is_sqlite_wal_reset_vulnerable`` (upstream
...
#      process environment: ``os.environ["ANIKA_VOICE_TTS"] = "1"``. The
...
#      ``anika_cli.voice.speak_text(final_response)`` on a daemon thread.
...
# to mock it:
#
#  • ``anika_cli.voice.speak_text`` — the synth+playback entry point both
#    gateway call sites late-import, so patching the module attribute catches
#    them wherever they import it from.
#  • ``anika_cli.voice.play_audio_file`` — the module-level binding
#    ``speak_text`` actually plays through. Patching the binding inside
#    ``anika_cli.voice`` (not ``tools.voice_mode``) keeps the real function
...
# OS gating
#
# Anika runs on Linux, macOS and native Windows, and a lot of its behaviour
# genuinely differs per host: PTY vs pywinpty, taskkill vs SIGTERM, launchd
# vs systemd, Keychain vs libsecret, ``%LOCALAPPDATA%`` vs ``~/.anika``.
#
...
def pytest_configure(config):  # noqa: D401 — pytest hook
    ...
    config.addinivalue_line(
        ...
        "SQLite WAL mode; skipped on builds where Anika falls back to "
...
def pytest_collection_modifyitems(config, items):  # noqa: D401 — pytest hook
    ...
    reason = (
        f"SQLite {sqlite3.sqlite_version} has the WAL-reset bug — Anika uses "
...
def _live_system_guard(request, monkeypatch):
    ...
    tokens[0]), so ``bash -c "systemctl restart anika-gateway"``,
    ``sudo systemctl ...``, ``env systemctl ...``, ``setsid systemctl ...``
    are all caught. ``pkill``/``killall``/``taskkill`` invocations
    targeting anika/python patterns are also blocked.
    """
    ...
    def _matches_anika_gateway(cmd_str: str) -> bool:
        low = cmd_str.lower()
        return any(tok in low for tok in _ANIKA_TOKENS)

    def _is_blocked_systemctl(cmd) -> bool:
        cmd_str = _cmd_to_string(cmd)
        if "systemctl" not in cmd_str:
            return False
        if not _matches_anika_gateway(cmd_str):
            return False
        try:
    ...
    def _is_process_killer(cmd) -> bool:
        ...
        for tok in killer_tokens:
            ...
            if head in _PROCESS_KILLERS:
                low = cmd_str.lower()
                # pkill -f pattern: catch anika-themed patterns + a
                # plain "python" -f which would catch the live gateway
                # whose cmdline contains "python -m anika_cli.main".
                if (
                    "anika" in low
                    or "gateway" in low
    ...
    def _check_subprocess_cmd(name, cmd):
        if _is_blocked_systemctl(cmd):
            raise RuntimeError(
                ...
                "live anika-gateway systemd unit. Mock "
        ...
        if _is_process_killer(cmd):
            raise RuntimeError(
                ...
                "targeting anika/python could hit the live gateway. "
                "Mark with @pytest.mark.live_system_guard_bypass if "
                "intentional."
            )
        # Block any subprocess that would run `anika update` (or the
        # equivalent `python -m anika_cli.main update`).  These commands
        ...
        if "update" in low and (
            # anika update / anika update --gateway / setsid bash -c ... anika update
            ("anika" in low and "update" in low.split())
            or
            # python -m anika_cli.main update --gateway
            ("anika_cli" in low and "update" in low.split())
            or
            # venv/bin/anika update  (absolute path variant used in tests)
            (".venv/bin/anika" in low and "update" in low)
        ):
            raise RuntimeError(
                f"tests/conftest.py live-system guard: blocked "
                f"subprocess.{name}({cmd!r}) — this command would run "
                "`anika update` against the real checkout, fetching "
...
def _audio_playback_guard(request, monkeypatch):
    ...
    depth behind ``_ANIKA_BEHAVIORAL_VARS``: the env blanking stops the flag
    ...
    try:
        import anika_cli.voice as _voice
    except Exception:
