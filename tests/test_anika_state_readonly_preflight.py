...
- files inside the Anika home tree are repaired with ``chmod u+rw``
...
import anika_state
from anika_state import SessionDB, preflight_db_writability
...
def anika_home(tmp_path, monkeypatch):
    """Isolated ANIKA_HOME so the repair scope covers tmp DBs."""
    home = tmp_path / ".anika"
    home.mkdir()
...
class TestRepairScope:
    def test_repairs_readonly_db_inside_home(self, anika_home):
        db = anika_home / "state.db"
        _make_db(db)
        os.chmod(db, 0o444)
    ...
    def test_repairs_readonly_sidecars(self, anika_home):
        db = anika_home / "state.db"
        _make_wal_db(db)
    ...
    def test_repairs_readonly_parent_directory(self, anika_home):
        sub = anika_home / "kanban"
        sub.mkdir()
        db = sub / "kanban.db"
        _make_db(db)
        os.chmod(sub, 0o555)
...
class TestRefusalOutsideScope:
    def test_actionable_error_names_file_and_chmod(self, anika_home, tmp_path):
    ...
    def test_wal_error_warns_against_deletion(self, anika_home, tmp_path):
        outside = tmp_path / "elsewhere"
        outside.mkdir()
        db = outside / "custom.db"
        _make_wal_db(db)
...
class TestSkips:
    ...
    def test_healthy_db_untouched(self, anika_home):
        db = anika_home / "state.db"
        _make_db(db)
...
class TestSessionDBIntegration:
    def test_sessiondb_selfheals_readonly_db_in_home(self, anika_home):
        db_path = anika_home / "state.db"
