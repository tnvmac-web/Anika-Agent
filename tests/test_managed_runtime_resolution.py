"""Guard: Anika-owned subprocesses must not resolve managed runtimes by bare PATH.

Anika installs runtimes for itself — ``uv`` at ``$ANIKA_HOME/bin/uv``, Node at
``$ANIKA_HOME/node``. Neither directory is on the ambient PATH of an arbitrary
process, so ``shutil.which("uv")`` / ``shutil.which("node")`` in Anika's own
...
* when a system copy also exists, the one Anika does not own wins — which is
...
``iter_anika_node_dirs()``, ``resolve_uv()``, or ``ensure_uv()``. This test is
...
# Runtimes Anika provisions into ANIKA_HOME and must therefore resolve
# through a managed-aware helper rather than PATH.
_MANAGED_COMMANDS = frozenset({"uv", "node", "npm", "npx"})

# Directories that are not Anika-owned subprocess code: plugins ship their own
...
_ALLOWED: dict[tuple[str, str], str] = {
    ...
    ("anika_cli/update_cmd_deps.py", "uv"): (
    ...
    ("anika_cli/update_cmd_deps.py", "npm"): (
...
def _source_files() -> list[Path]:
    ...
    # (anika_agent-<version>/) gets created and deleted concurrently, and
...
def test_no_unreviewed_bare_managed_runtime_lookups():
    ...
    assert not unexpected, (
        "Bare PATH lookup for a Anika-managed runtime.\n\n"
        + "\n".join(f"  {rel}:{lineno}  which({cmd!r})" for rel, cmd, lineno in unexpected)
        + "\n\n$ANIKA_HOME/bin (uv) and $ANIKA_HOME/node are not on an "
        ...
        "  node/npm -> anika_constants.find_node_executable()\n"
        "  PATH env -> anika_constants.iter_anika_node_dirs()\n"
...
def test_managed_node_helpers_exist(helper):
    ...
    assert callable(getattr(anika_constants, helper))


def test_managed_uv_helpers_exist():
    from anika_cli.managed_uv import ensure_uv, managed_uv_path, resolve_uv
