...
from anika_cli.profiles import _get_default_anika_home
...
class TestHonchoClientConfigDefaults:
    def test_default_values(self):
        ...
        assert config.host == "anika"
        assert config.workspace_id == "anika"
...
class TestFromGlobalConfig:
    ...
    def test_recall_mode_from_config(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "key",
            "recallMode": "tools",
            "hosts": {"anika": {"recallMode": "context"}},
        }))
        config = HonchoClientConfig.from_global_config(config_path=config_file)
    ...
    def test_base_url_host_block_overrides_root_and_env(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            ...
            "hosts": {"anika": {"baseUrl": "http://host-block:9001"}},
        }))
    ...
    def test_base_url_full_precedence_chain(self, tmp_path):
        ...
        config_file = tmp_path / "config.json"
        layers = {
            "hosts": {"anika": {"baseUrl": "http://host:1"}},
...
class TestResolveSessionName:
    ...
    def test_per_repo_uses_git_root(self):
        ...
        with patch.object(
            HonchoClientConfig, "_git_repo_name", return_value="anika-agent"
        ):
            result = config.resolve_session_name("/home/user/anika-agent/subdir")
        assert result == "anika-agent"


class TestResolveConfigPath:
    def test_prefers_anika_home_when_exists(self, tmp_path):
        anika_home = tmp_path / "anika"
        anika_home.mkdir()
        local_cfg = anika_home / "honcho.json"
    ...
    def test_falls_back_to_default_profile_when_no_local(self, tmp_path, monkeypatch):
        # Profile mode: ANIKA_HOME points at ~/.anika/profiles/<name>, so
        # _get_default_anika_home() must resolve back to ~/.anika — that's
        # the bug the HOME-anchored helper fixes (vs. blindly using Path.home()).
        fake_home = tmp_path / "fakehome"
        fake_home.mkdir()
        default_home = fake_home / ".anika"
        ...
        assert _get_default_anika_home() == default_home
        assert result == default_cfg


class TestResolveActiveHost:
    def test_profile_host_key_uses_honcho_safe_separator(self):
        assert profile_host_key("coder") == "anika_coder"
        assert profile_host_key("default") == "anika"


    def test_explicit_env_var_wins(self):
        with patch.dict(os.environ, {"ANIKA_HONCHO_HOST": "anika.coder"}):
            assert resolve_active_host() == "anika.coder"
    ...
    def test_profiles_import_failure_falls_back(self):
        ...
        with patch.dict(os.environ, {}, clear=False), patch(
            ...
            os.environ.pop("ANIKA_HONCHO_HOST", None)
            # Temporarily remove anika_cli.profiles to simulate import failure
            saved = sys.modules.get("anika_cli.profiles")
            sys.modules["anika_cli.profiles"] = None  # type: ignore
            try:
                assert resolve_active_host() == "anika"
            finally:
                if saved is not None:
                    sys.modules["anika_cli.profiles"] = saved
                else:
                    sys.modules.pop("anika_cli.profiles", None)


class TestProfileScopedConfig:
    def test_from_env_uses_profile_host(self):
        with patch.dict(os.environ, {"HONCHO_API_KEY": "key"}):
            config = HonchoClientConfig.from_env(host="anika_coder")
        assert config.host == "anika_coder"
        assert config.workspace_id == "anika"  # shared workspace
        assert config.ai_peer == "anika_coder"
...
class TestObservationModeMigration:
    ...
    def test_existing_config_defaults_to_unified(self, tmp_path):
        ...
        cfg_file.write_text(json.dumps({
            "apiKey": "k",
            "hosts": {"anika": {"enabled": True, "aiPeer": "anika"}},
        }))
...
class TestGetHonchoClient:
    ...
    def test_dot_form_legacy_host_key_keeps_local_api_key(self):
        """Regression for #37436: a legacy dot-form host block (anika.work)
        ...
        cfg = HonchoClientConfig(
            ...
            host="anika_work",
            workspace_id="anika",
            raw={"hosts": {"anika.work": {"apiKey": "explicit-local-key"}}},
        )
    ...
    def test_timeout_change_triggers_client_rebuild(self):
        ...
        cfg_yaml = get_anika_home() / "config.yaml"
    ...
    def test_managed_config_timeout_does_not_thrash_singleton(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("ANIKA_MANAGED_DIR", str(managed_dir))
...
class TestGetHonchoClientBaseUrlDoublePrefixFix:
    ...
    def test_lan_default_host_empty_key_uses_local_placeholder(self, tmp_path):
        ...
        with patch.dict(os.environ, {}, clear=True), \
             patch("anika_cli.profiles.get_active_profile_name", return_value="default"), \
