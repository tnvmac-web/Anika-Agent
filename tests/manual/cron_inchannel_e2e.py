...
Uses a throwaway ANIKA_HOME so it never touches ~/.anika. No real names.
"""
...
def _fresh_home():
    """Point ANIKA_HOME at a throwaway dir BEFORE importing gateway modules
    (mirror.py binds _SESSIONS_INDEX from get_anika_home() at import time)."""
    d = tempfile.mkdtemp(prefix="cron_inchannel_e2e_")
    os.environ["ANIKA_HOME"] = d
    return Path(d)


HOME = _fresh_home()

# Import AFTER ANIKA_HOME is set.
...
# a different get_anika_home() at import if something imported it earlier).
...
def main():
    ...
    print(f"ANIKA_HOME (throwaway): {HOME}")
