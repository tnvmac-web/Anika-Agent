...
The ``anika update`` Python path already clears the conflict with ``git reset``
...
def test_install_ps1_stops_venv_resident_processes_before_parking_venv() -> None:
    ...
    ``venv\\Scripts\\pythonw.exe -m anika_cli.main gateway run`` — image name
    ``pythonw``, not ``anika.exe`` — so the ``taskkill /IM anika.exe`` guard
    ...
    # The anika.exe tree-kill is preserved (kills spawned child processes too).
    assert 'taskkill /F /T /IM anika.exe' in text
