...
CASES = {
    ...
    "product python → nix": (["anika_cli/config.py"], _lanes(python=True, scan=True)),
    ...
    # files change — not on every anika_state.py PR.
    ...
    "python source alone → no desktop_updater lane": (["anika_state.py"], _lanes(python=True, scan=True)),
...
def test_pull_request_changed_files_skips_non_pr_events(monkeypatch):
    ...
    monkeypatch.setenv("REPO", "__NEW_ORG__/anika-agent")
