...
class TestToolsetConsistency:
    ...
    def test_anika_platforms_share_core_tools(self):
        """All anika-* platform toolsets share the same core tools.

        Platform-specific additions (e.g. ``discord`` / ``discord_admin``
        on anika-discord, gated on DISCORD_BOT_TOKEN) are allowed on top —
        the invariant is that the core set is identical across platforms.
        """
        platforms = ["anika-cli", "anika-telegram", "anika-discord", "anika-whatsapp", "anika-slack", "anika-signal", "anika-homeassistant"]
...
class TestDefaultPlatformWebSearchCoverage:
    def test_anika_whatsapp_toolset_includes_web_search(self):
        assert "web_search" in resolve_toolset("anika-whatsapp")
...
class TestResolveToolsetMemo:
    ...
    def test_second_resolution_is_cached(self, monkeypatch):
        ...
        first = resolve_toolset("anika-cli")
        second = resolve_toolset("anika-cli")
        ...
        assert (
            "anika-cli", True, registry_id, generation
    ...
    def test_memo_result_matches_fresh_resolution(self):
        ...
        toolsets_mod._resolve_toolset_memo.clear()
        first = resolve_toolset("anika-cli", include_registry=False)
        second = resolve_toolset("anika-cli", include_registry=False)
        assert first == second
