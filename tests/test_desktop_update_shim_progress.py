...
def progress(tmp_path):
    ...
    status = tmp_path / "anika-update-status"
...
# Stands in for `anika update`, and reports the stage that was on screen
...
n="$(cat "$ANIKA_TEST_CALLS" 2>/dev/null || echo 0)"; n=$((n + 1))
printf '%s' "$n" > "$ANIKA_TEST_CALLS"
for f in "$TMPDIR"/anika-update-status.[0-9]*; do
  case "$f" in *.tmp) continue ;; esac
  cp "$f" "$ANIKA_TEST_CAPTURE.$n" 2>/dev/null
done
exit "$(cat "$ANIKA_TEST_EXITS.$n" 2>/dev/null || echo 0)"
"""
...
def _run_handoff(tmp_path, exits: dict[int, int]) -> list[dict]:
    ...
    install_root = tmp_path / "anika-agent"
    (install_root / "venv" / "bin").mkdir(parents=True)
    anika = install_root / "venv" / "bin" / "anika"
    anika.write_text(FAKE_ANIKA)
    anika.chmod(0o755)
    ...
    env = {
        **os.environ,
        "TMPDIR": str(tmp_path),
        "ANIKA_TEST_CAPTURE": str(capture),
        "ANIKA_TEST_CALLS": str(calls),
        "ANIKA_TEST_EXITS": str(tmp_path / "exits"),
    }
    ...
    result = tmp_path / ".anika-update-result.json"
...
def test_update_gate_publishes_its_stage_before_running(tmp_path):
    """`anika update` is the longest wait in the hand-off and the one the
