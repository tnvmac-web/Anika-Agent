"""Unit tests for the __NEW_ORG__ Portal profile's reasoning wiring.
...
@pytest.fixture
def nous_profile():
    """Resolve the registered __NEW_ORG__ profile through the real discovery path."""
    # ``model_tools`` triggers plugin discovery on import, which is what
    # registers the __NEW_ORG__ profile in the global provider registry.
...
class TestNousReasoningWireShape:
    ...
    def test_disable_dropped_for_non_reasoning_route(self, nous_profile, portal_catalog):
        ...
        Anika' own ``supports_reasoning`` can disagree with the Portal about a
