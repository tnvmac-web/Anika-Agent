...
def test_gated_api_rejects_opaque_bearer_with_401_not_503(_gated_nous):
    r = _gated___NEW_ORG__.get("/api/auth/me", headers={"Authorization": f"Bearer {OPAQUE_PEER_KEY}"})
...
def test_gated_api_rejects_opaque_cookie_with_401_not_503(_gated_nous):
    _gated___NEW_ORG__.cookies.set(SESSION_AT_COOKIE, OPAQUE_PEER_KEY)
    r = _gated___NEW_ORG__.get("/api/auth/me")
    assert r.status_code != 503, r.text
    assert "unreachable" not in r.text.lower()
