...
class TestGenerate:
    ...
    def test_aspect_ratio_mapping(self):
        """Anika 'square' must map to Krea '1:1' in the wire payload."""
        from plugins.image_gen.krea import KreaImageGenProvider
...
# ---------------------------------------------------------------------------
# Managed __NEW_ORG__ gateway path
# ---------------------------------------------------------------------------
...
class TestManagedGateway:
    def test_managed_submit_uses_gateway_origin_and_nous_token(self, monkeypatch):
        """Managed mode submits to the gateway origin with the __NEW_ORG__ token."""
        ...
        # Poll is bound to the same gateway + __NEW_ORG__ token.
        poll_url = mock_get.call_args[0][0]
