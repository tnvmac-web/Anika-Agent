...
_RUNTIME = "anika_cli.runtime_provider.resolve_runtime_provider"
...
class TestProviderClass:
    ...
    def test_display_names(self):
        ...
        assert by_name["nous"].display_name == "__NEW_ORG__ Portal"
    ...
    def test_nous_honors_top_level_model(self):
        ...
        with patch("plugins.image_gen.openrouter._load_image_gen_config", return_value=cfg):
            assert __NEW_ORG__._resolve_model_chain() == ["openai/gpt-image-2"]
...
class TestLiveCatalog:
    ...
    def test_nous_portal_picker_excludes_image_api_catalog(self):
        """__NEW_ORG__ Portal has no /images route; its picker must not offer
        Image-API-only models it cannot serve."""
        from plugins.image_gen.openrouter import _build_providers

        nous = {p.name: p for p in _build_providers()}["nous"]
        with patch(_RUNTIME, side_effect=RuntimeError("no creds")):
            ids = [m["id"] for m in __NEW_ORG__.list_models()]
        from plugins.image_gen.openrouter import DEFAULT_MODEL, _FALLBACK_MODEL
...
class TestGenerate:
    ...
    def test_posts_to_resolved_base_url(self):
        """__NEW_ORG__ routes to its own base URL — proves the same code serves both."""
        nous_runtime = _runtime_ok(
            provider="nous", base_url="https://inference.__NEW_DOMAIN__/v1", api_key="nous-tok"
        )
        with patch(_RUNTIME, return_value=nous_runtime), \
            ...
            nous = {p.name: p for p in _build_providers()}["nous"]
            result = __NEW_ORG__.generate(prompt="a pet")

        assert result["success"] is True
        assert result["provider"] == "nous"
        url = mock_post.call_args[0][0]
        assert url == "https://inference.__NEW_DOMAIN__/v1/chat/completions"
...
class TestImageApiSurface:
    ...
    def test_nous_never_uses_the_image_api(self):
        """__NEW_ORG__ Portal proxies chat-completions and has no /images route."""
        from plugins.image_gen.openrouter import _build_providers
        ...
        with patch(_RUNTIME, return_value=nous_runtime), \
             ...
             patch("plugins.image_gen.openrouter.save_b64_image", return_value=Path("/tmp/x.png")):
            nous = {p.name: p for p in _build_providers()}["nous"]
            result = __NEW_ORG__.generate(prompt="a pet", model="openai/gpt-image-2")

        assert result["success"] is True
        assert mock_post.call_args[0][0] == "https://inference.__NEW_DOMAIN__/v1/chat/completions"
