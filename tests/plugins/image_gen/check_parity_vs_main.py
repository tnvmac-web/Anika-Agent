...
# worktree), ``MAIN_DIR`` falls back to a sibling ``anika-agent-main``
...
def _resolve_main_dir() -> Path:
    ...
    if (candidate / "tools" / "image_generation_tool.py").exists() and candidate != REPO_ROOT:
        return candidate
    sibling = REPO_ROOT.parent / "anika-agent-main"
...
assert (PR_DIR / "tools" / "image_generation_tool.py").exists(), (
    f"PR_DIR={PR_DIR} doesn't look like a anika-agent checkout"
)
...
# Isolated ANIKA_HOME so the config write is hermetic.
...
SCENARIOS: list[tuple[str, str, dict[str, str]]] = [
    ...
    (
        ...
        "",
        {
            "TOOL_GATEWAY_DOMAIN": "__NEW_DOMAIN__",
...
def main() -> int:
    ...
    if MAIN_DIR == PR_DIR:
        print(
            ...
            "      Set up a sibling 'anika-agent-main' checkout pinned to "
