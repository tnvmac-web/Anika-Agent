...
Run from the PR worktree:
    ...
    cd ~/.anika/anika-agent/.worktrees/browser-providers-plugin
...
# checkout lives two levels up at ``~/.anika/anika-agent``.
MAIN_DIR = REPO_ROOT.parent.parent  # ~/.anika/anika-agent
PR_DIR = REPO_ROOT  # the worktree we're in
assert (MAIN_DIR / "tools" / "browser_tool.py").exists(), (
    f"MAIN_DIR={MAIN_DIR} doesn't look like a anika-agent checkout"
)
...
# Isolated ANIKA_HOME for the config write.
