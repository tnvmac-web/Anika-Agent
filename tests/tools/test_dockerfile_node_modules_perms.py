...
Older images made /opt/anika/ui-tui and /opt/anika/node_modules writable so a
...
def test_dockerfile_uses_prebuilt_tui_instead_of_writable_node_modules() -> None:
    ...
    assert "ENV ANIKA_TUI_DIR=/opt/anika/ui-tui" in text
    assert "cd ../ui-tui && npm run build" in text
    assert "chown -R anika:anika /opt/anika/ui-tui" not in text
    assert "chown -R anika:anika /opt/anika/node_modules" not in text
