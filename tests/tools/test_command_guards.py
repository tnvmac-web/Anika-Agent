...
def _mode_manual(monkeypatch):
    ...
    The test conftest redirects ANIKA_HOME to an empty tempdir, so the
...
def _clean_state():
    ...
    saved = {}
    for k in ("ANIKA_INTERACTIVE", "ANIKA_GATEWAY_SESSION", "ANIKA_EXEC_ASK", "ANIKA_YOLO_MODE"):
        if k in os.environ:
...
class TestTirithWarnSafe:
    ...
    def test_warn_session_approved(self, mock_tirith):
        os.environ["ANIKA_INTERACTIVE"] = "1"
        session_key = os.getenv("ANIKA_SESSION_KEY", "default")
    ...
    def test_warn_non_interactive_auto_allow(self, mock_tirith):
        # No ANIKA_INTERACTIVE or ANIKA_GATEWAY_SESSION set
        result = check_all_command_guards("curl https://bit.ly/abc", "local")
        assert result["approved"] is True
