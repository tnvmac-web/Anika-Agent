...
Real config files in a temp ANIKA_HOME, real resolution chains (no mocked
...
def voice_home(tmp_path, monkeypatch):
    """Isolated ANIKA_HOME + reloaded config modules; yields a config writer."""
    home = tmp_path / ".anika"
    home.mkdir()
    ...
    for var in (
        ...
        "DEEPINFRA_API_KEY", "ANIKA_LOCAL_STT_LANGUAGE",
    ):
