...
Covers:
- :class:`AnikaTokenStorage` ``.meta.json`` roundtrip (save / load / remove)
- The production manager provider
  (:class:`tools.mcp_oauth_manager.AnikaMCPOAuthProvider`) restoring metadata
...
from tools.mcp_oauth_manager import _ANIKA_PROVIDER_CLS
...
# ---------------------------------------------------------------------------
# AnikaTokenStorage metadata roundtrip
# ---------------------------------------------------------------------------
...
class TestMetadataStorage:
    def test_save_and_load_roundtrip(self, tmp_path, monkeypatch):
        ...
        storage = AnikaTokenStorage("example-server")
    ...
    def test_remove_deletes_meta_file(self, tmp_path, monkeypatch):
        monkeypatch.setenv("ANIKA_HOME", str(tmp_path))
        storage = AnikaTokenStorage("cleanup-server")
...
# ---------------------------------------------------------------------------
# Manager-path provider (AnikaMCPOAuthProvider) — production code path
# ---------------------------------------------------------------------------


def _manager_provider_with_context(storage: AnikaTokenStorage, **context_attrs):
    ...
    provider = _ANIKA_PROVIDER_CLS.__new__(_ANIKA_PROVIDER_CLS)
    provider._anika_server_name = context_attrs.get("server_name", "srv")
    context = MagicMock()
...
class TestManagerOAuthProviderMetadata:
    def test_initialize_restores_metadata_from_disk(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("ANIKA_HOME", str(tmp_path))
        storage = AnikaTokenStorage("mgr-srv")
        ...
        with patch.object(
            _ANIKA_PROVIDER_CLS.__bases__[-1], "_initialize", new=AsyncMock()
        ):
    ...
    def test_async_auth_flow_persists_on_completion(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("ANIKA_HOME", str(tmp_path))
        storage = AnikaTokenStorage("flow-srv")
        ...
        with patch.object(
            _ANIKA_PROVIDER_CLS.__bases__[-1],
            "async_auth_flow",
