...
class TestApprovalModeParsing:
    ...
    def test_config_bool_false_maps_to_off(self):
        with mock_patch("anika_cli.config.load_config_readonly", return_value={"approvals": {"mode": False}}):
...
class TestSmartApproval:
    ...
    def test_smart_approval_does_not_allowlist_the_pattern_for_session(self, monkeypatch):
        ...
        monkeypatch.setenv("ANIKA_SESSION_KEY", session_key)
        monkeypatch.setenv("ANIKA_EXEC_ASK", "1")
        monkeypatch.delenv("ANIKA_CRON_SESSION", raising=False)
        monkeypatch.setattr(
...
class TestDetectDangerousRm:
    ...
    def test_nonrecursive_verification_artifact_cleanup_is_not_dangerous(self):
        with mock_patch("tempfile.gettempdir", return_value="/tmp"):
            for prefix in ("anika-verify-", "anika-ad-hoc-"):
    ...
    def test_symlinked_temp_dir_only_exempts_canonical_target(self, tmp_path):
        ...
        basename = "anika-verify-example.py"
    ...
    def test_verification_cleanup_exemption_rejects_broader_deletions(self):
        commands = (
            "rm -rf /tmp/anika-verify-example.py",
            "rm -f /tmp/anika-verify-example.py /tmp/other.py",
            "rm -f /tmp/nested/../anika-verify-example.py",
            "rm -f /tmp/a/../../tmp/anika-verify-example.py",
            "rm -f /var/tmp/anika-verify-example.py",
            "rm -f /tmp/anika-verify-*",
            "rm -f /tmp/anika-verify-$(touch>/tmp/pwned).py",
            "rm -f /tmp/anika-ad-hoc-`touch>/tmp/pwned`.py",
            "rm -f /tmp/anika-verify-example.py; touch /tmp/pwned",
        )
        with mock_patch("tempfile.gettempdir", return_value="/tmp"):
...
class TestWindowsShellDestructiveCommands:
    def test_windows_destructive_requires_approval(self):
        cases = [
            (r"cmd /c del /f /q C:\tmp\anika-victim\file.txt", "Windows cmd destructive delete"),
            (r"cmd.exe /k rmdir /s /q C:\tmp\anika-victim", "Windows cmd destructive delete"),
            ...
            (r"powershell Remove-Item -Recurse -Force C:\tmp\anika-victim",
...
class TestSessionKeyContext:
    def test_context_session_key_overrides_process_env(self):
        ...
        try:
            with mock_patch.dict("os.environ", {"ANIKA_SESSION_KEY": "bob"}, clear=False):
...
class TestTeePattern:
    ...
    def test_tee_to_sensitive_target(self):
        for command in (
            ...
            "echo x | tee /dev/sda",
            "echo x | tee ~/.anika/.env",
            "echo x | tee $ANIKA_HOME/.env",
            'echo x | tee "$ANIKA_HOME/.env"',
        ):
            dangerous, key, desc = detect_dangerous_command(command)
            assert dangerous is True, command
...
class TestAnikaConfigWriteProtection:
    """Terminal-side pairing for the file_tools write_file/patch deny on
    ~/.anika/config.yaml (#14639). config.yaml IS the security policy
    ...
    def test_write_idioms_against_config(self):
        for command in (
            "echo 'approvals:' > ~/.anika/config.yaml",
            "echo '  mode: off' >> ~/.anika/config.yaml",
            "echo x | tee ~/.anika/config.yaml",
            "echo x | tee $ANIKA_HOME/config.yaml",
            "cp /tmp/evil.yaml ~/.anika/config.yaml",
        ):
            dangerous, key, desc = detect_dangerous_command(command)
            assert dangerous is True, command
            assert key is not None, command
    ...
    def test_reads_and_unrelated_writes_are_safe(self):
        # Reading config is not a write; a non-Anika absolute config.yaml is
        # handled by the project patterns, not the Anika-home rule.
        for cmd in (
            "cat ~/.anika/config.yaml",
...
class TestSensitiveCopyMovePattern:
    ...
    shell rc files, or ~/.anika/config.yaml/.env must require approval — the
...
class TestWindowsAbsolutePathFolding:
    """Windows absolute home / Anika-home prefixes must fold to ~/ and
    ~/.anika/ in dangerous-command detection.
    ...
    SSH, and Anika config/env files returned "safe" without an approval prompt.
    The OS-specific ``Path.home()`` / ``get_anika_home()`` tests above only
    exercise this branch on a Windows host; these monkeypatch a Windows-style
    HOME/ANIKA_HOME so the fold is verified on the POSIX CI runner too."""
...
class TestSmartDeniedPrompt:
    ...
    def test_smart_deny_uses_locale_specific_once_deny_choices(self, monkeypatch, capsys):
        monkeypatch.setenv("ANIKA_LANGUAGE", "tr")
...
class TestGatewayProtection:
    ...
    def test_gateway_run_backgrounded_detected(self):
        cmd = "kill 1605 && cd ~/.anika/anika-agent && source venv/bin/activate && python -m anika_cli.main gateway run --replace &disown; echo done"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "systemctl" in desc
        for variant in (
            "python -m anika_cli.main gateway run --replace &",
            "nohup python -m anika_cli.main gateway run --replace",
        ):
    ...
    def test_systemctl_restart_flagged(self):
        ...
        cmd = "systemctl --user restart anika-gateway"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
...
class TestWebhookApprovalExclusion:
    ...
    def test_webhook_platform_returns_false(self, monkeypatch):
        ...
        monkeypatch.setenv("ANIKA_SESSION_PLATFORM", "webhook")
    ...
    def test_all_unattended_platforms_return_false(self, monkeypatch):
        ...
        for platform in _UNATTENDED_APPROVAL_PLATFORMS:
            monkeypatch.setenv("ANIKA_SESSION_PLATFORM", platform)
    ...
    def test_non_webhook_gateway_session_returns_true(self, monkeypatch):
        ...
        monkeypatch.delenv("ANIKA_CRON_SESSION", raising=False)
        monkeypatch.setenv("ANIKA_GATEWAY_SESSION", "1")
        monkeypatch.setenv("ANIKA_SESSION_PLATFORM", "telegram")
    ...
    def test_cron_session_returns_false_regardless_of_platform(self, monkeypatch):
        ...
        monkeypatch.setenv("ANIKA_CRON_SESSION", "1")
        monkeypatch.setenv("ANIKA_SESSION_PLATFORM", "telegram")
    ...
    def test_webhook_dangerous_command_denies_by_default(self, monkeypatch):
        ...
        monkeypatch.setenv("ANIKA_SESSION_PLATFORM", "webhook")
        monkeypatch.setenv("ANIKA_SESSION_KEY", "test-webhook-session")
    ...
    def test_api_server_dangerous_command_denies_by_default(self, monkeypatch):
        ...
        monkeypatch.delenv("ANIKA_GATEWAY_SESSION", raising=False)
        monkeypatch.delenv("ANIKA_INTERACTIVE", raising=False)
        monkeypatch.setenv("ANIKA_SESSION_PLATFORM", "api_server")
        monkeypatch.setenv("ANIKA_SESSION_KEY", "test-api-session")
...
class TestIFSWhitespaceBypass:
    ...
    def test_ifs_forms_still_flagged_dangerous(self):
        for cmd in (
            ...
            # In-place edit of the Anika security config via IFS.
            "sed${IFS}-i ~/.anika/config.yaml",
        ):
            dangerous, key, desc = detect_dangerous_command(cmd)
...
class TestPgrepKillExpansion:
    """kill -9 $(pgrep anika) bypasses the pkill/killall name-matching
    ...
    def test_kill_pidof_expansion_detected(self):
        """`kill $(pidof anika)` is the BSD/Linux equivalent of the
        pgrep expansion and bypasses the pkill/killall name pattern
        in the same way. See issue #33071."""
        dangerous, _, desc = detect_dangerous_command("kill -TERM $(pidof anika_cli.main)")
        assert dangerous is True
        assert "pidof" in desc.lower() or "pgrep" in desc.lower()
        assert detect_dangerous_command("kill -9 `pidof anika`")[0] is True
...
class TestLaunchctlGatewayLifecycle:
    """launchctl stop/kickstart/bootout/unload against the Anika service
    label achieves the same effect as `anika gateway stop|restart` and
    must require the same approval. See issue #33071.
    """

    def test_launchctl_against_anika_label_detected(self):
        for cmd in (
            "launchctl stop ai.anika.gateway",
            "launchctl kickstart -k system/ai.anika.gateway",
            "launchctl bootout system/ai.anika.gateway",
            "launchctl unload ~/Library/LaunchAgents/ai.anika.gateway.plist",
        ):
            dangerous, _, desc = detect_dangerous_command(cmd)
            assert dangerous is True, cmd

    def test_unrelated_labels_not_flagged(self):
        """Read-only inspection, and lifecycle ops on non-Anika labels, are
    ...
    def test_spliced_detection_does_not_flag_prose_or_other_services(self):
        ...
        a anika-gateway identifier, so quoted prose and non-gateway anika
    ...
    def test_label_built_before_verb_detected(self):
        ...
        "anika"/"ai.anika" to appear AFTER the verb and missed this
...
class TestApprovalTimeoutIsNotConsent:
    ...
    def setup_method(self):
        ...
        self._saved_env = {
            k: os.environ.get(k)
            for k in ("ANIKA_GATEWAY_SESSION", "ANIKA_CRON_SESSION",
                      "ANIKA_YOLO_MODE",
                      "ANIKA_SESSION_KEY", "ANIKA_INTERACTIVE")
        }
        os.environ.pop("ANIKA_YOLO_MODE", None)
        os.environ.pop("ANIKA_INTERACTIVE", None)
        # ANIKA_CRON_SESSION takes priority over ANIKA_GATEWAY_SESSION in
        ...
        os.environ.pop("ANIKA_CRON_SESSION", None)
        os.environ["ANIKA_GATEWAY_SESSION"] = "1"
        os.environ["ANIKA_SESSION_KEY"] = self.SESSION_KEY
    ...
    * ``tirith_fail_open: false`` → surface a Tirith-style warning through
    ...
    def test_import_error_allows_when_fail_open_or_disabled(self, enabled, fail_open):
        ...
        with _patch("builtins.__import__", side_effect=self._make_failing_import(real_import)):
            with _patch("anika_cli.config.load_config_readonly", return_value=cfg):
                with _patch("tools.approval.detect_dangerous_command", return_value=(False, None, None)):
                    with mock_patch.dict("os.environ", {"ANIKA_INTERACTIVE": "1"}, clear=False):
...
GATEWAY_LIFECYCLE_LAUNCHCTL = (
    ...
    "launchctl unload ~/Library/LaunchAgents/ai.anika.gateway.plist",
    "launchctl load ~/Library/LaunchAgents/ai.anika.gateway.plist",
    "launchctl stop ai.anika.gateway",
    "launchctl restart ai.anika.gateway",
    "launchctl bootout gui/501/ai.anika.gateway",
    "launchctl remove ai.anika.gateway",
    "launchctl disable gui/501/ai.anika.gateway",
)
...
class TestLifecycleGuardLaunchctlParity:
    ...
    under ``_ANIKA_GATEWAY == "1"``, the ``cron.lifecycle_guard`` block is
    ...
    def test_unrelated_labels_are_not_blocked(self):
        ...
        services, including other Anika ones, stay runnable."""
        from cron.lifecycle_guard import contains_gateway_lifecycle_command

        for cmd in (
            "launchctl bootout gui/501/com.example.unrelated",
            "launchctl remove ai.anika.update-checker",
