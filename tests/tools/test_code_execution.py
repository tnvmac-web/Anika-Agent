...
UDS socket lifecycle, anika_tools generation, timeout enforcement,
...
class TestAnikaToolsGeneration(unittest.TestCase):
    ...
    def test_file_transport_uses_tempfile_fallback_for_rpc_dir(self):
        src = generate_anika_tools_module(["terminal"], transport="file")
        self.assertIn("import json, os, shlex, tempfile, threading, time", src)
        self.assertIn("os.path.join(tempfile.gettempdir(), \"anika_rpc\")", src)
        self.assertNotIn('os.environ.get("ANIKA_RPC_DIR", "/tmp/anika_rpc")', src)
    ...
    def test_uds_transport_serializes_concurrent_calls(self):
        ...
        src = generate_anika_tools_module(["terminal"], transport="uds")
...
class TestExecuteCodeRemoteTempDir(unittest.TestCase):
    def test_execute_remote_uses_backend_temp_dir_for_sandbox(self):
        ...
        mkdir_cmd = next(cmd for cmd, _, _ in env.commands
                         if "mkdir -p" in cmd and "anika_exec_" in cmd)
        run_cmd = next(cmd for cmd, _, _ in env.commands if "python3 script.py" in cmd)
        cleanup_cmd = next(cmd for cmd, _, _ in env.commands
                           if "rm -rf" in cmd and "anika_exec_" in cmd)
        self.assertIn("mkdir -p /data/data/com.termux/files/usr/tmp/anika_exec_", mkdir_cmd)
        self.assertIn("ANIKA_RPC_DIR=/data/data/com.termux/files/usr/tmp/anika_exec_", run_cmd)
        self.assertIn("rm -rf /data/data/com.termux/files/usr/tmp/anika_exec_", cleanup_cmd)
        self.assertNotIn("mkdir -p /tmp/anika_exec_", mkdir_cmd)

    def test_timezone_shell_quoted_in_remote_execution(self):
        """ANIKA_TIMEZONE must be shell-quoted in remote env_prefix to prevent injection."""
        class FakeEnv:
            def __init__(self):
                self.commands = []
        ...
        with patch("tools.code_execution_tool._load_config",
             ...
             patch.dict(os.environ, {"ANIKA_TIMEZONE": malicious_tz}):
            result = json.loads(_execute_remote("print('hello')", "task-1", ["terminal"]))
...
class TestExecuteCode(unittest.TestCase):
    ...
    def test_repo_root_modules_are_importable(self):
        ...
        result = self._run('import anika_constants; print(anika_constants.__file__)')
        self.assertEqual(result["status"], "success")
        self.assertIn("anika_constants.py", result["output"])
...
"""
    ...
    def test_shell_quote_helper(self):
        ...
        code = """
from anika_tools import shell_quote
...
"""
    ...
    def test_json_parse_helper_bom(self):
        ...
        code = """
from anika_tools import json_parse
...
class TestEnvVarFiltering(unittest.TestCase):
    ...
    def test_anika_rpc_socket_injected(self):
        child_env = self._get_child_env()
        self.assertIn("ANIKA_RPC_SOCKET", child_env)
...
class TestRpcTokenAuthorization(unittest.TestCase):
    ...
    request without the matching ANIKA_RPC_TOKEN must be rejected before
    ...
    def test_generated_module_sends_token(self):
        """The generated anika_tools module reads ANIKA_RPC_TOKEN and sends it."""
        src = generate_anika_tools_module(["terminal"], transport="uds")
        self.assertIn("ANIKA_RPC_TOKEN", src)
