"""Tests for tools.wake_word — the "Hey Anika" hotword detector.
...
def test_config_defaults_and_clamping():
    ...
    assert ww.wake_phrase({"phrase": "hey anika"}) == "hey anika"
    assert ww.wake_phrase({}) == "hey anika"
...
def test_looks_like_path():
    ...
    assert _looks_like_path("models/hey_anika.onnx")
...
def test_load_wake_word_config_guards_non_dict(monkeypatch):
    monkeypatch.setattr(
        "anika_cli.config.load_config", lambda: {"wake_word": "oops"}
    )
...
def test_requirements_openwakeword_available(monkeypatch):
    ...
    r = ww.check_wake_word_requirements(
        {"provider": "openwakeword", "phrase": "hey anika"}
    )
    assert r["available"] is True
    assert r["provider"] == "openwakeword"
    assert r["phrase"] == "hey anika"
...
def _install_fake_openwakeword(monkeypatch):
    ...
    class _FakeModel:
        def __init__(self, wakeword_models, inference_framework="onnx"):
            ...
            self.models = {"hey_anika": object()}

        def predict(self, frame):
            return {"hey_anika": 0.0}
...
def test_openwakeword_ensures_base_models_for_custom_path(monkeypatch):
    ...
    eng = ww._OpenWakeWordEngine(
        {"provider": "openwakeword", "openwakeword": {"model": "/models/hey_anika.onnx"}}
    )
    assert calls["download"] == [["/models/hey_anika.onnx"]]
    assert eng._labels == ["hey_anika"]


def test_bundled_hey_anika_model_ships_on_disk():
    # The "hey anika" wake word works out of the box only if the model is
