...
class TestWriteDenyExactPaths:
    ...
    def test_anika_root_env_when_running_under_profile(self, tmp_path, monkeypatch):
        ...
        ``~/.anika/.env`` (whose credentials are inherited by every profile)
        could be silently overwritten by ``write_file`` while a profile was
        active.
        """
        root = tmp_path / "anika_root"
        ...
        # Sanity check: ANIKA_HOME does point to the profile dir, not the root.
        from anika_constants import get_anika_home, get_default_anika_root
...
class TestWriteAllowed:
    ...
    def test_anika_control_files_requested_writable(self):
