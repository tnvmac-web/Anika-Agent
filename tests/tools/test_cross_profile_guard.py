...
@pytest.fixture
def fake_anika(tmp_path, monkeypatch):
    """Build a two-profile Anika layout and point ANIKA_HOME at
    the anika-security profile (matching the original-incident shape).
    """
    root = tmp_path / "fake-anika"
    (root / "skills" / "shared-skill").mkdir(parents=True)
    (root / "skills" / "shared-skill" / "SKILL.md").write_text(
        "---\nname: shared-skill\ndescription: default copy.\n---\n"
    )

    sec_home = root / "profiles" / "anika-security"
    (sec_home / "skills").mkdir(parents=True)

    coder_home = root / "profiles" / "coder"
    (coder_home / "skills").mkdir(parents=True)

    monkeypatch.setenv("ANIKA_HOME", str(sec_home))

    import anika_constants
    monkeypatch.setattr(anika_constants, "get_default_anika_root", lambda: root)

    import agent.file_safety as fs
    monkeypatch.setattr(fs, "_anika_home_path", lambda: sec_home)
    monkeypatch.setattr(fs, "_anika_root_path", lambda: root)
...
class TestWriteFileCrossProfileGuard:
    def test_in_profile_write_allowed(self, fake_anika):
        from tools.file_tools import write_file_tool
        target = fake_anika["sec_home"] / "skills" / "new-skill" / "SKILL.md"
    ...
    def test_cross_profile_write_allowed_guard_retired(self, fake_anika):
        ...
        from tools.file_tools import write_file_tool
        target = fake_anika["root"] / "skills" / "shared-skill" / "SKILL.md"
    ...
    def test_non_anika_path_unaffected(self, fake_anika, tmp_path):
        from tools.file_tools import write_file_tool
...
class TestPatchCrossProfileGuard:
    def test_cross_profile_patch_allowed_guard_retired(self, fake_anika):
    ...
    def test_cross_profile_patch_bypass(self, fake_anika):
        from tools.file_tools import patch_tool
        target = fake_anika["root"] / "skills" / "shared-skill" / "SKILL.md"
        result_json = patch_tool(
            mode="replace",
    ...
    def test_v4a_patch_writes_through_guard_retired(self, fake_anika):
...
class TestSkillManageCrossProfileErrorUX:
    ...
    def test_error_names_other_profile_when_skill_lives_there(
        self, fake_anika, monkeypatch
    ):
        """The original incident shape — model expects 'foo' in active
        profile, but 'foo' lives in default. Error must point at default."""
        self._make_skill_in_profile(fake_anika["root"], "default-only-skill")

        # Re-import the module so SKILLS_DIR picks up ANIKA_HOME (set in
        ...
        assert "not found in active profile 'anika-security'" in err
...
class TestSystemPromptActiveProfile:
    def test_default_profile_line_in_prompt(self, tmp_path, monkeypatch):
        ...
        about ~/.anika/profiles/<name>/."""
        # Don't set ANIKA_HOME — falls back to default.
        import agent.file_safety as fs
        monkeypatch.setattr(fs, "_anika_home_path", lambda: tmp_path / "fake")
        monkeypatch.setattr(fs, "_anika_root_path", lambda: tmp_path / "fake")
    ...
    def test_named_profile_line_in_prompt_text(self, fake_anika):
        """When active profile is 'anika-security', the prompt warns
        ...
        assert "Active Anika profile" in src
        assert "cross_profile=True" not in src  # guard retired
        assert "~/.anika/profiles/" in src
        # Both branches present (default and named profile).
        assert "Active Anika profile: default" in src
        assert "Active Anika profile: {active_profile}" in src
