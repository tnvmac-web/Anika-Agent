...
def _restore_tool_modules():
    original_anika_home = os.environ.get("ANIKA_HOME")
    original_modules = {
        name: module
    ...
    try:
        ...
        if original_anika_home is None:
            os.environ.pop("ANIKA_HOME", None)
        else:
            os.environ["ANIKA_HOME"] = original_anika_home
        _reset_modules(("tools", "anika_cli", "modal"))
...
def _install_modal_test_modules(
    ...
    *,
    ...
    anika_cli = types.ModuleType("anika_cli")
    anika_cli.__path__ = []  # type: ignore[attr-defined]
    sys.modules["anika_cli"] = anika_cli
    anika_home = tmp_path / "anika-home"
    os.environ["ANIKA_HOME"] = str(anika_home)
    sys.modules["anika_cli.config"] = types.SimpleNamespace(
        get_anika_home=lambda: anika_home,
    )
    ...
    async def _lookup_aio(_name: str, create_if_missing: bool = False):
        return types.SimpleNamespace(name="anika-agent", create_if_missing=create_if_missing)
    ...
    return {
        "snapshot_store": anika_home / "modal_snapshots.json",
