...
Covers the canonical fix for issues #4146, #27303, #30882, #33057:
  ...
  4. tools.code_execution_env._scrub_child_env — broad ANIKA_ prefix dropped,
...
def gw_session(monkeypatch):
    """A clean gateway session: ANIKA_GATEWAY_SESSION set, a bound session
...
def test_env_scrub_anika_allowlist_and_secret_blocks():
    from tools.code_execution_env import _scrub_child_env

    env = {
        # operational allowlist → kept
        "ANIKA_HOME": "/h", "ANIKA_PROFILE": "p",
        "ANIKA_CONFIG": "/c.yaml", "ANIKA_ENV": "/e",
        "ANIKA_DELEGATED_CHILD_CONTEXT": "1",
        # other ANIKA_* → dropped (broad prefix removed)
        "ANIKA_BASE_URL": "https://x", "ANIKA_INTERACTIVE": "1",
        "ANIKA_KANBAN_DB": "postgres://u:p@h/db",
    ...
    for kept in (
        ...
        "ANIKA_DELEGATED_CHILD_CONTEXT", "PATH",
    ):
        assert kept in out, f"{kept} should be kept"
    for dropped in (
        "ANIKA_BASE_URL", "ANIKA_INTERACTIVE", "ANIKA_KANBAN_DB",
...
def test_env_scrub_no_log_when_nothing_dropped(caplog):
    """No diagnostic noise when there are no dropped ANIKA_* vars."""
    import logging
