...
def test_resolve_stdio_command_falls_back_to_anika_node_bin(tmp_path):
    ...
    with patch("tools.mcp_tool_config.shutil.which", return_value=None), \
         patch.dict("os.environ", {"ANIKA_HOME": str(tmp_path)}, clear=False):
...
def test_resolve_stdio_command_falls_back_to_usr_local_bin():
    """When ``npx`` isn't on the filtered PATH and isn't under ``$ANIKA_HOME/node/bin``
    ...
    the upstream ``node:bookworm-slim`` image (which the Anika Docker image
    ...
    # the other candidates ($ANIKA_HOME/node/bin/npx and ~/.local/bin/npx)
