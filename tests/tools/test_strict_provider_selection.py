"""Strict tool-provider selection: the `anika tools` choice always wins.

Policy (owner decision): the provider string stored in config.yaml is what
runs at call time. "nous" → managed __NEW_ORG__ Tool Gateway only; a vendor name →
...
selection and pointing at `anika tools`.
...
MANAGED = SimpleNamespace(
    ...
    gateway_origin="https://gateway.__NEW_DOMAIN__",
)
...
class TestImageFalStrictSelection:
    ...
    def test_nous_selection_unentitled_raises_selection_error(self):
        ...
        assert "anika tools" in str(exc.value)
...
class TestSttStrictSelection:
    def test_nous_selection_beats_direct_openai_key(self):
        ...
        assert base_url.startswith("https://gateway.__NEW_DOMAIN__")
...
class TestWriteProviderConfig:
    def test_managed_row_writes_nous_and_clears_legacy_flag(self):
        ...
        provider = {"name": "__NEW_ORG__ Subscription", "tts_provider": "openai"}
    ...
    def test_managed_image_row_persists_nous_provider(self):
        ...
        config = {}
        provider = {"name": "__NEW_ORG__ Subscription", "imagegen_backend": "fal"}
