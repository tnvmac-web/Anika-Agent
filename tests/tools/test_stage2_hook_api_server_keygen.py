...
was gated on ``[ -f "$ANIKA_HOME/.env" ]`` while the first-boot seed that
...
def _run_keygen(
    ...
    script = (
        ...
        f"{env_setup}"
        f'ANIKA_HOME="{home}"\n'
        # In tests we run unprivileged; as_anika is a passthrough then.
        'as_anika() { "$@"; }\n'
...
def test_keygen_skips_when_container_env_provides_key(
    ...
    Anika loads $ANIKA_HOME/.env with override=True, so a key generated
...
def test_dockerignore_keeps_env_example_template() -> None:
    """The first-boot seed copies /opt/anika/.env.example -> $ANIKA_HOME/.env.
    ...
    assert verdict == "included", (
        ...
        "seeds $ANIKA_HOME/.env from it on first boot"
    )
