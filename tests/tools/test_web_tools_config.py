...
class TestFirecrawlClientConfig:
    ...
    def test_tool_gateway_domain_builds_firecrawl_gateway_origin(self):
        ...
        with patch.dict(os.environ, {"TOOL_GATEWAY_DOMAIN": "__NEW_DOMAIN__"}):
            with patch("tools.managed_tool_gateway.read_nous_access_token", return_value="nous-token"):
                with patch("plugins.web.firecrawl.provider.Firecrawl") as mock_fc:
                    ...
                    mock_fc.assert_called_once_with(
                        api_key="nous-token",
                        api_url="https://firecrawl-gateway.__NEW_DOMAIN__",
                    )
...
class TestBackendSelection:
    ...
    ``anika tools``.  Falls back to key-based detection for legacy/manual
    setups.
    """
    ...
    def test_managed_gateway_does_not_preempt_explicit_exa(self):
        """Regression: a __NEW_ORG__ OAuth token (managed gateway "ready") must NOT
        beat an explicitly configured EXA_API_KEY in the fallback path.
        Free __NEW_ORG__ tiers don't include web search, so the user's deliberate
    ...
    def test_managed_gateway_does_not_preempt_explicit_tavily(self):
        """A __NEW_ORG__ OAuth token must not beat an explicit TAVILY_API_KEY."""
        from tools.web_tools import _get_backend
        with patch("tools.web_tools._load_web_config", return_value={}), \
             patch("tools.web_tools._is_tool_gateway_ready", return_value=True), \
             patch.dict(os.environ, {"TAVILY_API_KEY": "tvly-test"}):
...
class TestFirecrawlEnvResolution:
    """Verify Firecrawl reads env values from anika_cli.config.get_env_value,
    not just os.getenv.  This catches the regression reported in #40190 where
    values stored in ~/.anika/.env were invisible to the provider."""
...
class TestSiblingProvidersEnvResolution:
    ...
    ~/.anika/.env), not bare os.getenv. Parametrized over the four
    ...
    def test_keenable_search_reads_key_via_get_env_value(self, monkeypatch):
        ...
        with patch(
            ...
            assert headers["X-Keenable-Title"] == "anika-agent"
    ...
    def test_tavily_request_reads_key_via_get_env_value(self, monkeypatch):
        ...
        with patch(
            ...
            assert headers["X-Client-Name"] == "anika-agent"
