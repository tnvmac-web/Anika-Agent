...
def test_output_path_rejects_anika_oauth_store(tmp_path, monkeypatch):
    """TTS output_path must not bypass the shared protected-file write guard."""
    import agent.file_safety as file_safety

    anika_home = tmp_path / "anika-home"
    anika_home.mkdir()
    monkeypatch.setattr(file_safety, "_anika_home_path", lambda: anika_home)
    monkeypatch.setattr(file_safety, "_anika_root_path", lambda: anika_home)

    target = anika_home / ".anthropic_oauth.json"
    result = json.loads(text_to_speech_tool(
        text="hello",
...
def test_output_path_rejects_mcp_token_directory(tmp_path, monkeypatch):
    ...
    anika_home = tmp_path / "anika-home"
    token_dir = anika_home / "mcp-tokens"
