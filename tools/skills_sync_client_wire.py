...
Independent of local skill state (no ``~/.anika`` reads); ``tools/skills_sync_client.py``
...
def _check_version(caps: Dict[str, Any]) -> None:
    ...
    if ver.split(".", 1)[0] != WIRE_VERSION:
        raise SyncError(f"this server speaks sync version {ver!r}, but this Anika speaks "
                        f"{WIRE_VERSION} — update Anika to sync with it")
...
class SyncClient:
    """Sync client bound to a base URL + __NEW_ORG__ bearer. Org refs/objects live behind SEPARATE ``org/``
