...
tick ``maybe_pull_skills`` and ``anika sync``. Lives under tools/ so it never imports the CLI at
module load; ``skills_sync_client_wire`` / ``skills_sync_client_org`` are re-exported here.
ACCESS GATE (pre-launch): INERT unless the user is a __NEW_ORG__ admin per the ``tool_gateway_admin``
...
cross-device state is the ``sync-manifest`` blob in the plane. Only ~/.anika/skills/ skills qualify."""
...
# Gate claim (NAS's wire name; means "__NEW_ORG__ admin" / Permissions.ADMIN_ACCESS). The bearer comes
...
class SyncInertError(RuntimeError):
    """Sync must no-op: not logged in, no bearer, or not a __NEW_ORG__ admin. Caught by the gate-and-swallow hooks."""
...
def resolve_identity() -> Dict[str, Any]:
    ...
    try:
        ...
        raise SyncInertError(f"no __NEW_ORG__ credentials: {e}") from e
...
# Configuration -- env-first so Anika Cloud can enable sync via environment alone. Every knob:
# ANIKA_SYNC_<KEY> env -> config.yaml ``sync.<key>`` -> default (base_url = the sync plane, NOT
# the inference URL; enabled; default_opt_in; org_auto_propose).
DEFAULT_SYNC_BASE_URL = "https://gateway-gateway.__NEW_DOMAIN__"
...
def resolve_sync_base_url() -> Optional[str]:
    """ANIKA_SYNC_BASE_URL -> ``sync.base_url`` -> production plane, without trailing slash
    (``/v1/sync/`` is appended by the client). None only if the default is blanked out."""
    env = os.getenv("ANIKA_SYNC_BASE_URL")
...
def sync_feature_enabled() -> bool:
    ...
    return _sync_config_bool("ANIKA_SYNC_ENABLED", "enabled", default=False)


def sync_org_auto_propose() -> bool:
    """False (default): edits to an org skill stay LOCAL until ``anika sync propose``. True: every
    edit is proposed right away (an admin still approves unless the editor is one)."""
    return _sync_config_bool("ANIKA_SYNC_ORG_AUTO_PROPOSE", "org_auto_propose", default=False)


def sync_default_opt_in() -> bool:
    """False (default): opt-IN -- a skill syncs only after ``anika sync enable`` or a plane manifest
    opting it in. True: opt-OUT -- every eligible skill syncs unless disabled (Anika Cloud default)."""
    return _sync_config_bool("ANIKA_SYNC_DEFAULT_OPT_IN", "default_opt_in", default=False)
...
def _rel_to_skills_dir(skill_dir: Path) -> Optional[Path]:
    """*skill_dir* relative to ~/.anika/skills/, or None if outside/unresolvable."""
    try:
...
def _skill_rel_path(skill_name: str) -> Optional[PurePosixPath]:
    """The skill's path relative to ~/.anika/skills/ (posix), or None."""
...
def _all_local_skill_names() -> List[str]:
    """Every local skill name (dir under ~/.anika/skills/ with SKILL.md; frontmatter ``name`` or dir name)."""
    names: List[str] = []
    root = _skills_dir()
    try:
...
def stable_device_id() -> str:
    """Per-device label at ~/.anika/skills/.sync_device_id. An existing file always wins; else seeded
    from ANIKA_SYNC_DEVICE_NAME (first use only, for Anika Cloud) or a friendly default, then persisted."""
    with suppress(OSError):
        val = _device_id_path().read_text(encoding="utf-8").strip()
        if val:
            return val
    val = (os.environ.get("ANIKA_SYNC_DEVICE_NAME") or "").strip() or _default_device_label()
    try:
...
# the bundled manifest (skills_sync.py) and the plane's `sync-manifest`. ~/.anika/skills/.sync_state.
...
def push_skills(client: Optional[SyncClient] = None, *, skill_names: Optional[List[str]] = None,
                identity: Optional[Dict[str, Any]] = None, message: str = "anika skill sync") -> Dict[str, Any]:
    ...
    if overlaps:
        ...
        return {"ok": False, "conflict": True, "conflict_ref": conflict_ref, "overlapping_skills": sorted(overlaps),
                "actual_head": actual_head, "message": (f"{len(overlaps)} skill(s) changed on both sides; wrote "
                                                        f"{conflict_ref}. Resolve out-of-band (anika sync / NAS UI).")}
...
def _gate_and_swallow(op: str, run: Callable[[Dict[str, Any]], Optional[Dict[str, Any]]]):
    """Run *run(identity)* only if all gates hold (__NEW_ORG__ admin, feature on, base URL); None if inert/error."""
    try:
...
def maybe_push_skills(*, message: str = "anika skill sync") -> Optional[Dict[str, Any]]:
...
def sync_status() -> Dict[str, Any]:
    """Snapshot for ``anika sync status``; never raises. ``org_available`` False = not in a shared org."""
