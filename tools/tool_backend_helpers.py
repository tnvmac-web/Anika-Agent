...
def managed_nous_tools_enabled(*, force_fresh: bool = False) -> bool:
    """Coarse gate: entitled to the __NEW_ORG__ Tool Gateway (paid Portal access OR a live free
...
def nous_tool_gateway_unavailable_message(capability: str = "the __NEW_ORG__ Tool Gateway", *,
                                          force_fresh: bool = False) -> str:
    """Return account-aware guidance for an unavailable __NEW_ORG__ Tool Gateway path."""
    try:
        ...
        pass
    return (f"{capability} is unavailable. Run `anika model` to refresh your "
            "__NEW_ORG__ Portal login and billing status.")
...
def _dotenv_value(env_var: str) -> str:
    """``.env`` value via ``anika_cli.config.get_env_value`` (``""`` when unavailable)."""
    try:
...
def resolve_provider_secret(env_var: str, provider_id: str, config_value: str = "",
    ...
    ``anika_cli.config.get_env_value``) -> credential pool for ``provider_id``. Under an
    ...
    Resolution order (fixes #68003 — keys added via ``anika auth add <provider>`` were invisible to the
...
def resolve_openai_audio_api_key() -> str:
    ...
    Outside a multiplexed turn, ``OPENAI_API_KEY`` additionally falls back to the credential pool (``anika
...
# Provider value the managed "__NEW_ORG__ Subscription" picker rows write for every category;
...
def read_selection(section: str) -> str | None:
    """THE single runtime read of the persisted `anika tools` selection: ``"nous"`` (managed
...
# (anika_cli.config.validate_config_structure) and selection_error() consult this map so the user learns
...
def selection_error(section: str, selection_name: str, failure: str) -> str:
    ...
    return (f"{section} is configured to use {selection_name} (set via anika "
            f"tools), but {failure}. Run 'anika tools' to change it.")
