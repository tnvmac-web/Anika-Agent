...
# match anika_cli.runtime_provider.RUNTIME_PROVIDER_TYPE_CUSTOM.
...
def _runtime_provider_credentials(v: dict, explicit_request_overrides) -> dict:
    ...
    if not api_key:
        raise ValueError(
            ...
            f"Set the appropriate environment variable or run 'anika auth'."
        )
...
def _load_config() -> dict:
    ...
    (follows ANIKA_HOME/profile; no deepcopy, since this runs on every get_definitions() rebuild) over the legacy
    ``cli.CLI_CONFIG``, which can hide user-set keys — except that ``ANIKA_IGNORE_USER_CONFIG=1`` is only honored
    by the legacy loader, so it stays authoritative when that flag is set."""
    if os.environ.get("ANIKA_IGNORE_USER_CONFIG") != "1":
        try:
...
_NOUS_PROVIDERS = frozenset({"nous", "nous-portal", "__NEW_ORG__"})
...
def _resolve_child_runtime(
    ...
    or is __NEW_ORG__ Portal (dual-wire); a pinned ``delegation.command`` must exist on PATH or the spawn fails loudly;
    ...
    # otherwise). __NEW_ORG__ Portal is dual-wire within one provider (anthropic/* → Messages, else chat_completions), so
    ...
    # would pin a child Anika/Qwen subagent onto the parent's Claude Messages wire (or the reverse).
