...
# Hard-blocked regardless of approval level (e.g. logout kills the session Anika runs in). Alt is
...
# Per-Anika-session cached backends (own cua-driver session, native target, refs, grant namespace).
...
# -> set of (action, delivery_mode) scope keys See __NEW_ORG__/anika-agent#67052 gap 4.
...
def _cua_permission_mode(session_id: str) -> str:
    """Map Anika's approval bypass onto Cua's immutable mode; fails closed. Both identity namespaces are consulted
...
def _new_backend(permission_mode: str) -> ComputerUseBackend:
    backend_name = os.environ.get("ANIKA_COMPUTER_USE_BACKEND", "cua").lower()
    if backend_name in {"cua", "cua-driver", ""}:
        from tools.computer_use.cua_backend import CuaDriverBackend
        return CuaDriverBackend(permission_mode=permission_mode)
    if backend_name != "noop":
        raise RuntimeError(f"Unknown ANIKA_COMPUTER_USE_BACKEND={backend_name!r}")
...
def _shutdown_backend_atexit() -> None:
    ...
    the Anika process that spawned it (#28152 item 3). #69903 kept the orphan from burning a core by
...
class _NoopBackend(ComputerUseBackend):  # pragma: no cover
    """Test/CI stub (ANIKA_COMPUTER_USE_BACKEND=noop). Records ``(name, kwargs)`` calls; returns trivial results."""
...
def handle_computer_use(args: Dict[str, Any], **kwargs) -> Any:
    ...
    try:
        ...
        return json.dumps({"error": f"computer_use backend unavailable: {e}",
                           "hint": "If the cua-driver binary is missing, run `anika computer-use install`. "
...
def _cache_file(subdir: str, legacy: str, name: str, pattern: str = "", cap: int = 0):
    """Path for a new file under ``$ANIKA_HOME/<subdir>`` (dir created). With ``pattern``/``cap``, first unlinks the
    oldest matching files so at most ``cap - 1`` remain (best-effort)."""
    from anika_constants import get_anika_dir  # lazy so tests can patch get_anika_dir
    cache_dir = get_anika_dir(subdir, legacy)
...
def _persist_capture_image(cap: CaptureResult) -> Optional[str]:
    """Copy of the capture in Anika' media cache so attachment surfaces can deliver it (None without an image)."""
...
def _route_capture_through_aux_vision(cap: CaptureResult, summary: str, *, visible_elements: Optional[List[UIElement]] = None,
    ...
    """Pre-analyse the capture via ``vision_analyze_tool`` (temp file under ``$ANIKA_HOME/cache/vision/``) and merge
...
def check_computer_use_requirements() -> bool:
    """macOS/Windows/Linux + cua-driver binary (or env override). `anika computer-use doctor` names blocked checks."""
