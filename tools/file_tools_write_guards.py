...
_anika_config_resolved: str | None = None
_anika_config_resolved_loaded = False
_real_anika_home_cached: str | None = None
_real_anika_home_loaded = False
...
def _anika_home_real() -> str:
    from anika_constants import get_anika_home
    return os.path.realpath(str(get_anika_home()))


def _get_anika_config_resolved() -> str | None:
    """Return the resolved absolute path of the Anika config file (cached)."""
    return _cached_lookup("_anika_config_resolved", "_anika_config_resolved_loaded", _config_path_resolved,
                          lambda: str(Path(_expand_tilde("~/.anika/config.yaml")).resolve()))


def _get_real_anika_home() -> str | None:
    """Return the realpath of the authoritative Anika home (cached)."""
    return _cached_lookup("_real_anika_home_cached", "_real_anika_home_loaded", _anika_home_real,
                          lambda: os.path.realpath(_expand_tilde("~/.anika")))
...
def _check_sensitive_path(filepath: str, task_id: str = "default") -> str | None:
    ...
    anika_config = _get_anika_config_resolved()
    if anika_config and anika_config in candidates:
        return (
            f"Refusing to write to Anika config file: {filepath}\n"
            "Agent cannot modify security-sensitive configuration. "
            "Edit ~/.anika/config.yaml directly or use 'anika config' instead.")
    return None
...
# vector (AGENTS.md / CLAUDE.md / SOUL.md / .cursorrules / project .anika tree).
...
def _protected_instruction_reason(filepath: str, task_id: str = "default",
    ...
    # ~/.anika itself is governed by its own guards (config.yaml hard-block,
    # mirror guard, write_approval); this gate targets PROJECT-LOCAL files only.
    # Must run before the ``.anika`` component rule, which would match the home.
    real_home = _get_real_anika_home()
    ...
    for candidate in (normalized, resolved):
        ...
        if base_lower in _PROTECTED_INSTRUCTION_BASENAMES or any(
            ...
            return base
        # Project-local .anika config dirs (<repo>/.anika/config.yaml) steer
        # behavior too. Only the IMMEDIATE parent counts — matching any ancestor
        # would gate every write inside a checkout living under ~/.anika.
        parts = candidate.replace("\\", "/").rstrip("/").split("/")
        if len(parts) >= 2 and parts[-2] == ".anika":
            return candidate
    return None
...
def _get_container_mirror_prefix_for_task(task_id: str = "default") -> str | None:
    """Return the container-side Anika mirror prefix for persistent Docker file tools."""
    try:
        ...
        if env is not None:
            ...
            return "/root/.anika" if persistent_docker else None
...
def _check_cross_profile_path(filepath: str, task_id: str = "default") -> str | None:
    ...
    Anika state (a write the host never reads). Not profile isolation — that guard was
