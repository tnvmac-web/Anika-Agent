...
_ANIKA_CONFIG_FILES = r'\.anika/(?:config\.yaml|SOUL\.md)'
...
THREAT_PATTERNS = [
    ...
    (r'\$HOME/\.anika/\.env|\~/\.anika/\.env',
     "anika_env_access", "critical", "exfiltration", "directly references Anika secrets file"),
    ...
    # quarantine only acts on "dangerous") but high for Anika/other config (setup docs routinely say
    ...
    (_prose_modify_re(_ANIKA_CONFIG_FILES),
     "anika_config_mod", "high", "persistence", "modification language aimed at Anika configuration files (verify intent)"),
    (_shell_write_re(_ANIKA_CONFIG_FILES),
     "anika_config_mod_shell", "critical", "persistence", "shell write (redirect/sed -i/tee/cp/mv) targeting Anika configuration files"),
    (r'\.anika/config\.yaml|\.anika/SOUL\.md',
     "anika_config_ref", "low", "persistence", "references Anika configuration files (informational; only modification intent is scored)"),
...
# `.skillignore` is Anika-native; `.clawhubignore` is honored for skills published through ClawHub.
