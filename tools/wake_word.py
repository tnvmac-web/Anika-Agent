"""Wake-word ("Hey Anika") detection — hands-free session trigger.
...
# Minimum gap between two wake fires, so one "hey anika" can't retrigger across
...
_DEFAULTS: Dict[str, Any] = {
    ...
    "provider": "openwakeword", "phrase": "hey anika", "sensitivity": 0.6,
    "confirmation_frames": _DEFAULT_CONFIRMATION_FRAMES, "start_new_session": True,
}

# Bundled "hey anika" model (tools/wakewords/) — the default; alias names resolve
# to it, not to an openWakeWord built-in.
_BUNDLED_MODEL_NAME = "hey_anika"
_BUNDLED_MODEL_ALIASES = frozenset({"", "hey_anika", "hey anika", "anika"})


def _bundled_wakeword_path(framework: str = "onnx") -> str:
    """Path to the shipped hey_anika model (.onnx/.tflite) for ``framework``."""
...
def wake_phrase(cfg: Optional[Dict[str, Any]] = None) -> str:
    ...
    return str(_get(cfg, "phrase")) or "hey anika"
...
def enrolled_profile_phrases() -> Dict[str, str]:
    ...
    with suppress(Exception):
        ...
        from anika_cli.profiles import get_profile_dir, list_profiles
...
def silent_audio_hint(details: Dict[str, Any]) -> str:
    ...
    if sys.platform == "darwin":
        return ("Microphone delivers only silence. Grant the Anika backend "
...
def check_wake_word_requirements(cfg: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    ladder = (
        ...
        (bool(missing), lambda: (f"Wake word needs {missing} configured — run `anika tools` "
...
# ── Process-wide singleton (mirrors anika_cli.voice's continuous API) ──
...
def _lock_path() -> Path:
    ...
    return get_default_anika_root() / "runtime" / "wake-word.lock"
