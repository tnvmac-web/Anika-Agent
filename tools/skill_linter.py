...
def _check_frontmatter(frontmatter: Dict[str, Any], skill_dir: Optional[Path]) -> Iterator[LintFinding]:
    ...
    anika_meta = meta.get("anika") if isinstance(meta, dict) else None
    if not isinstance(anika_meta, dict):
        yield _warn("missing-metadata", "frontmatter is missing metadata.anika.{tags, related_skills}.")
    elif "tags" not in anika_meta:
        yield _warn("missing-metadata", "metadata.anika.tags is missing.")
    author = str(frontmatter.get("author", ""))
    if author and author.strip().lower() in ("anika", "agent", "anika agent") and (
        author != "Anika Agent"):
        yield _warn("author-caps", f"author '{author}' should be 'Anika Agent' (proper caps) "
