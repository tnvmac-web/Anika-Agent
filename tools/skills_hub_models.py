...
def _anika_tags(fm: dict) -> Any:
    """``metadata.anika.tags`` from parsed frontmatter, or ``[]`` (unvalidated type)."""
    metadata = fm.get("metadata", {})
    anika_meta = metadata.get("anika", {}) if isinstance(metadata, dict) else None
    return anika_meta.get("tags", []) if isinstance(anika_meta, dict) else []
