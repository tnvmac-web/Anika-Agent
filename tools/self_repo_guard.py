...
def _block_message(operation: str, root: Path) -> str:
    ...
    scratch = (Path(anika_home).expanduser() if anika_home else Path.home() / ".anika") / "scratch"
    return (
        f"Blocked: `{operation}` would rewrite Anika's live source checkout "
        ...
        "is pushed. To change this checkout, stop Anika, run the command externally, then restart "
        "Anika.")
