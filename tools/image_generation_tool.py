#!/usr/bin/env python3
"""Image generation via FAL.ai (model picked in ``anika tools``, persisted to ``image_gen.model``).
...
# --- Managed FAL gateway (__NEW_ORG__ Subscription) ---
def _resolve_managed_fal_gateway():
    """Managed gateway config for the stored `anika tools` selection, or ``None`` for direct FAL.
    ...
    if selected == NOUS_MANAGED_PROVIDER:
        ...
        if gateway is None:
            raise ValueError(selection_error(
                ...
                "the __NEW_ORG__ Tool Gateway is not available (not entitled or unreachable)"))
        return gateway
...
def _submit_fal_request(model: str, arguments: Dict[str, Any]):
    ...
    try:
        ...
        if status is not None and 400 <= status < 500:
            ...
            raise ValueError(
                f"__NEW_ORG__ Subscription gateway rejected model '{model}' (HTTP {status}). This model "
                f"may not yet be enabled on the __NEW_ORG__ Portal's FAL proxy. Either:\n"
                f"  • Set FAL_KEY in your environment to use FAL.ai directly, or\n"
                f"  • Pick a different model via `anika tools` → Image Generation."
...
def _plugin_provider_name() -> Optional[str]:
    """Configured provider that must go through the plugin registry; None for unset/fal/__NEW_ORG__."""
...
_CACHE_BASE_BY_BACKEND = {"docker": "/root/.anika", "singularity": "/root/.anika",
                          "modal": "/root/.anika", "ssh": "~/.anika"}
...
def _agent_cache_base_for_env(env: Any) -> str | None:
    if env is not None:
        ...
        if remote_home:
            return f"{str(remote_home).rstrip('/')}/.anika"
...
def _prepare_fal_request(model_id, meta, prompt, aspect_ratio, seed, overrides, source_images):
    ...
    if source_images and not edit_endpoint:
        raise ValueError(
            ...
            f"via `anika tools` → Image Generation.")
...
def _build_no_backend_setup_message() -> str:
    ...
    if managed:
        lines.append("  2. Sign in to a __NEW_ORG__ account that has the managed FAL gateway enabled "
                     "(`anika setup`)")
    lines.append("  3. Configure a different image_gen provider via `anika tools` → Image Generation "
                 "(run `anika plugins list` to see installed backends)")
    return "\n".join(lines)
...
def _dispatch_to_plugin_provider(
    ...
    if provider is None:
        return _provider_error(
            ...
            f"Run `anika plugins list` to see available image gen backends.", "provider_not_registered")
    ...
    try:
        ...
        if is_type_error and ("image_url" in kwargs or "reference_image_urls" in kwargs):
            ...
            return _provider_error(
                ...
                f"text-to-image, or pick a backend that supports editing via `anika tools` → "
