...
_FORCE_SYNC_ENV = "ANIKA_FORCE_FILE_SYNC"
...
def iter_sync_files(container_base: str = "/root/.anika") -> list[tuple[str, str]]:
    """Enumerate all (host_path, remote_path) pairs to sync to a remote. Credential paths are
    remapped from the hardcoded /root/.anika to *container_base* (remote home may differ)."""
    ...
    files = [
        (entry["host_path"], entry["container_path"].replace("/root/.anika", container_base, 1))
...
class FileSyncManager:
    ...
    def sync(self, *, force: bool = False) -> None:
        ...
        per ``sync_interval`` unless *force* or ``ANIKA_FORCE_FILE_SYNC=1``. Transactional:
    ...
    def sync_back(self, anika_home: Path | None = None) -> None:
        """Pull remote changes back to the host: download the remote ``.anika/`` as a tar and
        ...
        with self._transaction_lock:
            self._sync_back_transaction(anika_home=anika_home)

    def _sync_back_transaction(self, anika_home: Path | None = None) -> None:
        ...
        # to avoid retry storms against an uninitialized remote .anika/.
        if not self._pushed_hashes and not self._synced_files:
            logger.debug("sync_back: no prior push state — skipping")
            return

        lock_path = (anika_home or get_anika_home()) / ".sync.lock"
    ...
    def _sync_back_impl(self) -> None:
        ...
        try:
            ...
            with tempfile.TemporaryDirectory(prefix="anika-sync-back-") as staging:
    ...
    def _infer_host_path(self, remote_path: str, file_mapping: list[tuple[str, str]] | None = None, *,
        ...
        substitution (``/root/.anika/skills/b.md`` -> ``~/.anika/skills/b.md``)."""
