...
class ManagedModalEnvironment(BaseEnvironment):
    """Gateway-owned Modal sandbox with Anika-compatible execute/cleanup."""
    ...
    def __init__(self, image: str, cwd: str = "/root", timeout: int = 60,
        ...
        if gateway is None:
            raise ValueError("Managed Modal requires a configured tool gateway and __NEW_ORG__ user token")
