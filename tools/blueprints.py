...
``metadata.anika.blueprint`` (``schedule`` required; optional ``deliver`` [default "origin"],
...
@dataclass
class BlueprintSpec:
    """Parsed ``metadata.anika.blueprint`` automation spec for a skill."""
...
def parse_blueprint(skill_md_text: str) -> Optional[BlueprintSpec]:
    ...
    A skill is a blueprint iff ``metadata.anika.blueprint`` is a mapping with a
    ...
    anika = meta.get("anika") if isinstance(meta, dict) else None
    blueprint = anika.get("blueprint") if isinstance(anika, dict) else None
    if blueprint is None:
        return None
    if not isinstance(blueprint, dict):
        raise BlueprintError("metadata.anika.blueprint must be a mapping")
...
def export_blueprint(job: Dict[str, Any], body: str, *, blueprint_name: Optional[str] = None) -> str:
    ...
    ``metadata.anika.blueprint`` block) ready for ``anika skills publish``.
    ...
    frontmatter = {
        ...
        "metadata": {"anika": {"tags": ["blueprint", "automation"], "blueprint": block}},
    }
