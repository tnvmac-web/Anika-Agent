...
def _modal_unavailable_reason(modal_state: Dict[str, Any]) -> tuple[str, str]:
    ...
    if modal_state["managed_mode_blocked"] or modal_state["mode"] == "managed":
        tail = (("__NEW_ORG__ Tool Gateway access is not currently available and no direct Modal credentials/config "
...
def _build_docker_env(*, image, cwd, timeout, cc, task_id, host_cwd, **_):
    ...
    # One-shot reaper for labeled containers orphaned by prior Anika processes that died before
...
def _build_plugin_env(*, env_type, image, cwd, timeout, cc, task_id, **_):
    ...
    if provider is not None:
        ...
        try:
            env_obj._anika_backend_name = provider.name.strip().lower()
        except AttributeError:
            pass
        return env_obj
    try:
