...
_FOREGROUND_GUIDANCE = (
    (
        ...
        "notify_on_complete=true) so Anika tracks the process, then run readiness "
...
def gateway_lifecycle_block(
    *,
    ...
    ``systemctl``/``launchctl``/``anika gateway restart|stop|uninstall``
    targeting anika-gateway would SIGTERM the gateway — and this very
    ...
    SUPERVISED-gateway probe, not the raw ``_ANIKA_GATEWAY`` marker: that
    marker leaks into every process that merely imports gateway.run (anika
    serve, CLI, web server), which must still be able to restart the gateway;
    an unsupervised foreground ``anika gateway run`` has no KeepAlive to turn
    ...
    if lifecycle_scan_root_within_budget(command) and contains_launchctl_submit_command(command):
        return _blocked_json(
            ...
            "Use Anika cron for one-shot delayed work, or install an "
    ...
    if contains_gateway_lifecycle_command_or_referenced_script(
        ...
        return _blocked_json(
            ...
            "to child processes). Run `anika gateway restart` from a "
