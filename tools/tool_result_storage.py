...
is ALWAYS host-side ``$ANIKA_HOME/cache/spillover/{id}.txt`` (works for sessions that never
...
def get_spillover_dir():
    """Return $ANIKA_HOME/cache/spillover as a Path (not created)."""
    from anika_constants import get_anika_home
    return get_anika_home() / SPILLOVER_SUBDIR
...
def _write_to_spillover(content: str, filename: str):
    """Write host-side to $ANIKA_HOME/cache/spillover; returns path str or None."""
    try:
...
def _resolve_storage_dir(env) -> str:
    ...
    return f"{temp_dir.rstrip('/') or '/'}/anika-results" if temp_dir else STORAGE_DIR
...
HEREDOC_MARKER = "ANIKA_PERSIST_EOF"
# ---- END PLUGIN-COMPAT ----
