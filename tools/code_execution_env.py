...
# prefix, the exact-name ANIKA_ allowlist, or (Windows) an OS-essential name.
# The broad "ANIKA_" prefix is deliberately NOT safe — it leaked config vars
# without a secret substring (ANIKA_BASE_URL, ANIKA_KANBAN_DB, *_WEBHOOK).
# ANIKA_RPC_SOCKET / ANIKA_RPC_DIR / TZ / HOME are injected after scrubbing.
...
# imports may read at import time. ANIKA_DELEGATED_CHILD_CONTEXT must ride
# along or a child that imports Anika code loses the Kanban mutation guard
# while still inheriting ANIKA_HOME.
_ANIKA_CHILD_ALLOWED = frozenset({
    "ANIKA_HOME", "ANIKA_PROFILE", "ANIKA_CONFIG", "ANIKA_ENV", "ANIKA_DELEGATED_CHILD_CONTEXT",
})
...
def _scrub_child_env(source_env, is_passthrough=None, is_windows=None):
    ...
    (4) operational ANIKA_* pass by exact name; (5) on Windows the
    ...
    scrubbed = {}
    # Non-secret ANIKA_* vars no allowlist admits are dropped on purpose; a script importing a
    ...
    for k, v in source_env.items():
        ...
        if (any(k.startswith(p) for p in _SAFE_ENV_PREFIXES)
                or k in _ANIKA_CHILD_ALLOWED
                or (is_windows and k.upper() in _WINDOWS_ESSENTIAL_ENV_VARS)):
            scrubbed[k] = v
        elif k.startswith("ANIKA_"):
            _dropped_anika.append(k)
    if _dropped_anika:
        logger.debug(
            "execute_code: dropped %d non-allowlisted ANIKA_* var(s) from the "
            ...
            len(_dropped_anika), ", ".join(sorted(_dropped_anika)),
        )
...
def _build_child_env(*, rpc_endpoint: str, rpc_token: str, tmpdir: str,
    ...
    from anika_constants import apply_subprocess_home_env
    child_env = _scrub_child_env(os.environ)
    child_env["ANIKA_RPC_SOCKET"] = rpc_endpoint
    child_env["ANIKA_RPC_TOKEN"] = rpc_token
    ...
    # Only TZ reaches the child; ANIKA_TIMEZONE is an internal setting.
    _tz_name = os.getenv("ANIKA_TIMEZONE", "").strip()
    if _tz_name:
        child_env["TZ"] = _tz_name
    child_env.pop("ANIKA_TIMEZONE", None)
    apply_subprocess_home_env(child_env)
    # PYTHONPATH: the staging dir (anika_tools.py) must always be importable even when project
    # mode changes CWD. Anika's root is added ONLY when the child runs in Anika's Python env —
    # exposing Anika's site-packages to an external interpreter can mix incompatible compiled
    # extensions (3.12 NumPy under a 3.9 venv). Inherited Anika-owned entries are stripped first.
    # Before re-injecting PYTHONPATH, strip Anika-owned entries that leaked through _scrub_child_env
    # (PYTHONPATH is in _SAFE_ENV_PREFIXES so it passes the scrub). They are redundant for same-Anika-
    ...
    _strip_anika_owned_pythonpath(child_env)
    _existing_pp = child_env.get("PYTHONPATH", "")
    _pp_parts = [tmpdir]
    if _uses_anika_python_environment(child_python):
        _pp_parts.append(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
    elif child_python not in _external_env_logged:
        # Surface once per interpreter so "import anika_constants fails" is diagnosable.
        _external_env_logged.add(child_python)
        logger.info("execute_code: child interpreter %s is outside the Anika "
                    "environment; anika root omitted from PYTHONPATH", child_python)
    if _existing_pp:
...
# Interpreter paths already reported as outside the Anika environment.
...
def _uses_anika_python_environment(python_path: str) -> bool:
    """Whether *python_path* belongs to Anika's active Python environment. Short-circuits when
    it IS the running interpreter (by path or realpath — covers ``uv run`` venvs) so no probe
    runs on the default strict path and a flaky probe can never drop the anika root."""
