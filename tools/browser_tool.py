...
from anika_cli.config import DEFAULT_CONFIG, cfg_get
...
# dependency could read every Anika secret from process.env.
...
def _build_browser_env() -> dict:
    ...
    env = anika_subprocess_env(inherit_credentials=False)
...
# than a concrete path (also compared in anika_cli/tools_config.py and doctor.py).
...
_REAL_PROFILE_SESSION = "anika-real-profile"
...
def _socket_safe_tmpdir() -> str:
    """Short temp dir for Unix sockets: macOS ``TMPDIR`` + ``agent-browser-anika_…``
...
# Owner Anika home per session: the janitor is one process-global thread, so each
...
def _maybe_start_recording(task_id: str):
    ...
    try:
        ...
        if not cfg_get(read_raw_config(), "browser", "record_sessions", default=False):
            return
        recordings_dir = anika_home / "browser_recordings"
...
def browser_vision(question: str, annotate: bool = False, task_id: Optional[str] = None) -> Union[str, Dict[str, Any]]:
    ...
    screenshots_dir = get_anika_dir("cache/screenshots", "browser_screenshots")
...
_PLUGIN_COMPAT_LAZY = {
    ...
    'get_anika_home_override': ('anika_constants', 'get_anika_home_override'),
    'anika_home_key': ('anika_constants', 'anika_home_key'),
    ...
    'node_tool_runnable': ('anika_constants', 'node_tool_runnable'),
