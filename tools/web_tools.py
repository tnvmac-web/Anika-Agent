...
Backend is selected during ``anika tools`` (``web.backend`` in config.yaml; per
...
def _env_value(name: str) -> str:
    """Resolve ``name`` via the config-aware env layer (``anika config set`` values), then process env.

    Mirrors the SearXNG provider's ``_searxng_url()`` so that values set through Anika' config/.env layer
    (``anika config set``, ``anika tools``) are honored here too — not just raw process-env exports.
...
def _get_backend() -> str:
    ...
    # Never-configured install. Explicit user credentials beat the managed-gateway probe (a __NEW_ORG__ OAuth
...
def web_search_tool(query: str, limit: int = 5) -> str:
    ...
    try:
        ...
        if provider is None:
            fallback = "No web search provider configured. Run `anika tools` to set one up."
...
def _provider_is_ready(provider) -> bool:
    ...
    False (so dispatch can emit a precise error), so readiness gates (tool check_fn, ``anika doctor``)
