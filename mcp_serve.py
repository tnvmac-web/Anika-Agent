"""
Anika MCP Server — expose messaging conversations as MCP tools (`anika mcp serve`).
...
Matches OpenClaw's 9-tool channel bridge surface plus the Anika-specific
channels_list. Client config: {"mcpServers": {"anika": {"command": "anika", "args": ["mcp", "serve"]}}}
"""
...
logger = logging.getLogger("anika.mcp_serve")
...
def _anika_home() -> Path:
    try:
        ...
        return Path(os.environ.get("ANIKA_HOME", Path.home() / ".anika"))


def _get_sessions_dir() -> Path:
    return _anika_home() / "sessions"


def _read_state_db_mtime() -> float:
    try:
        return (_anika_home() / "state.db").stat().st_mtime
...
def _load_channel_directory() -> dict:
    ...
    return _read_json(_anika_home() / "channel_directory.json")
...
class EventBridge:
    ...
    event queue with waiter support (the Anika analogue of OpenClaw's WebSocket
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "MCPServer":
    """Create and return the Anika MCP server with all tools registered."""
    if not _MCP_SERVER_AVAILABLE:
        raise ImportError(f"MCP server requires the 'mcp' package. Install with: {sys.executable} -m pip install 'mcp'")
    mcp = MCPServer("anika", instructions=(
        "Anika Agent messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the Anika MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
