...
1. Bundled plugins: ``plugins/model-providers/<name>/`` (shipped with anika-agent)
2. User plugins: ``$ANIKA_HOME/plugins/model-providers/<name>/``
3. Pip-installed plugins: distributions exposing a ``anika_agent.plugins``
...
def register_provider(profile: ProviderProfile) -> None:
    ...
    plugins under ``$ANIKA_HOME/plugins/model-providers/`` can override
...
def _user_plugins_dir() -> Path | None:
    """Return ``$ANIKA_HOME/plugins/model-providers/`` if it exists."""
    try:
        from anika_constants import get_anika_home

        d = get_anika_home() / "plugins" / "model-providers"
...
def _installed_plugins_dir() -> Path | None:
    """Return ``$ANIKA_HOME/plugins/`` if it exists.

    This is where ``anika plugins install`` clones a plugin — flat, one
...
def _import_plugin_dir(plugin_dir: Path, source: str) -> None:
    ...
    # multiple ANIKA_HOME profiles don't alias each other.
    safe_name = plugin_dir.name.replace("-", "_")
    if source == "bundled":
        module_name = f"plugins.model_providers.{safe_name}"
    else:
        module_name = f"_anika_user_provider_{safe_name}"
...
def _discover_entry_point_providers() -> None:
    """Import pip-installed provider plugins via the ``anika_agent.plugins``
    ...
    * **Provider targets only.** The ``anika_agent.plugins`` group is shared
    ...
    first, so filesystem plugins (bundled + ``$ANIKA_HOME``) keep their
    ...
    try:
        from anika_cli.plugins import _get_disabled_plugins, _get_enabled_plugins
...
def _discover_providers() -> None:
    ...
    Order:
      ...
      2. User plugins at ``$ANIKA_HOME/plugins/model-providers/<name>/``
      2b. Plugins installed by ``anika plugins install`` at
          ``$ANIKA_HOME/plugins/<name>/`` that declare ``kind: model-provider``
    ...
    # 0. Pip-installed plugins — entry points in the ``anika_agent.plugins``
    ...
    #    filesystem steps means a bundled or ``$ANIKA_HOME`` profile of the
    ...
    # 1. Bundled plugins — shipped with anika-agent.
    ...
    # 2. User plugins — under $ANIKA_HOME/plugins/model-providers/<name>/.
    ...
    # 2b. Plugins installed by ``anika plugins install`` / the plugin index.
    #     Those clone into $ANIKA_HOME/plugins/<name>/ — flat, NOT under
