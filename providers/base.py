...
def _profile_user_agent() -> str:
    """Return a ``anika-cli/<version>`` UA string, with a stable fallback.
    ...
    """
    try:
        from anika_cli import __version__ as _ver  # lazy: avoid layer cycle at import time
        return f"anika-cli/{_ver}"
    except Exception:
        return "anika-cli"
...
class ProviderProfile:
    ...
    # describe how to launch it; anika_cli/auth.py's
    ...
    def create_client(self, **client_kwargs: Any) -> Any | None:
        ...
        a profile registered from ``~/.anika/plugins/model-providers/`` or a
    ...
    def fetch_models(
        ...
        *,
        ...
        # anika-cli UA so the catalog endpoint is reachable.
