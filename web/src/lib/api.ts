import { buildAnikaWebSocketUrl } from "@anika/shared";

// The dashboard can be served either at the root of its host (e.g.
// https://kanban.tilos.com/) or under a URL prefix when reverse-proxied
// (e.g. https://mission-control.tilos.com/anika/). The Python backend
// injects ``window.__ANIKA_BASE_PATH__`` into index.html based on the
// incoming ``X-Forwarded-Prefix`` header so the SPA can address its own
// ``/api/...`` and ``/dashboard-plugins/...`` URLs correctly without a
// rebuild. Empty string means "served at root".
function readBasePath(): string {
  if (typeof window === "undefined") return "";
  const raw = window.__ANIKA_BASE_PATH__ ?? "";
  if (!raw) return "";
...
export const ANIKA_BASE_PATH = readBasePath();
const BASE = ANIKA_BASE_PATH;
...
declare global {
  interface Window {
    __ANIKA_SESSION_TOKEN__?: string;
    __ANIKA_BASE_PATH__?: string;
    ...
    __ANIKA_AUTH_REQUIRED__?: boolean;
  }
}
const SESSION_HEADER = "X-Anika-Session-Token";
...
export async function fetchJSON<T>(
  ...
  const token = window.__ANIKA_SESSION_TOKEN__;
  if (token) {
  ...
  if (res.status === 401) {
    ...
    // (``anika update``, ``anika gateway restart``, etc.). A tab kept
    // open across the restart holds the OLD token in
    // ``window.__ANIKA_SESSION_TOKEN__`` from the previous HTML render,
    ...
    if (!window.__ANIKA_AUTH_REQUIRED__ && !options?.allowUnauthorized) {
  ...
  if (res.ok) {
    ...
    // current ``window.__ANIKA_SESSION_TOKEN__`` is valid, so the next
 ...
 *
 * The dashboard's gated-mode WS auth (``anika_cli.web_server._ws_auth_ok``)
...
export async function buildWsAuthParam(): Promise<[string, string]> {
  if (window.__ANIKA_AUTH_REQUIRED__) {
 ...
 *  - loopback / ``--insecure``: attach the ``X-Anika-Session-Token`` header.
 *  - gated OAuth: no token header (it's absent by design); the
 *    ``anika_session_at`` cookie rides along via ``credentials: 'include'``.
 *
 ...
 * ``window.__ANIKA_SESSION_TOKEN__`` directly, so the gated-mode ticket
...
export const api = {
  ...
  updateAnika: () =>
    fetchJSON<ActionResponse>("/api/anika/update", { method: "POST" }),
  checkAnikaUpdate: (force = false) =>
    fetchJSON<UpdateCheckResponse>(
      `/api/anika/update/check${force ? "?force=true" : ""}`,
    ),
 ...
 * __NEW_ORG__ Portal contract V1 (the access token has no email/name claims —
...
export interface SkillHubSource {
  ...
  /** anika-index only: whether the centralized index loaded. */
  available?: boolean;
}
...
export interface StatusResponse {
   ...
   * updates are handled by the outer launcher instead of ``anika update``. */
  can_update_anika?: boolean;
  config_path: string;
  ...
  /** NS-656: disk-usage rollup for the ANIKA_HOME volume. Absent on
   * older gateways. */
 ...
 * sample of the ANIKA_HOME volume — no staleness dimension, so no
 * sampled_at. */
