...
import { Badge } from "@__NEW_ORG__/ui/ui/components/badge";
import { Button } from "@__NEW_ORG__/ui/ui/components/button";
import { Spinner } from "@__NEW_ORG__/ui/ui/components/spinner";
import { H2 } from "@__NEW_ORG__/ui/ui/components/typography/h2";
import { Card, CardContent } from "@__NEW_ORG__/ui/ui/components/card";
import { Checkbox } from "@__NEW_ORG__/ui/ui/components/checkbox";
import { Input } from "@__NEW_ORG__/ui/ui/components/input";
import { Label } from "@__NEW_ORG__/ui/ui/components/label";
import { Select, SelectOption } from "@__NEW_ORG__/ui/ui/components/select";
import { Toast } from "@__NEW_ORG__/ui/ui/components/toast";
import { useToast } from "@__NEW_ORG__/ui/hooks/use-toast";
import { useConfirmDelete } from "@__NEW_ORG__/ui/hooks/use-confirm-delete";
import { ConfirmDialog } from "@__NEW_ORG__/ui/ui/components/confirm-dialog";
import { useModalBehavior } from "@/hooks/useModalBehavior";
import { DeleteConfirmDialog } from "@/components/DeleteConfirmDialog";
import { AnikaConsoleModal } from "@/components/AnikaConsoleModal";
...
export default function SystemPage() {
  ...
  // spawned `anika import` runs non-interactively (stdin is /dev/null), so
  ...
  const checkForUpdate = useCallback(
    async (force = false) => {
      if (status?.can_update_anika === false) return;
      setCheckingUpdate(true);
      try {
        const info = await api.checkAnikaUpdate(force);
        setUpdateInfo(info);
        if (force) {
    ...
    [showToast, status?.can_update_anika],
  );
  ...
  const applyUpdate = async () => {
    ...
    if (status?.can_update_anika === false) {
      showToast(
        "Anika updates are managed outside this dashboard.",
        "success",
      );
      return;
    }
    try {
      const resp = await api.updateAnika();
      if (!resp.ok) {
        showToast(
          resp.message ??
      ...
      setActiveAction(resp.name ?? "anika-update");
  ...
  const canUpdateAnika = status?.can_update_anika !== false;
  ...
  return (
    <div className="flex flex-col gap-8">
      ...
      <ConfirmDialog
        open={canUpdateAnika && updateConfirmOpen}
        onCancel={() => setUpdateConfirmOpen(false)}
        onConfirm={() => void applyUpdate()}
        title="Update Anika?"
        description={
          updateInfo && updateInfo.behind && updateInfo.behind > 0
            ? `This will run 'anika update' (${updateInfo.update_command}) and pull ${updateInfo.behind} new commit${updateInfo.behind === 1 ? "" : "s"}. The gateway restarts when the update finishes; the current session keeps its prompt cache until then.`
            : `This will run 'anika update' (${updateInfo?.update_command ?? "anika update"}) and restart the gateway when it finishes.`
        }
        ...
        <Card>
          <CardContent className="py-4">
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-y-3 gap-x-6 text-sm">
              ...
              <div>
                <div className="text-xs uppercase tracking-wider text-muted-foreground">Anika</div>
                <div className="flex items-center gap-2">
                  <span>v{stats?.anika_version}</span>
                  {canUpdateAnika &&
                    updateInfo &&
      ...
      <section className="flex flex-col gap-3">
        <H2 variant="sm" className="flex items-center gap-2 text-muted-foreground">
          <Globe className="h-4 w-4" /> __NEW_ORG__ Portal
        </H2>
        <Card>
          <CardContent className="flex flex-col gap-3 py-4">
            <div className="flex items-center gap-3">
              ...
              <a
                href={portal?.subscription_url || "https://portal.__NEW_DOMAIN__/manage-subscription"}
                target="_blank"
                rel="noreferrer"
            ...
            {!portal?.logged_in && (
              <p className="text-xs text-muted-foreground">
                Log in with <span className="font-mono">anika portal</span>.
              </p>
            )}
          </CardContent>
        </Card>
        ...
        <Card>
          <CardContent className="flex flex-col gap-4 py-4">
            ...
            <div className="flex flex-col gap-3 border-t border-border pt-4 sm:flex-row sm:items-end">
              <div className="grid min-w-0 flex-1 gap-2">
                ...
                <Input
                  ...
                  placeholder="$ANIKA_HOME/backups/anika-backup.zip"
                />
              </div>
              <Button
                size="sm"
            ...
            <ConfirmDialog
              ...
              title="Restore full Anika backup?"
              description={`This will overwrite your current Anika configuration, skills, sessions, and data with the contents of ${backupImportLabel(importConfirmTarget)}. This cannot be undone.`}
              destructive
              ...
              <div className="flex items-start gap-2">
                ...
                <div className="flex flex-col">
                  ...
                  <span className="text-xs text-muted-foreground max-w-prose">
                    ...
                    returns links to send the Anika team. Pastes auto-delete
                    after 6 hours.
                  </span>
                </div>
              </div>
