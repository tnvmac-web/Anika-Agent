...
import { api, ANIKA_BASE_PATH } from "@/lib/api";
...
export const MANIFEST_CACHE_KEY = "anika:plugin-manifests";
...
export function usePlugins() {
  ...
  useEffect(() => {
    ...
    for (const manifest of manifests) {
      ...
      if (manifest.css) {
        const cssUrl = `${ANIKA_BASE_PATH}/dashboard-plugins/${manifest.name}/${manifest.css}`;
      ...
      const baseUrl = `${ANIKA_BASE_PATH}/dashboard-plugins/${manifest.name}/${manifest.entry}`;
      const scriptSrc = import.meta.env.DEV
        ? `${baseUrl}?anika_dv=${Date.now()}`
        : baseUrl;
      ...
      script.setAttribute("data-anika-plugin", manifest.name);
