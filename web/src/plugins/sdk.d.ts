/**
 * Anika Dashboard Plugin SDK — typed contract (SPIKE)
 * ====================================================
 *
 * This is the public type surface for ``window.__ANIKA_PLUGIN_SDK__`` and
 * ``window.__ANIKA_PLUGINS__``, the globals the dashboard host exposes to
 ...
 * window.__ANIKA_PLUGIN_SDK__`` because:
 *   1. The runtime object is assembled from many internal modules
 *      (``@/lib/api``, ``@__NEW_ORG__/ui``, …). Deriving the type would
 ...
 *
 * Versioning: bump ``AnikaPluginSDK["sdkVersion"]`` (and the
 ...
 *   - Ship as a published ``@anika/dashboard-plugin-sdk`` types package, or
 ...
 *     (pulls @__NEW_ORG__/ui types into the contract) or is the loose
 ...
 * of calling ``fetch`` with a hand-read ``window.__ANIKA_SESSION_TOKEN__``.
 */
...
// ---------------------------------------------------------------------------
// Registry surface (window.__ANIKA_PLUGINS__)
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// SDK surface (window.__ANIKA_PLUGIN_SDK__)
// ---------------------------------------------------------------------------

export interface AnikaPluginSDK {
  ...
  /**
   * Shared UI primitives (__NEW_ORG__ DS / shadcn). Typed permissively at the
...
declare global {
  interface Window {
    __ANIKA_PLUGIN_SDK__?: AnikaPluginSDK;
    __ANIKA_PLUGINS__?: PluginRegistry;
  }
}
