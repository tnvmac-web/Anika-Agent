...
interface AnikaConsoleModalProps {
  open: boolean;
  onClose: () => void;
}
...
export function AnikaConsoleModal({ open, onClose }: AnikaConsoleModalProps) {
  const modalRef = useModalBehavior({ open, onClose });
  const hostRef = useRef<HTMLDivElement | null>(null);
  const termRef = useRef<XtermTerminal | null>(null);
  const wsRef = useRef<WebSocket | null>(null);
  const lineRef = useRef("");
  const promptRef = useRef("anika> ");
  const inputPromptRef = useRef("anika> ");
  ...
  const handleFrame = useCallback(
    (frame: ConsoleFrame) => {
      ...
      if (frame.type === "ready") {
        const nextPrompt = frame.prompt || "anika> ";
  ...
  useEffect(() => {
    ...
    writeLine(term, "\x1b[2mConnecting to Anika Console...\x1b[0m");
  ...
  return createPortal(
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="anika-console-title"
    >
      <div
        className={cn(
          themedBody,
