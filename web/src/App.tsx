...
import { SelectionSwitcher } from "@__NEW_ORG__/ui/ui/components/selection-switcher";
import { Spinner } from "@__NEW_ORG__/ui/ui/components/spinner";
import { Typography } from "@__NEW_ORG__/ui/ui/components/typography/index";
import { ConfirmDialog } from "@__NEW_ORG__/ui/ui/components/confirm-dialog";
import { cn } from "@/lib/utils";
import { SidebarFooter } from "@/components/SidebarFooter";
import { SidebarStatusStrip, gatewayLine } from "@/components/SidebarStatusStrip";
import { useBelowBreakpoint } from "@__NEW_ORG__/ui/hooks/use-below-breakpoint";
...
const SIDEBAR_COLLAPSED_KEY = "anika-sidebar-collapsed";
          ...
          >
            ...
            >
              ...
              {sidebarNav.pluginItems.length > 0 && (
                <div
                  aria-labelledby="anika-sidebar-plugin-nav-heading"
                ...
                >
                  <span
                    ...
                    id="anika-sidebar-plugin-nav-heading"
                  >
...
function SidebarSystemActions({
  ...
  const { activeAction, isBusy, isRunning, pendingAction, runAction } =
    useSystemActions();
  const canUpdateAnika = status?.can_update_anika === true;
  ...
  const updateConfirmDescription = useMemo(() => {
    if (updateConfirmInfo?.behind && updateConfirmInfo.behind > 0) {
      ...
      return `This will run 'anika update' (${cmd}) and pull ${n} new commit${n === 1 ? "" : "s"}. The gateway restarts when the update finishes; the current session keeps its prompt cache until then.`;
    }
    const cmd = updateConfirmInfo?.update_command ?? "anika update";
    return (
      t.status.updateAnikaConfirmMessage ??
      `This will run 'anika update' (${cmd}) and restart the gateway when it finishes.`
    );
  }, [t.status.updateAnikaConfirmMessage, updateConfirmInfo]);
  ...
  if (canUpdateAnika) {
    items.push({
      action: "update",
      icon: Download,
      label: t.status.updateAnika,
      runningLabel: t.status.updatingAnika,
      spin: false,
    });
  }
  ...
  return (
    ...
    <ConfirmDialog
      ...
      description={
        ...
        "This restarts the Anika gateway process. Connected channels and active sessions will reconnect afterward."
      }
    ...
    <ConfirmDialog
      cancelLabel={t.common.cancel}
      confirmLabel={t.status.updateAnikaConfirmNow ?? "Update now"}
      description={
      ...
      title={t.status.updateAnikaConfirmTitle ?? `${t.status.updateAnika}?`}
    />
    </>
  );
}
