...
const BACKEND = process.env.ANIKA_DASHBOARD_URL ?? "http://127.0.0.1:9119";

/**
 * In production the Python `anika dashboard` server injects a one-shot
 * session token into `index.html` (see `anika_cli/web_server.py`). The
 * Vite dev server serves its own `index.html`, so unless we forward that
 * token, every protected `/api/*` call 401s.
 *
 * This plugin fetches the running dashboard's `index.html` on each dev page
 * load, scrapes the `window.__ANIKA_SESSION_TOKEN__` assignment, and
 * re-injects it into the dev HTML. No-op in production builds.
 */
function anikaDevToken(): Plugin {
  const TOKEN_RE = /window\.__ANIKA_SESSION_TOKEN__\s*=\s*"([^"]+)"/;
  const EMBEDDED_RE =
    /window\.__ANIKA_DASHBOARD_EMBEDDED_CHAT__\s*=\s*(true|false)/;

  return {
    name: "anika:dev-session-token",
    apply: "serve",
    async transformIndexHtml() {
      try {
        ...
        if (!match) {
          console.warn(
            `[anika] Could not find session token in ${BACKEND} — ` +
              `is \`anika dashboard\` running? /api calls will 401.`,
          );
          return;
        }
        ...
        return [
          {
            ...
            injectTo: "head",
            children:
              `window.__ANIKA_SESSION_TOKEN__="${match[1]}";` +
              `window.__ANIKA_DASHBOARD_EMBEDDED_CHAT__=${embeddedJs};`,
          },
        ];
      } catch (err) {
        console.warn(
          `[anika] Dashboard at ${BACKEND} unreachable — ` +
            `start it with \`anika dashboard\` or set ANIKA_DASHBOARD_URL. ` +
...
export default defineConfig({
  ...
  resolve: {
    alias: {
      ...
      "@anika/shared": path.resolve(__dirname, "../apps/shared/src"),
    },
    // When @__NEW_ORG__/ui is symlinked via `file:../../design-language`,
      ...
      output: {
        codeSplitting: {
          ...
          groups: [
            ...
            {
              name: "ui",
              test: /node_modules[\\/]@__NEW_ORG__[\\/]ui([\\/]|$)/,
            },
            {
              name: "vendor",
              test: /node_modules[\\/]/,
  ...
  server: {
    proxy: {
      ...
      // Same host as `anika dashboard` must serve these; Vite has no
