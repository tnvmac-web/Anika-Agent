...
path containing one) → ``DEFAULT_MODEL``. Auth via ``FAL_KEY`` or the managed __NEW_ORG__ gateway; output is an HTTPS URL.
"""
...
def _resolve_managed_fal_video_gateway():
    ...
    if selected == NOUS_MANAGED_PROVIDER:
        ...
        if gateway is None:
            raise ValueError(selection_error("video_gen", NOUS_MANAGED_PROVIDER, "the __NEW_ORG__ Tool Gateway is not available (not entitled or unreachable)"))
        return gateway
...
def _submit_fal_video_request(endpoint: str, arguments: Dict[str, Any]):
    ...
    try:
        ...
        if status is not None and 400 <= status < 500:
            raise ValueError(f"__NEW_ORG__ Subscription gateway rejected endpoint '{endpoint}' (HTTP {status}). This model may not yet be enabled "
                             f"on the __NEW_ORG__ Portal's FAL proxy. Either:\n  • Set FAL_KEY in your environment to use FAL.ai directly, or\n"
                             f"  • Pick a different model via `anika tools` → Video Generation.") from exc
        raise
...
_NO_BACKEND_MSG = ("No FAL backend available. Either set FAL_KEY (run `anika tools` → Video Generation → FAL to configure) "
                   "or sign in to __NEW_ORG__ (`anika setup`) for managed gateway access.")
_MODALITY_MISSING_MSG = {
    "image": "FAL family {fid} has no image-to-video endpoint. Pick a family with image-to-video support via `anika tools` → Video Generation.",
