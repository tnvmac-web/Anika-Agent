...
``api_mode="codex_responses"`` + the ``api.router.com`` host mandate in ``anika_cli/providers.py``
...
def _resolve_api_key() -> str:
    ...
    try:
        from anika_cli.config import get_env_value_prefer_dotenv as prefer_dotenv
    except Exception:
        prefer_dotenv = None
...
def _disk_path() -> Optional[Path]:
    try:
        ...
        return get_anika_home() / "cache" / "router_catalog.json"
    except Exception:
        return None
...
router = RouterProfile(
    ...
    default_headers={"User-Agent": f"Anika-Agent/{_ANIKA_VERSION}"},
