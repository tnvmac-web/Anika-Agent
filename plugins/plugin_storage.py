"""Per-plugin persistent storage: ``<anika home>/plugin-data/<name>/``.

Plugins must NOT park state in ``<anika home>/plugins/<name>/`` (the install dir, deleted by
...
# Mirrors the plugin-name shape `anika plugins install` accepts (no separators/traversal).
...
def plugin_data_dir(name: str) -> Path:
    """Return (and create) ``<anika home>/plugin-data/<name>/``; resolves ``get_anika_home()`` on
    ...
    root = get_anika_home() / "plugin-data" / _validate_name(name)
