...
- Hosted: API key from [app.supermemory.ai/integrations?connect=anika](http://app.supermemory.ai/integrations?connect=anika)
...
Before running `anika memory setup`, add the local endpoint to
...
Then run `anika memory setup` and enter the local server's API key. Configuring
the endpoint first ensures the setup connection probe also stays local.

## Config

Config file: `$ANIKA_HOME/supermemory.json`
...
| `container_tag` | `anika` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags (e.g. `anika-{identity}` → `anika-coder`). |
...
`https://api.supermemory.ai`. Anika resolves it once and uses the same endpoint
...
All Supermemory API calls send `x-sm-source: anika`, and document writes stamp
`metadata.sm_source: anika`. This is a **functional routing key, not telemetry**:
it groups Anika-written memories into a dedicated "Anika" Space in the
...
Use `{identity}` in the `container_tag` to scope memories per Anika profile:

```json
{
  "container_tag": "anika-{identity}"
}
```

For a profile named `coder`, this resolves to `anika-coder`. The default profile resolves to `anika-default`. Without `{identity}`, all profiles share the same container.
