...
> **Honcho docs:** <https://docs.honcho.dev/v3/guides/integrations/anika>
...
```bash
anika memory setup honcho   # configure Honcho directly (works on a fresh install)
anika memory setup          # generic picker, choose Honcho from the list
```
...
Or manually:
```bash
anika config set memory.provider honcho
echo "HONCHO_API_KEY=***" >> ~/.anika/.env
```

> `anika honcho setup` also works, but only **after** Honcho is the active
> memory provider — the `honcho` subcommand is registered for the active
> provider only. On a fresh install, use `anika memory setup honcho`.
...
cycle (not per pass). Select a fast, inexpensive model under `anika model`
...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `$ANIKA_HOME/honcho.json` | Profile-local (isolated Anika instances) |
| 2 | `~/.anika/honcho.json` | Default profile (shared host blocks) |
| 3 | `~/.honcho/config.json` | Global (cross-app interop) |

Host key is derived from the active Anika profile: `anika` (default) or `anika_<profile>`.
...
> **Deprecated:** `pinPeerName` is a legacy alias for `pinUserPeer`, still read for back-compat (`pinUserPeer` wins where both are set). `anika honcho setup` migrates it onto `pinUserPeer` on touch and never writes it.
...
| 4 | `per-session` strategy | Anika session ID (`20260415_a3f2b1`) |
| 5 | `per-repo` strategy | Git root directory name (`anika-agent`) |
| 6 | `per-directory` strategy | Current directory basename (`src`) |
| 7 | `global` strategy | Workspace name (`anika`) |

Gateway platforms always resolve via priority 3 (per-chat isolation) regardless of `sessionStrategy`. The strategy setting only affects CLI sessions.

If `sessionPeerPrefix` is `true`, the peer name is prepended: `alice-anika-agent`.

#### What each strategy produces

- **`per-directory`** — basename of `$PWD`. Opening anika in `~/code/myapp` and `~/code/other` gives two separate sessions. Same directory = same session across runs.
...
- **`per-session`** — Anika session ID (timestamp + hex). Every `anika` invocation starts a fresh Honcho session. Falls back to `per-directory` if no session ID is available.
- **`global`** — workspace name. One session for everything. Memory accumulates across all directories and runs.

### Multi-Profile Pattern

Multiple Anika profiles can share one workspace while maintaining separate AI identities. Config resolution is **host block > root > env var > default** — host blocks inherit from root, so shared settings only need to be declared once:
...
Both profiles see the same user (`yourname`) in the same shared environment (`anika`), but each AI peer builds its own observations, conclusions, and behavior patterns. The coder's memory stays code-oriented; the main agent's stays broad.

Host key is derived from the active Anika profile: `anika` (default) or `anika_<profile>` (e.g. `anika -p coder` -> host key `anika_coder`). Older `anika.<profile>` host blocks are still read for compatibility and are migrated when the CLI writes profile-scoped Honcho config.
...
| `ANIKA_HONCHO_HOST` | Host key override |
...
| `HONCHO_OAUTH_CLIENT_ID` | OAuth client (default `anika-agent`) |
...
|---------|-------------|
| `anika memory setup honcho` | Configure Honcho directly — works on a fresh install |
| `anika honcho setup` | Interactive setup wizard (only registered once Honcho is the active provider; redirects to `anika memory setup`) |
| `anika honcho status` | Show resolved config for active profile |
| `anika honcho enable` / `disable` | Toggle Honcho for active profile |
| `anika honcho mode <mode>` | Change recall or observation mode |
| `anika honcho peer --user <name>` | Update user peer name |
| `anika honcho peer --ai <name>` | Update AI peer name |
| `anika honcho tokens --context <N>` | Set context token budget |
| `anika honcho tokens --dialectic <N>` | Set dialectic max chars |
| `anika honcho map <name>` | Map current directory to a session name |
| `anika honcho sync` | Create host blocks for all Anika profiles |
