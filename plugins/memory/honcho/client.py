...
Config file resolution: $ANIKA_HOME/honcho.json -> ~/.honcho/config.json -> env vars
...
def profile_host_key(profile: str | None) -> str:
    """Return the safe Honcho host key for a Anika profile."""
...
def resolve_active_host() -> str:
    """Honcho host key: ANIKA_HONCHO_HOST env, else the active profile. The config's
    ``defaultHost`` is honored only for the default profile so named profiles stay isolated."""
    explicit = os.environ.get("ANIKA_HONCHO_HOST", "").strip()
    if explicit:
        return explicit
    try:
...
def resolve_config_path() -> Path:
    """Active Honcho config path: $ANIKA_HOME/honcho.json -> default profile's honcho.json
    ...
    local_path = get_anika_home() / "honcho.json"
    if local_path.exists():
        return local_path
    default_path = _get_default_anika_home() / "honcho.json"
...
def _connection_fields(look: _HostLookup, host: str, path: Path) -> dict[str, Any]:
    ...
    # read it before the flat Anika spellings.
...
class HonchoClientConfig:
    ...
    # re-resolving resolve_config_path()/get_anika_home() later — those resolvers read a ContextVar that
    ...
    @classmethod
    def from_env(cls, workspace_id: str = "anika", host: str | None = None) -> HonchoClientConfig:
        ...
        return cls(
            ...
            config_path=resolve_config_path(), anika_home=get_anika_home(),
        )
    ...
    def from_global_config(cls, host: str | None = None, config_path: Path | None = None) -> HonchoClientConfig:
        ...
        derives it from the active Anika profile."""
        resolved_host = host or resolve_active_host()
        ...
        return cls(
            ...
            config_path=path, anika_home=get_anika_home(),
        )
...
def spawn_context_thread(target, *, name: str, daemon: bool = True, args: tuple = ()) -> "_threading.Thread":
    ...
    (set_anika_home_override) and a plain Thread starts EMPTY, so ambient resolution on it
...
def get_honcho_client(config: HonchoClientConfig | None = None) -> Honcho:
    ...
    if not config.api_key and not config.base_url:
        raise ValueError("Honcho API key not found. Get your API key at https://app.honcho.dev, "
                         "then run 'anika honcho setup' or set HONCHO_API_KEY. "
...
def _build_client(config: HonchoClientConfig) -> "Honcho":
    ...
    try:
        ...
        raise ImportError("honcho-ai is required for Honcho integration. Install it with: pip install honcho-ai  "
                          "(or run `anika honcho setup` to configure).")
