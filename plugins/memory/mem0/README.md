...
```bash
anika memory setup    # select "mem0"
```
...
Behavioral settings live in `$ANIKA_HOME/mem0.json` (set them via `anika memory setup`). Only the secret `MEM0_API_KEY` belongs in `~/.anika/.env`.
...
| `user_id` | `anika-user` | User identifier on Mem0 |
...
2. Point the plugin at it — via the setup wizard:
   ...
   ```
3. Start a fresh Anika session and call `mem0_search` — it connects to your server.
...
```bash
anika memory setup mem0 --mode oss \
...
```bash
anika memory setup mem0 --mode oss --oss-llm-key sk-...
```

Or edit `$ANIKA_HOME/mem0.json` directly:
```json
{
  "mode": "oss",
  "oss": {
    ...
    "vector_store": {"provider": "qdrant", "config": {"path": "~/.anika/mem0_qdrant"}}
  }
}
```
...
```bash
anika memory setup mem0 --mode platform --api-key sk-...
```
...
- Check `user_id` matches between sessions (`$ANIKA_HOME/mem0.json`).
