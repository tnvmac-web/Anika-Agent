...
- OpenViking server running and reachable from Anika

OpenViking 0.2.10 or newer is recommended. For backward compatibility,
Anika can identify older servers that expose the legacy status-only health
...
connection values into Anika, or create a minimal `ovcli.conf` when one does
not exist.
...
default profile that is `~/.anika/.env`; for a named profile use
...
OpenViking's server config is separate from Anika:
...
Anika-side provider config is read from environment variables in the active
profile's `.env`:
...
When `OPENVIKING_API_KEY` is set, Anika lets OpenViking derive account/user
identity from the key. In local or trusted deployments without an API key,
Anika sends `OPENVIKING_ACCOUNT` and `OPENVIKING_USER` as identity headers.
Anika also sends `User-Agent: openviking-memory-anika/<version>` on
OpenViking requests. This standard harness identifier contains the Anika
...
does not ask for a peer ID. Without a configured peer, Anika sends neither
...
order remains environment, linked OpenViking config, then Anika YAML. To use
...
on the old implicit `anika` peer now use user memory for new writes. Without
...
Set `agent: anika` to restore peer-scoped writes. Memories written at user
...
`viking_remember` creates a one-shot `anika-remember-<random>` OpenViking
...
not advertised or used. The one-shot session is separate from the live Anika
...
Use the same OpenViking profile and credentials as Anika for manual recovery.
...
Anika built-in `memory` tool additions are mirrored to OpenViking after the
local memory operation succeeds:

| Anika action | OpenViking operation |
|---------------|----------------------|
| `add` | `content/write` with `mode=create` under user memory, or the configured peer memory directory |

Built-in `replace` and `remove` operations are not mirrored because Anika
...
file URIs, such as
`viking://user/default/peers/anika/memories/preferences/mem_abc123.md` (any
