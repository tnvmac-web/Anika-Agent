...
catalog, filter to the ``anika`` integration, apply a manifest via
...
def fetch_anika_templates(url: str | None = None) -> list[dict]:
    """Return catalog entries tagged for the ``anika`` integration."""
    catalog = _get_json(url or catalog_url())
    entries = catalog.get("templates", []) if isinstance(catalog, dict) else []
    return [e for e in entries if "anika" in (e.get("integrations") or [])]
