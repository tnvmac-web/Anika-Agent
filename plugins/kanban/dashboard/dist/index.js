/**
 * Anika Kanban — Dashboard Plugin
 *
 * Board view for the multi-agent collaboration board backed by
 * ~/.anika/kanban.db. Calls the plugin's backend at /api/plugins/kanban/
 * and tails task_events over a WebSocket for live updates.
 *
 * Plain IIFE, no build step. Uses window.__ANIKA_PLUGIN_SDK__ for React +
...
(function () {
  ...
  const SDK = window.__ANIKA_PLUGIN_SDK__;
  if (!SDK) return;
  ...
  const COLUMN_DOT = {
    triage: "anika-kanban-dot-triage",
    todo: "anika-kanban-dot-todo",
    ready: "anika-kanban-dot-ready",
    running: "anika-kanban-dot-running",
    blocked: "anika-kanban-dot-blocked",
    review: "anika-kanban-dot-review",
    done: "anika-kanban-dot-done",
    archived: "anika-kanban-dot-archived",
  };
  ...
  const MIME_TASK = "text/x-anika-task";
  ...
  const DOCS_URL = "https://anika-agent.__NEW_DOMAIN__/docs/user-guide/features/kanban";
  const DOCS_TUTORIAL_URL = "https://anika-agent.__NEW_DOMAIN__/docs/user-guide/features/kanban-tutorial";
  ...
  const LS_BOARD_KEY = "anika.kanban.selectedBoard";
  ...
  function renderMarkdown(src) {
    ...
    html = html.replace(/\u0000CODE(\d+)\u0000/g, (_m, i) =>
      `<pre class="anika-kanban-md-code"><code>${escapeHtml(blocks[Number(i)])}</code></pre>`,
    );
    return html;
  }
  ...
  function sanitizeMarkdownAttrs(tag, attrs) {
    ...
    if (tag === "pre" && /\sclass=(["'])anika-kanban-md-code\1/i.test(attrs)) {
      return ' class="anika-kanban-md-code"';
    }
    return "";
  }
  ...
  function MarkdownBlock(props) {
    ...
    if (!enabled) {
      return h("pre", { className: "anika-kanban-pre" }, props.source || "");
    }
    return h("div", {
      className: "anika-kanban-md",
  ...
  // standard `drop` event and our `anika-kanban:drop` event.
  // -------------------------------------------------------------------------
  ...
  function attachTouchDrag(el, taskId) {
    ...
    function onDown(e) {
      ...
      proxy.classList.add("anika-kanban-touch-proxy");
      ...
      function move(ev) {
        ...
        if (target !== lastTarget) {
          if (lastTarget) lastTarget.classList.remove("anika-kanban-column--drop");
          if (target) target.classList.add("anika-kanban-column--drop");
          lastTarget = target;
        }
      }
      function up() {
        ...
        if (lastTarget) {
          lastTarget.classList.remove("anika-kanban-column--drop");
          const status = lastTarget.getAttribute("data-kanban-column");
          const isTrash = lastTarget.hasAttribute("data-kanban-trash");
          if (isTrash) {
            lastTarget.dispatchEvent(new CustomEvent("anika-kanban:delete", {
              detail: { taskId },
              bubbles: true,
            }));
          } else if (status) {
            lastTarget.dispatchEvent(new CustomEvent("anika-kanban:drop", {
  ...
  function KanbanPage() {
    ...
    useEffect(function () {
      ...
      function openWs() {
        ...
        // loopback. Reading window.__ANIKA_SESSION_TOKEN__ directly (the old
   ...
   const deleteTask = useCallback(function (taskId) {
    ...
    return h(ErrorBoundary, null,
      h("div", { className: "anika-kanban flex flex-col gap-4" },
        h(BoardSwitcher, {
          board: board,
          boardList: boardList,
  ...
  function AttentionStrip(props) {
    ...
    return h("div", {
      className: cn(
        "anika-kanban-attention",
        "anika-kanban-attention--" + topSev,
      ),
    },
      h("div", { className: "anika-kanban-attention-bar" },
        h("span", { className: "anika-kanban-attention-icon" },
          topSev === "critical" ? "!!!" : topSev === "error" ? "!!" : "⚠"),
        h("span", { className: "anika-kanban-attention-text" },
          diagTasks.length === 1
      ...
      expanded
        ? h("div", { className: "anika-kanban-attention-list" },
            diagTasks.map(function (task) {
              ...
              return h("div", {
                key: task.id,
                className: cn(
                  "anika-kanban-attention-row",
                  "anika-kanban-attention-row--" + sev,
                ),
              },
                h("span", { className: "anika-kanban-attention-row-sev" },
                  sev === "critical" ? "!!!" : sev === "error" ? "!!" : "⚠"),
                h("span", { className: "anika-kanban-attention-row-id" }, task.id),
                h("span", { className: "anika-kanban-attention-row-title" },
                  task.title || tx(t, "untitled", "(untitled)")),
                h("span", { className: "anika-kanban-attention-row-meta" },
                  task.assignee ? "@" + task.assignee : tx(t, "unassigned", "unassigned"),
                  " \u00b7 ",
                  kinds.length > 0 ? kinds.join(", ") : tx(t, "diagnostic", "diagnostic"),
                ),
                h("button", {
                  className: "anika-kanban-attention-row-btn",
  ...
  function DiagnosticActionButton(props) {
    ...
    const cls = cn(
      "anika-kanban-diag-action-btn",
      action.suggested ? "anika-kanban-diag-action-btn--suggested" : "",
    );
    ...
    return h("span", { className: cls + " anika-kanban-diag-action-btn--unknown" },
      label);
  }
  ...
  function DiagnosticCard(props) {
    ...
    const execAction = function (action) {
      ...
      if (action.kind === "comment") {
        ...
        const ta = document.querySelector(".anika-kanban-drawer-comment-row input, .anika-kanban-drawer-comment-row textarea");
        if (ta) {
    ...
    const sevClass = "anika-kanban-diag--" + (diag.severity || "warning");
    return h("div", { className: cn("anika-kanban-diag", sevClass) },
      h("div", { className: "anika-kanban-diag-header" },
        h("span", { className: "anika-kanban-diag-sev" },
          diag.severity === "critical" ? "!!!" :
          diag.severity === "error" ? "!!" : "\u26a0"),
        h("span", { className: "anika-kanban-diag-title" },
          diag.title),
      ),
      h("div", { className: "anika-kanban-diag-detail" },
        diag.detail),
      diag.data && Object.keys(diag.data).length > 0
        ? h("div", { className: "anika-kanban-diag-data" },
            Object.keys(diag.data).map(function (k) {
              const v = diag.data[k];
              if (Array.isArray(v) && v.length > 0 && typeof v[0] === "string" &&
                  v[0].indexOf("t_") === 0) {
                // Task-id list — render as chips.
                return h("div", { key: k, className: "anika-kanban-diag-data-row" },
                  h("span", { className: "anika-kanban-diag-data-key" }, k + ":"),
                  v.map(function (x) {
                    return h("code", {
                      key: x, className: "anika-kanban-event-phantom-chip",
                    }, x);
                  }),
                );
              }
              return h("div", { key: k, className: "anika-kanban-diag-data-row" },
                h("span", { className: "anika-kanban-diag-data-key" }, k + ":"),
                h("span", { className: "anika-kanban-diag-data-val" },
      ...
      reassignAction
        ? h("div", { className: "anika-kanban-diag-reassign-row" },
            h("span", { className: "anika-kanban-diag-reassign-label" },
              tx(t, "reassignTo", "Reassign to:")),
            h("select", {
              className: "anika-kanban-recovery-select",
        ...
        : null,
      h("div", { className: "anika-kanban-diag-actions" },
      ...
      msg
        ? h("div", {
            className: cn(
              "anika-kanban-diag-msg",
              msg.ok ? "anika-kanban-diag-msg--ok" : "anika-kanban-diag-msg--err",
            ),
          }, msg.text)
        : null,
    );
  ...
  function DiagnosticsSection(props) {
    ...
    return h("div", { className: "anika-kanban-section" },
      h("div", { className: "anika-kanban-section-head-row" },
        h("span", { className: "anika-kanban-section-head" },
          hasOpenDiags
            ? h("span", { className: "anika-kanban-section-head-warning" },
                `\u26a0 ${tx(t, "diagnostics", "Diagnostics")} (${diags.length})`)
            : tx(t, "diagnostics", "Diagnostics"),
        ),
        h("button", {
          className: "anika-kanban-section-toggle",
          onClick: function () { setOpen(function (x) { return !x; }); },
          type: "button",
        }, open ? tx(t, "hide", "Hide") : tx(t, "show", "Show")),
      ),
      open
        ? h("div", { className: "anika-kanban-diag-list" },
  ...
  function DocsLink() {
    return h("a", {
      ...
      className: "anika-kanban-docs-link",
      title: "Open Anika Kanban docs in a new tab",
      "aria-label": "Anika Kanban documentation",
    }, "?");
  }
  ...
  function OrchestrationPanel() {
    ...
    // each triage card (or runs `anika kanban decompose <id>`) and tasks
    ...
    return h(Card, { className: "p-3" },
      h(CardContent, { className: "p-2 flex flex-col gap-3" },
        ...
        msg ? h("div", {
          className: msg.ok ? "anika-kanban-msg-ok" : "anika-kanban-msg-err",
        }, msg.text) : null,
  ...
  function BoardSwitcher(props) {
    ...
    if (!shouldShow) {
      return h("div", {
        className: "anika-kanban-boardswitcher-compact",
    ...
    return h("div", { className: "anika-kanban-boardswitcher" },
      h("div", { className: "anika-kanban-boardswitcher-inner" },
  ...
  function NewBoardDialog(props) {
    ...
    return h("div", {
      className: "anika-kanban-dialog-backdrop",
      onClick: function (e) { if (e.target === e.currentTarget) props.onCancel(); },
    },
      h("form", {
        className: "anika-kanban-dialog",
        onSubmit: onSubmit,
      },
        h("div", { className: "anika-kanban-dialog-title" },
        ...
        h("div", { className: "anika-kanban-dialog-actions" },
          h(Button, {
            type: "button",
  ...
  function BoardToolbar(props) {
    ...
    return h("div", { className: "flex flex-wrap items-end gap-3" },
      ...
      h("div", { className: "flex flex-col gap-1",
                 title: "Filter by assigned Anika profile. Profiles are the named agent identities that claim and work on tasks." },
  ...
  function BulkActionBar(props) {
    ...
    return h("div", { className: "anika-kanban-bulk" },
      h("span", { className: "anika-kanban-bulk-count" },
      ...
      h("div", { className: "anika-kanban-bulk-priority",
      ...
      h("div", { className: "anika-kanban-bulk-reassign",
                 title: "Reassign selected tasks to a different Anika profile. Pick a profile (or unassign) and click Apply." },
        h(Select, Object.assign({
          value: assignee,
          className: "h-7 text-xs",
      ...
      h("label", { className: "anika-kanban-bulk-reclaim-first", title: "Reclaim any active claims before reassigning" },
        h(Checkbox, {
  ...
  function TrashDropZone(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("anika-kanban:delete", onTouchDelete);
      return function () { el.removeEventListener("anika-kanban:delete", onTouchDelete); };
    }, [props.onDelete]);
    ...
    return h("div", {
      ...
      className: cn(
        "anika-kanban-trash",
        dragOver ? "anika-kanban-trash--drop" : "",
        props.draggingTaskId ? "anika-kanban-trash--active" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
    },
      h("span", { className: "anika-kanban-trash-icon" }, "🗑️"),
      h("span", { className: "anika-kanban-trash-label" },
  ...
  function BoardColumns(props) {
    ...
    const isPanBlockedTarget = useCallback(function (target) {
      ...
      if (target.closest && target.closest(".anika-kanban-card")) return true;
      if (target.closest && target.closest(".anika-kanban-column-add")) return true;
      if (target.closest && target.closest(".anika-kanban-col-check")) return true;
    ...
    const stopPan = useCallback(function () {
      ...
      if (el) {
        ...
        el.classList.remove("anika-kanban-columns--panning");
        el.style.userSelect = "";
      }
    ...
    const handleMouseDown = useCallback(function (e) {
      ...
      setIsPanning(true);
      el.classList.add("anika-kanban-columns--panning");
    ...
    const handleDragStart = useCallback(function (e) {
      const card = e.target.closest && e.target.closest(".anika-kanban-card");
      if (!card) return;
    ...
    return h("div", {
      ref: columnsRef,
      className: cn(
        "anika-kanban-columns",
        isScrollable ? "anika-kanban-columns--scrollable" : "",
        isPanning ? "anika-kanban-columns--panning" : "",
      ),
      onDragStart: handleDragStart,
      onDragEnd: handleDragEnd,
  ...
  function Column(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("anika-kanban:drop", onTouchDrop);
      return function () { el.removeEventListener("anika-kanban:drop", onTouchDrop); };
    ...
    return h("div", {
      ...
      className: cn(
        "anika-kanban-column",
        dragOver ? "anika-kanban-column--drop" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
    },
      h("div", { className: "anika-kanban-column-header",
                 title: colHelp || "" },
        h(Checkbox, {
          className: "anika-kanban-col-check",
        ...
        h("span", { className: cn("anika-kanban-dot", COLUMN_DOT[props.column.name]) }),
        h("span", { className: "anika-kanban-column-label" },
          colLabel || props.column.name),
        h("span", { className: "anika-kanban-column-count",
                    title: `${props.column.tasks.length} task${props.column.tasks.length === 1 ? "" : "s"} in this column` },
          props.column.tasks.length),
        h("button", {
          type: "button",
          className: "anika-kanban-column-add",
          title: tx(t, "createTask", "Create task in this column"),
          onClick: function () { setShowCreate(function (v) { return !v; }); },
        }, showCreate ? "×" : "+"),
      ),
      h("div", { className: "anika-kanban-column-sub" },
        colHelp || ""),
      ...
      h("div", { className: "anika-kanban-column-body" },
        props.column.tasks.length === 0
          ? h("div", { className: "anika-kanban-empty" }, tx(t, "noTasks", "— no tasks —"))
          : lanes
            ? lanes.map(function (lane) {
                return h("div", { key: lane.assignee, className: "anika-kanban-lane" },
                  h("div", { className: "anika-kanban-lane-head" },
                    h("span", { className: "anika-kanban-lane-name" }, lane.assignee),
                    h("span", { className: "anika-kanban-lane-count" }, lane.tasks.length),
                  ),
  ...
  function stalenessClass(task) {
    ...
    if (age >= tier.red)   return "anika-kanban-card--stale-red";
    if (age >= tier.amber) return "anika-kanban-card--stale-amber";
    return "";
  }
  ...
  function TaskCard(props) {
    ...
    const handleDragStart = function (e) {
      ...
      const selectedCards = document.querySelectorAll(".anika-kanban-card--selected");
      if (selectedCards.length > 1 && props.selected) {
        const ghost = document.createElement("div");
        ghost.className = "anika-kanban-drag-ghost";
    ...
    return h("div", {
      ...
      className: cn(
        "anika-kanban-card",
        props.selected ? "anika-kanban-card--selected" : "",
        props.failed ? "anika-kanban-card--failed" : "",
        props.draggingSource ? "anika-kanban-card--dragging-source" : "",
        stalenessClass(t),
      ),
      draggable: true,
      tabIndex: 0,
      ...
      h(Card, null,
        h(CardContent, { className: "anika-kanban-card-content" },
          h("div", { className: "anika-kanban-card-row" },
            h("label", {
              className: "anika-kanban-card-check-wrap",
              title: tx(i18n, "selectForBulk", "Select for bulk actions"),
              onClick: function (e) { e.stopPropagation(); },
            },
              h(Checkbox, {
                className: "anika-kanban-card-check",
            ...
            h("span", { className: "anika-kanban-card-id",
                        title: `Task id: ${t.id}. Use this id with kanban_show, /kanban show, or anika kanban show.` }, t.id),
            t.warnings && t.warnings.count > 0
              ? h("span", {
                  className: cn(
                    "anika-kanban-warning-badge",
                    "anika-kanban-warning-badge--" + (t.warnings.highest_severity || "warning"),
                  ),
                  title: (
            ...
            needsAssignee
              ? h(Badge, {
                  variant: "outline",
                  className: "anika-kanban-needs-assignee",
          ...
          h("div", { className: "anika-kanban-card-title" },
            t.title || tx(i18n, "untitled", "(untitled)")),
          h("div", { className: "anika-kanban-card-row anika-kanban-card-meta" },
            t.assignee
              ? h("span", { className: "anika-kanban-assignee",
                            title: `Assigned to Anika profile @${t.assignee}` }, "@", t.assignee)
              : h("span", { className: "anika-kanban-unassigned",
                            title: needsAssignee
            ...
            t.comment_count > 0
              ? h("span", { className: "anika-kanban-count",
            ...
            t.link_counts && (t.link_counts.parents + t.link_counts.children) > 0
              ...
              : null,
            h("span", { className: "anika-kanban-ago",
  ...
  // centered modal reusing the anika-kanban-dialog chrome so the form is
  ...
  function InlineCreate(props) {
    ...
    return h("div", {
      ...
      h("form", {
        className: "anika-kanban-dialog anika-kanban-create-dialog",
            ...
            h("div", { className: "flex flex-col gap-1 flex-1" },
              ...
              h(Input, {
                ...
                title: props.columnName === "triage"
                  ? "Anika profile that will spec this task (default: the dispatcher's configured specifier). Leave blank to let the dispatcher pick."
                  : "Anika profile to assign. Leave blank and the dispatcher will pick from available profiles when the task is Ready.",
  ...
  function TaskDrawer(props) {
    ...
    return h("div", { className: "anika-kanban-drawer-shade", onClick: props.onClose },
      h("div", {
        className: "anika-kanban-drawer",
        onClick: function (e) { e.stopPropagation(); },
      },
        h("div", { className: "anika-kanban-drawer-head" },
          h("span", { className: "text-xs text-muted-foreground" }, props.taskId),
          h("button", {
            type: "button",
            onClick: props.onClose,
            className: "anika-kanban-drawer-close",
        ...
        data ? h("div", { className: "anika-kanban-drawer-comment-foot" },
          h("div", {
            className: "anika-kanban-comment-hint text-xs text-muted-foreground",
          ...
          h("div", { className: "anika-kanban-drawer-comment-row" },
            h(Input, {
              value: newComment,
  ...
  function AttachmentsSection(props) {
    ...
    return h("div", { className: "anika-kanban-section" },
      h("div", { className: "anika-kanban-section-head" },
      ...
      atts.length === 0
        ...
        : atts.map(function (a) {
            return h("div", {
              ...
              h("button", {
                type: "button",
                className: "anika-kanban-attachment-link truncate",
                title: a.filename,
  ...
  function TaskDetail(props) {
    ...
    return h("div", { className: "anika-kanban-drawer-body" },
      h("div", { className: "anika-kanban-drawer-title" },
        h("span", { className: cn("anika-kanban-dot", COLUMN_DOT[t.status]) }),
        props.editing
          ? h(TitleEditor, {
              initial: t.title || "",
          ...
          : h("span", {
              className: "anika-kanban-drawer-title-text",
              title: tx(i18n, "clickToEdit", "Click to edit"),
              onClick: function () { props.setEditing(true); },
            }, t.title || tx(i18n, "untitled", "(untitled)")),
      ),
      h("div", { className: "anika-kanban-drawer-meta" },
      ...
      (function () {
        ...
        if (finalResult) {
          ...
          return h("div", { className: "anika-kanban-section" },
            h("div", { className: "anika-kanban-section-head" }, label),
            h(MarkdownBlock, { source: finalResult, enabled: props.renderMarkdown }),
          );
        }
        if (isDone && isParent) {
          return h("div", { className: "anika-kanban-section" },
            h("div", { className: "anika-kanban-section-head" }, tx(i18n, "result", "Result")),
            h("div", { className: "anika-kanban-done-no-result anika-kanban-done-parent-note" },
        ...
        if (isDone) {
          return h("div", { className: "anika-kanban-section" },
            h("div", { className: "anika-kanban-section-head" }, tx(i18n, "result", "Result")),
            h("div", { className: "anika-kanban-done-no-result" },
      ...
      childResults.length > 0 ? h("div", { className: "anika-kanban-section" },
        h("div", { className: "anika-kanban-section-head" },
        ...
        childResults.map(function (child) {
          ...
          return h("div", { key: child.id, className: "anika-kanban-comment" },
            h("div", { className: "anika-kanban-comment-head" },
              h("span", { className: "anika-kanban-comment-author" },
                `${child.id} · ${child.title || tx(i18n, "untitled", "(untitled)")}`),
              h(Badge, { variant: "outline" }, child.status),
              h("button", {
                type: "button",
                className: "anika-kanban-diag-action-btn",
      ...
      h("div", { className: "anika-kanban-section" },
        h("div", { className: "anika-kanban-section-head" },
        ...
        comments.length === 0
          ...
          : comments.map(function (c) {
              return h("div", { key: c.id, className: "anika-kanban-comment" },
                h("div", { className: "anika-kanban-comment-head" },
                  h("span", { className: "anika-kanban-comment-author" }, c.author || "anon"),
                  h("span", { className: "anika-kanban-comment-ago" },
      ...
      h("div", { className: "anika-kanban-section" },
        ...
        events.slice().reverse().slice(0, 20).map(function (e) {
          ...
          return h("div", {
            ...
            className: cn(
              "anika-kanban-event",
              isDiag ? "anika-kanban-event--hallucination" : "",
            ),
          },
            isDiag
              ? h("div", { className: "anika-kanban-event-header" },
                  h("span", { className: "anika-kanban-event-warning-icon" }, "⚠"),
                  h("span", { className: "anika-kanban-event-warning-label" },
                    getDiagnosticEventLabel(i18n, e.kind) || e.kind),
                  h("span", { className: "anika-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                )
              : h("div", { className: "anika-kanban-event-header-plain" },
                  h("span", { className: "anika-kanban-event-kind" }, e.kind),
                  h("span", { className: "anika-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                ),
            isDiag && phantoms.length > 0
              ? h("div", { className: "anika-kanban-event-phantom-row" },
                  h("span", { className: "anika-kanban-event-phantom-label" },
                    tx(i18n, "phantomIds", "Phantom ids:")),
                  phantoms.map(function (pid) {
                    return h("code", {
                      key: pid,
                      className: "anika-kanban-event-phantom-chip",
                    }, pid);
                  }),
                )
              : null,
            e.payload && !isDiag
              ? h("code", { className: "anika-kanban-event-payload" },
    ...
    return h("div", { className: "anika-kanban-section" },
      h("div", { className: "anika-kanban-section-head-row" },
        ...
        !showAll
          ? h("button", {
              type: "button",
              onClick: function () { setExpanded(true); },
              className: "anika-kanban-edit-link",
      ...
      visible.map(function (r) {
        const outcomeClass = r.ended_at
          ? `anika-kanban-run--${r.outcome || r.status || "ended"}`
          : "anika-kanban-run--active";
        return h("div", { key: r.id, className: cn("anika-kanban-run", outcomeClass) },
          h("div", { className: "anika-kanban-run-head" },
            h("span", { className: "anika-kanban-run-outcome" },
              r.ended_at ? (r.outcome || r.status || tx(t, "ended", "ended")) : tx(t, "active", "active")),
            h("span", { className: "anika-kanban-run-profile" },
              r.profile ? `@${r.profile}` : tx(t, "noProfile", "(no profile)")),
            h("span", { className: "anika-kanban-run-elapsed" }, fmtElapsed(r)),
            h("span", { className: "anika-kanban-run-ago" },
              timeAgo ? timeAgo(r.started_at) : ""),
          ),
          r.summary
            ? h("div", { className: "anika-kanban-run-summary" }, r.summary)
            : null,
          r.error
            ? h("div", { className: "anika-kanban-run-error" }, r.error)
            : null,
          (r.metadata && Object.keys(r.metadata).length > 0)
            ? (function () {
                ...
                return h("details", {
                    className: "anika-kanban-run-meta-block",
                    open: !collapsed,
                  },
                  h("summary", { className: "anika-kanban-run-meta-label" }, "Metadata"),
                  h("code", { className: "anika-kanban-run-meta" }, json),
                );
              })()
            : null,
        );
  ...
  function WorkerLogSection(props) {
    ...
    if (state.loading) {
      ...
      body = h("pre", { className: "anika-kanban-pre anika-kanban-log" },
        data.content || "(empty)");
    }
  ...
  function MetaRow(props) {
    return h("div", { className: "anika-kanban-meta-row" },
      h("span", { className: "anika-kanban-meta-label" }, props.label),
      h("span", { className: "anika-kanban-meta-value" }, props.value),
    );
  }
  ...
  function TitleEditor(props) {
    ...
    return h("div", { className: "anika-kanban-edit-row" },
      h(Input, {
  ...
  function AssigneeEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "anika-kanban-meta-row" },
        h("span", { className: "anika-kanban-meta-label" }, tx(t, "assignee", "Assignee")),
        h("span", {
          className: "anika-kanban-meta-value anika-kanban-editable",
  ...
  function PriorityEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "anika-kanban-meta-row" },
        h("span", { className: "anika-kanban-meta-label" }, tx(t, "priority", "Priority")),
        h("span", {
          className: "anika-kanban-meta-value anika-kanban-editable",
          onClick: function () { setEditing(true); },
  ...
  function ModelEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "anika-kanban-meta-row" },
        h("span", { className: "anika-kanban-meta-label" }, tx(t, "model", "Model")),
        h("span", {
          className: cn(
            "anika-kanban-meta-value anika-kanban-editable",
    ...
    return h("div", { className: "anika-kanban-meta-row" },
      h("span", { className: "anika-kanban-meta-label" }, tx(t, "model", "Model")),
      loading
        ? h("span", { className: "anika-kanban-meta-value text-muted-foreground" },
  ...
  function BodyEditor(props) {
    ...
    return h("div", { className: "anika-kanban-section" },
      h("div", { className: "anika-kanban-section-head-row" },
        h("span", { className: "anika-kanban-section-head" }, tx(t, "description", "Description")),
        editing
      ...
      editing
        ? h("textarea", {
            className: "anika-kanban-textarea",
            value: v,
            rows: 8,
            onChange: function (e) { setV(e.target.value); },
          })
  ...
  function DependencyEditor(props) {
    ...
    return h("div", { className: "anika-kanban-section" },
      h("div", { className: "anika-kanban-section-head" }, tx(t, "dependencies", "Dependencies")),
      h("div", { className: "anika-kanban-deps-row" },
        h("span", { className: "anika-kanban-deps-label" }, tx(t, "parents", "Parents:")),
        h("div", { className: "anika-kanban-deps-chips" },
          (links.parents || []).length === 0
            ? h("span", { className: "anika-kanban-deps-empty" }, tx(t, "none", "none"))
            : (links.parents || []).map(function (id) {
                return h("span", { key: id, className: "anika-kanban-dep-chip" },
                  id,
                  h("button", {
                    type: "button",
                    className: "anika-kanban-dep-chip-x",
      ...
      h("div", { className: "anika-kanban-deps-row" },
        h("span", { className: "anika-kanban-deps-label" }, tx(t, "children", "Children:")),
        h("div", { className: "anika-kanban-deps-chips" },
  ...
  function StatusActions(props) {
    ...
    return h("div", null,
      h("div", { className: "anika-kanban-actions" },
        specifyButton,
        decomposeButton,
  ...
  function HomeSubsSection(props) {
    ...
    return h("div", { className: "anika-kanban-section" },
      ...
      h("div", { className: "anika-kanban-home-subs" },
        channels.map(function (hc) {
          ...
          return h(Button, {
            ...
            className: hc.subscribed
              ? "anika-kanban-home-sub anika-kanban-home-sub--on"
              : "anika-kanban-home-sub",
          }, label);
        })
      )
    );
  ...
  if (window.__ANIKA_PLUGINS__ && typeof window.__ANIKA_PLUGINS__.register === "function") {
    window.__ANIKA_PLUGINS__.register("kanban", KanbanPage);
  }
})();
