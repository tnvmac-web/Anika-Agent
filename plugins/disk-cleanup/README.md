...
Auto-tracks and cleans up ephemeral files created during Anika Agent
sessions — test scripts, temp outputs, cron logs, stale chrome profiles.
Scoped strictly to `$ANIKA_HOME` and `/tmp/anika-*`.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `ANIKA_HOME`, track it silently as `test` / `temp` / `cron-output`. |
...
- `is_safe_path()` rejects anything outside `ANIKA_HOME` or `/tmp/anika-*`
- Windows mounts (`/mnt/c` etc.) are rejected
- The state directory `$ANIKA_HOME/disk-cleanup/` is itself excluded
- `$ANIKA_HOME/logs/`, `memories/`, `sessions/`, `skills/`, `plugins/`,
