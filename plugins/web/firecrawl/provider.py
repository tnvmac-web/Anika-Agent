"""Firecrawl web search + extract provider (direct SDK, keyless cloud, or __NEW_ORG__ tool-gateway).
...
def _is_tool_gateway_ready() -> bool:
    """True when gateway URL + __NEW_ORG__ Subscriber token are available."""
    return _gateway.resolve_managed_tool_gateway("firecrawl", token_reader=_gateway.peek_nous_access_token) is not None


def check_firecrawl_api_key() -> bool:
    """True when the route selected via ``anika tools`` (or, on a never-configured
...
def _firecrawl_backend_help_suffix() -> str:
    return ", or use the __NEW_ORG__ Tool Gateway via your subscription (FIRECRAWL_GATEWAY_URL or TOOL_GATEWAY_DOMAIN)" if _backend_helpers.managed_nous_tools_enabled() else ""
...
def _get_firecrawl_client() -> Any:
    ...
    def _unconfigured_message() -> str:
        ...
        if _backend_helpers.managed_nous_tools_enabled():
            return message + " With your __NEW_ORG__ subscription you can also use the Tool Gateway. run `anika tools` and select __NEW_ORG__ Subscription as the web provider."
    ...
    if selected == NOUS_MANAGED_PROVIDER:
        resolved, log, message = _managed(), "the __NEW_ORG__ Subscription web selection is stored but the tool gateway is unavailable.", lambda: selection_error(
            "web", NOUS_MANAGED_PROVIDER, "the __NEW_ORG__ Tool Gateway is not available (not entitled or unreachable)")
...
class FirecrawlWebSearchProvider(BaseWebSearchProvider):
    ...
    def get_setup_schema(self) -> Dict[str, Any]:
        return setup_schema(
            ...
            "Full search + extract; supports keyless cloud, direct API, and __NEW_ORG__ tool-gateway routing.",
