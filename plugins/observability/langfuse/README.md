...
This plugin ships bundled with Anika but is **opt-in** — it only loads when
...
anika tools  # → Langfuse Observability
...
Set these in `~/.anika/.env` (or via `anika tools`):
...
anika chat -q "hello"              # then check Langfuse for a "Anika turn" trace
```

Generation observations include the Anika system prompt when the provider
uses a separate `system` param (Anthropic Messages API). Open an **LLM call**
child span to inspect `role: system` (truncated via `ANIKA_LANGFUSE_MAX_CHARS`).

## Optional tuning

```bash
ANIKA_LANGFUSE_ENV=production       # environment tag
ANIKA_LANGFUSE_RELEASE=v1.0.0       # release tag
ANIKA_LANGFUSE_SAMPLE_RATE=0.5      # sample 50% of traces
ANIKA_LANGFUSE_MAX_CHARS=12000      # max chars per field (default: 12000)
ANIKA_LANGFUSE_CAPTURE=sanitized    # content capture mode (see below)
ANIKA_LANGFUSE_DEBUG=true           # verbose plugin logging
```

## Capture modes

`ANIKA_LANGFUSE_CAPTURE` controls how much *content* (prompts, responses,
...
```bash
anika plugins disable observability/langfuse
```
