...
(enabled/policy/allow_from) falling back independently. Config ~/.anika/feishu_comment_rules.json (mtime-cached,
hot-reload); pairing store ~/.anika/feishu_comment_pairing.json."""
...
# long after profile/ANIKA_HOME overrides have been applied, so freezing is safe.
RULES_FILE = get_anika_home() / "feishu_comment_rules.json"
PAIRING_FILE = get_anika_home() / "feishu_comment_pairing.json"
...
def _main() -> int:
    try:
        __import__("anika_cli.env_loader", fromlist=["load_anika_dotenv"]).load_anika_dotenv()
    except Exception:
        pass
