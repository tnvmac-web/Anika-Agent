// Anika Agent — Photon Spectrum sidecar
//
...
//
// Inbound  (gRPC -> Anika): the SDK's `app.messages` async iterator is a
...
//   gateway connects.
// Outbound (Anika -> gRPC): `/send` drives `space.send(...)`; `/typing`
//   sends the documented `typing("start" | "stop")` content builder.
//
// Protocol (all requests require `X-Anika-Sidecar-Token: ${TOKEN}`):
...
//                          default off — toggle with `anika photon telemetry`)
  ...
  streamRestartTimer = setTimeout(() => {
    ...
    console.error(
      `photon-sidecar: upstream stream degraded for ${degradedForMs}ms; ` +
        "exiting so Anika can restart the Photon adapter"
    );
    process.exit(75);
...
const PROBE_MSG_PREFIX = "anika-liveness-probe-";
...
// instead of a cryptic module-resolution error during import. Apply Anika'
...
function inboundStreamErrorMessage(e) {
  ...
  // iMessage internal error. Local Anika allowlists cannot cause or fix this:
  ...
  if (
    ...
    out +=
      ...
      "this is upstream of Anika, so inbound iMessages may not be delivered " +
...
const server = http.createServer(async (req, res) => {
  if (!tokenOk(req.headers["x-anika-sidecar-token"])) {
  ...
  try {
    ...
    if (req.url === "/send-attachment") {
      ...
      // overrides only when Anika supplied them so a known-good
