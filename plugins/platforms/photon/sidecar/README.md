...
Small Node helper that bridges Anika Agent to Photon's Spectrum SDK
(`spectrum-ts`).  Anika is Python; Photon has no public HTTP
...
- exposes a loopback-only HTTP control channel for the Python adapter
  to push send/typing requests (auth via `X-Anika-Sidecar-Token`)
- drains the inbound message stream so `spectrum-ts` keeps its
  reconnect/heartbeat machinery alive and Anika can receive inbound messages
...
The Anika plugin's `anika photon setup` command runs `npm install`
here automatically.
...
`Space.send(...)` API. Anika is Python, so replies go through this sidecar
