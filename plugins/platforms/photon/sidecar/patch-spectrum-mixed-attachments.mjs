...
// `message.content.text` before Anika can see it. We rewrite the two inbound
...
const MARKER = "Anika patch: Preserve mixed text + attachment iMessage payloads";
...
export function patchSpectrumTs(root = scriptDir()) {
  ...
  for (const file of files) {
    ...
    // Anika to patch; keep the legacy v8 path below for older pinned installs.
    if (
