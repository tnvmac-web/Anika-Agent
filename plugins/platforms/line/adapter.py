...
class LineAdapter(BasePlatformAdapter):
    ...
    async def _handle_media(self, request) -> Any:
        ...
        path is rechecked against allowed roots (tempdir, ``/tmp``→``/private/tmp`` on macOS, ANIKA_HOME).
        ...
        try:
            ...
            anika_home = Path(get_anika_home()).resolve()
        except Exception:
            anika_home = Path.home().joinpath(".anika").resolve()
        resolved = path.resolve()
        if not any(resolved.is_relative_to(r) for r in (Path(tempfile.gettempdir()).resolve(), Path("/tmp").resolve(), anika_home)):
...
def is_connected(config) -> bool:
    """Surface in ``anika status`` even before the adapter is instantiated."""
    return validate_config(config)


def _env_enablement() -> Optional[Dict[str, Any]]:
    """Seed PlatformConfig.extra from env-only setups so ``anika status`` sees them."""
    if not _env_credentials_present():
        return None
    seeded: Dict[str, Any] = {}
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``anika setup line`` (writes ``~/.anika/.env``)."""
    print("\nLINE Messaging API setup\n------------------------\n"
          "Create a Messaging API channel at https://developers.line.biz/console/\nthen copy the values below.\n")
    try:
        from anika_cli.config import get_env_value as _get_env, save_env_value as _set_env
    except ImportError:
        print("anika_cli.config not available; set LINE_* vars manually in ~/.anika/.env")
        return
