...
ANIKA_SIMPLEX_TEXT_BATCH_DELAY (quiet seconds, default 0.8, merging rapid-fire inbound text).
...
_CORR_PREFIX = "anika-"  # marks requests we sent so our own echoes can be ignored
...
class SimplexAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig, **kwargs):
        ...
        self._text_batch_delay = float(os.getenv("ANIKA_SIMPLEX_TEXT_BATCH_DELAY", "0.8"))
...
async def _standalone_send(
    ...
    this process (``anika cron``). ``thread_id``/``force_document`` are signature parity only;
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``anika setup gateway`` → SimpleX; writes ``~/.anika/.env``."""
    print(
        "\nSimpleX Chat setup\n------------------\nRequirements:\n"
    ...
    try:
        ...
        print("anika_cli.config not available; set SIMPLEX_* vars manually in ~/.anika/.env")
        return
