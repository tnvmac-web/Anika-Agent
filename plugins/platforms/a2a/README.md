# A2A — Agent-to-Agent protocol for Anika
...
peer (another Anika, LangChain, CrewAI, Google ADK, OpenClaw, …). Stdlib only —
no `a2a-sdk` dependency.

## Enable

```bash
anika gateway setup      # pick A2A, or:
```
...
When the `a2a` platform is enabled, Anika serves a v1.0 Agent Card at
...
- Every exchange is logged to `~/.anika/a2a_audit.jsonl`.
- Conversations persist to `~/.anika/a2a_conversations/` — they survive context
