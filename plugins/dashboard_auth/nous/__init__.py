"""NousDashboardAuthProvider — __NEW_ORG__ Portal OAuth (authorization-code + PKCE).

Implements ``nous-account-service/docs/agent-dashboard-oauth-contract.md``; registers only
when a client_id (``dashboard.oauth.client_id`` / ``ANIKA_DASHBOARD_OAUTH_CLIENT_ID``, shape
...
from anika_cli.dashboard_auth import LoginStart, ProviderError, Session
...
_DEFAULT_PORTAL_URL = "https://portal.__NEW_DOMAIN__"
...
class NousDashboardAuthProvider(JwtOAuthProvider):
    """__NEW_ORG__ Portal OAuth via authorization-code + PKCE (S256)."""

    name = "nous"
    display_name = "__NEW_ORG__"
    ...
    def _claims_for(self, access_token: str) -> Dict[str, Any]:
        ...
        if contract_version is None:
            logger.warning(
                "__NEW_ORG__ Portal token missing oauth_contract_version claim (contract says it should be %d); proceeding anyway.",
...
def _settings() -> dict:
    ...
    client_id = resolve_env_or_cfg("ANIKA_DASHBOARD_OAUTH_CLIENT_ID", section.get("client_id", ""))
    portal_url = resolve_env_or_cfg("ANIKA_DASHBOARD_PORTAL_URL", section.get("portal_url", "")) or _DEFAULT_PORTAL_URL
    if not client_id:
        raise SkipRegistration(
            "ANIKA_DASHBOARD_OAUTH_CLIENT_ID is not set (and dashboard.oauth.client_id "
            "in config.yaml is empty). The __NEW_ORG__ Portal provisions this env var (shape "
            "'agent:{instance_id}') when it deploys a Anika Agent instance — set it to "
    ...
    if not client_id.startswith("agent:"):
        raise SkipRegistration(
            f"ANIKA_DASHBOARD_OAUTH_CLIENT_ID={client_id!r} doesn't match the contract "
            f"shape 'agent:{{instance_id}}'. The __NEW_ORG__ Portal provisions this value at deploy "
...
_PLUGIN_COMPAT_LAZY = {
    'DashboardAuthProvider': ('anika_cli.dashboard_auth', 'DashboardAuthProvider'),
    'InvalidCodeError': ('anika_cli.dashboard_auth', 'InvalidCodeError'),
    'RefreshExpiredError': ('anika_cli.dashboard_auth', 'RefreshExpiredError'),
    'classify_jwks_lookup_error': ('anika_cli.dashboard_auth', 'classify_jwks_lookup_error'),
}
