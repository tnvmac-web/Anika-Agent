"""OpenRouter-compatible image generation backend (OpenRouter + __NEW_ORG__ Portal).
...
via :func:`anika_cli.runtime_provider.resolve_runtime_provider`. OpenRouter alone also
...
_ATTRIBUTION_HEADERS = {
    ...
    # OpenRouter attribution headers (harmless against __NEW_ORG__ Portal).
    "HTTP-Referer": "https://github.com/__NEW_ORG__/anika-agent",
    "X-Title": "Anika Agent",
}
...
class OpenRouterCompatImageProvider(ImageGenProvider):
    """One instance per backend (OpenRouter, __NEW_ORG__ Portal); they differ only in the runtime
    ...
    def list_models(self) -> List[Dict[str, Any]]:
        ...
        releases selectable); __NEW_ORG__ Portal = chat-completions only. Offline: default chain + snapshot."""
        ...
        **kwargs: Any,
        ...
        if not api_key or not base_url:
            return fail(
                ...
                f"Configure {self._display} in `anika tools` → Image Generation.",
                "missing_api_key")
...
def _build_providers() -> List[OpenRouterCompatImageProvider]:
    return [
        ...
        OpenRouterCompatImageProvider(
            provider_name="nous", display_name="__NEW_ORG__ Portal", runtime_name="nous", config_key="nous",
            model_env_var="NOUS_IMAGE_MODEL",
            setup_schema={
                "name": "__NEW_ORG__ Portal (image)",
                "badge": "subscription",
                "tag": "Reference-grounded image generation via __NEW_ORG__ Portal (OpenRouter-backed)",
                "env_vars": [],
...
def register(ctx: Any) -> None:
    """Register the OpenRouter + __NEW_ORG__ Portal image gen providers."""
