...
# Anika' 3 abstract ratios → Krea's enum (1:1, 4:3, 3:2, 16:9, 2.35:1, 4:5, 2:3, 9:16).
...
_USER_AGENT = "Anika-Agent/1.0 (krea-image-gen)"
        ...
        if failure.kind == "http":
            ...
            if managed and 400 <= status < 500:
                hint = (
                    ...
                    f"Model '{model_id}' may not be enabled/priced on the __NEW_ORG__ Portal's Krea gateway. "
                    "Set KREA_API_KEY to use Krea directly, or pick a different model via "
                    "`anika tools` → Image Generation.")
                return None, fail(
                    f"__NEW_ORG__ Subscription Krea gateway rejected '{model_id}' "
...
class KreaImageGenProvider(StaticImageGenProvider):
    ...
    setup = dict(
        ...
        tag="Krea 2 foundation model — Medium ($0.03), Large ($0.06), Medium Turbo ($0.015). Style transfer, moodboards, reference-guided generation. Direct key or managed __NEW_ORG__ Subscription gateway.",
        key="KREA_API_KEY", prompt="Krea API key", url="https://www.krea.ai/settings/api-tokens")

    def is_available(self) -> bool:
        # Direct key OR managed __NEW_ORG__ gateway (portal users without a Krea key).
    ...
    def generate(
        ...
        **kwargs: Any,
        ...
        # __NEW_ORG__ access token); otherwise direct Krea with a BYO ``KREA_API_KEY``.
        ...
        if managed is not None:
            ...
            if not auth_token:
                return error_factory("krea", aspect)(
                    "KREA_API_KEY not set. Run `anika tools` → Image "
                    "Generation → Krea to configure, get a key at "
                    "https://www.krea.ai/settings/api-tokens, or sign in to "
                    "a __NEW_ORG__ account with the managed Krea gateway enabled "
                    "(`anika setup`).",
                    "auth_required")
        ...
        if managed is not None:
            for what, arg in _MANAGED_UNSUPPORTED:
                if arg in payload:
                    return fail(
                        f"Managed Krea (__NEW_ORG__ Subscription) does not support {what}. "
        ...
        # 2. Poll — same principal as submit, so the managed path polls the gateway with the __NEW_ORG__ token.
