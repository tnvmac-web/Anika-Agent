...
``BROWSER_USE_API_KEY`` (https://browser-use.com) or the managed __NEW_ORG__ tool gateway (bills to a
__NEW_ORG__ subscription). Direct first, managed second, unless ``tool_gateway.browser: gateway`` flips
...
class BrowserUseBrowserProvider(CloudBrowserProvider):
    ...
    # registered for the __NEW_ORG__ gateway path and legacy cloud_provider configs.
    setup_tag = None
    ...
    def _get_config_or_none(self, *, refresh_token: bool = True) -> Optional[Dict[str, Any]]:
        # Lazy: managed_tool_gateway pulls in the __NEW_ORG__ auth stack direct-key users never need.
