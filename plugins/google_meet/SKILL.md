...
anika meet install                 # pip deps + Chromium (transcribe only)
anika meet install --realtime      # + pulseaudio-utils / brew blackhole+ffmpeg
anika meet auth                    # optional; skips guest-lobby wait
anika meet setup                   # preflight checks
```

`anika meet install --realtime` prompts before running `sudo apt-get` (Linux)
...
#   Then set OPENAI_API_KEY or ANIKA_MEET_REALTIME_KEY in ~/.anika/.env
```
...
pip install playwright websockets && python -m playwright install chromium
anika plugins enable google_meet
anika meet node run --display-name my-mac    # persistent server
# copy the printed token

# on the gateway:
anika meet node approve my-mac ws://<mac-ip>:18789 <token>
anika meet node ping my-mac                   # confirm reachable
```

Run `anika meet setup` to preflight local prereqs.

## Flow

1. **Join** — call `meet_join(url=..., mode=..., node=...)`. Returns immediately.
2. **Announce yourself** — no auto-consent. Say (in whatever channel the user is watching): "A Anika agent bot is in this call taking notes."
...
- Guest mode sits in the lobby until a host admits. Warn the user; `anika meet auth` avoids this.
- **Lobby timeout**: if the host doesn't admit the bot within 5 minutes (configurable via `ANIKA_MEET_LOBBY_TIMEOUT` env), the bot leaves and `meet_status` reports `leaveReason: "lobby_timeout"`.
...
| `realtimeDevice` | Audio device name the bot is feeding (e.g. `anika_meet_src`). |
...
Local:
```
$ANIKA_HOME/workspace/meetings/<meeting-id>/transcript.txt
```
...
- Remote nodes use bearer-token auth; tokens are generated on the node (32 hex chars, persisted in `$ANIKA_HOME/workspace/meetings/node_token.json`) and must be copied to the gateway via `anika meet node approve`.
