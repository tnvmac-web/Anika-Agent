...
Let the anika agent join a Google Meet call, transcribe it, optionally speak
...
```
┌─ gateway (Linux box, where anika runs) ────────────────────────────┐
│                                                                      │
...
│                                                                      │
│   NodeServer (from `anika meet node run`)                           │
│     │                                                                │
...
| `__init__.py` | `register(ctx)` — registers 5 tools + `on_session_end` hook + `anika meet` CLI |
...
| `cli.py` | `anika meet setup / auth / join / status / transcript / say / stop / node ...` |
...
| `node/registry.py` | v3: `$ANIKA_HOME/workspace/meetings/nodes.json` |
| `node/server.py` | v3: `NodeServer` (runs on host machine) |
| `node/client.py` | v3: `NodeClient` (used by tool handlers + CLI on gateway) |
| `node/cli.py` | v3: `anika meet node {run,list,approve,remove,status,ping}` |
...
anika meet install                                      # pip + Chromium
anika meet setup                                        # preflight
anika meet auth                                         # optional
anika meet join https://meet.google.com/abc-defg-hij    # transcribe
```
...
```bash
anika meet install --realtime                     # installs pulseaudio-utils
echo 'OPENAI_API_KEY=sk-...' >> ~/.anika/.env
anika meet join https://meet.google.com/abc-defg-hij --mode realtime
# then from the agent or CLI:
anika meet say "Good morning everyone, I'm the note-taker bot."
```

macOS:
```bash
anika meet install --realtime     # runs: brew install blackhole-2ch ffmpeg
# then — manually! — open System Settings → Sound → Input → BlackHole 2ch
echo 'OPENAI_API_KEY=sk-...' >> ~/.anika/.env
anika meet join https://meet.google.com/abc-defg-hij --mode realtime
```

On macOS, anika will **not** switch your system audio input automatically — the
...
anika plugins enable google_meet
anika meet node run --display-name my-mac --host 0.0.0.0 --port 18789
