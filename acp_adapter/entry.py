"""CLI entry point for the anika-agent ACP adapter.

Loads ``~/.anika/.env``, routes logging to stderr (stdout is reserved for ACP
JSON-RPC), and starts the ACP agent server.

Usage::

    python -m acp_adapter.entry   # or: anika acp / anika-acp
"""

# IMPORTANT: anika_bootstrap must be the very first import — UTF-8 stdio
# on Windows.  No-op on POSIX.  See anika_bootstrap.py for full rationale.
try:
    import anika_bootstrap  # noqa: F401
except ModuleNotFoundError:
    # Partial ``anika update`` (git-reset landed, ``uv pip install -e .`` did not):
    # UTF-8 stdio setup is skipped on Windows; POSIX is unaffected.
    pass
else:
    # Stop a ``utils/``/``proxy/``/``ui/`` package in the launch cwd from shadowing Anika modules.
...
def _load_env() -> None:
    """Load .env from ANIKA_HOME (default ``~/.anika``)."""
    from anika_cli.env_loader import load_anika_dotenv

    anika_home = get_anika_home()
    loaded = load_anika_dotenv(anika_home=anika_home)
    log = logging.getLogger(__name__)
    for env_file in loaded or ():
        log.info("Loaded env from %s", env_file)
    if not loaded:
        log.info("No .env found at %s, using system env", anika_home / ".env")


def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(prog="anika-acp", description="Run Anika Agent as an ACP stdio server.")
    parser.add_argument("--version", action="store_true", help="Print Anika version and exit")
    parser.add_argument("--check", action="store_true", help="Verify ACP dependencies and adapter imports, then exit")
    parser.add_argument("--setup", action="store_true",
                        help="Run interactive Anika provider/model setup for ACP terminal auth")
    parser.add_argument("--setup-browser", action="store_true",
                        help="Install agent-browser + Playwright Chromium into ~/.anika/node/ "
...
def _print_version() -> None:
    from anika_cli import __version__ as anika_version
...
def _run_check() -> None:
    ...
    from acp_adapter.server import AnikaACPAgent  # noqa: F401

    print("Anika ACP check OK")


def _run_setup() -> None:
    from anika_cli.main import main as anika_main

    old_argv = sys.argv[:]
    try:
        sys.argv = [old_argv[0] if old_argv else "anika", "model"]
        anika_main()
    finally:
        sys.argv = old_argv
...
def main(argv: list[str] | None = None) -> None:
    ...
    logger.info("Starting anika-agent ACP adapter")
    ...
    import acp
    from .server import AnikaACPAgent
    ...
    if os.environ.get("ANIKA_ACP_SKIP_CONFIGURED_MCP", "").strip() != "1":
        try:
