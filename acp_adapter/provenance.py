...
Additive Anika extension under ACP ``_meta.anika`` (unknown to other clients,
...
rotates the internal Anika head, ``build_session_provenance`` exposes the
...
def build_session_provenance(
    db: Any, acp_session_id: str, current_anika_session_id: str, *, previous_anika_session_id: Optional[str] = None,
) -> Optional[Dict[str, Any]]:
    """Build ``_meta.anika.sessionProvenance`` for an ACP session.

    ``db`` must expose ``get_session``. ``current_anika_session_id`` is the live
    internal id (``state.agent.session_id``); ``previous_anika_session_id`` is
    the id before the most recent turn, supplied by ``prompt()`` to flag a
    rotation. Returns ``None`` if the session can't be read."""
    row = _get_row(db, current_anika_session_id)
    if not row:
        return None
    ...
    root_id, compression_depth, cursor_parent = current_anika_session_id, 0, parent_id
    seen = {current_anika_session_id}
    ...
    provenance: Dict[str, Any] = {
        "acpSessionId": acp_session_id, "currentAnikaSessionId": current_anika_session_id,
        "rootAnikaSessionId": root_id, "parentAnikaSessionId": parent_id,
        "sessionKind": "continuation" if is_continuation else "root", "compressionDepth": compression_depth,
    }
    if previous_anika_session_id:
        provenance["previousAnikaSessionId"] = previous_anika_session_id
        if previous_anika_session_id != current_anika_session_id:
...
def session_provenance_meta(
    ...
    """Return a ready ``_meta`` payload: ``{"anika": {"sessionProvenance": ...}}``."""
    prov = build_session_provenance(db, acp_session_id, current_anika_session_id,
                                    previous_anika_session_id=previous_anika_session_id)
    return None if prov is None else {"anika": {"sessionProvenance": prov}}
