"""ACP auth helpers — detect and advertise Anika authentication methods."""
...
TERMINAL_SETUP_AUTH_METHOD_ID = "anika-setup"


def detect_provider() -> Optional[str]:
    """Resolve the active Anika runtime provider, or None if unavailable.
...
def build_auth_methods() -> list[Any]:
    """Return registry-compatible ACP auth methods for Anika.

    The ACP registry requires at least one usable auth method in the initial
    handshake. A fresh Zed install may have no Anika credentials yet, so the
    ...
    if provider:
        methods.append(AuthMethodAgent(
            ...
            description=f"Authenticate Anika using the currently configured {provider} runtime credentials.",
        ))
    methods.append(TerminalAuthMethod(
        id=TERMINAL_SETUP_AUTH_METHOD_ID, name="Configure Anika provider", type="terminal", args=["--setup"],
        description=("Open Anika' interactive model/provider setup in a terminal. "
                     "Use this when Anika has not been configured on this machine yet."),
    ))
    return methods
...
def has_provider() -> bool:
    """Return True if Anika can resolve any runtime provider credentials."""
