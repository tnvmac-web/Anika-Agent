...
try:
    ...
    pass  # partial `anika update` — only skips the Windows UTF-8 stdio setup
...
def _launch_cwd_for_session(source: str) -> Optional[str]:
    """cwd to stamp on a new session row (``anika -c`` / ``--resume``), or None.
...
def _session_source_for_agent(platform: Optional[str]) -> str:
    try:
        ...
        source = get_session_env("ANIKA_SESSION_SOURCE", "")
    except Exception:
        source = os.environ.get("ANIKA_SESSION_SOURCE", "")
...
_anika_home = get_anika_home()  # read by agent_init via _ra()._anika_home
_loaded_env_paths = load_anika_dotenv(anika_home=_anika_home, project_env=Path(__file__).parent / '.env')
...
class AIAgent(
    ...
    _TOOL_CALL_ARGUMENTS_CORRUPTION_MARKER = (
        "[anika-agent: tool call arguments were corrupted in this session and "
    ...
    def _ensure_lmstudio_runtime_loaded(self, config_context_length: Optional[int] = None) -> Any:
        ...
        if (getattr(self, "lmstudio_load_mode", "explicit") or "explicit").strip().lower() == "jit":
            ...
            return None
        from anika_cli.models_local import ensure_lmstudio_model_loaded
    ...
    def _resolved_api_call_timeout(self) -> float:
        ...
        ``ANIKA_API_TIMEOUT`` > 1800s."""
        cfg = get_provider_request_timeout(self.provider, self.model)
        return cfg if cfg is not None else env_float("ANIKA_API_TIMEOUT", 1800.0)

    def _resolved_api_call_stale_timeout_base(self) -> tuple[float, bool]:
        """Base non-stream stale timeout: per-model ``stale_timeout_seconds`` > provider-wide >
        ``ANIKA_API_CALL_STALE_TIMEOUT`` > reasoning floor > 90s.
        ...
        if cfg is not None:
            return cfg, False
        env_timeout = os.getenv("ANIKA_API_CALL_STALE_TIMEOUT")
    ...
    def _stale_timeout_is_explicit(self) -> bool:
        ...
        return (get_provider_stale_timeout(self.provider, self.model) is not None
                or os.getenv("ANIKA_API_CALL_STALE_TIMEOUT") is not None)
    ...
    def _codex_silent_hang_hint(self, model: Optional[str] = None) -> Optional[str]:
        ...
        return (
            ...
            "See anika-agent#21444 for symptom history."
        )
    ...
    def _provider_model_requires_responses_api(model: str, *, provider: Optional[str] = None) -> bool:
        ...
        # __NEW_ORG__ serves GPT-5.x via chat completions (its /v1/responses returns 404); generic custom endpoints
