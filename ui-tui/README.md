...
React + Ink terminal UI for Anika. TypeScript owns the screen. Python owns sessions, tools, model calls, and most command logic.
...
Interpreter resolution order is: `ANIKA_PYTHON` → `PYTHON` → `$VIRTUAL_ENV/bin/python` → `./.venv/bin/python` → `./venv/bin/python` → `python3` (or `python` on Windows).
...
- `src/app/setupHandoff.ts` — launches external `anika setup`, suspends Ink while it runs, opens a new session on success
...
- `commands/billing.ts` — `/billing`: manage __NEW_ORG__ remote spending — buy credits, auto-reload, limits
...
- Input history is stored in `~/.anika/.anika_history` or under `ANIKA_HOME`.
...
### Billing (`billing.ts`)
`/billing` — manage __NEW_ORG__ remote spending — buy credits, auto-reload, limits
...
### Credits (`credits.ts`)
`/credits` — __NEW_ORG__ credit balance and browser top-up

### Setup (`setup.ts`)
`/setup` — launches external `anika setup` wizard, suspends Ink while it runs
...
```text
ui-tui/
  packages/anika-ink/   forked Ink renderer (local dep)
  src/
    ...
    app/
      ...
      setupHandoff.ts               launches external anika setup, suspends Ink while it runs
      ...
      slash/
        ...
        commands/
          billing.ts                /billing — manage __NEW_ORG__ remote spending
    ...
    lib/
      ...
      parentLog.ts               append-only log to ~/.anika/tui-parent.log
    ...
    types/
      anika-ink.d.ts            type declarations for @anika/ink
...
tui_gateway/
  ...
  slash_worker.py        persistent AnikaCLI subprocess for slash commands
```
