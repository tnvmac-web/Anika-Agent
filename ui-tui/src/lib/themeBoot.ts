 ...
 * `anika-boot-background` / `anika-boot-color-scheme` localStorage keys.
 *
...
// Profile-aware: the Python launcher exports ANIKA_HOME (set by
// _apply_profile_override) before spawning the TUI. Falling back to
// ~/.anika matches get_anika_home()'s default.
const bootFilePath = () => join(process.env.ANIKA_HOME ?? join(homedir(), '.anika'), 'tui-theme-boot.json')

// Never touch the user's real ~/.anika from test runs (the TS suite has no
// ANIKA_HOME isolation fixture).
...
export interface BootSeedResult {
  ...
  /** True when the cached config pin was seeded into ANIKA_TUI_THEME. */
  seededPin: boolean
}
...
export function seedBootEnvironment(boot: BootTheme | null, env: NodeJS.ProcessEnv): BootSeedResult {
  ...
  if (!boot || env.ANIKA_TUI_THEME || env.ANIKA_TUI_LIGHT) {
    return result
  }
  ...
  if (
    ...
    !env.ANIKA_TUI_BACKGROUND
  ) {
    env.ANIKA_TUI_BACKGROUND = boot.background
...
export function invalidateBootBackground(env: NodeJS.ProcessEnv = process.env): boolean {
  if (!seeded.seededBackground || env.ANIKA_TUI_BACKGROUND !== seeded.seededBackground) {
    return false
  }

  delete env.ANIKA_TUI_BACKGROUND
...
/** True when this boot replayed a cached config pin into ANIKA_TUI_THEME.
