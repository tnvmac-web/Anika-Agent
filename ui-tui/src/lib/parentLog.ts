...
const logDir = join(process.env.ANIKA_HOME?.trim() || join(homedir(), '.anika'), 'logs')
...
// a real ~/.anika (tests must stay hermetic — see AGENTS.md).
...
export function recordParentLifecycle(line: string): void {
  ...
  try {
    ...
    if (!warned) {
      warned = true
      process.stderr.write('anika-tui: parent lifecycle log unavailable\n')
    }
  }
}
