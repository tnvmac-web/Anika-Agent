...
import { Box, Text } from '@anika/ink'
...
/**
 * User widget apps — Anika authors its own TUI widgets, mirroring the
 * Python plugin contract: drop `<name>.mjs` into `$ANIKA_HOME/tui-widgets/`,
 ...
 * React.createElement.
 *
 * Trust model matches `~/.anika/plugins/`: files under ANIKA_HOME execute
...
const widgetsDir = () => join(process.env.ANIKA_HOME?.trim() || join(homedir(), '.anika'), 'tui-widgets')
 ...
 *  every change, so a widget Anika writes appears within ~a second — no
...
export function watchUserWidgets(dir = widgetsDir()): void {
  ...
  if (!attach()) {
    ...
    // user (or Anika) ever writes must hot-load too — a 10s poll here read
