...
import { terminalBackgroundHex } from '@anika/ink'
...
export const debugCommands: SlashCommand[] = [
  ...
  {
    help: 'rescan $ANIKA_HOME/tui-widgets and (re)register user widget apps',
    name: 'widgets-reload',
    run: (_arg, ctx) => {
  ...
  {
    help: 'write a V8 heap snapshot + memory diagnostics (see ANIKA_HEAPDUMP_DIR)',
    name: 'heapdump',
    run: (_arg, ctx) => {
      ...
      ctx.transcript.panel('Theme', [
        {
          rows: [
            ...
            ['ANIKA_TUI_BACKGROUND', process.env.ANIKA_TUI_BACKGROUND ?? '(unset)'],
            ['ANIKA_TUI_THEME', process.env.ANIKA_TUI_THEME ?? '(unset)'],
