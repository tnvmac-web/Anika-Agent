...
function buildManageUrl(s: SubscriptionStateResponse, tierId?: string): string | null {
  ...
  // on the Python side (e.g. https://portal.__NEW_DOMAIN__/billing). Strip any
...
export const subscriptionCommands: SlashCommand[] = [
  {
    help: 'View or change your __NEW_ORG__ subscription plan',
    name: 'subscription',
    aliases: ['upgrade'],
      ...
      ctx.gateway
        ...
        .then(
          ctx.guarded<SubscriptionStateResponse>(s => {
            if (!s.logged_in) {
              sys('Not logged into __NEW_ORG__ Portal — run /portal to log in, then /subscription.')
