import { driveChargeSettlement, type SettlementOutcome } from '@anika/shared/charge-settlement'
...
const renderBillingError = (
  ...
  switch (env.error) {
    ...
    case 'remote_spending_disabled':
      ...
      // turn it on from the portal's Anika Agent page; this is NOT a per-terminal stop.
      sys(
      ...
      ctx.gateway
        ...
        .then(
          ctx.guarded<BillingStateResponse>(s => {
            if (!s.logged_in) {
              sys('💳 Not logged into __NEW_ORG__ Portal — run /portal to log in, then /topup.')
