import type { RunExternalProcess } from '@anika/ink'
...
export async function runExternalSetup({ args, ctx, done, launcher, suspend }: RunExternalSetupOptions) {
  ...
  transcript.sys(`launching \`anika ${args.join(' ')}\`…`)
  ...
  if (result.error) {
    transcript.sys(`error launching anika: ${result.error}`)
  ...
  if (result.code !== 0) {
    transcript.sys(`anika ${args[0]} exited with code ${result.code}`)
    patchUiState({ status: 'setup required' })
