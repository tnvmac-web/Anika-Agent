import type { InputEvent, Key } from '@anika/ink'
import * as Ink from '@anika/ink'
 ...
 * but that accepts `key.meta` on macOS — and anika-ink reports Option as
...
export function supportsFastEchoTerminal(env: NodeJS.ProcessEnv = process.env): boolean {
  ...
  if (isTermuxTuiMode(env)) {
    const override = String(env.ANIKA_TUI_TERMUX_FAST_ECHO ?? '')
      .trim()
      .toLowerCase()
  ...
  useInput(
    (inp: string, k: Key, event: InputEvent) => {
      ...
      if (actionHome) {
        ...
        // ESC d, which anika-ink decodes as meta+'d'; without this branch it
