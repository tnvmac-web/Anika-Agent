...
``anika pause`` writes a sentinel at ``$ANIKA_HOME/ESTOP``; ``anika resume``
...
(fail safe, e.g. ``touch ~/.anika/ESTOP``). Ported from gastownhall/gastown estop.go (MIT).
"""
...
# Same profile-aware / fleet-root resolvers the file-safety guards use (fail-open to ~/.anika).
from agent.file_safety import _anika_home_path as _anika_home, _anika_root_path as _canonical_root
...
def sentinel_path() -> Path:
    """Path of the ESTOP sentinel this process would write on `anika pause`."""
    return _anika_home() / SENTINEL_NAME


def _candidate_sentinel_paths() -> list:
    """Profile home first, then the fleet root if it is a different directory: a profile
    gateway (ANIKA_HOME=~/.anika/profiles/<n>) must still honor an operator's ~/.anika/ESTOP."""
...
def paused_reply() -> Optional[str]:
    ...
    return f"⏸️ Anika is paused{tag}. New work is on hold; run `anika resume` to pick things back up."
...
def check_paused(component: str, logger: logging.Logger) -> bool:
    ...
    if first:
        ...
        logger.info(
            "%s dispatch paused by global emergency stop%s — remove with `anika resume` (%s)",
