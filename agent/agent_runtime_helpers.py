...
def try_recover_primary_transport(
    ...
    if (
        (agent.provider or "").strip().lower() in {"nous", "nous-portal", "__NEW_ORG__"}
...
def dump_api_request_debug(
    ...
    try:
        ...
        # Sanitize the session ID (may come from an untrusted X-Anika-Session-Id header) so a
        ...
        if env_var_enabled("ANIKA_DUMP_REQUEST_STDOUT"):
...
def _moa_aggregator_cache_policy(agent, eff_model: str) -> tuple[bool, bool]:
    ...
    try:
        from anika_cli.config import load_config as _load_moa_cfg
...
def _route_may_be_custom(agent, eff_provider: str, provider_lower: str, eff_base_url: str) -> bool:
    ...
    if custom_providers:
        ...
        from anika_cli.providers import custom_provider_aliases
...
def anthropic_prompt_cache_policy(
    ...
    # __NEW_ORG__ Portal proxies to OpenRouter; treat as OpenRouter-equivalent for cache layout.
    is_nous_portal = base_url_host_matches(eff_base_url, "__NEW_DOMAIN__")
    ...
    if supports_cache_markers and (
        ...
        try:
            from anika_cli.config import get_custom_provider_model_capability
    ...
    if (is_openrouter or is_nous_portal) and (is_claude or is_kimi) and not is_anthropic_wire:
        return True, False
    # __NEW_ORG__ Portal Qwen takes the envelope path too; the alibaba-family check below only matches
...
def create_openai_client(agent, client_kwargs: dict, *, reason: str, shared: bool) -> Any:
    ...
    # before the built-in ladder so a profile registered from ~/.anika/plugins/ or a pip entry
    ...
    # Delegate all rate-limit / 5xx retry to anika's outer conversation loop, which honors Retry-After and
    ...
    if agent.provider == "opencode-free":
        from anika_cli.models import opencode_zen_free_headers
        client_kwargs["default_headers"] = {**(client_kwargs.get("default_headers") or {}), **opencode_zen_free_headers()}
    # All primary construction and recovery paths must identify Anika to the official Codex
...
def _apply_switched_provider_request_overrides(agent, new_provider):
    ...
    if custom_providers is None:
        try:
            from anika_cli.config import load_config, get_compatible_custom_providers
...
def _resolve_switch_destination(agent, new_model, new_provider, base_url, api_mode, capabilities, old_norm, new_norm):
    ...
    from anika_cli.providers import determine_api_mode
    from agent.native_compaction import resolve_native_compaction_capabilities
    from anika_cli.models import opencode_provider_family
    # Pass model so dual-wire providers (__NEW_ORG__ Portal anthropic/* -> Messages) resolve correctly.
    if not api_mode:
...
def _update_switch_compressor(agent, custom_providers, effective_context_length, snapshot) -> None:
    ...
    if custom_providers is None:
        try:
            from anika_cli.config import get_compatible_custom_providers, load_config
            custom_providers = get_compatible_custom_providers(load_config())
        except Exception:
            custom_providers = None
...
def switch_model(
    ...
    try:
        ...
        from anika_cli.config import load_config as _sm_load_config
...
def _pre_tool_block_message(agent, function_name, function_args, effective_task_id, tool_call_id, middleware_trace):
    ...
    try:
        from anika_cli.plugins import _dispatch_pre_tool_call_hooks
...
def invoke_tool(agent, function_name: str, function_args: dict, effective_task_id: str,
    ...
    try:
        from anika_cli.middleware import apply_tool_request_middleware
    ...
    from anika_cli.middleware import run_tool_execution_middleware
...
def _session_id_for_heal_log() -> str:
    try:
        from anika_logging import _session_context
        ...
        if escalate:
            ...
            if key not in _empty_heal_user_notified:
                ...
                _empty_heal_pending_notice[key] = (
                    ...
                    "session's history — run /debug share or `anika "
...
def _realign_tool_result_names(messages: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    #   the OpenAI-compat serializer keeps it provider-agnostic: Gemini reaches Anika under many model
...
def _iter_pool_sockets(client: Any):
    ...
    if not pools:
        return
    from agent.process_bootstrap import ANIKA_TRANSPORT_OWNER_EXT
    seen: set[int] = set()
    for pool, owner in pools:
        ...
        for pool_req in list(getattr(pool, "_requests", None) or []):
            if owner is not None:
                ...
                if exts.get(ANIKA_TRANSPORT_OWNER_EXT) != owner:
                    continue
