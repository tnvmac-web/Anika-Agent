...
_GLOBAL_ENV_EXACT = frozenset({
    # Anika runtime / deployment
    "ANIKA_HOME", "ANIKA_PROFILE", "ANIKA_GATEWAY_LOCK_DIR",
    "ANIKA_MAX_ITERATIONS", "ANIKA_MAX_TOKENS", "ANIKA_API_TIMEOUT",
    "ANIKA_REDACT_SECRETS", "ANIKA_NOUS_TIMEOUT_SECONDS",
    "_ANIKA_GATEWAY",
    # OS / interpreter
    "PATH", "HOME", "USER", "LANG", "LC_ALL", "TZ", "PWD", "SHELL", "TMPDIR",
    "VIRTUAL_ENV", "PYTHONPATH", "SSL_CERT_FILE",
    # Kanban paths (per-board, not per-profile-secret)
    "ANIKA_KANBAN_DB", "ANIKA_KANBAN_WORKSPACES_ROOT", "ANIKA_KANBAN_BOARD",
...
_GLOBAL_ENV_PREFIXES = (
    "ANIKA_KANBAN_",
    "ANIKA_TELEGRAM_",   # tuning knobs (batch delays, fallback toggles) — NOT the token
...
def load_env_file(env_path: Path) -> Dict[str, str]:
    ...
    from anika_cli.config import _parse_env_value
...
def build_profile_secret_scope(anika_home: Path) -> Dict[str, str]:
    ...
    secrets = load_env_file(Path(anika_home) / ".env")
    try:
        from anika_cli.env_loader import get_secret_source_values
        external_secrets = get_secret_source_values(Path(anika_home))
    except Exception:
        external_secrets = {}
