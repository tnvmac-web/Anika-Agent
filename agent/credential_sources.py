"""Unified removal contract for every credential source Anika reads from.

Readers live in ``agent.credential_pool``; what is unified here is **removal**:
``anika auth remove <provider> <N>`` must make the entry stay gone across
...
def _remove_env_source(provider: str, removed) -> RemovalResult:
    """env:<VAR> — clear from ~/.anika/.env; hint when the shell exports it."""
    from anika_cli.config import get_env_path, remove_env_value
    ...
    # .env as utf-8-sig like anika_cli/config.py: a BOM-sensitive read would
    ...
    if env_in_process and not env_in_dotenv:
        result.hints.extend([
            ...
            "launchd plist, etc.) or it will keep being visible to Anika.",
            f"  The pool entry is now suppressed — Anika will ignore "
            f"{env_var} until you run `anika auth add {provider}`.",
        ])
    else:
        result.hints.append(
...
def _remove_anika_pkce(provider: str, removed) -> RemovalResult:
    """~/.anika/.anthropic_oauth.json is ours — delete it outright."""
    from anika_constants import get_anika_home

    result = RemovalResult()
    oauth_file = get_anika_home() / ".anthropic_oauth.json"
    if oauth_file.exists():
        try:
            oauth_file.unlink()
            result.cleaned.append("Cleared Anika Anthropic OAuth credentials")
...
def _remove_xai_oauth_device_code(provider: str, removed) -> RemovalResult:
    ...
    result.hints.append(
        "Run `anika model` → xAI Grok OAuth (SuperGrok / Premium+) to re-authenticate if needed."
    )
    return result
...
def _remove_codex_device_code(provider: str, removed) -> RemovalResult:
    ...
    result.hints.extend([
        ...
        "Run `anika auth add openai-codex` to re-enable if needed.",
    ])
    return result
...
def _remove_copilot_gh(provider: str, removed) -> RemovalResult:
    ...
    return RemovalResult(hints=[
        ...
        "Run `anika auth add copilot` to re-enable if needed.",
    ])
...
_REGISTRY: List[RemovalStep] = [
    ...
    RemovalStep(
        ...
        remove_fn=_suppress_only(
            ...
            "Run `anika auth add anthropic` to re-enable if needed.",
        ),
        description="~/.claude/.credentials.json",
    ),
    RemovalStep(
        provider="anthropic", source_id="anika_pkce",
        remove_fn=_remove_anika_pkce,
        description="~/.anika/.anthropic_oauth.json",
    ),
    RemovalStep(
    ...
    RemovalStep(
        ...
        remove_fn=_suppress_only(
            ...
            "Run `anika auth add qwen-oauth` to re-enable if needed.",
        ),
