...
# OpenRouter/__NEW_ORG__ run a quality-first model chain and may fall back depending on
...
_PROVIDER_LABELS = {"nous": "__NEW_ORG__ Portal", "openrouter": "OpenRouter", "openai": "OpenAI", "openai-codex": "OpenAI (Codex)", "krea": "Krea"}
...
def resolve_provider(*, require_references: bool = True, prefer: str | None = None) -> SpriteProvider:
    ...
    Preference: ``ANIKA_PET_IMAGE_PROVIDER`` (QA override, unknown values ignored),
    ...
    forced = os.environ.get("ANIKA_PET_IMAGE_PROVIDER", "").strip().lower()
    ...
    raise GenerationError(
        ...
        "Open `anika tools` → Image Generation and configure __NEW_ORG__ Portal, "
...
def generate(
    ...
    *aspect_ratio* ``"landscape"`` (row strips) gives every frame horizontal room so
    ...
    # Providers disagree on the ref kwarg name: our OpenRouter/__NEW_ORG__ backends read
