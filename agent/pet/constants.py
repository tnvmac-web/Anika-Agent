...
inferred from the atlas shape so Anika renders both legacy 8-row sheets and
...
class PetState(str, Enum):
    """Animation state a pet can be shown in (Anika names; Codex rows say ``jumping``/``running`` for ``jump``/``run``)."""
...
# Legacy Anika/petdex row order (top -> bottom) for the older 8-row, 9-column atlas.
...
# Canonical Anika names -> accepted row-name aliases in descending preference.
