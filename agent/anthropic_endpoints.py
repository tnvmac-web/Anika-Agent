...
def _is_deepseek_anthropic_endpoint(base_url: str | None) -> bool:
    ...
    keep-unsigned policy used for Kimi's ``/coding`` endpoint. See anika-agent#16748.
    """
...
def _is_nous_portal_endpoint(base_url: str | None) -> bool:
    """__NEW_ORG__ Portal's Anthropic Messages route (Bearer JWT, verbatim catalog ids, native
    thinking-signature replay). Trusted hosts only: prod ``inference-api.__NEW_DOMAIN__`` or the
    ...
    if base_url_host_matches(base_url or "", "inference-api.__NEW_DOMAIN__"):
        return True
    try:
...
def _requires_bearer_auth(base_url: str | None) -> bool:
    ...
    Foundry, Palantir Foundry's LLM proxy, CommandCode, __NEW_ORG__ Portal. Palantir/CommandCode use
