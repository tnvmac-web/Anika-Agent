...
def _codex_native_auto_compaction(agent: Any) -> bool:
    """Codex app-server threads are compacted by the codex agent itself; Anika only
    initiates compaction in "anika" mode."""
    return (
        # See #36801.
...
def _preflight_compression(
    ...
    if _preflight_deferred:
        ...
        logger.info(
            "Skipping Anika preflight compression for codex app-server "
            "(mode=%s); Anika will not start thread compaction here.",
