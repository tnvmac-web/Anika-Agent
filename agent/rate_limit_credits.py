...
class RateLimitCreditsMixin:
    ...
    def _capture_credits(self, http_response: Any) -> None:
        ...
        depletion-notice bug cannot vanish silently. ANIKA_DEV_CREDITS_FIXTURE injects a chosen state instead.
        """
        try:
        ...
        if fixture is not None:
            ...
            logger.info(
                ...
                "(real headers bypassed — `echo clear` / unset ANIKA_DEV_CREDITS_FIXTURE to restore)",
        ...
        if not headers:
            return
        dev = is_truthy_value(os.environ.get("ANIKA_DEV_CREDITS"))
        ...
        if state is None:
            if dev:
                logger.info("credits ▸ response had no valid x-nous-credits-* headers "
                            "(miss — producer off / non-__NEW_ORG__ path / >TTL stale)")
            return

        _adopt_credits_state(self, state)
        if dev:
            # ANIKA_DEV_CREDITS streams each capture to agent.log (`anika logs -f`, grep 'credits ▸').
