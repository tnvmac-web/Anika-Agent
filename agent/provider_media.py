"""``$ANIKA_HOME/cache/<kind>/`` materialisation helpers for the image/video
...
def cache_dir(kind: str) -> Path:
    """Return ``$ANIKA_HOME/cache/<kind>/``, creating parents as needed."""
    from anika_constants import get_anika_home
    path = get_anika_home() / "cache" / kind
