"""Centralized __NEW_ORG__ Portal request tags.

Every Anika request to the __NEW_ORG__ Portal (main loop, auxiliary client, fallback
paths) must carry the same product-attribution tags, sent in OpenAI-compatible
``extra_body['tags']``: ``["product=anika-agent", "client=anika-client-v<__version__>"]``.
The version is read live from ``anika_cli.__version__`` — do NOT pre-compute it
...
_affinity_scope: ContextVar[Optional[str]] = ContextVar("anika_affinity_scope", default=None)
...
def anika_client_tag() -> str:
    """``client=anika-client-v<MAJOR>.<MINOR>.<PATCH>`` ("unknown" if anika_cli is unimportable)."""
    try:
        from anika_cli import __version__
    except Exception:
        __version__ = "unknown"
    return f"client=anika-client-v{__version__}"
...
def nous_portal_tags(session_id: str | None = None) -> List[str]:
    """Fresh list of the canonical __NEW_ORG__ Portal tags.
    ...
    """
    tags = ["product=anika-agent", anika_client_tag()]
