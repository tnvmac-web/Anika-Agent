...
# Parameters Anika sends on SOME routes only → hosts where that is deliberate.
...
# A reasoning-mandatory route answering ``reasoning: {enabled: false}`` (__NEW_ORG__ Portal + OpenRouter wording).
...
# 404: __NEW_ORG__ API surfaces credit depletion as a paid model vanishing from the
...
def _plugin_verdict(c: _Ctx) -> Optional[Verdict]:
    ...
    try:
        from anika_cli.plugins import get_plugin_error_classification
...
def _classify_400(c: _Ctx) -> Verdict:
    ...
    # Reasoning-mandatory route rejecting a disable (GLM-5.3 on __NEW_ORG__ Portal / OpenRouter). Deterministic
