"""Anthropic Messages API adapter: client construction + the Messages call for Anika's
...
# Anika effort -> Anthropic adaptive-thinking effort (output_config.effort). 4.7+ exposes
...
# Anthropic's OAuth billing classifier fingerprints certain Anika tool schemas/prose as a
...
def _attribution_headers() -> Dict[str, str]:
    ...
    return {
        "HTTP-Referer": "https://anika-agent.__NEW_DOMAIN__", "X-Title": "Anika Agent",
...
def _base_client_kwargs(base_url, timeout) -> tuple[str, Dict[str, Any]]:
    ...
    anika's outer loop (``max_retries=0``): the SDK default of 2 uses its own backoff that ignores
...
def _new_sdk_client(sdk, kwargs: Dict[str, Any], headers: Dict[str, str]):
    ...
    ``api_key`` unset, so the SDK fills it from ANTHROPIC_API_KEY (loaded from ~/.anika/.env) and
...
def build_anthropic_bedrock_client(region: str):
    ...
    return sdk.AnthropicBedrock(
        ...
        max_retries=0,  # retry belongs to anika's outer loop (honors Retry-After)
...
def _normalize_to_mcp_wire(name: str) -> str:
    ...
    Anika tools (``read_file``) and native MCP tools registered as ``mcp_<server>_<tool>`` must
...
_OAUTH_SYSTEM_REPLACEMENTS = (
    ("Anika Agent", "Claude Code"), ("Anika agent", "Claude Code"),
    ("anika-agent", "claude-code"), ("__NEW_ORG__", "Anthropic"),
)
...
def _thinking_kwargs(reasoning_config: Dict[str, Any], model: str, effective_max_tokens: int) -> Dict[str, Any]:
    ...
    4.7+ ``thinking.display`` defaults to "omitted", hiding the reasoning Anika shows in its CLI,
...
def build_anthropic_kwargs(
    ...
    # __NEW_ORG__ Portal routes on its own catalog ids (``anthropic/claude-opus-4.8``); normalizing would
    ...
    # which silently hides reasoning text that Anika surfaces in its CLI. We request "summarized" so the
...
def create_anthropic_message(
    ...
    ``on_response(httpx_response)`` exposes headers the parsed Message drops (__NEW_ORG__ Portal's
...
_PLUGIN_COMPAT_LAZY = {
    ...
    'get_anika_home': ('anika_constants', 'get_anika_home'),
    ...
    'read_anika_oauth_credentials': ('agent.anthropic_credentials', 'read_anika_oauth_credentials'),
    ...
    'run_anika_oauth_login_pure': ('agent.anthropic_credentials', 'run_anika_oauth_login_pure'),
