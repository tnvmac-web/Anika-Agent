...
thread refreshes) → disk cache (~/.anika/models_dev_cache.json, any age) →
...
# Anika provider names → models.dev provider IDs
PROVIDER_TO_MODELS_DEV: Dict[str, str] = {
    ...
    # opencode-free is Zen-hosted (anika_cli/models.py) and models.dev's "opencode" catalog lists
    ...
    # Anika provider is "meta-ai"; both aliases are needed or muse-spark-*
...
# Reverse mapping: models.dev id → Anika ids (built lazily; many-to-one).
_MODELS_DEV_TO_PROVIDER: Optional[Dict[str, List[str]]] = None


def _models_dev_to_anika_ids(mdev_id: str) -> List[str]:
    """Return the Anika provider ids that map to *mdev_id* (may be [])."""
    global _MODELS_DEV_TO_PROVIDER
    if _MODELS_DEV_TO_PROVIDER is None:
        _MODELS_DEV_TO_PROVIDER = {}
        for anika_id, mapped in PROVIDER_TO_MODELS_DEV.items():
            _MODELS_DEV_TO_PROVIDER.setdefault(mapped, []).append(anika_id)
...
def _anika_path(name: str) -> Path:
    from anika_constants import get_anika_home
    return get_anika_home() / name


def _get_cache_path() -> Path:
    return _anika_path("models_dev_cache.json")


def _get_etag_path() -> Path:
    return _anika_path("models_dev_cache.etag")
...
def _get_provider_models(provider: str, *, allow_network: bool = False) -> Optional[Dict[str, Any]]:
    """Resolve a Anika provider ID to its models dict, or None if unknown.
...
# models the catalog does not know and never displace catalog data. Provider keys accept the Anika
...
def _provider_override_section(provider: str) -> Optional[Dict[str, Any]]:
    """Override section for *provider* (keyed by Anika OR models.dev id), or None."""
    overrides = _load_model_overrides()
    provider_key = (provider or "").strip()
    if not overrides or not provider_key:
        return None
    # Forward (Anika → models.dev id) and reverse (caller passed a models.dev id, config keyed by Anika id) aliases.
    candidates = [provider_key, PROVIDER_TO_MODELS_DEV.get(provider_key), *_models_dev_to_anika_ids(provider_key)]
...
def get_provider_info(provider_id: str, *, allow_network: bool = True) -> Optional[ProviderInfo]:
    """Provider metadata by Anika or models.dev ID, or None if not cataloged. ``allow_network`` defaults to True (interactive setup)."""
...
def get_model_info(provider_id: str, model_id: str, *, allow_network: bool = False) -> Optional[ModelInfo]:
    """Full model metadata by Anika or models.dev provider ID (exact match, then case-insensitive), or
