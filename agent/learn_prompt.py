...
``skill_manage`` per the Anika authoring standards; large prose sources get the knowledge-base layout (lean SKILL.md
...
Follow the Anika skill-authoring standards exactly. These are the same
...
- version: 0.1.0
- author: always the literal value `Anika`. NEVER fill it from the host
  ...
  a privacy leak the user never opted into; the skill names itself as Anika.
- platforms: declare `[macos]`, `[linux]`, and/or `[windows]` IF the skill
  ...
  skills.
- metadata.anika.tags: a few Capitalized, Relevant, Tags.
...
4. "## How to Run" — the canonical invocation, framed through Anika tools.
...
Anika-tool framing (this is what makes it a skill, not shell docs):
- Frame running scripts as "invoke through the `terminal` tool".
- Reference Anika tools by name in backticks: `terminal`, `read_file`,
