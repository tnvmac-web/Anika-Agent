...
try:
    import anika_cli as _anika_cli

    _ANIKA_VERSION = str(_anika_cli.__version__)
except Exception:
    _ANIKA_VERSION = "0.0.0"
_API_CLIENT = f"anika-agent/{_ANIKA_VERSION}"  # client context per Gemini's partner-integration guidance
...
_FREE_TIER_GUIDANCE = (
    ...
    "Anika typically makes 3-10 API calls per user turn, so the free tier is exhausted in a handful of "
...
_STANDARD_KEY_GUIDANCE = (
    ...
    "generativelanguage.googleapis.com). Then update GEMINI_API_KEY / GOOGLE_API_KEY in ~/.anika/.env and "
...
_MISSING_KEY_ERROR = (
    ...
    "environment / ~/.anika/.env (get one at https://aistudio.google.com/app/apikey), or run `anika setup` to "
...
class GeminiAPIError(Exception):
    """Error shape compatible with Anika retry/error classification."""
...
class GeminiNativeClient:
    ...
    # (No ANIKA_SKIP_ASYNC_WRAP — the async path has a real conversion, AsyncGeminiNativeClient.)
