...
def _routermint_headers() -> dict:
    ...
    return {"User-Agent": f"AnikaAgent/{_ANIKA_VERSION}"}
...
_ROUTE_DEFAULT_HEADERS = (
    ...
    ("githubcopilot.com", lambda self, url: _lazy_attr("anika_cli.models", "copilot_default_headers")()),
    ...
    ("x.ai", lambda self, url: _lazy_attr("tools.xai_http", "anika_xai_default_headers")()),
)
...
class ClientLifecycleMixin:
    ...
    def _try_refresh_codex_client_credentials(self, *, force: bool = True) -> bool:
        ...
        try:
            from anika_cli import auth as _auth
            resolve = (
    ...
    def _try_refresh_nous_client_credentials(self, *, force: bool = True) -> bool:
        ...
        try:
            ...
            timeout = env_float("ANIKA_NOUS_TIMEOUT_SECONDS", 15)
            ...
            logger.debug("__NEW_ORG__ credential refresh failed: %s", exc)
            return False
        api_key, base_url = creds.get("api_key"), creds.get("base_url")
        if not _valid_credential_pair(api_key, base_url):
            return False
        if self.api_mode == "anthropic_messages":
            ...
            return True
        # __NEW_ORG__ requests should not inherit OpenRouter-only attribution headers.
    ...
    def _try_refresh_env_client_credentials(self) -> bool:
        """Adopt ``~/.anika/.env`` credential/base-url edits at the turn boundary (a Settings save updates ``.env``
    ...
    def _try_refresh_copilot_client_credentials(self) -> bool:
        ...
        if not self._is_copilot_provider():
            return False
        try:
            from anika_cli.copilot_auth import resolve_copilot_token, get_copilot_api_token, evict_cached_exchanged_token
    ...
    def _apply_client_headers_for_base_url(self, base_url: str, *, apply_user_headers: bool = True) -> None:
        ...
        if self.api_mode not in ("anthropic_messages", "bedrock_converse"):
            try:
                from anika_cli.config import apply_custom_provider_extra_headers_to_client_kwargs
