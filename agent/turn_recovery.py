...
def _try_refresh_nous_paid_entitlement_credentials(agent) -> bool:
    """Refresh __NEW_ORG__ runtime credentials after a fresh paid-entitlement check."""
    try:
...
def _print_nous_401_diagnostics(agent: Any, api_error: Exception) -> None:
    """__NEW_ORG__ 401 that survived a credential refresh: likely Portal OAuth expired/revoked,
    ...
    try:
        ...
        pass
    _plines(agent, "🔐 __NEW_ORG__ 401 — Portal authentication failed.")
    if _body_text:
        _plines(agent, f"   Response: {_body_text}")
    if not _print_nous_entitlement_guidance(agent, "__NEW_ORG__ model access"):
        _plines(agent, "   Most likely: Portal OAuth expired, account out of credits, or agent key revoked.")
    _plines(
        agent,
        "   Troubleshooting:",
        "     • Re-authenticate: anika auth add nous",
        "     • Check credits / billing: https://portal.__NEW_DOMAIN__",
        f"     • Verify stored credentials: {display_anika_home()}/auth.json",
...
def _print_anthropic_401_diagnostics(agent: Any, key: Any) -> None:
    ...
    if is_token_provider(key):
        ...
        _plines(
            ...
            "   Run `anika doctor` for credential-chain diagnostics, or",
    ...
    _dhh = display_anika_home()
    _plines(
        agent,
        "   Troubleshooting:",
        f"     • Check ANTHROPIC_TOKEN in {_dhh}/.env for Anika-managed OAuth/setup tokens",
        ...
        "     • Legacy cleanup: anika config set ANTHROPIC_TOKEN \"\"",
        "     • Clear stale keys: anika config set ANTHROPIC_API_KEY \"\"",
    )
...
def _refresh_credentials_after_401(
    ...
    if (
        ...
        if agent._try_refresh_nous_client_credentials(force=True):
            agent._buffer_vprint("🔐 __NEW_ORG__ agent key refreshed after 401. Retrying request...")
            return True
...
def recover_after_classification(
    ...
    __NEW_ORG__ paid-entitlement refresh → credential-pool rotation → image shrink →
    ...
    Returns ``(retry_now, recovered_with_pool)``; the latter feeds the __NEW_ORG__ rate-limit guard."""
    ...
    if (
        ...
        if _try_refresh_nous_paid_entitlement_credentials(agent):
            _vlines(agent, "🔐 __NEW_ORG__ paid access verified — refreshed runtime credentials and retrying request...")
            return True, False
    ...
    # Reasoning-mandatory route (__NEW_ORG__ Portal / OpenRouter, e.g. GLM-5.3) 400s on
    ...
    if (
        ...
        try:
            from anika_cli.models_reasoning_caps import refresh_reasoning_caps_async
...
def _print_nonretryable_auth_guidance(
    ...
    if classified.reason == FailoverReason.billing and _print_billing_or_entitlement_guidance(
        ...
        return
    if provider == "nous" and _print_nous_entitlement_guidance(agent, "__NEW_ORG__ model access"):
        return
    if provider in {"openai-codex", "xai-oauth", "nous"} and status_code == 401:
        if provider == "openai-codex":
            _vlines(
                ...
                "      2. Then run `anika auth` to re-authenticate.",
            )
        elif provider == "xai-oauth":
            _vlines(
                agent,
                "   💡 xAI OAuth token was rejected (HTTP 401). To fix:",
                "      re-authenticate with xAI Grok OAuth (SuperGrok / Premium+) from `anika model`.",
            )
        else:  # nous
            _vlines(
                agent,
                "   💡 __NEW_ORG__ Portal OAuth token was rejected (HTTP 401). Your token may be",
                "      expired, revoked, or your account may be out of credits. To fix:",
                "      1. Re-authenticate: anika portal",
                "      2. Check your portal account: https://portal.__NEW_DOMAIN__",
            )
            # ``:free`` is OpenRouter slug syntax; __NEW_ORG__ Portal will reject the model
            ...
            if isinstance(model, str) and model.endswith(":free"):
                _vlines(
                    ...
                    "         __NEW_ORG__ Portal won't recognize that model name. Either switch to a",
                    f"         __NEW_ORG__ catalog model, or run `/model openrouter:{model}` to use OpenRouter.",
                )
        return
    _vlines(
        agent,
        "   💡 Your API key was rejected by the provider. Check:",
        "      • Is the key valid? Run: anika setup",
...
def nonretryable_client_error_result(
    ...
    if classified.reason == FailoverReason.content_policy_blocked:
        _vlines(
            ...
            "        anika fallback add   (interactive picker — same as `anika model`)",
        )
...
def max_retries_exhausted_result(
    ...
    if _is_thinking_timeout:
        _vlines(
            ...
            "in `~/.anika/config.yaml` to extend the per-call timeout. (Anika's built-in floor is 600s for "
...
def log_api_error_attempt(
    ...
    if getattr(api_error, "status_code", None) == 404:
        try:
            from anika_cli.model_normalize import suggest_prefixed_model_id
        ...
        if _suggestion:
            _blines(
                ...
                f"      Did you mean '{_suggestion}'?  Re-pick it with `anika model`.",
            )
...
def _is_genuine_nous_rate_limit(agent: Any, api_error: Exception, error_context: Any) -> bool:
    """Record a genuine account-level __NEW_ORG__ 429 to the cross-session breaker; upstream
    ...
    try:
        ...
        if _genuine:
            ...
            logger.info(
                "__NEW_ORG__ 429 looks like upstream capacity "
...
def route_classified_error(
    ...
    chain once; genuine __NEW_ORG__ 429 → cross-session breaker + re-enter the loop exactly once."""
    ...
    # __NEW_ORG__ Portal: a genuine account-level 429 is recorded to a shared file so ALL
    ...
    if (
        ...
        # Re-enter the loop exactly once so the top-of-loop __NEW_ORG__ guard runs
