...
def _constants_path(getter_name: str) -> Path:
    """Call ``anika_constants.<getter_name>()`` (local import avoids cycles); ``~/.anika`` on any failure."""
    try:
        import anika_constants

        return getattr(anika_constants, getter_name)()
    except Exception:
        return Path(os.path.expanduser("~/.anika"))


def _anika_home_path() -> Path:
    """Active ANIKA_HOME (profile-aware). Tests monkeypatch this name."""
    return _constants_path("get_anika_home")


def _anika_root_path() -> Path:
    """Anika root dir (parent of any profile, never per-profile)."""
    return _constants_path("get_default_anika_root")


def _anika_dirs() -> list[Path]:
    """Resolved active ANIKA_HOME and global root, deduplicated.

    Both are checked so credential stores at <root>/... stay guarded when
    running under a profile (ANIKA_HOME = <root>/profiles/<name>).
    """
    return list(dict.fromkeys(_resolve_each((_anika_home_path(), _anika_root_path()))))
...
def build_write_denied_paths(home: str) -> set[str]:
    ...
    anika_files = (".env", ".anthropic_oauth.json", os.path.join("cache", "bws_cache.enc.json"))
    paths = [
        *(os.path.join(home, *f) for f in home_files),
        *(str(base / f) for f in anika_files for base in (_anika_home_path(), _anika_root_path())),
...
def get_safe_write_roots() -> set[str]:
    """Resolved ANIKA_WRITE_SAFE_ROOT paths (``os.pathsep``-separated list)."""
    roots: set[str] = set()
    for path in filter(None, os.getenv("ANIKA_WRITE_SAFE_ROOT", "").split(os.pathsep)):
...
# ANIKA_HOME / root subpaths that the agent's generic file tools must not
...
_ANIKA_PROTECTED_SUBPATHS = ("state.db", "sessions", "mcp-tokens", "pairing")
...
def _classify_write_denial(path: str) -> Optional[str]:
    ...
    for base in _anika_dirs():
        for sub in _ANIKA_PROTECTED_SUBPATHS:
            with suppress(Exception):
...
def get_write_denied_error(path: str, *, verb: str = "Write") -> Optional[str]:
    ...
    if denial == "safe_root":
        ...
        return (
            f"{verb} denied: '{path}' is outside ANIKA_WRITE_SAFE_ROOT "
...
# Exact-file credential stores under ANIKA_HOME / <root>. The agent never
...
# Directory-prefix read denies under ANIKA_HOME / <root>: (subdir, message for
...
_READ_DENIED_DIRS = (
    ("mcp-tokens",
     "is the Anika MCP token directory and cannot be read directly.",
     "is a Anika MCP token file and cannot be read directly."),
    ("browser-profile",
     "is the Anika real-profile browser snapshot directory (copied cookies/logins) and cannot be read directly.",
     "is inside the Anika real-profile browser snapshot (copied cookies/logins) and cannot be read directly."),
)


def get_read_block_error(path: str) -> Optional[str]:
    """Return an error message when a read targets a denied Anika path.

    Blocked: internal skill-hub caches (prompt-injection carriers), credential
    stores under ANIKA_HOME and the global root (exact files, plus anything
    ...
    anika_dirs = _anika_dirs()
    reason = None
    if any(_is_under(resolved, hd / "skills" / ".hub") for hd in anika_dirs):
        reason = (
            "is an internal Anika cache file and cannot be read directly to prevent "
            "prompt injection. Use the skills_list or skill_view tools instead."
        )
    elif any(resolved in _resolve_each(hd / name for hd in anika_dirs) for name in _CREDENTIAL_FILE_NAMES):
        reason = (
            "is a Anika credential store and cannot be read directly. Provider tools "
            "consume these credentials through internal channels." + _DID_SUFFIX
        )
    else:
        for subdir, dir_msg, file_msg in _READ_DENIED_DIRS:
            for blocked_dir in _resolve_each(hd / subdir for hd in anika_dirs):
...
def raise_if_read_blocked(path: str) -> None:
    """Raise ``ValueError`` if ``path`` is a denied Anika read (see ``get_read_block_error``).
...
def _resolve_active_profile_name() -> str:
    """Active profile name from ANIKA_HOME: ``~/.anika`` -> ``"default"``,
    ``~/.anika/profiles/X`` -> ``"X"``; ``"default"`` on any resolution failure."""
    try:
        parts = _anika_home_path().resolve().relative_to(_anika_root_path().resolve() / "profiles").parts
...
#   <ANIKA_HOME>/profiles/<name>/sandboxes/<backend>/<task>/home/.anika/...
...
def classify_sandbox_mirror_target(path: str) -> Optional[dict]:
    """Classify a write target as a sandbox-mirror of authoritative Anika state: ``None``
    for non-mirror paths, else ``target_path`` (resolved), ``mirror_root`` (the
    ``…/home/.anika`` prefix) and ``inner_path`` (what the agent meant on the host)."""
    target = _resolve_target(path)
    parts = target.parts if target is not None else ()
    # Need at least: sandboxes / <backend> / <task> / home / .anika / <thing>; inner_idx = the .anika part.
    inner_idx = next(
        (i + 4 for i, part in enumerate(parts)
         if part == "sandboxes" and i + 5 < len(parts) and parts[i + 3] == "home" and parts[i + 4] == ".anika"),
        None,
    )
...
def get_sandbox_mirror_warning(path: str) -> Optional[str]:
    ...
    return _mirror_warning(
        ...
        "Writes here land on a copy that the host Anika process never reads — the "
        "authoritative file is likely {inner_path!r} under the real ANIKA_HOME.",
...
def classify_container_mirror_target(path: str, mirror_prefix: str | None = None) -> Optional[dict]:
    ...
    the bind mount strips the ``sandboxes/`` prefix (the agent sees plain ``/root/.anika/…``),
...
def get_container_mirror_warning(path: str, mirror_prefix: str | None = None) -> Optional[str]:
    ...
    return _mirror_warning(
        ...
        "the container's bind-mounted home — a per-task mirror that the host Anika "
...
def classify_cross_profile_target(path: str) -> Optional[dict]:
    ...
    Returns ``None`` when the target is outside Anika scope, or is inside
    ...
    try:
        ...
        root_real = _anika_root_path().resolve()
