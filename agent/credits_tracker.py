"""__NEW_ORG__ credits: parse ``x-nous-credits-*`` / ``x-nous-tool-pool-*`` response
...
def is_free_tier_model(model: str, base_url: str = "") -> bool:
    """True when *model* is a __NEW_ORG__ free-tier model, using ONLY local data: (1) ``:free`` suffix — canonical
    __NEW_ORG__ free SKU marker; (2) ``stealth/`` prefix — stealth-preview SKUs are free without the suffix
    (naming-convention trust: a PAID ``stealth/`` model would wrongly suppress the banner); (3) a PEEK into
    ``anika_cli.models``' pricing cache (filled by the model picker; a miss never fetches). Fail-open to
    ...
    if not base_url:
        return False
    try:
        from anika_cli.models import _is_model_free
        from anika_cli.models_pricing import peek_cached_pricing
...
def evaluate_credits_notices(state: CreditsState, latch: dict, *, model_is_free: bool = False) -> tuple[list[AgentNotice], list[str]]:
    ...
    if target_band != latch.get("usage_band"):
        ...
        if target_band is not None:
            # Absolute dollars used (a bare "N%" is only meaningful against a __NEW_ORG__ cap): cap − remaining,
...
def parse_credits_headers(headers: Mapping[str, str], provider: str = "") -> Optional[CreditsState]:
    ...
    try:
        ...
        # version header is absent — the hot path for non-__NEW_ORG__ providers.
        ...
        if version_val != 1:
            if version_val > 1 and not _version_warning_emitted:
                ...
                logger.warning("credits header version %d unsupported, ignoring — update Anika", version_val)
            return None
        fields: dict[str, Any] = {
...
# ── Dev fixtures (ANIKA_DEV_CREDITS_FIXTURE): throwaway scaffolding to trigger any notice state
...
def dev_fixture_credits_state() -> Optional[CreditsState]:
    """Fixture CreditsState for ANIKA_DEV_CREDITS_FIXTURE, or None (unknown name / unset). Prod-leak guard:
    applies ONLY when ANIKA_DEV_CREDITS is also on, so a stray fixture env var never surfaces fabricated balances."""
    name = os.environ.get("ANIKA_DEV_CREDITS_FIXTURE", "").strip()
    if not name or not is_truthy_value(os.environ.get("ANIKA_DEV_CREDITS")):
        return None
