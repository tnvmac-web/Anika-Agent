...
In a code workspace on an interactive surface Anika adopts a **coding posture**: a
...
from anika_cli._subprocess_compat import bounded_git_probe

logger = logging.getLogger("anika.coding_context")
...
_EDIT_FORMAT_GUIDANCE: dict[str, tuple[tuple[str, ...], str]] = {
    ...
    "replace": (
        ("claude", "sonnet", "opus", "haiku",
         ...
         "anika", "llama", "mistral", "devstral", "minimax"),
        "- Edit format: author new files with `write_file`; for edits to "
...
# Operating brief. Tool names referenced here are in the coding toolset and _ANIKA_CORE_TOOLS.
...
def _agent_config_value(config: Optional[dict[str, Any]], key: str, default: Any, *, readonly: bool) -> Any:
    ...
    if config is None:
        try:
            from anika_cli.config import load_config, load_config_readonly
...
class RuntimeMode:
    ...
    def toolset_selection(self, config: Optional[dict[str, Any]] = None) -> Optional[list[str]]:
        ...
        apply it only when the user hasn't pinned a selection (``--toolsets``, ``ANIKA_TUI_TOOLSETS``)."""
...
def is_coding_context(*, platform: Optional[str] = None, cwd: Optional[str | Path] = None, config: Optional[dict[str, Any]] = None) -> bool:
    """Whether Anika should operate in its coding posture right now."""
