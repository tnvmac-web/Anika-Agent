"""OpenAI-shape bridge shared by Anika' ACP clients.

ACP has no OpenAI-style ``tools``/``tool_calls`` channel, so Anika' tool schemas travel INTO the
...
read/edit/execute tools forwards only Anika' agent-level tools, since re-offering overlapping ones
makes Anika redo finished work.
"""
...
class StreamChunks(list):
    """Chunk list that also carries response-level attributes (e.g. ``anika_projected_messages``)
    Anika reads off the ``create`` result; a plain list would drop them on the stream path."""
