...
a cache problem must never block Anika startup.
"""
...
def resolve_cache_home(home_path: Optional[Path] = None) -> Path:
    """``home_path`` as resolved by ``load_anika_dotenv()``, else ``$ANIKA_HOME``/``~/.anika``."""
    if home_path is None:
        from anika_constants import get_anika_home

        home_path = get_anika_home()
    return home_path
...
class DiskCache(Generic[K]):
    ...
    One JSON object per backend at ``<anika_home>/cache/<basename>``::
