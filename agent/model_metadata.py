...
def _resolve_requests_verify(base_url: str = "") -> bool | str:
    ...
    for env_var in ("ANIKA_CA_BUNDLE", "REQUESTS_CA_BUNDLE", "SSL_CERT_FILE"):
...
def _cache_file(name: str) -> Path:
    ...
    return get_anika_home() / "cache" / name
...
def _endpoint_disk_cache_get(normalized: str) -> Optional[Dict[str, Dict[str, Any]]]:
    ...
    runs (``anika -q``, cron) start cold and __NEW_ORG__ bypasses the persistent context cache, so
...
DEFAULT_CONTEXT_LENGTHS = {
    ...
    # 1M verified on OpenRouter & __NEW_ORG__ portal 2026-08; qwen3-max = 256K Coding Plan snapshot)
...
_URL_TO_PROVIDER: Dict[str, str] = {
    ...
    "inference-api.__NEW_DOMAIN__": "nous", "api.deepseek.com": "deepseek",
...
def _get_context_cache_path() -> Path:
    ...
    from anika_constants import get_anika_home
    return get_anika_home() / "context_length_cache.yaml"
...
def _normalize_model_version(model: str) -> str:
    """Dots -> dashes so __NEW_ORG__ ids (claude-opus-4-6) compare with OpenRouter's (claude-opus-4.6)."""
...
def _resolve_codex_oauth_context_length_with_source(model: str, access_token: str = "") -> Tuple[Optional[int], str]:
    ...
    # ``-900k`` variants are Anika picker aliases — the Codex catalog only knows the base slug, so resolve
...
def _resolve_nous_context_length(model: str, base_url: str = "", api_key: str = "") -> Tuple[Optional[int], str]:
    """``(context_length, source)`` for a __NEW_ORG__ Portal model: portal /v1/models is authoritative
    ("portal"). Fallback matches OR's prefixed ids against the bare __NEW_ORG__ id with dot/dash
    ...
    def _safe_ctx(or_id: str, entry: dict) -> Optional[int]:
        ...
        if ctx is not None and ctx <= 32768 and _model_name_suggests_stale_32k_underreport(or_id):
            logger.info("Rejecting OpenRouter metadata context=%s for %r (known 32K underreport, __NEW_ORG__ path); falling through to hardcoded defaults", ctx, or_id)
            return None
        return ctx
...
def _validate_cached_context_length(model: str, base_url: str, cached: int, is_bedrock_context: bool, *, api_key: str = "") -> Optional[int]:
    ...
    for hit, log, msg, shown in drop_rules:
        if hit:
            ...
            _invalidate_cached_context_length(model, base_url)
            return None
    # __NEW_ORG__ Portal: /v1/models is authoritative. Bypass (don't drop) the cache so step
    # 5b reconciles OR-seeded entries without touching disk when the portal is down.
    if _infer_provider_from_url(base_url) == "nous":
        logger.debug("Bypassing persistent cache for %s@%s (__NEW_ORG__ portal authoritative)", model, base_url)
        return None
...
def _resolve_provider_aware_context_length(model: str, base_url: str, api_key: str, provider: str, effective_provider: str) -> Optional[int]:
    ...
    if effective_provider in {"copilot", "copilot-acp", "github-copilot"}:
        with contextlib.suppress(Exception):  # fall through to models.dev
            from anika_cli.models import get_copilot_model_context
            ctx = get_copilot_model_context(model, api_key=api_key)
            if ctx:
                return ctx
    # 5b/5c. __NEW_ORG__ portal and Codex OAuth (lower limits than the direct API for the same slug; its
    ...
    if not effective_provider:
        ...
        if model in metadata:
            ...
            if or_ctx == 32768 and _model_name_suggests_stale_32k_underreport(model):
                ...
                return or_ctx
    # 7. Local server before hardcoded defaults — ``Anika-3-Llama-3.1-70B`` matches ``llama``
