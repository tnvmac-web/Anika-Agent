...
def build_gateway_health_snapshot(
    ...
    base = {
        ...
        "anika.supervision_mode": mode if mode in _SUPERVISION_MODES else "unknown",
    }
    ...
    metrics: list[GatewayMetric] = [
        metric("anika.gateway.up", int(bool(gateway_running))),
        metric("anika.gateway.active_agents", active_agents),
        metric("anika.gateway.busy", int(bool(busy))),
        metric("anika.gateway.drainable", int(bool(drainable))),
        metric("anika.gateway.restart_requested", int(bool(runtime.get("restart_requested")))),
        metric("anika.gateway.state", 1, **{"anika.gateway.state": gateway_state}),
    ]
    fatal_count = 0
    events: list[GatewayHealthEvent | GatewayDiagnosticEvent] = []
    ...
    for platform, pdata in platforms.items():
        ...
        pattrs = {"anika.platform": str(platform), "anika.platform.state": state}
        metrics.append(metric("anika.platform.up", int(state in _RUNNING_PLATFORM_STATES), **pattrs))
        metrics.append(metric("anika.platform.degraded", int(is_degraded), **pattrs, **{"anika.error_code": error_code}))
        if is_degraded:
