...
_OBSERVABLE_METRIC_NAMES = (
    "anika.gateway.up", "anika.gateway.state", "anika.gateway.active_agents", "anika.gateway.busy",
    "anika.gateway.drainable", "anika.gateway.restart_requested", "anika.gateway.background_work",
    "anika.gateway.background_delegations", "anika.platform.up", "anika.platform.degraded",
    "anika.cron.scheduler.heartbeat_age_seconds", "anika.cron.scheduler.last_success_age_seconds",
    "anika.cron.scheduler.catch_up_occurrences", "anika.cron.jobs.enabled", "anika.cron.jobs.running",
    "anika.cron.jobs.overdue",
)
...
class GatewayHealthExportRuntime:
    ...
    def shutdown(self) -> None:
        ...
        if closeables:
            worker = threading.Thread(target=_close, name="anika-gateway-health-export-shutdown", daemon=True)
            worker.start()
...
def _read_runtime_snapshot(config: Dict[str, Any]):
    ...
    try:
        ...
        for name, read in (
            ("anika.gateway.background_work", _read_background_work_count),
            ("anika.gateway.background_delegations", _read_background_delegations_count),
        ):
...
def _start_metric_provider(config: Dict[str, Any], sdk: Dict[str, Any]) -> Any:
    ...
    meter = provider.get_meter("anika.gateway.health")
...
class GatewayDiagnosticLogStreamer(EmitterStreamer):
    ...
    def __init__(self, config: Dict[str, Any], sdk: Dict[str, Any]):
        ...
        self._logger = self._provider.get_logger("anika.gateway.diagnostics")
        self._sdk = sdk
        self.exported = 0
...
def start_gateway_health_export(config: Dict[str, Any]) -> GatewayHealthExportRuntime:
    ...
    if metrics_on or diagnostics_on:
        try:
            ...
            logger.warning("monitoring.gateway_health_export.enabled but OTLP SDK is unavailable; install 'anika-agent[otlp]'", exc_info=True)
    ...
    if diagnostics_on:
        try:
            ...
            thread = threading.Thread(target=_run, name="anika-gateway-health-export", daemon=True)
            thread.start()
