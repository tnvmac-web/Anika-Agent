...
def nous_logged_in() -> bool:
    """Cheap local auth-state check: a __NEW_ORG__ access token is present. Fail-closed."""
    try:
...
def build_usage_model(*, timeout: float = 10.0) -> UsageModel:
    """Fetch account-info and build the usage model; fail-open. ``ANIKA_DEV_CREDITS_FIXTURE`` short-circuits to a fixture."""
...
def _dev_fixture_usage_model() -> Optional[UsageModel]:
    """``ANIKA_DEV_CREDITS_FIXTURE`` -> fixture model (``free|healthy|low|topup|depleted``), else None."""
    name = (os.getenv("ANIKA_DEV_CREDITS_FIXTURE") or "").strip().lower()
