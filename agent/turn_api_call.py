...
the attempt while another session's __NEW_ORG__ Portal rate limit is active), ``perform_api_call``
...
def perform_api_call(
    ...
    from anika_cli.middleware import run_llm_execution_middleware
...
def nous_rate_limit_guard(
    ...
    """Skip the call if another session recorded a __NEW_ORG__ Portal rate limit: every attempt (incl.
    ...
    if agent.provider == "nous":
        try:
            ...
            if _nous_remaining is not None and _nous_remaining > 0:
                _nous_msg = (
                    f"__NEW_ORG__ Portal rate limit active — resets in {_fmt_nous_remaining(_nous_remaining)}."
                )
