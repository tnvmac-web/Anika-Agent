...
def _moa_privacy_mode(moa_raw: Any) -> str:
    ...
    from anika_cli.moa_config import coerce_privacy_filter
...
def _run_references_parallel(
    ...
    *agent*, the wait polls every ``_REFERENCE_POLL_INTERVAL_S`` so a user interrupt
    ...
    # Propagate the turn's contextvars (approval callbacks, __NEW_ORG__ conversation tag).
...
def aggregate_moa_context(
    *, user_prompt: str, api_messages: list[dict[str, Any]], reference_models: list[dict[str, Any]],
    ...
    synth_prompt = (
        ...
        "Anika agent. Focus on next steps, tool-use strategy, risks, and any "
    ...
    return (
        ...
        "normal Anika agent loop. You may call tools, continue reasoning, or "
        "finish normally.]\n"
