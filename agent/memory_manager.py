...
def _ctx_bound(fn: Callable[[], Any]) -> Callable[[], Any]:
    ...
    ContextVar-scoped ANIKA_HOME override, and an unbound worker would silently use the default profile."""
...
class MemoryManager:
    ...
    def add_provider(self, provider: MemoryProvider) -> None:
        ...
        for raw_schema in provider.get_tool_schemas():
            ...
            if tool_name in _ANIKA_CORE_TOOLS:
                logger.warning(
    ...
    def get_all_tool_schemas(self) -> List[Dict[str, Any]]:
        ...
        def _collect(provider: MemoryProvider) -> None:
            for raw_schema in provider.get_tool_schemas():
                ...
                elif schema["name"] not in _ANIKA_CORE_TOOLS and schema["name"] not in seen:
    ...
    def initialize_all(self, session_id: str, **kwargs) -> None:
        """Initialize all providers, injecting ``anika_home`` so they resolve profile-scoped paths."""
        if "anika_home" not in kwargs:
            from anika_constants import get_anika_home
            kwargs["anika_home"] = str(get_anika_home())
