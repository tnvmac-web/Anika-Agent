...
Text auto chain: main provider+model → OpenRouter → __NEW_ORG__ Portal → custom endpoint →
native Anthropic → direct API-key providers → None. Vision auto chain: main
provider (if a supported vision backend) → OpenRouter → __NEW_ORG__ → Anthropic → custom.
...
from anika_constants import OPENROUTER_BASE_URL
...
def _resolve_aux_verify(base_url: Optional[str]) -> Any:
    ...
    ``ssl_verify``, ``ANIKA_CA_BUNDLE`` / ``SSL_CERT_FILE``); any failure → httpx default (``True``)."""
    try:
        from agent.ssl_verify import resolve_httpx_verify
        from anika_cli.config import get_custom_provider_tls_settings, load_config_readonly
    ...
    try:
        ...
        if not _WARNED_KEEPALIVE_IMPORT_SKEW:
            ...
            logger.warning(
                ...
                "back to the SDK default HTTP client. Run `anika update` (or "
...
def _create_openai_client(*, api_key: str, base_url: str, **kwargs: Any) -> Any:
    ...
    with contextlib.suppress(Exception):
        from anika_cli.models import OPENCODE_ZEN_FREE_KEYLESS_PLACEHOLDER, opencode_zen_free_headers
    ...
    # Anika owns aux retry/fallback policy; the SDK default (max_retries=2) would triple
    # wall time on a hung endpoint before Anika sees one failure.
    # Anika owns auxiliary retry + provider/model fallback policy (the same-provider transient retry in
    ...
    # so a 120s timeout can stall ~360s before Anika sees a single failure (issue #54465). Disable
    # SDK-internal retries by default and let Anika control the budget; explicit callers can still override
    # via kwargs.
...
def _run_protected_sync_provider_call(callback: Callable[[dict[str, Any]], Any], kwargs: dict[str, Any]) -> Any:
    ...
    threading.Thread(
        target=provider_context.run, args=(_provider_worker,), name="anika-protected-aux-provider",
        daemon=True).start()
    while True:
...
def _client_declares(client_obj: Any, flag: str) -> bool:
    ...
    not isinstance, so out-of-tree clients can opt out of wrappers unimported (cf. SUPPORTS_ANIKA_TOOL_CALLS)."""
    try:
...
def _fast_model_from_catalog(provider_id: str) -> str:
    ...
    try:
        from anika_cli.auth import resolve_api_key_provider_credentials
        from anika_cli.models_pricing import fetch_models_with_pricing
        ...
        if not api_key and is_nous:
            # __NEW_ORG__ is OAuth (resolver raises); anonymous reads return the full catalog.
            try:
                from anika_cli.models_pricing import _resolve_nous_pricing_credentials
                api_key, base_url = _resolve_nous_pricing_credentials()
            except Exception:
                logger.debug("No __NEW_ORG__ credentials for catalog", exc_info=True)
        if not base_url:
        ...
        _nous_kwargs = {}
        if is_nous:
            from anika_cli.models_pricing import _NOUS_CATALOG_TTL_SECONDS
    ...
    if is_nous:
        ...
        try:
            from anika_cli.models_pricing import nous_policy_allowed_ids, restrict_to_nous_policy
            ids = restrict_to_nous_policy(ids, nous_policy_allowed_ids())
        except Exception:
            logger.debug("__NEW_ORG__ policy filter unavailable", exc_info=True)
...
def _get_aux_model_for_provider(provider_id: str, *, prefer_fast: bool = False) -> str:
    ...
    if picked and provider_id.strip().lower() == "nous":
        try:
            ...
            logger.debug("__NEW_ORG__ policy check unavailable", exc_info=True)
    return picked
...
_OR_HEADERS_BASE = {
    "HTTP-Referer": "https://anika-agent.__NEW_DOMAIN__",
    "X-Title": "Anika Agent",
...
def _apply_user_default_headers(headers: dict | None) -> dict | None:
    ...
    try:
        from anika_cli.config import cfg_get, load_config
        _cfg = load_config()
...
def build_or_headers(or_config: dict | None = None) -> dict:
    ...
    Precedence env > config > default: ``ANIKA_OPENROUTER_CACHE`` overrides
    ``openrouter.response_cache``; ``ANIKA_OPENROUTER_CACHE_TTL`` (1-86400 s) overrides
    ``openrouter.response_cache_ttl``. ``or_config=None`` reads from disk.
    """
    headers = dict(_OR_HEADERS_BASE)
    if or_config is None:
        try:
            from anika_cli.config import load_config_readonly
            or_config = load_config_readonly().get("openrouter", {})
        except Exception:
            or_config = {}
    env_cache = os.environ.get("ANIKA_OPENROUTER_CACHE", "").strip().lower()
    if not (env_cache in {"1", "true", "yes", "on"} if env_cache else or_config.get("response_cache", False)):
        return headers
    headers["X-OpenRouter-Cache"] = "true"
    env_ttl = os.environ.get("ANIKA_OPENROUTER_CACHE_TTL", "").strip()
    if env_ttl:
...
_NVIDIA_NIM_CLOUD_HEADERS = {"X-BILLING-INVOKE-ORIGIN": "AnikaAgent"}
...
from anika_cli import __version__ as _ANIKA_VERSION

_AI_GATEWAY_HEADERS = {
    "HTTP-Referer": "https://anika-agent.__NEW_DOMAIN__",
    "X-Title": "Anika Agent",
    "User-Agent": f"AnikaAgent/{_ANIKA_VERSION}",
}

# __NEW_ORG__ Portal attribution extra_body. Tags come from agent.portal_tags so the client= marker
# tracks anika_cli.__version__ — never inline a literal here.
from agent.portal_tags import nous_portal_tags as _nous_portal_tags


def _nous_extra_body() -> dict:
    """Fresh __NEW_ORG__ Portal ``extra_body`` (per call, so a hot-reloaded version is reflected)."""
    return {"tags": _nous_portal_tags()}


# Set at resolve time — True if the auxiliary client points to __NEW_ORG__ Portal
...
_NOUS_DEFAULT_BASE_URL = "https://inference-api.__NEW_DOMAIN__/v1"
_ANTHROPIC_DEFAULT_BASE_URL = "https://api.anthropic.com"
_AUTH_JSON_PATH = get_anika_home() / "auth.json"
...
def _pool_runtime_base_url(entry: Any, fallback: str = "") -> str:
    ...
    if getattr(entry, "provider", None) == "nous":
        ...
        from anika_cli.auth import _nous_inference_env_override
...
def _nous_min_key_ttl_seconds() -> int:
    try:
        return max(60, int(os.getenv("ANIKA_NOUS_MIN_KEY_TTL_SECONDS", "1800")))
...
class _CodexCompletionsAdapter:
    ...
    def _build_responses_kwargs(self, kwargs: Dict[str, Any]) -> Tuple[Dict[str, Any], str, Any]:
        ...
        resp_kwargs: Dict[str, Any] = {
            # Codex only knows the base slug; strip the Anika ``-900k`` picker suffix.
...
class _AnthropicCompletionsAdapter:
    ...
    def __init__(self, real_client: Any, model: str, is_oauth: bool = False, base_url: str | None = None):
        ...
        # Caller URL first; fall back to the SDK client's host only for __NEW_ORG__ Portal — a blanket
    ...
    def create(self, **kwargs) -> Any:
        ...
        # (already TRANSLATED to native fields — raw would 400 on strict gateways) and ``_`` Anika plumbing.
...
def _endpoint_speaks_anthropic_messages(base_url: str) -> bool:
    ...
    Mirrors ``anika_cli.runtime_provider._detect_api_mode_for_url`` so aux and main agree: any
...
def _maybe_wrap_anthropic(
    ...
    """
    # Anthropic/Bedrock/Codex wrappers, plus any client declaring ANIKA_SKIP_TRANSPORT_WRAP
    ...
    if (
        ...
        or _client_declares(client_obj, "ANIKA_SKIP_TRANSPORT_WRAP")
    ):
        return client_obj
...
def _read_nous_auth() -> Optional[dict]:
    """__NEW_ORG__ provider state dict from the credential pool or ~/.anika/auth.json; None when not active with tokens."""
    ...
    try:
        ...
        logger.debug("Could not read __NEW_ORG__ auth: %s", exc)
        return None


def _nous_api_key(provider: dict) -> str:
    """Extract a usable __NEW_ORG__ inference JWT from stored auth state."""
    from anika_cli.auth import _nous_invoke_jwt_is_usable
...
def _resolve_nous_pool_runtime_api(*, force_refresh: bool = False) -> Optional[tuple[str, str]]:
    """Resolve __NEW_ORG__ auxiliary credentials from the selected pool entry."""
    try:
        from anika_cli.auth import _agent_key_is_usable
        pool = load_pool("nous")
    except Exception as exc:
        logger.debug("Auxiliary __NEW_ORG__ pool credential resolution failed: %s", exc)
        return None
    ...
    try:
        ...
        logger.debug("Auxiliary __NEW_ORG__ pool selection failed: %s", exc)
        return None
    if entry is None:
        return None
    ...
    if force_refresh or not _agent_key_is_usable(_entry_state(entry), _nous_min_key_ttl_seconds()):
        try:
            ...
            logger.debug("Auxiliary __NEW_ORG__ pool refresh failed: %s", exc)
            refreshed = None
...
def _resolve_nous_runtime_api(
    *, force_refresh: bool = False, stale_access_token: Optional[str] = None
    ...
    """Fresh __NEW_ORG__ runtime credentials (pool first, then auth store + JWT refresh) — mirrors the main
    ...
    if pooled is not None:
        return pooled
    try:
        from anika_cli.auth import resolve_nous_runtime_credentials
        creds = resolve_nous_runtime_credentials(
            timeout_seconds=env_float("ANIKA_NOUS_TIMEOUT_SECONDS", 15),
            force_refresh=force_refresh,
            stale_access_token=stale_access_token or None,
        )
    except Exception as exc:
        logger.debug("Auxiliary __NEW_ORG__ runtime credential resolution failed: %s", exc)
        return None
...
def _resolve_xai_oauth_for_aux() -> Optional[Tuple[str, str]]:
    ...
    """
    try:
        from anika_cli.auth import DEFAULT_XAI_OAUTH_BASE_URL, _xai_validate_inference_base_url
        ...
        if pool and pool.has_credentials():
            ...
            if entry is not None:
                ...
                base_url = _xai_validate_inference_base_url(
                    _url(os.getenv("ANIKA_XAI_BASE_URL", ""))
    ...
    try:
        from anika_cli.auth import resolve_xai_oauth_runtime_credentials
...
def _read_codex_access_token() -> Optional[str]:
    ...
    if pool_present:
        ...
        if token:
            return token
    try:
        from anika_cli.auth import _read_codex_tokens
...
def _resolve_api_key_provider() -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    try:
        from anika_cli.auth import PROVIDER_REGISTRY, resolve_api_key_provider_credentials
    except ImportError:
    ...
    for provider_id, pconfig in PROVIDER_REGISTRY.items():
        ...
        if provider_id == "anthropic":
            ...
            with contextlib.suppress(ImportError):
                from anika_cli.auth import is_provider_explicitly_configured
        ...
        if base_url_host_matches(base_url, "api.kimi.com"):
            ...
            from anika_cli.models import copilot_default_headers
...
def _endpoint_default_headers(
    ...
    if base_url_host_matches(base_url, "api.kimi.com"):
        ...
        from anika_cli.copilot_auth import copilot_request_headers
        ...
        from tools.xai_http import anika_xai_default_headers
        headers = dict(anika_xai_default_headers())
    else:
...
def _aux_openrouter_settings() -> Tuple[bool, str]:
    ...
    try:
        from anika_cli.config import cfg_get, load_config_readonly
...
def _try_nous(vision: bool = False) -> Tuple[Optional[OpenAI], Optional[str]]:
    # Cross-session rate guard: another session's 429 means skip __NEW_ORG__ rather than pile onto the tapped RPH bucket.
    with contextlib.suppress(Exception):
        from agent.nous_rate_guard import nous_rate_limit_remaining
        _remaining = nous_rate_limit_remaining()
        if _remaining is not None and _remaining > 0:
            logger.debug("Auxiliary: skipping __NEW_ORG__ Portal (rate-limited, resets in %.0fs)", _remaining)
            _mark_provider_unhealthy("nous", ttl=_remaining)
            return None, None
    nous = _read_nous_auth()
    runtime = _resolve_nous_runtime_api(force_refresh=False)
    if runtime is None and not nous:
        logger.warning("Auxiliary __NEW_ORG__ client unavailable: no __NEW_ORG__ authentication found (run: anika auth).")
    ...
    logger.debug("Auxiliary client: __NEW_ORG__ Portal")
    ...
    if not _aux_probe_active():
        try:
            from anika_cli.models import get_nous_recommended_aux_model
    ...
    if runtime is not None:
        ...
        if not api_key:
            logger.warning(
                "Auxiliary __NEW_ORG__ client unavailable: no usable inference JWT found "
                "(run: anika auth add nous)."
            )
            _mark_provider_unhealthy("nous", ttl=60)
            return None, None
        base_url = str(
...
def _refresh_nous_recommended_model(*, vision: bool, stale_model: Optional[str]) -> Optional[str]:
    ...
    try:
        ...
        logger.debug("__NEW_ORG__ recommended-model refresh failed (%s); using default %s", exc, _NOUS_MODEL)
...
def _read_main_field(field: str, *, readonly: bool, lower: bool = False) -> str:
    ...
    with contextlib.suppress(Exception):
        from anika_cli import config as _cfg_mod
...
def _resolve_moa_aggregator(preset_name: Optional[str]) -> Tuple[Optional[str], Optional[str]]:
    ...
    try:
        ...
        from anika_cli.moa_config import resolve_moa_preset
...
def _validate_base_url(base_url: str) -> None:
    ...
    try:
        ...
        raise RuntimeError(
            ...
            "Run `anika setup` or `anika model` and enter a valid http(s) base URL."
        ) from exc
...
def _build_xai_oauth_aux_client(model: str) -> Tuple[Optional[Any], Optional[str]]:
    ...
    real_client = _create_openai_client(
        api_key=api_key, base_url=base_url, default_headers=anika_xai_default_headers()
    )
    return CodexAuxiliaryClient(real_client, model), model
...
def _try_azure_foundry(
    *, model: Optional[str] = None, explicit_api_key: Optional[str] = None,
    ...
    try:
        from anika_cli.runtime_provider import _resolve_azure_foundry_runtime
...
def _nous_portal_account_has_fresh_paid_access() -> bool:
    """Return True only when the fresh __NEW_ORG__ account API says paid access is allowed."""
    try:
        from anika_cli.nous_account import get_nous_portal_account_info
        return get_nous_portal_account_info(force_fresh=True).paid_service_access is True
    except Exception as exc:
        logger.debug("Auxiliary __NEW_ORG__ paid-entitlement refresh check failed: %s", exc)
        return False
...
_POOL_PROVIDER_BY_HOST = (
    ...
    ("inference-api.__NEW_DOMAIN__", "nous"), ("api.anthropic.com", "anthropic"),
...
_AUTH_REFRESH_PROVIDER_BY_HOST = (
    ...
    ("api.anthropic.com", "anthropic"), ("inference-api.__NEW_DOMAIN__", "nous"),
)
...
def _recoverable_pool_provider(
    ...
    if main_runtime:
        ...
        if rt_provider and rt_provider not in {"", "auto", "custom"}:
            with contextlib.suppress(Exception):
                from anika_cli.auth import PROVIDER_REGISTRY
...
def _refresh_copilot_credentials() -> bool:
    from anika_cli.copilot_auth import _jwt_cache, _token_fingerprint, exchange_copilot_token, resolve_copilot_token
...
def _refresh_codex_credentials() -> bool:
    from anika_cli.auth import resolve_codex_runtime_credentials
...
def _refresh_nous_credentials() -> bool:
    ...
    return _creds_have_api_key(resolve_nous_runtime_credentials(
        timeout_seconds=env_float("ANIKA_NOUS_TIMEOUT_SECONDS", 15), force_refresh=True
    ))
...
def _fallback_destination(
    ...
    attached = getattr(fb_client, "_anika_fallback_destination", None)
...
def _resolve_fallback_entry(entry: Dict[str, Any]) -> Tuple[Optional[Any], Optional[str]]:
    ...
    if client is not None:
        with contextlib.suppress(Exception):
            client._anika_fallback_destination = _fallback_destination_from_entry(entry, client, resolved_model)
...
def _warn_stale_openai_base_url(runtime_provider: str) -> None:
    ...
    ~/.anika/.env value after `anika model` poisons routing)."""
    ...
    if (_env_base and _cfg_provider and _cfg_provider != "custom" and not _cfg_provider.startswith("custom:")):
        logger.warning(
            ...
            "Run: anika model to reconfigure, or remove "
            "OPENAI_BASE_URL from ~/.anika/.env",
...
def _resolve_auto_route(
    ...
    top-level chain; (3) OpenRouter → __NEW_ORG__ → custom → Codex → API-key providers, only with no policy
...
def _effective_provider_for_client(client: Any, fallback: str) -> str:
    ...
    effective_provider = getattr(client, "_anika_aux_effective_provider", "")
...
def _to_async_client(sync_client, model: str, is_vision: bool = False):
    ...
    if _client_declares(sync_client, "ANIKA_SKIP_ASYNC_WRAP"):
        return sync_client, model
    ...
    # Anika owns the auxiliary retry/timeout budget; disable SDK-internal retries.
    # See #54465.
...
def _normalize_resolved_model(model_name: Optional[str], provider: str) -> Optional[str]:
    ...
    if not model_name:
        return model_name
    try:
        from anika_cli.model_normalize import normalize_model_for_provider
...
def _resolve_auto_branch(req: _ResolveRequest) -> _ResolveResult:
    ...
    if routed_client is not None and effective_provider:
        try:
            setattr(routed_client, "_anika_aux_effective_provider", effective_provider)
...
def _resolve_nous_branch(req: _ResolveRequest) -> _ResolveResult:
    ...
    if client is None:
        logger.warning("resolve_provider_client: nous requested but __NEW_ORG__ Portal not configured (run: anika auth)")
        return None, None
    ...
    from anika_cli.providers import nous_api_mode
...
def _resolve_openai_codex_branch(req: _ResolveRequest) -> _ResolveResult:
    ...
    if not model:
        ...
        return None, None
    no_token_msg = "resolve_provider_client: openai-codex requested but no Codex OAuth token found (run: anika model)"
    if req.raw_codex:
...
def _resolve_xai_oauth_branch(req: _ResolveRequest) -> _ResolveResult:
    ...
    return _route_or_warn(req, client, default,
                          ...
                          "OAuth token found (run: anika model -> xAI Grok OAuth — SuperGrok / Premium+)")
...
def _resolve_azure_foundry_branch(req: _ResolveRequest) -> _ResolveResult:
    ...
    return _route_or_warn(req, client, default_model,
                          ...
                          "runtime resolution failed (run: anika doctor for diagnostics)")
...
def _resolve_api_key_branch(req: _ResolveRequest, pconfig: Any, resolve_creds: Callable) -> _ResolveResult:
    ...
    try:
        from anika_cli.models import opencode_zen_free_runtime as _oc_free_rt
    ...
    if provider == "copilot" and final_model and not req.raw_codex:
        with contextlib.suppress(ImportError):
            from anika_cli.models import _should_use_copilot_responses_api
...
def _resolve_registry_branch(req: _ResolveRequest) -> _ResolveResult:
    ...
    try:
        ...
        logger.debug("anika_cli.auth not available for provider %s", provider)
        return None, None
...
def resolve_provider_client(
    ...
    # Excluded: ``auto`` (a stale main slug could pair with any picked provider) and __NEW_ORG__ + vision (the
    ...
    if not model and provider != "auto" and not (provider == "nous" and is_vision):
        ...
        # not explicitly request one. (# compression-current-model) __NEW_ORG__ + vision is the one carve-out: the
...
def get_available_vision_backends() -> List[str]:
    """Available vision backends in auto-selection order (active provider → OpenRouter → __NEW_ORG__ → DeepInfra).
...
def _vision_main_provider_client(
    ...
    if main_provider == "nous":
        # __NEW_ORG__ picks its vision model from Portal tier-aware slots inside _try_nous(vision=True);
...
def _vision_auto_route(
    ...
    """Auto-detect order: 1. main provider + model, 2. OpenRouter, 3. __NEW_ORG__ Portal, 4. DeepInfra, 5. stop."""
...
def get_auxiliary_extra_body() -> dict:
    """Return extra_body kwargs (__NEW_ORG__ Portal product tags when Nous-backed, else {})."""
...
def _refresh_nous_auxiliary_client(
    *, cache_provider: str, model: Optional[str], async_mode: bool, base_url: Optional[str] = None,
    ...
    """Refresh __NEW_ORG__ runtime creds, rebuild the client, and replace the cache entry.
...
def _preserve_provider_with_base_url(prov: Optional[str]) -> bool:
    ...
    if normalized in {"", "auto", "custom"} or normalized.startswith("custom:"):
        return False
    try:
        from anika_cli.providers import get_provider
...
_NOUS_PROVIDER_NAMES = frozenset({"nous", "nous-portal", "__NEW_ORG__"})


def _nous_on_messages_wire(provider_norm: str, model: str) -> bool:
    """True when a __NEW_ORG__ Portal route serves ``model`` over /v1/messages (dual-wire catalog)."""
...
def _merge_aux_extra_body(
    ...
    """Caller extra_body + profile body/reasoning + generic reasoning fallback + __NEW_ORG__ tags."""
...
def _managed_local_netloc() -> str:
    ...
    if now - ts < _MANAGED_LOCAL_STATE_TTL_S:
        return cached
    try:
        from anika_cli.local_runtime.supervisor import state_path
...
def _is_managed_local_endpoint(base_url: Optional[str]) -> bool:
    """True when *base_url* targets the llama-server this Anika manages."""
    if not base_url:
        return False
...
def _resolve_call_client(
            ...
            if _explicit and _explicit not in {"auto", "openrouter", "custom"}:
                ...
                if fb_client is None:
                    raise RuntimeError(
                        ...
                        f"a different provider with `anika model`.")
    ...
    if client is None:
        raise RuntimeError(f"No LLM provider configured for task={task} "
                           f"provider={resolved_provider}. Run: anika setup")
...
def _refreshed_nous_step(route: _LadderRoute, kwargs: Dict[str, Any], message: str) -> Optional[_LadderStep]:
    """Rebuild the __NEW_ORG__ client after a credential event; None when nothing refreshed."""
...
def _ladder_nous_rungs(
    ...
    if _is_model_not_found_error(first_err) and client_is_nous:
        ...
        if healed_model and healed_model != kwargs.get("model"):
            logger.warning("Auxiliary %s%s: model %r no longer in __NEW_ORG__ catalog; "
    ...
    if _is_payment_error(first_err) and client_is_nous and _nous_portal_account_has_fresh_paid_access():
        step = _refreshed_nous_step(
            route, kwargs,
            "Auxiliary %s%s: refreshed __NEW_ORG__ runtime credentials after paid account check, retrying")
        if step is not None:
            resp, first_err = yield from _rung(
    ...
    if _is_auth_error(first_err) and client_is_nous:
        step = _refreshed_nous_step(
            route, kwargs, "Auxiliary %s%s: refreshed __NEW_ORG__ runtime credentials after 401, retrying")
        if step is not None:
...
def _aux_recovery_ladder(
    ...
    strips → __NEW_ORG__ heal/refresh → credential refresh/pool rotation → provider fallback.
    ...
    client_is_nous = (resolved_provider == "nous"
                      or base_url_host_matches(base_info, "inference-api.__NEW_DOMAIN__"))
