...
Owns one Codex thread per Anika session: drives ``turn/start``, consumes
...
# Anika' tools.terminal.security_mode -> Codex permissions profile id.
# Missing config -> workspace-write (Codex's own default).
_ANIKA_TO_CODEX_PERMISSION_PROFILE = {
...
class CodexAppServerSession:
    """One Codex thread per Anika session, lifetime owned by AIAgent. Not thread-safe: one caller at a time."""
    ...
    def __init__(
        ...
        self._permission_profile = permission_profile or _ANIKA_TO_CODEX_PERMISSION_PROFILE.get(
            os.environ.get("ANIKA_TERMINAL_SECURITY_MODE", "auto"), "workspace-write"
        )
    ...
    def ensure_started(self) -> str:
        ...
        self._client.initialize(client_name="anika", client_title="Anika Agent", client_version=_get_anika_version())
    ...
    def _handle_server_request(self, req: dict) -> None:
        """Answer a codex server request (approval / elicitation) via Anika' approval flow.
    ...
    def _respond_elicitation(self, params: dict) -> dict:
        """MCP elicitation: auto-accept our own anika-tools server (opted in by enabling the runtime;
        exposes nothing codex's shell can't do); decline others so the user opts in via codex's own flow."""
        action = "accept" if (params.get("serverName") or "") == "anika-tools" else "decline"
...
# Anika approval choice -> codex decision (app-server-protocol v2). "deny" and
...
def _approval_choice_to_codex_decision(choice: str) -> str:
    """Map a Anika approval choice onto codex's approval decision wire value."""
...
def _get_anika_version() -> str:
    """Best-effort Anika version string for codex's userAgent line."""
    try:
