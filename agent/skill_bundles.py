...
YAML files in ``<ANIKA_HOME>/skill-bundles/`` (``name``, ``description``,
...
def _bundles_dir() -> Path:
    """Bundles directory: ``ANIKA_BUNDLES_DIR`` override (tests) or ``<ANIKA_HOME>/skill-bundles``."""
    override = os.environ.get("ANIKA_BUNDLES_DIR")
    return Path(override).expanduser() if override else get_anika_home() / "skill-bundles"
...
# File-level CRUD — used by `anika bundles`.
