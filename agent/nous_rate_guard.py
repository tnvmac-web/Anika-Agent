"""Cross-session rate limit guard for __NEW_ORG__ Portal.

Writes rate limit state to a shared file so all sessions (CLI, gateway, cron,
auxiliary) can check whether __NEW_ORG__ Portal is currently rate-limited before making
requests. Without it each 429 fans out into up to 9 calls per turn (3 SDK
retries x 3 Anika retries), all counted against RPH.
"""
...
def _state_path() -> str:
    """Path to the __NEW_ORG__ rate limit state file."""
    try:
        from anika_constants import get_anika_home
        base = get_anika_home()
    except ImportError:
        base = os.path.join(os.path.expanduser("~"), ".anika")
    return os.path.join(base, "rate_limits", "__NEW_ORG__.json")
...
def record_nous_rate_limit(
    *, headers: Optional[Mapping[str, str]] = None, error_context: Optional[dict[str, Any]] = None,
    ...
    """Record that __NEW_ORG__ Portal is rate-limited in the shared state file.
    ...
    try:
        ...
        logger.info("__NEW_ORG__ rate limit recorded: resets in %.0fs (at %.0f)", reset_at - now, reset_at)
    except Exception as exc:
        logger.debug("Failed to write __NEW_ORG__ rate limit state: %s", exc)
...
def clear_nous_rate_limit() -> None:
    """Clear the rate limit state (e.g., after a successful __NEW_ORG__ request)."""
    try:
        os.unlink(_state_path())
    except FileNotFoundError:
        pass
    except OSError as exc:
        logger.debug("Failed to clear __NEW_ORG__ rate limit state: %s", exc)
...
def is_genuine_nous_rate_limit(
    *, headers: Optional[Mapping[str, str]] = None, last_known_state: Optional[Any] = None,
    ...
    """Decide whether a 429 from __NEW_ORG__ Portal is a real account rate limit.

    __NEW_ORG__ multiplexes upstream providers behind one key, so a 429 may be one
    upstream model out of capacity (clears in seconds) rather than our quota;
    tripping the breaker on that would block every __NEW_ORG__ model for minutes.
