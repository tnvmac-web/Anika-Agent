...
class BillingBlock:
    ...
    __NEW_ORG__ has an in-app billing surface (Settings → Billing, ``/topup``) preferred over ``billing_url``;
...
def is_nous_inference_route(provider: str, base_url: str) -> bool:
    ...
    return (provider or "").strip().lower() == "nous" or base_url_host_matches(str(base_url or ""), "inference-api.__NEW_DOMAIN__")


def _nous_billing_url() -> Optional[str]:
    """Best-effort __NEW_ORG__ portal billing URL (text-surface fallback; __NEW_ORG__ prefers the in-app flow)."""
    try:
        from anika_cli.nous_account import nous_portal_billing_url
        return nous_portal_billing_url(None)
    except Exception:
        return "https://portal.__NEW_DOMAIN__/billing"
...
def build_billing_block(*, provider: str, base_url: str, model: str, message: str = "") -> BillingBlock:
    ...
    if is_nous_inference_route(slug, base_url):
        return BillingBlock(slug or "nous", "__NEW_ORG__ Portal", model, _nous_billing_url(), True, message or "")
