...
``auth.json`` credential pool. ``~/.anika/.anthropic_oauth.json`` (Anika PKCE) and
...
_SPENT_ROTATION_SIDECAR_COMMENT = (
    ...
    "consumed server-side but never durably committed. Written by Anika so sibling "
...
def _spent_rotation_sidecar_path(source_path: Path) -> Path:
    return source_path.with_name(source_path.name + ".anika-spent-rotations.json")
...
_SINGLETON_SOURCE_PATHS = {
    "claude_code": lambda: claude_code_credentials_path(), "anika_pkce": lambda: _get_anika_oauth_file()
}
...
def _refresh_oauth_token(creds: Dict[str, Any]) -> Optional[str]:
    ...
    try:
        from anika_cli.auth import AUTH_LOCK_TIMEOUT_SECONDS, _auth_store_lock, env_float
        refresh_timeout_seconds = env_float("ANIKA_ANTHROPIC_REFRESH_TIMEOUT_SECONDS", 20)
...
def _prefer_refreshable_claude_code_token(env_token: str, creds: Optional[Dict[str, Any]]) -> Optional[str]:
    """Prefer refreshable Claude Code creds over a static env OAuth token: Anika historically persisted setup tokens
...
def _resolve_anthropic_pool_token() -> Optional[str]:
    ...
    refresh=False`` (never ``select()``) so diagnostic call sites (account_usage, ``anika models``) never mutate
...
# ── Anika-native PKCE OAuth flow (~/.anika/.anthropic_oauth.json); mirrors Claude Code / pi-ai / OpenCode ──


def _get_anika_oauth_file() -> Path:
    return get_anika_home() / ".anthropic_oauth.json"


def _root_anika_oauth_file() -> Optional[Path]:
    ...
    try:
        ...
        root = get_default_anika_root()
        return None if root.resolve(strict=False) == get_anika_home().resolve(strict=False) else root / ".anthropic_oauth.json"
    except Exception:
        return None
...
def run_anika_oauth_login_pure() -> Optional[Dict[str, Any]]:
    """Run Anika-native OAuth PKCE flow and return credential state."""
    import webbrowser
    ...
    print("\n".join([
        "", "Authorize Anika with your Claude Pro/Max subscription.", "",
    ...
    try:
        from anika_cli.auth import _can_open_graphical_browser as _can_open_gui
    except Exception:
...
def read_anika_oauth_credentials() -> Optional[Dict[str, Any]]:
    """Read Anika-managed OAuth credentials from ~/.anika/.anthropic_oauth.json."""
    data = _load_json_if_exists(_get_anika_oauth_file(), "Anika OAuth credentials")
    return data if data is not None and data.get("accessToken") else None


def _write_anika_oauth_credentials(
    access_token: str, refresh_token: Optional[str], expires_at_ms: Optional[int], *, target: Optional[Path] = None
) -> None:
    """Commit refreshed anika_pkce tokens to ~/.anika/.anthropic_oauth.json (``CredentialPersistError`` on failure).
    ``target`` lets a named profile commit a grant it BORROWED from the global root back to the ROOT singleton
    instead of forking a copy under its own ANIKA_HOME; without this write-through the next ``load_pool()``
    re-seeds the stale (consumed) pair from the file over the rotated pool entry."""
    _commit_private_json(
        target if target is not None else _get_anika_oauth_file(),
        {"accessToken": access_token, "refreshToken": refresh_token, "expiresAt": expires_at_ms},
        "Anika OAuth credentials",
    )
