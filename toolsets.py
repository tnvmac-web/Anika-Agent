...
_ANIKA_WEBHOOK_SAFE_TOOLS = ["web_search", "web_extract", "vision_analyze", "clarify"]
...
def _bundle(description, extras=()):
    """A `anika-*` platform bundle: the shared core tools plus optional platform extras."""
    return _ts(description, _ANIKA_CORE_TOOLS + list(extras))


def _core_without(*excluded, kanban=True):
    """_ANIKA_CORE_TOOLS minus *excluded* (and, unless kanban=True, every kanban_* tool); order preserved."""
    return [t for t in _ANIKA_CORE_TOOLS if t not in excluded and (kanban or not t.startswith("kanban_"))]
...
TOOLSETS = {
    ...
    "x_search": _ts(
        ...
        "(SuperGrok OAuth or XAI_API_KEY). Off by default; enable in `anika tools` → "
        "X (Twitter) Search.",
        ["x_search"],
    ),
    ...
    "video_gen": _ts(
        ...
        "separate tools. Configure via ``anika tools`` → Video Generation.",
    ...
    "browser": _ts(
        ...
        "iframes, hold-click)",
        [t for t in _ANIKA_CORE_TOOLS if t.startswith("browser_")],
    ),
    "cronjob": _ts(
    ...
    "kanban": _ts(
        ...
        "the kanban dispatcher (ANIKA_KANBAN_TASK env set). The dispatcher runs "
        ...
        [t for t in _ANIKA_CORE_TOOLS if t.startswith("kanban_")],
    ),
    ...
    # config (see the non-configurable-toolset recovery loop in anika_cli/tools_config.py).
    "coding": _ts(
    ...
    # Full Anika toolsets (CLI + messaging platforms). All share the core tools;
    # there is deliberately no agent-callable send_message tool. anika-acp is the
    ...
    "anika-cli": _bundle("Full interactive CLI toolset - all default tools plus cronjob management"),

    # Mirrors anika-cli; `anika tools` platform config filters it down and
    # _get_platform_tools() drops _DEFAULT_OFF_TOOLSETS unless user-enabled.
    "anika-cron": _bundle("Default cron toolset - same core tools as anika-cli; gated by `anika tools`"),
    "anika-telegram": _bundle("Telegram bot toolset - full access for personal use (terminal has safety checks)"),
    "anika-discord": _bundle(
        "Discord bot toolset - full access (terminal has safety checks via dangerous "
        "command approval)",
        ["discord", "discord_admin"],
    ),
    "anika-whatsapp": _bundle("WhatsApp bot toolset - similar to Telegram (personal messaging, more trusted)"),
    "anika-slack": _bundle("Slack bot toolset - full access for workspace use (terminal has safety checks)"),
    "anika-signal": _bundle("Signal bot toolset - encrypted messaging platform (full access)"),
    "anika-bluebubbles": _bundle("BlueBubbles iMessage bot toolset - Apple iMessage via local BlueBubbles server"),
    "anika-homeassistant": _bundle("Home Assistant bot toolset - smart home event monitoring and control"),
    "anika-email": _bundle("Email bot toolset - interact with Anika via email (IMAP/SMTP)"),
    "anika-mattermost": _bundle("Mattermost bot toolset - self-hosted team messaging (full access)"),
    "anika-matrix": _bundle("Matrix bot toolset - decentralized encrypted messaging (full access)"),
    "anika-dingtalk": _bundle("DingTalk bot toolset - enterprise messaging platform (full access)"),
    "anika-feishu": _bundle("Feishu/Lark bot toolset - enterprise messaging via Feishu/Lark (full access)", _FEISHU_TOOLS),
    "anika-weixin": _bundle("Weixin bot toolset - personal WeChat messaging via iLink (full access)"),
    "anika-qqbot": _bundle("QQBot toolset - QQ messaging via Official Bot API v2 (full access)"),
    "anika-wecom": _bundle("WeCom bot toolset - enterprise WeChat messaging (full access)"),
    "anika-wecom-callback": _bundle("WeCom callback toolset - enterprise self-built app messaging (full access)"),
    "anika-yuanbao": {
        "description": "Yuanbao Bot 元宝消息平台工具集 - 群信息、成员查询、私聊、贴纸表情",
        "tools": _ANIKA_CORE_TOOLS + _YUANBAO_TOOLS,
        "module": "tools.yuanbao_tools",
        "includes": [],
    },
    "anika-sms": _bundle("SMS bot toolset - interact with Anika via SMS (Twilio)"),
    "anika-webhook": _ts("Webhook toolset - receive and process external webhook events", _ANIKA_WEBHOOK_SAFE_TOOLS),
    "anika-gateway": _ts(
        "Gateway toolset - union of all messaging platform tools",
        [],
        includes=[
            "anika-telegram", "anika-discord", "anika-whatsapp", "anika-slack",
            "anika-signal", "anika-bluebubbles", "anika-homeassistant", "anika-email",
            "anika-sms", "anika-mattermost", "anika-matrix", "anika-dingtalk",
            "anika-feishu", "anika-wecom", "anika-wecom-callback", "anika-weixin",
            "anika-qqbot", "anika-webhook", "anika-yuanbao",
        ],
    ),
}
...
def bundle_non_core_tools(toolset_name: str) -> Set[str]:
    """A bundle's tools minus _ANIKA_CORE_TOOLS (one level of includes).

    Disabling a `core + extras` bundle must not strip the core tools every other
    toolset shares. One `includes` pass suffices (only anika-gateway nests
    bundles). Unknown names: full resolution minus core.
    """
    core = set(_ANIKA_CORE_TOOLS)
...
def _plugin_platform_bundle(name: str) -> List[str]:
    """Implicit `anika-<platform>` bundle for a registered plugin platform: core
    tools plus whatever the plugin registered under the platform name. [] otherwise."""
    if not name.startswith("anika-"):
        return []
    platform_name = name[len("anika-"):]
    try:
