...
2. The unified Anika Skills Index at ``website/static/api/skills-index.json``,
    ...
    for base_dir, source_label in LOCAL_SKILL_DIRS:
        ...
        for root, _dirs, files in os.walk(base_path):
            ...
            if isinstance(metadata, dict):
                anika_meta = metadata.get("anika", {})
                if isinstance(anika_meta, dict):
                    tags = anika_meta.get("tags", [])
            if not tags:
                tags = fm.get("tags", [])
