#!/usr/bin/env python3
"""Generate llms.txt and llms-full.txt for the Anika docs site.
...
LLM reads to learn what Anika does.

Both publish at:
  https://anika-agent.__NEW_DOMAIN__/docs/llms.txt
  https://anika-agent.__NEW_DOMAIN__/docs/llms-full.txt
...
SITE_BASE = "https://anika-agent.__NEW_DOMAIN__/docs"
...
SECTIONS: list[tuple[str, list[tuple[str, str, str | None]]]] = [
    ...
    ("Guides & Tutorials", [
        ...
        ("guides/python-library", "Use Anika as a Python Library", None),
        ("guides/use-mcp-with-anika", "Use MCP with Anika", None),
        ("guides/use-voice-mode-with-anika", "Use Voice Mode with Anika", None),
        ("guides/use-soul-with-anika", "Use SOUL.md with Anika", None),
    ...
    ("Reference", [
        ...
        ("reference/skills-catalog", "Bundled Skills Catalog", "Table of all ~90 skills bundled with Anika"),
...
def emit_llms_index() -> str:
    ...
    lines: list[str] = []
    lines.append("# Anika Agent")
    lines.append("")
    lines.append(
        "> The self-improving AI agent built by __NEW_ORG__. A terminal-native "
        ...
        "backends. Works with __NEW_ORG__ Portal, OpenRouter, OpenAI, Anthropic, Google, or any "
        "OpenAI-compatible endpoint."
    )
    lines.append("")
    lines.append(
        "Install: `curl -fsSL https://raw.githubusercontent.com/__NEW_ORG__/"
        "anika-agent/main/scripts/install.sh | bash`  "
        "(Linux, macOS, WSL2, Termux)"
    )
    lines.append("")
    lines.append("Repo: https://github.com/__NEW_ORG__/anika-agent")
    lines.append("")
...
def emit_llms_full() -> str:
    ...
    seen: set[Path] = set()
    chunks: list[str] = [
        "# Anika Agent — Full Documentation\n",
        (
            "This file is the entire Anika Agent documentation concatenated for LLM "
            "context ingestion. Section order reflects docs-site navigation: Getting "
            "Started, Using Anika, Features, Messaging, Integrations, Guides, "
            "Developer Guide, Reference, then everything else.\n"
        ),
        "Canonical site: https://anika-agent.__NEW_DOMAIN__/docs\n",
        "Short index: https://anika-agent.__NEW_DOMAIN__/docs/llms.txt\n",
        "\n---\n\n",
    ]
