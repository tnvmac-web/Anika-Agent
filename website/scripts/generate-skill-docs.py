...
def rewrite_relative_links(body: str, meta: dict[str, Any]) -> str:
    ...
    base = f"https://github.com/__NEW_ORG__/anika-agent/blob/main/{source_dir}/{meta['rel_path']}"
...
def render_skill_page(
    ...
    anika_meta = (fm.get("metadata") or {}).get("anika") or {}
    tags = anika_meta.get("tags") or []
    related = anika_meta.get("related_skills") or []
    ...
    if meta["source_kind"] == "bundled":
        ...
        info_rows.append(
            (
                "Source",
                "Optional — install with `anika skills install official/"
                + meta["category"]
                + "/"
                + meta["slug"]
                + "`",
...
def build_catalog_md_bundled(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Catalog of bundled skills that ship with Anika Agent"',
        "---",
        "",
        "# Bundled Skills Catalog",
        "",
        "Anika ships with a large built-in skill library copied into `~/.anika/skills/` on install. Each skill below links to a dedicated page with its full definition, setup, and usage.",
        "",
        "Anika also syncs bundled skills on `anika update`, but the sync manifest respects local deletions and user edits. If a skill listed here is missing from your profile's `~/.anika/skills/` tree, it is still shipped with Anika; restore it with `anika skills reset <name> --restore`.",
        "",
...
def build_catalog_md_optional(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Official optional skills shipped with anika-agent — install via anika skills install official/<category>/<skill>"',
        "---",
        "",
        "# Optional Skills Catalog",
        "",
        "Optional skills ship with anika-agent under `optional-skills/` but are **not active by default**. Install them explicitly:",
        "",
        "```bash",
        "anika skills install official/<category>/<skill>",
        "```",
        "",
        "For example:",
        "",
        "```bash",
        "anika skills install official/blockchain/solana",
        "anika skills install official/mlops/flash-attention",
        "```",
        "",
        ...
        "",
        "```bash",
        "anika skills uninstall <skill-name>",
        "```",
        "",
    ]
    for category in sorted(by_cat):
