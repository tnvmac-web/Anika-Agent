...
If you only run one Anika agent, you don't need this page — see
...
[Connecting Desktop to Many Anika Instances](./multi-connection-desktop.md).

## When to use this

You want this setup when you have two or more Anika agents that should all
...
(`ai.anika.gateway-<name>.plist`) or systemd user service
(`anika-gateway-<name>.service`). This guide adds the patterns for managing
them collectively.
...
anika profile create coder
anika profile create personal-bot
anika profile create research
...
```bash
anika config set gateway.multiplex_profiles true
anika gateway restart
```

Equivalently, in the default profile's `~/.anika/config.yaml`:
...
You do **not** run `anika gateway start` for the secondary profiles — the
...
With a multiplexer running, a named-profile `anika gateway start` / `run` is a
...
- `/p/coder/...` API-server requests must use `API_SERVER_KEY` from
  `~/.anika/profiles/coder/.env`; the default listener key is rejected.
...
home). `anika status` reports the multiplexer and the profiles it serves;
`anika status -p <name>` slices to one profile. Each profile still writes its
...
`~/.local/bin/anika-gateways` and `chmod +x` it:
...
usage() {
  echo "Usage: anika-gateways {start|stop|restart|status|list}"
}
...
run_for_profile() {
  ...
  if [ "$profile" = "default" ]; then
    anika gateway "$action"
  else
    anika -p "$profile" gateway "$action"
  fi
}
...
```bash
anika-gateways start      # start every configured profile
anika-gateways stop       # stop every configured profile
anika-gateways restart    # restart all
anika-gateways status     # status across all
anika-gateways list       # delegates to `anika gateway list`
```

:::tip
The `default` profile is targeted with `anika gateway <action>` (no `-p`),
not `anika -p default gateway <action>`. The wrapper above handles both forms.
:::
...
These are equivalent to `anika -p coder gateway <action>` — useful if a
...
| Platform | Path                                                              |
| -------- | ----------------------------------------------------------------- |
| macOS    | `~/Library/LaunchAgents/ai.anika.gateway-<profile>.plist`        |
| Linux    | `~/.config/systemd/user/anika-gateway-<profile>.service`         |

The default profile keeps the historical names: `ai.anika.gateway.plist` /
`anika-gateway.service`.
...
```bash
# Default profile
tail -f ~/.anika/logs/gateway.log
tail -f ~/.anika/logs/gateway.error.log

# Named profile
tail -f ~/.anika/profiles/<name>/logs/gateway.log
tail -f ~/.anika/profiles/<name>/logs/gateway.error.log
```

Stream every profile's log simultaneously:

```bash
tail -f ~/.anika/logs/gateway.log ~/.anika/profiles/*/logs/gateway.log
```

The CLI also has a structured log viewer:

```bash
anika logs -f                  # follow default profile
anika -p coder logs -f         # follow one profile
anika logs --help              # filters, levels, JSON output
```

## Identify what's actually running

```bash
anika profile list             # profiles + model + gateway state
anika-gateways status          # full status across every profile
launchctl list | grep anika    # macOS — PIDs and labels
systemctl --user list-units 'anika-gateway-*'   # Linux — units
```
...
The default profile uses `~/.anika/` directly with the same three files.

Edit them with any editor or via the CLI:

```bash
anika config set model.model anthropic/claude-sonnet-4    # default profile
...
caffeinate -i -w $(cat ~/.anika/gateway.pid) &   # awake while default gateway runs
...
systemd-inhibit --what=idle:sleep --who=anika --why="gateways running" \
  sleep infinity &
...
`anika-gateway-<profile>.service`) continue running across SSH disconnects
and reboots.
...
grep -H 'TELEGRAM_BOT_TOKEN\|DISCORD_BOT_TOKEN' \
     ~/.anika/.env ~/.anika/profiles/*/.env
```

## Updating the code

`anika update` pulls the latest code once and syncs new bundled skills into
every profile:
...
You ran `anika gateway start` after a previous `anika gateway stop`. The
...
```bash
ps -ef | grep "anika_cli.*-p <profile>"
cat ~/.anika/profiles/<profile>/gateway.pid
...
```bash
# macOS
launchctl unload ~/Library/LaunchAgents/ai.anika.gateway-<profile>.plist
launchctl load   ~/Library/LaunchAgents/ai.anika.gateway-<profile>.plist

# Linux
systemctl --user restart anika-gateway-<profile>.service
```

### Health check

```bash
anika doctor                  # default profile
anika -p <profile> doctor     # one profile
```
