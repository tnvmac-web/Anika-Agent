...
Connect Anika to WeCom (Enterprise WeChat) as a self-built enterprise application using the callback/webhook model.

:::info WeCom Bot vs WeCom Callback
Anika supports two WeCom integration modes:
...
> Run `anika gateway setup` and pick **WeCom Callback** for a guided walk-through.
...
3. Anika decrypts the message, queues it for the agent
...
(Use `anika gateway start` only after `anika gateway install` has registered the systemd/launchd service.)
...
console. A mismatch between the token configured in Anika and the token the
...
in `~/.anika/.env` values around `=` will also break signature checks. After
fixing, restart `anika gateway run`.
...
Check `anika gateway run` logs for the bound host/port. If the adapter bound to
