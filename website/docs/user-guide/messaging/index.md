...
description: "Chat with Anika from Telegram, Discord, Slack, WhatsApp, Signal, SMS, Email, Home Assistant, Mattermost, Matrix, DingTalk, Yuanbao, Microsoft Teams, LINE, Raft, Webhooks, or any OpenAI-compatible frontend via the API server — architecture and setup overview"
---

# Messaging Gateway

Chat with Anika from Telegram, Discord, Slack, WhatsApp, Signal, SMS, Email, Home Assistant, Mattermost, Matrix, DingTalk, Feishu/Lark, WeCom, Weixin, BlueBubbles (iMessage), QQ, Yuanbao, Microsoft Teams, LINE, ntfy, or your browser. The gateway is a single background process that connects to all your configured platforms, handles sessions, runs cron jobs, and delivers voice messages.

For the full voice feature set — including CLI microphone mode, spoken replies in messaging, and Discord voice-channel conversations — see [Voice Mode](/user-guide/features/voice-mode) and [Use Voice Mode with Anika](/guides/use-voice-mode-with-anika).

:::tip
Bots need both a model provider and tool providers (TTS, web). A [__NEW_ORG__ Portal](/integrations/nous-portal) subscription bundles all of them.
:::
...
```mermaid
flowchart TB
    subgraph Gateway["Anika Gateway"]
...
For group chats, hooks, and automation flows, Anika supports explicit silence tokens. If the agent's final response is exactly one supported token, the gateway suppresses outbound delivery and sends nothing to the chat.
...
Silence is a delivery decision only. Anika keeps the assistant silence turn in the session transcript, so the conversation still alternates normally:
...
Failed turns still surface as errors; Anika does not hide failures just because the text resembles a silence token.
...
```bash
anika gateway setup        # Interactive setup for all messaging platforms
```
...
```bash
anika gateway              # Run in foreground
anika gateway setup        # Configure messaging platforms interactively
anika gateway install      # Install as a user service (Linux) / launchd service (macOS)
sudo anika gateway install --system   # Linux only: install a boot-time system service
anika gateway start        # Start the default service
anika gateway stop         # Stop the default service
anika gateway status       # Check default service status
anika gateway status --system         # Linux only: inspect the system service explicitly
```
...
```bash
anika gateway install --force
```

A positive value makes the generated unit use `Type=notify`,
`NotifyAccess=main`, and the matching `WatchdogSec`. Anika sends heartbeats
...
| `/update` | Update Anika Agent to the latest version |
...
with the `session_reset` section in `~/.anika/config.yaml`:
...
Configure per-platform overrides in `~/.anika/gateway.json`:
...
Different channels can run different models and personas from a **single gateway** — e.g. a cheap fast model in `#daily` and a frontier model with a specialist prompt in `#dev`. Configure `channel_overrides` under the platform in `~/.anika/gateway-config.yaml`:
...
anika pairing approve telegram XKGH5N7P

# Other pairing commands:
anika pairing list          # View pending + approved users
anika pairing revoke telegram 123456789  # Remove access
```
...
The first time you message a busy agent on any platform, Anika appends a one-line reminder to the busy-ack explaining the knob (`"💡 First-time tip — …"`). The reminder fires once per install — a flag under `onboarding.seen.busy_input_prompt` latches it. Delete that key to see the tip again.
...
Control how much tool activity is displayed in `~/.anika/config.yaml`:
...
Setting `display.tool_progress: log` sends **no** progress bubbles to chat. Instead, each tool call is appended as a line to `~/.anika/logs/tool_calls.log` — a rotating audit file (5 MB × 3 backups) run through the same secret-redacting formatter as regular logs, so credentials never land on disk. Use it when you want a full tool-call trail without any chat noise.

### Configurable status phrases

Long-running gateway status lines ("still working…"-style heartbeats) draw from a phrase catalog. Built-in defaults ship in `gateway/assets/status_phrases.yaml`; you can add your own with profile-portable files under `ANIKA_HOME`:

- `~/.anika/status_phrases.yaml` or any `*.yaml` in `~/.anika/status_phrases/` (conventional paths, auto-loaded), or
- point config at a relative path:

```yaml
display:
  status_phrases:
    path: status_phrases/whatsapp.yaml  # relative to ANIKA_HOME
...
Off by default. When enabled, Anika prepends a human-readable timestamp
...
When the agent running a background session uses `terminal(background=true)` to start long-running processes (servers, builds, etc.), the gateway can push status updates to your chat. Control this with `display.background_process_notifications` in `~/.anika/config.yaml`:
...
```bash
ANIKA_BACKGROUND_NOTIFICATIONS=result
```
...
```bash
anika gateway install          # user service
sudo loginctl enable-linger $USER   # one-time: start at boot, survive logout
```

After that, `anika update` can restart the gateway without any privileges. If you prefer to keep the system service, either run updates with `sudo anika update`, or grant the service account passwordless sudo for systemctl, e.g. in `sudo visudo -f /etc/sudoers.d/anika-gateway`:

```
anika ALL=(root) NOPASSWD: /usr/bin/systemctl --no-ask-password reset-failed anika-gateway*, /usr/bin/systemctl --no-ask-password start anika-gateway*, /usr/bin/systemctl --no-ask-password restart anika-gateway*
```
:::

Avoid keeping both the user and system gateway units installed at once unless you really mean to. Anika will warn if it detects both because start/stop/status behavior gets ambiguous.

:::info Multiple installations
If you run multiple Anika installations on the same machine (with different `ANIKA_HOME` directories), each gets its own systemd service name. The default `~/.anika` uses `anika-gateway`; other installations use `anika-gateway-<hash>`. The `anika gateway` commands automatically target the correct service for your current `ANIKA_HOME`.
:::

### macOS (launchd)

```bash
anika gateway install               # Install as launchd agent
anika gateway start                 # Start the service
anika gateway stop                  # Stop the service
anika gateway status                # Check status
tail -f ~/.anika/logs/gateway.log   # View logs
```

The generated plist lives at `~/Library/LaunchAgents/ai.anika.gateway.plist`. It includes three environment variables:
...
- **ANIKA_HOME** — scopes the gateway to your Anika installation.

:::tip PATH changes after install
launchd plists are static — if you install new tools (e.g. a new Node.js version via nvm, or ffmpeg via Homebrew) after setting up the gateway, run `anika gateway install` again to capture the updated PATH. The gateway will detect the stale plist and reload automatically.
:::

:::info Multiple installations
Like the Linux systemd service, each `ANIKA_HOME` directory gets its own launchd label. The default `~/.anika` uses `ai.anika.gateway`; other installations use `ai.anika.gateway-<suffix>`.
:::
...
|----------|---------|--------------|
| CLI | `anika-cli` | Full access |
| Telegram | `anika-telegram` | Full tools including terminal |
| Discord | `anika-discord` | Full tools including terminal |
| WhatsApp | `anika-whatsapp` | Full tools including terminal |
| WhatsApp Cloud API | `anika-whatsapp` | Full tools including terminal (shares toolset with the Baileys bridge) |
| Slack | `anika-slack` | Full tools including terminal |
| Google Chat | `anika-google_chat` | Full tools including terminal |
| Signal | `anika-signal` | Full tools including terminal |
| SMS | `anika-sms` | Full tools including terminal |
| Email | `anika-email` | Full tools including terminal |
| Home Assistant | `anika-homeassistant` | Full tools + HA device control (ha_list_entities, ha_get_state, ha_call_service, ha_list_services) |
| Mattermost | `anika-mattermost` | Full tools including terminal |
| Matrix | `anika-matrix` | Full tools including terminal |
| DingTalk | `anika-dingtalk` | Full tools including terminal |
| Feishu/Lark | `anika-feishu` | Full tools including terminal |
| WeCom | `anika-wecom` | Full tools including terminal |
| WeCom Callback | `anika-wecom-callback` | Full tools including terminal |
| Weixin | `anika-weixin` | Full tools including terminal |
| BlueBubbles | `anika-bluebubbles` | Full tools including terminal |
| QQBot | `anika-qqbot` | Full tools including terminal |
| Yuanbao | `anika-yuanbao` | Full tools including terminal |
| Microsoft Teams | `anika-teams` | Full tools including terminal |
| API Server | `anika-api-server` | Full tools (drops `clarify`, `text_to_speech` — programmatic access doesn't have an interactive user) |
| Webhooks | `anika-webhook` | Full tools including terminal |
| Raft | `anika-raft` | Wake-only channel; agent uses Raft CLI for message I/O |
...
`platforms.<name>.enabled: false` in `~/.anika/config.yaml` is authoritative.
...
1. **Gateway log** (`~/.anika/logs/gateway.log` or the systemd / launchd unit log). Search for the platform name and `circuit breaker`, `paused`, or `disabled`. The trip event includes the failure count and the last error.
