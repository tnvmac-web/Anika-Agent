...
sidebar_position: 12
title: "Google Chat"
description: "Set up Anika Agent as a Google Chat bot using Cloud Pub/Sub"
---

# Google Chat Setup

Connect Anika Agent to Google Chat as a bot. The integration uses Cloud Pub/Sub
pull subscriptions for inbound events and the Chat REST API for outbound messages.
Equivalent ergonomics to Slack Socket Mode or Telegram long-polling: your Anika
...
> Run `anika gateway setup` and pick **Google Chat** for a guided walk-through.
...
download the file. Save it somewhere only Anika can read (e.g.,
`~/.anika/google-chat-sa.json`, `chmod 600`).
...
- Delivery type: **Pull**
- Message retention: **7 days** (so backlog survives a anika restart)
...
- Principal: `anika-chat-bot@<your-project>.iam.gserviceaccount.com`
- Role: `Pub/Sub Subscriber`

Also grant `Pub/Sub Viewer` on the same subscription — Anika calls
...
- **App name**: whatever you want users to see ("Anika" is reasonable).
...
- **Connection settings**: select **Cloud Pub/Sub**, enter the topic name
  `projects/<your-project>/topics/anika-chat-events`.
...
`ADDED_TO_SPACE` event that Anika uses to cache the bot's own `users/{id}` for
self-message filtering.

---

## Step 9: Configure Anika

Add the Google Chat section to `~/.anika/.env`:
...
Send "hola" in the test DM. The bot posts a "Anika is thinking…" marker, then
...
`~/.anika/config.yaml` — e.g. a kitten assistant named Ada:

```yaml
platforms:
  google_chat:
    # Custom working-state marker text (default: "Anika is thinking…").
...
Thread support: when a user replies inside a thread, Anika detects the
`thread.name` and posts its reply in the same thread, so each thread gets a
separate Anika session.
...
3. Download the JSON. Move it onto the host that runs Anika.
4. Register the client with Anika (run under the profile you want it scoped to):
...
anika -p <profile> python -m plugins.platforms.google_chat.oauth \
    --client-secret /path/to/client_secret.json
```

That writes the client secret into the active profile's Anika home (e.g.
`~/.anika/google_chat_user_client_secret.json` for the default profile). The
...
The token lands at `~/.anika/google_chat_user_tokens/<sanitized_email>.json`.
...
single-user token at `~/.anika/google_chat_user_token.json` (if present from
...
1. Check the Pub/Sub subscription has undelivered messages in the console.
   If it does, Anika isn't authenticated — verify `GOOGLE_CHAT_SERVICE_ACCOUNT_JSON`
...
3. Check `anika gateway` logs for `[GoogleChat] Connected`. If you see
...
- **Attachment download protection**: Anika will only attach the SA bearer
...
- **User OAuth scope**: the per-user attachment flow requests *only*
  ...
  `~/.anika/google_chat_user_tokens/<sanitized_email>.json` (filesystem
