...
sidebar_position: 16
title: "Yuanbao"
description: "Connect Anika Agent to the Yuanbao enterprise messaging platform via WebSocket gateway"
---

# Yuanbao

Connect Anika to [Yuanbao](https://yuanbao.tencent.com/), Tencent's enterprise messaging platform. The adapter uses a WebSocket gateway for real-time message delivery and supports both direct (C2C) and group conversations.
...
After initial setup, verify these variables in `~/.anika/.env`:
...
All standard Anika commands work on Yuanbao:
...
4. Review gateway logs: `tail -f ~/.anika/logs/gateway.log`
...
4. Consider enabling verbose logging: `anika gateway run -vv`
...
Yuanbao has a maximum message size. Anika automatically chunks large responses with Markdown-aware splitting (respects code fences, tables, and paragraph boundaries).
...
```bash
anika chat -q "Send 'Hello from CLI' to yuanbao:group:group_code"
```
