...
When using [OpenRouter](https://openrouter.ai) as your LLM provider, Anika Agent supports **provider routing** — fine-grained control over which underlying AI providers handle your requests and how they're prioritized.
...
:::note
[__NEW_ORG__ Portal](/integrations/nous-portal) decides routing centrally per model and does not accept caller-supplied provider preferences; Anika never sends the `provider` object to Portal, so `provider_routing` is simply ignored there.
:::

## Configuration

Add a `provider_routing` section to your `~/.anika/config.yaml`:
...
:::info
Provider routing only applies when using OpenRouter. It has no effect on __NEW_ORG__ Portal or direct provider connections (e.g., connecting directly to the Anthropic API).
:::
...
own pins. Edit `config.yaml` directly for these keys: model ids contain dots, which `anika config set`
...
- **CLI mode** — configured in `~/.anika/config.yaml`, loaded at startup
