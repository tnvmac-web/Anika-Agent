...
description: "Use your __NEW_ORG__ Portal subscription (or other OAuth provider) as an OpenAI-compatible endpoint for external apps"
---
...
chat completions — use your Anika-managed provider subscription as their
...
| Use case | "Use Anika as a chat backend" | "Use my Portal sub from another app" |
...
This opens your browser for the __NEW_ORG__ Portal OAuth flow. Anika stores
the refresh token in `~/.anika/auth.json` — the same place all Anika
...
Model:      Anika-4-70B    # or Anika-4.3-36B, Anika-4-405B
```
...
Currently shipped: `nous` (__NEW_ORG__ Portal) and `xai` (xAI / Grok). More
OAuth providers can be added by implementing the `UpstreamAdapter`
interface in `anika_cli/proxy/adapters/`.
...
If you see `not logged in`, run `anika portal`. If you see
...
supports; check `portal.__NEW_DOMAIN__/models`.
...
[portal.__NEW_DOMAIN__](https://portal.__NEW_DOMAIN__).
...
`anika_cli/proxy/adapters/<provider>.py` and registering it in
