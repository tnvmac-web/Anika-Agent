...
When Anika Agent runs inside a messaging gateway (Slack, Discord, Telegram,
...
persona in `~/.anika/SOUL.md`, or as a named preset under
`agent.personalities` in `~/.anika/config.yaml` (switchable per session
via `/personality`).
...
If you use Anika' kanban multi-agent workflow, workers can attach
...
Install MCP servers via `~/.anika/config.yaml` under the `mcp_servers`
...
back into the thread as a native attachment. Anika Agent's deliverable
