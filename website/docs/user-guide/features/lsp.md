...
Anika runs full language servers — pyright, gopls, rust-analyzer,
...
This is the same architecture top-tier coding agents use. Anika
...
1. Anika captures a baseline of current diagnostics for the file.
...
…). Anika auto-detects the binary on PATH or in
`<ANIKA_HOME>/lsp/bin/`.
...
2. Download the latest release zip from
   ...
   and extract it.
3. Point Anika at the extracted bundle — the directory that contains
   `PowerShellEditorServices/Start-EditorServices.ps1`. Either:
   - set `lsp.servers.powershell.command: ["/path/to/bundle"]` in
     `config.yaml`, or
   - extract it to `<ANIKA_HOME>/lsp/PowerShellEditorServices`, or
   - export `PSES_BUNDLE_PATH=/path/to/bundle`.

`anika lsp status` reports `installed` once `pwsh` is found; if the
...
`node_modules` tree — Anika installs both packages together when you
run `anika lsp install typescript` or auto-install fires on first
use.

## CLI

```
anika lsp status          # service state + per-server install status
anika lsp list            # registry, optionally --installed-only
anika lsp install <id>    # eagerly install one server
anika lsp install-all     # try every server with a known recipe
anika lsp restart         # tear down running clients
anika lsp which <id>      # print resolved binary path
```

`anika lsp status` is the best starting point — it shows which
...
lsp:
  ...
  #   auto    — install via npm/pip/go install into <ANIKA_HOME>/lsp/bin
...
When `install_strategy: auto`, Anika installs binaries into
`<ANIKA_HOME>/lsp/bin/`. NPM packages land in
`<ANIKA_HOME>/lsp/node_modules/` with bin symlinks one level up.
Go binaries come from `go install` with `GOBIN` pointed at the
staging dir.

Nothing is ever installed to `/usr/local/`, `~/.local/`, or any other
shared location — the staging dir is fully Anika-owned and is
...
**single process** per Anika process: the first Python project spawns
...
**`anika lsp status` shows a server as "missing"**

The binary isn't on PATH and isn't in `<ANIKA_HOME>/lsp/bin/`. Run
`anika lsp install <server_id>` to attempt an auto-install, or
install the binary manually through the language's normal toolchain.

**`Backend warnings` section in `anika lsp status`**
...
When `anika lsp status` shows a `Backend warnings` section, install
...
`~/.anika/logs/agent.log`.

**Server starts but never returns diagnostics**

Check `~/.anika/logs/agent.log` for `[agent.lsp.client]` entries —
...
the rest of the session. Run `anika lsp restart` to clear the set;
