...
Anika Agent can drive your desktop — clicking, typing, scrolling,
...
The built-in `computer_use` toolset is the recommended Anika integration. It
...
**Fresh installs already have the driver.** The Anika installer
...
- **`anika tools`** → pick `🖱️  Computer Use` — installs the driver
...
```
anika computer-use install
```

This fetches and runs the upstream cua-driver installer — `install.sh`
on macOS/Linux, `install.ps1` on Windows. Use `anika computer-use
status` to verify the install.

Already have cua-driver? Anika reuses it when it supports the 0.20 runtime
contract. During setup, toolset enablement, `anika update`, and the first
`computer_use` call of a session, Anika checks the local version and
...
selected with `ANIKA_CUA_DRIVER_CMD` stays
under your control, so Anika reports the incompatibility and leaves it
unchanged.

If you install Cua Driver first, `cua-driver skills install` installs Cua's
skill pack under `~/.cua-driver/skills/cua-driver`. Anika autodetection is a
planned cua-driver follow-up, so currently point Anika at that directory or
...
interface. The built-in toolset provides Anika actions, configuration,
...
| Platform | Prereqs |
|---|---|
| **macOS** | System Settings → Privacy & Security → **Accessibility** + **Screen Recording**. Grant the identity named by `anika computer-use doctor`. Standard mode uses CuaDriver.app; bounded and unrestricted modes use the Anika host identity. |
...
```
anika -t computer_use chat
```

or add `computer_use` to your enabled toolsets in `~/.anika/config.yaml`.

## Permission modes and logged-in browser profiles

Anika maps its existing approval UX onto cua-driver's immutable runtime
modes. Permission mode and capability manifest approval are launch settings.
They cannot change after the runtime starts:

| Anika session | cua-driver mode | Human intervention |
|---|---|---|
| Manual or smart approvals (default) | `standard` | Normal Anika approvals; Cua stops at its protected boundary |
...
| `--yolo`, `/yolo`, or `approvals.mode: off` | private `unrestricted` daemon | One explicit Anika risk acceptance; no runtime Cua prompts |
...
computer_use:
  ...
  capability_manifest: ~/.anika/cua-manifest.yaml
```
...
for the format). Anika launches a private runtime with
...
identity instead of resetting with every Anika build), and Anika verifies
...
resetting or closing the Anika session, cancellation cleanup, or process exit
closes that transport session. Anika also stops private runtimes that it
launched for bounded or unrestricted access. One Anika
conversation cannot change another runtime's mode or grants. Bounded and
unrestricted modes use a private
embedded service under the Anika host identity.
...
## `anika computer-use doctor` — your first triage stop

`anika computer-use doctor` runs cua-driver's structured
`health_report` MCP tool and prints a per-check matrix. It's the single
fastest way to find out *why* an action isn't working.

```
$ anika computer-use doctor
...
Anika run declares a public cua-driver **session name** (something like
`anika-3a7b9c14d2e8`). The name labels cursor identity and related state, so
...
without it. Anika disables it automatically where it is a known failure
...
Anika keeps its wrapper skill (`skills/autonomous-ai-agents/computer-use/SKILL.md`)
focused on the Anika-side `computer_use` workflow and action vocabulary. For
...
The command installs the pack under `~/.cua-driver/skills/cua-driver`. Anika
autodetection is a planned cua-driver follow-up, so currently point Anika at
...
These are **platform deep dives, not duplicates of the Anika skill** —
...
Code, Codex, OpenCode, OpenClaw, and Antigravity; **Anika
...
image capture also saves a bounded, shareable copy under Anika' image cache
...
Anika applies multi-layer guardrails:
...
Pair with `approvals.mode: manual` in `~/.anika/config.yaml` if you
want every action confirmed.

## Token efficiency

Screenshots are expensive. Anika applies four layers of optimisation:
...
- **Windows: elevated (admin) windows can't be driven from a normal
  ...
  Anika agent) cannot enumerate the UIA tree of, or inject mouse input
  ...
  run the Anika agent itself at High integrity (launch from an
...
- **Platform-specific deployment gotchas:**
  - **macOS** uses private SkyLight SPIs. Apple can change them in any
    OS update. Anika warns when the installed cua-driver is older than
    the version it was tested against.
  - **Windows** SSH sessions run in **Session 0**, which has no
    interactive desktop. Drive Anika from inside the RDP / console
...
On Linux, native Wayland support remains an explicit opt-in. Anika passes the
...
```
ANIKA_CUA_DRIVER_CMD=/path/to/your/cua-driver
```

Swap the backend entirely (for testing):

```
ANIKA_COMPUTER_USE_BACKEND=noop   # records calls, no side effects
```

### Telemetry

cua-driver ships with anonymous usage telemetry (PostHog) enabled by default
upstream. **Anika disables it for you** — on every cua-driver invocation
(the MCP backend, `status`, `doctor`, and install) Anika sets
...
When it's on, `anika computer-use doctor` reports `telemetry: enabled`;
...
unreleased fix — point Anika at a binary you built from source instead
of the published release. Anika resolves the driver with
`shutil.which("cua-driver")` and **does not enforce
`ANIKA_CUA_DRIVER_VERSION`**, so a local build (reported as
...
- `-NoAutoStart` skips registering the `cua-driver-serve` logon daemon
  — you don't need it for Anika testing (see notes).
...
### Option B — point Anika straight at the built binary (fastest loop)

Skip the install ceremony entirely: `cargo build` and set
`ANIKA_CUA_DRIVER_CMD` to the resulting binary. Best for rapid
edit/build/test.
...
```
# Windows (.env)
ANIKA_CUA_DRIVER_CMD=C:\path\to\cua\libs\cua-driver\rust\target\debug\cua-driver.exe
# macOS / Linux (.env)
ANIKA_CUA_DRIVER_CMD=/path/to/cua/libs/cua-driver/rust/target/debug/cua-driver
```

### Confirm Anika is using your build

- `anika computer-use status` prints the resolved binary path and
  version.
- `anika computer-use doctor` confirms the binary is reachable and
...
- **Anika spawns a `cua-driver mcp` stdio proxy.** In a normal session the
  proxy connects to (and may start) the standard machine daemon. In explicit
  Anika YOLO, Anika instead owns a private `cua-driver serve --embedded`
...
- **Rebuild loop.** After editing cua-driver source, re-run
  ...
  for Option A, or just re-`cargo build` for Option B — no Anika
  change needed either way.
- **Local builds skip the version check.** Anika warns when the
...
**First action when anything's off: run `anika computer-use doctor`.**
...
Run `anika computer-use install` to fetch the cua-driver binary, or
run `anika tools` and enable the Computer Use toolset.
...
Wayland without an XWayland bridge. `anika computer-use doctor` will
...
- **Anika-side skill** — `skills/autonomous-ai-agents/computer-use/SKILL.md` — teaches the
  Anika `computer_use` action vocabulary; this is what the agent loads.
- **cua-driver skill pack** — for platform-specific deep dives
  ...
  `LINUX.md` / `RECORDING.md` / `WEB_APPS.md`. Anika autodetection is a
  planned follow-up; currently point Anika at the installed pack directory
...
- **cua.ai/docs** — the cua-driver project's documentation:
  ...
  - [Connect your agent](https://cua.ai/docs/how-to-guides/driver/connect-your-agent) — register cua-driver with various harnesses (Anika among them)
