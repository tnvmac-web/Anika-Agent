...
Type `@` followed by a reference to inject content directly into your message. Anika expands the reference inline and appends the content under an `--- Attached Context ---` section.
...
- Anika env: `$ANIKA_HOME/.env`

These directories are fully blocked (any file inside):
- `~/.ssh/`, `~/.aws/`, `~/.gnupg/`, `~/.kube/`, `$ANIKA_HOME/skills/.hub/`
