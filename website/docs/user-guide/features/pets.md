...
Anika can show an animated **pet** — a small mascot sprite that reacts to what
...
- Pets are installed into your profile's `pets/` directory
  (`<ANIKA_HOME>/pets/<slug>/`), so each [profile](../profiles.md) keeps its
  own set.
...
In the terminal (CLI/TUI), Anika renders the sprite at full fidelity when your
...
anika pets install boba --select
...
| Goal | Command |
| --- | --- |
| Browse the gallery | `anika pets list [query] [--limit N]` |
| List installed pets | `anika pets list --installed` |
| Install a pet | `anika pets install <slug> [--select] [--force]` |
...
| Preview/animate | `anika pets show [slug] [--state <s>] [--cycle] [--once] [--mode <m>] [--scale <f>]` |
| Disable the pet | `anika pets off` |
| Remove an installed pet | `anika pets remove <slug>` |
| Diagnose setup | `anika pets doctor` |

`anika pets show` flags:
...
Beyond installing pre-made pets from the gallery, Anika can **generate a brand-new pet** from a text description — its own AI sprite-generation pipeline.

- CLI/TUI: `/hatch <description>` (alias `/generate-pet`), or `anika pets` → the generate flow.
...
2. **Hatch** — the chosen base is used as a reference image to generate one grounded animation row per Anika state (idle, thinking, tool use, etc.), which are deterministically sliced into frames and packed into a standard petdex/Codex atlas (8×9 grid of 192×208 cells). The result is a valid spritesheet you keep — and could `petdex submit`.

### Image backend

Generation uses the active [image-generation provider](/user-guide/features/image-generation), but it requires **reference-image grounding** so each animation row stays the same character as the base. Reference-capable backends: **__NEW_ORG__ Portal**, **OpenRouter**, **OpenAI** (`gpt-image-2`), and **Krea**. OpenRouter/__NEW_ORG__ run a quality-first model chain by default.

- Resolution order prefers __NEW_ORG__ Portal → OpenAI → OpenRouter.
- If no reference-capable backend is configured, generation surfaces an actionable error pointing you to `anika tools` → Image Generation. (Installing/adopting existing gallery pets needs no image backend.)
- Override the backend with the `ANIKA_PET_IMAGE_PROVIDER` env var (e.g. `ANIKA_PET_IMAGE_PROVIDER=openrouter`).
...
always-on-top desktop window. Out there it stays visible while Anika is
...
Run `anika pets doctor` — it reports:
...
- The [`anika-agent` skill](../skills/bundled/autonomous-ai-agents/autonomous-ai-agents-anika-agent.md)
