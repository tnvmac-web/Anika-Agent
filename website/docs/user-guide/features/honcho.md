...
[Honcho](https://github.com/plastic-labs/honcho) is an AI-native memory backend that adds dialectic reasoning and deep user modeling on top of Anika's built-in memory system. Instead of simple key-value storage, Honcho maintains a running model of who the user is — their preferences, communication style, goals, and patterns — by reasoning about conversations after they happen.
...
**Multi-agent profiles**: When multiple Anika instances talk to the same user (e.g., a coding assistant and a personal assistant), Honcho maintains separate "peer" profiles. Each peer sees only its own observations and conclusions, preventing cross-contamination of context.

## Setup

```bash
anika memory setup    # select "honcho" from the provider list
```
...
- `per-session` — each `anika` run gets a fresh session. Clean starts, memory via tools. Recommended for new users.
...
Server-side toggles set via the [Honcho dashboard](https://app.honcho.dev) win over local defaults — Anika syncs them back at session init.
...
The `anika honcho` subcommand is **only registered when Honcho is the active memory provider** (`memory.provider: honcho` in `config.yaml`). On a fresh install, configure Honcho directly with `anika memory setup honcho` (or run `anika memory setup` and pick it from the list); the `anika honcho` subcommand then appears on the next invocation.

```bash
anika memory setup honcho    # Configure Honcho directly (works before activation)
anika honcho status          # Connection status, config, and key settings
anika honcho setup           # Redirects to `anika memory setup` (post-activation alias)
anika honcho strategy        # Show or set session strategy (per-session/per-directory/per-repo/global)
anika honcho peer            # Show or update peer names + dialectic reasoning level
anika honcho mode            # Show or set recall mode (hybrid/context/tools)
anika honcho tokens          # Show or set token budget for context and dialectic
anika honcho identity        # Seed or show the AI peer's Honcho identity
anika honcho sync            # Sync Honcho config to all existing profiles
anika honcho peers           # Show peer identities across all profiles
anika honcho sessions        # List known Honcho session mappings
anika honcho map             # Map current directory to a Honcho session name
anika honcho enable          # Enable Honcho for the active profile
anika honcho disable         # Disable Honcho for the active profile
anika honcho migrate         # Step-by-step migration guide from openclaw-honcho
```

## Migrating from `anika honcho`

If you previously used the standalone `anika honcho setup`:
...
No re-login or re-setup needed. Run `anika memory setup` and select "honcho" — the wizard detects your existing config.
