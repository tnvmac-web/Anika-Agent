...
title: "Git Worktrees"
description: "Run multiple Anika agents safely on the same repository using git worktrees and isolated checkouts"
---

# Git Worktrees

Anika Agent is often used on large, long‑lived repositories. When you want to:
...
This page shows how to combine worktrees with Anika so each session has a clean, isolated working directory.

## Why Use Worktrees with Anika?

Anika treats the **current working directory** as the project root:

- CLI: the directory where you run `anika` or `anika chat`
- Messaging gateways: the directory set by `terminal.cwd` in `~/.anika/config.yaml`
...
Anika creates `.worktrees/my-experiment/` inside the repo (branch
`anika/my-experiment`, based on the freshly-fetched remote tip unless
`worktree_sync: false`), and retargets the session's terminal and file tools
into it — no restart needed. Omit the name to get a random `anika-<id>`
...
git worktree add ../repo-feature feature/anika-experiment
```

This creates:

- A new directory: `../repo-feature`
- A new branch: `feature/anika-experiment` checked out in that directory

Now you can `cd` into the new worktree and run Anika there:

```bash
cd ../repo-feature

# Start Anika in the worktree
anika
```
...
git worktree add ../repo-experiment-a feature/anika-a
git worktree add ../repo-experiment-b feature/anika-b
```
...
- Works on its own branch (`feature/anika-a` vs `feature/anika-b`).
...
- Anika checkpoint data under `~/.anika/checkpoints/` is not automatically pruned when you remove a worktree, but it is usually very small.

## Best Practices

- **One worktree per Anika experiment**
...
- **Name branches after the experiment**
  - e.g. `feature/anika-checkpoints-docs`, `feature/anika-refactor-tests`.
- **Commit frequently**
...
- **Avoid running Anika from the bare repo root when using worktrees**
  - Prefer the worktree directories instead, so each agent has a clear scope.

## Using `anika -w` (Automatic Worktree Mode)

Anika has a built‑in `-w` flag that **automatically creates a disposable git worktree** with its own branch. You don't need to set up worktrees manually — just `cd` into your repo and run:
...
- Check out an isolated branch (e.g. `anika/anika-<hash>`).
...
```bash
anika -w -z "Fix issue #123"
```

For parallel agents, open multiple terminals and run `anika -w` in each — every invocation gets its own worktree and branch automatically.

## Putting It All Together

- Use **git worktrees** to give each Anika session its own clean checkout.
