...
sidebar_position: 1
title: "CLI Interface"
description: "Master the Anika Agent terminal interface — commands, keybindings, personalities, and more"
---

# CLI Interface

Anika Agent's CLI is a full terminal user interface (TUI) — not a web UI. It features multiline editing, slash-command autocomplete, conversation history, interrupt-and-redirect, and streaming tool output. Built for people who live in the terminal.

:::tip First-time setup
One command — `anika setup --portal` — and you're ready to `anika chat`. See [__NEW_ORG__ Portal](/integrations/nous-portal).
:::

:::tip
Anika also ships a modern TUI with modal overlays, mouse selection, and non-blocking input. Launch it with `anika --tui` — see the [TUI](tui.md) guide.
:::
...
anika chat --query-file prompt.txt
anika chat --query-file - < prompt.txt

# With a specific model
anika chat --model "anthropic/claude-sonnet-4"

# With a specific provider
anika chat --provider nous        # Use __NEW_ORG__ Portal
anika chat --provider openrouter  # Force OpenRouter

# With specific toolsets
anika chat --toolsets "web,terminal,skills"

# Start with one or more skills preloaded
anika -s anika-agent-dev,github-auth
anika chat -s github-pr-workflow -q "open a draft PR"

# Resume previous sessions
anika --continue             # Resume the most recent CLI session (-c)
anika --resume <session_id>  # Resume a specific session by ID (-r)
anika --resume latest        # Resume the most recent session (same as -c)
anika --resume latest --in ./dir  # Resume ./dir's latest session, staying in ./dir

# Verbose mode (debug output)
anika chat --verbose

# Isolated git worktree (for running multiple agents in parallel)
anika -w                         # Interactive mode in worktree
anika -w -z "Fix issue #123"     # Single query in worktree
```

### Worktree cleanup

`anika -w` sessions create disposable worktrees under `<repo>/.worktrees/`.
...
```bash
anika worktree list              # audit: age, size, verdict, reason per tree
anika worktree prune             # remove safe trees + delete merged branches
anika worktree prune --dry-run   # show the plan without changing anything
anika worktree prune --trees-only     # leave local branches alone
anika worktree prune --branches-only  # leave worktrees alone
```
...
- Trees **in use by a running anika session** are never touched.
- **Untracked-only scratch** (PR body drafts, notes) is archived to
  `~/.anika/archive/worktree-prune/` before its tree is removed — never
  destroyed.
...
launches `anika -w` for days — no longer accumulate merged scratch trees
...
The `anika plugins` commands manage native Anika plugins and portable Agent
...
anika plugins disable <plugin-name>
anika plugins update <plugin-name>
anika plugins remove <plugin-name>
```

Portable packages remain disabled until explicitly enabled. Anika currently
...
<img className="docs-terminal-figure" src="/docs/img/docs/cli-layout.svg" alt="Stylized preview of the Anika CLI layout showing the banner, conversation area, and fixed input prompt." />
<p className="docs-figure-caption">The Anika CLI banner, conversation stream, and fixed input prompt rendered as a stable docs figure instead of fragile text art.</p>
...
| ⚠ YOLO | **YOLO mode warning** — shown whenever `ANIKA_YOLO_MODE` is on (either `anika --yolo` at launch or `/yolo` toggled mid-session). Mirrors the banner-line warning so you can't forget you're in auto-approve mode. |
...
| `Ctrl+D` | Exit |
| `Ctrl+Z` | Suspend Anika to background (Unix only). Run `fg` in the shell to resume. |
...
- **Approvals still apply.** A dangerous command (`rm -rf`, writes to `~/.anika/config.yaml`, etc.) goes through the same approval prompt the agent's `terminal` tool uses. `!` is a cost/latency shortcut, not a security bypass.
...
Type `/` to see the autocomplete dropdown. Anika supports a large set of CLI slash commands, dynamic skill commands, and user-defined quick commands.
...
| `/voice tts` | Toggle spoken playback for Anika replies |
...
```bash
anika -s anika-agent-dev,github-auth
anika chat -s github-pr-workflow -s github-auth
```

Anika loads each named skill into the session prompt before the first turn. The same flag works in interactive mode and single-query mode.

## Skill Slash Commands

Every installed skill in `~/.anika/skills/` is automatically registered as a slash command. The skill name becomes the command:
...
You can also define custom personalities in `~/.anika/config.yaml`:

```yaml
personalities:
  helpful: "You are a helpful, friendly AI assistant."
  kawaii: "You are a kawaii assistant! Use cute expressions..."
  pirate: "Arrr! Ye be talkin' to Captain Anika..."
  # Add your own!
```
...
`Ctrl+J` and backslash continuation are enabled by default, matching Claude Code / Codex / OpenCode multiline shortcuts. On supported terminals such as iTerm2, Anika also requests extended key reporting so `Shift+Enter` arrives as a distinct newline key. If your terminal sends LF for plain `Enter` and you need the legacy `Ctrl+J`-as-submit fallback, opt out:
...
Most terminals send the same byte sequence for `Enter` and `Shift+Enter` by default, so applications cannot distinguish them. Anika recognises `Shift+Enter` only when the terminal sends a distinct sequence via the [Kitty keyboard protocol](https://sw.kovidgoyal.net/kitty/keyboard-protocol/) or xterm's `modifyOtherKeys` mode.
...
Where the terminal cannot distinguish them, `Alt+Enter` and `Ctrl+J` continue to work by default. **On Windows Terminal specifically, `Alt+Enter` is captured by the terminal (toggles fullscreen) and never reaches Anika — use `Ctrl+Enter` (delivered as `Ctrl+J`) or `Ctrl+J` directly for a newline.**
...
The first time you press Enter while Anika is working, Anika prints a one-line reminder explaining the `/busy` knob. It only fires once per install; `onboarding.seen.busy_input_prompt` in `config.yaml` records that it was shown. Delete that key to see the tip again.
:::

### Suspending to Background

On Unix systems, press **`Ctrl+Z`** to suspend Anika to the background — just like any terminal process. The shell prints a confirmation:

```
Anika Agent has been suspended. Run `fg` to bring Anika Agent back.
```
...
Resume this session with:
  anika --resume 20260225_143052_a1b2c3
...
```bash
anika --continue                          # Resume the most recent CLI session
anika -c                                  # Short form
anika -c "my project"                     # Resume a named session (latest in lineage)
anika --resume 20260225_143052_a1b2c3     # Resume a specific session by ID
anika --resume "refactoring auth"         # Resume by title
anika --resume latest                     # Resume the most recent session (same as -c)
anika --resume latest --in ./my-project   # Latest session for ./my-project's workspace
anika -r 20260225_143052_a1b2c3           # Short form
```

Resuming restores the full conversation history from SQLite. The agent sees all previous messages, tool calls, and responses — just as if you never left.

Use `/title My Session Name` inside a chat to name the current session, or `anika sessions rename <id> <title>` from the command line. Use `anika sessions list` to browse past sessions.

### Session Storage

CLI sessions are stored in Anika's SQLite state database under `~/.anika/state.db`. The database keeps:
...
Anika immediately confirms the task and gives you back the prompt:
