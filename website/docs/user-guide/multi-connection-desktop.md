...
# Connecting Desktop to Many Anika Instances

Register every Anika backend you own — the local runtime, remote gateways on
your LAN or VPS, SSH hosts, and Anika Cloud instances — in one desktop app,
...
- **The sidebar profile rail** — the plug button at the right end of the rail
  (tooltip: **"Connect another Anika gateway…"**) deep-links straight to
...
list of Anika gateways. Its intro says it plainly: *"Manage this device and
every Anika gateway it can reach through remote, SSH, or Cloud connections."*
Each entry is a *connection*:

| Kind | What it is | Auth |
|---|---|---|
| **Local** | "The Anika runtime managed by this app." | automatic |
| **Remote gateway** | "A Anika gateway reachable over HTTP(S) — LAN, Tailscale, or the internet." | session token or OAuth |
| **SSH** | "A Anika install reached over SSH." The app opens the tunnel and starts the dashboard for you | SSH key + adopted token |
| **Anika Cloud** | "A hosted instance discovered through your Anika Cloud account." | portal sign-in |
...
- **Duplicates are rejected when you save**: there is only ever one **local**
  ...
  **SSH** entries are deduplicated on the normalized `user@host:port` plus
  remote profile.
- Cloud entries normally come from the Anika Cloud sign-in/discovery flow at
  the top of the Gateways page — the **Anika Cloud** kind in the add-connection
...
3. Pick the kind: **Local**, **Anika Cloud**, **Remote gateway**, or **SSH**.
   (**Local** is disabled while the app-managed local entry exists — which is
   almost always; **Anika Cloud** directs you to the cloud sign-in/discovery
   flow above.)
4. Fill the fields:
   - **Name** — required, unique; the "device name" shown everywhere this
     instance appears (placeholder: `Homelab`). Max 64 characters.
   - *Remote gateway only:*
     - **Gateway URL** — the base URL of a running `anika serve` backend,
     ...
     - **Authentication** — choose **Session token** or **OAuth**:
       - **Session token** — paste the dashboard session token from the
         ...
         token."*
       - **OAuth** — sign in through the __NEW_ORG__ Portal browser flow; no token
         to paste.
   - *SSH only:*
...
Gateways are machines or hosted backends; profiles are isolated Anika agents
...
connection is registered) dispatches `anika update` to every eligible
...
- **Anika Cloud** instances are skipped with a *"Managed by Anika Cloud"*
...
panel, ⌘K **Update Anika**, the update-ready toast) run the same fan-out
...
- **"Connection test failed"** — the backend isn't reachable at that URL from
  this machine. Check that `anika serve` is running on the remote host, the
...
- **"Update Anika Desktop to chat with agents on other connections"** — the
