...
`~/.anika/config.yaml`, `~/.anika/.env`, and even the shell environment — for
...
Managed scope is read from a system-level directory, default `/etc/anika`:

```text
/etc/anika/
├── config.yaml     # managed config layer (wins over ~/.anika/config.yaml)
└── .env            # managed env layer (wins over ~/.anika/.env + shell)
```
...
The location can be relocated with the `ANIKA_MANAGED_DIR` environment variable
(for containers or non-`/etc` deployments). This is a deployment/bootstrap path
knob — like `ANIKA_HOME` — set by the same administrator who owns the managed
files. It is **never persisted** to any `.env` by Anika.

```bash
# Point managed scope at a custom directory (set by IT / the deployment, not the user)
export ANIKA_MANAGED_DIR=/opt/org/anika-policy
```

:::warning
A user who can set `ANIKA_MANAGED_DIR` can repoint managed scope at a directory
...
left user-settable. `anika doctor` reports the *resolved* managed directory so
...
|---|---|---|
| 1 | `/etc/anika/config.yaml` (managed) | `/etc/anika/.env` (managed) |
| 2 | `~/.anika/config.yaml` (user) | `~/.anika/.env` (user) |
...
```bash
anika config        # shows a header naming the managed source + the pinned keys
anika doctor        # reports the resolved managed dir + pinned key counts
```

If you try to change a managed value, Anika refuses and names the source:

```bash
$ anika config set model.default my/model
Cannot set 'model.default': it is managed by your administrator
(/etc/anika/config.yaml) and cannot be changed.
```

The same applies to managed secrets — `anika config set` / setup will not write
a user value for an env key pinned by the managed `.env`.

## Setting up a managed scope (administrators)

```bash
sudo mkdir -p /etc/anika

# Pin some config values for every user on this machine
sudo tee /etc/anika/config.yaml >/dev/null <<'YAML'
model:
  provider: nous
security:
  redact_secrets: true
...
sudo tee /etc/anika/.env >/dev/null <<'ENV'
OPENAI_API_BASE=https://inference.example.com/v1
ENV

sudo chmod 0755 /etc/anika
sudo chmod 0644 /etc/anika/config.yaml /etc/anika/.env
```

Changes take effect on the next Anika start (a malformed managed file is logged
loudly and ignored — it never blocks startup, but the admin should check
`anika doctor` to confirm the policy is being applied).

## Security model and limitations (v1)

- **Enforcement is filesystem permissions only.** If a user has write access to
  the managed directory (or runs Anika as `root`), managed scope is advisory.
