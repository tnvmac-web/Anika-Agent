...
| | |
|---|---|
| Source | Optional — install with `anika skills install official/security/1password` |
| Path | `optional-skills/security\1password` |
| Version | `1.0.0` |
| Author | arceus77-7, enhanced by Anika Agent |
| License | MIT |
...
- `tmux` available for stable authenticated sessions during Anika terminal calls (desktop app flow only)
...
### Service Account (recommended for Anika)

Set `OP_SERVICE_ACCOUNT_TOKEN` in `${ANIKA_HOME:-~/.anika}/.env` (the skill will prompt for this on first load).
...
## Anika Execution Pattern (desktop app flow)

Anika terminal commands are non-interactive by default and can lose auth context between calls.
...
```bash
SOCKET_DIR="${TMPDIR:-/tmp}/anika-tmux-sockets"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/anika-op.sock"
