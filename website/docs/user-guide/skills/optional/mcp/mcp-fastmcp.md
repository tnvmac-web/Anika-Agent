...
| | |
|---|---|
| Source | Optional — install with `anika skills install official/mcp/fastmcp` |
...
| Related skills | [`anika-agent`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-anika-agent), [`mcporter`](/docs/user-guide/skills/optional/mcp/mcp-mcporter) |
...
- smoke-test a server with the FastMCP CLI before wiring it into Anika or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to Anika. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.anika/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.anika/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is Anika integration, either:

- configure the server in `~/.anika/config.yaml` using the `native-mcp` skill, or
...
### Anika cannot see the deployed server

The server-building part may be correct while the Anika config is not. Load the `native-mcp` skill and configure the server in `~/.anika/config.yaml`, then restart Anika.
