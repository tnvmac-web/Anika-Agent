---
title: "Honcho — Configure and troubleshoot Honcho memory for Anika"
sidebar_label: "Honcho"
description: "Configure and troubleshoot Honcho memory for Anika"
---
...
Configure and troubleshoot Honcho memory for Anika.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `anika skills install official/autonomous-ai-agents/honcho` |
...
# Honcho Memory for Anika

Honcho provides AI-native cross-session user modeling. It learns who the user is across conversations and gives every Anika profile its own peer identity while sharing a unified view of the user.
...
See: https://docs.honcho.dev/v3/guides/integrations/anika#running-honcho-locally-with-anika

### Verify

```bash
anika honcho status    # shows resolved config, connection test, peer info
```
...
3. **AI peer card** -- the identity card for this Anika profile's AI peer
...
Honcho models conversations as interactions between **peers**. Anika creates two peers per session:

- **User peer** (`peerName`): represents the human. Honcho builds a user representation from observed messages.
- **AI peer** (`aiPeer`): represents this Anika instance. Each profile gets its own AI peer so agents develop independent views.
...
| `per-session` | New Honcho session each Anika run |
| `global` | Single session across all directories |

Manual override: `anika honcho map my-project-name`
...
Each Anika profile gets its own Honcho AI peer while sharing the same workspace (user context). This means:
...
# creates host block anika.coder, AI peer "coder", inherits config from default
```

What `--clone` does for Honcho:
1. Creates a `anika.coder` host block in `honcho.json`
...
```bash
anika honcho sync    # creates host blocks for all profiles that don't have one yet
```
...
Guidelines for Anika when Honcho memory is active.
...
Config file: `$ANIKA_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global).
...
Run `anika honcho setup`. Ensure `memory.provider: honcho` is in `~/.anika/config.yaml`.

### Memory not persisting across sessions
Check `anika honcho status` -- verify `saveMessages: true` and `writeFrequency` isn't `session` (which only writes on exit).

### Profile not getting its own peer
Use `--clone` when creating: `anika profile create <name> --clone`. For existing profiles: `anika honcho sync`.
...
| Command | Description |
|---------|-------------|
| `anika honcho setup` | Interactive setup wizard (cloud/local, identity, observation, recall, sessions) |
| `anika honcho status` | Show resolved config, connection test, peer info for active profile |
| `anika honcho enable` | Enable Honcho for the active profile (creates host block if needed) |
| `anika honcho disable` | Disable Honcho for the active profile |
| `anika honcho peer` | Show or update peer names (`--user <name>`, `--ai <name>`, `--reasoning <level>`) |
| `anika honcho peers` | Show peer identities across all profiles |
| `anika honcho mode` | Show or set recall mode (`hybrid`, `context`, `tools`) |
| `anika honcho tokens` | Show or set token budgets (`--context <N>`, `--dialectic <N>`) |
| `anika honcho sessions` | List known directory-to-session-name mappings |
| `anika honcho map <name>` | Map current working directory to a Honcho session name |
| `anika honcho identity` | Seed AI peer identity or show both peer representations |
| `anika honcho sync` | Create host blocks for all Anika profiles that don't have one yet |
| `anika honcho migrate` | Step-by-step migration guide from OpenClaw native memory to Anika + Honcho |
| `anika memory setup` | Generic memory provider picker (selecting "honcho" runs the same wizard) |
| `anika memory status` | Show active memory provider and config |
| `anika memory off` | Disable external memory provider |
