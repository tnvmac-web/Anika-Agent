...
| | |
|---|---|
| Source | Optional — install with `anika skills install official/productivity/siyuan` |
...
3. Store it in `${ANIKA_HOME:-~/.anika}/.env`:
   ```
...
```yaml
# In ~/.anika/config.yaml under mcp_servers:
mcp_servers:
  siyuan:
    command: npx
