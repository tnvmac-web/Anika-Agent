...
| | |
|---|---|
| Source | Optional — install with `anika skills install official/productivity/here-now` |
...
- Skill helper paths:
  - `${ANIKA_SKILL_DIR}/scripts/publish.sh` for publishing sites
  - `${ANIKA_SKILL_DIR}/scripts/drive.sh` for private Drive storage

## Create a site

```bash
PUBLISH="${ANIKA_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --client anika
```
...
```bash
PUBLISH="${ANIKA_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --slug {slug} --client anika
```
...
```bash
DRIVE="${ANIKA_SKILL_DIR}/scripts/drive.sh"
bash "$DRIVE" default
...
| `--client {name}`      | Agent name for attribution (e.g. `anika`)    |
