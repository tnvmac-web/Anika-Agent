...
| | |
|---|---|
| Source | Optional — install with `anika skills install official/productivity/canvas` |
...
4. Name the token (e.g., "Anika Agent"), set an optional expiry, and click **Generate Token**
5. Copy the token and add to `${ANIKA_HOME:-~/.anika}/.env`:
...
```bash
CANVAS="python $ANIKA_HOME/skills/productivity/canvas/scripts/canvas_api.py"
