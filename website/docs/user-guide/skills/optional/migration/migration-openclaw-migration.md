---
title: "Openclaw Migration — Import an OpenClaw setup (memories, skills) into Anika"
sidebar_label: "Openclaw Migration"
description: "Import an OpenClaw setup (memories, skills) into Anika"
---
...
Import an OpenClaw setup (memories, skills) into Anika.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `anika skills install official/migration/openclaw-migration` |
| Path | `optional-skills/migration\openclaw-migration` |
| Version | `1.0.0` |
| Author | Anika Agent (__NEW_ORG__) |
| License | MIT |
| Platforms | linux, macos, windows |
| Tags | `Migration`, `OpenClaw`, `Anika`, `Memory`, `Persona`, `Import` |
...
# OpenClaw -> Anika Migration

Use this skill when a user wants to move their OpenClaw setup into Anika Agent with minimal manual cleanup.
...
```bash
anika claw migrate              # Full interactive migration
...
anika claw migrate --source /custom/path/.openclaw  # Custom source
```
...
**First-time setup:** The `anika setup` wizard automatically detects `~/.openclaw` and offers migration before configuration begins.

## What this skill does

It uses `scripts/openclaw_to_anika.py` to:

- import `SOUL.md` into the Anika home directory as `SOUL.md`
- transform OpenClaw `MEMORY.md` and `USER.md` into Anika memory entries
- merge OpenClaw command approval patterns into Anika `command_allowlist`
- migrate Anika-compatible messaging settings such as `TELEGRAM_ALLOWED_USERS`, and map OpenClaw workspace settings to Anika working-directory configuration
- copy OpenClaw skills into `~/.anika/skills/openclaw-imports/`
- optionally copy the OpenClaw workspace instructions file into a chosen Anika workspace
- mirror compatible workspace assets such as `workspace/tts/` into `~/.anika/tts/`
- archive non-secret docs that do not have a direct Anika destination
...
- `scripts/openclaw_to_anika.py`

When this skill is installed from the Skills Hub, the normal location is:

- `~/.anika/skills/migration/openclaw-migration/scripts/openclaw_to_anika.py`

Do not guess a shorter path like `~/.anika/skills/openclaw-migration/...`.

Before running the helper:

1. Prefer the installed path under `~/.anika/skills/migration/openclaw-migration/`.
...
With `--migrate-secrets`, it will also import a small allowlisted set of Anika-compatible secrets, currently:
...
Anika CLI supports the `clarify` tool for interactive prompts, but it is limited to:
...
- `{"question":"One or more imported OpenClaw skills already exist in Anika. How should I handle those skill conflicts?","choices":["keep existing skills","overwrite conflicting skills with backup","import conflicting skills under renamed folders"]}`
...
- `{"question":"Do you want to copy the OpenClaw workspace instructions file into a Anika workspace?","choices":["skip workspace instructions","copy to a workspace path","decide later"]}`
...
13. If a renamed imported skill has an empty `details.backup`, do not imply the existing Anika skill was renamed or backed up. Say only that the imported copy was placed in the new destination and reference `details.renamed_from` as the pre-existing folder that remained in place.
...
```bash
python ~/.anika/skills/migration/openclaw-migration/scripts/openclaw_to_anika.py
```

When using the terminal tool, prefer an absolute invocation pattern such as:

```json
{"command":"python /home/USER/.anika/skills/migration/openclaw-migration/scripts/openclaw_to_anika.py","workdir":"/home/USER"}
```

Dry run with the user-data preset:

```bash
python ~/.anika/skills/migration/openclaw-migration/scripts/openclaw_to_anika.py --preset user-data
```

Execute a user-data migration:

```bash
python ~/.anika/skills/migration/openclaw-migration/scripts/openclaw_to_anika.py --execute --preset user-data --skill-conflict skip
```

Execute a full compatible migration:

```bash
python ~/.anika/skills/migration/openclaw-migration/scripts/openclaw_to_anika.py --execute --preset full --migrate-secrets --skill-conflict skip
```

Execute with workspace instructions included:

```bash
python ~/.anika/skills/migration/openclaw-migration/scripts/openclaw_to_anika.py --execute --preset user-data --skill-conflict rename --workspace-target "/absolute/workspace/path"
```
...
2. Do not migrate secrets by default. Tokens, auth blobs, device credentials, and raw gateway config should stay out of Anika unless the user explicitly asks for secret migration.
3. Do not silently overwrite non-empty Anika targets unless the user explicitly wants that. The helper script will preserve backups when overwriting is enabled.
...
6. Even in secret-migration mode, only migrate secrets with a clean Anika destination. Unsupported auth blobs must still be reported as skipped.
...
- Anika persona state imported
- Anika memory files populated with converted OpenClaw knowledge
- OpenClaw skills available under `~/.anika/skills/openclaw-imports/`
