...
| | |
|---|---|
| Source | Optional — install with `anika skills install official/creative/unreal-mcp` |
...
Companion skill for the `unreal-engine` entry in the Anika MCP catalog. The
...
Two halves, in this order: the editor side must be up before Anika connects.
...
### One-time, Anika side
    ...
    anika mcp install unreal-engine
...
`~/.anika/config.yaml` under `mcp_servers.unreal-engine` to match.

Do NOT use `ModelContextProtocol.GenerateClientConfig` for Anika — that
writes `.mcp.json`-style files for Claude Code/Cursor/etc. Anika connects
...
2. Start the Anika session. Tools register as `mcp_unreal_engine_*`. If
...
three meta-tools, and every real tool is reached through them. Through Anika
they appear as:
...
then happens at `anika mcp install`/`configure` time instead. Tool-search
...
3. **NEVER issue overlapping calls.** Do not batch multiple
   `mcp_unreal_engine_*` calls in one turn — Anika runs batched calls
...
- **Start order matters.** Editor + server up first, then the Anika
...
- **Timeouts on long operations.** Anika' per-call default is 120 s; asset
  imports, big level saves, and renders can exceed it. Raise
  `mcp_servers.unreal-engine.timeout` in `~/.anika/config.yaml` for
