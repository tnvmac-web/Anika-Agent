...
| | |
|---|---|
| Source | Optional — install with `anika skills install official/creative/pixel-art` |
...
import sys
import os
sys.path.insert(0, os.path.expanduser("~/.anika/skills/creative/pixel-art/scripts"))
...
```bash
cd ~/.anika/skills/creative/pixel-art/scripts
...
- ffmpeg on PATH (only needed for video — Anika installs package this)
