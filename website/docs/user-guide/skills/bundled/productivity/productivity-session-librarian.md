...
| List/filter by metadata (age, source, cost, tokens, workspace) | `anika sessions list` / `stats` via terminal |
| Rename | `anika sessions rename <session_id> <title...>` |
| Bulk soft-hide (reversible) | `anika sessions archive <filters>` |
| Delete (destructive) | `anika sessions delete` / `anika sessions prune <filters>` |
| Export before deleting anything valuable | `anika sessions export --session-id <id> --format md` |
...
`anika sessions list --source telegram --limit 50` instead.
...
- Before deleting anything with meaningful content, offer
  `anika sessions export --format md` as a backup.
...
After a cleanup pass, re-run the discovery query and `anika sessions list`
