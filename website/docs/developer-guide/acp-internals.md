...
The ACP adapter wraps Anika' synchronous `AIAgent` in an async JSON-RPC stdio server.
...
```text
anika acp / anika-acp / python -m acp_adapter
  -> acp_adapter.entry.main()
  -> parse --version / --check / --setup before server startup
  -> load ~/.anika/.env
  -> configure stderr logging
  -> construct AnikaACPAgent
...
- `allow_once` -> Anika `once`
- `allow_always` -> Anika `always`
- reject options -> Anika `deny`

Timeouts and bridge failures deny by default.

### Tool rendering helpers

`acp_adapter/tools.py` maps Anika tools to ACP tool kinds and builds editor-facing content.
...
new_session(cwd)
  -> create SessionState
  -> create AIAgent(platform="acp", enabled_toolsets=["anika-acp"])
...
Instead it reuses Anika' runtime resolver:

- `acp_adapter/auth.py`
- `anika_cli/runtime_provider.py`

So ACP advertises and uses the currently configured Anika provider/credentials. It also always advertises a terminal setup auth method (`anika-setup`, args `--setup`) so first-run ACP clients can open Anika' interactive model/provider configuration before starting a normal ACP session.
...
- ACP sessions are persisted to the shared `~/.anika/state.db` (SessionDB) and transparently restored across process restarts; they appear in `session_search`
...
- `toolsets.py` — `anika-acp` toolset definition
- `anika_cli/main.py` — `anika acp` CLI subcommand
- `pyproject.toml` — `[acp]` optional dependency + `anika-acp` script
