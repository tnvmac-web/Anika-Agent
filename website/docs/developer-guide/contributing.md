...
sidebar_position: 4
title: "Contributing"
description: "How to contribute to Anika Agent — dev setup, code style, PR process"
---

# Contributing

Thank you for contributing to Anika Agent! This guide covers setting up your dev environment, understanding the codebase, and getting your PR merged.
...
- Building a custom/local tool without modifying Anika core? Start with [Build a Anika Plugin](../developer-guide/plugins/index.md)
- Building a new built-in core tool for Anika itself? Start with [Adding Tools](./adding-tools.md)
...
You can also run a fully isolated Anika instance (throwaway ANIKA_HOME, separate Electron
userData, distinct Electron app name to avoid the single-instance lock):

```bash
scripts/dev-sandbox.sh python -m anika_cli.main
scripts/dev-sandbox.sh --persistent python -m anika_cli.main desktop  # state survives restarts, but lives in the worktree :)
```
...
echo 'OPENROUTER_API_KEY=sk-or-v1-your-key' >> ~/.anika/.env
```
...
- **Profile-safe paths**: Never hardcode `~/.anika` — use `get_anika_home()` from `anika_constants` for code paths and `display_anika_home()` for user-facing messages. See [AGENTS.md](https://github.com/__NEW_ORG__/anika-agent/blob/main/AGENTS.md#profiles-multi-instance-support) for full rules.
...
Projects built on (or reviewed by) Anika can keep reviewer checklists inside the repository under `.agents/checks/`. Each file is a focused, plain-markdown checklist that an agent loads before reviewing a change touching the matching area:
...
When you ask Anika to review a PR in a repository that has `.agents/checks/`, tell it (or teach it via a skill) to read the relevant checklists first and report against them. This gives review agents the project-specific bar that generic review prompts miss.
...
- **Discord**: [discord.gg/__NEW_ORG__](https://discord.gg/__NEW_ORG__)
- **GitHub Discussions**: For design proposals and architecture discussions
- **Skills Hub**: Upload specialized skills and share with the community

## License

By contributing, you agree that your contributions will be licensed under the [MIT License](https://github.com/__NEW_ORG__/anika-agent/blob/main/LICENSE).
