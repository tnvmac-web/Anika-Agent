...
sidebar_position: 1
title: "Architecture"
description: "Anika Agent internals — major subsystems, execution paths, data flow, and where to read next"
---

# Architecture

This page is the top-level map of Anika Agent internals. Use it to orient yourself in the codebase, then dive into subsystem-specific docs for implementation details.
...
│ anika_state.py   │              │ Browser (5 backends)  │
...
├── cli.py                    # AnikaCLI facade — mixins in anika_cli/cli_*_mixin.py
├── model_tools.py            # Tool discovery, schema collection, dispatch
├── toolsets.py               # Tool groupings and platform presets
├── anika_state.py           # SQLite session/state database facade (+ anika_state_*.py siblings)
├── anika_constants.py       # ANIKA_HOME, profile-aware paths
...
│
├── anika_cli/               # CLI subcommands and setup
│   ├── main.py               # Entry point — `anika` subcommands (parsers in subcommands/, main_*.py)
...
│   ├── skills_config.py      # anika skills — enable/disable per platform
│   ├── skills_hub.py         # /skills slash command
│   ├── tools_config.py       # anika tools — enable/disable per platform
│   ├── plugins.py            # PluginManager — discovery, loading, hooks
│   ├── callbacks.py          # Terminal callbacks (clarify, sudo, approval)
│   └── gateway.py            # anika gateway start/stop
│
...
```text
User input → AnikaCLI.process_input()
...
Three discovery sources: `~/.anika/plugins/` (user), `.anika/plugins/` (project), and pip entry points. Plugins register tools, hooks, and CLI commands through a context API. Two specialized plugin types exist: memory providers (`plugins/memory/`) and context engines (`plugins/context_engine/`). Both are single-select — only one of each can be active at a time, configured via `anika plugins` or `config.yaml`.
...
Exposes Anika as an editor-native agent over stdio/JSON-RPC for VS Code, Zed, and JetBrains.
...
| **Profile isolation** | Each profile (`anika -p <name>`) gets its own ANIKA_HOME, config, memory, sessions, and gateway PID. Multiple profiles run concurrently. |
