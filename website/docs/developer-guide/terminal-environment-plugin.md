...
Anika runs shell commands through a pluggable set of **terminal backends**.
...
installed under `~/.anika/plugins/`, registering a backend the user selects
...
| `anika setup` backend picker | `display_name`, `description`, `setup_instructions()`, `post_setup()` |
| Dashboard terminal-backend picker (probe status) | `probe()` |
| `anika status` / `anika doctor` | `doctor_checks()` |
...
```python title="~/.anika/plugins/acmebox/__init__.py"
...
class AcmeBoxProvider(TerminalEnvironmentProvider):
    ...
    def cache_path_base(self):
        return "~/.anika"    # where synced cache files land, or None
...
```yaml title="~/.anika/plugins/acmebox/plugin.yaml"
name: acmebox
version: 0.1.0
...
```bash
anika plugins enable acmebox
anika config set terminal.backend acmebox
```
