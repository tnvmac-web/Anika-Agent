...
title: "Desktop Plugin SDK (@anika/plugin-sdk)"
description: "Extend the native Anika Desktop app — panes, pages, sidebar nav, status bar, palette commands, keybinds, themes, and a scoped backend namespace, with one import and no build step."
---

# Desktop Plugin SDK

The native [Anika Desktop](/user-guide/desktop) app is contribution-driven: every
...
A **desktop plugin** is a single ESM file that default-exports a `AnikaPlugin`.
It imports one module — `@anika/plugin-sdk` — and gets everything: the app's
...
`$ANIKA_HOME/desktop-plugins/<id>/plugin.js` and the app loads it within seconds
and hot-reloads every save.

:::warning This is not the web-dashboard plugin SDK
"Plugin" means several unrelated things across Anika. This page is the **native
desktop app** (`anika desktop`) SDK — the `@anika/plugin-sdk` module and
`$ANIKA_HOME/desktop-plugins/`. The **web dashboard** (`anika dashboard`) has
its own, unrelated plugin system on `window.__ANIKA_PLUGIN_SDK__` with a
`manifest.json` — documented at
[Extending the Dashboard](/user-guide/features/extending-the-dashboard). Python
CLI/gateway plugins are documented at [Build a Anika Plugin](/developer-guide/plugins).
...
|------|-------|-----|------------|
| **Disk** (recommended) | `$ANIKA_HOME/desktop-plugins/<id>/plugin.js` | users, agents | none — plain ESM, loaded uncompiled |
| **Unified package** | `$ANIKA_HOME/plugins/<id>/desktop/plugin.js` | plugins that also ship agent-side code | none — same disk pipeline |
| **Bundled** | `apps/desktop/src/plugins/<id>/plugin.tsx` | in-tree, shipped with the app | the app's own Vite build |

All three take the same `AnikaPlugin` contract, appear in **Settings → Plugins**,
...
[`anika-example-plugins`](https://github.com/__NEW_ORG__/anika-example-plugins)
repo.

## Quick start — your first plugin

Create `$ANIKA_HOME/desktop-plugins/hello/plugin.js` (that's `~/.anika/...`
by default, or `~/.anika/profiles/<name>/...` under a named profile). The folder
name must equal the plugin `id`.

```javascript
// ~/.anika/desktop-plugins/hello/plugin.js
import { host, haptic, useValue } from '@anika/plugin-sdk'
...
function HelloPane() {
  ...
  return jsxs('div', {
    ...
    children: [
      jsx('div', { className: 'font-medium', children: 'Hello, Anika' }),
      jsx('div', {
...
The only importable specifiers are `@anika/plugin-sdk`, `react`, and
`react/jsx-runtime` — everything else fails to resolve, on purpose.
:::

## The plugin contract

A plugin default-exports a `AnikaPlugin`:

```ts
interface AnikaPlugin {
...
interface PluginContext {
  ...
  os: PluginOs
  /** Plugin-scoped JSON persistence (keys live under `anika.plugin.<id>.`). */
...
```javascript
import { ROUTES_AREA, SIDEBAR_NAV_AREA } from '@anika/plugin-sdk'
...
```javascript
import { STATUSBAR_AREAS, TITLEBAR_AREAS } from '@anika/plugin-sdk'
...
```javascript
import { PALETTE_AREA, KEYBINDS_AREA } from '@anika/plugin-sdk'
...
```javascript
import { THEMES_AREA } from '@anika/plugin-sdk'
...
```javascript
import { Button, useTheme } from '@anika/plugin-sdk'
...
```javascript
import { host, requestTheme } from '@anika/plugin-sdk'
...
```javascript
import { TRANSCRIPT_DIRECTIVE_AREA } from '@anika/plugin-sdk'
...
`window.anika.send('get-price eth')` (or a declarative
`<button data-anika-send="get-price eth">` — no script needed) hands that
...
```javascript
import { Contribute, TITLEBAR_AREAS } from '@anika/plugin-sdk'
...
for requests; `targetProfile` is the backend Anika profile served by that route.
...
approval/turn alerts use. It fires only while the user is away from Anika
...
ctx.os.notify({
  ...
  // Body click → focus Anika + navigate. Same vocabulary as OS deep links:
  activate: 'anika://index-network/intent/1',
  ...
  actions: [
    { id: 'open', label: 'Open', activate: 'anika://index-network/intent/1' },
    { id: 'dismiss', label: 'Dismiss', onAction: () => dismiss('1') },
  ],
})
```

`activate` is deeplink-compatible: `anika://index-network/intent/1` and the
hash path `/index-network/intent/1` resolve to the same in-app route (and the
same `anika://…` URL works as an OS deep link). Action buttons only render on
...
```javascript
import { useQuery, useMutation, useQueryClient, atom, computed, useValue } from '@anika/plugin-sdk'
...
```javascript
import { queryClient } from '@anika/plugin-sdk'
...
a package into `~/.anika/plugins` is inert on every surface until the user
...
backend, the remote box's `~/.anika/plugins` is not reachable as a filesystem —
...
the `anika://` scheme — a plain anchor on your website or README:
...
`dashboard/` subfolder of a regular Anika plugin and declare it in a
`manifest.json`:
...
anika-agent codebase directly (`anika_state`, `anika_cli.config`, …). See
...
`config.yaml` (and not in `plugins.disabled`). Project plugins (`./.anika/`)
...
(`anika.plugin.<id>.*`) so plugins can't read or clobber each other:
...
export a `AnikaPlugin`). It's discovered by `discoverBundledPlugins()` at boot —
...
1. It goes through the app's Vite build, so you can write **real JSX** and import
   the SDK by its `@anika/plugin-sdk` alias.
2. It's still lint-fenced to `@anika/plugin-sdk` + `react` only — no `@/…` app
   internals.
...
- **Only three specifiers resolve:** `@anika/plugin-sdk`, `react`,
...
| Plugin contract | `AnikaPlugin`, `PluginContext`, `PluginContribution`, `PluginStorage`, `PluginOs`, `PluginRestOptions`, `PluginNativeNotificationInput`, `PluginNotificationAction`, `AnikaOpenTarget`, `Contribution` |
...
### Agents: the `anika-desktop-plugins` skill

When an agent writes a desktop plugin, it should load the bundled
**`anika-desktop-plugins`** skill — it carries the same contract as this page in
...
`$ANIKA_HOME/desktop-plugins/<id>/plugin.js` and the folder name matches the
export `id`. Run ⌘K → **Reload desktop plugins**. Check the app for an error
toast naming the failure, and tail `anika logs gui -f`.

**"unsupported import" on load.** A disk plugin may only import
`@anika/plugin-sdk`, `react`, and `react/jsx-runtime`. Remove any other import.
...
`~/.anika/plugins/<id>/dashboard/manifest.json` has `"api": "plugin_api.py"`,
that the plugin is in `plugins.enabled` in `config.yaml`, and restart the gateway
(backend routes mount at startup). Tail `~/.anika/logs/errors.log` for
