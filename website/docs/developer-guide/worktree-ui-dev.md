...
| Variable | Meaning |
|----------|---------|
| `ANIKA_MAIN_CHECKOUT` | The deps checkout — where `node_modules` really lives, and whose `.venv/bin/python` runs the backend. |
| `ANIKA_GUI_DEPS_CHECKOUT` | Where the desktop deps (`apps/desktop/node_modules`) live. Defaults to `ANIKA_MAIN_CHECKOUT`; override only if you keep desktop deps elsewhere. |

Neither is read by Anika itself — they're private to these helpers. The variables Anika *does* read are covered in [Environment Variables](../reference/environment-variables.md).

## `htui` — TUI from the worktree

The Ink TUI has a dev path already: `anika --tui --dev` runs the TypeScript sources via `tsx` instead of the prebuilt bundle. `htui` is a one-liner over it that also points the run at the current worktree's `ui-tui/`:

```bash
htui() {
  local root
  root="$(_anika_root)" || { echo "htui: not in a Anika checkout" >&2; return 1; }
  ( cd "$root" && PYTHONPATH="$root" \
      "$ANIKA_MAIN_CHECKOUT/.venv/bin/python" -m anika_cli.main --tui --dev "$@" )
}
```

`--dev` compiles from source, so it links `ui-tui/node_modules` from `ANIKA_MAIN_CHECKOUT` when the root lockfile matches and installs locally otherwise (see [`_anika_root` / linking helpers](#shared-helpers)).

:::warning `--dev` and `ANIKA_TUI_DIR` are mutually exclusive
`ANIKA_TUI_DIR` points Anika at a *prebuilt* bundle (Nix, system packages), which has no source to hot-reload. If it's set in your shell, `anika --tui --dev` exits with an error. Run `unset ANIKA_TUI_DIR` before `htui`.
:::
...
hgui() {
  ...
  root="$(_anika_root)" || { echo "hgui: not in a Anika checkout" >&2; return 1; }
  deps="${ANIKA_GUI_DEPS_CHECKOUT:-$ANIKA_MAIN_CHECKOUT}"
  ...
  if cmp -s "$root/package-lock.json" "$deps/package-lock.json"; then
    _anika_link_deps "$desktop" "$deps/apps/desktop"
    _anika_link_deps "$root" "$deps"
  else
  ...
  trap '_anika_gui_cleanup "$root"' INT TERM EXIT

  ( cd "$desktop"
    export PATH="$root/node_modules/.bin:$PATH"
    ANIKA_DESKTOP_ANIKA_ROOT="$root" \
    ANIKA_DESKTOP_PYTHON="$ANIKA_MAIN_CHECKOUT/.venv/bin/python" \
    ANIKA_DESKTOP_IGNORE_EXISTING=1 \
    ANIKA_DESKTOP_CWD="$root" \
    npm run dev )
}
```
...
|----------|----------------|
| `ANIKA_DESKTOP_ANIKA_ROOT` | Runs the backend from **this worktree**, not the packaged/PATH `anika`. |
| `ANIKA_DESKTOP_PYTHON` | Reuses the deps checkout's venv instead of re-resolving a Python. |
| `ANIKA_DESKTOP_IGNORE_EXISTING` | Ignores any `anika` on `PATH` so it can't shadow the worktree. |
| `ANIKA_DESKTOP_CWD` | Opens the desktop chat rooted at the worktree. |
...
```bash
# The enclosing worktree, verified as a real Anika checkout.
_anika_root() {
  local root
  root="$(git rev-parse --show-toplevel 2>/dev/null)" || return 1
  [[ -f "$root/anika_cli/main.py" && -d "$root/ui-tui" ]] && print -r "$root"
}
...
_anika_gui_cleanup() {
  ...
  lsof -t -i:5174 >/dev/null 2>&1 && killport 5174
  pgrep -f 'anika_cli\.main.*dashboard.*--port 0' 2>/dev/null | xargs -r kill -TERM 2>/dev/null
}
```
...
- [TUI](../user-guide/tui.md) — `anika --tui --dev` and the `ANIKA_TUI_DIR` prebuild path
- [Desktop App](../user-guide/desktop.md) — building from source and the backend resolution ladder
- [`apps/desktop/README.md`](https://github.com/__NEW_ORG__/anika-agent/blob/main/apps/desktop/README.md) — dev server, sandbox script, and packaging
- [Environment Variables](../reference/environment-variables.md) — every `ANIKA_*` variable Anika reads
