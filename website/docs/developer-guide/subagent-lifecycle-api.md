...
Plugins can launch and supervise fresh Anika child sessions without importing
...
active agent turn fails closed with `No active Anika parent session`.
...
overrides, and per-launch timeouts are explicitly rejected until Anika can
support them without weakening isolation. Use `allowed_toolsets` to narrow a
child; Anika's existing unsafe-tool block remains enforced.
