...
sidebar_position: 15
title: "CLI Internals"
description: "How anika_cli is shaped: slash dispatch, config loaders, the skin engine, the transactional update pipeline, and process-identity rules"
---

# CLI Internals

Companion to `anika_cli/AGENTS.md` (the rules) — this page holds the longer explanations.

## Update pipeline

The stage-by-stage contract (`plan → snapshot → apply → restart-per-kind → verify → report`) and the
field failure each stage guards are documented in `anika_cli/AGENTS.md`; user-facing behaviour
...
- Use the canonical matchers: `gateway.status.looks_like_gateway_command_line` (gateway run),
  `anika_cli.update_cmd._anika_holder_subcommand` (top-level subcommand of any Anika argv). Never
...
Built-in skins (`_BUILTIN_SKINS` in `anika_cli/skin_engine.py`): `default` (classic gold/kawaii),
...
User skins are `~/.anika/skins/<name>.yaml` with the same keys, activated with `/skin <name>` or
`display.skin: <name>`; the full YAML template is in the
[Skins & Themes](../user-guide/features/skins.md) user guide.

## Profiles: multi-instance support

Anika supports profiles — fully isolated instances, each with its own `ANIKA_HOME` (config, API
keys, memory, sessions, skills, gateway). `_apply_profile_override()` in `anika_cli/main.py` sets
`ANIKA_HOME` before any module imports, so every `get_anika_home()` reference scopes to the active
profile. Profile operations are HOME-anchored (`_get_profiles_root()` returns
`Path.home() / ".anika" / "profiles"`, not `get_anika_home() / "profiles"`) so
`anika -p coder profile list` sees all profiles regardless of which one is active — intentional.
