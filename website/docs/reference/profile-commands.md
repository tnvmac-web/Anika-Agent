...
This page covers all commands related to [Anika profiles](../user-guide/profiles.md). For general CLI commands, see [CLI Commands Reference](./cli-commands.md).
...
Top-level command for managing profiles. Running `anika profile` without a subcommand shows help.
...
```bash
anika profile use <name>
```

Sets `<name>` as the active profile. All subsequent `anika` commands (without `-p`) will use this profile.
...
anika profile use default
```

## `anika profile create`

```bash
anika profile create <name> [options]
```
...
| `--description "<text>"` | One- or two-sentence description of what this profile is good at. Used by the kanban orchestrator to route tasks based on role instead of profile name alone. Skip and add later via `anika profile describe`. Persisted in `<profile_dir>/profile.yaml`. |
| `--no-skills` | Create an **empty** profile with zero bundled skills enabled. Writes a `.no-bundled-skills` marker into the profile so future `anika update` runs won't re-seed the bundled set, and refuses to combine with `--clone`, `--clone-from`, or `--clone-all` (which would copy skills in anyway). Useful for narrow orchestrator profiles or sandbox profiles that should not inherit the full skill catalog. To toggle this on an already-created profile (including the default `~/.anika`), use `anika skills opt-out` / `anika skills opt-in`. |
...
anika profile create work2 --clone-from work

# Clone everything from a specific profile
anika profile create work2-backup --clone-from work --clone-all
```

## `anika profile describe`

```bash
anika profile describe [<name>] [options]
```
...
anika profile describe researcher

# Set it explicitly
anika profile describe researcher --text "Reads source code and writes findings."

# Let the LLM generate one
anika profile describe researcher --auto

# Fill in descriptions for every profile that doesn't have one
anika profile describe --all --auto
```

## `anika profile delete`

```bash
anika profile delete <name> [options]
```
...
```bash
anika profile delete mybot
anika profile delete mybot --yes
```

:::warning
This permanently deletes the profile's entire directory including all config, memories, sessions, and skills. The `default` profile (`~/.anika`) cannot be deleted — use `anika uninstall` to remove everything.
:::

## `anika profile show`

```bash
anika profile show <name>
```

Displays details about a profile including its home directory, configured model, gateway status, skills count, and configuration file status.

This shows the profile's Anika home directory, not the terminal working directory. Terminal commands start from `terminal.cwd` (or the launch directory on the local backend when `cwd: "."`).
...
```bash
$ anika profile show work
Profile: work
Path:    ~/.anika/profiles/work
...
```bash
anika profile alias <name> [options]
```

Regenerates the shell alias script at `~/.local/bin/<name>`. Useful if the alias was accidentally deleted or if you need to update it after moving your Anika installation.
...
```bash
anika profile alias work
# Creates/updates ~/.local/bin/work

anika profile alias work --name mywork
# Creates ~/.local/bin/mywork

anika profile alias work --remove
# Removes the wrapper script
```

## `anika profile rename`

```bash
anika profile rename <old-name> <new-name>
```
...
```bash
anika profile rename mybot assistant
# ~/.anika/profiles/mybot → ~/.anika/profiles/assistant
# ~/.local/bin/mybot → ~/.local/bin/assistant
```

## `anika profile export`

```bash
anika profile export <name> [options]
```
...
```bash
anika profile export work
# Creates work.tar.gz in the current directory

anika profile export work -o ./work-2026-03-29.tar.gz
```
...
## `anika profile import`

```bash
anika profile import <archive> [options]
```
...
```bash
anika profile import ./work-2026-03-29.tar.gz
# Infers profile name from the archive

anika profile import ./work-2026-03-29.tar.gz --name work-restored
```
...
| `--alias` | Also create a shell wrapper (e.g. `telemetry` → `anika -p telemetry`). |
...
anika profile install github.com/kyle/telemetry-distribution --alias

# Install from a full HTTPS git URL
anika profile install https://github.com/kyle/telemetry-distribution.git

# Install from SSH
anika profile install git@github.com:kyle/telemetry-distribution.git

# Install from a local directory during development
anika profile install ./telemetry/
```

### `anika profile update`

```bash
anika profile update <name> [--force-config] [--yes]
```
...
```bash
anika profile info <name>
```

Prints the profile's distribution manifest — name, version, required
Anika version, author, env var requirements, the source URL/path, and
...
`anika profile list` also shows the distribution name and version in a
`Distribution` column, and `anika profile show <name>` / `delete <name>`
...
anika profile install git@github.com:your-org/internal-assistant.git

# Uses your git credential helper
anika profile install https://github.com/your-org/internal-assistant.git
```
...
`anika_requires` supports `>=`, `<=`, `==`, `!=`, `>`, `<`, or a bare
version (treated as `>=`). Install fails with a clear error if the current
Anika version doesn't satisfy the spec.
...
2. Initialize a git repo (or use an existing one) and push to GitHub /
   GitLab / any host Anika can clone from.
3. Tell recipients to run `anika profile install <your-repo-url>`.
...
## `anika -p` / `anika --profile`

```bash
anika -p <name> <command> [options]
anika --profile <name> <command> [options]
```

Global flag to run any Anika command under a specific profile without changing the sticky default. This overrides the active profile for the duration of the command.
...
```bash
anika -p work chat -q "Check the server status"
anika --profile dev gateway start
anika -p personal skills list
anika -p work config edit
```

## `anika completion`

```bash
anika completion <shell>
```
...
- `anika profile <TAB>` — subcommands (list, use, create, etc.)
- `anika profile use <TAB>` — profile names
- `anika -p <TAB>` — profile names
