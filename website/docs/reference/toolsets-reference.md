...
description: "Reference for Anika core, composite, platform, and dynamic toolsets"
---
...
- **Platform** — A complete tool configuration for a specific deployment context (e.g., `anika-cli` is the default for interactive CLI sessions)
...
```bash
anika chat --toolsets web,file,terminal
anika chat --toolsets debugging        # composite — expands to file + terminal + web
anika chat --toolsets all              # everything
```
...
```yaml
toolsets:
  - anika-cli          # default for CLI
  # - anika-telegram   # override for Telegram gateway
```

### Interactive management

```bash
anika tools                            # curses UI to enable/disable per platform
```
...
| `code_execution` | `execute_code` | Run Python scripts that call Anika tools programmatically. |
...
| `discord` | `discord` | Core Discord text/embed/DM actions (gateway-only). Active on the `anika-discord` toolset. |
| `discord_admin` | `discord_admin` | Discord moderation (bans, role changes, channel management). Active on the `anika-discord` toolset; requires the bot to hold the relevant Discord permissions. |
...
| `feishu_drive` | `feishu_drive_add_comment`, `feishu_drive_list_comments`, `feishu_drive_list_comment_replies`, `feishu_drive_reply_comment` | Feishu/Lark drive comment operations. Scoped to the comment agent; not exposed on `anika-cli` or other messaging toolsets. |
...
| `x_search` | `x_search` | Read-only public X discovery via xAI's built-in `x_search` Responses tool. Use the `xurl` skill for authenticated X API reads and account actions. Off by default; opt in via `anika tools`. Schema only registered when xAI credentials (SuperGrok OAuth or `XAI_API_KEY`) are configured. |
| `yuanbao` | `yb_query_group_info`, `yb_query_group_members`, `yb_search_sticker`, `yb_send_dm`, `yb_send_sticker` | Yuanbao DM/group actions and sticker search. Registered only on `anika-yuanbao`. |

## Platform Toolsets

Platform toolsets define the complete tool configuration for a deployment target. Most messaging platforms use the same set as `anika-cli`:

| Toolset | Differences from `anika-cli` |
|---------|-------------------------------|
| `anika-cli` | Full toolset — the default for interactive CLI sessions. Includes file, terminal, web, browser, memory, skills, vision, image_gen, todo, tts, delegation, code_execution, cronjob, session_search, clarify, computer_use, Home Assistant, and the kanban tools (all check_fn-gated at runtime). |
| `anika-acp` | Drops `clarify`, `cronjob`, `image_generate`, `text_to_speech`, `computer_use`, all four Home Assistant tools, and the kanban tools. Focused on coding tasks in IDE context. |
| `anika-api-server` | Drops `clarify`, `text_to_speech`, `computer_use`, and the kanban tools. Keeps everything else — suitable for programmatic access where user interaction isn't possible. |
| `anika-cron` | Same as `anika-cli`. |
| `anika-telegram` | Same as `anika-cli`. |
| `anika-discord` | Adds `discord` and `discord_admin` on top of `anika-cli`. |
| `anika-slack` | Same as `anika-cli`. |
| `anika-whatsapp` | Same as `anika-cli`. |
| `anika-signal` | Same as `anika-cli`. |
| `anika-matrix` | Same as `anika-cli`. |
| `anika-mattermost` | Same as `anika-cli`. |
| `anika-email` | Same as `anika-cli`. |
| `anika-sms` | Same as `anika-cli`. |
| `anika-bluebubbles` | Same as `anika-cli`. |
| `anika-dingtalk` | Same as `anika-cli`. |
| `anika-feishu` | Adds the five `feishu_doc_*` / `feishu_drive_*` tools (only used by the document-comment handler, not the regular chat adapter). |
| `anika-qqbot` | Same as `anika-cli`. |
| `anika-wecom` | Same as `anika-cli`. |
| `anika-wecom-callback` | Same as `anika-cli`. |
| `anika-weixin` | Same as `anika-cli`. |
| `anika-yuanbao` | Adds the five `yb_*` tools (DM/group/sticker) on top of `anika-cli`. |
| `anika-homeassistant` | Same as `anika-cli` (the Home Assistant tools are already present by default and activate when `HASS_TOKEN` is set). |
| `anika-webhook` | Restricted safe subset — only `web_search`, `web_extract`, `vision_analyze`, and `clarify`. Webhook-triggered runs get no terminal, file, or browser access. |
| `anika-gateway` | Internal gateway orchestrator toolset — union of every `anika-<platform>` toolset; used when the gateway needs to accept any message source. |
...
- **Workflow-gated** tools — the `kanban` toolset — are deliberately opt-in. `all`/`*` does **not** enable kanban; you must list `kanban` explicitly (or be a dispatcher-spawned worker with `ANIKA_KANBAN_TASK` set). Kanban tools mutate shared board state, so they stay off by default even under `all`.

## Relationship to `anika tools`

The `anika tools` command provides a curses-based UI for toggling individual tools on or off per platform. This operates at the tool level (finer than toolsets) and persists to `config.yaml`. Disabled tools are filtered out even if their toolset is enabled.
