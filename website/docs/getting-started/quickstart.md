---
sidebar_position: 1
title: "Anika Agent Quickstart"
description: "Your first conversation with Anika Agent — from install to chatting in under 5 minutes"
---

# Anika Agent Quickstart

This guide gets you from zero to a working Anika setup that survives real use. Install, choose a provider, verify a working chat, and know exactly what to do when something breaks.

## Prefer to watch?

**Onchain AI Garage** put together a Masterclass walkthrough of installation, setup, and basic commands — a good companion to this page if you'd rather follow along on video. For more, see the full [Anika Agent Tutorials & Use Cases](https://www.youtube.com/playlist?list=PLmpUb_PWAkDxewld5ZYyKifuHxgIbiq2d) playlist.

<div style={{position: 'relative', paddingBottom: '56.25%', height: 0, overflow: 'hidden', maxWidth: '100%', marginBottom: '1.5rem'}}>
  <iframe
    style={{position: 'absolute', top: 0, left: 0, width: '100%', height: '100%'}}
    src="https://www.youtube-nocookie.com/embed/R3YOGfTBcQg"
    title="Anika Agent Masterclass: Installation, Setup, Basic Commands"
    frameBorder="0"
...
- Setting up Anika for a team, bot, or always-on workflow
...
|---|---|---|
| I just want Anika working on my machine | `anika setup` | Run a real chat and verify it responds |
| I already know my provider | `anika model` | Save the config, then start chatting |
| I want a bot or always-on setup | `anika gateway setup` after CLI works | Connect Telegram, Discord, Slack, or another platform |
| I want a local or self-hosted model | `anika model` → custom endpoint | Verify the endpoint, model name, and context length |
| I want multi-provider fallback | `anika model` first | Add routing and fallback only after the base chat works |

**Rule of thumb:** if Anika cannot complete a normal chat, do not add more features yet. Get one clean conversation working first, then layer on gateway, cron, skills, voice, or routing.

---

## 1. Install Anika Agent
### With the Anika Desktop installer on macOS or Windows (recommended)
...
The single most important setup step. Use `anika model` to walk through the choice interactively:

```bash
anika model
```

:::tip Easiest path: __NEW_ORG__ Portal
...
That logs you in, sets __NEW_ORG__ as your provider, and turns on the Tool Gateway in one command.
:::

:::info Setup modes
On a fresh install, `anika setup` offers three modes:

- **Quick Setup (__NEW_ORG__ Portal)** — free OAuth login, no API keys; sets up a model plus the Tool Gateway tools. The recommended fast path.
...
Blank Slate writes an explicit `platform_toolsets.cli` list plus `agent.disabled_toolsets`, so nothing you didn't choose ever loads — not even after `anika update`. Re-enable anything later with `anika tools`, seed skills with `anika skills opt-in --sync`, or tune settings with `anika setup agent`.
:::
...
|----------|-----------|---------------|
| **__NEW_ORG__ Portal** | Subscription-based, zero-config | OAuth login via `anika model` |
| **OpenAI Codex** | ChatGPT or Codex subscription, uses Codex models | Device code auth via `anika model` → **ChatGPT or Codex Subscription** |
| **Anthropic** | Claude models directly — Max plan + extra usage credits (OAuth), or API key for pay-per-token | `anika model` → OAuth login (requires Max + extra credits), or an Anthropic API key |
...
| **MiniMax (OAuth)** | MiniMax frontier model via browser OAuth — no API key needed (model name in `anika_cli/models.py` may change between releases) | `anika model` → MiniMax (OAuth) |
...
| **xAI Grok OAuth** | SuperGrok / Premium+ subscription, no API key needed | `anika model` → xAI Grok OAuth |
...
| **Qwen OAuth** | Qwen Portal browser OAuth — no API key needed | `anika model` → Qwen OAuth |
...
| **GitHub Copilot** | GitHub Copilot subscription (GPT-5.x, Claude, Gemini, etc.) | OAuth via `anika model`, or `COPILOT_GITHUB_TOKEN` / `GH_TOKEN` |
| **GitHub Copilot ACP** | Copilot ACP agent backend (spawns local `copilot` CLI) | `anika model` (requires `copilot` CLI + `copilot login`) |
...
:::tip
You can switch providers at any time with `anika model` — no lock-in. For a full list of all supported providers and setup details, see [AI Providers](../integrations/providers.md).
:::

### How settings are stored

Anika separates secrets from normal config:

- **Secrets and tokens** → `~/.anika/.env`
- **Non-secret settings** → `~/.anika/config.yaml`

The easiest way to set values correctly is through the CLI:

```bash
anika config set model anthropic/claude-opus-4.6
...
```bash
anika            # classic CLI
anika --tui      # modern TUI (recommended)
```

You'll see a welcome banner with your model, available tools, and skills. Use a prompt that's specific and easy to verify:

:::tip Pick your interface
Anika ships with two terminal interfaces: the classic `prompt_toolkit` CLI and a newer [TUI](../user-guide/tui.md) with modal overlays, mouse selection, and non-blocking input. Both share the same sessions, slash commands, and config — try each with `anika` vs `anika --tui`.
:::
...
```bash
anika --continue    # Resume the most recent session
anika -c            # Short form
```
...
```bash
anika gateway setup    # Interactive platform configuration
```
...
- `anika tools` — tune tool access per platform
- `anika skills` — browse and install reusable workflows
...
```bash
anika config set terminal.backend docker    # Docker isolation
anika config set terminal.backend ssh       # Remote server
```

For Docker sandboxes, you can also enable the **egress credential-injection proxy** so the sandbox never sees your real API keys — only opaque proxy tokens that work exclusively from behind a local TLS-intercepting daemon. See [Egress proxy](../user-guide/egress/iron-proxy.md). Setup is `anika egress setup && anika egress start`; `anika setup terminal` also points Docker users at it. Modal, SSH, Daytona, and Singularity are not wired yet.

### Voice mode

```bash
# From the Anika install directory (the curl installer placed it at
# ~/.anika/anika-agent on Linux/macOS or %LOCALAPPDATA%\anika\anika-agent on Windows):
cd ~/.anika/anika-agent
...
Skills are on-demand instruction documents that teach Anika how to do a specific task — deploy to Kubernetes, open a GitHub PR, fine-tune a model, search for GIFs. Each is a `SKILL.md` file with a name, a description, and a step-by-step procedure. The agent reads the short descriptions for free and only loads a skill's full content when a task actually calls for it, so adding skills doesn't bloat every request.

Anika ships with a catalog of bundled skills already installed in `~/.anika/skills/`. You can add more from the Skills Hub, or write your own.

**Browse and install from the hub:**

```bash
anika skills browse                      # list everything available
anika skills search kubernetes           # find skills by keyword
anika skills install openai/skills/k8s   # install one (runs a security scan first)
```

The install argument is a `source/path` slug from the hub — `openai/skills/k8s` means the `k8s` skill from OpenAI's catalog. `anika skills browse` shows the exact slugs to use.
...
/k8s                                       # load it and let Anika ask what you need
```
...
(If you installed without `[all]`, run `cd ~/.anika/anika-agent && uv pip install -e ".[acp]"` first.)
...
|---|---|---|
| Anika opens but gives empty or broken replies | Provider auth or model selection is wrong | Run `anika model` again and confirm provider, model, and auth |
...
| Gateway starts but nobody can message it | Bot token, allowlist, or platform setup is incomplete | Re-run `anika gateway setup` and check `anika gateway status` |
| `anika --continue` can't find old session | Switched profiles or session never saved | Check `anika sessions list` and confirm you're in the right profile |
...
| `anika doctor` flags config problems | Config values are missing or stale | Fix the config, retest a plain chat before adding features |
...
2. `anika model`
3. `anika setup`
4. `anika sessions list`
5. `anika --continue`
6. `anika gateway status`
...
| `anika tools` | Configure which tools are enabled per platform |
| `anika setup` | Full setup wizard (configures everything at once) |
| `anika doctor` | Diagnose issues |
| `anika update` | Update to latest version |
| `anika gateway` | Start the messaging gateway |
| `anika --continue` | Resume last session |
...
- **[Moving to another machine](/reference/faq#exporting-anika-to-another-machine)** — `anika backup` migrates your whole setup (or [a single profile](/reference/faq#moving-a-single-profile-to-another-machine)); no need to rebuild from scratch
