---
sidebar_position: 8
title: "Use Voice Mode with Anika"
description: "A practical guide to setting up and using Anika voice mode across CLI, Telegram, Discord, and Discord voice channels"
---

# Use Voice Mode with Anika
...
:::tip
[__NEW_ORG__ Portal](/integrations/nous-portal) bundles both the LLM and TTS through one OAuth — voice mode works end-to-end with no extra credentials.
:::
...
- you want Anika sitting in a Discord voice channel for live conversation
...
There are really three different voice experiences in Anika.
...
## Step 1: make sure normal Anika works first
...
Anika supports both local and cloud speech stacks.
...
### If you use `anika setup`

If you choose NeuTTS in the setup wizard, Anika checks whether `neutts` is already installed. If it is missing, the wizard tells you NeuTTS needs the Python package `neutts` and the system package `espeak-ng`, offers to install them for you, installs `espeak-ng` with your platform package manager, and then runs:
...
4. Anika transcribes and responds
...
- asking Anika to structure your thoughts in real time

#### Accessibility / low-typing sessions

If typing is inconvenient, voice mode is one of the fastest ways to stay in the full Anika loop.

## Tuning CLI behavior

### Silence threshold

If Anika starts/stops too aggressively, tune:
...
Anika stays a normal chat bot, but can speak replies.
...
- you want Anika to function like a portable research or ops assistant
...
Anika joins a Discord VC, listens to user speech, transcribes it, runs the normal agent pipeline, and speaks replies back into the channel.
...
- Anika detects speech boundaries
- transcripts are posted in the associated text channel
- Anika responds in text and audio
...
1. get text Anika working
2. run `anika setup tts` to enable voice support
