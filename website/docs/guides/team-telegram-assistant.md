...
This tutorial walks you through setting up a Telegram bot powered by Anika Agent that multiple team members can use. By the end, your team will have a shared AI assistant they can message for help with code, research, system administration, and anything else — secured with per-user authorization.
...
- **Anika Agent installed** on a server or VPS (not your laptop — the bot needs to stay running). Follow the [installation guide](/getting-started/installation) if you haven't yet.
- **A Telegram account** for yourself (the bot owner)
- **An LLM provider configured** — at minimum, an API key for OpenAI, Anthropic, or another supported provider in `~/.anika/.env`

:::tip
A $5/month VPS is plenty for running the gateway. Anika itself is lightweight — the LLM API calls are what cost money, and those happen remotely.
:::
...
2. **Send `/newbot`** — BotFather will ask you two things:
   - **Display name** — what users see (e.g., `Team Anika Assistant`)
   - **Username** — must end in `bot` (e.g., `myteam_anika_bot`)
...
4. **Set a description** (optional but recommended):
   ...
   ```
   Team AI assistant powered by Anika Agent. DM me for help with code, research, debugging, and more.
   ```
...
```
[Gateway] Starting Anika Gateway...
...
# View live logs
journalctl --user -u anika-gateway -f
...
:::tip macOS PATH
The launchd plist captures your shell PATH at install time so gateway subprocesses can find tools like Node.js and ffmpeg. If you install new tools later, re-run `anika gateway install` to update the plist.
:::
...
```bash
anika gateway stop && anika gateway start
```
...
anika pairing revoke telegram 987654321
...
**Option 2:** Set it manually in `~/.anika/.env`:
...
Control how much detail the bot shows when using tools. In `~/.anika/config.yaml`:
...
Customize how the bot communicates by editing `~/.anika/SOUL.md`:

For a full guide, see [Use SOUL.md with Anika](/guides/use-soul-with-anika).
...
```bash
# From the CLI
anika cron list          # View all scheduled jobs
anika cron status        # Check if scheduler is running
...
| What | Location |
|------|----------|
| Gateway logs | `journalctl --user -u anika-gateway` (Linux) or `~/.anika/logs/gateway.log` (macOS) |
| Cron job output | `~/.anika/cron/output/{job_id}/{timestamp}.md` |
| Cron job definitions | `~/.anika/cron/jobs.json` |
| Pairing data | `~/.anika/pairing/` |
| Session history | `~/.anika/sessions/` |
