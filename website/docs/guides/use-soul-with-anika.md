---
sidebar_position: 7
title: "Use SOUL.md with Anika"
description: "How to use SOUL.md to shape Anika Agent's default voice, what belongs there, and how it differs from AGENTS.md and /personality"
---

# Use SOUL.md with Anika

`SOUL.md` is the **primary identity** for your Anika instance. It's the first thing in the system prompt — it defines who the agent is, how it speaks, and what it avoids.

If you want Anika to feel like the same assistant every time you talk to it — or if you want to replace the Anika persona entirely with your own — this is the file to use.
...
- personality
- communication style
- how direct or warm Anika should be
- what Anika should avoid stylistically
- how Anika should relate to uncertainty, disagreement, and ambiguity

In short:
- `SOUL.md` is about who Anika is and how Anika speaks
...
Anika now uses only the global SOUL file for the current instance:

```text
~/.anika/SOUL.md
```

If you run Anika with a custom home directory, it becomes:
...
Anika automatically seeds a starter `SOUL.md` for you if one does not already exist.

That means most users now begin with a real file they can read and edit immediately.

Important:
- if you already have a `SOUL.md`, Anika does not overwrite it
- if the file exists but is empty, Anika adds nothing from it to the prompt

## How Anika uses it

When Anika starts a session, it reads `SOUL.md` from `ANIKA_HOME`, scans it for prompt-injection patterns, truncates it if needed, and uses it as the **agent identity** — slot #1 in the system prompt. This means SOUL.md completely replaces the built-in default identity text.
...
That alone can noticeably change how Anika feels.
...
Anika already tries to be helpful and clear. `SOUL.md` should add real personality and style, not restate obvious defaults.
...
# Style
How Anika should sound.

# Avoid
What Anika should not do.

# Defaults
How Anika should behave when ambiguity appears.
```
...
Then restart Anika or start a new session.
...
### I edited SOUL.md but Anika still sounds the same

Check:
- you edited `~/.anika/SOUL.md` or `$ANIKA_HOME/SOUL.md`
...
### Anika is ignoring parts of my SOUL.md
