---
sidebar_position: 6
title: "Use MCP with Anika"
description: "A practical guide to connecting MCP servers to Anika Agent, filtering their tools, and using them safely in real workflows"
---

# Use MCP with Anika

This guide shows how to actually use MCP with Anika Agent in day-to-day workflows.
...
Use MCP when:
- a tool already exists in MCP form and you do not want to build a native Anika tool
- you want Anika to operate against a local or remote system through a clean RPC layer
- you want fine-grained per-server exposure control
- you want to connect Anika to internal APIs, databases, or company systems without modifying Anika core

Do not use MCP when:
- a built-in Anika tool already solves the job well
...
- Anika remains the agent
- MCP servers contribute tools
- Anika discovers those tools at startup or reload time
...
If you installed Anika with the standard install script, MCP support is already included (the installer runs `uv pip install -e ".[all]"`).
...
- Anika banner/status should show MCP integration when configured
- ask Anika what tools it has available
...
## WSL2: bridge Anika in WSL to Windows Chrome

This is the practical setup when:

- Anika runs inside WSL2
...
In this setup, Anika does **not** connect to Chrome directly. Instead:

- Anika runs in WSL
- Anika starts a local stdio MCP server
...
```text
Anika (WSL) -> MCP stdio bridge -> Windows Chrome
```

### Why this mode is useful

- you keep your real Windows browser profile, cookies, and logins
- Anika stays in its supported Unix environment (WSL2)
- browser control is exposed as MCP tools instead of relying on Anika core browser transport
...
```bash
anika mcp add chrome-devtools-win --command cmd.exe --args /c npx -y chrome-devtools-mcp@latest --autoConnect --no-usage-statistics
```

After saving the server:

```bash
anika mcp test chrome-devtools-win
```

Then start a fresh Anika session or run:
...
Once loaded, Anika can use the MCP-prefixed browser tools directly. For example:
...
If Anika runs in WSL and Chrome runs on Windows, `/browser connect` may fail even though Chrome is open and debuggable.
...
- Start Anika from a Windows-mounted path like `/mnt/c/Users/<you>` or `/mnt/c/workspace/...` when using Windows stdio executables through MCP.
- If you start Anika from `/root` or `/home/...`, Windows may emit a `UNC` current-directory warning before the MCP server starts.
...
There are two categories of MCP-exposed functionality in Anika:
...
2. Anika-added utility wrappers
- filtered with:
  - `tools.resources`
  - `tools.prompts`
...
So Anika will not pretend a server has resources/prompts if it does not.

## Common patterns

### Pattern 1: local project assistant

Use MCP for a repo-local filesystem or git server when you want Anika to reason over a bounded workspace.
...
Use [Open Scaffold](https://github.com/graphanov/open-scaffold) when you want Anika to read a repository's durable AI-work record: mission, plans, evidence notes, handoff packets, and review/gate results. Anika remains the agent; Open Scaffold remains the repo-local record.

Add the server for one scaffolded repository:

```bash
anika mcp add open_scaffold --command npx --args -y open-scaffold@latest mcp serve --repo /absolute/path/to/repo
anika mcp test open_scaffold
```

Then keep the exposed surface read-oriented. Choose `select` in the `anika mcp add` prompt, or edit `config.yaml` afterward:
...
- Its write tools require the server to be started with `--allow-write`; do not enable that until you explicitly want Anika to mutate `.osc` files.
- Open Scaffold records and gates work; it does not authorize Anika to merge, publish, deploy, or spawn runtimes.
...
Now Anika can combine them:

```text
Inspect the local project files, then create a GitHub issue summarizing the bug you find.
```

That is where MCP gets powerful: multi-system workflows without changing Anika core.
...
Because Anika now respects your per-server policy and capability-aware registration. That is expected, and usually desirable.
