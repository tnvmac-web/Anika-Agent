...
description: "Diagnose and fix common Anika cron issues — jobs not firing, delivery failures, skill loading errors, and performance problems"
---
...
If you're expecting jobs to fire automatically, you need a running gateway (`anika gateway` for foreground, or `anika gateway start` for the installed service). For one-off debugging, you can manually trigger a tick with `anika cron tick`.
...
```bash
date
anika cron list   # Compare next_run times with local time
```
...
| Target | Requires |
|--------|----------|
| `telegram` | `TELEGRAM_BOT_TOKEN` in `~/.anika/.env` |
| `discord` | `DISCORD_BOT_TOKEN` in `~/.anika/.env` |
| `slack` | `SLACK_BOT_TOKEN` in `~/.anika/.env` |
...
| `local` | Write access to `~/.anika/cron/output/` |
...
If delivery fails, the job still runs — it just won't send anywhere. Check `anika cron list` for updated `last_error` field (if available).
...
### Check 5: Relay-fronted platforms (Anika Cloud / Team Gateway)
...
Skills must be installed before they can be attached to cron jobs. If a skill is missing, install it first with `anika skills install <skill-name>` or via `/skills` in the CLI.

### Check 2: Check skill name vs. skill folder name

Skill names are case-sensitive and must match the installed skill's folder name. If your job specifies `ai-funding-report` but the skill folder is `ai-funding-daily-report`, confirm the exact name from `anika skills list`.
...
2. `~/.anika/logs/agent.log` for scheduler messages (or `errors.log` for warnings)
3. The job's `last_run` metadata via `anika cron list`

### Check 2: Common error patterns

**"No such file or directory" for scripts**
The `script` path must be an absolute path (or relative to the Anika config directory). Verify:
```bash
ls ~/.anika/scripts/your-script.py   # Must exist
anika cron edit <job_id> --script ~/.anika/scripts/your-script.py
```

**"Skill not found" at job execution**
The skill must be installed on the machine running the scheduler. If you move between machines, skills don't automatically sync — reinstall them with `anika skills install <skill-name>`.
...
Jobs are stored in `~/.anika/cron/jobs.json`. If this file is not readable/writable by your user, the scheduler will fail silently:

```bash
ls -la ~/.anika/cron/jobs.json
chmod 600 ~/.anika/cron/jobs.json   # Your user should own it
```
...
```bash
anika cron list                    # Show all jobs, states, next_run times
anika cron run <job_id>            # Schedule for next tick (for testing)
anika cron edit <job_id>           # Fix configuration issues
anika logs                         # View recent Anika logs
anika skills list                  # Verify installed skills
```
...
1. Run the job with `anika cron run <job_id>` (fires on next gateway tick) and watch for errors in the chat output
2. Check `~/.anika/logs/agent.log` for scheduler messages and `~/.anika/logs/errors.log` for warnings
3. Open an issue at [github.com/__NEW_ORG__/anika-agent](https://github.com/__NEW_ORG__/anika-agent) with:
