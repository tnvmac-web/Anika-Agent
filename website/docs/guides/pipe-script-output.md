...
description: "Send text from any shell script, cron job, CI hook, or monitoring daemon to Telegram, Discord, Slack, Signal, and other platforms using `anika send`."
---

# Pipe Script Output to Messaging Platforms

`anika send` is a small, scriptable CLI that pushes a message to any
messaging platform Anika is already configured for. Think of it as a
...
- Piping any tool's output anywhere (`make | anika send --to slack:#builds`)

The command reuses the same credentials and platform adapters that `anika
...
anika send --to slack:#eng --subject "[CI] build.log" --file build.log

# Thread target (Telegram topic, Discord thread)
anika send --to telegram:-1001234567890:17585 "threaded reply"

# List every configured target
anika send --list

# Filter by platform
anika send --list telegram
```
...
Any platform Anika ships adapters for works as a target:
...
`anika send` resolves the message body in this order:

1. **Positional argument** — `anika send --to telegram "hi"`
2. **`--file PATH`** — `anika send --to telegram --file msg.txt`
3. **Piped stdin** — `echo hi | anika send --to telegram`

When stdin is a TTY (no pipe), Anika does **not** wait for input — you'll
...
if [ "$ram_pct" -ge 85 ]; then
  anika send --to telegram --subject "⚠ MEMORY WARNING" \
    "RAM ${ram_pct}% on $(hostname)"
fi
```

Because `anika send` reuses your Anika config, the same script works on
any host where Anika is installed — no need to export bot tokens into
...
`anika send`. If the Python interpreter can't load because the box is
...
if ./scripts/deploy.sh; then
  anika send --to slack:#deploys "✅ ${CI_COMMIT_SHA:0:7} deployed"
else
  tail -n 100 deploy.log | anika send \
...
```bash
./train.py --epochs 200 && \
  anika send --to telegram "training done" || \
  anika send --to telegram "training failed (exit $?)"
```
...
anika send --to telegram --quiet "keepalive" || {
  echo "Telegram delivery failed" >&2
  exit 1
}

# Capture the message ID for later editing / threading
msg_id=$(anika send --to discord:#ops --json "build started" \
  | jq -r .message_id)
```

---

## Does `anika send` Need the Gateway Running?

**Usually no.** For any bot-token platform — Telegram, Discord, Slack,
Signal, SMS, WhatsApp Cloud API, and most others — `anika send` calls
the platform's REST endpoint directly using credentials from
`~/.anika/.env` and `~/.anika/config.yaml`. It's a standalone subprocess
...
pointing at the gateway; start it with `anika gateway start` and retry.
...
# Machine-readable
anika send --list --json
```

The listing is built from `~/.anika/channel_directory.json`, which the
gateway refreshes every few minutes while it's running. If you see
"no channels discovered yet", start the gateway once (`anika gateway
...
| Approach | Multi-platform | Reuses Anika creds | Needs gateway | Best for |
|----------|----------------|---------------------|---------------|----------|
| `anika send` | ✅ | ✅ | No (bot-token) | Everything below |
...
`anika send` is intentionally the simplest possible surface. If you need
...
If you just need to pipe a raw string, reach for `anika send`.
...
- [Gateway Internals](/developer-guide/gateway-internals) —
  the delivery router that `anika send` shares with cron delivery.
