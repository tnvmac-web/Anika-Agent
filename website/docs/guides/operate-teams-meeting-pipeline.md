...
```bash
anika teams-pipeline token-health
anika teams-pipeline token-health --force-refresh
```
...
```bash
anika teams-pipeline subscriptions
```

### Renew near-expiry subscriptions

```bash
anika teams-pipeline maintain-subscriptions
anika teams-pipeline maintain-subscriptions --dry-run
```
...
#### Option 1: Anika cron (recommended if you already run the Anika gateway)

Anika ships a built-in cron scheduler. The `--no-agent` mode runs a script as the job (rather than using an LLM), and `--script` must point at a file under `~/.anika/scripts/`. First create the script:

```bash
mkdir -p ~/.anika/scripts
cat > ~/.anika/scripts/maintain-teams-subscriptions.sh <<'EOF'
#!/usr/bin/env bash
exec anika teams-pipeline maintain-subscriptions
EOF
chmod +x ~/.anika/scripts/maintain-teams-subscriptions.sh
```

Then register a script-only cron job that runs every 12 hours (gives 6x headroom against the 72h expiry window):

```bash
anika cron create "0 */12 * * *" \
...
Create `/etc/systemd/system/anika-teams-pipeline-maintain.service`:

```ini
[Unit]
Description=Anika Teams pipeline subscription maintenance
After=network-online.target

[Service]
Type=oneshot
User=anika
EnvironmentFile=/etc/anika/env
ExecStart=/usr/local/bin/anika teams-pipeline maintain-subscriptions
```

And `/etc/systemd/system/anika-teams-pipeline-maintain.timer`:

```ini
[Unit]
Description=Run Anika Teams pipeline subscription maintenance every 12 hours
...
sudo systemctl enable --now anika-teams-pipeline-maintain.timer
systemctl list-timers anika-teams-pipeline-maintain.timer
```

#### Option 3: Plain crontab

```cron
0 */12 * * * /usr/local/bin/anika teams-pipeline maintain-subscriptions >> /var/log/anika/teams-pipeline-maintain.log 2>&1
```

Make sure the cron environment has the `MSGRAPH_*` credentials. Simplest fix: source `~/.anika/.env` at the top of a wrapper script that crontab calls.
...
```bash
anika teams-pipeline subscriptions   # should show expirationDateTime advanced
anika teams-pipeline maintain-subscriptions --dry-run   # should show "0 expiring soon" most of the time
```
...
```bash
anika teams-pipeline list
anika teams-pipeline list --status failed
anika teams-pipeline show <job-id>
```

### Replay a stored job

```bash
anika teams-pipeline run <job-id>
```

### Dry-run meeting artifact fetches

```bash
anika teams-pipeline fetch --meeting-id <meeting-id>
anika teams-pipeline fetch --join-web-url "<join-url>"
anika teams-pipeline fetch --join-web-url "<join-url>" --organizer-user-id <entra-user-id>
```
...
- run `anika teams-pipeline maintain-subscriptions --dry-run`
- inspect `anika teams-pipeline list --status failed`
...
- run `anika teams-pipeline validate`
...
Check:
- whether you manually replayed a job with `anika teams-pipeline run`
...
- [ ] `anika teams-pipeline validate` returns an OK snapshot
- [ ] `anika teams-pipeline token-health --force-refresh` succeeds
- [ ] **`maintain-subscriptions` is scheduled** (Anika cron, systemd timer, or crontab — see [Automating subscription renewal](#automating-subscription-renewal-required-for-production)). Without this, Graph subscriptions silently expire within 72 hours.
