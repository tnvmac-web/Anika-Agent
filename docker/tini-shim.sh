...
# Anika WebUI, NAS "update" flows that preserve an old entrypoint, etc.)
...
#
# Test hook: ANIKA_TINI_SHIM_TARGET overrides the `/init` path (and
# ANIKA_TINI_SHIM_WRAPPER the main-wrapper path) so unit tests can
# record argv without a real s6 tree.

set -e

INIT_TARGET="${ANIKA_TINI_SHIM_TARGET:-/init}"
WRAPPER="${ANIKA_TINI_SHIM_WRAPPER:-/opt/anika/docker/main-wrapper.sh}"
