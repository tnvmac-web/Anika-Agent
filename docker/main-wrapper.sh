#!/bin/sh
# shellcheck shell=sh
# /opt/anika/docker/main-wrapper.sh — wraps the container's CMD with
...
# with-contenv before touching ANIKA_HOME / PATH. On the non-PID-1
# fallback path the Dockerfile env is still intact, so we skip the
# re-exec and continue directly.
#
# Routing:
#   no args                       → exec `anika` (the default)
#   first arg is an executable    → exec it directly (sleep, bash, sh, …)
#   first arg is anything else    → exec `anika <args>` (subcommand passthrough)
#
# Drop to anika via s6-setuidgid, but skip it when already non-root.
set -e

if [ -z "${ANIKA_MAIN_WRAPPER_ENV_READY:-}" ] && \
   [ -z "${ANIKA_HOME:-}" ] && \
   [ -x /command/with-contenv ]; then
    export ANIKA_MAIN_WRAPPER_ENV_READY=1
    exec /command/with-contenv sh "$0" "$@"
fi
unset ANIKA_MAIN_WRAPPER_ENV_READY

drop() { [ "$(id -u)" = 0 ] && set -- s6-setuidgid anika "$@"; exec "$@"; }
...
# an arbitrary non-root, non-anika UID, the bootstrap was skipped and the
# baked image dirs (owned by the anika build UID) are unwritable, so fail
...
if [ "$cur_uid" != 0 ] && [ "$cur_uid" != "$(id -u anika)" ]; then
    cat >&2 <<EOF
[anika] ERROR: container started with --user $cur_uid (an arbitrary, non-anika UID) — not supported.
...
# to the anika user's home before dropping privileges so libraries that
...
_anika_orig_cwd="${ANIKA_ORIG_CWD:-$PWD}"
...
# the user's command so `anika chat` starts in the Docker -w
