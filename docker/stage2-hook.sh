...
#
# Wired into the image as /etc/cont-init.d/01-anika-setup by the
...
ANIKA_HOME="${ANIKA_HOME:-/opt/data}"
INSTALL_DIR="/opt/anika"

# Drop to anika via s6-setuidgid, but skip it when already non-root.
as_anika() { [ "$(id -u)" = 0 ] || { "$@"; return; }; s6-setuidgid anika "$@"; }
...
# starts non-root. The baked install tree under /opt/anika is intentionally
# root-owned and non-writable; mutable runtime state must live under
# $ANIKA_HOME. An arbitrary `--user` UID therefore cannot repair or populate
...
# default) and pass ANIKA_UID/ANIKA_GID — or the PUID/PGID aliases — which the
...
# `id -u anika` is the unremapped build UID (10000) because no root-only remap
# could run. root starts (id -u = 0) and the normal supervised drop to the
# anika UID are both unaffected.
cur_uid="$(id -u)"
if [ "$cur_uid" != 0 ] && [ "$cur_uid" != "$(id -u anika)" ]; then
    cat >&2 <<EOF
[stage2] ERROR: container started with --user $cur_uid (an arbitrary, non-anika UID).
...
and the baked /opt/anika install tree is intentionally root-owned and
...
Start the container as root (the default) and pass your host UID/GID instead:
    ...
    docker run -e ANIKA_UID=\$(id -u) -e ANIKA_GID=\$(id -g) ...

NAS users (Synology / unRAID / UGOS) can use the PUID/PGID aliases:

    docker run -e PUID=\$(id -u) -e PGID=\$(id -g) ...

The image remaps the anika user to that UID/GID at boot and chowns the data
...
# --- Bootstrap ANIKA_HOME as root ---
# Create the directory (and any missing parents) while we still have root
# privileges so the chown checks below see real metadata and the later
# `s6-setuidgid anika mkdir -p` block doesn't EACCES on root-owned
# ancestors. Without this, custom ANIKA_HOME paths whose parents only
# root can create (e.g. `ANIKA_HOME=/home/anika/.anika` in a Compose
...
# --- UID/GID remap ---
# Accept PUID/PGID as aliases for ANIKA_UID/ANIKA_GID.  NAS users (UGOS,
...
# UID 10000 leaves the runtime unable to read the volume.  ANIKA_UID/
# ANIKA_GID still win when both are set.  See #15290, salvages #25872.
ANIKA_UID="${ANIKA_UID:-${PUID:-}}"
ANIKA_GID="${ANIKA_GID:-${PGID:-}}"

if [ -n "${ANIKA_UID:-}" ] && validate_uid_gid "$ANIKA_UID" && [ "$ANIKA_UID" != "$(id -u anika)" ]; then
    echo "[stage2] Changing anika UID to $ANIKA_UID"
    usermod -u "$ANIKA_UID" anika
fi
if [ -n "${ANIKA_GID:-}" ] && validate_uid_gid "$ANIKA_GID" && [ "$ANIKA_GID" != "$(id -g anika)" ]; then
    echo "[stage2] Changing anika GID to $ANIKA_GID"
    # -o allows non-unique GID (e.g. macOS GID 20 "staff" may already
    # exist as "dialout" in the Debian-based container image).
    groupmod -o -g "$ANIKA_GID" anika 2>/dev/null || true
fi
...
# host's `docker` group (or root). The supervised anika user (UID 10000)
...
# anika process with `Groups: 10000` (998 gone); after this hook adds
...
# matching entry that includes anika. Idempotent across container
...
#   - anika is already a member of the right group (idempotent restart)
...
for sock in /var/run/docker.sock /run/docker.sock; do
    ...
    if id -G anika 2>/dev/null | tr ' ' '\n' | grep -qx "$sock_gid"; then
        echo "[stage2] anika already in group $sock_gid for $sock"
        break
    fi
    ...
    if usermod -aG "$sock_group" anika 2>/dev/null; then
        echo "[stage2] Added anika to group $sock_group (GID $sock_gid) for $sock"
    else
        echo "[stage2] Warning: usermod -aG $sock_group anika failed; docker backend may fail with EACCES"
    fi
    break
done

# --- Fix ownership of data volume ---
# When ANIKA_UID is remapped or the top-level $ANIKA_HOME isn't owned by
# the runtime anika UID, restore ownership to anika — but ONLY for the
# directories anika actually writes to. The full $ANIKA_HOME may be a
# host-mounted bind containing unrelated user files; `chown -R` would
# silently destroy host ownership of those (see issue #19788).
#
# The canonical list of anika-owned subdirs is the same one the s6-setuidgid
# mkdir -p block below seeds. Keep them in sync if the seed list changes.
actual_anika_uid=$(id -u anika)
...
chown_anika_tree() {
    ...
    chown -R anika:anika "$target" 2>/dev/null || \
        echo "[stage2] Warning: chown $target failed (rootless container?) — continuing"
}

tree_has_non_anika_owner() {
    target="$1"
    find "$target" \( ! -user anika -o ! -group anika \) -print -quit 2>/dev/null | grep -q .
}

needs_chown=false
if [ "$(stat -c %u "$ANIKA_HOME" 2>/dev/null)" != "$actual_anika_uid" ]; then
    needs_chown=true
fi
if [ "$needs_chown" = true ]; then
    echo "[stage2] Fixing ownership of $ANIKA_HOME (targeted) to anika ($actual_anika_uid)"
    # In rootless Podman the container's "root" is mapped to an
    # unprivileged host UID — chown will fail. That's fine: the volume
    # is already owned by the mapped user on the host side.
    #
    # Top-level $ANIKA_HOME: chown the directory itself (not its contents)
    # so anika can mkdir new subdirs but bind-mounted host files keep
    # their existing ownership.
    if refuse_symlinked_path "chown" "$ANIKA_HOME"; then
        :
    else
        chown anika:anika "$ANIKA_HOME" 2>/dev/null || \
            echo "[stage2] Warning: chown $ANIKA_HOME failed (rootless container?) — continuing"
    fi
    # Anika-owned subdirs: recursive chown is safe here because these are
    # created and managed exclusively by anika (see the s6-setuidgid mkdir
    ...
    for sub in cron sessions logs hooks memories skills skins plans workspace home profiles pairing platforms/pairing lazy-packages; do
        if [ -e "$ANIKA_HOME/$sub" ] && tree_has_non_anika_owner "$ANIKA_HOME/$sub"; then
            chown_anika_tree "$ANIKA_HOME/$sub"
        fi
    done
fi
...
# anika user. Hosted/container instances keep mutable state under
# $ANIKA_HOME (/opt/data) and run with PYTHONDONTWRITEBYTECODE plus
# ANIKA_DISABLE_LAZY_INSTALLS=1. Keeping /opt/anika root-owned and
...
# $ANIKA_HOME/lazy-packages dir on the data volume (Dockerfile sets
# ANIKA_LAZY_INSTALL_TARGET). That dir is appended to the END of sys.path,
...
# even though installs are re-enabled. The dir is seeded + chowned to anika
...
# Always reset ownership of $ANIKA_HOME/profiles to anika on every
# boot. Profile dirs and files can land owned by root when commands
# are invoked via `docker exec <container> anika …` (which defaults
# to root unless `-u` is passed), and that breaks the cont-init
# reconciler (02-reconcile-profiles) which runs as anika and walks
...
if [ -d "$ANIKA_HOME/profiles" ] && tree_has_non_anika_owner "$ANIKA_HOME/profiles"; then
    chown_anika_tree "$ANIKA_HOME/profiles"
fi

# Always reset ownership of $ANIKA_HOME/cron on every boot for the same
# docker-exec/root-write reason as profiles/. The cron scheduler state
# (jobs.json) must stay readable by the unprivileged anika runtime even
...
# gate as profiles/).
if [ -d "$ANIKA_HOME/cron" ] && tree_has_non_anika_owner "$ANIKA_HOME/cron"; then
    chown_anika_tree "$ANIKA_HOME/cron"
fi

# Always ensure logs/gateways is anika-owned (#45258). Formerly healed by
...
# top-level $ANIKA_HOME is mis-owned, so a warm volume with anika-owned
# ANIKA_HOME but root-owned logs/gateways would otherwise leave
# s6-setuidgid anika mkdir failing with Permission denied. Non-recursive:
# profile leaf dirs are each created/owned by their own log/run as anika.
if [ -d "$ANIKA_HOME/logs/gateways" ]; then
    if refuse_symlinked_path "chown" "$ANIKA_HOME/logs/gateways"; then
        :
    else
        chown anika:anika "$ANIKA_HOME/logs/gateways" 2>/dev/null || true
    fi
fi
...
# anika pairing approve …` defaults to uid=0 and writes 0600 root-owned
# approval files that the unprivileged anika gateway cannot read,
# silently leaving the approved user unauthorized (#10270). The targeted
# data-volume chown above only runs when the top-level $ANIKA_HOME is
...
# and cron/.
if [ -d "$ANIKA_HOME/platforms/pairing" ] && tree_has_non_anika_owner "$ANIKA_HOME/platforms/pairing"; then
    chown_anika_tree "$ANIKA_HOME/platforms/pairing"
fi
# Legacy location (pre-consolidated layout).
if [ -d "$ANIKA_HOME/pairing" ] && tree_has_non_anika_owner "$ANIKA_HOME/pairing"; then
    chown_anika_tree "$ANIKA_HOME/pairing"
fi

# Reset ownership of anika-owned top-level state files on every boot.
# The targeted data-volume chown above only covers anika-owned
# *subdirectories*; loose state files living directly under $ANIKA_HOME
# are missed. When those files are created or rewritten by
# `docker exec <container> anika …` (root unless `-u` is passed) they
# land root-owned, and the unprivileged anika runtime then hits
...
# sweep so host-owned files in a bind-mounted $ANIKA_HOME are never
...
# entries of anika_cli.profile_distribution.USER_OWNED_EXCLUDE plus the
...
for f in \
    ...
    active_profile; do
    if [ -e "$ANIKA_HOME/$f" ]; then
        if refuse_symlinked_path "chown" "$ANIKA_HOME/$f"; then
            :
        else
            chown anika:anika "$ANIKA_HOME/$f" 2>/dev/null || true
        fi
    fi
done

# --- config.yaml permissions ---
# Ensure config.yaml is readable by the anika runtime user even if it
# was edited on the host after initial ownership setup.
if [ -f "$ANIKA_HOME/config.yaml" ]; then
    if refuse_symlinked_path "chown/chmod" "$ANIKA_HOME/config.yaml"; then
        :
    else
        chown anika:anika "$ANIKA_HOME/config.yaml" 2>/dev/null || true
        chmod 640 "$ANIKA_HOME/config.yaml" 2>/dev/null || true
    fi
fi

# --- Seed directory structure as anika user ---
# Run as anika via s6-setuidgid so dirs end up owned correctly (matters
# under rootless Podman where chown back to root would fail).
#
# Use direct `mkdir -p` invocation (no `sh -c "..."` wrapper) so the
# shell isn't a second interpreter — defends against $ANIKA_HOME values
...
as_anika mkdir -p \
    ...
    "$ANIKA_HOME/sessions" \
    "$ANIKA_HOME/logs" \
    "$ANIKA_HOME/logs/gateways" \
    "$ANIKA_HOME/hooks" \
    "$ANIKA_HOME/memories" \
    "$ANIKA_HOME/skills" \
    "$ANIKA_HOME/skins" \
    "$ANIKA_HOME/plans" \
    "$ANIKA_HOME/workspace" \
    "$ANIKA_HOME/home" \
    "$ANIKA_HOME/pairing" \
    "$ANIKA_HOME/platforms/pairing" \
    "$ANIKA_HOME/lazy-packages"

# --- Install-method stamp ---
# The 'docker' stamp is baked into the immutable install tree at
# /opt/anika/.install_method (see Dockerfile), NOT written here into
# $ANIKA_HOME. detect_install_method() reads the code-scoped stamp first.
#
# Why we no longer stamp $ANIKA_HOME: it is a shared DATA volume, commonly
# bind-mounted from the host (~/.anika:/opt/data) and sometimes shared with a
...
# 'anika update'. To heal homes already poisoned by older images, remove a
# stale 'docker' stamp from $ANIKA_HOME if one is present (the host install's
# own installer re-creates its code-scoped stamp; a genuine container relies on
# the baked /opt/anika stamp, so deleting the data-dir copy is safe).
if [ -f "$ANIKA_HOME/.install_method" ]; then
    stamped="$(tr -d '[:space:]' < "$ANIKA_HOME/.install_method" 2>/dev/null || true)"
    if [ "$stamped" = "docker" ]; then
        rm -f "$ANIKA_HOME/.install_method" 2>/dev/null || true
    fi
fi
...
seed_one() {
    dest=$1
    src=$2
    if [ ! -f "$ANIKA_HOME/$dest" ] && [ -f "$INSTALL_DIR/$src" ]; then
        if refuse_symlinked_path "seed" "$ANIKA_HOME/$dest"; then
            :
        else
            as_anika cp "$INSTALL_DIR/$src" "$ANIKA_HOME/$dest"
        fi
    fi
}
...
# the first-boot seed above depends on /opt/anika/.env.example being present
...
# not generate one. Anika loads $ANIKA_HOME/.env with override=True, so
...
if [ -n "${API_SERVER_KEY:-}" ]; then
    if [ -f "$ANIKA_HOME/.env" ] && grep -q '^API_SERVER_KEY=..*' "$ANIKA_HOME/.env" 2>/dev/null; then
        echo "[stage2] Warning: API_SERVER_KEY is set in both the container environment and $ANIKA_HOME/.env — the .env value wins at runtime (loaded with override=True)"
    else
        ...
        if [ -f "$ANIKA_HOME/.env" ] && ! refuse_symlinked_path "clean" "$ANIKA_HOME/.env"; then
            sed -i '/^API_SERVER_KEY=$/d' "$ANIKA_HOME/.env" 2>/dev/null || true
        fi
        echo "[stage2] API_SERVER_KEY provided via container environment — skipping generation"
    fi
elif ! grep -q '^API_SERVER_KEY=..*' "$ANIKA_HOME/.env" 2>/dev/null; then
    if refuse_symlinked_path "append" "$ANIKA_HOME/.env"; then
        :
    else
        if [ ! -f "$ANIKA_HOME/.env" ]; then
            ...
            (umask 077 && as_anika touch "$ANIKA_HOME/.env") 2>/dev/null || true
        fi
        if [ -f "$ANIKA_HOME/.env" ]; then
            ...
            if [ -n "$_gen_key" ]; then
                ...
                sed -i '/^API_SERVER_KEY=$/d' "$ANIKA_HOME/.env" 2>/dev/null || true
                if printf 'API_SERVER_KEY=%s\n' "$_gen_key" >> "$ANIKA_HOME/.env" 2>/dev/null; then
                    echo "[stage2] Generated API_SERVER_KEY for the loopback gateway api_server"
                else
                    echo "[stage2] Warning: could not write API_SERVER_KEY to $ANIKA_HOME/.env (read-only volume?) — gateway api_server (cron fires) will be unavailable"
                fi
            fi
            unset _gen_key
        else
            echo "[stage2] Warning: could not create $ANIKA_HOME/.env — gateway api_server (cron fires) will be unavailable"
        fi
    fi
fi
...
if [ -f "$ANIKA_HOME/.env" ]; then
    if refuse_symlinked_path "chown/chmod" "$ANIKA_HOME/.env"; then
        :
    else
        chown anika:anika "$ANIKA_HOME/.env" 2>/dev/null || true
        chmod 600 "$ANIKA_HOME/.env" 2>/dev/null || true
    fi
fi
...
# as the unprivileged `anika` user — without this it gets EACCES on every
...
# anika group (any group member could e.g. stop/suspend this machine), not
...
if [ -S /.fly/api ]; then
    if refuse_symlinked_path "chgrp/chmod" /.fly/api; then
        :
    elif chgrp anika /.fly/api 2>/dev/null && chmod g+w /.fly/api 2>/dev/null; then
        echo "[stage2] Granted anika group access to the Fly Machines API socket"
    else
...
# $ANIKA_HOME on the mounted volume. Run the same safe, non-interactive
# config-schema migrations that `anika update` runs for non-Docker installs,
# after first-boot seeding and before supervised gateway services start.
# Set ANIKA_SKIP_CONFIG_MIGRATION=1 for controlled/manual migrations.
if [ -f "$ANIKA_HOME/config.yaml" ]; then
    s6-setuidgid anika "$INSTALL_DIR/.venv/bin/python" "$INSTALL_DIR/scripts/docker_config_migrate.py" \
...
if [ ! -f "$ANIKA_HOME/auth.json" ] && [ -n "${ANIKA_AUTH_JSON_BOOTSTRAP:-}" ]; then
    if refuse_symlinked_path "seed" "$ANIKA_HOME/auth.json"; then
        :
    else
        printf '%s' "$ANIKA_AUTH_JSON_BOOTSTRAP" > "$ANIKA_HOME/auth.json"
        chown anika:anika "$ANIKA_HOME/auth.json" 2>/dev/null || true
        chmod 600 "$ANIKA_HOME/auth.json"
    fi
fi

# auth.json: re-seed a TERMINALLY-DEAD __NEW_ORG__ bootstrap session (self-heal).
#
# The [ ! -f ] guard above deliberately refuses to clobber an existing
# auth.json, so a container whose __NEW_ORG__ bootstrap session took a terminal
# invalid_grant (tokens cleared, providers.__NEW_ORG__.last_auth_error.relogin_required
...
# supply a freshly-issued session via ANIKA_AUTH_JSON_REBOOTSTRAP (distinct
...
if [ -f "$ANIKA_HOME/auth.json" ] && [ -n "${ANIKA_AUTH_JSON_REBOOTSTRAP:-}" ]; then
    if refuse_symlinked_path "reseed" "$ANIKA_HOME/auth.json"; then
        :
    else
        s6-setuidgid anika "$INSTALL_DIR/.venv/bin/python" \
            "$INSTALL_DIR/scripts/docker_rebootstrap_nous_session.py" \
            "$ANIKA_HOME/auth.json" \
...
# can set ANIKA_GATEWAY_BOOTSTRAP_STATE=running; we seed the state file
...
# ANIKA_AUTH_JSON_BOOTSTRAP seeds auth.json. The [ ! -f ] guard is the
...
if [ ! -f "$ANIKA_HOME/gateway_state.json" ] && \
        [ "${ANIKA_GATEWAY_BOOTSTRAP_STATE:-}" = "running" ]; then
    if refuse_symlinked_path "seed" "$ANIKA_HOME/gateway_state.json"; then
        :
    else
        printf '{"gateway_state":"running"}\n' > "$ANIKA_HOME/gateway_state.json"
        chown anika:anika "$ANIKA_HOME/gateway_state.json" 2>/dev/null || true
        chmod 644 "$ANIKA_HOME/gateway_state.json"
    fi
fi
...
if [ -d "$INSTALL_DIR/skills" ]; then
    as_anika "$INSTALL_DIR/.venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" \
        || echo "[stage2] Warning: skills_sync.py failed; continuing"
fi

# --- Discover agent-browser's Chromium binary ---
# The image's Dockerfile runs `npx playwright install chromium`, which
# populates ``$PLAYWRIGHT_BROWSERS_PATH`` (=/opt/anika/.playwright) with
# a ``chromium_headless_shell-<build>/chrome-headless-shell-linux64/``
# directory. agent-browser (the runtime CLI Anika spawns for the
...
# main-wrapper.sh propagates it into the supervised ``anika`` process
...
if [ -z "${AGENT_BROWSER_EXECUTABLE_PATH:-}" ] && \
    ...
    if [ -n "$browser_bin" ]; then
        ...
        # up for all supervised services (main-anika, dashboard, etc.).
