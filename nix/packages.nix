# nix/packages.nix — Anika Agent package built with uv2nix
{ inputs, ... }:
{
  perSystem =
    {
    ...
    let
      ...
      minimal = pkgs.callPackage ./anika-agent.nix {
    ...
    in
    {
      packages = {
        ...
        tui = full.anikaTui;
        web = full.anikaWeb;
        desktop = full.anikaDesktop;

        update-npm-lockfile = full.anikaNpmLib.updateNpmLockfile;
      };
    };
}
