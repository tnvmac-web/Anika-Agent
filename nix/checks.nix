...
{ inputs, ... }: {
  perSystem = { pkgs, lib, self', ... }:
    let
      anika-agent = self'.packages.default;
      anikaVenv = anika-agent.anikaVenv;
      ...
      evalHomeModule =
        ...
        inputs.home-manager.lib.homeManagerConfiguration {
          ...
          modules = [
            ...
            {
              home = {
                username = "anika-check";
                homeDirectory = "/home/anika-check";
                stateVersion = "24.11";
              };
            }
            { services.anika-agent = settings; }
      ...
      evalHomeSplit =
        ...
        inputs.home-manager.lib.homeManagerConfiguration {
          ...
          modules = [
            ...
            {
              programs.anika-agent = programs;
              services.anika-agent = services;
            }
          ];
        };

      # The option names that each module defines under
      # services.anika-agent. The internal names that the module system adds
      # are not in the list.
      moduleOptionNames =
        eval: lib.attrNames (lib.filterAttrs (n: _: !lib.hasPrefix "_" n) eval.options.services.anika-agent);
      ...
      configKeys = pkgs.runCommand "anika-config-keys" {} ''
        set -euo pipefail
        export HOME=$TMPDIR
        ${anikaVenv}/bin/python3 -c '
import json, sys
...
' > $out
    ...
    in {
      ...
      checks = {
        ...
        in pkgs.runCommand "anika-cross-eval" { } (
          if failures != [] then
        ...
        build-package = pkgs.runCommand "anika-build-package" { } ''
          echo "PASS: package built at ${anika-agent}"
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify the devShell builds successfully (cross-platform).
        build-devshell = pkgs.runCommand "anika-build-devshell" { } ''
          ...
          let
            enabled = evalHomeSplit {
              ...
              services = {
                ...
                environment.ANIKA_TEST = "1";
                environmentFiles = [ "/run/secrets/anika-env" ];
                anikaHomeFiles."SOUL.md" = "test soul";
            ...
            processes =
              if pkgs.stdenv.hostPlatform.isDarwin then
                ...
                }) (lib.filterAttrs (n: _: lib.hasPrefix "anika" n) cfg.launchd.agents)
              else
                lib.mapAttrs (_: unit: {
                  argv = [ unit.Service.ExecStart ];
                  env = unit.Service.Environment;
                }) (lib.filterAttrs (n: _: lib.hasPrefix "anika" n) cfg.systemd.user.services);
            ...
            activation = cfg.home.activation.anikaAgentSetup.data;

            failures =
              lib.optional (names != [
                "anika-agent"
                "anika-backend"
              ]) "expected anika-agent + anika-backend processes, got: ${toString names}"
              ++ lib.optional (
                !lib.hasInfix "bin/anika gateway" (argvOf "anika-agent")
              ) "gateway process does not run `anika gateway`: ${argvOf "anika-agent"}"
              ++ lib.optional (
                !lib.hasInfix "bin/anika serve" (argvOf "anika-backend")
              ) "backend process does not run `anika serve`: ${argvOf "anika-backend"}"
              ++ lib.optional (
                !lib.hasInfix "--no-open" (argvOf "anika-backend")
              ) "backend must pass --no-open so a service never opens a browser"
              ++ lib.optional (
                lib.any (n: !lib.hasInfix "/home/anika-check/.anika" (envOf n)) names
              ) "gateway and backend must share one ANIKA_HOME"
              ++ lib.optional (
                cfg.home.sessionVariables.ANIKA_HOME or null != "/home/anika-check/.anika"
              ) "programs.anika-agent.enable must export ANIKA_HOME for interactive shells"
              ++ lib.optional (
                !lib.hasInfix "anika-config-merge" activation
              ) "activation must deep-merge config.yaml, not overwrite it"
              ++ lib.optional (
                !lib.hasInfix "/home/anika-check/.anika/SOUL.md" activation
              ) "anikaHomeFiles must install into ANIKA_HOME"
              ++ lib.optional (
                !lib.hasInfix "/home/test-user/workspace/AGENTS.md" activation
              ) "documents must install into workingDirectory"
              # The CLI reads ANIKA_MANAGED to name the rebuild command when
              # it refuses to write the configuration. A Home Manager install
              # has no nixos-rebuild command. Thus it must not report NixOS.
              ++ lib.optional (
                !lib.any (n: lib.hasInfix "ANIKA_MANAGED=home-manager" (envOf n)) names
              ) "processes must report ANIKA_MANAGED=home-manager"
              ++ lib.optional (
                !lib.hasInfix "anika-managed" activation
              ) "activation must write a .managed marker naming the managing system";
          in
          pkgs.runCommand "anika-home-manager-module" { } (
            if failures != [ ] then
        ...
        workspace-files-need-a-directory =
          let
            ...
            # the priority.
            sameAsDefault = "/home/anika-check";
            ...
            cases = [
              ...
              {
                name = "anikaHomeFiles needs no directory";
                ok = accepts { anikaHomeFiles."SOUL.md" = "x"; };
              }
              {
          ...
          in
          pkgs.runCommand "anika-workspace-files-need-a-directory" { } (
            if failed != [ ] then
        ...
        # ── The desktop application shares one ANIKA_HOME ───────────────
        # `programs.enable` exports ANIKA_HOME with home.sessionVariables,
        ...
        # no shell profile. Thus the desktop application would open ~/.anika
        # while the services use the ANIKA_HOME of the module, and the user
        ...
        home-manager-desktop =
          let
            tokenFile = "/run/secrets/anika-desktop-token";
            ...
            enabled = evalHomeSplit {
              ...
              services = {
                enable = true;
                anikaHome = "/home/anika-check/.anika-work";
            ...
            desktopPackages = builtins.filter (p: (p.pname or "") == "anika-desktop") cfg.home.packages;
            ...
            # second Anika runtime beside the services is the fault that
            # `programs.enable` plus a plain desktop package would give.
            agentPackages = builtins.filter (p: (p.pname or "") == "anika-agent") cfg.home.packages;
            ...
            backendScript =
              let
                argv =
                  if pkgs.stdenv.hostPlatform.isDarwin then
                    cfg.launchd.agents.anika-backend.config.ProgramArguments
                  else
                    [ cfg.systemd.user.services.anika-backend.Service.ExecStart ];
            ...
            failures =
              ...
              ) "programs.desktop.enable must install exactly one anika-desktop package, got ${toString (lib.length desktopPackages)}"
              ++ lib.optional (
                setValue "ANIKA_HOME" != "/home/anika-check/.anika-work"
              ) "the launcher must carry ANIKA_HOME: a GUI launcher reads no shell profile, so home.sessionVariables never reaches it (got: ${toString (setValue "ANIKA_HOME")})"
              ++ lib.optional (
                setValue "ANIKA_MANAGED" != "home-manager"
              ) "the launcher must report ANIKA_MANAGED=home-manager while the services own the configuration (got: ${toString (setValue "ANIKA_MANAGED")})"
              ++ lib.optional (
                lib.length agentPackages == 1
                && setValue "ANIKA_DESKTOP_ANIKA" != "${lib.head agentPackages}/bin/anika"
              ) "the launcher must pin the agent package that programs.enable installs, and not a second runtime: ${toString (setValue "ANIKA_DESKTOP_ANIKA")}"

              # ── The application reaches the backend of the service ──────
              ++ lib.optional (
                setValue "ANIKA_DESKTOP_REMOTE_URL" != "http://127.0.0.1:9231"
              ) "the launcher must name the backend of the service, or the application starts a second one (got: ${toString (setValue "ANIKA_DESKTOP_REMOTE_URL")})"
              ++ lib.optional (
                !lib.hasInfix "ANIKA_DESKTOP_REMOTE_TOKEN" wrapper
              ) "the launcher must give a token with the URL: the desktop resolver throws when the URL is set alone"
              ++ lib.optional (
                !lib.hasInfix "ANIKA_DASHBOARD_SESSION_TOKEN" backendScript
              ...
              ++ lib.optional (
                setValue "ANIKA_DESKTOP_REMOTE_TOKEN" != null
              ) "the token must never be a --set value: makeWrapper writes it into the world-readable Nix store";
          in
          pkgs.runCommand "anika-home-manager-desktop" { } (
            if failures != [ ] then
              throw "Home Manager desktop check failed:\n${lib.concatMapStringsSep "\n" (f: "  - ${f}") failures}"
            else
              ''
                echo "PASS: the desktop launcher shares ANIKA_HOME, the runtime and the backend of the service"
                mkdir -p $out
                echo "ok" > $out/result
              ''
          );
        ...
        home-manager-desktop-standalone =
          let
            ...
            failures =
              ...
              ++ lib.optional (
                !lib.hasInfix "--set ANIKA_HOME" wrapper
              ) "the launcher must carry ANIKA_HOME even with no services"
              ++ lib.optional (
                lib.hasInfix "ANIKA_MANAGED" wrapper
              ) "the launcher must not claim a managed install when no activation writes one"
              ++ lib.optional (
                lib.hasInfix "ANIKA_DESKTOP_REMOTE_URL" wrapper
              ) "the launcher must not name a backend when the services run none"
              ++ lib.optional (
                cfg.systemd.user.services ? anika-backend || cfg.launchd.agents ? anika-backend
              ) "programs.enable alone must start no service";
          in
          pkgs.runCommand "anika-home-manager-desktop-standalone" { } (
            if failures != [ ] then
        ...
        # command line. A silent removal thus leaves them with no `anika`
        ...
        home-manager-install-package-removed =
          let
            ...
            cases = [
              {
                value = true;
                expect = "programs.anika-agent.enable = true;";
              }
              {
                value = false;
                expect = "programs.anika-agent.enable = false;";
              }
            ];
          ...
          in
          pkgs.runCommand "anika-home-manager-install-package-removed" { } (
            if failures != [ ] then
        ...
        module-option-parity =
          ...
          in
          pkgs.runCommand "anika-module-option-parity" { } (
            if failures != [ ] then
        ...
        nixos-module =
          let
            ...
            units = lib.filterAttrs (n: _: lib.hasPrefix "anika" n) cfg.systemd.services;
            names = lib.attrNames units;
            execOf = name: units.${name}.serviceConfig.ExecStart;
            activation = cfg.system.activationScripts."anika-agent-setup".text;

            failures =
              lib.optional (names != [
                "anika-agent"
                "anika-backend"
              ]) "expected anika-agent + anika-backend units, got: ${toString names}"
              ++ lib.optional (
                !lib.hasInfix "bin/anika gateway" (execOf "anika-agent")
              ) "gateway unit does not run `anika gateway`: ${execOf "anika-agent"}"
              ++ lib.optional (
                !lib.hasInfix "bin/anika dashboard" (execOf "anika-backend")
              ) "backend unit does not run `anika dashboard`: ${execOf "anika-backend"}"
              ++ lib.optional (
                units.anika-agent.environment.ANIKA_HOME != units.anika-backend.environment.ANIKA_HOME
              ) "gateway and backend must share one ANIKA_HOME"
              ++ lib.optional (
                !lib.hasInfix "/var/lib/anika/.anika/SOUL.md" activation
              ) "anikaHomeFiles must install into ANIKA_HOME";
          ...
          in
          pkgs.runCommand "anika-nixos-module" { } (
            if failures != [ ] then
        ...
        # polls and then execs anika, and the assertions reject a
        # configuration that cannot work.
        backend-bind-wait =
          let
            execOf =
              settings:
              (evalNixosModule ({ enable = true; } // settings)).config.systemd.services.anika-backend.serviceConfig.ExecStart;
            ...
            failures =
              ...
              lib.optional (!lib.hasInfix "bin/anika serve --host 127.0.0.1" direct)
                "without waitFor the backend must exec anika directly, got: ${direct}"
              ++ lib.optional (lib.hasInfix "anika-backend-launch" direct)
                "without waitFor the backend must not use the launcher"

              # The hostname mode polls the resolver, then binds the name.
              ++ lib.optional (!lib.hasInfix "anika-backend-launch" hostnameWait)
              ...
              ++ lib.optional (!lib.hasInfix "exec " hostnameScript)
                "the launcher must exec anika, so that it keeps the MainPID"
              ...
              ++ lib.optional (!lib.hasInfix "bin/anika dashboard" interfaceScript)
          ...
          in
          pkgs.runCommand "anika-backend-bind-wait" { } (
            if failures != [ ] then
        ...
        # $ANIKA_HOME/.env. The important property is that a second run
        ...
        env-file-assembly =
          let
            envScript = (import ./moduleCommon.nix { inherit lib; }).mkEnvScript {
              inherit pkgs;
              environment = {
                ANIKA_PUBLIC = "visible";
              };
            };
          in
          pkgs.runCommand "anika-env-file-assembly" { } ''
            set -e
            workdir=$(mktemp -d)
            ...
            grep -qx 'ANIKA_PUBLIC=visible' "$workdir/.env" || \
        ...
        service-argv =
          let
            ...
            sentinel = "--anika-nix-argv-probe";
            probe = argv: lib.escapeShellArgs (argv ++ [ sentinel ]);
          in
          pkgs.runCommand "anika-service-argv" { } ''
            set -e
        ...
        package-contents = pkgs.runCommand "anika-package-contents" { } ''
          set -e
          echo "=== Checking binaries ==="
          test -x ${anika-agent}/bin/anika || (echo "FAIL: anika binary missing"; exit 1)
          test -x ${anika-agent}/bin/anika-agent || (echo "FAIL: anika-agent binary missing"; exit 1)
          echo "PASS: All binaries present"

          echo "=== Checking version ==="
          ${anika-agent}/bin/anika --version 2>&1 | grep -qi "anika" || (echo "FAIL: version check"; exit 1)
        ...
        entry-points-sync = pkgs.runCommand "anika-entry-points-sync" { } ''
          set -e
          echo "=== Checking entry points match pyproject.toml [project.scripts] ==="
          for bin in anika anika-agent anika-acp; do
            test -x ${anika-agent}/bin/$bin || (echo "FAIL: $bin binary missing from Nix package"; exit 1)
        ...
        cli-commands = pkgs.runCommand "anika-cli-commands" { } ''
          set -e
          export HOME=$(mktemp -d)

          echo "=== Checking anika --help ==="
          ${anika-agent}/bin/anika --help 2>&1 | grep -q "gateway" || (echo "FAIL: gateway subcommand missing"; exit 1)
          ${anika-agent}/bin/anika --help 2>&1 | grep -q "config" || (echo "FAIL: config subcommand missing"; exit 1)
        ...
        bundled-skills = pkgs.runCommand "anika-bundled-skills" { } ''
          set -e
          echo "=== Checking bundled skills ==="
          test -d ${anika-agent}/share/anika-agent/skills || (echo "FAIL: skills directory missing"; exit 1)
          echo "PASS: skills directory exists"

          # -L: skills/ is a symlink to the filtered source store path
          SKILL_COUNT=$(find -L ${anika-agent}/share/anika-agent/skills -name "SKILL.md" | wc -l)
          test "$SKILL_COUNT" -gt 0 || (echo "FAIL: no SKILL.md files found in skills directory"; exit 1)
          echo "PASS: $SKILL_COUNT bundled skills found"

          grep -q "ANIKA_BUNDLED_SKILLS" ${anika-agent}/bin/anika || \
            (echo "FAIL: ANIKA_BUNDLED_SKILLS not in wrapper"; exit 1)
          echo "PASS: ANIKA_BUNDLED_SKILLS set in wrapper"

          # Optional skills ship via the wrapper too (pythonSrc excludes
          # them from the wheel, so the env var is the only path in nix).
          test -d ${anika-agent}/share/anika-agent/optional-skills || \
            (echo "FAIL: optional-skills directory missing"; exit 1)
          OPT_COUNT=$(find -L ${anika-agent}/share/anika-agent/optional-skills -name "SKILL.md" | wc -l)
          test "$OPT_COUNT" -gt 0 || (echo "FAIL: no SKILL.md files in optional-skills"; exit 1)
          grep -q "ANIKA_OPTIONAL_SKILLS" ${anika-agent}/bin/anika || \
            (echo "FAIL: ANIKA_OPTIONAL_SKILLS not in wrapper"; exit 1)
          echo "PASS: $OPT_COUNT optional skills found, ANIKA_OPTIONAL_SKILLS set in wrapper"
        ...
        bundled-plugins = pkgs.runCommand "anika-bundled-plugins" { } ''
          set -e
          echo "=== Checking bundled plugins ==="
          test -d ${anika-agent}/share/anika-agent/plugins || (echo "FAIL: plugins directory missing"; exit 1)
          echo "PASS: plugins directory exists"

          test -f ${anika-agent}/share/anika-agent/plugins/platforms/irc/plugin.yaml || \
            (echo "FAIL: irc plugin manifest missing"; exit 1)
          echo "PASS: irc plugin manifest present"

          grep -q "ANIKA_BUNDLED_PLUGINS" ${anika-agent}/bin/anika || \
            (echo "FAIL: ANIKA_BUNDLED_PLUGINS not in wrapper"; exit 1)
          echo "PASS: ANIKA_BUNDLED_PLUGINS set in wrapper"
        ...
        bundled-locales = pkgs.runCommand "anika-bundled-locales" { } ''
          set -e
          echo "=== Checking bundled locales ==="
          test -d ${anika-agent}/share/anika-agent/locales || (echo "FAIL: locales directory missing"; exit 1)
          echo "PASS: locales directory exists"

          # -L: locales/ is a symlink to the source store path
          LOC_COUNT=$(find -L ${anika-agent}/share/anika-agent/locales -name "*.yaml" | wc -l)
          test "$LOC_COUNT" -ge 16 || (echo "FAIL: expected >=16 catalogs, found $LOC_COUNT"; exit 1)
          echo "PASS: $LOC_COUNT locale catalogs found"

          test -f ${anika-agent}/share/anika-agent/locales/en.yaml || (echo "FAIL: en.yaml missing"; exit 1)
          echo "PASS: en.yaml present"

          grep -q "ANIKA_BUNDLED_LOCALES" ${anika-agent}/bin/anika || \
            (echo "FAIL: ANIKA_BUNDLED_LOCALES not in wrapper"; exit 1)
          echo "PASS: ANIKA_BUNDLED_LOCALES set in wrapper"

          # locales/ is a bare data dir (no __init__.py), shipped via a
          # symlink + ANIKA_BUNDLED_LOCALES (not via wheel data-files).
          # Verify the wrapper override resolves real strings.
          export HOME=$(mktemp -d)
          RENDERED=$(cd "$HOME" && ANIKA_BUNDLED_LOCALES=${anika-agent}/share/anika-agent/locales \
            ${anikaVenv}/bin/python3 -c "from agent import i18n; print(i18n.t('gateway.reset.header_default', lang='en'))")
          echo "rendered: $RENDERED"
          test "$RENDERED" != "gateway.reset.header_default" || (echo "FAIL: i18n returned the raw key with ANIKA_BUNDLED_LOCALES set"; exit 1)
        ...
        # ANIKA_OPTIONAL_MCPS (not via wheel data-files).
        bundled-mcps = pkgs.runCommand "anika-bundled-mcps" { } ''
          set -e
          echo "=== Checking bundled optional-mcps ==="
          test -d ${anika-agent}/share/anika-agent/optional-mcps || (echo "FAIL: optional-mcps directory missing"; exit 1)
          echo "PASS: optional-mcps directory exists"

          MANIFEST_COUNT=$(find -L ${anika-agent}/share/anika-agent/optional-mcps -name "manifest.yaml" | wc -l)
          test "$MANIFEST_COUNT" -gt 0 || (echo "FAIL: no manifest.yaml files found"; exit 1)
          echo "PASS: $MANIFEST_COUNT catalog manifests found"

          grep -q "ANIKA_OPTIONAL_MCPS" ${anika-agent}/bin/anika || \
            (echo "FAIL: ANIKA_OPTIONAL_MCPS not in wrapper"; exit 1)
          echo "PASS: ANIKA_OPTIONAL_MCPS set in wrapper"

          export HOME=$(mktemp -d)
          CATALOG=$(cd "$HOME" && ${anika-agent}/bin/anika mcp catalog 2>/dev/null || true)
          echo "catalog output: $CATALOG"
          test -n "$CATALOG" || (echo "FAIL: anika mcp catalog returned empty"; exit 1)
        ...
        bundled-tui = pkgs.runCommand "anika-bundled-tui" { } ''
          set -e
          echo "=== Checking bundled TUI ==="
          test -d ${anika-agent}/ui-tui || (echo "FAIL: ui-tui directory missing"; exit 1)
          echo "PASS: ui-tui directory exists"

          test -f ${anika-agent}/ui-tui/dist/entry.js || (echo "FAIL: compiled entry.js missing"; exit 1)
          echo "PASS: compiled entry.js present"

          # self-contained bundle; no runtime node_modules expected

          grep -q "ANIKA_TUI_DIR" ${anika-agent}/bin/anika || \
            (echo "FAIL: ANIKA_TUI_DIR not in wrapper"; exit 1)
          echo "PASS: ANIKA_TUI_DIR set in wrapper"
        ...
        # Verify ANIKA_NODE is set in wrapper and points to Node 26+
        # (Anika pins its toolchain to Node 26 everywhere)
        anika-node = pkgs.runCommand "anika-node-version" { } ''
          set -e
          echo "=== Checking ANIKA_NODE in wrapper ==="
          grep -q "ANIKA_NODE" ${anika-agent}/bin/anika || \
            (echo "FAIL: ANIKA_NODE not set in wrapper"; exit 1)
          echo "PASS: ANIKA_NODE present in wrapper"

          ANIKA_NODE=$(sed -n "s/^export ANIKA_NODE='\(.*\)'/\1/p" ${anika-agent}/bin/anika)
          test -x "$ANIKA_NODE" || (echo "FAIL: ANIKA_NODE=$ANIKA_NODE not executable"; exit 1)
          echo "PASS: ANIKA_NODE executable at $ANIKA_NODE"

          NODE_MAJOR=$("$ANIKA_NODE" --version | sed 's/^v//' | cut -d. -f1)
          test "$NODE_MAJOR" -ge 26 || \
            (echo "FAIL: Node v$NODE_MAJOR < 26, Anika requires Node 26"; exit 1)
          echo "PASS: Node v$NODE_MAJOR >= 26"

          echo "=== All ANIKA_NODE checks passed ==="
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify ANIKA_MANAGED guard works on all mutation commands
        managed-guard = pkgs.runCommand "anika-managed-guard" { } ''
          set -e
          export HOME=$(mktemp -d)
          ...
          check_blocked() {
            local label="$1"
            shift
            OUTPUT=$(ANIKA_MANAGED=true "$@" 2>&1 || true)
          ...
          echo "=== Checking ANIKA_MANAGED guards ==="
          check_blocked "config set" ${anika-agent}/bin/anika config set model foo
          check_blocked "config edit" ${anika-agent}/bin/anika config edit
        ...
        extra-python-packages = let
          ...
          anikaWithExtra = anika-agent.override {
            extraPythonPackages = [ testPkg ];
          };
        in pkgs.runCommand "anika-extra-python-packages" { } ''
          set -e
          echo "=== Checking extraPythonPackages PYTHONPATH injection ==="

          grep -q "PYTHONPATH" ${anikaWithExtra}/bin/anika || \
            (echo "FAIL: PYTHONPATH not in wrapper"; exit 1)
          echo "PASS: PYTHONPATH present in wrapper"

          grep -q "${testPkg}" ${anikaWithExtra}/bin/anika || \
          ...
          if grep -q "PYTHONPATH" ${anika-agent}/bin/anika; then
        ...
        extra-dependency-groups = let
          anikaWithGroups = anika-agent.override {
            extraDependencyGroups = [ "honcho" ];
          };
        in pkgs.runCommand "anika-extra-dependency-groups" { } ''
          set -e
          ...
          echo "derivation: ${anikaWithGroups}"
          echo "venv: ${anikaWithGroups.anikaVenv}"
        ...
        messaging-variant = pkgs.runCommand "anika-messaging-variant" { } ''
          set -e
          echo "=== Checking discord.py importable from messaging variant ==="
          ${self'.packages.messaging.anikaVenv}/bin/python3 -c \
        ...
        in pkgs.runCommand "anika-config-roundtrip" {
          ...
          merge_and_load() {
            local anika_home="$1"
            export ANIKA_HOME="$anika_home"
            ${configMergeScript} ${nixSettings} "$anika_home/config.yaml"
            ${anikaVenv}/bin/python3 -c '
import json, sys
