...
let
    ...
    (callPackage pyproject-nix.build.packages {
      ...
      (
        lib.composeManyExtensions [
          ...
          # Anika derivation. This is deliberately a derivation environment
          # variable, not a devShell variable: ``nix develop -c uv build``
          # must remain blocked.
          (final: prev: {
            anika-agent = prev.anika-agent.overrideAttrs (_old: {
              ANIKA_NIX_BUILD = "1";
            });
          })
        ]
  ...
  # $ANIKA_PYTHON_SRC_ROOT at runtime.
  workspaceRoot = ./..;
  ...
  editableOverlay = editableWorkspace.mkEditablePyprojectOverlay {
    root = "$ANIKA_PYTHON_SRC_ROOT"; # resolved at shellHook time
  };
  ...
  editableSet = pythonSet.overrideScope (
    lib.composeManyExtensions [
      editableOverlay
      (final: prev: {
        anika-agent = prev.anika-agent.overrideAttrs (old: {
...
in
{
  venv = pythonSet.mkVirtualEnv "anika-agent-env" {
    anika-agent = dependency-groups;
  };
  editableVenv = editableSet.mkVirtualEnv "anika-agent-editable-env" {
    anika-agent = dependency-groups;
  };
}
