# nix/desktop.nix — Anika Desktop (Electron) app build + wrapper
#
# `anikaAgent` is the fully-built `.#default` package — it ships the
# `anika` binary with the venv, runtime PATH, bundled skills/plugins, etc.
# already wired up.  We point the desktop at it via the existing
# `ANIKA_DESKTOP_ANIKA` override env var, so the desktop's resolver
# uses our fully wrapped binary at step 4 ("existing Anika CLI").
# No reimplementation of the agent resolution in this wrapper.
{
  ...
  # ANIKA_HOME and ANIKA_MANAGED here, which gives the app the same state
...
let
  ...
  targetPlatform =
    if stdenv.hostPlatform.isDarwin then
      ...
      throw "anika-desktop: unsupported host platform for node-pty staging";
  ...
  targetArch =
    if stdenv.hostPlatform.isAarch64 then
      ...
      throw "anika-desktop: unsupported host arch for node-pty staging";

  # Build the renderer (dist/ + electron/ + package.json).
  renderer = anikaNpmLib.buildNpmPackage {
    dirs = [
      "apps/desktop"
      "apps/shared"
    ];
    pname = "anika-desktop-renderer";
    ...
    buildPhase = ''
      ...
      pushd apps/desktop
        ...
        ${lib.getExe anikaNpmLib.node-gyp} rebuild \
...
stdenv.mkDerivation {
  ...
  installPhase = ''
    ...
    mkdir -p $out/share/anika-desktop $out/bin
    cp -r ${renderer}/* $out/share/anika-desktop/
    ...
    substituteInPlace $out/share/anika-desktop/dist/electron-main.mjs \
      --replace-fail "process.resourcesPath" "'$out/share/anika-desktop'"

    # Wrap the nixpkgs electron binary to launch our app.  Set
    # ANIKA_DESKTOP_ANIKA to the absolute path of the nix-built `anika`
    # binary so the desktop's resolver step 4 ("existing Anika CLI on
    # PATH") uses our fully wrapped binary — venv with all deps,
    # bundled skills/plugins, runtime PATH (ripgrep/git/ffmpeg/etc).
    # No reimplementation of the agent resolver in the wrapper.
    makeWrapper ${lib.getExe electron} $out/bin/anika-desktop \
      --add-flags "$out/share/anika-desktop" \
      --set ANIKA_DESKTOP_ANIKA "${lib.getExe anikaAgent}" \
    ...
    install -m 0644 ${../apps/desktop/assets/icon.png} \
      $out/share/icons/hicolor/1024x1024/apps/anika.png
    export PYTHONPATH=$(mktemp -d)
    cp ${../anika_cli/linux_desktop_entry.py} "$PYTHONPATH/linux_desktop_entry.py"
    export DESKTOP_EXEC="$out/bin/anika-desktop"
    export DESKTOP_ICON="$out/share/icons/hicolor/1024x1024/apps/anika.png"
    python3 -c 'import os; from linux_desktop_entry import render_desktop_entry; print(render_desktop_entry(os.environ["DESKTOP_EXEC"], os.environ["DESKTOP_ICON"]))' > $out/share/applications/anika.desktop
    runHook postInstall
  '';
  ...
  meta = with lib; {
    description = "Native Electron desktop shell for Anika Agent";
    homepage = "https://github.com/__NEW_ORG__/anika-agent";
    license = licenses.mit;
    platforms = platforms.unix;
    mainProgram = "anika-desktop";
  };
}
