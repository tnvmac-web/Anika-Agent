...
#
# `services.anika-agent` is the same option set on both modules. Both modules
...
#   homeManagerModules.nix  anikaHome, programs.anika-agent (the CLI and
...
let
  ...
  deepConfigType = types.mkOptionType {
    name = "anika-config-attrs";
    description = "Anika YAML config (attrset), merged deeply via lib.recursiveUpdate.";
    ...
    options = {
      ...
      auth = mkOption {
        ...
        description = ''
          ...
          (remote MCP servers). Tokens are stored in $ANIKA_HOME/mcp-tokens/.
        '';
      };
  ...
  sharedOptions =
    ...
    {
      enable = lib.mkEnableOption "Anika Agent";
      ...
      package = mkOption {
        ...
        description = "The anika-agent package to use.";
      };
      ...
      settings = mkOption {
        ...
        default = { };
        description = ''
          The Anika configuration, as an attribute set. The module joins the
          ...
          which includes the keys that `anika config set` and the settings
      ...
      environmentFiles = mkOption {
        ...
        description = ''
          ...
          $ANIKA_HOME/.env. Anika reads that file at each start, with
          load_anika_dotenv().

          Each activation writes .env again from the start. Thus a secret
          file cannot go into .env two times.
        '';
        example = literalExpression ''[ config.sops.secrets."anika/env".path ]'';
      };
      ...
      authFile = mkOption {
        ...
        description = ''
          ...
          does not exist. Thus a token that Anika refreshes at runtime stays
          after an activation.
        '';
      };
      ...
      documents = mkOption {
        ...
        description = ''
          ...
          Anika reads SOUL.md and memories/ from ANIKA_HOME, so put those
          files in `anikaHomeFiles`.
      ...
      anikaHomeFiles = mkOption {
        type = documentsType;
        default = { };
        description = ''
          Files that the module installs into ANIKA_HOME. Each key is a path
          relative to that directory, and the module makes the necessary
          subdirectories. Each value is a string or a path.

          Anika reads SOUL.md and the memory files from ANIKA_HOME and not
          from the working directory. Declare those files here, or Anika
          does not load them.
        '';
        example = literalExpression ''
          {
      ...
      extraPlugins = mkOption {
        ...
        default = [ ];
        description = ''
          Directory-based plugin packages to symlink into the anika plugins
          directory. Each package must contain a plugin.yaml and __init__.py
          at its root. Anika discovers these automatically on startup.
        '';
        example = literalExpression ''
          [
      ...
      extraPythonPackages = mkOption {
        ...
        description = ''
          ...
          anika_agent.plugins entry-point group. Each package must be built
          with the same Python interpreter as anika (python312).
        '';
        example = literalExpression ''
          [
      ...
      extraDependencyGroups = mkOption {
        ...
        description = ''
          ...
          Use this for optional extras already declared in anika-agent's
      ...
      extraArgs = mkOption {
        type = types.listOf types.str;
        default = [ ];
        description = "Extra command-line arguments for `anika gateway`.";
      };
      ...
      # ── The backend: `anika serve` or `anika dashboard` ──────────────
      # `anika serve` and `anika dashboard` are the same entry point,
      # anika_cli.main:cmd_dashboard, with one flag of difference. serve runs
      # without a user interface. dashboard also serves the web application.
      # Both give the /api/ws and /api/pty sockets that Anika Desktop
      ...
      # controls an external gateway, with `anika gateway restart`. It does
      ...
      backend = {
        mode = mkOption {
          ...
          description = ''
            ...
            - "serve"     — the backend without a user interface. It gives
                            the /api/ws and /api/pty sockets that Anika
                            Desktop connects to.
        ...
        sessionTokenFile = mkOption {
          ...
          description = ''
            ...
            ANIKA_DASHBOARD_SESSION_TOKEN. That token authorizes the /api
            routes and the /api/ws socket. Anika Desktop presents the same
          ...
          '';
          example = literalExpression ''config.sops.secrets."anika/desktop-token".path'';
        };
      };
    };
  ...
  # removal leaves them with no `anika` on the PATH and no message. The
  ...
  installPackageRemovedMessage =
    value:
    ''
      services.anika-agent.installPackage was removed. Anika now
      separates the installation from the services, which is the
      Home Manager convention:

        programs.anika-agent.enable = ${lib.boolToString (value != false)};  # the anika CLI, and ANIKA_HOME for your shells
        programs.anika-agent.desktop.enable = true;  # the desktop application

      `services.anika-agent` keeps the state, the configuration and
  ...
  mkConfigFiles =
    ...
    let
      generated = pkgs.writeText "anika-config.yaml" (
  ...
  mkDocumentTree =
    { pkgs, documents }:
    pkgs.runCommand "anika-documents" { } (
      ''
        mkdir -p $out
      ''
  ...
  mkEnvScript =
    { pkgs, environment }:
    let
      base = pkgs.writeText "anika-env-base" (
        lib.concatStringsSep "\n" (lib.mapAttrsToList (k: v: "${k}=${v}") environment)
        + lib.optionalString (environment != { }) "\n"
      );
    in
    pkgs.writeShellScript "anika-env-merge" ''
      set -eu
      ...
      for file in "$@"; do
        if [ -r "$file" ]; then
          ...
          echo "anika-agent: WARNING cannot read environmentFile $file" >&2
        fi
      done
    '';
  ...
  mkStateScript =
    {
      ...
      # ANIKA_MANAGED variable of the service. The value tells the shell
    ...
    in
    ''
      # Directories. The service units and Anika make most of these
      ...
      ${run}mkdir -p ${
        lib.escapeShellArgs (
          ...
          ++ map (d: "${anikaHome}/${d}") stateDirs
        )
      }

      # config.yaml: merge the Nix settings into the file on disk. Anika
      ...
      ${
        if cfg.configFile != null then
          "${inst} -m ${modes.config} -D ${configFiles.effective} ${anikaHome}/config.yaml"
        else
          ''
            ${run}${configFiles.mergeScript} ${configFiles.generated} ${anikaHome}/config.yaml
            ${run}chmod ${modes.config} ${anikaHome}/config.yaml
          ''
      }

      # The managed-mode marker. It makes an interactive shell also refuse to
      # change the configuration that Nix owns.
      ${inst} -m ${modes.managed} ${pkgs.writeText "anika-managed" managedSystem} ${anikaHome}/.managed

      ${lib.optionalString (cfg.environment != { } || cfg.environmentFiles != [ ]) ''
        ${run}${envScript} ${anikaHome}/.env ${modes.env} ${lib.escapeShellArgs cfg.environmentFiles}
        ${lib.optionalString (owner != null) "${run}chown ${owner} ${anikaHome}/.env"}
      ''}

      ${lib.optionalString (cfg.authFile != null) (
        if cfg.authFileForceOverwrite then
          "${inst} -m ${modes.auth} ${cfg.authFile} ${anikaHome}/auth.json"
        else
          ''
            if [ ! -e ${anikaHome}/auth.json ]; then
              ${inst} -m ${modes.auth} ${cfg.authFile} ${anikaHome}/auth.json
            fi
          ''
      )}
      ...
      ${installDocuments homeDocumentTree anikaHome cfg.anikaHomeFiles}
      ...
      ${run}find ${anikaHome}/plugins -maxdepth 1 -type l -name 'nix-managed-*' -delete 2>/dev/null || true
      ${lib.concatMapStringsSep "\n" (plugin: ''
        if [ ! -f ${plugin}/plugin.yaml ]; then
          echo "anika-agent: ERROR extraPlugins entry '${plugin}' has no plugin.yaml" >&2
          exit 1
        fi
        ${run}ln -sfn ${plugin} ${anikaHome}/plugins/nix-managed-${lib.getName plugin}
      '') cfg.extraPlugins}
    '';
  ...
  gatewayArgv =
    cfg:
    [
      "${effectivePackage cfg}/bin/anika"
      "gateway"
    ]
    ++ cfg.extraArgs;
  ...
  #
  # `exec` on the last line keeps anika as the MainPID of the unit. No shell
  ...
  backendLauncher =
    ...
    pkgs.writeShellScript "anika-backend-launch" (
      builtins.replaceStrings [ "@HOST@" ] [ ''"$_target"'' ] ''
        ...
        ${lib.optionalString (cfg.backend.sessionTokenFile != null) ''
          ...
          if [ ! -r "$_token_file" ]; then
            echo "anika-backend: cannot read the session token file '$_token_file'. The unit stops." >&2
            echo "anika-backend: backend.sessionTokenFile must name a runtime path that this user can read." >&2
            exit 1
          fi

          ANIKA_DASHBOARD_SESSION_TOKEN="$(${pkgs.coreutils}/bin/tr -d '\r\n' < "$_token_file")"
          export ANIKA_DASHBOARD_SESSION_TOKEN

          if [ -z "$ANIKA_DASHBOARD_SESSION_TOKEN" ]; then
            echo "anika-backend: the session token file '$_token_file' is empty. The unit stops." >&2
            exit 1
          fi
        ''}
        ${
          if cfg.backend.waitFor == null then
            ...
            ''
              ...
              while :; do
                ...
                if [ "$_waited" -ge "$_timeout" ]; then
                  echo "anika-backend: '$_target' did not resolve after ''${_timeout}s. The unit stops." >&2
                  exit 1
                fi

                if [ "$_waited" = 0 ]; then
                  echo "anika-backend: waits for '$_target' to resolve..." >&2
                fi
            ...
            ''
              ...
              while :; do
                ...
                if [ "$_waited" -ge "$_timeout" ]; then
                  echo "anika-backend: interface '$_iface' had no IPv4 address after ''${_timeout}s. The unit stops." >&2
                  echo "anika-backend: a fallback address can expose the backend more widely than you intend." >&2
                  exit 1
                fi

                if [ "$_waited" = 0 ]; then
                  echo "anika-backend: waits for an IPv4 address on '$_iface'..." >&2
                fi
                ${pkgs.coreutils}/bin/sleep 2
                _waited=$(( _waited + 2 ))
              done
        ...
        echo "anika-backend: binds to $_target:${toString cfg.backend.port} (from $_how)" >&2
  ...
  backendDescription =
    ...
    if cfg.backend.mode == "dashboard" then
      "Anika Agent web dashboard and desktop backend"
    else
      "Anika Agent backend for Anika Desktop";

  # The environment that each Anika process needs, from either module.
  #
  # managedSystem gives the value of ANIKA_MANAGED. The CLI reads that
    ...
    [
      {
        ...
        message = ''
          ...
          To give Anika an identity and a memory, use
          ${optionPath}.anikaHomeFiles instead. Those files go to
          ANIKA_HOME. Anika reads SOUL.md and memories/ only from there.
        '';
      }
    ];
  ...
  # The subdirectories of ANIKA_HOME that both modules make.
  stateSubdirs = [
    "cron"
    "sessions"
    "logs"
