# nix/nixosModules.nix — the NixOS module for anika-agent
#
...
#
# Container mode: anika runs from /nix/store bind-mounted read-only into a
...
# container recreation. Environment variables are written to $ANIKA_HOME/.env
# and read by anika at startup — no container recreation needed for env changes.
#
# Tool resolution: the anika wrapper uses --suffix PATH for nix store tools,
...
{ inputs, ... }:
{
  flake.nixosModules.default =
    ...
    let
      ...
      # ANIKA_HOME via addToSystemPackages, so they can save settings through the
      ...
      # anika user + sudo on first boot (writable layer persists), then
      # drops privileges. Supports arbitrary base images (Debian, Alpine, etc).
      containerEntrypoint = pkgs.writeShellScript "anika-container-entrypoint" ''
        set -eu

        ANIKA_UID="''${ANIKA_UID:?ANIKA_UID must be set}"
        ANIKA_GID="''${ANIKA_GID:?ANIKA_GID must be set}"

        # ── Group: ensure a group with GID=$ANIKA_GID exists ──
        # Check by GID (not name) to avoid collisions with pre-existing groups
        # (e.g. GID 100 = "users" on Ubuntu)
        EXISTING_GROUP=$(getent group "$ANIKA_GID" 2>/dev/null | cut -d: -f1 || true)
        if [ -n "$EXISTING_GROUP" ]; then
          GROUP_NAME="$EXISTING_GROUP"
        else
          GROUP_NAME="anika"
          if command -v groupadd >/dev/null 2>&1; then
            groupadd -g "$ANIKA_GID" "$GROUP_NAME"
          elif command -v addgroup >/dev/null 2>&1; then
            addgroup -g "$ANIKA_GID" "$GROUP_NAME" 2>/dev/null || true
          fi
        fi

        # ── User: ensure a user with UID=$ANIKA_UID exists ──
        PASSWD_ENTRY=$(getent passwd "$ANIKA_UID" 2>/dev/null || true)
        if [ -n "$PASSWD_ENTRY" ]; then
          TARGET_USER=$(echo "$PASSWD_ENTRY" | cut -d: -f1)
          TARGET_HOME=$(echo "$PASSWD_ENTRY" | cut -d: -f6)
        else
          TARGET_USER="anika"
          TARGET_HOME="/home/anika"
          if command -v useradd >/dev/null 2>&1; then
            useradd -u "$ANIKA_UID" -g "$ANIKA_GID" -m -d "$TARGET_HOME" -s /bin/bash "$TARGET_USER"
          elif command -v adduser >/dev/null 2>&1; then
            adduser -u "$ANIKA_UID" -D -h "$TARGET_HOME" -s /bin/sh -G "$GROUP_NAME" "$TARGET_USER" 2>/dev/null || true
          fi
        fi
        mkdir -p "$TARGET_HOME"
        chown "$ANIKA_UID:$ANIKA_GID" "$TARGET_HOME"
        chmod 0750 "$TARGET_HOME"

        # Ensure ANIKA_HOME is owned by the target user.
        ...
        if [ -n "''${ANIKA_HOME:-}" ] && [ -d "$ANIKA_HOME" ]; then
          find "$ANIKA_HOME" \! -user "$ANIKA_UID" -exec chown "$ANIKA_UID:$ANIKA_GID" {} +
        fi
        ...
        if [ ! -f /var/lib/anika-tools-provisioned ] && command -v apt-get >/dev/null 2>&1; then
          ...
          touch /var/lib/anika-tools-provisioned
        fi

        if command -v sudo >/dev/null 2>&1 && [ ! -f /etc/sudoers.d/anika ]; then
          mkdir -p /etc/sudoers.d
          echo "$TARGET_USER ALL=(ALL) NOPASSWD:ALL" > /etc/sudoers.d/anika
          chmod 0440 /etc/sudoers.d/anika
        fi
        ...
        if command -v setpriv >/dev/null 2>&1; then
          exec setpriv --reuid="$ANIKA_UID" --regid="$ANIKA_GID" --init-groups "$@"
      ...
      # so they can update without recreation. Env vars go through $ANIKA_HOME/.env.
      ...
      containerModeFile = pkgs.writeText "anika-container-mode" ''
        # Written by the NixOS activation script. Do not edit manually.
        backend=${cfg.container.backend}
        container_name=${containerName}
        exec_user=${cfg.user}
        anika_bin=${containerDataDir}/current-package/bin/anika
      '';

      # Default: /var/lib/anika/workspace → /data/workspace.
      ...
      commonServiceConfig = {
        ...
        # so interactive users in the anika group can read/write them.
        UMask = "0007";
      ...
      // common.processEnvironment { inherit anikaHome; };
    ...
    in
    {
      options.services.anika-agent =
        ...
        // (
          with lib;
          {
            ...
            stateDir = mkOption {
              type = types.str;
              default = "/var/lib/anika";
              description = "State directory. Contains .anika/ subdir (ANIKA_HOME).";
            };
            ...
            addToSystemPackages = mkOption {
              ...
              default = false;
              description = ''
                Add the anika CLI to environment.systemPackages and export
                ANIKA_HOME system-wide (via environment.variables) so interactive
            ...
            container = {
              ...
              hostUsers = mkOption {
                ...
                default = [ ];
                description = ''
                  Interactive users who get a ~/.anika symlink to the service
                  stateDir. These users are automatically added to the anika group.
                '';
                example = [ "sidbin" ];
              };
            };
      ...
      config = lib.mkIf cfg.enable (
        lib.mkMerge [
          ...
          # ── Host CLI ──────────────────────────────────────────────────────
          # Add the anika CLI to system PATH and export ANIKA_HOME system-wide
          # so interactive shells share state (sessions, skills, cron) with the
          # gateway service instead of creating a separate ~/.anika/.
          (lib.mkIf cfg.addToSystemPackages {
            environment.systemPackages = [ effectivePackage ];
            environment.variables.ANIKA_HOME = anikaHome;
          })
              ...
              ++ [
                {
                  ...
                  message = "services.anika-agent: backend.mode is not supported together with container.enable — the container runs the gateway only.";
                }
              ];
          }

          # ── Per-user profile for extraPackages ───────────────────────────
          # Wire extraPackages into the anika user's per-user profile so the
          ...
          (lib.mkIf
            (cfg.container.enable && !cfg.addToSystemPackages && cfg.container.hostUsers != [ ])
            {
              warnings = [
                ''
                  services.anika-agent: container.enable is true and container.hostUsers
                  is set, but addToSystemPackages is false. Without a host-installed anika
                  binary, container routing will not work for interactive users.
                  Set addToSystemPackages = true or ensure anika is on PATH.
                ''
              ];
            }
          )
          ...
          # ── Directories ───────────────────────────────────────────────────
          {
            systemd.tmpfiles.rules = [
              ...
              "d ${anikaHome}                  2770 ${cfg.user} ${cfg.group} - -"
              "d ${cfg.stateDir}/home           0750 ${cfg.user} ${cfg.group} - -"
              "d ${cfg.workingDirectory}        2770 ${cfg.user} ${cfg.group} - -"
            ]
            ++ map (d: "d ${anikaHome}/${d} 2770 ${cfg.user} ${cfg.group} - -") common.stateSubdirs;
          }

          # ── Activation: link config + auth + documents ────────────────────
          {
            system.activationScripts."anika-agent-setup" =
              lib.stringAfter
                (
                ...
                ''
                  ...
                  chown ${cfg.user}:${cfg.group} ${cfg.stateDir} ${anikaHome} ${cfg.stateDir}/home ${cfg.workingDirectory}
                  chmod 2770 ${cfg.stateDir} ${anikaHome} ${cfg.workingDirectory}
                  ...
                  find ${anikaHome} -maxdepth 1 \
                    \( -name "*.db" -o -name "*.db-wal" -o -name "*.db-shm" -o -name "SOUL.md" \) \
                    -exec chmod g+rw {} + 2>/dev/null || true
                  for _subdir in ${lib.concatStringsSep " " common.stateSubdirs}; do
                    mkdir -p "${anikaHome}/$_subdir"
                    chown ${cfg.user}:${cfg.group} "${anikaHome}/$_subdir"
                    chmod 2770 "${anikaHome}/$_subdir"
                    find "${anikaHome}/$_subdir" -type f \
                      -exec chmod g+rw {} + 2>/dev/null || true
                  done

                  ${common.mkStateScript {
                    inherit pkgs cfg anikaHome;
                    workingDirectory = cfg.workingDirectory;
                  ...
                  chown -h ${cfg.user}:${cfg.group} ${anikaHome}/plugins/nix-managed-* 2>/dev/null || true
                  ...
                  ${
                    if cfg.container.enable then
                      ''
                        install -o ${cfg.user} -g ${cfg.group} -m 0644 ${containerModeFile} ${anikaHome}/.container-mode
                      ''
                    else
                      ''
                        rm -f ${anikaHome}/.container-mode
                        ...
                        ${lib.concatStringsSep "\n" (
                          map (
                            ...
                            let
                              ...
                              symlinkPath = "${userHome}/.anika";
                            in
                            ''
                              if [ -L "${symlinkPath}" ] && [ "$(readlink "${symlinkPath}")" = "${anikaHome}" ]; then
                                rm -f "${symlinkPath}"
                                echo "anika-agent: removed symlink ${symlinkPath}"
                              fi
                            ''
                  ...
                  # Create ~/.anika -> stateDir/.anika for each hostUser so the
          ...
          (lib.mkIf (!cfg.container.enable) {
            systemd.services.anika-agent = {
              ...
              # $ANIKA_HOME/.env by the activation script. load_anika_dotenv()
          ...
          (lib.mkIf (!cfg.container.enable && cfg.backend.mode != "none") {
            systemd.services.anika-backend = {
          ...
          (lib.mkIf cfg.container.enable {
            ...
            systemd.services.anika-agent = {
              description = "Anika Agent Gateway (container)";
              wantedBy = [ "multi-user.target" ];
              after = [
              ...
              preStart = ''
                ...
                if [ "$NEED_CREATE" = "true" ]; then
                  ...
                  ANIKA_UID=$(${pkgs.coreutils}/bin/id -u ${cfg.user})
                  ANIKA_GID=$(${pkgs.coreutils}/bin/id -g ${cfg.user})
                  ...
                  ${containerBin} create \
                    ...
                    --env ANIKA_UID="$ANIKA_UID" \
                    --env ANIKA_GID="$ANIKA_GID" \
                    --env ANIKA_HOME=${containerDataDir}/.anika \
                    --env ANIKA_MANAGED=true \
                    --env HOME=${containerHomeDir} \
                    ${lib.concatStringsSep " " cfg.container.extraOptions} \
                    ${cfg.container.image} \
                    ${containerDataDir}/current-package/bin/anika gateway run --replace ${lib.concatStringsSep " " cfg.extraArgs}
