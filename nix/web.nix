# nix/web.nix — Anika Web Dashboard (Vite/React) frontend build
{ anikaNpmLib, ... }:
anikaNpmLib.buildNpmPackage {
  dirs = [
    "web"

    # @anika/shared ships as a file: workspace dep of web, so its source
  ...
  buildPhase = ''
    ...
    # outDir in vite.config.ts points to ../anika_cli/web_dist for the
