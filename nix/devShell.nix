...
{ ... }:
{
  perSystem =
    ...
    let
      ...
      anikaNpmLib = self'.packages.default.passthru.anikaNpmLib;
      ...
      anikaAgentDevShellHook = self'.packages.default.passthru.devShellHook;
    in
    {
      devShells.default = pkgs.mkShell {
        packages = with pkgs; [
          (pkgs.runCommand "anika" { } ''
            mkdir -p $out/bin
            install -Dm755 ${../anika} $out/bin/anika
          '')
        ...
        shellHook = ''
          ${anikaAgentDevShellHook}
          ${anikaNpmLib.mkNpmDevShellHook npmPackageJsonPaths}
          ...
          export ANIKA_PYTHON_SRC_ROOT=$(git rev-parse --show-toplevel)
          ...
          echo "Anika Agent dev shell in $ANIKA_PYTHON_SRC_ROOT"
          echo "Ready. Run 'anika' or 'sandbox anika' to start."
        '';
      };
    };
}
