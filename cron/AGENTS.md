...
schedule via the `cronjob` tool; users via `anika cron list|add|edit|pause|resume|run|remove` or
...
- File lock `~/.anika/cron/.tick.lock` prevents duplicate ticks across processes.
...
- The cron ticker runs in the desktop-spawned backend when `ANIKA_DESKTOP=1` — that env var means
...
Durable SQLite-backed board letting multiple profiles/workers collaborate. Users: `anika kanban
...
- **CLI:** `anika_cli/kanban.py` facade + 14 `kanban_*.py` siblings (`boards`, `db`, `db_connect`,
...
- **Dispatcher:** long-lived loop (default 60s) that reclaims stale claims, promotes ready tasks,
  ...
  (`kanban.dispatch_in_gateway: true`). Standalone: `plugins/kanban/systemd/anika-kanban-dispatcher.service`.
...
Isolation: **board** is the hard boundary — workers get `ANIKA_KANBAN_BOARD` pinned in their env and
...
`tests/cron/`, `tests/anika_cli/test_kanban*.py`, `tests/tools/test_kanban*.py`. Schedule parsing
