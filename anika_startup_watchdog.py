...
timeout. Config is env-only (``ANIKA_STARTUP_WATCHDOG=0``,
``ANIKA_STARTUP_WATCHDOG_TIMEOUT_S``) because config.yaml parsing is itself in scope.
...
ENV_STARTUP_WATCHDOG = "ANIKA_STARTUP_WATCHDOG"
ENV_STARTUP_WATCHDOG_TIMEOUT_S = "ANIKA_STARTUP_WATCHDOG_TIMEOUT_S"
...
# Module singleton: the arm sites (anika_cli.main / anika_cli.gateway /
...
def _process_anika_home() -> Path:
    """ANIKA_HOME for diagnostic files — stdlib-only replica of the anika_constants default."""
...
def get_startup_watchdog_dump_path(home: Optional[Path] = None) -> Path:
    """Return ``<ANIKA_HOME>/logs/gateway-startup-watchdog.log``."""
    base = home if home is not None else _process_anika_home()
    return base.joinpath(*_DUMP_RELATIVE)


def startup_watchdog_disabled() -> bool:
    """True when ``ANIKA_STARTUP_WATCHDOG`` opts out explicitly."""
...
def arm_startup_watchdog(
    ...
    *,
    ...
    ``ANIKA_STARTUP_WATCHDOG=0`` or when the thread could not be started.
    """
    global _handle
    try:
