"""Windows UTF-8 bootstrap for Anika entry points (no-op on POSIX).
...
point (``anika``, ``anika-agent``, ``anika-acp``, ``gateway.run``, ``batch_runner``,
...
def suppress_platform_ver_console() -> None:
    ...
    Mirrors ``anika_cli._subprocess_compat.suppress_platform_ver_console`` for callers
    that never import ``anika_cli.main``; double application is harmless.
    """
    if not _IS_WINDOWS:
        return
    try:
...
def harden_import_path(src_root: str | None = None) -> None:
    """Stop a package in the current directory from shadowing Anika modules.

    Anika ships top-level modules with common names (``utils``, ``proxy``, ``ui``); a
    ...
    dropped and the Anika root is *relocated* to the front, not merely inserted when
    ...
    """
    root = src_root or os.environ.get("ANIKA_PYTHON_SRC_ROOT") or os.path.dirname(
...
def activate_durable_lazy_target() -> None:
    """Put the durable lazy-install dir (``ANIKA_LAZY_INSTALL_TARGET``) on ``sys.path``.
    ...
    """
    if not os.environ.get("ANIKA_LAZY_INSTALL_TARGET", "").strip():
        return
    try:
...
# Apply on import — entry points only need ``import anika_bootstrap`` first.
