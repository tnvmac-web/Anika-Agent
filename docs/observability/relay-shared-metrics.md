...
Anika includes NeMo Relay as a normal runtime dependency on platforms for
which Relay publishes a native wheel. The shared-metrics integration is built
into Anika and does not require a Anika observability plugin. Anika remains
importable without Relay on other native targets. Those targets use an
explicit reduced-capability no-op host:
Anika execution remains available, while Relay scopes, middleware, plugins,
and subscribers are unavailable. The `anika-agent[nemo-relay]` extra remains
as a no-op compatibility alias for existing installation commands.

> [!WARNING]
> This removes the Anika `observability/nemo_relay` plugin. Existing users
...
> `plugins.toml` selected with `ANIKA_NEMO_RELAY_PLUGINS_TOML`. The legacy
> `ANIKA_NEMO_RELAY_ATOF_*` and `ANIKA_NEMO_RELAY_ATIF_*` variables no
> longer activate exporters. Without the new variable, Anika does not run
> Relay plugin discovery, configuration layering, middleware, or exporters.

Anika requires NeMo Relay 0.8.3 or later within the 0.8 release line. That
line provides the provider-codec and canonical tool-result contracts Anika
uses for managed provider and tool calls.

## Runtime Dependency and Data Boundary

Anika installs the platform-specific `nemo-relay` native wheel from the
...
through that native module in the Anika process so configured interceptors can
...
Collection remains off unless Anika policy enables it:
...
opt-in. Set `ANIKA_NEMO_RELAY_PLUGINS_TOML` to a selected `plugins.toml` to
activate configured middleware, exporters, or dynamic plugins. When the
variable is unset, Anika does not invoke Relay's plugin initializer, so Relay
...
selected file cannot be loaded, Anika reports the error and does not invoke
...
retain the same `session_id` and add `anika.session.segment` plus
`anika.session.segment_reason` (`compaction` or `max_turns`).

## Process-Wide Plugin Policy and Profile Isolation

Relay plugin configuration is a process-level deployment choice, not a Anika
profile setting. The first hosted profile triggers lazy initialization, and
every additional profile hosted by that Anika process shares the resulting
static middleware, dynamic plugins, subscribers, exporters, and guardrail
policy. After initialization succeeds, Anika logs:
...
profile's Relay scope stack. Native dynamic plugins are loaded into the Anika
process and share the same policy boundary.

Run profiles in separate Anika processes when they require different trust
...
Anika core owns one Relay host and one isolated Relay session scope per Anika
...
marks do not require Anika plugin registration. Shared-metrics marks must
...
```text
Anika turn, API, tool, and approval hooks
  -> Relay session, task, LLM, tool, and mark lifecycle
  -> Anika shared-metrics subscriber
  -> SQLite counters
  -> immutable JSON delta package
```

Anika sends an empty `LLMRequest` into the metrics-owned lifecycle. This does
...
provider route that Anika used for the logical call, such as
...
New calls use `anika.model_route.count`. The previous
`anika.model_call.count` contract remains readable only so pending local
counters created by older builds can be exported without losing data.

The first consented session start emits an empty `anika.client.active` Relay
...
counter in any rolling 24-hour window. The metric has no dimensions; Anika
version, OS family, architecture, and install method remain bounded package
resources. Concurrent Anika processes share the SQLite latch, so simultaneous
...
Each task run is a Relay `Function` scope named `anika.task_run`, parented to
the owning Anika session. The start counter contains only bounded execution
...
provider attempts for the same Anika API request ID; they do not inflate the
logical model-call count. Tool calls are deduplicated by their Anika tool-call
...
cancellations. Active task ownership follows the task ID if Anika rotates its
conversation session during context compression.

Each tool invocation is represented by a Relay tool lifecycle named
`anika.tool_call`. The terminal counter contains only bounded tool category,
outcome, approval outcome, latency, and explicit retry-count buckets. Anika
...
tool or plugin names. Anika does not infer retries from repeated tool names or
adjacent calls; when the
hook does not provide an explicit retry relationship, the retry bucket is
`unknown`. Approval decisions are emitted as `anika.tool_approval` marks and
...
Successful skill mutations emit `anika.skill.lifecycle` marks with only a
bounded action and provenance. Successful loads emit `anika.skill.load`
marks with bounded provenance, first-use or reuse state, reuse-after-patch
state, and a use-count bucket. Anika derives reuse and patch-generation
...
```text
$ANIKA_HOME/telemetry/shared_metrics/metrics.sqlite3
$ANIKA_HOME/telemetry/shared_metrics/outbox/*.json
```
...
are written with atomic replacement. Each package records the Anika version,
...
schema field name, its current scope is one `ANIKA_HOME`, so it is more
...
Deleting `$ANIKA_HOME/telemetry/shared_metrics` resets the identifier together
...
The install identity is scoped to one `ANIKA_HOME`. To reset it, stop Anika
processes and remove `$ANIKA_HOME/telemetry/shared_metrics`. This deliberately
...
Run a real Anika CLI turn against the deterministic local model server:
...
`$ANIKA_HOME/telemetry/shared_metrics/outbox/` to the Anika telemetry ingest
...
telemetry:
  shared_metrics:
    ...
    send: false      # NEW: transmit to the __NEW_ORG__ telemetry service
```
...
Removing `$ANIKA_HOME/telemetry/shared_metrics` still resets local identity,
