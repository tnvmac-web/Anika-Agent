# Anika Observer Hooks

Anika observer hooks are the read-only telemetry contract for plugins that
...
They do not replace Anika' planner, model providers, memory, tool registry,
...
Anika also has a first-party NeMo Relay shared-metrics path. It uses these
...
```text
telemetry_schema_version = "anika.observer.v1"
```

Hook callbacks are fail-open. Anika catches callback exceptions, logs a
...
payload construction is gated behind `has_hook(...)`, so Anika only builds
...
The native NeMo Relay SDK integration maps Anika session, turn, LLM, and tool
