...
has to be thrown away or summarized. Anika has always done this in one batch:
...
Anika folds the single oldest un-absorbed exchange into a running summary. The
...
The in-memory splice alone isn't enough. Anika's normal session flush is
...
With `micro_compact` unset or `false` Anika behaves exactly as it always has:
...
Defaults to `$ANIKA_HOME/logs/agent.log`. It reports passes, outcome mix, net
