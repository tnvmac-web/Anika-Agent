...
Anika supports multiple gateway processes running concurrently — one per profile
...
needed. On every other profile gateway, add to `~/.anika/config.yaml`:
...
Or set the env var: `ANIKA_KANBAN_DISPATCH_IN_GATEWAY=false`
