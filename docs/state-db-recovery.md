...
`anika doctor` reports the explicit repair command.
...
repaired or restored file. Do not run `anika doctor --fix` while the gateway
is still up. Next steps:

```bash
anika gateway stop
ANIKA_HOME="$HOME/.anika" anika sessions recover --source "$HOME/.anika/state.db" --inspect-only
# if recoverable:
ANIKA_HOME="$HOME/.anika" anika sessions recover --source "$HOME/.anika/state.db" --output "$HOME/recovered-state.db"
```
...
```bash
anika gateway stop
ANIKA_HOME="$HOME/.anika" anika sessions repair --check-only
ANIKA_HOME="$HOME/.anika" anika sessions repair
```
