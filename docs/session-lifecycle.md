...
The canonical transcript store is SQLite via `SessionDB` (from `anika_state`). The
...
Counts consecutive restarts via a JSON file (`{ANIKA_HOME}/restart_counts.json`). If a
...
This prevents unwanted auto-resets after `anika update`, `anika gateway restart`,
or `/restart`.
