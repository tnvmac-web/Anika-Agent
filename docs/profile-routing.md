...
`anika config set gateway.profile_routes ...` writes).
...
| `profile` | yes | Target profile name (must exist under `~/.anika/profiles/<name>`). |
...
activates the per-profile runtime scope (per-profile `ANIKA_HOME`, secret scope, and
