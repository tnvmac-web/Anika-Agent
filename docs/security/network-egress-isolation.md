...
When running Anika inside Docker, the default `network_mode: host` gives the
...
The Anika [SECURITY.md](../../SECURITY.md) §2 defines the trust model. The
...
#
# Usage:
#   ANIKA_UID=$(id -u) ANIKA_GID=$(id -g) docker compose up -d
#
...
services:
  gateway:
    ...
    environment:
      ...
      - NO_PROXY=anika,anika-dashboard,localhost
...
docker compose exec gateway \
  curl -sf --max-time 5 http://anika-dashboard:9119/health && echo "OK: internal reachable" || echo "FAIL"
...
- [SECURITY.md](../../SECURITY.md) — Anika trust model and vulnerability reporting
