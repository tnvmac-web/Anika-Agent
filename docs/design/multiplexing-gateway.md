...
_profile_runtime_scope(profile_home)           (gateway/run.py)
   ├── set_anika_home_override(home)          config / state.db / skills /
...
- `get_secret(name)` resolves: global allowlist → active scope → fallback.
  ...
  - multiplexing **on**, no scope installed: **raises `UnscopedSecretError`**
    ...
    value.
- A small allowlist (`ANIKA_HOME`, `ANIKA_PROFILE`, proxy settings,
...
reload helper: `anika_cli.env_loader.load_anika_dotenv` skips the
...
## The ANIKA_HOME override

`anika_constants.py` holds a context-local override consulted by
`get_anika_home()` before the `ANIKA_HOME` env var. Everything that resolves
...
automatically. `get_process_anika_home()` exists for the few machine-level
assets that must not follow the override. `anika_home_key()` gives
...
`profiles_to_serve(multiplex, profile_allowlist)` in `anika_cli/profiles.py`
...
DB handles are resolved at call time through the active ANIKA_HOME override —
...
ANIKA_HOME override. Asset writes are atomic, type- and size-capped.
...
| Built-in tool registry | Built-ins are process-global; plugin-registered tools are overlaid per profile via `anika_home_key()`. |
...
- `agent/secret_scope.py`, `anika_constants.py`, `gateway/profile_routing.py`,
  `gateway/run.py` (`_profile_runtime_scope`), `anika_cli/profiles.py`
