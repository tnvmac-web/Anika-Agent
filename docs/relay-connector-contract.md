...
This document is the formal interface between the **Anika gateway** (Python,
`gateway/relay/`) and the **connector** (Node/TypeScript,
`__NEW_ORG__/gateway-gateway`). The connector implementer's first action is to
read this file.
...
- **Registration.** The gateway registers a **wake URL** at enroll/provision —
  ...
  a dashboard host). Self-hosted: `anika gateway enroll --wake-url <url>` (or
...
**per-gateway secret** (`anika gateway enroll` → connector `/relay/enroll`, or
...
(`__NEW_ORG__/gateway-gateway`); this section is the gateway-facing summary.
...
driven by the managed Portal / `anika` CLI.
