"""__NEW_ORG__ Portal Remote Spending HTTP client.
...
DEFAULT_PORTAL_BASE_URL = "https://portal.__NEW_DOMAIN__"
...
def resolve_portal_base_url(state: Optional[dict[str, Any]] = None) -> str:
    ...
    env = os.getenv("ANIKA_PORTAL_BASE_URL") or os.getenv("NOUS_PORTAL_BASE_URL")
...
def _billing_not_logged_in(exc: Optional[BaseException] = None) -> "BillingAuthError":
    ...
    err = BillingAuthError("Not logged into __NEW_ORG__ Portal — run `anika portal` to log in.", status=401, error="invalid_token")
    if exc is not None:
        err.__cause__ = exc
    return err
...
def _request(
    ...
    try:
        ...
        raise BillingError(f"Could not reach __NEW_ORG__ Portal: {exc.reason}", error="network_error") from exc
        ...
        raise BillingError("Could not reach __NEW_ORG__ Portal: timed out", error="network_error") from exc
