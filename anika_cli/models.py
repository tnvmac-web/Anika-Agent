...
``anika_cli.models.<name>`` stays the stable import/monkeypatch surface:
...
from anika_cli.urllib_security import open_credentialed_url
from anika_cli.models_catalog_static import (
    CANONICAL_PROVIDERS,
    OPENROUTER_MODELS,
...
from anika_cli.models_reasoning_caps import (
...
_ANIKA_USER_AGENT = f"anika-cli/{_ANIKA_VERSION}"
...
def _custom_provider_ssl_context(base_url: str):
    ...
    if not base_url:
        return None
    try:
        from anika_cli.config import get_custom_provider_tls_settings
...
# ---------------------------------------------------------------------------
# __NEW_ORG__ Portal free-model helpers — the Portal models endpoint is the source of truth for what is
...
def partition_nous_models_by_tier(
    ...
    """Split __NEW_ORG__ models into (selectable, unavailable): free-tier users may only select free models
...
def _pricing_profile_key() -> str:
    ...
    from anika_constants import anika_home_key

    return anika_home_key()
...
def check_nous_free_tier(*, force_fresh: bool = False, cached_only: bool = False) -> bool:
    """True only when the __NEW_ORG__ Portal user is KNOWN to be free-tier (unknown/error → False so this
...
# ---------------------------------------------------------------------------
# __NEW_ORG__ Portal recommended models — curated paid/free suggestions plus dedicated compaction (aux)
...
def _nous_recommended_disk_path() -> "Path":
    ...
    return get_anika_home() / "cache" / "nous_recommended_cache.json"
...
def fetch_nous_recommended_models(
    ...
    """
    base = (portal_base_url or "https://portal.__NEW_DOMAIN__").rstrip("/")
    now = time.monotonic()
...
def _resolve_nous_portal_url() -> str:
    ...
    try:
        from anika_cli.auth import DEFAULT_NOUS_PORTAL_URL, get_provider_auth_state

        state = get_provider_auth_state("nous") or {}
        portal = str(state.get("portal_base_url") or "").strip()
        return (portal or str(DEFAULT_NOUS_PORTAL_URL)).rstrip("/")
    except Exception:
        return "https://portal.__NEW_DOMAIN__"
...
def get_preferred_silent_default_model(provider: str = "openrouter") -> str:
    ...
    try:
        from anika_cli.model_catalog import get_default_model_from_cache
...
def _openrouter_model_supports_tools(item: Any) -> bool:
    """True when ``supported_parameters`` advertises ``tools`` (anika-agent is tool-calling-first).
    Permissive when the field is absent/malformed: some OpenRouter-compatible gateways (__NEW_ORG__ Portal,
...
# Reasoning-capability cache slots, one set per catalog (OpenRouter, __NEW_ORG__ Portal). The logic
...
def fetch_openrouter_models(
    ...
    try:
        from anika_cli.model_catalog import get_curated_openrouter_models
    ...
    for preferred_id, _ in fallback:
        ...
        if live_item is None or not _openrouter_model_supports_tools(live_item):
            continue
        # Hide models that don't advertise tool-calling support — anika-agent requires it and surfacing
...
def get_curated_nous_model_ids() -> list[str]:
    """Curated __NEW_ORG__ Portal model ids: the remote catalog manifest, else the in-repo
    ``_PROVIDER_MODELS["nous"]`` snapshot. Always a list."""
    try:
        from anika_cli.model_catalog import get_curated_nous_models
...
def fetch_ai_gateway_models(
    ...
    from anika_constants import AI_GATEWAY_BASE_URL
...
def _configured_custom_provider_ids() -> set[str]:
    ...
    try:
        ...
        from anika_cli.providers import custom_provider_slug
...
def _provider_has_credentials(pid: str) -> bool:
    try:
        from anika_cli.auth import get_auth_status, has_usable_secret
...
def list_available_providers() -> list[dict[str, str]]:
    ...
    derived from :data:`CANONICAL_PROVIDERS` (shared with ``anika model`` and ``/model``)."""
...
def _resolve_static_model_alias(
    ...
    try:
        from anika_cli.model_switch import MODEL_ALIASES
    except Exception:
        return None
...
def normalize_provider(provider: Optional[str]) -> str:
    ...
    ``anika_cli.auth.resolve_provider()`` to resolve it from credentials."""
...
def model_supports_fast_mode(model_id: Optional[str]) -> bool:
    """Return whether Anika should expose the /fast toggle for this model."""
...
def _first_exchangeable_copilot_token(raw_tokens) -> str:
    ...
    from anika_cli.copilot_auth import exchange_copilot_token, validate_copilot_token
...
def _resolve_copilot_catalog_api_key() -> str:
    ...
    def _pool_token() -> str:
        from anika_cli.auth import read_credential_pool
...
def _model_dedup_key(model_id: str) -> str:
    ...
    try:
        from anika_cli.model_search import model_alias_canonical
...
def _codex_catalog(normalized: str, force_refresh: bool) -> list[str]:
    from anika_cli.codex_models import get_codex_model_ids
...
def _nous_catalog(normalized: str, force_refresh: bool) -> Optional[list[str]]:
    try:
        from anika_cli.auth import fetch_nous_models, resolve_nous_runtime_credentials
    ...
    # Portal models still surface without a Anika release.
...
def _openai_catalog(normalized: str, force_refresh: bool) -> Optional[list[str]]:
    ...
    # entries, so intersect with the curated agentic catalog so ``/model`` matches ``anika model``.
    ...
    # pure-catalog helper (no extra live fetch) so we only accept models Anika actually ships. (#46850)
    ...
    from anika_cli.providers import is_official_openai_host
...
# every authed provider's /v1/models). One JSON file at $ANIKA_HOME/provider_models_cache.json;
...
def _provider_models_cache_path() -> Path:
    from anika_constants import get_anika_home
    return get_anika_home() / "provider_models_cache.json"
...
def _credential_fingerprint(provider: str) -> str:
    ...
    try:
        ...
        for rel in ("auth.json", "credentials.json"):
            _mtime_part(rel, get_anika_home() / rel)
    except Exception:
        pass
...
def clear_provider_models_cache(provider: Optional[str] = None) -> None:
    ...
    ``/model --refresh`` and ``anika model --refresh``."""
    try:
...
def opencode_zen_free_headers() -> dict:
    ...
    try:
        from anika_cli import __version__ as _v
    except Exception:
        _v = "0"
    return {
        "Authorization": "",
...
def probe_api_models(
    ...
    tried: list[str] = []
    headers: dict[str, str] = {"User-Agent": _ANIKA_USER_AGENT}
    if urllib.parse.urlparse(normalized).hostname == "generativelanguage.googleapis.com":
        headers["X-Goog-Api-Client"] = f"anika-agent/{_ANIKA_VERSION}"
    ...
    if isinstance(request_headers, dict):
        ...
        from anika_cli.config import normalize_extra_headers
...
_DEEPINFRA_MODELS_QUERY = "filter=true&sort_by=anika"
...
def _fetch_ai_gateway_models(timeout: float = 5.0) -> Optional[list[str]]:
    ...
    headers = {"Authorization": f"Bearer {api_key}", "User-Agent": _ANIKA_USER_AGENT}
    try:
...
_PLUGIN_COMPAT_LAZY = {
    'LMStudioLoadResult': ('anika_cli.models_local', 'LMStudioLoadResult'),
    'PROVIDER_GROUPS': ('anika_cli.models_catalog_static', 'PROVIDER_GROUPS'),
    'ProviderEntry': ('anika_cli.models_catalog_static', 'ProviderEntry'),
    'atomic_json_write': ('utils', 'atomic_json_write'),
    'base_url_host_matches': ('utils', 'base_url_host_matches'),
    'compute_sale_discount': ('anika_cli.models_pricing', 'compute_sale_discount'),
    'ensure_lmstudio_model_loaded': ('anika_cli.models_local', 'ensure_lmstudio_model_loaded'),
    'fetch_ai_gateway_pricing': ('anika_cli.models_pricing', 'fetch_ai_gateway_pricing'),
    'fetch_lmstudio_models': ('anika_cli.models_local', 'fetch_lmstudio_models'),
    'fetch_models_with_pricing': ('anika_cli.models_pricing', 'fetch_models_with_pricing'),
    'fetch_ollama_local_models': ('anika_cli.models_local', 'fetch_ollama_local_models'),
    'get_cached_nous_inference_base_url': ('anika_cli.models_pricing', 'get_cached_nous_inference_base_url'),
    'get_pricing_for_provider': ('anika_cli.models_pricing', 'get_pricing_for_provider'),
    'group_providers': ('anika_cli.models_catalog_static', 'group_providers'),
    'lmstudio_model_reasoning_options': ('anika_cli.models_local', 'lmstudio_model_reasoning_options'),
    'nous_catalog_url': ('anika_cli.models_reasoning_caps', 'nous_catalog_url'),
    'nous_model_reasoning_capabilities': ('anika_cli.models_reasoning_caps', 'nous_model_reasoning_capabilities'),
    'nous_policy_allowed_ids': ('anika_cli.models_pricing', 'nous_policy_allowed_ids'),
    'ollama_model_supports_thinking': ('anika_cli.models_local', 'ollama_model_supports_thinking'),
    'openrouter_model_reasoning_capabilities': ('anika_cli.models_reasoning_caps', 'openrouter_model_reasoning_capabilities'),
    'parse_openrouter_reasoning_capabilities': ('anika_cli.models_reasoning_caps', 'parse_openrouter_reasoning_capabilities'),
    'peek_cached_pricing': ('anika_cli.models_pricing', 'peek_cached_pricing'),
    'pricing_cache_scope': ('anika_cli.models_pricing', 'pricing_cache_scope'),
    'probe_lmstudio_models': ('anika_cli.models_local', 'probe_lmstudio_models'),
    'probe_ollama_local_models': ('anika_cli.models_local', 'probe_ollama_local_models'),
    'provider_group_for_slug': ('anika_cli.models_catalog_static', 'provider_group_for_slug'),
    'refresh_reasoning_caps_async': ('anika_cli.models_reasoning_caps', 'refresh_reasoning_caps_async'),
    'restrict_to_nous_policy': ('anika_cli.models_pricing', 'restrict_to_nous_policy'),
    'should_use_ollama_native_catalog': ('anika_cli.models_local', 'should_use_ollama_native_catalog'),
    'url_origin': ('anika_cli.urllib_security', 'url_origin'),
    'validate_requested_model': ('anika_cli.models_validate', 'validate_requested_model'),
    'warm_nous_reasoning_caps_async': ('anika_cli.models_reasoning_caps', 'warm_nous_reasoning_caps_async'),
    'warm_openrouter_reasoning_caps_async': ('anika_cli.models_reasoning_caps', 'warm_openrouter_reasoning_caps_async'),
}
