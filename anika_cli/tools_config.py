"""Unified tool configuration for Anika Agent."""
...
from anika_cli.cli_output import print_info as _print_info
from anika_cli.colors import Colors, color
from anika_cli.config import cfg_get, load_config, save_config, get_env_value
from anika_cli.nous_subscription import (
    NousSubscriptionFeatures, apply_nous_managed_defaults, get_nous_subscription_features)
from anika_cli.platforms import PLATFORMS as _PLATFORMS_REGISTRY
from anika_cli.toolset_scope import (
    _TOOLSET_PLATFORM_RESTRICTIONS, toolset_allowed_for_platform as _toolset_allowed_for_platform)
# Re-exports: keep ``anika_cli.tools_config.X`` callers and test patch targets resolving.
from anika_cli.tools_config_cua import (  # noqa: F401
...
from anika_cli.tools_config_post_setup import (  # noqa: F401
...
from anika_cli.tools_config_providers import (  # noqa: F401
...
from anika_cli.tools_config_mcp import (  # noqa: F401
...
# OFF by default for new installs (still in _ANIKA_CORE_TOOLS; the checklist won't pre-select them). x_search
...
# Config-only capabilities: provider setup in `anika tools` (TOOL_CATEGORIES) but not model toolsets — zero
...
def _xai_credentials_present() -> bool:
    ...
    try:
        from anika_cli.auth import _read_xai_oauth_tokens
...
def _get_effective_configurable_toolsets():
    ...
    try:
        from anika_cli.plugins import discover_plugins, get_plugin_toolsets
...
def _get_plugin_toolset_keys() -> set:
    ...
    try:
        ...
        from anika_cli.plugins import get_plugin_toolset_keys_nowait
...
def _checklist_toolset_keys(platform: str) -> Set[str]:
    """Toolset keys the ``anika tools`` checklist offers for ``platform`` (mirrors ``_prompt_toolset_checklist``);
...
def _platform_default_toolset(platform: str) -> str:
    """Composite toolset a platform falls back to (plugin platforms derive ``anika-<platform>``)."""
    return PLATFORMS[platform]["default_toolset"] if platform in PLATFORMS else f"anika-{platform}"
...
def _row(name: str, badge: str = "", tag: str = "", env_vars: list = (), **markers) -> dict:
    """One TOOL_CATEGORIES provider row; ``markers`` are the ``*_provider`` / ``post_setup`` / __NEW_ORG__ keys."""
    row = {"name": name}
    if badge:
        row["badge"] = badge
    if tag:
...
_LANGFUSE_PUBLIC = ("ANIKA_LANGFUSE_PUBLIC_KEY", "Langfuse public key (pk-lf-...)")
_LANGFUSE_SECRET = ("ANIKA_LANGFUSE_SECRET_KEY", "Langfuse secret key (sk-lf-...)")
...
TOOL_CATEGORIES = {
    "tts": {
        ...
        "providers": [
            ...
            _row("__NEW_ORG__ Subscription", "subscription", "Managed OpenAI TTS billed to your subscription", tts_provider="openai",
    ...
    "stt": {
        ...
        "providers": [
            ...
            _row("__NEW_ORG__ Subscription", "subscription", "Managed OpenAI transcription billed to your subscription",
    ...
    "web": {
        ...
        # non-provider firecrawl setup-flow rows live here: managed via __NEW_ORG__ subscription, and self-hosted.
        "providers": [
            {"name": "__NEW_ORG__ Subscription", "badge": "subscription", "tag": "Managed Firecrawl billed to your subscription",
    ...
    "image_gen": {
        ...
        # _plugin_image_gen_providers(). Only the managed "__NEW_ORG__ Subscription" row lives here — fal backend, distinct UX.
        "providers": [
            _row("__NEW_ORG__ Subscription", "subscription", "Managed FAL image generation billed to your subscription", **_NOUS,
                 managed_nous_feature="image_gen", override_env_vars=["FAL_KEY"], imagegen_backend="fal"),
        ],
    },
    "video_gen": {
        "name": "Video Generation", "icon": "🎬",
        # Mirrors image_gen: managed FAL video billed via the __NEW_ORG__ Portal. Plugin-backed rows (FAL BYOK, xAI, …)
        ...
        "providers": [
            _row("__NEW_ORG__ Subscription", "subscription", "Managed FAL video generation billed to your subscription", **_NOUS,
    ...
    "x_search": {
        ...
        "setup_note": (
            "Anika routes X searches through xAI's built-in x_search Responses tool for read-only public X "
    ...
    "browser": {
        ...
        # first so a fresh install's Enter lands on the free local backend (index 0), never on the paid __NEW_ORG__ row.
        ...
        "providers": [
            ...
            _row("Lightpanda", "free · local · no Chromium", "Zig headless browser spawned by Anika, text-only (no screenshots)",
            ...
            _row("__NEW_ORG__ Subscription (Browser Use cloud)", "subscription", "Managed Browser Use billed to your subscription",
    ...
    "computer_use": {
        ...
        # cua-driver reads HOME/TMPDIR from the process env; ANIKA_CUA_DRIVER_CMD selects a specific
        ...
        "providers": [
            ...
            _row("Langfuse Self-Hosted", tag="Self-hosted Langfuse instance", post_setup="langfuse",
                 env_vars=[_key(*_LANGFUSE_PUBLIC), _key(*_LANGFUSE_SECRET),
                           _key("ANIKA_LANGFUSE_BASE_URL", "Langfuse server URL (e.g. http://localhost:3000)", default="http://localhost:3000")]),
        ],
    },
}
...
def enabled_mcp_server_names(config: dict) -> Set[str]:
    ...
    try:
        from anika_cli.plugins import get_portable_mcp_server_names_nowait
...
#: "declined": saving ``anika tools`` freezes a platform's composite into an explicit list nothing adds to, so
#: a later toolset stays off forever for picker users while ``[anika-cli]`` users inherit it.
...
def _enable_recently_shipped_toolsets(enabled_toolsets: Set[str], config: dict, platform: str) -> None:
    ...
    for ts_key in sorted(_RECENTLY_SHIPPED_TOOLSETS):
        ...
        # composites (anika-acp, anika-webhook) stay narrow.
...
def _explicit_toolsets(
    ...
    ``anika-cli`` would re-enable disabled toolsets). A mixed list (``[anika-cli, spotify]``) still expands the
...
def _get_platform_tools(config: dict, platform: str, *, include_default_mcp_servers: bool = True) -> Set[str]:
    ...
    # An explicitly saved list (even a composite like ``anika-discord``) is an opt-in to the platform's
    ...
    # Plugin toolsets are first-class on a saved list: ``[anika-cli, a2a]`` must survive filtering.
    # Plugin-provided toolsets are first-class on a platform-toolsets list — explicit config like
    # ``[anika-cli, a2a]`` must survive filtering just like a built-in configurable toolset would. See
    # issue #81163.
    ...
    # above. It may arrive as a JSON-array string ("['memory']") from `anika config set` or a JSON-mode editor.
...
def _prune_toolsets_stripped_by_disabled(enabled_toolsets: Set[str], disabled_names: List[str]) -> Set[str]:
    ...
    never appear in the list. A name-only subtraction here left inspection surfaces (``anika tools
...
def _recover_platform_native_toolsets(enabled_toolsets: Set[str], platform: str, *, skip: Set[str]) -> None:
    ...
    skip = skip | {k for k in TOOLSETS if k.startswith("anika-")} | (set(_DEFAULT_OFF_TOOLSETS) - {platform})
...
def _warn_all_invalid_platform_toolsets(platform: str, explicit: list) -> None:
    """Warn once when an explicit platform list has only invalid names (``anika`` for ``anika-cli`` → no
    ...
    if named and not any(validate_toolset(t) for t in named) and platform not in _warned_invalid_platform_toolsets:
        ...
        logger.warning(
            ...
            "name(s): %s) - tools will be unavailable. Run `anika tools` "
...
def _save_platform_tools(config: dict, platform: str, enabled_toolset_keys: Set[str]):
    ...
    # names): platform defaults (anika-cli, ...) resolve to ALL tools and would silently override the user's
...
def _prompt_choice(question: str, choices: list, default: int = 0) -> int:
    ...
    from anika_cli.curses_ui import curses_radiolist
...
def _prompt_toolset_checklist(platform_label: str, enabled: Set[str], platform: str = "cli", *, force_fresh: bool = True) -> Set[str]:
    ...
    from anika_cli.curses_ui import curses_checklist
    from toolsets import resolve_toolset
...
def _configure_shared_metrics_interactive(config: dict) -> None:
    ...
    from anika_cli.setup import setup_telemetry
...
def _print_tools_summary(config: dict, enabled_platforms: List[str]) -> None:
    """``anika tools --summary``: enabled toolsets per platform, non-interactive."""
    total = len(_get_effective_configurable_toolsets())
...
def _first_install_flow(config: dict, enabled_platforms: List[str]) -> None:
    ...
    for pkey in enabled_platforms:
        ...
        for ts_key in sorted(auto_configured):
            ...
            print(color(f"  ✓ {label}: using your __NEW_ORG__ subscription defaults", Colors.GREEN))
...
def tools_command(args=None, first_install: bool = False, config: dict = None):
    """Entry point for `anika tools` / `anika setup tools`. ``first_install`` skips the menu (checklist + key
    ...
    if getattr(args, "summary", False):
        ...
        return
    print(color("⚕ Anika Tool Configuration", Colors.CYAN, Colors.BOLD))
    print(color("  Enable or disable tools per platform.", Colors.DIM))
    print(color("  Tools that need API keys will be configured when enabled.", Colors.DIM))
    print(color("  Guide: https://anika-agent.__NEW_DOMAIN__/docs/user-guide/features/tools", Colors.DIM))
    print()
    if first_install:
    ...
    print(color(f"  Tool configuration saved to {display_anika_home()}/config.yaml", Colors.DIM))
    print(color("  Changes take effect on next 'anika' or gateway restart.", Colors.DIM))
    print()
...
_PLUGIN_COMPAT_LAZY = {
    'MANAGED_FEATURE_COVERAGE_CATEGORY': ('anika_cli.nous_subscription', 'MANAGED_FEATURE_COVERAGE_CATEGORY'),
    ...
    'format_nous_portal_entitlement_message': ('anika_cli.nous_account', 'format_nous_portal_entitlement_message'),
