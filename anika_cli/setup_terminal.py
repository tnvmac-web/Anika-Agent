...
setup.py names are resolved through the module object so test patches on ``anika_cli.setup.<name>``
...
def _pip_install_vercel(package):
    """uv when Anika has one ($ANIKA_HOME/bin is never on PATH, so which() misses it and
...
def _ensure_sdk(package: str, manual_hint: str, *, show_stderr: bool = False, install=None) -> None:
    ...
    try:
        ...
        if install is None:
            from anika_cli.tools_config import _pip_install
...
def _setup_backend_local(config: dict) -> None:
    ...
    # Gateway cwd defaults to home; sudo stays off. Both configurable via `anika setup terminal`.
...
def _setup_backend_docker(config: dict) -> None:
    ...
    # Image and resource limits use defaults; tune via `anika setup terminal`.
    ...
    if _setup.prompt_yes_no("  Enable egress firewall for Docker sandboxes?", False):
        ...
        _setup.print_info(
            "Run `anika egress setup` then `anika egress start` to mint tokens and launch the proxy.")
    else:
        _setup.print_info("Skipping egress firewall. You can enable it later with `anika egress setup`.")
...
def _setup_backend_modal(config: dict) -> None:
    ...
    if managed_modal_available:
        ...
        use_managed_modal = _setup.prompt_choice(
            ...
            ["Use my __NEW_ORG__ subscription", "Use my own Modal account"], default_idx) == 0
    if use_managed_modal:
        config["terminal"]["modal_mode"] = "managed"
        _setup.print_info("Modal execution will use the managed __NEW_ORG__ gateway and bill to your subscription.")
...
def _setup_backend_vercel(config: dict) -> None:
    ...
    _setup._info("Cloud microVM sandboxes with snapshot-backed filesystem persistence.",
                 "Requires the optional SDK: pip install 'anika-agent[vercel]'")
    _ensure_sdk("vercel", "pip install 'anika-agent[vercel]'", show_stderr=True, install=_pip_install_vercel)
...
def setup_terminal_backend(config: dict):
    ...
    _setup._info("Choose where Anika runs shell commands and code.",
    ...
    # Plugin-registered backends (~/.anika/plugins/). Fail-soft: a broken plugin must not take
    # the wizard down.
