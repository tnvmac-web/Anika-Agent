"""On-demand supply-chain audit for Anika Agent installs.
...
def _discover_plugins(anika_home: Path) -> list[Component]:
    """Python deps declared by plugins under ``~/.anika/plugins``. Plugins typically don't install
...
def _discover_components(
    *, skip_venv: bool = False, skip_plugins: bool = False, skip_mcp: bool = False, anika_home: Optional[Path] = None
) -> list[Component]:
    """Discover all scannable components across the enabled sources."""
    home = anika_home or Path(get_anika_home())
...
def cmd_security_audit(args: argparse.Namespace) -> int:
    """Implementation of `anika security audit`."""
    home = Path(get_anika_home())
    ...
    components = _discover_components(anika_home=home, **skips)
    ...
    try:
        findings = run_audit(anika_home=home, components=components)
