...
All HttpOnly, ``SameSite=Lax`` unless noted, Path = proxy prefix or /: ``anika_session_at``
(access token; Max-Age = token TTL), ``anika_session_rt`` (rotating refresh token; written only
when the provider returned one, always cleared on logout/expiry), ``anika_session_provider``
(non-secret routing hint so an RT is not handed to the wrong provider), ``anika_session_pkce``
...
cookies set on such a 302, crbug 40508226), ``anika_sso_attempt`` (auto-SSO loop guard, 60 s).
...
SESSION_AT_COOKIE = "anika_session_at"
SESSION_RT_COOKIE = "anika_session_rt"
SESSION_PROVIDER_COOKIE = "anika_session_provider"
PKCE_COOKIE = "anika_session_pkce"
SSO_ATTEMPT_COOKIE = "anika_sso_attempt"
...
def _cookie_path(prefix: str) -> str:
    """``Path=/anika`` under a proxy prefix (no leak to sibling apps), else ``/``."""
...
def set_session_cookies(
    ...
    __NEW_ORG__ Portal issues a 24h rotating refresh token (anika #37247); a provider that omits it returns
