"""anika fallback — manage the fallback provider chain (tried in order when the primary fails).

Subcommands: ``list`` (default), ``add`` (same picker as `anika model`), ``remove``, ``clear``.
"""
...
def cmd_fallback_list(args) -> None:  # noqa: ARG001
    ...
    if chain is None:
        print("  Add one with:  anika fallback add\n")
        return
    print()
    ...
    print("  Docs: https://anika-agent.__NEW_DOMAIN__/docs/user-guide/features/fallback-providers\n")


def cmd_fallback_add(args) -> None:
    """Launch the same picker as `anika model`, then append the selection to the chain."""
    from anika_cli.main import _require_tty, select_provider_and_model
    from anika_cli.config import load_config, save_config
    ...
    print("\n  Adding a fallback provider.  The picker below is the same one used by\n"
          "  `anika model` — select the provider + model you want as a fallback.\n")
    ...
    print("  Run `anika fallback list` to view, or `anika fallback remove` to delete.")
...
def cmd_fallback(args) -> None:
    """Top-level dispatcher for ``anika fallback [subcommand]``."""
