"""Background keepalive for long-lived __NEW_ORG__ Portal sessions."""
...
def _timeout_seconds(value: Optional[float]) -> float:
    if value is not None:
        return float(value)
    try:
        return float(os.getenv("ANIKA_NOUS_TIMEOUT_SECONDS", "15"))
...
def _interval_seconds(value: Optional[int]) -> int:
    """Tick interval: explicit argument, then ``__NEW_ORG__.keepalive_interval_seconds`` in config.yaml,
    ...
    try:
        ...
        logger.warning(
            "Ignoring invalid __NEW_ORG__.%s=%r; using %ds",
...
def _observed_lifetime_seconds() -> Optional[int]:
    """Server-issued lifetime (seconds) of the current __NEW_ORG__ credentials; the shorter of the access
...
def _refresh_selected_pool_entry(*, min_key_ttl_seconds: int, min_access_ttl_seconds: Optional[int] = None) -> Optional[bool]:
    ...
    no usable entry; None = no __NEW_ORG__ pool.
    """
    try:
        ...
        logger.debug("__NEW_ORG__ auth keepalive: credential pool unavailable: %s", exc)
        return None
    ...
    try:
        ...
        logger.debug("__NEW_ORG__ auth keepalive: credential pool selection failed: %s", exc)
        return False
    if entry is None:
        return False
    ...
    if access_expiring or not key_usable:
        if pool.try_refresh_current() is None:
            return False
        logger.debug("__NEW_ORG__ auth keepalive: refreshed credential pool entry")
    return True
...
def refresh_nous_auth_keepalive_once(
    *, min_key_ttl_seconds: int = NOUS_INVOKE_JWT_MIN_TTL_SECONDS,
    ...
    """Refresh __NEW_ORG__ auth once if credentials are configured (pool entry first, then singleton state)."""
    ...
    try:
        ...
        logger.debug("__NEW_ORG__ auth keepalive: refreshed singleton auth state")
        return True
    except Exception as exc:
        if isinstance(exc, AuthError) and exc.relogin_required:
            logger.info("__NEW_ORG__ auth keepalive requires re-login: %s", exc)
        else:
            logger.debug("__NEW_ORG__ auth keepalive failed: %s", exc)
        return False
...
def start_nous_auth_keepalive(
    *, interval_seconds: Optional[int] = None,
    ...
    """Start the process-wide __NEW_ORG__ auth keepalive thread (idempotent; None when disabled)."""
    ...
    with _keepalive_lock:
        ...
        logger.debug("__NEW_ORG__ auth keepalive started")
        return _keepalive_thread
