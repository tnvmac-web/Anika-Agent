...
from anika_cli.auth_constants import DEFAULT_NOUS_PORTAL_URL
...
def _confirm_selection_guards(
    ...
    """
    try:
        from anika_cli.model_selection_guards import combined_message, selection_warnings
...
class _ModelPickerRows:
    """Column-aligned picker rows (name + $/Mtok + __NEW_ORG__ sale chrome).
    ...
    def __init__(
        ...
        from anika_cli.models_pricing import _format_price_per_mtok, compute_sale_discount
        self.current_model = current_model
        self.has_pricing = bool(pricing and any(pricing.get(m) for m in all_models))
        # Leave room for a leading "★ " on sale rows (__NEW_ORG__ only).
...
def _prompt_model_selection(
    ...
    *unavailable_models* render grayed out and unselectable with an upgrade link to *portal_url*.
    ...
    # Sale chrome is __NEW_ORG__ Portal-only, even if pricing.original is present for another provider.
    ...
    try:
        ...
        from anika_cli.model_search import model_search_text
        model_search_labels = []
        for mid in ordered:
    ...
    from anika_cli.curses_ui import format_radio_item_ansi
...
def _save_model_choice(model_id: str) -> None:
    ...
    from anika_cli.config import save_config, load_config
    config = load_config()
