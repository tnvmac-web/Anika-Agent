...
Mixin split out of ``cli.py``; bound onto ``AnikaCLI`` via the MRO. cli.py-internal
...
module-level functions taking ``cli`` and siblings are called as ``AnikaCLI.<name>(self, ...)``.
"""
...
def _merge_preflight_warning(cli, result, custom_providers) -> None:
    ...
    if cli.agent is None:
        return
    try:
        from anika_cli.context_switch_guard import merge_preflight_compression_warning
...
def _print_switch_summary(cli, result, old_model, *, one_turn: bool, strict_context: bool) -> None:
    ...
    from anika_cli.model_switch import format_model_for_display, resolve_display_context_length
    ...
    # Provider-aware context chain: Codex OAuth / Copilot / __NEW_ORG__ caps win over the raw
...
def _commit_model_switch(
    ...
    from cli import AnikaCLI, _cprint
    old_model = cli.model
    ...
    if not one_turn:
        AnikaCLI._persist_model_switch_to_session(cli, result)
...
def _persist_global_switch(cli, result) -> None:
    ...
    from cli import AnikaCLI, save_config_value
    AnikaCLI._clear_persisted_context_for_model_switch(cli, result)
...
def _show_model_picker(cli, ctx, force_refresh: bool) -> None:
    ...
    from cli import _cprint
    from anika_cli.inventory import build_models_payload
...
class CLIModelSwitchMixin:
    ...
    def _normalize_model_for_provider(self, resolved_provider: str) -> bool:
        ...
        if resolved_provider == "copilot":
            from anika_cli.models import copilot_model_api_mode, normalize_copilot_model_id
        ...
        if opencode_provider_family(resolved_provider) is not None:
            from anika_cli.models import normalize_opencode_model_id, opencode_model_api_mode
            return _adopt_with_mode(
    ...
    def _restore_session_model(self, session_meta: dict, *, quiet: bool = False) -> None:
        ...
        from anika_state import SessionDB as _SessionDB
    ...
    def _confirm_expensive_model_switch(self, result) -> bool:
        ...
        if warning is None:
            return True
        choices = [
            ("once", "Switch anyway", "Use this model for the current Anika session."),
    ...
    def _handle_model_picker_selection(self, persist_global: bool = False) -> None:
        ...
        if stage == "provider":
            ...
            # Curated list (same as `anika model` / gateway pickers); live catalog only when
    ...
    def _handle_model_switch(self, cmd_original: str):
        ...
        """
        from cli import _cprint
        from anika_cli.model_switch import parse_model_switch_args, resolve_persist_behavior
    ...
    def _cmd_moa(self, cmd_original: str):
        ...
        from anika_cli.moa_config import moa_usage, normalize_moa_config
