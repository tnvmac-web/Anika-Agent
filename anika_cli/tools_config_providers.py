"""Provider rows, active-provider detection, model pickers and provider-selection persistence for `anika tools`."""
...
from anika_cli.config import cfg_get, get_env_value, load_config, save_config, save_env_value
from anika_cli.nous_account import format_nous_portal_entitlement_message
from anika_cli.nous_subscription import MANAGED_FEATURE_COVERAGE_CATEGORY, NousSubscriptionFeatures
from tools.tool_backend_helpers import NOUS_MANAGED_PROVIDER, fal_key_is_configured
from utils import base_url_hostname, is_truthy_value

logger = logging.getLogger("anika_cli.tools_config")
...
def _plugin_registry(module: str):
    ...
    try:
        ...
        from anika_cli.plugins import _ensure_plugins_discovered
...
def _visible_providers(
    ...
    from anika_cli.tools_config import get_nous_subscription_features
...
def _toolset_needs_configuration_prompt(ts_key: str, config: dict, *, force_fresh: bool = False) -> bool:
    ...
    from anika_cli.tools_config import TOOL_CATEGORIES, _post_setup_already_installed, _toolset_has_keys
...
def _configure_tool_category(ts_key: str, cat: dict, config: dict, *, force_fresh: bool = True, reconfigure: bool = False):
    ...
    ``reconfigure`` ("Reconfigure an existing tool"): no setup note / skip row / __NEW_ORG__ marker, and the
    chosen provider goes through the key-update prompts instead of the new-enable prompts."""
    from anika_cli.tools_config import _prompt_choice, _provider_env_ready, get_nous_subscription_features
    ...
    # Logged-in __NEW_ORG__ users get a marker on rows included in their subscription (cost-extra vs. included).
    ...
    for p in providers:
        ...
        if not reconfigure and p.get("managed_nous_feature"):
            sub_marker = "  ★ Included with your __NEW_ORG__ subscription" if _nous_logged_in else "  ★ via __NEW_ORG__ Portal (login on select)"
...
def _detect_active_provider_index(providers: list, config: dict, *, force_fresh: bool = False) -> int:
    ...
    from anika_cli.tools_config import _provider_env_ready
...
def _pick_model_from_catalog(
    ...
    selection."""
    from anika_cli.tools_config import _cfg_section, _prompt_choice
...
def _select_into(config: dict, section: str, key: str, vendor, managed) -> dict:
    ...
    from anika_cli.tools_config import _cfg_section
...
def _write_provider_config(provider: dict, config: dict, *, managed_feature) -> None:
    ...
    if managed_feature and managed_feature not in {"web", "tts", "stt", "browser"}:
        # Managed rows without a marker above (image_gen/video_gen "__NEW_ORG__ Subscription" rows carry only
...
def _nous_provider_gate(provider: dict, config: dict, managed_feature, *, force_fresh: bool) -> bool:
    ...
    Managed Tool Gateway rows are always listed but only *activate* with paid __NEW_ORG__ Portal access —
    ...
    if managed_feature:
        from anika_cli.nous_subscription import ensure_nous_portal_access

        if not ensure_nous_portal_access(
            capability=f"{provider.get('name', 'the __NEW_ORG__ Tool Gateway')}",
            coverage_category=MANAGED_FEATURE_COVERAGE_CATEGORY.get(managed_feature)):
            _print_warning("  Not enabled — __NEW_ORG__ Portal access is required for this backend.")
            return False
        return True
    ...
    if provider.get("requires_nous_auth"):
        ...
        if not features.nous_auth_present or not entitled:
            message = format_nous_portal_entitlement_message(
                features.account_info, capability=f"{provider.get('name', '__NEW_ORG__ Subscription')}")
            _print_warning(f"  {message or '__NEW_ORG__ Subscription is only available after logging into __NEW_ORG__ Portal.'}")
            return False
    return True
...
def _show_portal_hint(provider: dict, config: dict, managed_feature, force_fresh: bool) -> bool:
    ...
    __NEW_ORG__ — a single dim hint tells them the key is avoidable via a Portal subscription."""
    from anika_cli.tools_config import TOOL_CATEGORIES, get_nous_subscription_features
...
def _configure_provider(provider: dict, config: dict, *, force_fresh: bool = True, reconfigure: bool = False):
    ...
    if not env_vars:
        ...
        if managed_feature:
            _print_info("  Requests for this tool will be billed to your __NEW_ORG__ subscription.")
    ...
    if not reconfigure and _show_portal_hint(provider, config, managed_feature, force_fresh):
        _print_info("  Available through __NEW_ORG__ Portal subscription.")
...
def _configure_vision_backend() -> None:
    ...
    Offers any authenticated provider + model (same surface as ``anika model``) or a custom endpoint
...
def _configure_vision_provider_model(config: dict, vision_cfg: dict) -> None:
    ...
    Rows come from ``build_aux_picker_rows()`` so this lists exactly what the ``anika model`` aux-task
    picker lists, including user-defined ``providers:`` / ``custom_providers:`` endpoints. Persists
    ``auxiliary.vision.provider`` + ``.model``."""
    from anika_cli.tools_config import _prompt_choice

    try:
        from anika_cli.inventory import build_aux_picker_rows, format_aux_picker_entries
    ...
    if not providers:
        _print_warning("  No authenticated providers found. Configure a provider first "
                       "with `anika model`, then re-run this.")
        return
...
def _configure_simple_requirements(ts_key: str, *, reconfigure: bool = False):
    ...
    from anika_cli.tools_config import TOOLSET_ENV_REQUIREMENTS, _toolset_has_keys, _toolset_label
