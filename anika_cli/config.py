"""Configuration management for Anika Agent: config.yaml / .env loading, saving,
validation, migration, and the ``anika config`` command."""
...
from anika_cli.default_soul import DEFAULT_SOUL_MD, is_legacy_template_soul
from anika_cli.secret_prompt import masked_secret_prompt
# Re-export from anika_constants — canonical definition lives there.
from anika_constants import get_anika_home, get_process_anika_home  # noqa: F401
...
_PARSE_FAILURE_FALLBACK_MSG = {
    ...
    "refuse-write": (
        ...
        "Fix the YAML (anika config edit) and retry.")}
...
def _warn_config_parse_failure(
    ...
    logger.warning(msg)
    try:
        sys.stderr.write(f"⚠️  anika config: {msg}\n")
        sys.stderr.flush()
    except Exception:
        pass
...
# interpreter init (PYTHON*, NODE_*: Anika restarts through them), PATH (fix tool lookup
...
# and Anika runtime-location / security-policy flags (config.yaml is the supported surface).
#
# ``ANIKA_*`` overall is NOT blocked — many integration credentials use that prefix
# (ANIKA_LANGFUSE_PUBLIC_KEY, ANIKA_SPOTIFY_CLIENT_ID, ...). The denylist is name-by-name so
...
_ENV_VAR_NAME_DENYLIST: frozenset[str] = frozenset({
    ...
    # Anika runtime location
    "ANIKA_HOME", "ANIKA_PROFILE", "ANIKA_CONFIG", "ANIKA_ENV",
    "ANIKA_CONFIG_PATH", "ANIKA_ENV_PATH",
    ...
    "ANIKA_COPILOT_ACP_COMMAND", "ANIKA_COPILOT_ACP_ARGS",
    # Security policy / approval-routing context — set via their dedicated controls only.
    "ANIKA_YOLO_MODE", "ANIKA_ACCEPT_HOOKS", "ANIKA_REDACT_SECRETS",
    "ANIKA_INTERACTIVE", "ANIKA_EXEC_ASK", "ANIKA_GATEWAY_SESSION",
    "ANIKA_CRON_SESSION", "ANIKA_SINGLE_QUERY_SESSION",
    "ANIKA_SESSION_KEY", "ANIKA_SESSION_PLATFORM"})
...
def validate_env_var_name_for_write(key: str) -> None:
    ...
    if _env_var_policy_name(key) in _ENV_VAR_NAME_DENYLIST:
        raise ValueError(
            ...
            "or Anika runtime location and security policy (ANIKA_HOME, ANIKA_YOLO_MODE, ...) "
            "cannot be persisted via the env writer. If you really need this, edit ~/.anika/.env "
            "directly.")
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    # back-compat fallback. The boolean ANIKA_TOOL_PROGRESS variant is unsupported (its only
    # consumer, the v3->4 migration, is below the v12 support floor); doctor flags it as ignored.
    "ANIKA_TOOL_PROGRESS_MODE",
    ...
    "ANIKA_LANGFUSE_ENV", "ANIKA_LANGFUSE_RELEASE", "ANIKA_LANGFUSE_SAMPLE_RATE",
    "ANIKA_LANGFUSE_MAX_CHARS", "ANIKA_LANGFUSE_CAPTURE", "ANIKA_LANGFUSE_DEBUG",
    "LANGFUSE_PUBLIC_KEY", "LANGFUSE_SECRET_KEY", "LANGFUSE_BASE_URL",
    # ACP (Agent Client Protocol) keys — profile-isolable so profiles can use different backends.
    "ANIKA_ACP_AUTH_METHOD", "ANIKA_ACP_AUTO_APPROVE", "ANIKA_COPILOT_ACP_COMMAND",
    "ANIKA_COPILOT_ACP_ARGS", "COPILOT_CLI_PATH", "COPILOT_ACP_BASE_URL"})
...
# ANIKA_MANAGED). Module-level so tests can patch it without touching /nix/store.
...
def get_managed_system() -> Optional[str]:
    ...
    Signals: ANIKA_MANAGED env var (systemd service) or a ``.managed`` marker file in
    ANIKA_HOME (NixOS activation script — interactive shells don't see the service env)."""
    marker = os.getenv("ANIKA_MANAGED", "").strip().lower() or None
    managed_marker = get_anika_home() / ".managed"
...
def is_managed() -> bool:
    """Check if Anika is running in package-manager-managed mode."""
...
_NIX_UPDATE_MSG = (
    "Update Anika through the Nix source that installed it "
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    """Detect how Anika was installed: apt/docker/nix/nixos/home-manager/git/unknown.
    Order: code-scoped ``<install tree>/.install_method`` stamp (authoritative) -> legacy
    ``$ANIKA_HOME/.install_method`` -> managed marker -> /nix/store path -> .git dir -> unknown.
    The stamp lives next to the code because ANIKA_HOME is shared data: a container and a host
    install can bind-mount the same home, so a home-scoped ``docker`` stamp would make the host
    ``anika update`` refuse to run. A legacy ``docker`` value is therefore ignored unless we are
    ...
    the code; - the published ``__NEW_ORG__/anika-agent`` image bakes a ``docker`` stamp into
    ``/opt/anika`` at build time. An unsupported manual install dropped into a container (no stamp) falls
    through to the ``.git`` checks and behaves like any off-path install. See issue #34397.
    """
    # The stamp is a property of the running code tree (parent of anika_cli/), NOT of $ANIKA_HOME,
    ...
    method = _install_method_stamp(get_anika_home() / ".install_method")
...
def _running_in_container() -> bool:
    """Import-safe wrapper around ``anika_constants.is_container``."""
    try:
...
_UPDATE_COMMAND_BY_METHOD = {
    "docker": "docker pull __NEW_ORG__/anika-agent:latest",
    "apt": "pkg upgrade anika-agent",  # "apt" == Termux APT by contract; uses Termux's `pkg`.
}
...
def recommended_update_command_for_method(method: str) -> str:
    ...
    return _UPDATE_COMMAND_BY_METHOD.get(method, "anika update")
...
# Shared by ``cmd_update`` and ``_cmd_update_check`` (anika_cli/main.py) so the wording never
# forks. The published image excludes ``.git``, so the git update path can never succeed there
# and the generic "reinstall via install.sh" fallback would install a NEW host-side Anika.
_DOCKER_UPDATE_MESSAGE = """\
✗ ``anika update`` doesn't apply inside the Docker container.

Anika Agent runs as a published image (__NEW_ORG__/anika-agent), not a
git checkout — the container has no working tree to pull into.  Update by
pulling a fresh image and restarting your container instead:

  docker pull __NEW_ORG__/anika-agent:latest
  # then restart whatever started the container, e.g.:
  docker compose up -d --force-recreate anika-agent
  # or, for ad-hoc runs, exit the current container and `docker run` again

Verify the new version after restart:
  docker run --rm __NEW_ORG__/anika-agent:latest --version

Notes:
  • If you pinned a specific tag (e.g. ``:v0.14.0``) the ``:latest`` tag
    won't move your container — pull the newer tag you actually want, or
    switch to ``:latest`` / ``:main`` for rolling updates.  See available
    tags at https://hub.docker.com/r/__NEW_ORG__/anika-agent/tags
  • Your config and session history live under ``$ANIKA_HOME`` (``/opt/data``
...
def format_docker_update_message() -> str:
    """Return the user-facing message for ``anika update`` inside Docker."""
    return _DOCKER_UPDATE_MESSAGE


def format_managed_message(action: str = "modify this Anika installation") -> str:
    """Build a user-facing error for managed installs."""
    managed_system = get_managed_system() or "a package manager"
    return (
        f"Cannot {action}: this Anika installation is managed by {managed_system}.\n"
        "Use your package manager to upgrade or reinstall Anika.")
...
def get_container_exec_info() -> Optional[dict]:
    """Read container mode metadata from ANIKA_HOME/.container-mode.
    ...
    already inside the container, or when ANIKA_DEV=1 is set. Only FileNotFoundError is
    swallowed; other errors (permissions, malformed data) propagate."""
    if os.environ.get("ANIKA_DEV") == "1":
        return None
    ...
    try:
        info = {}
        with open(get_anika_home() / ".container-mode", "r", encoding="utf-8") as f:
            for line in f:
                line = line.strip()
    ...
    return {
        ...
        "container_name": info.get("container_name", "anika-agent"),
        "exec_user": info.get("exec_user", "anika"),
        "anika_bin": info.get("anika_bin", "/data/current-package/bin/anika")}


# ---- Config paths / ANIKA_HOME skeleton ----

def get_config_path() -> Path:
    """Get the main config file path."""
    return get_anika_home() / "config.yaml"
...
def require_parseable_user_config(*, ignore_user_config: bool = False) -> None:
    ...
    states; ``--ignore-user-config`` / ANIKA_IGNORE_USER_CONFIG=1 remain authoritative."""
    if ignore_user_config or os.environ.get("ANIKA_IGNORE_USER_CONFIG") == "1":
        return
...
def _resolve_anika_uid_gid() -> tuple[Optional[int], Optional[int]]:
    """Read ANIKA_UID / ANIKA_GID (set by Docker deployments); (None, None) if unset/invalid/Windows.
    The entrypoint chowns ANIKA_HOME once, but subdirs created at runtime (``profiles/<name>/``)
    need the same chown or they land root:root and block later uid-mapped workers.

    Docker containers running Anika commonly set these to map the in-container user to a host user so
    ...
    return _env_int("ANIKA_UID"), _env_int("ANIKA_GID")


def _chown_to_anika_uid(path) -> None:
    """Chown ``path`` to ``ANIKA_UID:ANIKA_GID`` when set; EPERM/ENOENT are non-fatal (the
    ...
    :func:`ensure_anika_home` on Docker deployments. See #34107.
    """
    uid, gid = _resolve_anika_uid_gid()
...
def _secure_dir(path):
    """chmod a directory owner-only (0700) and apply ANIKA_UID/GID ownership. No-op when managed.
    ANIKA_HOME_MODE (e.g. 0701) overrides the mode so a web server can traverse ANIKA_HOME to
    a served subdirectory without directory listings.

    Also applies ``ANIKA_UID``/``ANIKA_GID``-based ownership when those env vars are set (#34107 — Docker
    ...
    if is_managed():
        return
    try:
        mode = int(os.environ.get("ANIKA_HOME_MODE", "").strip() or "700", 8)
    except ValueError:
        mode = 0o700
    try:
        os.chmod(path, mode)
        ...
        pass
    _chown_to_anika_uid(path)


def _is_container() -> bool:
    """Detect Docker/Podman/LXC (or ANIKA_CONTAINER / ANIKA_SKIP_CHMOD opt-out).
    ...
    if (os.environ.get("ANIKA_CONTAINER") or os.environ.get("ANIKA_SKIP_CHMOD")
...
_ANIKA_HOME_ENSURED: set = set()
_ANIKA_HOME_SUBDIRS = (
    "cron", "sessions", "logs", "logs/curator", "memories",
    "pairing", "hooks", "image_cache", "audio_cache", "skills")


def ensure_anika_home():
    """Ensure the ~/.anika directory skeleton exists with secure permissions.
    ...
    from anika_constants import assert_named_profile_home_live
    assert_named_profile_home_live(home)
    if key in _ANIKA_HOME_ENSURED and home.is_dir():
        return
    if is_managed():
        ...
        try:
            if not home.is_dir():
                raise RuntimeError(f"ANIKA_HOME {home} does not exist.")
        ...
        _secure_dir(home)
        for subdir in _ANIKA_HOME_SUBDIRS:
            d = home / subdir
            d.mkdir(parents=True, exist_ok=True)
            _secure_dir(d)
        _ensure_default_soul_md(home)

    _ANIKA_HOME_ENSURED.add(key)


# ---- Config loading/saving ----

from anika_cli.config_defaults import DEFAULT_CONFIG, OPTIONAL_ENV_VARS  # noqa: E402,F401
from anika_cli.config_providers import (  # noqa: E402,F401  (re-exported; callers/tests use anika_cli.config.<name>)
...
# Back-compat re-exports — :mod:`anika_cli.personality` owns personality/overlay semantics.
from anika_cli.personality import (  # noqa: E402,F401
...
def _split_key_path(key: str) -> list[str]:
    ...
    ``anika config set`` uses ``.`` as the nesting separator, so a key that itself contains a literal dot
...
def _is_env_config_key(key: str) -> bool:
    """Return whether `anika config set` routes this key to .env."""
    if "." in key:
        return False
    key_upper = key.upper()
    return (
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    try:
        ...
        # A malformed SKILL.md must never break `anika update`; this prompting is a nicety.
...
_EXTRA_KNOWN_ROOT_KEYS = {
    ...
    "plugins",           # plugin enable/disable lists (anika_cli/plugins_cmd.py)
    "smart_model_routing",   # written by the setup wizard
    "platform_toolsets",     # written by the setup wizard
    "known_plugin_toolsets", # anika_cli/tools_config.py toolset-save flow
    ...
    for _key in ("backend", "search_backend", "extract_backend"):
        ...
        if note:
            _issue(issues, "warning",
                   ...
                   "Run 'anika tools' and pick a different Web Search & Extract provider")
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            return [ConfigIssue("error", "Could not load config.yaml", "Run 'anika setup' to create a valid config")]
    ...
    if cp and not config.get("model"):
        _issue(issues, "warning",
               "custom_providers defined but no 'model' section — Anika won't know which provider to use",
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines.append("  \033[2mRun 'anika doctor' for fix suggestions.\033[0m")
    sys.stderr.write("\n".join(lines) + "\n\n")
...
def warn_deprecated_cwd_env_vars() -> None:
    ...
    if lines:
        from anika_constants import display_anika_home

        hint_path = display_anika_home()
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    from anika_cli.config_migrations import (
    ...
    if floor_refused:
        ...
        sys.stderr.write(f"⚠ anika config: {msg}\n")
        if not quiet:
            print(f"  ⚠ {msg}")
    else:
...
def _disable_suspicious_mcp_servers(results: Dict[str, Any], quiet: bool) -> None:
    ...
    if not isinstance(raw_mcp_servers, dict):
        return
    try:
        from anika_cli.mcp_security import validate_mcp_server_entry
    except Exception:
        return
    mcp_touched = False
...
def _warn_invalid_platform_toolsets(results: Dict[str, Any], quiet: bool) -> None:
    ...
    try:
        ...
        from anika_cli.toolset_validation import validate_platform_toolsets
        from anika_cli.toolset_scope import toolset_allowed_for_platform
...
def _offer_list(heading: str, items: List[str], question: str) -> bool:
    ...
    if not _ask_yes_no(question):
        print("  Set later with: anika config set <key> <value>")
        return False
    print()
    return True
...
def _env_expand_match(m: re.Match) -> str:
    ...
    if inner.startswith("env:"):
        logger.warning(
            "Config ref %r: %s is not set (check ~/.anika/.env); "
...
def _env_ref_snapshot(obj, snapshot=None):
    ...
    made against a different environment (load before ``load_anika_dotenv()``, in-process
...
def _normalize_root_model_keys(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    LiteLLM users reach for, and ``anika config set`` blindly accepts any dotted key — so
    ...
    from OpenAI-compatible backends) — while display paths (``anika status``/``dump``) read ``name`` and
...
# the ANIKA_MAX_ITERATIONS env bridge, works in every ``<``/``>=``/``max - used`` comparison in
...
def _is_ssh_remote_tilde_cwd(backend: str, cwd: str) -> bool:
    """Whether the remote SSH shell must expand *cwd* itself: ``~`` expanded on the Anika host
...
def _load_config_impl(*, want_deepcopy: bool) -> Dict[str, Any]:
    with _CONFIG_LOCK:
        ...
        if cached is not None and cache_sig is not None and cached[:4] == cache_sig:
            ...
            # load_anika_dotenv() pins unexpanded literals for the process lifetime.
            # Without this, a load_config() that ran before load_anika_dotenv() pins unexpanded literals
...
#   openai-codex (OAuth — anika auth) — OpenAI Codex
#   nous         (OAuth — anika auth) — __NEW_ORG__ Portal
...
def save_config(
    ...
    """Save configuration to ~/.anika/config.yaml.
...
def _parse_env_value(raw_value: str) -> str:
    """Parse the small .env value subset Anika writes itself (bare, 'single', or "double" with
    ``\\"`` / ``\\\\`` escapes)."""
...
def load_env() -> Dict[str, str]:
    """Load ~/.anika/.env as a dict (memoised; ``get_env_value()`` runs hundreds of times per
...
def sanitize_env_file() -> int:
    """Rewrite ~/.anika/.env with normalized line formatting; returns the number of changed lines."""
    env_path = get_env_path()
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.anika/.env (also matching ``export KEY=`` lines, so a save
...
def custom_endpoint_key_env(identity: str) -> str:
    ...
    so two endpoints on one host get separate slots. The fixed ``ANIKA_CUSTOM_`` prefix keeps the
    ...
    return f"ANIKA_CUSTOM_{slug}_API_KEY" if slug else "ANIKA_CUSTOM_API_KEY"


def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.anika/.env and os.environ; True if it was found and removed."""
...
def save_env_value_secure(key: str, value: str) -> Dict[str, Any]:
    ...
    from anika_cli.credential_lifecycle import save_provider_env_credential
...
def reload_env() -> int:
    """Re-read ~/.anika/.env into os.environ; returns count of vars changed.
    Removes deleted vars only when known to Anika (OPTIONAL_ENV_VARS and _EXTRA_ENV_KEYS) so
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ``os.environ`` (scope-aware) or ``~/.anika/.env``.
...
def get_env_value_prefer_dotenv(key: str) -> Optional[str]:
    """Resolve a Anika-managed credential preferring ``~/.anika/.env`` over ``os.environ``, so a
...
def _show_model_section(config: Dict[str, Any]) -> None:
    ...
    # Read the .env FILE directly so a stale ANIKA_MAX_ITERATIONS ghost is caught even when the
    # gateway bridge already overrode os.environ.
    try:
        env_ghost = load_env().get("ANIKA_MAX_ITERATIONS")
    except Exception:
        env_ghost = None
    if env_ghost is not None and str(env_ghost).strip() != str(cfg_max_turns).strip():
        print(color(f"                ⚠ .env has stale ANIKA_MAX_ITERATIONS={env_ghost} "
                    f"(run 'anika doctor --fix' to remove)", Colors.YELLOW))
...
def _show_display_section(config: Dict[str, Any]) -> None:
    ...
    try:
        from anika_cli.personality import active_personality_name
...
def show_config():
    ...
    print(color("│              ⚕ Anika Configuration                    │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    from anika_cli.auth import get_anthropic_key
    ...
    print(color("  anika config edit     # Edit config file", Colors.DIM))
    print(color("  anika config set <key> <value>", Colors.DIM))
    print(color("  anika setup           # Run setup wizard", Colors.DIM))
    print()
...
def resolve_cron_model_drift_defaults(
    ...
    Mirrors the scheduler's precedence: a truthy configured model wins over ``ANIKA_MODEL``; the
    ...
    return provider, configured_model or _model_assignment_text(env.get("ANIKA_MODEL", ""))
...
def warn_unpinned_cron_jobs_after_model_config_change(
    ...
    print(
        ...
        "model/provider. Inspect with `anika cron list`, then pin the intended values with "
        "`anika cron edit <job_id> --provider <provider> --model <model>`.")
...
def _coerce_config_set_value(key: str, value: str) -> Any:
    """Auto-coerce a ``anika config set`` string to bool/None/int/float/list/dict.
...
def _redirect_platform_display_key(key: str) -> tuple[str, Optional[str]]:
    ...
    Before #71047 a write such as ``anika config set platforms.telegram.streaming false`` landed on a key
...
def _guard_section_overwrite(key: str, value: Any, user_config: Dict[str, Any], force: bool) -> str:
    ...
    err += [
        ...
        f"    anika config set {key}.<sub-key> <value>",
        "  Or use --force to replace the entire section:",
        f"    anika config set --force {key} {value!r}"]
...
def _touch_skin_file(key: str, value: Any) -> None:
    ...
    if key == "display.skin" and isinstance(value, str) and value:
        try:
            skin_file = get_anika_home() / "skins" / f"{value}.yaml"
            if skin_file.exists():
                skin_file.touch()
        except Exception:
            pass
...
def _print_unknown_key_notice(key: str, suggestion: Optional[str]) -> None:
    print(color(
        ...
        "but Anika may not read it.", Colors.YELLOW))
    if suggestion:
...
def set_config_value(key: str, value: str, force: bool = False):
    ...
    if key.strip().lower() in ("model.api_base", "api_base"):
        # Normalize the api_base → base_url alias at set-time too (issue #8919), so a fresh `anika config
...
def unset_config_value(key: str):
    ...
    if _is_env_config_key(key):
        ...
        # See #51071.
        from anika_cli.credential_lifecycle import remove_provider_env_credential
...
_USAGE_GET = ("Usage: anika config get <key> [--json]", [
    "anika config get model", "anika config get terminal.backend",
    "anika config get skills.config --json"], None)
_USAGE_SET = ("Usage: anika config set [--force] <key> <value>", [
    "anika config set model anthropic/claude-sonnet-4", "anika config set terminal.backend docker",
    "anika config set OPENROUTER_API_KEY sk-or-..."], [
    "", "  --force: skip the unknown-key notice for unrecognized keys,",
    "           and allow a scalar to replace a whole mapping section"])
_USAGE_UNSET = ("Usage: anika config unset <key>", [
    "anika config unset model", "anika config unset terminal.backend",
    "anika config unset OPENROUTER_API_KEY"], None)
...
def _cmd_config_check(args):
    ...
    if missing_config:
        ...
        print("    Run 'anika config migrate' to add them")
...
_CONFIG_USAGE = """Available commands:
  anika config           Show current configuration
  anika config edit      Open config in editor
  anika config get <key>          Print a resolved config value
  anika config set <key> <value>   Set a config value
  anika config unset <key>        Remove a config value
  anika config check     Check for missing/outdated config
  anika config migrate   Update config with new options
  anika config path      Show config file path
  anika config env-path  Show .env file path"""
...
def _inject_platform_plugin_env_vars() -> None:
    ...
    Chat etc. are configurable in ``anika config`` UI without the core knowing they exist.
...
def _install_method_project_root(project_root: Optional[Path] = None) -> Path:
    ...
    This is the parent of ``anika_cli/`` — i.e. the git checkout for source
    installs, ``/opt/anika`` inside the published image. It is a property of
    the running interpreter, NOT of ``$ANIKA_HOME``, which is why a
...
def stamp_install_method(method: str, project_root: Optional[Path] = None) -> None:
    ...
    not in ``$ANIKA_HOME``, so that two installs sharing one data directory
    ...
    ``/opt/anika`` in the published image, which instead bakes the stamp at
...
_PLUGIN_COMPAT_LAZY = {
    'normalize_route_base_url': ('anika_cli.route_identity', 'normalize_route_base_url'),
}
