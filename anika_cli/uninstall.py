...
def _strip_anika_path_lines(content: str) -> str:
    """Drop the ``# Anika Agent`` marker (+ its PATH line) and any anika PATH line; squash blank runs."""
    new_lines = []
    skip_next = False
    for line in content.split('\n'):
        if '# Anika Agent' in line or '# anika-agent' in line:
            skip_next = True
            continue
        if skip_next and ('anika' in line.lower() and 'PATH' in line):
            skip_next = False
            continue
        skip_next = False
        if 'anika' in line.lower() and ('PATH=' in line or 'path=' in line.lower()):
            continue
...
def remove_path_from_shell_configs():
    """Remove Anika PATH entries from shell configuration files."""
    removed_from = []
    for config_path in (c for c in (Path.home() / n for n in _SHELL_RC_NAMES) if c.exists()):
        try:
            content = config_path.read_text(encoding="utf-8")
            new_content = _strip_anika_path_lines(content)
...
def remove_wrapper_script():
    """Remove the anika wrapper script if it exists."""
    def _unlink_ours(wrapper: Path) -> bool:
        # Only our wrapper (contains a anika_cli / anika-agent reference).
        content = wrapper.read_text(encoding="utf-8")
        return _unlink_if('anika_cli' in content or 'anika-agent' in content, wrapper)

    candidates = (
        bin_dir / name
        for bin_dir in (Path.home() / ".local" / "bin", Path("/usr/local/bin"))
        for name in ("anika", "anika-acp", "anika-agent"))
...
def remove_node_symlinks(anika_home: Path) -> list:
    ...
    node_dir = (anika_home / "node").resolve()
...
def uninstall_gateway_service():
    ...
    try:
        from anika_cli.gateway import kill_gateway_processes, find_gateway_pids
...
def _remove_systemd_gateway() -> bool:
    ...
    from anika_cli.gateway import _systemctl_cmd, get_service_name, get_systemd_unit_path
...
def _remove_launchd_gateway() -> bool:
    ...
    from anika_cli.gateway import get_launchd_plist_path
...
def _remove_windows_gateway() -> bool:
    ...
    from anika_cli import gateway_windows as gw
...
# ANIKA_HOME / ANIKA_GIT_BASH_PATH (HKCU\Environment), User-scope PATH entries
# (%LOCALAPPDATA%\anika\git\{cmd,bin,usr\bin}, ...\anika\node), PortableGit + Node copies
...
def _anika_path_markers(anika_home: Path, *, include_managed_bin: bool = False) -> list[str]:
    """Prefixes identifying Anika-owned User-PATH entries (prefix match sweeps git\cmd, git\bin,
    ...
    root = str(anika_home).rstrip("\\/")
    subs = ("anika-agent", "git", "node", "venv") + (("bin",) if include_managed_bin else ())
    return [f"{root}\\{sub}" for sub in subs]


def remove_path_from_windows_registry(anika_home: Path, *, include_managed_bin: bool = False) -> list[str]:
    """Strip Anika-owned entries from User-scope PATH in the registry (see ``_anika_path_markers``)."""
    markers = tuple(m.lower() for m in _anika_path_markers(anika_home, include_managed_bin=include_managed_bin))
...
def remove_anika_env_vars_windows() -> list[str]:
    """Delete ANIKA_HOME and ANIKA_GIT_BASH_PATH from User-scope env vars."""
    def edit(winreg, key, removed):
        for name in ("ANIKA_HOME", "ANIKA_GIT_BASH_PATH"):
            try:
...
def remove_portable_tooling_windows(anika_home: Path) -> list[Path]:
    """Delete the PortableGit / Node / gateway-service dirs the Windows installer created under
    ``anika_home`` (isolated from any system Git / Node, so nothing else breaks)."""
    targets = (anika_home / sub for sub in ("git", "node", "gateway-service"))
...
def remove_windows_bin_launchers(*, windows: bool | None = None) -> list[Path]:
    """Delete the ``anika`` launchers install.ps1 staged in the managed ``bin`` dir: every mode
    ...
    try:
        ...
        from anika_cli._install_repair import _WINDOWS_BIN_LAUNCHERS
        from anika_constants import get_default_anika_root
        bin_dir = get_default_anika_root() / "bin"
    except Exception as e:
...
def _is_default_anika_home(anika_home: Path) -> bool:
    """Return True when ``anika_home`` points at the default (non-profile) root."""
    try:
        from anika_constants import get_default_anika_root
        return anika_home.resolve() == get_default_anika_root().resolve()
    except Exception:
        return False
...
def _uninstall_profile(profile) -> None:
    ...
    out to ``anika -p <name> gateway stop|uninstall`` because service names / unit paths derive
    from the current ANIKA_HOME and can't be switched in-process."""
    name = profile.name
    log_info(f"Uninstalling profile '{name}'...")

    # 1. Gateway service, via `python -m anika_cli.main` (the `anika` wrapper may be half-gone).
    anika_invocation = [sys.executable, "-m", "anika_cli.main", "--profile", name]
    for subcmd in ("stop", "uninstall"):
        try:
            subprocess.run(
                anika_invocation + ["gateway", subcmd], capture_output=True, text=True,
    ...
    # 3. Wipe the profile's ANIKA_HOME directory.
    _rmtree_step(profile.path, indent="  ", fully=False)


def run_gui_uninstall(args):
    """``anika uninstall --gui``: remove the desktop app's built artifacts, packaged bundle
    (best-effort) and Electron userData — never config/sessions/.env, the agent or its venv."""
    from anika_cli.gui_uninstall import agent_is_installed, gui_install_summary, uninstall_gui
    anika_home = get_anika_home()
    summary = gui_install_summary(anika_home)
    skip_confirm = bool(getattr(args, "yes", False))

    print()
    _print_box("│         ⚕ Anika Chat GUI Uninstaller                  │", Colors.MAGENTA)
    print()

    if not summary["gui_installed"]:
        print("No Anika Chat GUI installation was found.")
        print(f"  Checked: {anika_home}, and the standard app locations for this OS.")
        return

    print(color("This removes the Chat GUI only. The Anika agent stays installed.", Colors.CYAN))
    print()
    ...
    print()
    if agent_is_installed(anika_home):
        print(color("Kept intact:", Colors.GREEN, Colors.BOLD))
        print(f"  • The Anika agent at {anika_home / 'anika-agent'}")
        print(f"  • Your config, sessions, and secrets under {anika_home}")
        print()
    ...
    print()
    uninstall_gui(anika_home)

    print()
    _print_box("│            ✓ Chat GUI Uninstalled!                      │", Colors.GREEN)
    print()
    print("The Anika agent is still installed. Run 'anika' to use the CLI,")
    print("or 'anika uninstall' to remove the agent too.")
    print()


def run_uninstall(args):
    """Interactive/``--yes`` uninstall: full removes code and ``~/.anika/``; keep-data removes only
    ...
    if bool(getattr(args, "dry_run", False)):
        _print_uninstall_dry_run(
            project_root=project_root, anika_home=anika_home, full_uninstall=full_flag)
        return

    # Named profiles (only when uninstalling from the default root) are offered for cleanup too,
    # instead of leaving zombie ANIKA_HOMEs and systemd units behind.
    is_default_profile = _is_default_anika_home(anika_home)
    ...
    print()
    _print_box("│            ⚕ Anika Agent Uninstaller                  │", Colors.MAGENTA)
    print()

    # Show what will be affected
    print(color("Current Installation:", Colors.CYAN, Colors.BOLD))
    print(f"  Code:    {project_root}")
    print(f"  Config:  {anika_home / 'config.yaml'}")
    print(f"  Secrets: {anika_home / '.env'}")
    print(f"  Data:    {anika_home / 'cron/'}, {anika_home / 'sessions/'}, {anika_home / 'logs/'}")
    print()
    ...
    # services, alias wrappers, ANIKA_HOME dirs) — otherwise they leave zombie services behind.
    ...
    print()
    if full_uninstall:
        print(color("⚠️  WARNING: This will permanently delete ALL Anika data!", Colors.RED, Colors.BOLD))
        print(color("   Including: configs, API keys, sessions, scheduled jobs, logs", Colors.RED))
        if remove_profiles:
            print(color(f"   Plus {n_profiles} profile(s): {profile_names}", Colors.RED))
    else:
        print("This will remove the Anika code but keep your configuration and data.")
    ...
    _perform_uninstall(
        project_root=project_root, anika_home=anika_home, full_uninstall=full_uninstall,
        remove_profiles=remove_profiles, named_profiles=named_profiles)


def _print_uninstall_dry_run(*, project_root: Path, anika_home: Path, full_uninstall: bool) -> None:
    ...
    print("  • Anika PATH entries from shell configs / Windows User PATH")
    print("  • Anika wrapper scripts and Anika-managed node/npm/npx symlinks")
    print("  • Desktop Chat GUI artifacts")
    print(f"  • Code checkout: {project_root}")
    if not full_uninstall:
        print(f"  • Keep Anika config/data: {anika_home}")
    else:
        print(f"  • Anika config/data: {anika_home}")
        profiles = _discover_named_profiles() if _is_default_anika_home(anika_home) else []
        if profiles:
...
def _perform_uninstall(
    *,
    ...
    the checkout -> (Windows) PortableGit/Node -> optionally ``$ANIKA_HOME`` and named profiles."""
    print()
    ...
    # 2-3b. PATH entries, wrapper, Windows launchers, node symlinks. Windows: anika_home is
    #    %VAR%-expanded because install.ps1 writes literal C:\Users\<u>\...; anika\bin (launchers +
    ...
    windows = _is_windows()
    sweep_managed_bin = windows and full_uninstall and _is_default_anika_home(anika_home)
    for on_this_platform, label, remove, success_fmt, none_msg in (
        ...
        (windows, "Removing PATH entries from Windows User environment...",
         lambda: remove_path_from_windows_registry(
             Path(os.path.expandvars(str(anika_home))), include_managed_bin=sweep_managed_bin),
         "Removed from User PATH: {}", "No Anika-owned PATH entries in User environment"),
        (windows, "Removing ANIKA_HOME / ANIKA_GIT_BASH_PATH User env vars...",
         remove_anika_env_vars_windows, "Removed User env var: {}", "No Anika-set User env vars to remove"),
        (True, "Removing anika command...", remove_wrapper_script, "Removed {}", "No wrapper script found"),
        (windows, "Removing Windows anika launchers...",
         remove_windows_bin_launchers, "Removed {}", "No Windows anika launchers found"),
        (True, "Removing Anika-managed node/npm/npx symlinks...",
         lambda: remove_node_symlinks(anika_home), "Removed {}", "No Anika-managed node/npm/npx symlinks found"),
    ):
        if on_this_platform:
    ...
    #     .env (safe in keep-data mode); the packaged app + Electron userData live OUTSIDE ANIKA_HOME.
    log_info("Removing desktop Chat GUI artifacts...")
    try:
        from anika_cli.gui_uninstall import uninstall_gui
        if not uninstall_gui(anika_home):
    ...
    if windows:
        _remove_step(
            ...
            lambda: remove_portable_tooling_windows(anika_home), "Removed {}",
            "No Windows installer artifacts to remove")

    # 5. Optionally remove ~/.anika/ data directory (and named profiles)
    if full_uninstall:
        ...
        _rmtree_step(anika_home)
    else:
        log_info(f"Keeping configuration and data in {anika_home}")
    ...
    print()
    print("Thank you for using Anika Agent! ⚕")
    print()


_REINSTALL_HINT = {
    True: "  iex (irm https://anika-agent.__NEW_DOMAIN__/install.ps1)",
    False: "  curl -fsSL https://anika-agent.__NEW_DOMAIN__/install.sh | bash"}
...
def main(argv=None) -> int:
    """``python -m anika_cli.uninstall --mode <gui|lite|full>``. Imports only stdlib +
    ``anika_constants`` + ``anika_cli.colors``, so it runs under a bare system Python (no venv)."""
    import argparse
    parser = argparse.ArgumentParser(prog="python -m anika_cli.uninstall")
    parser.add_argument(
