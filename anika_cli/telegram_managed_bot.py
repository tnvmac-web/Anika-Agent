"""Telegram Managed Bot onboarding client: creates a user-owned child bot via the __NEW_ORG__ onboarding
...
DEFAULT_API_URL = "https://setup.anika-agent.__NEW_DOMAIN__"
TELEGRAM_ONBOARDING_URL_ENV = "TELEGRAM_ONBOARDING_URL"
DEFAULT_BOT_NAME = "Anika Agent"
...
def auto_setup_telegram_bot_result(
    api_url: str | None = None, manager_bot: str = "AnikaSetupBot",
    ...
    print(f"\n  Contacting Anika Telegram onboarding service: {resolved_api_url}")
    sys.stdout.flush()
    pairing = create_pairing(resolved_api_url)
    if not pairing:
        print("  ✗ Could not reach the Anika Telegram onboarding service.\n"
...
DEFAULT_MANAGER_BOT = "AnikaSetupBot"
...
def generate_username_slug(length: int = 16) -> str:
    ...
    keeping ``anika_<slug>_bot`` under Telegram's 32-character username limit.
    """
...
def generate_bot_username(profile_name: Optional[str] = None) -> str:
    """Generate a secure suggested bot username like ``anika_<slug>_bot``.
    ...
    """
    _ = profile_name
    return f"anika_{generate_username_slug()}_bot"
