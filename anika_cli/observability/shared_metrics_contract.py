"""Bounded product contract for the first Anika shared-metrics slice."""
...
SCHEMA_KEY = "anika.metrics.schema_version"
SCHEMA_VERSION = "anika.metrics.event.v2"
MODEL_CALL_SCOPE = "anika.model_call"
MODEL_CALL_PROFILE_MODEL = "unknown"
TASK_SCOPE = "anika.task_run"
TOOL_CALL_SCOPE = "anika.tool_call"
CLIENT_ACTIVE_MARK = "anika.client.active"
TOOL_APPROVAL_MARK = "anika.tool_approval"
SKILL_LIFECYCLE_MARK = "anika.skill.lifecycle"
SKILL_LOAD_MARK = "anika.skill.load"
SUBSCRIBER_NAME = "anika.nemo_relay.shared_metrics"
CLIENT_ACTIVE_METRIC = "anika.client.active"
LEGACY_MODEL_CALL_METRIC = "anika.model_call.count"
MODEL_ROUTE_METRIC = "anika.model_route.count"
TASK_STARTED_METRIC = "anika.task_run.started"
TASK_FINISHED_METRIC = "anika.task_run.finished"
TOOL_CALL_METRIC = "anika.tool_call.count"
TOOL_APPROVAL_METRIC = "anika.tool_approval.count"
SKILL_LIFECYCLE_METRIC = "anika.skill.lifecycle.count"
SKILL_LOAD_METRIC = "anika.skill.load.count"
...
CLIENT_RESOURCE_KEYS = frozenset({"architecture", "anika_version", "install_method", "os_family"})
...
def client_install_method(value: Any) -> str:
    """Return an allowlisted Anika installation method."""
    normalized = _norm(value)
    return _allowlisted("nixos" if normalized == "nix" else normalized, CLIENT_INSTALL_METHODS)


def client_resource(
    anika_version: Any, *, os_name: Any, architecture: Any, install_method: Any
) -> dict[str, str]:
    """Build the bounded client resource attached to aggregate packages."""
    version = str(anika_version or "").strip()
    return {
        "architecture": client_architecture(architecture),
        "anika_version": version if 0 < len(version) <= 64 else "unknown",
...
def client_resource_is_valid(resource: Any) -> bool:
    ...
    if not isinstance(resource, dict) or set(resource) != CLIENT_RESOURCE_KEYS:
        return False
    version = resource.get("anika_version")
    return (
...
def _auxiliary_model_call_dimensions(event: Any) -> dict[str, str] | None:
    """Project a terminal auxiliary route from its Anika logical scope."""
    metadata = _relay_metadata(
        event, RUNTIME_SCHEMA_KEY, RUNTIME_SCHEMA_VERSION, "anika.call_role"
    )
    call_role = (metadata or {}).get("anika.call_role")
...
def execution_surface(kwargs: dict[str, Any]) -> str:
    ...
    try:
        from anika_cli.platforms import get_all_platforms
...
def task_terminal_state(kwargs: dict[str, Any]) -> tuple[str, str, str]:
    """Map Anika terminal state to bounded (outcome, end_reason, termination)."""
...
_TOOL_CATEGORY_PREFIXES = (
    ...
    (("discord", "email", "feishu", "anika-yuanbao", "slack", "sms"), "communication"),
)


def tool_category(kwargs: dict[str, Any]) -> str:
    """Map Anika registry toolset metadata to a low-cardinality category."""
...
def tool_outcome(kwargs: dict[str, Any]) -> str:
    """Normalize the terminal Anika tool status without inspecting its result."""
...
def model_call_fields(kwargs: dict[str, Any]) -> dict[str, str]:
    """Return the terminal model identity and provider route known to Anika."""
