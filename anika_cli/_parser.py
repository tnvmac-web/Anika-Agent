"""Top-level argparse construction for the anika CLI.

Only the top-level parser and the ``chat`` subparser live here. Every other subparser (model,
gateway, sessions, …) is built by ``anika_cli/subcommands/<group>.py`` and wired in
...
# (it sets ``ANIKA_HOME`` and strips itself from ``sys.argv``), so it isn't on the parser.
...
# live parser fails. The derived path is authoritative; tests/anika_cli/
...
def top_level_value_flag_sets() -> tuple[frozenset[str], frozenset[str]]:
    ...
    argparse surface — the drift that made ``anika --reasoning high chat …`` misread ``high`` as
...
def _inherited_flag(parser, *args, **kwargs):
    """``parser.add_argument`` + tag the Action ``inherit_on_relaunch`` for ``anika_cli.relaunch``."""
...
_EPILOGUE = """
Examples:
    anika                        Start interactive chat
    anika chat -q "Hello"        Single query mode
    anika --tui                  Launch the modern TUI (or set display.interface: tui)
    anika --cli                  Force the classic REPL (overrides display.interface: tui)
    anika -c                     Resume the most recent session
    anika -c "my project"        Resume a session by name (latest in lineage)
    anika --resume <session_id>  Resume a specific session by ID
    anika --resume latest        Resume the most recent session (same as -c)
    anika --tui --resume latest --in ./dir   Resume ./dir's latest session in the TUI
    anika setup                  Run setup wizard
    anika logout                 Clear stored authentication
    anika auth add <provider>    Add a pooled credential
    anika auth list              List pooled credentials
    anika auth remove <p> <t>    Remove pooled credential by index, id, or label
    anika auth reset <provider>  Clear exhaustion status for a provider
    anika model                  Select default model
    anika fallback [list]        Show fallback provider chain
    anika fallback add           Add a fallback provider (same picker as `anika model`)
    anika fallback remove        Remove a fallback provider from the chain
    anika config                 View configuration
    anika config edit            Edit config in $EDITOR
    anika config set model gpt-4 Set a config value
    anika gateway                Run messaging gateway
    anika -s anika-agent-dev,github-auth
    anika -w                     Start in isolated git worktree
    anika gateway install        Install gateway background service
    anika sessions list          List past sessions
    anika sessions browse        Interactive session picker
    anika sessions rename ID T   Rename/title a session
    anika logs                   View agent.log (last 50 lines)
    anika logs -f                Follow agent.log in real time
    anika logs errors            View errors.log
    anika logs --since 1h        Lines from the last hour
    anika debug share             Upload debug report for support
    anika console                Open the safe Anika command console
    anika update                 Update to latest version
    anika dashboard              Start web UI dashboard (port 9119)
    anika dashboard --stop       Stop running dashboard processes
    anika dashboard --status     List running dashboard processes
...
def _add_top_level_flags(parser: argparse.ArgumentParser) -> None:
    ...
    inherited(parser, "-m", "--model", default=None, help=(
        ...
        "Applies to -z/--oneshot and --tui. Also settable via ANIKA_INFERENCE_MODEL env var."))
    inherited(parser, "--provider", default=None, help=(
        "Provider override for this invocation (e.g. openrouter, anthropic). "
        "Applies to -z/--oneshot and --tui. The persistent provider lives in config.yaml "
        "under model.provider — use `anika setup` or edit the file to change it."))
    ...
    inherited(parser, "--accept-hooks", action="store_true", default=False, help=(
        ...
        "without a TTY prompt.  Equivalent to ANIKA_ACCEPT_HOOKS=1 or "
    ...
    inherited(parser, "--ignore-user-config", action="store_true", default=False,
              help="Ignore ~/.anika/config.yaml and fall back to built-in defaults (credentials in .env are still loaded)")
...
def _build_chat_parser(subparsers) -> argparse.ArgumentParser:
    ...
    ``anika -m foo chat`` argparse first sets ``args.model`` from the top-level parser, then
    ...
    unless the flag is actually passed after the subcommand (tests/anika_cli/
    test_argparse_flag_propagation.py).
    """
    chat_parser = subparsers.add_parser(
        "chat", help="Interactive chat with the agent",
        description="Start an interactive chat session with Anika Agent")
    ...
    inherited(chat_parser, "--accept-hooks", action="store_true", default=SUPPRESS, help=(
        "Auto-approve any unseen shell hooks declared in config.yaml "
        "without a TTY prompt (see also ANIKA_ACCEPT_HOOKS env var and "
    ...
    inherited(chat_parser, "--ignore-user-config", action="store_true", default=SUPPRESS,
              help="Ignore ~/.anika/config.yaml and fall back to built-in defaults (credentials in .env are still loaded). Useful for isolated CI runs, reproduction, and third-party integrations.")
    ...
    inherited(chat_parser, "--safe-mode", action="store_true", default=SUPPRESS,
              help="Troubleshooting mode: disable ALL customizations — user config, AGENTS.md/memory injection, plugins, and MCP servers (implies --ignore-user-config and --ignore-rules). Use to isolate whether a problem comes from your setup or from Anika itself.")
...
def build_top_level_parser():
    ...
    parser = argparse.ArgumentParser(
        prog="anika", description="Anika Agent - AI assistant with tool-calling capabilities",
