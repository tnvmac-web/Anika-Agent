"""Cross-process mutual exclusion for in-flight Anika updates.
...
MARKER_NAME = ".anika-update-in-progress"

# Set by an orchestrating updater (Tauri `anika-setup --update`) to its own pid before
# spawning `anika update` as a child stage; the parent holds the marker for its whole run,
...
HANDOFF_PID_ENV = "ANIKA_UPDATE_HANDOFF_PID"
...
# Tauri updater (UPDATE_EXIT_CONCURRENT in update.rs) to show "Anika is still running".
...
def update_marker_path() -> Path:
    ...
    Uses the *process* Anika home (never the context-local profile override): the Rust
    updater resolves ``$ANIKA_HOME`` or the platform default and the desktop pins that same
    ...
    return get_process_anika_home() / MARKER_NAME
...
def _is_ancestor_pid(pid: int) -> bool:
    ...
    The orchestrating updater spawns ``anika update`` as a (grand)child, so a live marker
...
def describe_holder(holder: UpdateHolder) -> str:
    ...
    return (
        f"✗ Another Anika update is already running (PID {holder.pid}, "
