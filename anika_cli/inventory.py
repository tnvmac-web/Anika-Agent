...
def _strip_aggregator_overlaps(rows: list[dict]) -> None:
    ...
    try:
        from anika_cli.providers import is_routing_aggregator
    except Exception:
        return
...
def _apply_capabilities(rows: list[dict]) -> None:
    ...
    from anika_cli.models import model_supports_fast_mode
...
def _apply_custom_aliases(rows: list[dict]) -> None:
    ...
    :func:`anika_cli.providers.custom_provider_aliases` — lets the frontend do a membership check instead.
    """
    ...
    for entry in CANONICAL_PROVIDERS:
        ...
        if entry.slug.lower() == cur:
            ...
            warning = (
                ...
                else "Configured provider is not authenticated; run `anika model` to reactivate. "
                "Showing the saved model only."
            )
...
def _anthropic_oauth_credentials_present() -> bool:
    """True when the user explicitly authenticated Anthropic via OAuth (Anika device flow or Claude Code
    ...
    try:
        ...
        readers = (read_anika_oauth_credentials, read_claude_code_credentials)
...
def _external_process_signed_in(slug: str) -> bool:
    ...
    try:
        from anika_cli.auth import PROVIDER_REGISTRY, get_external_process_provider_status
...
def _provider_is_keyless(slug: str) -> bool:
    """True when the provider's Anika overlay declares it keyless."""
    try:
        from anika_cli.providers import ANIKA_OVERLAYS
        overlay = ANIKA_OVERLAYS.get(slug)
...
def _apply_picker_hints(rows: list[dict]) -> None:
    ...
    for row in rows:
        ...
        row["warning"] = (f"paste {key_env} to activate" if auth_type == "api_key" and key_env
                          else f"run `anika model` to configure ({auth_type})")
...
def _apply_pricing(rows: list[dict], *, force_fresh_nous_tier: bool = False, cached_only: bool = False) -> None:
    """Set ``row["pricing"] = {model_id: {input, output, cache | None, free}}``; for __NEW_ORG__ also
    ``free_tier`` (account is free-tier) and ``unavailable_models`` (paid models a free user can't pick).
    ``cached_only`` never hits the network: unknown __NEW_ORG__ entitlement fails closed (``free_tier_pending``,
    ...
    for row in rows:
        ...
        if slug == "nous" and cached_only:
            ...
            if cached_nous_tier is None:
                ...
                if not row.get("warning"):  # say why every model renders locked
                    row["warning"] = ("Checking __NEW_ORG__ plan entitlement… models unlock on the "
        ...
        for mid in models:
            ...
            # Sale chrome is __NEW_ORG__ Portal-only (other catalogs' nested pricing.original is ignored); free
...
def _local_runtime_row(ctx: "ConfigContext") -> dict | None:
    ...
    try:
        from anika_cli.local_runtime.bootstrap import staged_model_ids
        ...
        if not current:
            ...
            try:
                from anika_cli.local_runtime.endpoint import _state_endpoint
...
def _prewarm_pricing_async(
    ...
    from anika_cli.models_pricing import pricing_cache_scope
    ...
    endpoint_scope = tuple(sorted(
        ...
        for slug in slugs))
    prewarm_key = (anika_home_key(), endpoint_scope)
    ...
    with _pricing_prewarm_lock:
        ...
        thread = Thread(target=copy_context().run, args=(_worker,),
                        name="anika-picker-pricing-prewarm", daemon=True)
