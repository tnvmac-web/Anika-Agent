"""``anika dashboard register`` — register a self-hosted dashboard OAuth client.

Automates the __NEW_ORG__ Portal ``/local-dashboards`` flow: resolve a fresh __NEW_ORG__ access token, POST
``{portal}/api/oauth/self-hosted-client`` (the ``agent:`` prefix is applied server-side),
write ``ANIKA_DASHBOARD_OAUTH_CLIENT_ID`` (+ portal/public URL when warranted) into ``.env``
...
_DEFAULT_PORTAL = "https://portal.__NEW_DOMAIN__"
    ...
    *, access_token: str, portal_base_url: str, name: Optional[str], custom_redirect_uri: Optional[str],
    ...
    try:
        ...
        try:  # structured JSON errors: {error, error_description}
            ...
            detail = ""
        if exc.code == 401:
            message = ("__NEW_ORG__ Portal rejected the access token (401). "
                       "Try `anika auth add nous` to re-authenticate.")
        elif exc.code == 403:
        ...
        raise RuntimeError(f"Could not reach __NEW_ORG__ Portal at {portal_base_url}: {exc.reason}") from exc
...
def _print_post_register_hint(
    *, client_id: str, portal_base_url: str, custom_redirect_uri: Optional[str],
    ...
    from anika_cli.config import get_env_path
    print(f"\n  Wrote to {get_env_path()}:\n    ANIKA_DASHBOARD_OAUTH_CLIENT_ID={client_id}")
    if wrote_portal_url:
        print("    ANIKA_DASHBOARD_PORTAL_URL=" + str(portal_base_url))
    if public_url:
        print("    ANIKA_DASHBOARD_PUBLIC_URL=" + str(public_url))
    print(
        "\n  Heads up — __NEW_ORG__ login only *engages* on a non-loopback bind. A plain\n"
        "  `anika dashboard` (localhost) leaves the gate off and serves locally\n"
    ...
    if custom_redirect_uri:
        try:  # example host matches the one the user registered
            ...
            host = "your-host"
        print(
            "  To require __NEW_ORG__ login on your registered host, run the dashboard\n"
            f"  bound publicly (it must be reachable at https://{host}) and log in\n"
            "  at its /login page.")
    else:
        print(
            "  To require __NEW_ORG__ login (e.g. exposing on your LAN or a public host):\n"
...
def _public_url_from_redirect(redirect_uri: Optional[str]) -> str:
    ...
    ``/auth/callback`` to ANIKA_DASHBOARD_PUBLIC_URL, so the raw URI would double the path."""
    try:
...
def cmd_dashboard_register(args) -> None:
    """Register a self-hosted dashboard OAuth client with __NEW_ORG__ Portal."""
    from anika_cli.auth import AuthError, resolve_nous_access_token
    from anika_cli.config import is_managed, save_env_value
    # Managed installs get the client id stamped in by the orchestrator (save_env_value refuses).
    if is_managed():
        print("✗ `anika dashboard register` is not available in a managed/hosted install.\n"
    ...
    try:
        ...
        if isinstance(exc, AuthError) and getattr(exc, "relogin_required", False):
            print("✗ You're not logged into __NEW_ORG__ Portal.\n"
                  "  Run `anika setup` (or `anika auth add nous`) first, then retry.")
        else:
            print(f"✗ Could not resolve a __NEW_ORG__ Portal access token: {exc}")
        sys.exit(1)
    ...
    portal_override = getattr(args, "portal_url", None) or os.environ.get("ANIKA_DASHBOARD_PORTAL_URL")
    ...
    stored = _env_value("ANIKA_DASHBOARD_OAUTH_CLIENT_ID")
    ...
    try:  # client_id is load-bearing: fatal on failure
        save_env_value("ANIKA_DASHBOARD_OAUTH_CLIENT_ID", client_id)
    except Exception as exc:
        print(f"✗ Failed to write ANIKA_DASHBOARD_OAUTH_CLIENT_ID to .env: {exc}\n"
              f"  Set it manually:  ANIKA_DASHBOARD_OAUTH_CLIENT_ID={client_id}")
        sys.exit(1)
    # Explicit portal → always persist (the user asked); inferred → only if unset AND non-default.
    existing_portal = _env_value("ANIKA_DASHBOARD_PORTAL_URL")
    should_write_portal = (
    ...
    wrote_portal_url = should_write_portal and _save_env_quietly("ANIKA_DASHBOARD_PORTAL_URL", portal_base_url)
    ...
    wrote_public_url = bool(
        public_url
        and _env_value("ANIKA_DASHBOARD_PUBLIC_URL") != public_url
        and _save_env_quietly("ANIKA_DASHBOARD_PUBLIC_URL", public_url))
