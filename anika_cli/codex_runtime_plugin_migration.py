"""Migrate Anika MCP server config and Codex's installed curated plugins into ~/.codex/config.toml.
"""
...
MIGRATION_END_MARKER = "# end anika-agent managed section"
...
class MigrationReport:
    ...
    def summary(self) -> str:
        ...
        if self.migrated:
            ...
            lines.append("No MCP servers found in Anika config.")
...
# Anika MCP keys codex understands (transport stdio/http, timeouts, general). Any other key is
# dropped with a warning: ``sampling`` has no codex equivalent; the rest are unknown Anika keys.
_KNOWN_ANIKA_KEYS = {
...
# (anika key, codex key, skip note) — timeouts are emitted as floats or skipped when non-numeric.
_TIMEOUT_KEYS = (
...
def _translate_one_server(name: str, anika_cfg: dict) -> tuple[Optional[dict], list[str]]:
    """Translate one Anika MCP server config to codex's inline-table dict.

    Returns ``(codex_entry, skipped_keys)``; ``codex_entry`` is None when the config is unusable.
    stdio (``command``) wins over ``url`` when both are set. Anika' ``transport: sse`` hint is
    informational only — codex auto-negotiates. ``enabled`` is emitted only when explicitly false
    (codex defaults to true).
    """
    if not isinstance(anika_cfg, dict):
        return None, []
    skipped: list[str] = []
    out: dict[str, Any] = {}
    if anika_cfg.get("command"):
        if anika_cfg.get("url"):
            skipped.append("url (both command and url set; preferring stdio)")
        out["command"] = str(anika_cfg["command"])
        if anika_cfg.get("args"):
            out["args"] = [str(a) for a in anika_cfg["args"]]
        if anika_cfg.get("env"):
            out["env"] = _str_map(anika_cfg["env"])
        if anika_cfg.get("cwd"):
            out["cwd"] = str(anika_cfg["cwd"])
    elif anika_cfg.get("url"):
        out["url"] = str(anika_cfg["url"])
        if anika_cfg.get("headers"):
            out["http_headers"] = _str_map(anika_cfg["headers"])
        if anika_cfg.get("transport") == "sse":
            skipped.append("transport=sse (codex auto-negotiates)")
    else:
        return None, ["no command or url field"]
    for anika_key, codex_key, note in _TIMEOUT_KEYS:
        if anika_key in anika_cfg:
            try:
                out[codex_key] = float(anika_cfg[anika_key])
            except (TypeError, ValueError):
                skipped.append(note)
    if anika_cfg.get("enabled") is False:
        out["enabled"] = False
    for key in anika_cfg:
        if key in _KEYS_DROPPED_WITH_WARNING:
            skipped.append(f"{key} (no codex equivalent)")
        elif key not in _KNOWN_ANIKA_KEYS:
            skipped.append(f"{key} (unknown Anika key)")
    return out, skipped
...
# env-var passthrough (ANIKA_HOME, PYTHONPATH) could carry one in pathological cases.
_TOML_ESCAPES = (
    ("\\", "\\\\"), ('"', '\\"'), ("\b", "\\b"), ("\t", "\\t"),
    ("\n", "\\n"), ("\f", "\\f"), ("\r", "\\r"))
...
def render_codex_toml_section(
    ...
    if not servers and not plugins and not default_permission_profile:
        out += ["# (no MCP servers, plugins, or permissions configured by Anika)", MIGRATION_END_MARKER]
        return "\n".join(out) + "\n"
...
def _query_codex_plugins(
    ...
    try:
        with CodexAppServerClient(codex_home=str(codex_home) if codex_home else None) as client:
            client.initialize(client_name="anika-migration")
...
def _looks_like_test_tempdir(path: str) -> bool:
    ...
    Such dirs are reaped between sessions; a ANIKA_HOME pointing there burned into
...
def _build_anika_tools_mcp_entry() -> dict:
    """Codex stdio entry launching Anika' own tool surface as an MCP server (browser/web/
    delegate_task/vision/memory/skills call-backs).

    ANIKA_HOME passes through only IF SET, read from os.environ (not get_anika_home()): when
    unset the codex subprocess must inherit its launcher's runtime ANIKA_HOME (systemd, gateway,
    kanban), not a migrate-time default burned into config.toml that pins the wrong profile. The
    pytest-tempdir guard keeps a sibling test's monkeypatched ANIKA_HOME out of the user's real
    config. PYTHONPATH passes through so a worktree-launched anika finds the branch's modules.
    """
    import sys
    env: dict[str, str] = {}
    # ANIKA_HOME passes through IF SET so the MCP subprocess sees the same config / auth / sessions DB as
    # the parent CLI. Read from os.environ (not get_anika_home()) on purpose: when the env var is unset we
    # want codex's subprocess to inherit whatever ANIKA_HOME its launcher sets at runtime (systemd unit,
    # gateway, kanban dispatcher, custom shell), rather than burning the migrate-time resolved default into
    # config.toml — that would override the launcher's ANIKA_HOME and pin the subprocess to the wrong
    # profile. The pytest-tempdir guard below catches the issue #26250 Bug C scenario: a sibling test's
    # monkeypatch.setenv("ANIKA_HOME", tmp_path) would otherwise leak a transient pytest tempdir into the
    # user's real ~/.codex/config.toml and silently brick codex once the tempdir is GC'd.
    anika_home = os.environ.get("ANIKA_HOME") or ""
    if anika_home and not _looks_like_test_tempdir(anika_home):
        env["ANIKA_HOME"] = anika_home
    if os.environ.get("PYTHONPATH"):
        env["PYTHONPATH"] = os.environ["PYTHONPATH"]
    # Quiet mode + redaction defaults so the MCP wire stays clean.
    env["ANIKA_QUIET"] = "1"
    env["ANIKA_REDACT_SECRETS"] = env.get("ANIKA_REDACT_SECRETS", "true")
    return {
...
def migrate(
    anika_config: dict, *, codex_home: Optional[Path] = None, dry_run: bool = False,
    discover_plugins: bool = True, default_permission_profile: Optional[str] = ":workspace",
    expose_anika_tools: bool = True) -> MigrationReport:
    """Translate Anika mcp_servers config + Codex curated plugins into ~/.codex/config.toml.
    ...
    approval prompt on every write. ``expose_anika_tools`` registers Anika' own tool surface
    (agent/transports/anika_tools_mcp_server.py, launched on demand by codex over stdio) as an MCP
    ...
    anika_servers = (anika_config or {}).get("mcp_servers") or {}
    if not isinstance(anika_servers, dict):
        report.errors.append("mcp_servers in Anika config is not a dict; cannot migrate.")
        return report
    translated: dict[str, dict] = {}
    for raw_name, cfg in anika_servers.items():
        name = str(raw_name)
    ...
    if expose_anika_tools:
        translated["anika-tools"] = _build_anika_tools_mcp_entry()
        if "anika-tools" not in report.migrated:
            report.migrated.append("anika-tools")
