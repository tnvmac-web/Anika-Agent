...
def _anthropic_oauth_login(args) -> dict:
    ...
    creds = anthropic_mod.run_anika_oauth_login_pure()
    if not creds:
...
def _qwen_oauth_login(args) -> dict:
    from anika_cli.auth_qwen import _mark_qwen_oauth_active
...
class _OAuthAddSpec:
    """Per-provider parameters for the generic ``anika auth add <provider> --type oauth`` path."""
...
_OAUTH_ADD_SPECS: dict[str, _OAuthAddSpec] = {
    "anthropic": _OAuthAddSpec(
        ...
        source=f"{SOURCE_MANUAL}:anika_pkce",
...
def _add_nous_oauth_credential(args, provider: str) -> None:
    """``anika auth add nous --type oauth``: shared-credential import, else device-code login."""
    ...
    # Codex-style auto-import: a shared __NEW_ORG__ credential at <anika-root>/shared/nous_auth.json
    # (written by any previous login) makes `anika --profile <name> auth add nous --type oauth`
    # a one-tap operation for multi-profile users.
    if auth_mod._read_shared_nous_state():
        try:
            found = f"Found existing __NEW_ORG__ OAuth credentials at {auth_mod._nous_shared_store_path()}"
        except RuntimeError:
            found = "Found existing shared __NEW_ORG__ OAuth credentials"
        print()
        print(found)
...
def auth_add_command(args) -> None:
    ...
    if spec is None:
        raise SystemExit(f"`anika auth add {provider}` is not implemented for auth type {requested_type} yet.")
...
def auth_remove_command(args) -> None:
    ...
    # Every credential source Anika reads from (env vars, external OAuth files, auth.json blocks,
...
def auth_status_command(args) -> None:
    ...
    if not provider:
        raise SystemExit("Provider is required. Example: `anika auth status spotify`.")
...
def _interactive_auth() -> None:
    """Interactive credential pool management when `anika auth` is called bare."""
