...
Imported by ``anika_cli/main.py`` BEFORE its heavy import wall (config, argparse tree, logging,
providers). Everything here must stay **stdlib-only and cheap** (os/sys file probes; no yaml, no
anika_cli.config, no argparse). Exists so version-printing stops being reimplemented as
...
def active_profile_may_override_home(anika_root: str) -> bool:
    """Cheap probe: does an active non-default profile redirect ANIKA_HOME?"""
    active = (_read_text(os.path.join(anika_root, "active_profile")) or "").strip()
    return bool(active and active != "default")


def _default_home() -> str:
    return os.path.join(os.path.expanduser("~"), ".anika")


def _resolved_home() -> str:
    return os.environ.get("ANIKA_HOME", "").strip() or _default_home()
...
def container_mode_may_be_active() -> bool:
    ...
    be active".
    """
    if os.environ.get("ANIKA_DEV") == "1" or is_container_startup_environment():
        return False
    anika_home = os.environ.get("ANIKA_HOME", "").strip()
    if anika_home:
        if os.path.exists(os.path.join(anika_home, ".container-mode")):
            return True
        parent_name = os.path.basename(os.path.dirname(os.path.normpath(anika_home)))
        return parent_name != "profiles" and active_profile_may_override_home(anika_home)
...
def print_fast_version_info(*, check_updates: bool = True) -> None:
    """THE canonical ``anika --version`` output (also used by /version).
    ...
    try:
        ...
        from anika_cli import __release_date__, __version__

        print(f"Anika Agent v{__version__} ({__release_date__})")
    ...
    try:
        from anika_cli.banner import UPDATE_AVAILABLE_NO_COUNT, check_for_updates
...
def try_fast_version(argv: list[str] | None = None) -> bool:
    """Handle ``anika --version`` before the heavy import wall.
    ...
    ANIKA_TERMUX_DISABLE_FAST_CLI escape hatch.
    """
    if argv is None:
        argv = sys.argv[1:]
    is_termux = is_termux_env()
    if is_termux and os.environ.get("ANIKA_TERMUX_DISABLE_FAST_CLI") == "1":
        return False
    if is_termux:
