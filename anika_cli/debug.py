"""``anika debug`` debug tools for Anika Agent."""
...
_REDACTION_BANNER = (
    "[anika debug share: log content redacted at upload time. "
...
_USER_AGENT = "anika-agent/debug-share"
...
# ``anika debug`` sweeps on entry (CLI-only users). Replaced a fork-and-sleep subprocess that
# leaked ~20 MB per share.


def _pending_file() -> Path:
    return get_anika_home() / "pastes" / "pending.json"
...
def _save_pending(entries: list[dict]) -> None:
    ...
    try:
        ...
        pass  # non-fatal — worst case the user runs ``anika debug delete`` manually
...
_GATEWAY_PRIVACY_NOTICE = (
    ...
    "Full logs are NOT included from the gateway — use `anika debug share` "
...
def _upload_dpaste_com(content: str, expiry_days: int = 7) -> str:
    boundary = "----AnikaDebugBoundary9f3c"
...
def _primary_log_path(log_name: str) -> Optional[Path]:
    ...
    from anika_cli.logs import LOG_FILES
    filename = LOG_FILES.get(log_name)
    return (get_anika_home() / "logs" / filename) if filename else None
...
_CLIENT_SIDE_LOGS = {
    "desktop": (
        "written by Anika Desktop on the machine running the app, not by this "
...
def _capture_dump() -> str:
    """Run ``anika dump`` and return its stdout as a string."""
    from anika_cli.dump import run_dump
    capture = io.StringIO()
...
def collect_debug_report(
    *, log_lines: int = 200, dump_text: str = "",
    ...
    ``dump_text`` is pre-captured dump output; when empty, ``anika dump`` is run internally.
    """
    buf = io.StringIO()
...
_NOUS_BUNDLE_FORMAT = "anika-debug-share/1"
...
def build_nous_bundle(bundle: dict[str, str], redact: bool = True) -> bytes:
    """Gzip a :func:`collect_share_bundle` mapping into the __NEW_ORG__ envelope (shape parsed by the
...
def build_debug_share(
    *, log_lines: int = 200, expiry: int = 7, redact: bool = True) -> DebugShareResult:
    ...
    Shared by ``anika debug share`` and the dashboard ``POST /api/ops/debug-share``. Blocking
    ...
    if redact:
        logger.info(
            "anika debug share: applied force-mode redaction to log snapshots before upload")
...
def run_debug_share(args):
    ...
    try:
        ...
        print("\nRun `anika debug share --local` to print the report instead.\n")
        sys.exit(1)
    ...
    print(f"\n⏱  Pastes will auto-delete in {result.auto_delete_seconds // 3600} hours.\n"
          "To delete now:  anika debug delete <url>\n"
          "\nShare these links with the Anika team for support.")
...
⚠️  --nous: This uploads your debug bundle to Nous-INTERNAL storage (AWS S3),
  ...
  • System info (OS, Python/Anika version, provider, which API keys are
  ...
  • The bundle is viewable only by __NEW_ORG__ staff (and allowlisted Discord mods)
...
def _run_debug_share_nous(args, *, log_lines: int, redact: bool) -> None:
    """``anika debug share --nous``: gzip the same bundle into the __NEW_ORG__ envelope → Nous-S3."""
    from anika_cli.diagnostics_upload import share_to_nous
    ...
    bundle = collect_share_bundle(log_lines=log_lines, redact=redact)
    if redact:
        logger.info("anika debug share --nous: applied force-mode redaction before upload")
    print("Uploading to __NEW_ORG__ diagnostics storage...")
    try:
        res = share_to_nous(build_nous_bundle(bundle, redact=redact))
    except Exception as exc:
        print(f"\n__NEW_ORG__ upload failed: {exc}\n"
              "\nThe __NEW_ORG__ diagnostics service may be unavailable or not yet provisioned.\n"
              "Run `anika debug share --local` to print the report instead, "
              "or `anika debug share` to upload to a public paste service.\n", file=sys.stderr)
        sys.exit(1)
    view_url = res.get("viewUrl") or res.get("view_url")
    expires_at = res.get("expiresAt") or res.get("expires_at")
    print("\nDebug bundle uploaded to __NEW_ORG__ (private):")
    ...
    print("\nShare this private link with the __NEW_ORG__ team — only __NEW_ORG__ staff "
          "(via Google login) can open it.\n"
          "\nPick up the discussion in:\n"
          "  GitHub Issues        https://github.com/__NEW_ORG__/anika-agent/issues\n"
          "  __NEW_ORG__ Portal Support  https://portal.__NEW_DOMAIN__/help\n"
...
def run_debug_delete(args):
    ...
    if not urls:
        print("Usage: anika debug delete <url> [<url> ...]\n"
              "  Deletes paste.rs pastes uploaded by 'anika debug share'.")
        return
    for url in urls:
        try:
            if delete_paste(url):
