"""CLI subcommand: ``anika send`` — pipe text from shell scripts to any configured messaging platform
...
def _read_message_body(positional: Optional[str], file_path: Optional[str]) -> Optional[str]:
    ...
    if file_path:
        ...
        try:
            ...
            _fail(
                f"anika send: {file_path} is not a text file. --file reads the "
                ...
                f'  anika send --to telegram "MEDIA:{file_path}"\n'
                f'  anika send --to telegram "optional caption MEDIA:{file_path}"\n'
                "Add [[as_document]] to deliver an image as an uncompressed file:\n"
                f'  anika send --to telegram "[[as_document]] MEDIA:{file_path}"',
                _USAGE_EXIT)
        except OSError as exc:
            _fail(f"anika send: cannot read {file_path}: {exc}", _USAGE_EXIT)
...
def _emit_result(result_json: str, *, json_mode: bool, quiet: bool) -> int:
    ...
    if json_mode:
        ...
        if payload.get("error"):
            print(f"anika send: {payload['error']}", file=sys.stderr)
...
def _list_targets(platform_filter: Optional[str], *, json_mode: bool) -> int:
    ...
    try:
        ...
        return _fail(f"anika send: failed to load channel directory: {exc}")
    try:
        raw = load_directory()
    except Exception as exc:
        return _fail(f"anika send: failed to read channel directory: {exc}")
    ...
    if platform_filter:
        ...
        if not filtered:
            return _fail(
                f"anika send: no targets found for platform '{platform_filter}'. "
    ...
    if not platforms:
        ...
        print("Set one up with `anika gateway setup`, or run the gateway once so")
        print("channel discovery can populate ~/.anika/channel_directory.json.")
        return _SUCCESS_EXIT
...
def _load_anika_env() -> None:
    """Populate ``os.environ`` from ``~/.anika/.env`` AND bridge top-level ``config.yaml`` keys into
...
def cmd_send(args: argparse.Namespace) -> None:
    ...
    _load_anika_env()  # the downstream gateway config loader reads credentials from os.environ
    if getattr(args, "list_targets", False):  # --list short-circuits everything else
        # `anika send --list telegram` lands "telegram" in the `message` positional.
        exit_code = _list_targets(getattr(args, "message", None), json_mode=getattr(args, "json", False))
        sys.exit(exit_code)
    target = (getattr(args, "to", None) or "").strip()
    if not target:
        _fail(
            "anika send: --to PLATFORM[:channel[:thread]] is required\n"
            "Examples:\n"
            "  anika send --to telegram \"hello\"\n"
            "  anika send --to discord:#ops --file report.md\n"
            "  anika send --list      # list available targets",
            _USAGE_EXIT)
    message = _read_message_body(getattr(args, "message", None), getattr(args, "file", None))
    if message is None or not message.strip():
        _fail(
            "anika send: no message provided. Pass text as a positional "
    ...
    # Lazy import keeps `anika send --help` fast (no tool registry / gateway config stack).
...
_SEND_ARGUMENTS = (
    ...
    (("-l", "--list"), dict(dest="list_targets", action="store_true", default=False,
                            help="List available targets. Optional positional filter: `anika send --list telegram`.")),
...
def register_send_subparser(subparsers) -> argparse.ArgumentParser:
    ...
    parser = subparsers.add_parser(
        ...
        description=(
            "Pipe text from any shell script to any messaging platform Anika "
            "is already configured for. Reuses the gateway's platform "
            "credentials (~/.anika/.env + ~/.anika/config.yaml) — no LLM, "
        ...
        epilog=(
            ...
            "  anika send --to telegram \"MEDIA:/tmp/chart.png\"   # send a media attachment\n"
