"""Image-authored deployment provenance for immutable Anika runtimes.

The image bakes ``/etc/anika/image-provenance.json`` outside ``$ANIKA_HOME`` and the checkout, so a
...
IMAGE_PROVENANCE_PATH = Path("/etc/anika/image-provenance.json")
