...
maps. Split out of ``anika_cli.models``.
"""
...
# OpenRouter entries the __NEW_ORG__ Portal does not carry (routing/fast variants, free tier).
_OPENROUTER_ONLY = {
...
def _codex_curated_models() -> list[str]:
    ...
    synthesis) so the gateway /model picker and the CLI ``anika model`` flow share one source."""
    from anika_cli.codex_models import DEFAULT_CODEX_MODELS, _finalize_codex_models
...
# Mirrors the xAI-direct IDs from $ANIKA_HOME/models_dev_cache.json; the cache overrides it on the
...
def _xai_merge_curated_extras(ids: list[str]) -> list[str]:
    """Append Anika-curated xAI models missing from models.dev, right after the pinned headline."""
    out = list(ids)
...
def _xai_curated_models() -> list[str]:
    """Offline curated floor for xAI / xAI OAuth pickers: $ANIKA_HOME/models_dev_cache.json
...
_PROVIDER_MODELS: dict[str, list[str]] = {
    ...
    # (see anika_cli/model_setup_flows.py); validated live against a GCP project (global region).
    "vertex": [
...
# lists, displays, or iterates providers (anika model, /model, list_authenticated_providers)
# derives from it. slug = internal ID (config.yaml, --provider); label = short display name;
# tui_desc = longer description for the `anika model` picker.
# ---------------------------------------------------------------------------
...
CANONICAL_PROVIDERS: list[ProviderEntry] = [ProviderEntry(*row) for row in (
    ("nous", "__NEW_ORG__ Portal", "__NEW_ORG__ Portal (Everything your agent needs, 300+ models with bundled tool use)"),
...
# ...) fold under one top-level row in the INTERACTIVE PICKERS (``anika model``, setup wizard,
...
# Offline/fresh-install fallback for the model Anika silently lands on when the user never picked
...
# (GUI onboarding / ``anika model``) uses the tier-aware ``get_recommended_default_model`` in
# anika_cli/web_server.py + ``partition_nous_models_by_tier``, which may hit the Portal.
...
# OpenRouter-style ids -> Copilot ids. Dash-notation Claude ids are accepted too: Anika' default
