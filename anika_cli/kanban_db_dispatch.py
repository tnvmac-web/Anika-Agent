...
Split out of ``anika_cli.kanban_db``; origin-resident helpers are reached
...
if TYPE_CHECKING:
    from anika_cli.kanban_db import Task
...
# ``ANIKA_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS``.
...
class DispatchResult:
    ...
    Code terminal like ``orion-cc``), not a Anika profile. Expected steady-state
    ...
    if sweep.exited_hook_payloads and _kb._kanban_observer_consumed("on_kanban_worker_exited"):
        ...
        for hook_fields in sweep.exited_hook_payloads:
            ...
            _kb._fire_kanban_lifecycle_hook(
                ...
                # fire in the DISPATCHER process (gateway-embedded dispatcher or ``anika kanban
...
def _profile_exists_fn() -> Optional[Callable[[str], bool]]:
    """``anika_cli.profiles.profile_exists``, or ``None`` when it cannot be
    imported (local import avoids a cycle; callers fall back to trusting the
    assignee)."""
    try:
        from anika_cli.profiles import profile_exists
    except Exception:
        return None
...
def has_spawnable_ready(conn: sqlite3.Connection) -> bool:
    """True iff a ready+assigned+unclaimed task maps to a real Anika profile.
...
def review_dispatch_enabled() -> bool:
    """Whether review tasks dispatch automatically. Default true (Anika ships
...
def count_running_tasks_other_boards(board: Optional[str] = None) -> int:
    ...
    Boards are matched by resolved DB path, so ``ANIKA_KANBAN_DB`` (pins every
...
def _dispatch_lane_task(
    ...
    *,
    ...
    # would fail ``anika -p <assignee>`` at startup and loop ready→crash→ready
    ...
    if guard_reason is not None:
        ...
        # Event so ``anika kanban tail`` shows why the task looks stuck.
...
def _module_anika_argv() -> list[str]:
    """Interpreter-bound Anika CLI invocation (``anika_cli.main`` is the
    console-script target — there is no top-level ``anika`` package)."""
    return [sys.executable, "-m", "anika_cli.main"]


def _absolute_anika_path(path: str) -> str:
    """Return an absolute filesystem path for a resolved Anika shim."""
...
def _anika_path_argv(path: str) -> list[str]:
    """argv for a resolved Anika executable path. Windows batch shims
    ...
    if _kb._IS_WINDOWS and _is_windows_batch_shim(path):
        return _module_anika_argv()
    return [_absolute_anika_path(path)]


def _resolve_anika_argv() -> list[str]:
    """Resolve the ``anika`` invocation as argv for ``Popen``: ``$ANIKA_BIN``
    (path-like -> absolute; bare names keep PATH semantics, never a
    same-directory file), then ``which("anika")`` (Windows: safe PATH search,
    batch shims fall back to the module form), then ``sys.executable -m
    anika_cli.main`` for shim-less environments (cron, systemd ``User=``,
    launchd). Mirrors ``gateway.run._resolve_anika_bin``; local because
    ``anika_cli`` sits below ``gateway`` in the dependency order.
    """
    import shutil

    env_bin = os.environ.get("ANIKA_BIN", "").strip()
    if env_bin:
        if _looks_like_path(env_bin):
            return _anika_path_argv(env_bin)
        resolved_env_bin = _safe_which_no_cwd(env_bin)
        if resolved_env_bin:
            return _anika_path_argv(resolved_env_bin)
        return _module_anika_argv()

    anika_bin = _safe_which_no_cwd("anika") if _kb._IS_WINDOWS else shutil.which("anika")
    if anika_bin:
        return _anika_path_argv(anika_bin)
    return _module_anika_argv()
...
def _resolve_worker_cli_toolsets(anika_home: Optional[str]) -> Optional[list[str]]:
    ...
    tools when ``ANIKA_KANBAN_TASK`` is set.
    """
    if not anika_home:
        return None
    try:
        ...
        token = set_anika_home_override(anika_home)
        try:
            cfg = load_config()
        ...
        _kb._log.debug(
            "kanban worker: could not resolve CLI toolsets for ANIKA_HOME=%r (%s)",
            anika_home,
            exc,
        )
        return None
...
def _worker_argv(task: Task, profile_arg: str, anika_home: Optional[str]) -> list[str]:
    """Build the ``anika -p <profile> --cli ... chat -q ...`` worker command."""
    cmd = [
        *_resolve_anika_argv(),
        "-p", profile_arg,
        ...
        "--cli",
        # Workers run under a profile-scoped ANIKA_HOME and so see that
    ...
    worker_toolsets = _resolve_worker_cli_toolsets(anika_home)
    if worker_toolsets:
...
def _open_worker_log(task: Task, board: Optional[str]):
    ...
    `anika kanban log` reads its own file and boards sharing task ids don't
    collide."""
...
def _default_spawn(task: Task, workspace: str, *, board: Optional[str] = None) -> Optional[int]:
    """Fire-and-forget ``anika -p <profile> chat -q ...`` subprocess.
    ...
    ``ANIKA_KANBAN_DB`` / ``ANIKA_KANBAN_BOARD`` / workspaces_root to the
    board the task was claimed from, so workers cannot see other boards.
    """
    if not task.assignee:
        raise ValueError(f"task {task.id} has no assignee")

    from anika_cli.profiles import normalize_profile_name, resolve_profile_env
    ...
    # Inject ANIKA_HOME so the worker reads the profile-scoped config.yaml:
    # without it the child's get_anika_home() falls back to the DEFAULT
    # profile root because `anika -p` applies its override before
    # anika_constants is imported.
    try:
        env["ANIKA_HOME"] = resolve_profile_env(profile_arg)
    except FileNotFoundError:
        # No profile dir (isolated test fixtures) — the CLI resolves it from
        # ANIKA_PROFILE (set below) instead.
        pass
    if task.tenant:
        env["ANIKA_TENANT"] = task.tenant
    env["ANIKA_KANBAN_TASK"] = task.id
    env["ANIKA_KANBAN_WORKSPACE"] = workspace
    ...
    env["ANIKA_SESSION_SOURCE"] = "kanban"
    ...
    if task.branch_name:
        env["ANIKA_KANBAN_BRANCH"] = task.branch_name
    if task.current_run_id is not None:
        env["ANIKA_KANBAN_RUN_ID"] = str(task.current_run_id)
    if task.claim_lock:
        env["ANIKA_KANBAN_CLAIM_LOCK"] = task.claim_lock
    ...
    if task.goal_mode:
        env["ANIKA_KANBAN_GOAL_MODE"] = "1"
        if task.goal_max_turns is not None:
            env["ANIKA_KANBAN_GOAL_MAX_TURNS"] = str(int(task.goal_max_turns))
    ...
    # match after `anika -p` rewrites ANIKA_HOME (symlink / Docker layouts).
    env["ANIKA_KANBAN_DB"] = str(_kb.kanban_db_path(board=board))
    env["ANIKA_KANBAN_WORKSPACES_ROOT"] = str(_kb.workspaces_root(board=board))
    _retag_legacy_worker_sessions(env["ANIKA_KANBAN_WORKSPACES_ROOT"])
    # Board slug — defense-in-depth pin if a path is resolved without the
    # DB / workspaces env vars.
    env["ANIKA_KANBAN_BOARD"] = _kb._normalize_board_slug(board) or _kb.get_current_board()
    # kanban_comment reads ANIKA_PROFILE for its default author; `-p` alone
    # doesn't set the env var.
    env["ANIKA_PROFILE"] = profile_arg
    # `--cli` is the highest-precedence TUI override; dropping ANIKA_TUI covers
    # older anika builds on PATH that predate the flag's precedence.
    env.pop("ANIKA_TUI", None)

    cmd = _worker_argv(task, profile_arg, env.get("ANIKA_HOME"))
    ...
    try:
        ...
        log_f.close()
        raise RuntimeError(
            "`anika` executable not found on PATH. "
            "Install Anika Agent or activate its venv before running the kanban dispatcher."
        )
...
def run_daemon(
    *,
    ...
    the gateway dispatcher and ``anika kanban dispatch`` — the standalone
...
from anika_cli import kanban_db as _kb  # noqa: E402
from anika_cli import kanban_db_connect as _kbc  # noqa: E402
from anika_cli import kanban_db_workspace as _kbw  # noqa: E402
