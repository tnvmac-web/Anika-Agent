"""Multi-provider authentication system for Anika Agent.

- ``ProviderConfig`` / ``PROVIDER_REGISTRY`` describe every known inference provider.
- The auth store (``~/.anika/auth.json``) holds per-provider state, the credential pool and
...
- ``OAUTH_PROVIDER_FLOWS`` maps each OAuth provider to its resolver/status builder; the flows live in
  ...
  re-imported here so ``anika_cli.auth.<name>`` stays the public/patchable surface."""
...
from anika_cli.config import (
    get_anika_home, get_config_path, read_raw_config, require_readable_config_before_write)
from anika_constants import OPENROUTER_BASE_URL, secure_parent_dir
...
from anika_cli.auth_zai_kimi import (  # noqa: F401  re-exported
...
from anika_cli.auth_model_picker import (  # noqa: F401  re-exported
    _prompt_model_selection, _save_model_choice)
from anika_cli.auth_device_flow import (  # noqa: F401  re-exported
...
from anika_cli.auth_oauth_grants import (  # noqa: F401  re-exported
...
from anika_cli.auth_nous import (  # noqa: F401  re-exported
...
from anika_cli.auth_minimax import (  # noqa: F401  re-exported
...
from anika_cli.auth_xai import (  # noqa: F401  re-exported
...
from anika_cli.auth_codex import (  # noqa: F401  re-exported
...
from anika_cli.auth_spotify import (  # noqa: F401  re-exported
    _refresh_spotify_oauth_state, get_spotify_auth_status, login_spotify_command,
    resolve_spotify_runtime_credentials)
from anika_cli.auth_qwen import (  # noqa: F401  re-exported
...
from anika_cli.auth_constants import (  # noqa: F401  re-exported
...
_REGISTRY_ROWS: Tuple[Any, ...] = (
    ProviderConfig(
        "nous", "__NEW_ORG__ Portal", "oauth_device_code", portal_base_url=DEFAULT_NOUS_PORTAL_URL,
    ...
    # is a 401), so there is no secret to configure. Select via `anika model` / `/model free`.
...
def get_anthropic_key() -> str:
    ...
    Checks both the ``.env`` file and the process environment, preferring ``~/.anika/.env`` so a deliberate
    key rotation isn't shadowed by a stale shell export (matches the api-key resolution path — see #20591).
    """
    from anika_cli.config import get_env_value_prefer_dotenv
...
def _resolve_api_key_provider_secret(provider_id: str, pconfig: ProviderConfig) -> tuple[str, str]:
    ...
    if provider_id == "copilot":
        ...
        try:
            from anika_cli.copilot_auth import resolve_copilot_token, get_copilot_api_token
    ...
    # Prefer ~/.anika/.env over os.environ so a deliberate key rotation in .env isn't shadowed by
...
def is_rate_limited_auth_error(error: Exception) -> bool:
    ...
    re-authenticating cannot fix it, so callers should say "retry later", not ``anika auth``."""
    return (isinstance(error, AuthError) and not error.relogin_required
            and error.code == CODEX_RATE_LIMITED_CODE)


# Entitlement failures: __NEW_ORG__ gets a Portal-aware message; other providers a fixed generic one (or
...
def format_auth_error(error: Exception) -> str:
    ...
    if error.relogin_required:
        return f"{error} Run `anika model` to re-authenticate."
...
# ── Auth Store — persistence layer for ~/.anika/auth.json ──────────────────────────────────────────

def _auth_file_path() -> Path:
    path = get_anika_home() / "auth.json"
    # Seat belt: under pytest, refuse to touch the real user's auth store (tests that forgot to
    # monkeypatch ANIKA_HOME or escaped the hermetic conftest). In production: one dict lookup.
    if (os.environ.get("PYTEST_CURRENT_TEST")
            and _same_path(path, Path.home() / ".anika" / "auth.json")):
        raise RuntimeError(
            f"Refusing to touch real user auth store during test run: {path}. "
            "Set ANIKA_HOME to a tmp_path in your test fixture, or run "
...
def _global_auth_file_path() -> Optional[Path]:
    ...
    try:
        ...
        global_root = get_default_anika_root()
    except Exception:
        return None
    return None if _same_path(get_anika_home(), global_root) else global_root / "auth.json"
...
def _load_global_auth_store() -> Dict[str, Any]:
    ...
    if os.environ.get("PYTEST_CURRENT_TEST") and os.environ.get("HOME"):
        real_root = Path(os.environ["HOME"]) / ".anika" / "auth.json"
        try:
...
def _file_lock(
    ...
    shared __NEW_ORG__ store) track reentrancy separately."""
...
def _save_auth_store(auth_store: Dict[str, Any], target_path: Optional[Path] = None) -> Path:
    ...
    # enforced); ignore failures. secure_parent_dir refuses to chmod /, top-level dirs, or the anika-agent
...
def mark_provider_active_if_unset(provider_id: str) -> None:
    """Set ``active_provider`` only when none is set yet: the first ``anika auth add`` credential must
...
def read_credential_pool(provider_id: Optional[str] = None) -> Dict[str, Any]:
    ...
    when the profile has zero entries for it (``anika auth add`` in the profile shadows global)."""
...
def write_credential_pool(
    ...
    *status_cleared_ids* were cleared deliberately (``anika auth reset``) and skip the
...
def nous_token_has_billing_scope() -> bool:
    """Return True if the currently-held __NEW_ORG__ token carries ``billing:manage``.
...
def _explicit_pool_entry_present(normalized: str) -> bool:
    """Pool rows from EXPLICIT Anika flows (manual add / device-code / PKCE) or live env keys;
...
# Set by Claude Code itself, not by the user explicitly configuring anthropic in Anika.
_IMPLICIT_ENV_VARS = frozenset({"CLAUDE_CODE_OAUTH_TOKEN"})
_EXPLICIT_POOL_SOURCES = frozenset({"device_code", "loopback_pkce", "anika_pkce", "manual"})
...
def _pool_entry_is_explicit(entry: Any) -> bool:
    """True for pool rows the user created via an explicit Anika flow (or a still-live env key)."""
...
def _keyless_provider_has_explicit_config(normalized: str) -> bool:
    """Vertex / Bedrock count as explicit when Anika-scoped routing config is present.
    ...
    Anika-scoped signals (VERTEX_PROJECT_ID / vertex.project_id / VERTEX_CREDENTIALS_PATH) count
    here."""
...
def is_provider_explicitly_configured(provider_id: str) -> bool:
    ...
    Anika-initiated flow, or Anika-scoped routing config for keyless cloud-SDK providers. Ambient
...
def clear_provider_auth(provider_id: Optional[str] = None) -> bool:
    ...
    ``anika logout``. Returns True if something was cleared."""
    with _auth_store_lock():
        auth_store = _load_auth_store()
...
def _get_config_hint_for_unknown_provider(provider_name: str) -> str:
    ...
    try:
        from anika_cli.config import validate_config_structure
        issues = validate_config_structure()
        if not issues:
            return ""
        lines = ["Config issue detected — run 'anika doctor' for full diagnostics:"]
        for ci in issues:
...
def _refuse_env_adoption_if_config_corrupt() -> None:
    ...
    try:
        from anika_cli.config import get_active_config_parse_failure
    ...
    raise AuthError(
        ...
        f"was saved next to it) or run anika setup.",
        code="corrupt_config")
...
def _openrouter_auto_detected(scoped_key_env: Callable[[str], str]) -> bool:
    ...
    `anika auth add openrouter` has no env var; without the pool check it is invisible to
    ...
    if any(has_usable_secret(scoped_key_env(v)) for v in ("OPENAI_API_KEY", "OPENROUTER_API_KEY")):
        return True
    try:
        # Auto-detect an OpenRouter credential added via `anika auth add openrouter` (manual pool entry, no
        # env var). Without this, a key that only lives in the credential pool is invisible to
        # auto-detection — the user sees `anika auth list` showing the credential while requests go out
...
def _env_key_auto_detected(
    ...
    preempts a logged-in OAuth provider so a stale key in ~/.anika/.env never switches silently."""
            ...
            if has_usable_secret(scoped_key_env(env_var)):
                if oauth_active and oauth_active != pid:
                    logger.warning(
                        ...
                        # ~/.anika/.env isn't silently switched without knowing why.
...
def resolve_provider(
    ...
    *,
    ...
    if normalized != "auto":
        ...
        tail = (f"\n\n{hint}" if hint else " Check 'anika model' for available providers, "
                "or run 'anika doctor' to diagnose config issues.")
    ...
    raise AuthError(
        "No inference provider configured. Run 'anika model' to choose a "
        "provider and model, or set an API key (OPENROUTER_API_KEY, "
        "OPENAI_API_KEY, etc.) in ~/.anika/.env.",
...
def _quarantine_flat_oauth_state(state: Dict[str, Any], provider: str, exc: "AuthError") -> None:
    ...
    subsequent calls fail fast without a network retry (mirrors the __NEW_ORG__ / xAI / Codex pattern)."""
...
# Valid __NEW_ORG__ Portal hosts; a stored portal_base_url outside this set is a misconfiguration and falls
...
_NOUS_PORTAL_ALLOWED_HOSTS: FrozenSet[str] = frozenset({
    "portal.__NEW_DOMAIN__", "localhost", "127.0.0.1"})
...
def _nous_portal_base_url(state: Dict[str, Any]) -> str:
    """ANIKA_PORTAL_BASE_URL / NOUS_PORTAL_BASE_URL is the trusted operator override and wins
...
def resolve_nous_access_token(
    *,
    ...
    """Resolve a refresh-aware __NEW_ORG__ Portal access token for managed tool gateways."""
    ...
    with _provider_state_transaction("nous") as (auth_store, state, state_source_path):
        if not state:
            raise _nous_err("Anika is not logged into __NEW_ORG__ Portal.", relogin=True)
        ...
        with _nous_shared_store_lock(timeout_seconds=lock_timeout):
            ...
            if not isinstance(access_token, str) or not access_token:
                raise _nous_err("No access token found for __NEW_ORG__ Portal login.", relogin=True)
...
def invalidate_nous_auth_status_cache() -> None:
    """Clear the get_nous_auth_status() memo (for code paths that mutate __NEW_ORG__ auth state without
...
def get_nous_auth_status() -> Dict[str, Any]:
    """Status snapshot for __NEW_ORG__ auth, memoised ~15s keyed on the auth.json mtime.
...
class OAuthProviderFlow:
    ...
    ``monkeypatch.setattr("anika_cli.auth.resolve_codex_runtime_credentials", ...)`` applies."""
    provider_id: str
    resolve_fn: str
    status_fn: str
    terminal_refresh_codes: FrozenSet[str] = frozenset()  # retrying the same refresh token cannot succeed
    # ``anika logout`` with no active provider falls back to config.yaml ``model.provider`` only
...
def _provider_is_keyless(provider_id: str) -> bool:
    """AnikaOverlay keyless flag — the same source the provider catalog and GUI contract tests use."""
    try:
        from anika_cli.providers import ANIKA_OVERLAYS
        return bool(getattr(ANIKA_OVERLAYS.get(provider_id), "keyless", False))
    except Exception:
        return False
...
def _external_process_auth_evidence(provider_id: str) -> tuple[bool, Optional[str]]:
    ...
    Anika can't read). Deliberately subprocess-free: spawning ``gh auth token`` from status
    ...
    try:
        from anika_cli.copilot_auth import COPILOT_ENV_VARS, validate_copilot_token
...
def _external_process_spec(
    ...
    Launch details come from the provider's own profile (copilot-acp: ANIKA_COPILOT_ACP_COMMAND /
    COPILOT_CLI_PATH / ANIKA_COPILOT_ACP_ARGS), so out-of-tree providers describe their binary."""
...
def get_auth_status(provider_id: Optional[str] = None) -> Dict[str, Any]:
    ...
    tests that patch ``anika_cli.auth.get_*_auth_status`` still apply."""
...
def _get_azure_foundry_auth_status() -> Dict[str, Any]:
    ...
    CLI startup flat; ``anika doctor`` runs the live probe). ``api_key`` (default): usable
    ``AZURE_FOUNDRY_API_KEY``."""
    info: Dict[str, Any] = {"provider": "azure-foundry"}
    try:
        from anika_cli.config import load_config, get_env_value_prefer_dotenv
        cfg = load_config()
    except Exception:
        cfg = {}
        ...
        try:
            ...
            info.update(
                ...
                hint=(
                    ...
                    "is skipped here. Run `anika doctor` to verify token acquisition."
                ) if installed else (
                    "azure-identity not installed. Install with: "
                    "pip install azure-identity  (or rely on Anika' "
...
def _update_config_for_provider(
    ...
    *default_model*, when given, is written as ``model.default`` in the same step so the gateway
    ...
    from anika_cli.config import clear_model_endpoint_credentials
...
def login_command(args) -> None:
    """Deprecated: use 'anika model' or 'anika setup' instead."""
    print("The 'anika login' command has been removed.\nUse 'anika auth' to manage credentials,\n"
          "'anika model' to select a provider, or 'anika setup' for full setup.")
    raise SystemExit(0)
...
def logout_command(args) -> None:
    ...
    if not should_reset_config:
        ...
        print("Anika will use OpenRouter for inference.")
    else:
        print("Run `anika model` or configure an API key to use Anika.")
...
_PLUGIN_COMPAT_LAZY = {
    'CODEX_OAUTH_USER_AGENT': ('anika_cli.auth_constants', 'CODEX_OAUTH_USER_AGENT'),
    'CODEX_QUOTA_PROBE_MIN_INTERVAL_SECONDS': ('anika_cli.auth_codex', 'CODEX_QUOTA_PROBE_MIN_INTERVAL_SECONDS'),
    'DEFAULT_SPOTIFY_REDIRECT_URI': ('anika_cli.auth_constants', 'DEFAULT_SPOTIFY_REDIRECT_URI'),
    'DEVICE_AUTH_POLL_INTERVAL_CAP_SECONDS': ('anika_cli.auth_constants', 'DEVICE_AUTH_POLL_INTERVAL_CAP_SECONDS'),
    'MINIMAX_OAUTH_GRANT_TYPE': ('anika_cli.auth_constants', 'MINIMAX_OAUTH_GRANT_TYPE'),
    'NOUS_INFERENCE_INVOKE_SCOPE': ('anika_cli.auth_constants', 'NOUS_INFERENCE_INVOKE_SCOPE'),
    'NOUS_SHARED_STORE_FILENAME': ('anika_cli.auth_nous', 'NOUS_SHARED_STORE_FILENAME'),
    'OAUTH_OVER_SSH_DOCS_URL': ('anika_cli.auth_constants', 'OAUTH_OVER_SSH_DOCS_URL'),
    'QWEN_OAUTH_CLIENT_ID': ('anika_cli.auth_constants', 'QWEN_OAUTH_CLIENT_ID'),
    'QWEN_OAUTH_TOKEN_URL': ('anika_cli.auth_constants', 'QWEN_OAUTH_TOKEN_URL'),
    'SINGLE_USE_OAUTH_SINGLETON_FILES': ('anika_cli.auth_oauth_grants', 'SINGLE_USE_OAUTH_SINGLETON_FILES'),
    'SPOTIFY_ACCESS_TOKEN_REFRESH_SKEW_SECONDS': ('anika_cli.auth_constants', 'SPOTIFY_ACCESS_TOKEN_REFRESH_SKEW_SECONDS'),
    'SPOTIFY_DASHBOARD_URL': ('anika_cli.auth_constants', 'SPOTIFY_DASHBOARD_URL'),
    'XAI_OAUTH_DEVICE_CODE_URL': ('anika_cli.auth_constants', 'XAI_OAUTH_DEVICE_CODE_URL'),
    'XAI_OAUTH_DISCOVERY_URL': ('anika_cli.auth_constants', 'XAI_OAUTH_DISCOVERY_URL'),
    'XAI_OAUTH_ISSUER': ('anika_cli.auth_constants', 'XAI_OAUTH_ISSUER'),
    'refresh_nous_oauth_pure': ('anika_cli.auth_nous', 'refresh_nous_oauth_pure'),
}
