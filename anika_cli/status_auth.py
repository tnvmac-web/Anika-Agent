"""Credential sections of `anika status`, run through ``status._SECTIONS`` with its shared context.
Origin helpers (``_row``, ``_first_env_value``, ...) are resolved through the ``anika_cli.status``
...
_OAUTH_BLOCKS = (
    ...
    ("OpenAI Codex", "get_codex_auth_status", "anika model", _FILE_REFRESH_ROWS),
    ...
    ("MiniMax OAuth", "get_minimax_oauth_auth_status", "anika auth add minimax-oauth", (
        ("Region:", "region", None, True),
        ("Access exp:", "expires_at", None, None),
        ("Error:", "error", None, False))),
    ("xAI OAuth", "get_xai_oauth_auth_status", "anika auth add xai-oauth", _FILE_REFRESH_ROWS))
...
# __NEW_ORG__ Tool Gateway per-feature state: first matching (predicate(feature, nous_auth), text(feature)).
_FEATURE_STATES = (
    (lambda f, _: f.managed_by_nous, lambda f: "active via __NEW_ORG__ subscription"),
...
def _render_auth_providers(ctx):
    ...
    try:
        # Read-only display: the refresh-free snapshot, so `anika status` never performs an OAuth
    ...
    _status._row("__NEW_ORG__ Portal", logged_in,
         "logged in" if logged_in else "not logged in (__NEW_ORG__ inference key configured)" if inference
         else "not logged in (run: anika portal)")
...
def _render_nous_gateway(ctx):
    if managed_nous_tools_enabled():
        ...
        _status._section("__NEW_ORG__ Tool Gateway")
        print("  __NEW_ORG__ Portal   ✓ managed tools available" if features.nous_auth_present
              else "  __NEW_ORG__ Portal   ✗ not logged in")
        ...
        # __NEW_ORG__ OAuth without entitlement, or an opaque inference key without Portal account
...
def _render_apikey_providers(ctx):
    ...
    for pname, env_vars in _APIKEY_PROVIDERS.items():
        ...
        _status._row(pname, configured, "configured" if configured else "not configured (run: anika model)", 16, " ")
    ...
    if _status._effective_provider_label() == "LM Studio":
        from anika_cli.models_local import probe_lmstudio_models
...
import anika_cli.status as _status  # noqa: E402  (bottom: anika_cli.status imports this module)
