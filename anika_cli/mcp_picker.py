"""MCP picker — interactive `anika mcp picker` (also the default `anika mcp`)."""
...
from anika_cli.colors import Colors
from anika_cli.cli_output import prompt_yes_no
from anika_cli.curses_ui import curses_single_select
from anika_cli.mcp_catalog import (
...
def _enable_disable(name: str, *, enable: bool) -> None:
    ...
    _say(
        ...
        "Start a new Anika session for changes to take effect."
    )
...
def _configure_tools(name: str) -> None:
    ...
    from anika_cli.mcp_config import cmd_mcp_configure
...
def _print_rows_text(rows: List[_Row]) -> None:
    """Plain-text catalog dump: `anika mcp catalog` output and the non-curses fallback."""
    print()
    if not rows:
    ...
    print()
    _say("  Install: anika mcp install <name>    Picker: anika mcp", Colors.DIM)
    # Manifest-version warnings: the user's Anika is too old to install everything listed.
    future = [d for d in catalog_diagnostics() if d[1] == "future_manifest"]
    if future:
        print()
        for name, _, _msg in future:
            _say(f"  ⚠ '{name}' requires a newer Anika — run `anika update` to install this entry.", Colors.YELLOW)
        print()
    print()


def show_catalog() -> None:
    """`anika mcp catalog` — print the curated list + custom servers, no interaction."""
    _print_rows_text(_build_rows())


def run_picker() -> None:
    """`anika mcp picker` (and default `anika mcp`) — interactive selector; re-renders after each
    action until ESC/q."""
    while True:
        rows = _build_rows()
...
def install_by_name(identifier: str) -> int:
    """`anika mcp install <name>` — non-interactive entry-point."""
    from anika_cli.mcp_catalog import get_entry

    entry = get_entry(identifier)
    if entry is None:
        _say(
            f"  ✗ '{identifier}' is not in the catalog. Run `anika mcp catalog` to see available entries.",
            Colors.RED,
        )
        return 1
