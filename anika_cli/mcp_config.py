"""MCP Server Management CLI — ``anika mcp`` subcommand."""
...
from anika_cli.config import (
    ...
    get_env_value,
    save_env_value,
    get_anika_home,  # noqa: F401 — used by test mocks
)
...
def _resolve_mcp_server_config(config: dict) -> dict:
    ...
    Mirrors ``_load_mcp_config()`` in ``tools/mcp_tool.py``: load ``~/.anika/.env`` into ``os.environ`` and
...
def _oauth_tokens_present(name: str) -> bool:
    ...
    try:
        ...
        return AnikaTokenStorage(name).has_cached_tokens()
...
def cmd_mcp_add(args):
    ...
    # --command uses dest="mcp_command" (see anika_cli/main.py for why the dest is renamed).
    ...
    if not url and not command:
        ...
        _info("Examples:")
        _info('  anika mcp add ink --url "https://mcp.ml.ink/mcp"')
        _info('  anika mcp add github --command npx --args @modelcontextprotocol/server-github')
        _info('  anika mcp add myserver --preset mypreset')
        return
    ...
    try:
        ...
        if _confirm("Save config anyway (you can test later)?", default=False):
            ...
            if _save_mcp_server(name, server_config):
                ...
                _info("Fix the issue, then: anika mcp test " + name)
        return
    ...
    if _save_mcp_server(name, server_config):
        print()
        _success(
            f"Saved '{name}' to {display_anika_home()}/config.yaml ({tool_count}/{len(tools)} tools enabled)"
        )
...
def cmd_mcp_remove(args):
    ...
    # an earlier `anika mcp test`) is evicted too.
    try:
        from tools.mcp_oauth_manager import get_manager
...
def cmd_mcp_list(args=None):
    ...
    if not servers:
        ...
        print()
        _info("Add one with:")
        _info('  anika mcp add <name> --url <endpoint>')
        _info('  anika mcp add <name> --command <cmd> --args <args...>')
        print()
        return
...
def _reauth_oauth_server(name: str, server_config: dict) -> bool:
    ...
    if server_config.get("auth") != "oauth":
        ...
        _info("Use `anika mcp remove` + `anika mcp add` to reconfigure auth.")
        return False
    ...
    # re-auth path in web_server.py. force_interactive_oauth: `anika mcp login` is explicitly
    ...
    try:
        ...
        if not _oauth_tokens_present(name):
            ...
            print()
            _info("Then re-run `anika mcp login " + name + "`.")
            return False
        if tools:
...
def cmd_mcp_reauth(args):
    ...
    if not name:
        ...
        _info("Usage: anika mcp reauth <name>   |   anika mcp reauth --all")
        return
...
def cmd_mcp_configure(args):
    ...
    if not _sys.stdin.isatty():
        print("Error: 'anika mcp configure' requires an interactive terminal.", file=_sys.stderr)
        _sys.exit(1)
    name = args.name
    cfg = _lookup_server(name, _get_mcp_servers(), "Available")
    if cfg is None:
...
_MCP_USAGE = (
    "anika mcp                                    Open the catalog picker (default)",
    "anika mcp catalog                            List Nous-approved MCPs",
    "anika mcp install <name>                     Install a catalog MCP",
    "anika mcp serve                              Run as MCP server",
    "anika mcp add <name> --url <endpoint>        Add a custom MCP server",
    "anika mcp add <name> --command <cmd>         Add a stdio server",
    "anika mcp add <name> --preset <preset>       Add from a known preset",
    "anika mcp remove <name>                      Remove a server",
    "anika mcp list                               List configured servers",
    "anika mcp test <name>                        Test connection",
    "anika mcp configure <name>                   Toggle tools",
    "anika mcp login <name>                       Re-authenticate OAuth",
    "anika mcp reauth <name> | --all              Re-auth one or all OAuth servers",
)


def mcp_command(args):
    """Main dispatcher for ``anika mcp`` subcommands."""
    ...
    if action in ("picker", "catalog", "install"):
        ...
        from anika_cli import mcp_picker
    ...
    if handler:
        handler(args)
        return
    # No subcommand — drop the user into the catalog picker (same UX as `anika plugin`).
    from anika_cli.mcp_picker import run_picker
    run_picker()
