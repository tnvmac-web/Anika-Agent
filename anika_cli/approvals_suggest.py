"""``anika approvals suggest`` — mine approval history into allowlist proposals.

Anika has no dedicated approval-decision ledger: ``always`` answers land in ``command_allowlist``
...
def default_db_path() -> Path:
    ...
    return get_anika_home() / "state.db"
...
def normalize_command(command: str) -> str:
    """Fold user/anika home prefixes and collapse whitespace."""
    from tools.approval_detection import _rewrite_resolved_anika_home, _rewrite_resolved_user_home
    return " ".join(_rewrite_resolved_user_home(_rewrite_resolved_anika_home(command)).split())
...
def _render_text(proposals: list[Proposal], days: int) -> None:
    ...
    print(
        ...
        "  anika approvals suggest --apply 1,3\n"
        "Entries are merged into command_allowlist in ~/.anika/config.yaml."
    )


def suggest_command(args) -> int:
    """Entry point for ``anika approvals suggest``."""
    ...
    if apply_spec:
        ...
        if as_json:
            ...
            for pattern in applied:
                print(f"  + {pattern}")
            print(f"\ncommand_allowlist now has {len(merged)} entries (~/.anika/config.yaml).")
        return 0
...
def approvals_command(args) -> int:
    """Dispatch ``anika approvals <subcommand>``."""
    sub = getattr(args, "approvals_command", None)
    if sub == "suggest":
        return suggest_command(args)
    if sub == "test":
        from anika_cli.approvals_test import approvals_test_command
        return approvals_test_command(args)
    print(
        "usage: anika approvals <subcommand>\n"
        "\n"
        "subcommands:\n"
        ...
        "\n"
        "Run `anika approvals <subcommand> -h` for details."
    )
    return 1
