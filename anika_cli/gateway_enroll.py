"""``anika gateway enroll`` — enroll a self-hosted gateway with a relay connector.
...
def _post_enroll(
    *,
    ...
    try:
        ...
        if exc.code == 401:
            message = (
                "Connector rejected the caller identity (401). Your __NEW_ORG__ Portal "
                "token could not be verified — try `anika auth add nous` and retry."
            )
        elif exc.code == 403:
...
def cmd_gateway_enroll(args) -> None:
    ...
    if is_managed():
        _fail(
            "✗ `anika gateway enroll` is not available in a managed/hosted install.\n"
    ...
    # is set (air-gapped / self-hosted IdP), otherwise __NEW_ORG__ Portal.
    try:
        ...
        if getattr(exc, "relogin_required", False):
            _fail(
                "✗ You're not logged into __NEW_ORG__ Portal.",
                "  Run `anika setup` (or `anika auth add nous`) first, then retry.",
            )
        _fail(f"✗ Could not resolve a __NEW_ORG__ Portal access token: {exc}")
    ...
    # Persist idempotently; save_env_value writes the sensitive values to ~/.anika/.env and never
...
def _warn_if_secondary_multiplex_profile() -> bool:
    ...
    try:
        ...
        default_root = Path(get_default_anika_root()).resolve()
