...
from anika_cli.auth import AuthError, resolve_provider
from anika_cli.colors import Colors, color
from anika_cli.config import get_env_path, get_env_value, get_anika_home, load_config
from anika_cli.models import provider_label
from anika_cli.runtime_provider import resolve_requested_provider
from anika_cli.vercel_auth import describe_vercel_auth
from anika_cli.status_auth import (  # renderers wired into _SECTIONS below
    _render_api_keys, _render_apikey_providers, _render_auth_providers, _render_nous_gateway)
from anika_constants import OPENROUTER_MODELS_URL
...
def _estop_status_line():
    """One-line pause banner for `anika status`, or None when not paused."""
    try:
    ...
    return f"⏸️  PAUSED (global emergency stop{f' — reason: {reason}' if reason else ''}; `anika resume` to lift)"
...
def _render_terminal(ctx):
    ...
    if terminal_env in _TERMINAL_ENV_ROWS:
        ...
        _kv_flag("SDK:", importlib.util.find_spec("vercel") is not None, "installed",
                 "missing (install: pip install 'anika-agent[vercel]')")
        ...
        # (fail-soft — never break `anika status`).
        try:
...
def _render_gateway(ctx):
    ...
    try:
        from anika_cli.gateway import get_gateway_runtime_snapshot, _format_gateway_pids
...
def _render_cron(ctx):
    ...
    jobs_file = get_anika_home() / "cron" / "jobs.json"
...
def _render_sessions(ctx):
    ...
    if gateway_rows:
        ...
        if freshest > 0:
            from anika_cli.timefmt import relative_time
            print(f"  Last activity:{relative_time(freshest):>13}")
    elif not (sessions_file := get_anika_home() / "sessions" / "sessions.json").exists():
        _kv("Active:", 0)
    else:
        try:
    ...
    try:
        from anika_cli.active_sessions import (
...
def _render_footer(ctx):
    _banner(("─" * 60, "  Run 'anika doctor' for detailed diagnostics", "  Run 'anika setup' to configure"),
            Colors.DIM)
    print()


# Print order of `anika status`; each renderer takes the shared _StatusContext.
_SECTIONS = (
...
def show_status(args):
    """Show status of all Anika Agent components."""
    # Shared by section renderers: config, --deep, and the __NEW_ORG__ login facts Auth Providers derives
    # for the later __NEW_ORG__ Tool Gateway section.
...
_PLUGIN_COMPAT_LAZY = {
    ...
    'get_nous_portal_account_info': ('anika_cli.nous_account', 'get_nous_portal_account_info'),
