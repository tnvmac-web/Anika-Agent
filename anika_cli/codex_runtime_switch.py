...
_ARG_SYNONYMS = {
    ...
    "off": "auto", "default": "auto", "disable": "auto", "anika": "auto"}

_ANIKA_TOOLS_CALLBACK_NOTE = (
    "Anika tool callback registered: codex can now use "
    "web_search, web_extract, browser_*, vision_analyze, "
    "image_generate, skill_view, skills_list, text_to_speech, "
    "kanban_* (worker + orchestrator) via MCP.",
    "  (delegate_task, memory, session_search, todo run "
    "only on the default Anika runtime — they need the "
    "agent loop context.)")
...
def _migration_lines(config: dict) -> list[str]:
    ...
    lines: list[str] = []
    try:
        from anika_cli.codex_runtime_plugin_migration import migrate
        mig_report = migrate(config)
        # The anika-tools callback is internal plumbing — surfaced separately below.
        user_servers = [s for s in mig_report.migrated if s != "anika-tools"]
        if user_servers:
        ...
        if "anika-tools" in mig_report.migrated:
            lines.extend(_ANIKA_TOOLS_CALLBACK_NOTE)
...
def apply(
    ...
    # correct but the world state (managed block in ~/.codex/config.toml, anika-tools MCP
    ...
    if new_value == "codex_app_server":
        ...
        # Migrate Anika' MCP servers + Codex's curated plugins into ~/.codex/config.toml so the
        # spawned codex subprocess sees the same tool surface AND can call back into Anika.
        ...
        msg_lines.append(
            ...
            "Anika tools available via MCP callback).")
        msg_lines.append(
            "Effective on next session — current cached agent keeps "
            "the prior runtime to preserve prompt cache.")
    else:
        msg_lines.append("OpenAI/Codex turns will use the default Anika runtime.")
