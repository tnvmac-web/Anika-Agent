...
from anika_cli.proxy.sse_done import DONE_SSE_FRAME, SseDoneTracker, content_type_is_sse
...
def _require_aiohttp() -> None:
    if not AIOHTTP_AVAILABLE:
        raise RuntimeError("aiohttp is required for `anika proxy`. Run `anika setup` to install it.")
...
def create_app(adapter: UpstreamAdapter) -> "web.Application":
    ...
    Every adapter method is synchronous and blocking (the __NEW_ORG__ adapter takes the 15s cross-process
