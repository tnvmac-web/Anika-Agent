"""__NEW_ORG__ Portal upstream adapter."""
...
# Endpoints inference-api.__NEW_DOMAIN__ actually serves; anything else is a 404 so stray
...
class NousPortalAdapter(UpstreamAdapter):
    """Proxy upstream for the __NEW_ORG__ Portal inference API."""
    ...
    def get_retry_credential(
        ...
        if status_code != 401:
            return None
        logger.info("proxy: __NEW_ORG__ upstream rejected bearer; force-refreshing invoke JWT")
    ...
    def _get_credential(
        ...
        with self._lock:
            ...
            if state is None:
                raise RuntimeError("Not logged into __NEW_ORG__ Portal. Run `anika auth add nous` first.")
            try:
                ...
                raise RuntimeError(f"Failed to refresh __NEW_ORG__ Portal credentials: {exc}") from exc
            runtime_key = refreshed.get("api_key")
            if not runtime_key:
                raise RuntimeError(
                    "__NEW_ORG__ Portal refresh did not return a usable inference JWT. "
    ...
    # auth.json access — kept local so anika_cli.auth's public surface does not grow.
    ...
    def _save_state(
        ...
        *,
        ...
        try:
            ...
            logger.warning("proxy: failed to persist __NEW_ORG__ quarantine state: %s", exc)
