...
def build_logs_parser(subparsers, *, cmd_logs: Callable) -> None:
    ...
    logs_parser = subparsers.add_parser(
        "logs", help="View and filter Anika log files",
        description="View, tail, and filter agent.log / errors.log / gateway.log / gui.log / desktop.log",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    anika logs                    Show last 50 lines of agent.log
    anika logs -f                 Follow agent.log in real time
    anika logs errors             Show last 50 lines of errors.log
    anika logs gateway -n 100     Show last 100 lines of gateway.log
    anika logs gui -f             Follow gui.log in real time
    anika logs desktop -f         Follow desktop.log (Electron app boot/backend)
    anika logs --level WARNING    Only show WARNING and above
    anika logs --session abc123   Filter by session ID
    anika logs --component tools  Only show tool-related lines
    anika logs --since 1h         Lines from the last hour
    anika logs --since 30m -f     Follow, starting from 30 min ago
    anika logs list               List available log files with sizes
""")
