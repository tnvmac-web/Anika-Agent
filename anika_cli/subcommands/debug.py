...
def build_debug_parser(subparsers, *, cmd_debug: Callable) -> None:
    ...
    debug_parser = subparsers.add_parser(
        ...
        description="Debug utilities for Anika Agent. Use 'anika debug share' to "
        "upload a debug report (system info + recent logs) to a paste "
        "service and get a shareable URL.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    anika debug share              Upload debug report (asks for confirmation)
    anika debug share --yes        Skip confirmation (for scripts/CI)
    anika debug share --lines 500  Include more log lines
    anika debug share --expire 30  Keep paste for 30 days
    anika debug share --local      Print report locally (no upload)
    anika debug share --no-redact  Disable upload-time secret redaction
    anika debug share --nous       Upload to Nous-internal storage (private)
    anika debug delete <url>       Delete a previously uploaded paste
""")
    ...
    share_parser.add_argument(
        ...
        help="Upload the debug bundle to Nous-internal storage (AWS S3) instead "
            ...
            "by __NEW_ORG__ staff (and allowlisted Discord mods) via a Google-login-"
            "gated viewer — and auto-deletes after 14 days. Still force-redacts "
            "secrets unless --no-redact is also passed.")
    delete_parser = debug_sub.add_parser(
        "delete", help="Delete a paste uploaded by 'anika debug share'")
