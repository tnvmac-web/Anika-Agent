"""``anika gateway`` and ``anika proxy`` subcommand parsers."""
...
from anika_cli.subcommands._shared import add_accept_hooks_flag
...
def build_gateway_parser(
    ...
    _flag(gateway_run, "--no-supervise",
        help="Inside the s6-overlay Docker image, normally `gateway run` is "
            ...
            "if ANIKA_DASHBOARD is set). Pass --no-supervise to opt out and "
    ...
    gateway_migrate_legacy = gateway_subparsers.add_parser(
        "migrate-legacy", help="Remove legacy anika.service units from pre-rename installs",
        description="Stop, disable, and remove legacy Anika gateway unit files "
            "(e.g. anika.service) left over from older installs. Profile "
            "units (anika-gateway-<profile>.service) and unrelated "
    ...
    gateway_enroll = gateway_subparsers.add_parser("enroll",
        ...
        description="Redeem a single-use enrollment token with a relay connector. "
            "Authenticates as your __NEW_ORG__ Portal account (the connector derives the "
            ...
            "GATEWAY_RELAY_SECRET / GATEWAY_RELAY_DELIVERY_KEY into ~/.anika/.env. "
            "Requires being logged in (anika setup). Not available in managed installs.")
    ...
    gateway_enroll.add_argument("--wake-url", dest="wake_url", default=None,
        help="Phase 5 §5.2 wake URL: a reachable URL the connector pokes "
            ...
            "GATEWAY_RELAY_WAKE_URL in ~/.anika/.env and forwarded at provision. "
    ...
    proxy_parser = subparsers.add_parser(
        ...
        description="Run a local HTTP server that forwards OpenAI-compatible requests "
            "to an OAuth-authenticated provider (e.g. __NEW_ORG__ Portal). External "
    ...
    proxy_start.add_argument("--provider", default="nous",
        help="Upstream provider: nous or xai (default: nous). See `anika proxy providers`.")
