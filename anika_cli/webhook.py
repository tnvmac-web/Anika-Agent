"""anika webhook — manage dynamic webhook subscriptions from the CLI."""
...
def _subscriptions_path() -> Path:
    ...
    return get_anika_home() / _SUBSCRIPTIONS_FILENAME
...
def _setup_hint() -> str:
  ...
  Then start the gateway: anika gateway run
"""


def webhook_command(args):
    """Entry point for 'anika webhook' subcommand."""
    sub = getattr(args, "webhook_action", None)
    if not sub:
        print("Usage: anika webhook {subscribe|list|remove|test}")
        print("Run 'anika webhook --help' for details.")
        return
...
def _cmd_subscribe(args):
    ...
    print("  The gateway must be running to receive events (anika gateway run).\n")
...
def _cmd_list(args):
    ...
    if not subs:
        ...
        print("  Create one with: anika webhook subscribe <name>")
        return
...
def _cmd_test(args):
    ...
    payload = args.payload or '{"test": true, "event_type": "test", "message": "Hello from anika webhook test"}'
    ...
    try:
        ...
        print(f"  Error: {e}")
        print("  Is the gateway running? (anika gateway run)")
