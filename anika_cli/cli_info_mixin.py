...
Mixin split out of ``cli.py``; bound onto ``AnikaCLI`` via the MRO. cli.py-internal symbols are
...
from anika_constants import is_termux as _is_termux_environment
from rich.markup import escape as _escape
from utils import base_url_hostname

from anika_cli.cli_modal_mixin import _gated_confirm
...
class CLIInfoMixin:
    ...
    def show_banner(self):
        ...
        from anika_cli.banner import build_welcome_banner
        self.console.clear()
        ctx_len = None
        ...
        if ctx_len and ctx_len < MINIMUM_CONTEXT_LENGTH:
            ...
            self._console_print(
                f"[dim]   Anika needs at least {MINIMUM_CONTEXT_LENGTH:,} tokens. Tool schemas + system prompt use a large fixed prefix.[/]"
            )
        ...
        from anika_cli.model_switch import is_nous_anika_non_agentic
        if is_nous_anika_non_agentic(getattr(self, "model", "") or ""):
            self._console_print()
            self._console_print(
                "[bold yellow]⚠  __NEW_ORG__ Anika 3 & 4 models are NOT agentic and are not "
                "designed for use with Anika Agent.[/]")
            self._console_print(
        ...
        # `anika skills trust`. Never raises.
        try:
            ...
            if _proj_dirs:
                ...
                if _untrusted is not None:
                    ...
                    self._console_print(
                        ...
                        f"loaded — run `anika skills trust` to enable them.[/]")
        except Exception:
    ...
    def show_help(self, arg: str = ""):
        ...
        from anika_cli.commands import COMMANDS_BY_CATEGORY, HELP_SESSION_SUBGROUPS
        ...
        try:
            from anika_cli.skin_engine import get_active_help_header
    ...
    def handle_bang_shell(self, text: str) -> bool:
        ...
        from anika_cli.bang_shell import (
    ...
    def _show_gateway_status(self):
        ...
        try:
            ...
            print()
            print(f"  Configuration file: {display_anika_home()}/config.yaml")
            print()
            ...
            print(f"    2. Or configure settings in {display_anika_home()}/config.yaml")
            print()

    def _print_random_tip(self) -> None:
        """Best-effort discovery tip (startup + /clear); never raises."""
        try:
            from anika_cli.tips import get_random_tip
            _tip = get_random_tip()
            try:
    ...
    def _toggle_verbose(self):
        ...
        if getattr(self, "_focus_view_enabled", False):
            ...
            try:
                from anika_cli.focus_view import FOCUS_CONFIG_KEY
    ...
    def _usage_reset(self, force: bool = False):
        ...
        if str(self._agent_or_self("provider") or "").strip().lower() != "openai-codex":
            ...
            print("  Switch with `/model` or `anika auth` first.")
            return
    ...
    def _show_usage(self):
        """Rate limits + session token usage (when a live agent exists) + __NEW_ORG__ credits.

        The __NEW_ORG__ credits block is agent-independent (portal fetch), so it runs even with no live
    ...
    def _check_config_mcp_changes(self) -> None:
        ...
        from anika_cli.config import get_config_path as _get_config_path
    ...
    def _reload_skills(self) -> None:
        """Reload skills: rescan ~/.anika/skills/ and queue a note for the next user turn.
