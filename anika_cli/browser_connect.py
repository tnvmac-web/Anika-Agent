"""Shared helpers for attaching Anika to a local Chromium-family CDP port.
...
# the real profile into ``~/.anika/browser-profile/`` and launch the user's real binary on
...
def real_profile_copy_dir(browser: str) -> str:
    """Return the anika-owned snapshot dir for ``browser``'s real profile."""
    return str(get_anika_home() / "browser-profile" / browser)
...
def _secure_snapshot(path: str, *, contents: bool = False) -> None:
    ...
    NixOS group-share carve-out, ANIKA_UID/GID) as every Anika secret dir — via ``_secure_dir``/
    ...
    ``ANIKA_HOME_MODE`` hatch. Best-effort; never blocks a launch."""
    try:
        from anika_cli.config import _secure_dir, _secure_file
        if not contents:
            _secure_dir(path)
            return
...
_SNAPSHOT_DONE_MARKER = ".anika-snapshot-complete"
...
def _locked_profile_error(browser: str) -> str:
    ...
    if _real_profile_autoclose():
        msg = (
            ...
            "yet. Anika can close it for you (this quits the browser — you'll lose unsaved "
        ...
        msg = (
            ...
            "browser.use_real_profile off. (Enable browser.real_profile_autoclose to let Anika "
...
def snapshot_real_profile(browser: str, src: str | None = None) -> tuple[str | None, str | None]:
    """Snapshot ``browser``'s real ACTIVE profile into the anika copy dir; returns ``(dst, err)``.
...
def cleanup_real_profile_snapshots() -> None:
    ...
    root = str(get_anika_home() / "browser-profile")
...
def chrome_debug_data_dir() -> str:
    return str(get_anika_home() / "chrome-debug")
