...
resolved through the module object so test patches on ``anika_cli.setup.<name>`` take effect."""

import logging
from tools import tool_backend_helpers
from anika_cli import nous_subscription
...
_TTS_SUMMARY_ROWS = {
    ...
    "neutts": ("NeuTTS", "neutts", "run 'anika setup tts'"),
    "kittentts": ("KittenTTS", "kittentts", "run 'anika setup tts'")}
...
_STT_SUMMARY_DEFAULT = ("Local Whisper", "faster_whisper", "run 'anika tools' → Speech-to-Text")
...
_EDIT_WIZARD_ROWS = (
    ("anika setup", "          Re-run the full wizard"), ("anika setup model", "    Change model/provider"),
    ("anika setup terminal", " Change terminal backend"), ("anika setup gateway", "  Configure messaging"),
    ("anika setup tools", "    Configure tool providers"))
_EDIT_CONFIG_ROWS = (
    ("anika config", "         View current settings"), ("anika config edit", "    Open config in your editor"),
    ("anika config set <key> <value>", ""))
_READY_ROWS = (
    ("anika", "              Start chatting"), ("anika gateway", "      Start messaging gateway"),
    ("anika doctor", "       Check for issues"))
...
def _vision_row(config, feats):
    ...
    try:
        ...
        ok = False
    return ("Vision (image analysis)", ok, None if ok else "run 'anika setup' to configure")
...
def _web_row(config, feats):
    ...
    return _managed_or_provider_row(feats.web, "Web Search & Extract", "__NEW_ORG__ subscription", _WEB_MISSING)
...
def _browser_row(config, feats):
    ...
    return _managed_or_provider_row(feats.browser, "Browser Automation", "__NEW_ORG__ Browser Use", hint)
...
def _image_gen_row(config, feats):
    ...
    if feats.image_gen.managed_by_nous:
        return ("Image Generation (__NEW_ORG__ subscription)", True, None)
...
def _video_gen_row(config, feats):
    # Opt-in via `anika tools` → Video Generation. Only show the row when a plugin reports
    # available so we don't badger users who don't care about video gen with a "missing" line.
    if feats.video_gen.managed_by_nous:
        return ("Video Generation (FAL via __NEW_ORG__ subscription)", True, None)
...
def _tts_row(config, feats):
    ...
    if feats.tts.managed_by_nous:
        return ("Text-to-Speech (OpenAI via __NEW_ORG__ subscription)", True, None)
...
def _stt_row(config, feats):
    ...
    if stt_feature is not None and stt_feature.managed_by_nous:
        return ("Speech-to-Text (OpenAI via __NEW_ORG__ subscription)", True, None)
...
def _modal_row(config, feats):
    if feats.modal.managed_by_nous:
        return ("Modal Execution (__NEW_ORG__ subscription)", True, None)
    if _setup.cfg_get(config, "terminal", "backend") == "modal":
        ...
        return ("Modal Execution", False, "run 'anika setup terminal'")
    if tool_backend_helpers.managed_nous_tools_enabled() and feats.nous_auth_present:
        return ("Modal Execution (optional via __NEW_ORG__ subscription)", True, None)
    return None
...
def _spotify_row(config, feats):
    # OAuth via anika auth spotify — check auth.json, not env vars
    try:
...
def _print_setup_summary(config: dict, anika_home):
    ...
    try:
        from anika_cli.auth import resolve_provider
        resolve_provider()
    except Exception:
        print()
        _setup.print_warning("No inference provider is configured — Anika cannot chat yet.")
        _setup._info("  Finish this one step with either of:",
              "    anika model            (pick any provider/model)",
              "    anika setup --portal   (__NEW_ORG__ Portal OAuth, no API key)")
    ...
    if available_count < len(tool_status):
        _setup.print_warning("Some tools are disabled. Run 'anika setup tools' to configure them,")
    ...
    for label, value in (("Settings:", f"  {_setup.get_config_path()}"), ("API Keys:", f"  {_setup.get_env_path()}"),
                         ("Data:", f"      {anika_home}/cron/, sessions/, logs/")):
...
import anika_cli.setup as _setup  # noqa: E402  (bottom: anika_cli.setup imports this module)
