"""Anika-managed uv and Python runtime repair.

The Python backing the install is shared by every Anika profile because the checkout's ``venv``
is shared. Runtime repair therefore uses an install-scoped store under
``<checkout>/.anika-runtime/python``. A vulnerable interpreter is never reinstalled in place.
"""
...
from anika_cli.sqlite_runtime import (
    SQLiteRuntimeInfo, isolated_interpreter_env, probe_sqlite_runtime)

logger = logging.getLogger(__name__)

_PROJECT_ROOT = Path(__file__).resolve().parents[1]
_RUNTIME_DIR_NAME = ".anika-runtime"
_VENV_NAME = "venv"
_ALT_VENV_NAME = ".venv"
_REPAIR_LOCK_NAME = "runtime-repair.lock"
_MACOS_MANAGED_PYTHON_IDENTIFIER = "com.__NEW_ORG__.anika.managed-python"

_Provisioned = tuple[Path, Path, SQLiteRuntimeInfo]


def managed_uv_path() -> Path:
    """Path of Anika' own uv binary (``$ANIKA_HOME/bin/uv[.exe]``); may not exist yet."""
    return get_anika_home() / "bin" / ("uv.exe" if platform.system() == "Windows" else "uv")
...
def managed_python_env(
    ...
    """Return a sanitized environment for Anika-private uv Python commands."""
    target = (
...
def _report_runtime_repair_failure(repair: RuntimeRepairResult) -> None:
    if repair.backup_venv is None:
        ...
        print("    Sessions stay protected meanwhile: Anika keeps databases "
              "out of WAL mode on this SQLite build. The next `anika update` "
              "will retry.")
        return
...
def _uv_self_update_stamp() -> Path:
    from anika_constants import get_anika_home
    return get_anika_home() / "cache" / ".uv_self_update_stamp"


def _uv_self_update_is_fresh(now: float | None = None) -> bool:
    """True when ``uv self update`` ran recently enough to skip.

    uv releases roughly weekly while many users run ``anika update`` daily; a blocking network
...
def _reload_anika_constants():
    """Re-execute ``anika_constants`` from disk (the imported one may predate venv_python_path)."""
    import anika_constants
    return importlib.reload(anika_constants)
...
def _attempt_install_generation(
    ...
    try:
        ...
        return reject("uv resolved Python outside the Anika generation: %s", python)
...
def _install_safe_python_generation(
    ...
    # supported minor (e.g. 3.11 → 3.12) so the user isn't stuck on every `anika update`. The
...
def _windows_runtime_holders() -> tuple[bool, str]:
    if platform.system() != "Windows":
        return False, ""
    main_module = sys.modules.get("anika_cli.main")
    ...
    if holders:
        ...
        return True, f"other Anika processes still hold the venv (PID {pids})"
    return False, ""
...
def _windows_runtime_self_lock(live: Path) -> tuple[bool, str]:
    ...
    (``anika update`` itself runs from the venv python), which is correct for the dependency-sync
    ...
    updater's own ``python.exe`` (or a waiting ``anika.exe`` launcher ancestor) can never be renamed from
    ...
    # Belt-and-braces: the venv\Scripts\anika.exe launcher stays mapped while it waits for this
...
def _repair_windows_preflight(
    ...
    if self_locked:
        ...
        for line in (
            ...
            # See #93032.
            "    Retrying `anika update` from inside this venv cannot help: "
            ...
            f"      cd {root}",
            "      <system Python> -m anika_cli.main update",
            "    Sessions stay protected meanwhile: Anika keeps databases "
...
def _install_uv(target: Path) -> None:
    ...
    into ``$ANIKA_HOME/bin/`` instead of ``~/.local/bin/``.
    """
