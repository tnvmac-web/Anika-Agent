...
Split out of ``anika_cli.models``. OpenRouter and
__NEW_ORG__ Portal share one implementation parametrized by :class:`_CapsSource`; the per-source module
globals (``_openrouter_reasoning_caps_cache``, ``_nous_caps_disk_checked``, ...) stay defined on
``anika_cli.models`` — tests reset them there — and are read/written by attribute name.
...
logger = logging.getLogger("anika_cli.models")

Caps = dict[str, Optional[dict[str, Any]]]


def _origin():
    from anika_cli import models
    return models
...
# that must never block on HTTP — so without a disk copy, `anika -p`, a cron job, or a freshly
...
def _reasoning_caps_disk_path() -> Path:
    ...
    return get_anika_home() / "cache" / "reasoning_caps.json"


def _read_reasoning_caps_disk() -> dict[str, Any]:
    from anika_cli.models import _read_json_cache
...
def _save_reasoning_caps_disk(url: str, caps: Caps) -> None:
    ...
    from anika_cli.models import _write_json_cache
    try:
...
def _fetch_reasoning_caps_catalog(url: str, timeout: float) -> Optional[Caps]:
    ...
    m = _origin()
    try:
        req = urllib.request.Request(url, headers={"Accept": "application/json", "User-Agent": m._ANIKA_USER_AGENT})
...
# ── Per-source cache (OpenRouter, __NEW_ORG__ Portal) ─────────────────────────

@dataclass(frozen=True)
class _CapsSource:
    """One catalog's cache slots on ``anika_cli.models`` plus how to name its URL.
...
def refresh_reasoning_caps_async(provider: Optional[str]) -> None:
    ...
    """
    src = {"nous": _NOUS_CAPS, "nous-portal": _NOUS_CAPS, "__NEW_ORG__": _NOUS_CAPS,
...
# __NEW_ORG__ Portal serves OpenRouter's catalog schema, so the same parser and contract apply. Its own
...
def nous_model_reasoning_capabilities(
    ...
    """__NEW_ORG__ Portal counterpart of :func:`openrouter_model_reasoning_capabilities`; warm the cache
...
def warm_nous_reasoning_caps_async() -> None:
    """__NEW_ORG__ Portal counterpart of :func:`warm_openrouter_reasoning_caps_async`."""
