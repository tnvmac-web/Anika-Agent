...
Custom-provider lookup lives in :mod:`anika_cli.runtime_provider_custom`; Azure Foundry,
OpenRouter/bare-custom, Bedrock and external-process builders in
:mod:`anika_cli.runtime_provider_backends` — both re-exported here so
``anika_cli.runtime_provider.<name>`` imports and test patches keep working."""
...
from anika_cli.auth import (  # resolve_external_process_provider_credentials is read via origin by runtime_provider_backends
...
from anika_cli import config as _config_mod
from anika_cli import models as _models  # attribute access keeps ``anika_cli.models.<name>`` patches effective
from anika_constants import OPENROUTER_BASE_URL
from anika_cli.providers import determine_api_mode, is_official_openai_host, nous_api_mode
from utils import base_url_host_matches, base_url_hostname, env_int


# Late-bound delegates, deliberately NOT module-level from-imports: this module is often imported
# lazily, so its first import can happen while a test has ``anika_cli.config.load_config`` patched
...
# Hosts that only speak one wire protocol. Mirrors host_mandated_api_mode in anika_cli/providers.py
...
def _api_key_provider_api_mode(provider: str, model_cfg: Dict[str, Any], api_key: str, base_url: str, effective_model: Any, *,
    ...
    if provider in ("xai", "actual"):
        ...
        # the host_mandated_api_mode clause in anika_cli/providers.py so the runtime resolver stays in
...
def _nous_min_key_ttl() -> int:
    return max(60, env_int("ANIKA_NOUS_MIN_KEY_TTL_SECONDS", 1800))


def _resolve_nous_creds() -> Dict[str, Any]:
    return resolve_nous_runtime_credentials(timeout_seconds=float(_getenv("ANIKA_NOUS_TIMEOUT_SECONDS", "15")))
...
def resolve_requested_provider(requested: Optional[str] = None) -> str:
    """Provider request from explicit arg, then config, then ``ANIKA_INFERENCE_PROVIDER``, else
    ...
    return _getenv("ANIKA_INFERENCE_PROVIDER", "").strip().lower() or "auto"


# ── extracted collaborators (re-exported; see module docstring) ────────────────────────────

from anika_cli.runtime_provider_custom import (  # noqa: E402,F401
...
from anika_cli.runtime_provider_backends import (  # noqa: E402,F401
...
def _refresh_nous_pool_entry(pool: CredentialPool, entry: Any, pool_api_key: str):
    """__NEW_ORG__ pool entries carry the agent_key (an invoke JWT) which the pool does not refresh on
    selection (avoids network calls in `anika auth list`); refresh here before falling back to
    ...
    logger.debug("__NEW_ORG__ pool entry agent_key expired/missing, refreshing selected pool entry")
    try:
        refreshed = pool.try_refresh_current()
    except Exception as exc:
        logger.debug("__NEW_ORG__ pool entry refresh failed: %s", exc)
        refreshed = None
    ...
    if not pool_api_key or not _nous_entry_key_usable(entry, min_ttl):
        logger.debug("__NEW_ORG__ pool entry agent_key still unavailable, falling through to runtime resolution")
        pool_api_key = ""
    return entry, pool_api_key
...
_OAUTH_RUNTIME_PROVIDERS: Dict[str, _OAuthRuntimeSpec] = {
    "nous": _OAuthRuntimeSpec(_resolve_nous_creds, nous_api_mode, "portal", "expires_at",
                              "Auto-detected __NEW_ORG__ provider but credentials failed"),
    "openai-codex": _OAuthRuntimeSpec(lambda: resolve_codex_runtime_credentials(), "codex_responses", "anika-auth-store",
                                      "last_refresh", "Auto-detected Codex provider but credentials failed"),
    "xai-oauth": _OAuthRuntimeSpec(lambda: resolve_xai_oauth_runtime_credentials(), "codex_responses", "anika-auth-store",
...
def _resolve_vertex_runtime(requested_provider: str) -> Dict[str, Any]:
    ...
    if not token or not base_url:
        raise AuthError("Vertex AI credentials could not be resolved. Vertex uses OAuth2 (not a static API key): provide a "
                        "service-account JSON via GOOGLE_APPLICATION_CREDENTIALS (or VERTEX_CREDENTIALS_PATH) in ~/.anika/.env, "
                        "or run 'gcloud auth application-default login' for ADC. Set the GCP project/region under vertex: in "
                        "config.yaml if they aren't embedded in the credentials. Run `anika setup` to install Vertex support.")
...
_PLUGIN_COMPAT_LAZY = {
    'custom_provider_aliases': ('anika_cli.providers', 'custom_provider_aliases'),
