...
The provider list shown by ``anika model`` (CLI/TUI) and the desktop Settings → Providers tabs
...
come from :data:`anika_cli.auth.PROVIDER_REGISTRY` (credential truth); ``display_name`` /
...
class ProviderDescriptor:
    ...
    order: int                     # CANONICAL_PROVIDERS index — mirrors `anika model`
...
def provider_catalog() -> list[ProviderDescriptor]:
    """One descriptor per provider in the ``anika model`` universe (:data:`CANONICAL_PROVIDERS`,
    ...
    PROVIDER_REGISTRY = _safe_import("anika_cli.auth", "PROVIDER_REGISTRY", {})
    OPTIONAL_ENV_VARS = _safe_import("anika_cli.config", "OPTIONAL_ENV_VARS", {})
    ...
    ANIKA_OVERLAYS = _safe_import("anika_cli.providers", "ANIKA_OVERLAYS", {})
    try:
