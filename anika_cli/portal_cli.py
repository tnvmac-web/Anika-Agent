"""``anika portal`` — the human-readable entry point for __NEW_ORG__ Portal."""
...
DEFAULT_PORTAL_URL = "https://portal.__NEW_DOMAIN__"
SUBSCRIPTION_URL = "https://portal.__NEW_DOMAIN__/manage-subscription"
DOCS_URL = "https://anika-agent.__NEW_DOMAIN__/docs/user-guide/features/tool-gateway"
...
def _cmd_status(args) -> int:
    ...
    if logged_in:
        ...
        print("  Login:   anika portal")

    # Provider selection (independent of auth)
    model_cfg = config.get("model") if isinstance(config.get("model"), dict) else {}
    provider = str(model_cfg.get("provider") or "").strip().lower()
    if provider == "nous":
        print(f"  Model:   {color('✓ using __NEW_ORG__ as inference provider', Colors.GREEN)}")
    elif provider:
        print(f"  Model:   currently {provider} (switch with `anika model`)")
    ...
    try:
        ...
        return 0
    rows = [(feat.label, _feature_state(feat, via_nous="via __NEW_ORG__ Portal")) for feat in features.items()]
...
def _cmd_tools(args) -> int:
    ...
    if not features.nous_auth_present:
        print(color("  Not logged into __NEW_ORG__ Portal — sign in with `anika portal`.", Colors.YELLOW))
        print()

    label_width = max(len(label) for _, label, _ in _CATALOG)
    for key, label, partner in _CATALOG:
        feat = features.features.get(key)
        state = color("unknown", Colors.DIM) if feat is None else _feature_state(feat, via_nous="✓ via __NEW_ORG__ Portal")
...
def _cmd_login(args) -> int:
    """One-shot __NEW_ORG__ Portal onboarding (login + model + provider + tools).

    Reuses the exact wiring behind ``anika setup --portal`` so the commands stay in lockstep.
    """
    from anika_cli.setup import _run_portal_one_shot
...
# Default (None/"") is the one-shot onboarding (alias for `anika auth add nous --type oauth` /
# `anika setup --portal`). `status` kept as a back-compat alias for `info`.
_SUBCOMMANDS = {
    None: _cmd_login,
    "": _cmd_login,
    "login": _cmd_login,
...
def portal_command(args) -> int:
    """Top-level dispatch for `anika portal <subcommand>`."""
    sub = getattr(args, "portal_command", None)
    handler = _SUBCOMMANDS.get(sub)
    if handler is not None:
        return handler(args)
    print(f"Unknown portal subcommand: {sub}", file=sys.stderr)
    print("Run `anika portal -h` for usage.", file=sys.stderr)
    return 1


def add_parser(subparsers) -> None:
    """Register `anika portal` on the given argparse subparsers object."""
    portal_parser = subparsers.add_parser(
        "portal",
        help="Set up __NEW_ORG__ Portal (login, model pick, Tool Gateway); see also `portal info`",
        description=(
            "Run `anika portal` with no subcommand to log in to __NEW_ORG__ Portal "
            "and set it up — pick a model, set __NEW_ORG__ as your provider, and offer "
            "the Tool Gateway (the human-readable alias for `anika auth add "
            "nous --type oauth`, identical to `anika setup --portal`). "
            "Subcommands: login (default), info, open, tools."
        ),
    )
    portal_sub = portal_parser.add_subparsers(dest="portal_command")

    # `status` is a hidden (no help) back-compat alias; registration order = `anika portal -h` order.
    for name, help_text in (
        ("login", "Log in to __NEW_ORG__ Portal + set it up (default; one-shot onboarding)"),
