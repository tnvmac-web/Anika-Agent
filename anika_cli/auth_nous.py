"""__NEW_ORG__ Portal OAuth: device-code login, refresh, shared-store mirroring, JWT selection, status.

Split out of ``anika_cli/auth.py``; origin helpers are imported lazily inside each function
so ``anika_cli.auth.<name>`` patches still intercept (and no import cycle).
"""
...
from anika_cli.auth_codex import _pool_entries
from anika_cli.auth_constants import (
...
if TYPE_CHECKING:  # annotation-only; the runtime import would be a cycle
    from anika_cli.auth import ProviderConfig

# Log-record parity with the origin module (caplog tests pin "anika_cli.auth").
logger = logging.getLogger("anika_cli.auth")

_UNUSABLE_JWT_RELOGIN = "Re-authenticate with: anika auth add nous"
...
def _unusable_invoke_jwt_error(reason: str, *, no_refresh_token: bool = False) -> AuthError:
    ...
    return _nous_err(
        f"__NEW_ORG__ Portal access token is not a usable inference JWT ({reason}){detail}. "
...
def _oauth_trace(event: str, *, sequence_id: Optional[str] = None, **fields: Any) -> None:
    if os.getenv("ANIKA_OAUTH_TRACE", "").strip().lower() not in {"1", "true", "yes", "on"}:
        return
...
# __NEW_ORG__ agent-key slots; a fresh login persists them as None, quarantine strips them.
...
_NOUS_STALE_PORTAL_HOSTS: FrozenSet[str] = frozenset({"api.__NEW_DOMAIN__"})
...
def _portal_entitlement_message(capability: str) -> str:
    ...
    from anika_cli.nous_account import (
...
def _format_nous_entitlement_auth_error(error: AuthError) -> str:
    with suppress(Exception):
        if message := _portal_entitlement_message("__NEW_ORG__ model access"):
            return message
    return f"{error} Check credits or billing in __NEW_ORG__ Portal, then retry."
...
def _migrate_stale_nous_portal_url(providers: Dict[str, Any]) -> None:
    ...
    if not isinstance(nous, dict):
        return
    stored = (__NEW_ORG__.get("portal_base_url") or "").strip()
...
# Allowlist of hosts the __NEW_ORG__ Portal proxy will forward inference JWTs to — a bearer sent anywhere
...
_ALLOWED_NOUS_INFERENCE_HOSTS: FrozenSet[str] = frozenset({"inference-api.__NEW_DOMAIN__"})
...
def _nous_inference_env_override() -> Optional[str]:
    ...
    """
    from anika_cli.auth import _optional_base_url
    return _optional_base_url(os.getenv("NOUS_INFERENCE_BASE_URL"))


def _nous_portal_env_override() -> Optional[str]:
    """``ANIKA_PORTAL_BASE_URL`` / ``NOUS_PORTAL_BASE_URL`` override or None.
    ...
    return _optional_base_url(
        os.getenv("ANIKA_PORTAL_BASE_URL") or os.getenv("NOUS_PORTAL_BASE_URL"))
...
def _nous_invoke_jwt_status(
    ...
    from anika_cli.auth import _is_expiring
...
def _nous_invoke_jwt_is_usable(
    ...
    from anika_cli.auth import _nous_invoke_jwt_status
...
def _remaining_ttl(expires_at: Any, fallback_expires_in: Any) -> int:
    ...
    from anika_cli.auth import _coerce_ttl_seconds, _parse_iso_timestamp
...
def _set_nous_agent_key_from_invoke_jwt(
    ...
    from anika_cli.auth import _nonempty_str
...
def _select_nous_invoke_jwt(
    ...
    logger.debug("__NEW_ORG__ inference auth: using NAS invoke JWT")
    _oauth_trace(
...
def _nous_shared_auth_dir() -> Path:
    """Directory of the shared __NEW_ORG__ token store: ``ANIKA_SHARED_AUTH_DIR`` or ``<root>/shared/``.

    Outside any named profile so all profiles share it (``anika --profile X auth add nous --type
    ...
    """
    override = os.getenv("ANIKA_SHARED_AUTH_DIR", "").strip()
    if override:
        return Path(override).expanduser()
    from anika_constants import get_default_anika_root
    return get_default_anika_root() / "shared"


def _nous_shared_store_path() -> Path:
    path = _nous_shared_auth_dir() / NOUS_SHARED_STORE_FILENAME
    # Seat belt (mirrors the _auth_file_path() guard): under pytest, refuse a path under the real
    # user's Anika root so a test that forgot ANIKA_SHARED_AUTH_DIR fails loudly instead of
    # corrupting cross-profile state.
    if os.environ.get("PYTEST_CURRENT_TEST"):
        from anika_constants import get_default_anika_root
        real_home_shared = (
            get_default_anika_root() / "shared" / NOUS_SHARED_STORE_FILENAME).resolve(strict=False)
        try:
        ...
        if resolved == real_home_shared:
            raise RuntimeError(
                f"Refusing to touch real user shared __NEW_ORG__ auth store during test run: "
                f"{path}. Set ANIKA_SHARED_AUTH_DIR to a tmp_path in your test fixture.")
    return path


@contextmanager
def _nous_shared_store_lock(timeout_seconds: float = AUTH_LOCK_TIMEOUT_SECONDS):
    """Cross-profile lock for the shared __NEW_ORG__ OAuth store.
    ...
    """
    from anika_cli.auth import _file_lock
    try:
        lock_path = _nous_shared_store_path().with_suffix(".lock")
    except RuntimeError:
        yield  # No ANIKA_HOME yet (pre-setup): fall through without locking.
        return
    with _file_lock(
        lock_path, _nous_shared_lock_holder, timeout_seconds,
        "Timed out waiting for shared __NEW_ORG__ auth lock"):
        yield
...
# OAuth fields mirrored between a profile's __NEW_ORG__ state and the shared cross-profile store.
...
def _merge_shared_nous_oauth_state(state: Dict[str, Any]) -> bool:
    """Copy fresher shared OAuth tokens into a profile-local __NEW_ORG__ state."""
    from anika_cli.auth import _nonempty_str, _parse_iso_timestamp, _read_shared_nous_state
...
def _write_shared_nous_state(state: Dict[str, Any]) -> None:
    """Persist a minimal copy of the __NEW_ORG__ OAuth state to the shared store.

    Best-effort: failures are logged and swallowed; per-profile auth.json stays the source of truth.
    """
    from anika_cli.auth import _nonempty_str, _write_private_file_atomic
    ...
    try:
        ...
        logger.debug("Failed to write shared __NEW_ORG__ auth store: %s", exc)


def _read_shared_nous_state() -> Optional[Dict[str, Any]]:
    """Shared __NEW_ORG__ OAuth state when present and well-formed, else None.
    ...
    try:
        ...
        logger.debug("Shared __NEW_ORG__ auth store at %s is unreadable: %s", path, exc)
        return None
...
def _clear_shared_nous_state(reason: str) -> None:
    """Remove the shared __NEW_ORG__ OAuth store after a terminal token failure."""
    try:
        with _nous_shared_store_lock():
            _nous_shared_store_path().unlink(missing_ok=True)
        _oauth_trace("nous_shared_store_cleared", reason=reason)
    except Exception as exc:
        logger.debug("Failed to clear shared __NEW_ORG__ auth store: %s", exc)
...
def _quarantine_forensics(state: Dict[str, Any], error: AuthError, reason: str) -> Dict[str, Any]:
    ...
    provenance is client_id + agent_key_id (__NEW_ORG__ state has no session_id).
    """
    from anika_cli.auth import _auth_file_path
...
def _quarantine_nous_oauth_state(state: Dict[str, Any], error: AuthError, *, reason: str) -> None:
    ...
    logger.warning(
        "__NEW_ORG__ OAuth state quarantined (terminal auth death): %s",
...
def _quarantine_nous_pool_entries(
    ...
    """Remove singleton-seeded __NEW_ORG__ pool entries that contain dead OAuth state."""
...
def _try_import_shared_nous_state(*, timeout_seconds: float = 15.0) -> Optional[Dict[str, Any]]:
    """Rehydrate __NEW_ORG__ OAuth state from the shared store via a forced refresh.
    ...
    try:
        ...
        logger.debug("Shared __NEW_ORG__ import failed: %s", exc)
        return None
    return refreshed
...
def _refresh_access_token(
    *, client: httpx.Client, portal_base_url: str, client_id: str, refresh_token: str,
    ...
    # self-heal hook) redeemed Anika's refresh_token without persisting the rotated token, so the
    ...
    if code == "refresh_token_reused" or "reuse" in description.lower():
        description = (
            "__NEW_ORG__ Portal detected refresh-token reuse and revoked this session.\n"
            "This usually means an external process (monitoring script, "
            "custom self-heal hook, or another Anika install sharing "
            "~/.anika/auth.json) called POST /api/oauth/token with Anika's "
            "refresh token without persisting the rotated token back.\n"
            "__NEW_ORG__ refresh tokens are single-use — only Anika may call the "
            "refresh endpoint. For health checks, use `anika auth status` "
            "instead.\n"
            "Re-authenticate with: anika auth add nous")
        relogin = True
...
def _refresh_nous_or_quarantine(
    *, client: httpx.Client, auth_store: Dict[str, Any], state: Dict[str, Any],
    ...
    from anika_cli.auth import _refresh_access_token, _is_terminal_nous_refresh_error
    try:
...
def _apply_nous_refreshed_tokens(
    ...
    """Write a successful __NEW_ORG__ token-refresh payload into *state* (tokens + expiry fields).
    ...
    *inference_base_url*, when given, is the healed network-provenance URL to persist alongside
    ...
    """
    from anika_cli.auth import _coerce_ttl_seconds
...
def fetch_nous_models(
    *, inference_base_url: str, api_key: str, timeout_seconds: float = 15.0,
    ...
    """Fetch available model IDs from the __NEW_ORG__ inference API."""
    from anika_cli.auth import _nonempty_str
    ...
    for item in data:
        ...
        # Anika models aren't reliable for agentic tool-calling
        if _nonempty_str(model_id) and "anika" not in model_id.lower():
...
def refresh_nous_oauth_pure(
    ...
    """Refresh __NEW_ORG__ OAuth state without mutating auth.json directly.
...
def refresh_nous_oauth_from_state(
    ...
    on_state_update: Optional[Callable[[Dict[str, Any], str], None]] = None) -> Dict[str, Any]:
    """Refresh __NEW_ORG__ OAuth from a state dict (defaults filled in) without mutating auth.json."""
    from anika_cli.auth import (
    ...
    with _nous_http_client(timeout_seconds or 15.0, verify) as client:
        ...
        if force_refresh or current_invoke_jwt_status is not None:
            ...
            if not isinstance(refresh_token_value, str) or not refresh_token_value:
                ...
                raise _nous_err("No refresh token is available for __NEW_ORG__ Portal.", relogin=True)
...
def persist_nous_credentials(creds: Dict[str, Any], *, label: Optional[str] = None):
    """Persist __NEW_ORG__ OAuth credentials as the singleton provider state.

    __NEW_ORG__ credentials are read from ``providers.nous`` (401 recovery, pool seeding) AND
    ...
    it.
    """
    from anika_cli.auth import _save_active_provider_state, _write_shared_nous_state
...
def _sync_nous_pool_from_auth_store() -> None:
    ...
    try:
        ...
        logger.debug("Failed to sync __NEW_ORG__ credential pool from auth store: %s", exc)
...
def _nous_effective_routing(state: Dict[str, Any]) -> tuple[str, str, str, str]:
    ...
    """
    from anika_cli.auth import _NOUS_PORTAL_ALLOWED_HOSTS, _optional_base_url
    portal_url = (
        _optional_base_url(state.get("portal_base_url")) or os.getenv("ANIKA_PORTAL_BASE_URL")
...
class _NousRuntimeResolve:
    ...
    def persist(self, reason: str) -> None:
        from anika_cli.auth import _save_provider_state_to_source, _write_shared_nous_state
        state = self.state
...
def resolve_nous_runtime_credentials(
    *, timeout_seconds: float = 15.0, insecure: Optional[bool] = None,
    ...
    """Resolve __NEW_ORG__ inference credentials for runtime use (refreshing under the auth-store lock).
    ...
    if not isinstance(api_key, str) or not api_key:
        raise _nous_err("Failed to resolve a __NEW_ORG__ inference API key", "server_error")
...
def _snapshot_nous_pool_status() -> Dict[str, Any]:
    ...
    """
    from anika_cli.auth import _parse_iso_timestamp
    try:
        from agent.credential_pool import load_pool
        pool = load_pool("nous")
...
def _nous_status_from_state(
    ...
    """Auth-store-backed __NEW_ORG__ status snapshot (shared by the live and refresh-free variants)."""
    access_token = state.get("access_token")
    return {
...
def _compute_nous_auth_status() -> Dict[str, Any]:
    ...
    from anika_cli.auth import get_provider_auth_state, resolve_nous_runtime_credentials
...
def get_nous_auth_status_local() -> Dict[str, Any]:
    """Refresh-free __NEW_ORG__ auth snapshot for read-only display surfaces.
    ...
    """
    from anika_cli.auth import get_provider_auth_state
    try:
        state = get_provider_auth_state("nous")
    except Exception:
        state = None
...
def get_nous_session_validity() -> str:
    """Classify the __NEW_ORG__ bootstrap session for the dashboard /api/status probe.
...
def _pool_first_oauth_status(
    ...
    Pool first (where `anika auth` / `anika model` store device_code tokens), then
...
def _nous_device_code_login(
    *, portal_base_url: Optional[str] = None, inference_base_url: Optional[str] = None,
    ...
    """Run the __NEW_ORG__ device-code flow and return full OAuth state without persisting."""
    from anika_cli.auth import (
    ...
    portal_base_url = (
        portal_base_url or os.getenv("ANIKA_PORTAL_BASE_URL") or os.getenv("NOUS_PORTAL_BASE_URL")
    ...
    if _is_remote_session():
        open_browser = False
    print(f"Starting Anika login via {pconfig.name}...")
    ...
    try:
        ...
        if exc.code == "subscription_required":
            ...
            print()
            print("After subscribing, run `anika model` again to finish setup.")
            raise SystemExit(1)
        raise
...
def _mirror_nous_state_best_effort(auth_state: Dict[str, Any]) -> None:
    ...
    from anika_cli.auth import _sync_nous_pool_from_auth_store, _write_shared_nous_state
    with suppress(Exception):
...
def _pick_nous_model_after_login(
    ...
    """Fetch the curated __NEW_ORG__ model list (tier/policy-filtered) and run the interactive picker.
    ...
    """
    from anika_cli.auth import _prompt_model_selection
    ...
    from anika_cli.models_pricing import (
    ...
    if model_ids:
        ...
        if free_tier:
            with suppress(Exception):
                unavailable_message = _portal_entitlement_message("paid __NEW_ORG__ models")
    ...
    if model_ids:
        from anika_cli.nous_account import nous_policy_notice
    ...
    if unavailable_models:
        ...
        print("No curated models available for __NEW_ORG__ Portal.")
    return None


def _offer_shared_nous_import(timeout_seconds: float) -> Optional[Dict[str, Any]]:
    """Codex-style auto-import: offer to rehydrate a __NEW_ORG__ credential from another profile.
    ...
    """
    from anika_cli.auth import _prompt_yes_no, _read_shared_nous_state
    shared = _read_shared_nous_state()
    if not shared:
        return None
    try:
        ...
        shared_path = None
    print()
    print(f"Found existing __NEW_ORG__ OAuth credentials at {shared_path}" if shared_path
          else "Found existing shared __NEW_ORG__ OAuth credentials")
    if not _prompt_yes_no("Import these credentials? [Y/n]: ", default="y"):
        return None
    print("Rehydrating __NEW_ORG__ session from shared credentials...")
...
def _restore_active_provider(prior_active_provider: Any) -> None:
    ...
    from anika_cli.auth import _auth_store_lock, _load_auth_store, _save_auth_store
...
def _login_nous(args, pconfig: ProviderConfig) -> None:
    """__NEW_ORG__ Portal device authorization flow."""
    from anika_cli.auth import (
    ...
    ca_bundle = (
        getattr(args, "ca_bundle", None) or os.getenv("ANIKA_CA_BUNDLE")
    ...
    try:
        ...
        # switch to __NEW_ORG__ with a mismatched model; the __NEW_ORG__ tokens stay saved for future use.
        if not selected_model:
            _restore_active_provider(prior_active_provider)
            print()
            print("No provider change. __NEW_ORG__ credentials saved for future use.")
            print("  Run `anika model` again to switch to __NEW_ORG__ Portal.")
            return
