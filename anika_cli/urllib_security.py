...
_CA_BUNDLE_ENV_VARS = ("ANIKA_CA_BUNDLE", "SSL_CERT_FILE", "REQUESTS_CA_BUNDLE", "CURL_CA_BUNDLE")
...
def _resolved_https_context() -> ssl.SSLContext | None:
    """Return the explicit CA context for Anika-owned urllib openers."""
...
def _secure_opener_from_installed_policy(original_url: str, *, ssl_context=None):
    ...
    (``ssl_ca_cert``/``ssl_verify``) apply; with None a Anika-owned opener gets the explicit CA
    ...
    secured._anika_initial_addheaders = list(getattr(installed, "addheaders", ()))
    secured.addheaders = []
    return secured
...
def open_credentialed_url(
    ...
    *,
    ...
    if opener_factory is None:
        ...
        for name, value in getattr(opener, "_anika_initial_addheaders", ()):
