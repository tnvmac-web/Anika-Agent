...
def _print_loopback_ssh_hint(redirect_uri: str, *, docs_url: str | None = None) -> None:
    ...
    print(
        ...
        f"Anika is waiting for the OAuth callback on {redirect_uri}\n"
...
def _resolve_verify(
    *, insecure: Optional[bool] = None, ca_bundle: Optional[str] = None,
    ...
    from anika_cli.auth import _default_verify
    ...
    effective_ca = (
        ca_bundle or tls_state.get("ca_bundle") or os.getenv("ANIKA_CA_BUNDLE")
...
def _poll_device_token_generic(
    ...
    """RFC 8628 device-code polling loop shared by the __NEW_ORG__ and xAI flows.
...
def _poll_for_token(
    ...
    """Poll the __NEW_ORG__ token endpoint until the user approves or the code expires."""
    ...
    *resolve* attempts a refresh, so a resolved token should be valid — but double-check the
    ...
    try:
        ...
        if isinstance(api_key, str) and api_key and not is_expiring(api_key, 60):
            print(f"Existing {display_name} credentials found in Anika auth store.")
