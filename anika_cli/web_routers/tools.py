...
from anika_cli.web_routers._common import (
    _CONFIG_MUTATION_LOCK, _profile_cli_args, _profile_scope, _spawn_anika_action,
...
def _terminal_backend_rows() -> List[Dict[str, str]]:
    ...
    from anika_cli.web_server_profiles import _TERMINAL_BACKENDS
...
def _resolve_toolset_model_plugin(ts_key: str, provider_row: dict) -> Optional[str]:
    ...
    the managed "__NEW_ORG__ Subscription" image row instead carries the legacy
...
def _toolset_model_catalog(ts_key: str, plugin_name: str):
    ...
    from anika_cli.tools_config import _plugin_image_gen_catalog, _plugin_video_gen_catalog
...
def _category_providers(ts_key: str, config: dict) -> list:
    ...
    from anika_cli.tools_config import TOOL_CATEGORIES, _visible_providers
...
def _find_toolset_provider_row(
    ...
    from anika_cli.tools_config import _is_provider_active
...
async def get_toolsets(profile: Optional[str] = None):
    from anika_cli.tools_config import (
        ...
        gui_toolset_label)
    from anika_cli.platforms import platform_label
...
async def toggle_toolset(name: str, body: ToolsetToggle, profile: Optional[str] = None):
    ...
    # install `anika tools` runs — otherwise the toggle "saves" but the tool
            ...
            if cat:
                ...
                for prov in _visible_providers(cat, config, force_fresh=True):
                    ...
                    row = {
                        ...
                        # automatically ready (logged-out __NEW_ORG__ rows, never-run
                        # post_setup installs).
...
async def select_toolset_provider(
    ...
    """
    from anika_cli.tools_config import apply_provider_selection, web_provider_capabilities
    ...
    def _run():
        with _profile_scope(body.profile or profile):
            ...
            # Entitlement check for managed __NEW_ORG__ rows (mirrors the CLI's
...
async def run_toolset_post_setup(
    ...
    """Spawn ``anika tools post-setup <key>`` (long-running installs) as a
    background action tailed via ``GET /api/actions/tools-post-setup/status``;
    ``profile`` is threaded so hooks see the drawer's ANIKA_HOME."""
    from anika_cli.tools_config import valid_post_setup_keys
...
async def grant_computer_use_permissions(profile: Optional[str] = None):
    """Spawn ``anika computer-use permissions grant`` (macOS-only: launches
...
_PLUGIN_COMPAT_LAZY = {
    'LateState': ('anika_cli.web_deps', 'LateState'),
}
