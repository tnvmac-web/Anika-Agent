...
from anika_cli.web_models import ModelAssignment, MoaConfigPayload, MoaModelSlot
...
def _nous_recommended_default() -> dict:
    from anika_cli import models as m
    from anika_cli import models_pricing as mp
...
def get_recommended_default_model(provider: str = ""):
    ...
    ``anika model``'s curation so GUI onboarding lands on a sensible default.
    __NEW_ORG__ honors the user's free/paid tier. Any other provider gets the preferred
    ...
    try:
        ...
        from anika_cli.models import pick_silent_default_model
...
def set_moa_models(body: MoaConfigPayload, profile: Optional[str] = None):
    ...
    with http_failure("PUT /api/model/moa failed", 500, detail="Failed to save MoA config"):
        from anika_cli.moa_config import normalize_moa_config, validate_moa_payload
...
async def set_model_assignment(body: ModelAssignment, profile: Optional[str] = None):
    ...
    ``~/.anika/config.yaml`` — applies to **new** sessions only; a running chat
