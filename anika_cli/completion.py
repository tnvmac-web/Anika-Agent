"""Shell completion script generation for anika CLI. Walks the live argparse parser tree, so
    ...
    for cmd, info in subcommands:
        if cmd == "profile" and info["subcommands"]:
            ...
            cases.append(
                ...
                f"                    COMPREPLY=($(compgen -W \"$(_anika_profiles)\" -- \"$cur\"))\n"
                f"                    return\n"
                f"                    ;;\n"
                f"            esac\n"
                f"            ;;")
            ...
            cases.append(
                ...
                f"            ;;")
    cases_str = "\n".join(cases)
    return f"""# Anika Agent bash completion
# Add to ~/.bashrc:
#   eval "$(anika completion bash)"

_anika_profiles() {{
    local profiles_dir="$HOME/.anika/profiles"
...
_anika_completion() {{
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_anika_profiles)" -- "$cur"))
        return
    fi
...
complete -F _anika_completion anika
"""
    ...
    for cmd, info in subcommands:
        ...
        if cmd == "profile":
            ...
            sub_cases.append(
                ...
                f"                    ;;")
    sub_cases_str = "\n".join(sub_cases)
    return f"""#compdef anika
# Anika Agent zsh completion
# Add to ~/.zshrc:
...
_anika_profiles() {{
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.anika/profiles" ]]; then
        profiles+=($HOME/.anika/profiles/*(N/:t))
    fi
...
_anika() {{
    ...
    _arguments -C \\
        ...
        '(-)'{{-p,--profile}}'[Profile name]:profile:_anika_profiles' \\
        '1:command:->commands' \\
        '*::arg:->args'
...
{top_cmds_str}
            ...
            _describe 'anika command' subcmds
            ;;
        args)
            case ${{line[1]}} in
{sub_cases_str}
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        "# Anika Agent fish completion",
        "# Add to your config:",
        "#   anika completion fish | source",
        "",
        "# Helper: list available profiles",
        "function __anika_profiles",
        "    echo default",
        "    if test -d $HOME/.anika/profiles",
        "        for d in $HOME/.anika/profiles/*/",
        "            basename $d",
        "        end",
        "    end",
        "end",
        ...
        "",
        "# Complete profile names after -p / --profile",
        "complete -c anika -f -s p -l profile"
        " -d 'Profile name' -xa '(__anika_profiles)'",
        "",
        "# Top-level subcommands"]
    for cmd, info in subcommands:
        lines.append(
            f"complete -c anika -f "
        ...
        if cmd == "profile":  # profile names for the actions that take one
            for action in sorted(_PROFILE_NAME_ACTIONS):
                lines.append(
                    ...
                    f"-a '(__anika_profiles)' -d 'Profile name'")
    lines.append("")
    return "\n".join(lines)
