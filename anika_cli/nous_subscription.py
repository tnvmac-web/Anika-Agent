"""Helpers for __NEW_ORG__ subscription managed-tool capabilities."""
...
from anika_cli.config import get_env_value, load_config
...
_DEFAULT_PLATFORM_TOOLSETS = {"cli": "anika-cli"}
...
def _has_agent_browser() -> bool:
    ...
    from anika_constants import agent_browser_runnable
    ...
    try:
        ...
        # Anika-managed Node dirs ($ANIKA_HOME/node, prepended to PATH at runtime but usually absent
        ...
        search_paths = [None, with_anika_node_path().get("PATH", ""), str(local_bin_dir) if local_bin_dir.is_dir() else ""]
        return any(
...
def _local_stt_backend_available() -> bool:
    ...
    if get_env_value("ANIKA_LOCAL_STT_COMMAND"):
        return True
    try:
...
def _fal_feature(key: str, tool_enabled: bool, direct: bool, managed: bool, selected: Optional[str]) -> NousFeatureState:
    ...
    if selected not in (None, "nous") or (selected is None and direct):
        ...
        label = "__NEW_ORG__ Subscription" if (fal_managed or selected == "nous") else ""
    return _state(
...
def _audio_features(
    ...
    # STT default is "local" (faster-whisper, needs a pip install); __NEW_ORG__ subscribers are routed to the
...
def prompt_enable_tool_gateway(config: Dict[str, object], *, force_fresh: bool = True) -> set[str]:
    ...
    if not unconfigured and not has_direct:
        return set()
    try:
        from anika_cli.setup import prompt_checklist
    except Exception:
        return set()
    ...
    source_label = "free tool pool" if pool_only else "__NEW_ORG__ subscription"
    ...
    # every __NEW_ORG__ model swap. See #92647.
    ...
    title = (
        "Your free __NEW_ORG__ tool pool — pick the tools to enable:" if pool_only
        else "Your __NEW_ORG__ subscription includes the Tool Gateway — pick the tools to enable:"
    )
    try:
...
# Inline __NEW_ORG__ Portal login for the Tool Gateway picker (`anika tools`)


def ensure_nous_portal_access(*, capability: str = "the __NEW_ORG__ Tool Gateway", coverage_category: Optional[str] = None) -> bool:
    """Make sure the user is entitled to the __NEW_ORG__ Tool Gateway, logging in if needed.
...
def _run_nous_portal_login_only(*, capability: str) -> bool:
    """Run the __NEW_ORG__ Portal device-code OAuth and persist credentials only (no model selection, no
    ...
    try:
        ...
        print(f"  Could not start __NEW_ORG__ Portal login: {exc}")
        return False
    print()
    print(f"  {capability} requires a __NEW_ORG__ Portal login.")
    proceed = _confirm("  Log in to __NEW_ORG__ Portal now? [Y/n]: ")
    if proceed is None:
        print()
        return False
    if not proceed:
        print("  Skipped __NEW_ORG__ Portal login.")
        return False
    try:
        # Snapshot active_provider so a tool-config login never silently switches inference to __NEW_ORG__.
        ...
        if auth._read_shared_nous_state() and _confirm("  Found existing __NEW_ORG__ OAuth credentials. Import them? [Y/n]: ") is not False:
        ...
        print("  __NEW_ORG__ Portal login successful.")
        return True
        ...
        print(f"  __NEW_ORG__ Portal login failed: {exc}")
        return False
