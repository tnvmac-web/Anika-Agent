...
# ``anika update`` is a single-threaded CLI command; a module singleton lets the 7k-line updater
...
class UpdateReceipt:
    """Collects the observable facts of one ``anika update`` run."""
    ...
    def gateway_restart_result(
        ...
        if fresh_recovery is not None:
            ...
            # ``anika serve`` hosts tui_gateway and is not a gateway profile, so neither the
...
def _receipt_dir() -> Path:
    ...
    return get_anika_home() / "logs" / "update_receipts"
...
def finalize_pending_update_receipt(exit_code: Optional[int] = None, stop_reason: str = "") -> Optional[Path]:
    ...
    ``anika update`` has many early ``sys.exit`` paths (preflight refusals, venv-holder refusal,
...
def _profile_homes() -> list[tuple[str, Path]]:
    ...
    from anika_cli.profiles import _get_default_anika_home, _get_profiles_root, _PROFILE_ID_RE
