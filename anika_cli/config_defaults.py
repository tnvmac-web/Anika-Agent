"""Default configuration data for Anika Agent: DEFAULT_CONFIG and OPTIONAL_ENV_VARS.

Pure-data leaf module — must not import from anika_cli.config. Comments are the user-facing
docs of config.yaml.
"""
...
DEFAULT_CONFIG = {
    ...
    "toolsets": ["anika-cli"],
    # journal_mode: SQLite journal mode for every Anika DB. "wal" default; use "delete" on
    ...
    "session": {
        # Per-terminal `anika -c`: each CLI session writes a breadcrumb under
        # $ANIKA_HOME/terminal-sessions/<terminal-id>, so bare -c/--continue resumes THIS
    ...
    "agent": {
        ...
        # Default 30 min is a safety valve for wedged agents, not a target latency — an interactive `anika
        ...
        # Anika-level retry attempts for API errors (connection drops, timeouts, 5xx) wrapping the
        ...
        # wrapping Anika (sandbox runner, managed platform) can describe proxy/credential/ mount
        # layout without editing SOUL.md. Env ANIKA_ENVIRONMENT_HINT overrides it.
        "environment_hint": "",
        ...
        # share or `anika doctor` (status channel only; prompt cache untouched). 0 = no escalation
        ...
        # gateway status (`anika status` / fleet monitoring). Retries never stop — a signal, not a
        ...
        # ANIKA_LOCAL_STREAM_STALE_TIMEOUT overrides.
    ...
    "terminal": {
        ...
        # sandboxes). Empty = TMPDIR/TMP/TEMP if set, else ANIKA_HOME/cache/terminal (auto-pruned
        ...
        # ANIKA_HOME/home; "real" = force real HOME; "profile" = force ANIKA_HOME/home when it
        ...
        # ["/home/user/.anika/cache/documents:/output"]. For gateway MEDIA delivery, write to
        ...
        # default for images whose entrypoints must start as root (e.g. the bundled Anika image,
        # which drops to `anika` via s6-setuidgid). When on, SETUID/SETGID caps are omitted.
    ...
    "web": {
        ...
        # parallel, firecrawl, keenable; tavily is opt-in keyless via `anika tools`, not a ring
        # member). Set by the `anika tools` picker. "free" = always anonymous endpoint even with a
    ...
    "browser": {
        ...
        # Consent to browse with the user's REAL logins locally: runs on a Anika-managed SNAPSHOT
        # of the ACTIVE default-Chromium profile (Local State -> profile.last_used; cookies, logins,
        # prefs copied and re-synced per fresh session) driven by Anika' packaged Chromium. The
        # snapshot dir sidesteps Chrome 136+'s default-profile debugging block and never contends
        # with the running browser. Turning off deletes ~/.anika/browser-profile/ so credentials
        ...
        # runs `anika browser close-profile` (kills that profile's browser tree, unsaved tabs lost)
    ...
    # write_file/patch call); restore with /rollback. Opt-in via `anika chat --checkpoints` or
    ...
    "checkpoints": {
        ...
        "max_snapshots": 20,
        # Hard ceiling on total ~/.anika/checkpoints/ size (MB); the oldest checkpoint per project
        ...
        # must not guess. Orphans: `anika checkpoints prune` (`--keep-orphans` to skip).
        "auto_prune": True,
        "retention_days": 7,
        "min_interval_hours": 24,
    },
    # Hard cap (chars) for one auto-loaded context file (SOUL.md, AGENTS.md, CLAUDE.md, .anika.md,
    ...
    # Same bound for single-query mode (``anika -q/-z``). With only ONE turn there is no
    ...
    "compression": {
        ...
        # Anika' summarizer cannot shrink it. native = codex decides; anika = Anika' threshold
    ...
    "auxiliary": {
        ...
        # of skills); route cheaper via `anika model` → auxiliary → Curator.
        "curator": _aux(600),
    ...
    "display": {
        ...
        # Interface bare `anika`/`anika chat` launches: "cli" (prompt_toolkit REPL) | "tui" (Ink).
        # Flags win: `--cli` forces the REPL, `--tui` / ANIKA_TUI=1 forces the TUI.
        "interface": "cli",
        # `anika --tui` auto-resumes the most recent human-facing session (like `anika -c`).
        # ANIKA_TUI_RESUME=<id> always wins.
        ...
        # __NEW_ORG__ credits status-bar notices (usage bands, grant-spent, depleted/restored). False mutes
        ...
        # `path`/`paths` = ANIKA_HOME-relative YAML files/dirs (or conventional status_phrases.yaml
        ...
        # Gateway runtime footer on the FINAL message, e.g. `model · 68% · ~/projects/anika`.
        ...
        # CLI/TUI/desktop, managed with `anika pets`. No effect on prompt caching.
        "pet": {
            "enabled": False,
            "slug": "",   # active pet slug in get_anika_home()/pets/; empty → first installed
    ...
    "dashboard": {
        ...
        # client disconnects. 0 = park forever. Env: ANIKA_TUI_WS_ORPHAN_REAP_GRACE_S.
        ...
        # newest message older than ANIKA_TUI_SESSION_TTL_S, default 6h) with
        ...
        # OAuth gate (engaged when --host is set and --insecure is not), read by the __NEW_ORG__ Portal
        # plugin. Env ANIKA_DASHBOARD_OAUTH_CLIENT_ID / ANIKA_DASHBOARD_PORTAL_URL win when
        ...
        # no-op. Env ANIKA_DASHBOARD_BASIC_AUTH_USERNAME / _PASSWORD_HASH / _PASSWORD / _SECRET /
        ...
        # ANIKA_DASHBOARD_DRAIN_SECRET; no-op unless >=256-bit, weak secrets rejected
        # (fail-closed). scope = capability label; min_secret_chars in url-safe-b64 chars.
        "drain_auth": {"scope": "drain", "min_secret_chars": 43},
        # Public URL (env ANIKA_DASHBOARD_PUBLIC_URL): full authority (scheme + host + optional
        # prefix, e.g. https://example.com/anika) for the OAuth redirect_uri; its hostname is
    ...
    "tts": {
        ...
        "piper": {
            ...
            # (~/.anika/cache/piper-voices/), use_cuda, length_scale (2.0 = twice as slow),
    ...
    # "Hey Anika" hands-free wake word: always-on, on-device hotword detection that starts a fresh
    ...
    "wake_word": {
        ...
        "openwakeword": {
            # "hey_anika" | built-in openWakeWord name ("hey_jarvis", "alexa", ...) | path to a
    ...
    # plugins/context_engine/<name>/ or ~/.anika/plugins/.
    "context": {
        "engine": "compressor",
    ...
    "delegation": {
        ...
        # ~/.anika/cache/delegation/ with a head+tail window + read_file offset footer, nothing
    ...
    "moa": {
        ...
        # <anika_home>/moa-traces/<session_id>.jsonl (or trace_dir) for auditing.
        "save_traces": False,
        "trace_dir": "",
    ...
    # and resolved; read-only — creation goes to ~/.anika/skills/ unless create_dir redirects it.
    "skills": {
        ...
        # ANIKA_HOME, scanned alongside the local dir.
        "create_dir": "",
        # In a git checkout, <root>/.anika/skills/ and <root>/.agents/skills/ load as the
        # highest-precedence tier — ONLY if the root is in trusted_project_dirs. false = no scan, no
        # untrusted-skills notice.
        "project_discovery": True,
        # Trusted project roots; managed by `anika skills trust` / `untrust`.
        "trusted_project_dirs": [],
        # Substitute ${ANIKA_SKILL_DIR} / ${ANIKA_SESSION_ID} in SKILL.md content.
        "template_vars": True,
        ...
        # Advisory NVIDIA SkillEvaluator Tier 1 scan on `anika skills install` (alongside the
        ...
        "write_approval": False,
        # Audit ledger: every skill mutation appends to ~/.anika/skills/.curator_ledger.jsonl with
        # before/after hashes (blobs under ~/.anika/.curator_backups/blobs/); powers `anika
    ...
    # daemon. `anika curator status` shows the last run.
    "curator": {
        "enabled": True,
        ...
        # aux-model cost. `anika curator run --consolidate` overrides once.
        "consolidate": False,
        # Also prune bundled built-ins (a suppression list stops `anika update` restoring them);
        ...
        "prune_builtins": True,
        # TTL purge of skills/.archive/: 0 = never; > 0 lets the explicit `anika curator purge`
        # delete older archived skills (never automatic; logged in the ledger).
        "archive_ttl_days": 0,
        # Before every real (non-dry-run) pass, snapshot ~/.anika/skills/ to
        # ~/.anika/skills/.curator_backups/<utc-iso>/skills.tar.gz (`anika curator rollback`).
        "backup": {
            "enabled": True,
    ...
    # peerName, sessions, enabled); anika-specific overrides only here.
    "honcho": {},
    ...
    "whatsapp": {
        # reply_prefix: None = built-in "⚕ *Anika Agent*" header; "" disables; \n allowed.
    },
    ...
    #   cron job, a -q session (ANIKA_INTERACTIVE=1 but nobody to answer), or an unattended
    ...
    "approvals": {
        ...
        # export ANIKA_INTERACTIVE=1 (for interactive sudo prompts) but have NO user waiting to answer
        ...
        # Approve" → false. ANIKA_TUI_NO_CONFIRM=1 skips the TUI modal.
    ...
    # are given.
    "platform_hints": {},
    # Plugin system. `enabled`/`disabled` lists are written by `anika plugins enable|disable` and
    ...
    "plugins": {
        ...
        # 2026-09-14 removal date (see COMPAT_MANIFEST.md, `anika plugins compat`). Stopgap only: the
    ...
    # approvals persist in ~/.anika/shell-hooks-allowlist.json. Schema + examples:
    # website/docs/user-guide/features/hooks.md.
    "hooks": {},
    # Auto-accept shell-hook registrations without a TTY prompt (also --accept-hooks or
    # ANIKA_ACCEPT_HOOKS=1). Gateway/cron/non-interactive runs need one of these to pick up
    # newly-added hooks.
    ...
    "security": {  # Security: pre-exec scanning via tirith plus related guards.
        ...
        # .anika config) always need human approval, even under yolo. Extra patterns are fnmatch
        ...
        # banner. Add via `anika doctor --ack <id>`; remove by editing the list. Catalog:
        # anika_cli/security_advisories.py.
        "acked_advisories": [],
    ...
    "cron": {
        ...
        # an installed provider (plugins/cron_providers/<name>/ or $ANIKA_HOME/plugins/ <name>/),
        ...
        # holds NO scheduler credentials (provision reuses the __NEW_ORG__ Portal token).
        "chronos": {
            # NAS/portal base URL that arms/cancels one-shots and mints the inbound fire JWT (used
            # as the expected issuer).
            "portal_url": "https://portal.__NEW_DOMAIN__",
        ...
        # Env override: ANIKA_CRON_MAX_PARALLEL.
        ...
        "output_retention": 50,
        # Timeout (seconds) for a no-agent cron script. Env: ANIKA_CRON_SCRIPT_TIMEOUT. Keep in
        ...
        # job's dispatch guard forever. Env: ANIKA_CRON_SESSION_DB_TIMEOUT. 0 = unlimited.
        "session_db_timeout_seconds": 10,
        # Timeout (seconds) per media attachment send during gateway delivery; large attachments
        # (long TTS audio, big exports) need more than 30s. Env: ANIKA_CRON_MEDIA_SEND_TIMEOUT.
    ...
    # promotes dependency-satisfied todos to ready, and fires `anika -p <assignee> chat -q ...` per
    ...
    "kanban": {
        ...
        # Auto-run the decomposer on Triage tasks every tick. False = manual via `anika kanban
    ...
    "code_execution": {  # execute_code settings (programmatic tool calls).
        ...
        # relative paths resolve. strict = isolated temp dir with anika-agent's own python
    ...
    # by tool_search / tool_describe / tool_call bridges and surfaced on demand. Core Anika tools
    ...
    "logging": {  # File logging to ~/.anika/logs/: agent.log captures INFO+, errors.log WARNING+.
        "level": "INFO",       # minimum level for agent.log: DEBUG, INFO, WARNING
        "max_size_mb": 5,      # max size per log file before rotation
        "backup_count": 3,     # rotated backups to keep
    },
    # Remote model-catalog manifest: curated OpenRouter / __NEW_ORG__ Portal model lists fetched from this
    ...
    "model_catalog": {
        "enabled": True,
        "url": "https://anika-agent.__NEW_DOMAIN__/docs/api/model-catalog.json",
        # Disk cache TTL in minutes. The gateway refreshes in the background on this cadence; the
        # CLI refetches on the next /model or `anika model` once the cache is older. Network
    ...
    # defaults (200K context, tools on, vision/reasoning off) and get patched. Provider keys: Anika
    ...
    "monitoring": {
        ...
        "gateway_health_export": {  # Gateway health & diagnostics export.
            ...
            "resource_attributes": {
                "service.name": "anika-gateway", "deployment.environment.name": "production"
            },
        },
    ...
    "gateway": {  # Gateway settings (messaging platforms: Telegram, Discord, Slack, ...).
        ...
        # to ANIKA_GATEWAY_PLATFORM_CONNECT_TIMEOUT, which wins if set explicitly.
        ...
        # run_gateway() bridges these to ANIKA_STARTUP_WATCHDOG / ANIKA_STARTUP_WATCHDOG_TIMEOUT_S
        # and re-arms the live handle; explicit env wins.
        "startup_watchdog": True,
        "startup_watchdog_timeout_seconds": 300,
        # Keep writing the legacy ~/.anika/sessions/sessions.json mirror of the routing index
        ...
        # (ANIKA_SCALE_TO_ZERO env stamp) AND messaging is relay-only/absent AND a wakeUrl is
        ...
        # ANIKA_GATEWAY_MAX_STARTS / ANIKA_GATEWAY_START_WINDOW_S.
        ...
        # credential/system denylist (/etc, /proc, ~/.ssh, ~/.aws, ~/.anika/.env, auth.json). True:
        # files must be under the Anika cache, media_delivery_allow_dirs, or fresher than
        # trust_recent_files_seconds — recommended for public-facing gateways so prompt injection
        # can't exfiltrate host secrets. Bridged to ANIKA_MEDIA_DELIVERY_STRICT.
        "strict": False,
        # Extra roots (project/scratch dirs, mounted shares) from which bare file paths may be
        # uploaded; the Anika cache is always trusted. List of absolute paths or one
        # os.pathsep-separated string; tildes expanded. Bridged to ANIKA_MEDIA_ALLOW_DIRS. Honored
        # in both modes.
        ...
        # pure-allowlist mode. Bridged to ANIKA_MEDIA_TRUST_RECENT_FILES. Only consulted when
        # strict is true.
        "trust_recent_files": True,
        # Recency window in seconds; 600 covers a multi-tool turn. Bridged to
        # ANIKA_MEDIA_TRUST_RECENT_SECONDS. Only consulted when strict is true.
    ...
    # Automatic cleanup of ~/.anika/state.db, which otherwise grows without bound and slows FTS5
    ...
    "sessions": {
        ...
        "auto_prune": True,
        # Inactive days of ended-session history to keep (= `anika sessions prune`).
        ...
        # processes).
        "min_interval_hours": 24,
        # Legacy ~/.anika/sessions/session_{sid}.json snapshots rewritten every turn. state.db is
        ...
        # stay until `anika sessions optimize-storage` runs, since the rebuild is disk-heavy on
        # large DBs. advise = `anika update` prints a one-line notice with reclaimable size when a
        ...
        # (native/fts5_cjk/build.sh → ~/.anika/lib/libfts5_cjk.so), 1-2 char CJK terms get exact
        # index matches instead of LIKE scans. True = use when present (inert otherwise); False =
        # never load/serve it. Bridged to ANIKA_CJK_FTS.
        "cjk_fts": True,
        ...
        # Max active messages per session for in-memory export (`anika sessions export`); checked
    ...
    # and transmission to __NEW_ORG__ (`send`) are SEPARATE opt-ins; see
    ...
    "telemetry": {
        "shared_metrics": {
            ...
            "endpoint": "https://telemetry.__NEW_DOMAIN__/v1/telemetry",
        },
    },

    "doctor": {
        # Per-probe timeout (seconds) for `anika doctor --live` real-call probes.
        "live_probe_timeout": 10,
    },

    "updates": {
        # Pre-update backup. quick = snapshot small critical state (pairing JSONs, cron jobs,
        # config.yaml, .env, auth.json, profile DBs) into <ANIKA_HOME>/state-snapshots/, skipping
        # files >1 GiB; restore via ``/snapshot``. full = quick PLUS a ``anika backup`` zip in
        # <ANIKA_HOME>/backups/ (``anika import`` restores; slow on large homes; ``--backup``
        ...
        # update cleanly. `anika update --switch-branch` overrides to switch for one run.
        "parked_branch_strategy": "switch",
        # Refresh an installed cua-driver during `anika update` (best-effort, macOS only). Turn off
    ...
    "lsp": {
        ...
        "wait_mode": "document",
        "wait_timeout": 5.0,
        # Missing server binaries: auto = install via npm/go/pip into <ANIKA_HOME>/lsp/bin/ on
    ...
    # (SuperGrok OAuth or XAI_API_KEY) AND the toolset is enabled in `anika tools`.
    "x_search": {
    ...
    "secrets": {
        ...
        "bitwarden": {
            ...
            # ~/.anika/.env (or the shell), never in config.yaml.
            ...
            # Last-good fallback for NETWORK/TIMEOUT outages: AES-GCM cache under ~/.anika/cache/,
            ...
            # Auto-download bws into ~/.anika/bin/ on first use; False = bws must be on PATH.
            "auto_install": True,
    ...
    "computer_use": {
        # cua-driver's upstream PostHog telemetry defaults ON; Anika sets
        ...
        # #47032; Linux/WSL2 idle spin #28152). cua-driver ≥ 0.6.x supports --no-overlay; Anika also calls
    ...
    "proxy": {
        ...
        "tunnel_port": 9090,
        # Auto-download the pinned binary into ~/.anika/bin/; False = iron-proxy on PATH.
        "auto_install": True,
    ...
    "desktop": {  # Anika Desktop (Electron) launch options; only affect `anika desktop`.
        ...
        # Bridged to ANIKA_DESKTOP_DISABLE_GPU: auto = disable GPU only on remote displays
        ...
        # to ANIKA_DESKTOP_PASSWORD_STORE; ignored off-Linux.
        "password_store": "auto",
    ...
    "nous": {
        # Upper bound (seconds) on the __NEW_ORG__ auth keepalive tick, which derives from the
    ...
    "local_runtime": {
        # Off = detection-only (Anika still finds an external llama-server you run).
        "enabled": False,
        # Pinned llama.cpp release tag; bumped by Anika releases after validation.
        "tag": "b10679",
...
OPTIONAL_ENV_VARS = {
    ...
    "NOUS_BASE_URL": _base_url("__NEW_ORG__ Portal"),
    ...
    "VERTEX_CREDENTIALS_PATH": _prov(
        ...
        "OAuth2, not a static API key — this points at the credentials Anika mints short-lived "
    ...
    "AZURE_FOUNDRY_BASE_URL": _prov(
        "Azure Foundry base URL (set via 'anika model' for endpoint-specific config)",
    ...
    "FIRECRAWL_GATEWAY_URL": _tool(
        "Exact Firecrawl tool-gateway origin override for __NEW_ORG__ Subscribers only (optional)",
        "Firecrawl gateway URL (leave empty to derive from domain)", None, password=False,
        advanced=True),
    "TOOL_GATEWAY_DOMAIN": _tool(
        "Shared tool-gateway domain suffix for __NEW_ORG__ Subscribers only, used to derive vendor "
        "hosts, e.g. __NEW_DOMAIN__ -> firecrawl-gateway.__NEW_DOMAIN__",
        "Tool-gateway domain suffix", None, password=False, advanced=True),
    "TOOL_GATEWAY_SCHEME": _tool(
        "Shared tool-gateway URL scheme for __NEW_ORG__ Subscribers only, used to derive vendor hosts "
    ...
    "TOOL_GATEWAY_USER_TOKEN": _tool(
        "Explicit __NEW_ORG__ Subscriber access token for tool-gateway requests (optional; otherwise "
        "read from the Anika auth store)", "Tool-gateway user token", None, advanced=True),
    ...
    "PORCUPINE_ACCESS_KEY": _tool(
        "Picovoice access key for the Porcupine 'Hey Anika' wake word engine (optional; "
    ...
    "ANIKA_LANGFUSE_PUBLIC_KEY": _tool("Langfuse project public key (pk-lf-...)",
        "Langfuse public key", "https://cloud.langfuse.com", password=False),
    "ANIKA_LANGFUSE_SECRET_KEY": _tool("Langfuse project secret key (sk-lf-...)",
        "Langfuse secret key", "https://cloud.langfuse.com"),
    "ANIKA_LANGFUSE_BASE_URL": _tool("Langfuse server URL (default: https://cloud.langfuse.com)",
    ...
    "SLACK_ALLOWED_USERS": _msg(
        "Comma-separated Slack member IDs allowed to use Anika, e.g. U01ABC2DEF3. Without "
    ...
    "MATRIX_USER_ID": _msg("Matrix user ID (e.g. @anika:example.org)",
    ...
    "MATRIX_DEVICE_ID": _msg(
        "Stable Matrix device ID for E2EE persistence across restarts (e.g. ANIKA_BOT)",
    ...
    "IRC_CHANNEL": _msg("IRC channel to join (e.g. #anika)", "IRC channel", None),
    "IRC_NICKNAME": _msg("Bot nickname on IRC (default: anika-bot)", "IRC nickname", None),
    ...
    "API_SERVER_MODEL_NAME": _msg(
        "Model name advertised on /v1/models. Defaults to the profile name (or 'anika-agent' "
    ...
    "GATEWAY_PROXY_URL": _msg(
        "URL of a remote Anika API server to forward messages to (proxy mode). When set, the "
        ...
        "Remote Anika API server URL (e.g. http://192.168.1.100:8642)", None, advanced=True),
    "GATEWAY_PROXY_KEY": _msg(
        "Bearer token for authenticating with the remote Anika API server (proxy mode). Must "
    ...
    # ANIKA_TOOL_PROGRESS_MODE (deprecated; use display.tool_progress) is intentionally NOT listed:
    # this dict feeds user-facing surfaces (dashboard keys page, setup checklists), so deprecated
    # knobs stay in config._EXTRA_ENV_KEYS only. ANIKA_TOOL_PROGRESS is unsupported.
    "ANIKA_PREFILL_MESSAGES_FILE": _setting(
        "Path to JSON file with ephemeral prefill messages for few-shot priming",
        "Prefill messages file path", None),
    "ANIKA_EPHEMERAL_SYSTEM_PROMPT": _setting(
