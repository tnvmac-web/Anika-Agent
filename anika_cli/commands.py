"""Slash command registry for the Anika CLI and gateway.
...
from anika_constants import INDICATOR_STYLES
...
class CommandDef:
    ...
    # Key in ``anika_cli.slash_exec.EXECUTORS`` (a string, not a callable: keeps this module
...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("snapshot", "Create or restore state snapshots of Anika config/state", "Session",
    ...
    CommandDef("goal", "Set a standing goal Anika works on across turns until achieved", "Session",
    ...
    CommandDef("plan", "Write a markdown implementation plan to .anika/plans/ without executing anything", "Session",
               args_hint="[task]"),
    ...
    CommandDef("wake", "Toggle the 'Hey Anika' wake word listener", "Configuration",
               cli_only=True, args_hint="[on|off|status]", subcommands=("on", "off", "status")),
    CommandDef("busy", "Control how messages behave while Anika is working", "Configuration",
    ...
    CommandDef("reload-skills", "Re-scan ~/.anika/skills/ for newly installed or removed skills",
    ...
    CommandDef("subscription", "View your __NEW_ORG__ plan and change it in the browser", "Info",
               cli_only=True, aliases=("upgrade",)),
    CommandDef("topup", "Show your __NEW_ORG__ balance and manage billing on the portal", "Info"),
    ...
    CommandDef("update", "Update Anika Agent to the latest version", "Info",
               busy_policy="dispatch", desktop="terminal"),
    CommandDef("version", "Show Anika Agent version", "Info", aliases=("v",),
...
def _collect_gateway_skill_entries(
    ...
    try:
        ...
        # ``anika skills list`` and the agent's ``/skill-name`` dispatch but
...
_PLUGIN_COMPAT_LAZY = {
    ...
    'discord_skill_commands_by_category': ('anika_cli.commands_platforms', 'discord_skill_commands_by_category'),
    'slack_app_manifest': ('anika_cli.commands_platforms', 'slack_app_manifest'),
    'slack_native_slashes': ('anika_cli.commands_platforms', 'slack_native_slashes'),
    'slack_subcommand_map': ('anika_cli.commands_platforms', 'slack_subcommand_map'),
    'telegram_bot_commands': ('anika_cli.commands_platforms', 'telegram_bot_commands'),
    'telegram_menu_commands': ('anika_cli.commands_platforms', 'telegram_menu_commands'),
    'telegram_menu_max_commands': ('anika_cli.commands_platforms', 'telegram_menu_max_commands'),
}
