"""Dashboard process-hygiene helpers — extracted from ``anika_cli/main.py``.

Helpers defined in ``anika_cli.main_dashboard`` / ``anika_cli.main_install_repair`` are imported at
...
_DASHBOARD_PATTERNS = tuple(
    ...
    for launcher in ("anika", "anika_cli.main", "anika_cli/main.py"))
...
def _iter_process_table() -> list[tuple[int, str]]:
    ...
    if sys.platform == "win32":
        ...
        return rows
    # ps, not `pgrep -f "anika.*dashboard"` (greedy regex; consistent with gateway pid scan).
...
def _scan_dashboard_processes(*, exclude_pids: set[int] | None = None) -> list[tuple[int, str]]:
    ...
    ``anika update`` (every API call 401s). *exclude_pids* (Desktop's ANIKA_DESKTOP_CHILD_PID
    backends) are never returned.

    *exclude_pids* is an optional set of PIDs that must never be returned. This is used by the Anika
    Desktop Electron app to protect its own backend child process: when the desktop spawns ``anika serve``
    as a backend and triggers an auto-update, the update must not kill the backend that the desktop itself
    manages. The desktop sets the environment variable ``ANIKA_DESKTOP_CHILD_PID`` on the spawned backend
    ...
    try:
        ...
        return []
    # Spawn-ledger augmentation: substring patterns miss profiled launches (`anika --profile p
    ...
    with contextlib.suppress(Exception):
        ...
        from anika_cli.process_identity import ledger_entries
...
def _anika_home_for_pid(pid: int) -> str | None:
    """Best-effort ``ANIKA_HOME`` from *pid*'s environment (psutil, then /proc)."""
    with contextlib.suppress(Exception):
        import psutil
        if home := psutil.Process(pid).environ().get("ANIKA_HOME"):
            return home
    try:
    ...
    for part in raw.split(b"\x00"):
        if part.startswith(b"ANIKA_HOME="):
...
def _is_ephemeral_port_zero_backend(argv: list[str]) -> bool:
    ...
    ``anika update`` multiplies listening backends because ``--port 0`` binds a fresh port.
...
def _profile_key_for_respawn(argv: list[str], anika_home: str | None = None) -> str:
    """Stable owner key: ``ANIKA_HOME`` when known, else ``--profile`` / ``-p``.
    ...
    """
    if anika_home:
        parts = _resolved_home(anika_home).parts
        if len(parts) >= 2 and parts[-2] == "profiles" and parts[-1]:
            return f"profile:{parts[-1]}"
        return f"home:{_normalized_home_for_compare(anika_home)}"
...
def _filter_dashboard_respawn_candidates(
    ...
    """Select which killed manual backends ``(pid, argv, anika_home)`` to respawn after update.

    Rules: never resurrect Desktop ``--port 0`` backends; never replay a backend from a
    **foreign** ``ANIKA_HOME`` (the argv-only respawn would come back on this install's home
    ...
    (``ANIKA_DESKTOP_CHILD_PID``) owns their lifecycle. These are also the PPID-1 orphans that previously
    ...
    Intentionally does **not** blanket-skip every PPID-1 process: a prior ``anika update`` respawn detaches
    ...
    for _pid, argv, anika_home in candidates:
        if not argv or _is_ephemeral_port_zero_backend(argv):
            continue
        if own_key and anika_home and _normalized_home_for_compare(anika_home) != own_key:
            continue
        norm = _normalize_dashboard_cmdline(argv)
        profile_key = _profile_key_for_respawn(argv, anika_home)
...
def _exclude_pids_from_env() -> set[int]:
    """PIDs Desktop marks as live backends (``ANIKA_DESKTOP_CHILD_PID``, comma-separated)."""
    out: set[int] = set()
    for part in os.environ.get("ANIKA_DESKTOP_CHILD_PID", "").split(","):
...
def _kill_pids_windows(pids: list[int], killed: list[int], failed: list[tuple[int, str]]) -> None:
    ...
    for pid in pids:
        try:
            ...
            elif not pid_is_anika(pid, expected_start_time=expected_start_time):
                failed.append((pid, "not anika-owned or process identity changed"))
            else:
...
def _kill_stale_dashboard_processes(
    ...
    """Kill running ``anika dashboard`` / ``anika serve`` processes (update end, ``--stop``).
    ...
    *already_restarted_units* names units (no ``.service`` suffix) the caller already restarted directly —
    e.g. ``anika update``'s systemd fleet-restart loop, which restarts ``anika-serve*`` units before this
    ...
    """
    from anika_cli import main_dashboard as _dash

    if restart_managed and _dash._restart_managed_dashboard_service(reason):
        # The dashboard unit is handled but other backends (e.g. anika-serve.service) are not:
        # mark the unit handled so the filter below drops its PIDs, and keep going.
        _dash_unit = getattr(_dash, "_DASHBOARD_SYSTEMD_UNIT", "anika-dashboard.service")
    ...
    if restart_managed and sys.platform != "win32":
        for pid in pids:
            ...
            if not pid_service[pid] and (cmdline := _dash._dashboard_cmdline_for_pid(pid)):
                # Manual process: exact argv + ANIKA_HOME for the respawn and its profile cap.
                # Manually-started process: preserve its exact argv so we can respawn it after the update
                # (#40449, #68934). Snapshot ANIKA_HOME before the kill so per-profile caps still work
                # after the process is gone (#78821).
                pid_cmdline[pid] = cmdline
                pid_home[pid] = _anika_home_for_pid(pid)
...
def _restart_killed_backends(
    ...
    # Two categories: Without this, a remote backend (anika serve) under Restart=on-failure never comes
...
def _detect_concurrent_anika_instances(
    ...
    Windows blocks DELETE/REPLACE on a running .exe, so a Desktop-spawned ``anika.EXE`` makes
    ...
    *shim* ancestor (the setuptools launcher is a separate native process from its
    ...
    """
    from anika_cli.main_install_repair import _anika_exe_shims, _is_windows
    ...
    try:
        ...
        return []
    shim_paths = {_norm_exe(shim) for shim in _anika_exe_shims(scripts_dir)}
    if not shim_paths:
        return []
...
def _is_desktop_local_serve_cmdline(command: str) -> bool:
    """True for the Desktop-local shape ``anika serve [--isolated] --host 127.0.0.1 --port 0``.
    ...
    """
    cmd = command.lower()
    if "serve" not in cmd or ("anika" not in cmd and "anika_cli" not in cmd):
        return False
...
# (apps/desktop/electron/remote-lifecycle.ts) for every ``anika serve`` it spawns. Such a backend
...
def _anika_home_dir() -> Path:
    """Resolved Anika home (ANIKA_HOME override or ~/.anika)."""
    override = os.environ.get("ANIKA_HOME", "").strip()
    return Path(override).expanduser() if override else Path.home() / ".anika"
...
def _valid_lockfile_payload(parsed: object, ownership_id: str) -> bool:
    ...
    if any(not isinstance(parsed.get(f), str) or len(parsed[f]) > 1024
           for f in ("profile", "anikaPath", "anikaHome", "logPath", "startedAt")):
        return False
    # Suffix-only check of logPath so a relocated ANIKA_HOME can't reject a legitimate backend.
...
def _lock_owned_serve_pids(base_dir: Path | None = None) -> set[int]:
    """PIDs claimed by valid ``{anika_home}/desktop-ssh/<ownershipId>/backend.lock.json`` records
    (best-effort: a bad record contributes no PID; never raises)."""
    import json
    root = base_dir if base_dir is not None else _anika_home_dir() / _REMOTE_LOCK_SUBDIR
    owned: set[int] = set()
    try:
...
def _reap_orphaned_desktop_local_serves(
    *, reason: str = "orphaned desktop-local anika serve", signal_term=None, signal_kill=None,
    sleep_fn=None, lock_owned_pids_fn=None, process_age_seconds_fn=None) -> dict[str, list]:
    """Kill leftover Desktop-local ``anika serve`` backends with no parent. Never raises.
    ...
    0/1; not self / parent / ANIKA_DESKTOP_CHILD_PID; not claimed by a valid
    ...
    after ANIKA_BACKEND_READY, so a live sibling mid-startup is briefly unowned).
    """
    import signal as _signal
    import time as _time
