"""``anika logs`` — view and filter Anika log files.

``anika logs [name] [-n N] [-f] [--level L] [--session S] [--component C] [--since 1h]``;
``anika logs list`` shows the available files.
"""
...
def tail_log(
    ...
    *,
    ...
    log_path = get_anika_home() / "logs" / filename
    if not log_path.exists():
        print(f"Log file not found: {log_path}")
        print("(Logs are created when Anika runs — try 'anika chat' first)")
        sys.exit(1)
    ...
    print(f"--- {display_anika_home()}/logs/{filename}{filter_desc} ({mode}) ---")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_anika_home()}/logs/")
        return

    print(f"Log files in {display_anika_home()}/logs/:\n")
    found = False
    ...
    if not found:
        print("  (no log files yet — run 'anika chat' to generate logs)")
