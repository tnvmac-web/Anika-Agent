"""Shared constants for Anika Agent.
...
_UNSET = object()
_ANIKA_HOME_OVERRIDE: ContextVar[str | object] = ContextVar("_ANIKA_HOME_OVERRIDE", default=_UNSET)

# --- Backward-compat: deprecated HERMES_* env-var aliases (Phase 9) ---
_deprecated_env_warned: set[str] = set()


def _warn_deprecated_env(legacy_name: str, new_name: str) -> None:
    """Print a one-time deprecation warning for a legacy ``HERMES_*`` env var."""
    if legacy_name in _deprecated_env_warned:
        return
    _deprecated_env_warned.add(legacy_name)
    print(
        f"[anika] WARNING: {legacy_name} is deprecated; set {new_name} instead. "
        f"(Legacy alias will be removed in a future release.)",
        file=sys.stderr,
    )
    sys.stderr.flush()


def _get_env_with_compat(new_name: str, legacy_name: str, default: str = "") -> str:
    """Read *new_name* from the environment, falling back to *legacy_name*.

    ``new_name`` takes precedence; if it is unset, ``legacy_name`` (the old
    ``HERMES_*`` alias) is consulted and a one-time deprecation warning is
    emitted.  This lets existing users keep ``HERMES_HOME`` / ``HERMES_QUIET``
    etc. working during the Anika rebrand transition.
    """
    val = os.environ.get(new_name, "").strip()
    if val:
        return val
    val = os.environ.get(legacy_name, "").strip()
    if val:
        _warn_deprecated_env(legacy_name, new_name)
        return val
    return default
...
def set_anika_home_override(path: str | Path | None) -> Token:
    """Set a context-local Anika home override and return its reset token.

    Deliberately does not mutate ``os.environ`` (shared by every thread in the process).
    """
    value: str | object = _UNSET if path is None else str(path)
    return _ANIKA_HOME_OVERRIDE.set(value)


def reset_anika_home_override(token: Token) -> None:
    """Restore the previous context-local Anika home override."""
    _ANIKA_HOME_OVERRIDE.reset(token)


def get_anika_home_override() -> str | None:
    """Return the active context-local Anika home override, if any."""
    override = _ANIKA_HOME_OVERRIDE.get()
    return str(override) if override is not _UNSET and override else None


def _get_platform_default_anika_home() -> Path:
    """Return the platform-native default Anika home path."""
    if sys.platform == "win32":
        local_appdata = os.environ.get("LOCALAPPDATA", "").strip()
        base = Path(local_appdata) if local_appdata else Path.home() / "AppData" / "Local"
        return base / "anika"
    return Path.home() / ".anika"


def _warn_profile_fallback_once() -> None:
    """Warn once when ANIKA_HOME is unset but a non-default profile is sticky-active (wrong fallback)."""
    global _profile_fallback_warned
    if _profile_fallback_warned:
        return
    try:
        fallback_home = _get_platform_default_anika_home()
    ...
    if active and active != "default":
        ...
        msg = (
            f"[ANIKA_HOME fallback] ANIKA_HOME is unset but active "
            f"profile is {active!r}. Falling back to {fallback_home}, which "
            f"is the DEFAULT profile — not {active!r}. Any data this "
            f"process writes will land in the wrong profile. The "
            f"subprocess spawner should pass ANIKA_HOME explicitly "
            f"(see issue #18594)."
        )
...
def get_anika_home() -> Path:
    """Anika home: context-local override → ``ANIKA_HOME`` env var → platform default."""
    override = get_anika_home_override()
    if override:
        return Path(override)
    if not os.environ.get("ANIKA_HOME", "").strip():
        _warn_profile_fallback_once()
    return get_process_anika_home()
...
def anika_home_key(path: str | Path | None = None) -> str:
    """Stable registry key for an Anika home/profile dir.
    ...
    """
    candidate = Path(path) if path is not None else get_anika_home()
    raw = str(candidate)
...
def reset_anika_home_key_cache() -> None:
    """Forget every remembered home key (for tests that move a home dir on disk)."""
    _HOME_KEY_CACHE.clear()


def get_process_anika_home() -> Path:
    """Anika home of the running process, ignoring task overrides.
    ...
    Reads ``ANIKA_HOME``; falls back to the legacy ``HERMES_HOME`` with a
    deprecation warning for backward compatibility during the Anika rebrand.
    """
    val = _get_env_with_compat("ANIKA_HOME", "HERMES_HOME")
    return Path(val) if val else _get_platform_default_anika_home()


# get_default_anika_root() memo keyed on (native home, ANIKA_HOME) so it stays
# fresh when a test or plugin mutates ANIKA_HOME; saves ~80us/call at 31+ sites.
_default_anika_root_memo: "tuple[str, str, Path] | None" = None


def get_default_anika_root() -> Path:
    """Root Anika dir for profile-level ops: ``<root>`` when ``ANIKA_HOME=<root>/profiles/<name>``."""
    global _default_anika_root_memo
    native_home = _get_platform_default_anika_home()
    env_home = os.environ.get("ANIKA_HOME", "")
    memo = _default_anika_root_memo
    ...
    if env_home:
        ...
        try:
            env_path.resolve().relative_to(native_home.resolve())  # under ~/.anika (normal or profile mode)
        except ValueError:  # Docker/custom root: <root>/profiles/<name> -> <root>, else ANIKA_HOME itself
            result = env_path.parent.parent if env_path.parent.name == "profiles" else env_path
    _default_anika_root_memo = (str(native_home), env_home, result)
    return result
...
# Files marking a real Anika home; arbitrary dirs with a ``profiles`` segment lack them.
_ANIKA_HOME_MARKERS = ("config.yaml", ".env", "state.db")


def _is_anika_profiles_root(profiles_dir: Path) -> bool:
    """True when *profiles_dir* is provably ``<anika-home>/profiles``.

    Accepts the classic ``~/.anika`` layout, a root carrying Anika-home marker files, a
    ...
    try:
        if (profiles_dir / _DELETED_PROFILES_DIR).is_dir() or any(
            (root / marker).exists() for marker in _ANIKA_HOME_MARKERS
        ):
            return True
    except OSError:
        pass
    try:
        return root.resolve(strict=False) == get_default_anika_root().resolve(strict=False)
    except OSError:
        return False
...
def named_profile_home(path: str | Path) -> Path | None:
    ...
    Requires ``<name>`` not to start with ``.`` and the ``profiles`` parent to be a real Anika home;
    ...
    for candidate in (current, *current.parents):
        if (candidate.parent.name == "profiles" and not candidate.name.startswith(".")
                and _is_anika_profiles_root(candidate.parent)):
            return candidate
        if candidate.name == ".anika":  # default home: a coincidental profiles/ ancestor is not a root
            return None
    return None
...
def mkdir_under_anika_home(path: str | Path) -> Path:
...
def _packaged_dir(env_var: str, default: Path | None, subdir: str) -> Path:
    ...
    return Path(override) if override else default if default is not None else get_anika_home() / subdir


def get_optional_skills_dir(default: Path | None = None) -> Path:
    """Return the optional-skills directory, honoring package-manager wrappers."""
    return _packaged_dir("ANIKA_OPTIONAL_SKILLS", default, "optional-skills")


def get_optional_mcps_dir(default: Path | None = None) -> Path:
    """Return the optional-mcps directory, honoring package-manager wrappers (``ANIKA_OPTIONAL_MCPS``)."""
    return _packaged_dir("ANIKA_OPTIONAL_MCPS", default, "optional-mcps")


def get_bundled_skills_dir(default: Path | None = None) -> Path:
    """Return the bundled skills directory, honoring package-manager wrappers (``ANIKA_BUNDLED_SKILLS``)."""
    return _packaged_dir("ANIKA_BUNDLED_SKILLS", default, "skills")


def get_anika_dir(new_subpath: str, old_name: str, *, home: Path | None = None) -> Path:
    """Resolve an Anika subdirectory, honouring a populated legacy ``<old_name>/`` (no migration).
    ...
    """
    home = home or get_anika_home()
    old_path = home / old_name
    return old_path if _legacy_path_has_content(old_path) else home / new_subpath


def iter_anika_node_dirs(home: Path | None = None) -> list[Path]:
    """Anika-managed Node dirs in lookup order; both Windows and POSIX shapes so migrated installs work.

    Keep in sync with anikaManagedNodePathEntries() in apps/desktop/electron/backend-env.ts.
    """
    node_dir = (home or get_anika_home()) / "node"
...
def _iter_managed_node_candidates(names: list[str], home: Path | None = None):
    ...
    for directory in iter_anika_node_dirs(home):
        for name in names:
...
def _version_probe_ok(path: str) -> bool:
    """True when ``<path> --version`` exits 0 under the Anika-managed Node PATH."""
    result = _run_version_probe([path, "--version"], env=with_anika_node_path())
    return result is not None and result.returncode == 0


_ANIKA_NODE_TARGET_MAJOR = int(os.environ.get("ANIKA_NODE_TARGET_MAJOR", "22"))
_managed_node_heal_attempted = False
_NODE_BOOTSTRAP_SCRIPT = Path(__file__).resolve().parent / "scripts" / "lib" / "node-bootstrap.sh"

# Install tree root (this file lives at <install_root>/anika_constants.py). Used by secure_parent_dir() to
# skip chmod on the install dir — chmodding it 0700 breaks anika-user traversal in Docker (UID 10000). See
# #25821, #93050.
...
def anika_managed_node_tree_present(home: Path | None = None) -> bool:
    """Return True when any Anika-managed node/npm/npx shim exists on disk."""
...
def _print_managed_node_in_use_notice() -> None:
    ...
    print(
        "→ Anika-managed Node.js is in use by a running app; deferring its "
        "upgrade until the app is closed (re-run `anika update` afterwards).", flush=True,
    )
...
def _stage_windows_node_zip(home: Path, node_arch: str) -> Path | None:
    ...
    index_url = f"https://nodejs.org/dist/latest-v{_ANIKA_NODE_TARGET_MAJOR}.x/"
    index_bytes = _fetch_url(index_url, 60)
    if index_bytes is None:
        return None
    pattern = rf"node-v{_ANIKA_NODE_TARGET_MAJOR}\.\d+\.\d+-win-{node_arch}\.zip"
...
def _heal_managed_node_windows(home: Path | None = None) -> bool | None:
    """Redownload the portable Node zip into ``%ANIKA_HOME%\\node`` on Windows.
    ...
    (images are mapped with ``FILE_SHARE_DELETE`` — the same mechanism as the anika.exe quarantine); when
...
def _run_node_bootstrap(func: str, *, timeout: int, **extra_env: str) -> bool:
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "ANIKA_HOME": str(get_anika_home()), **extra_env},
...
def bootstrap_anika_managed_node() -> str | None:
    """Install a Anika-managed Node tree under ``$ANIKA_HOME/node`` and return its npm path.

    Anika never modifies a user-owned toolchain (system, nvm, brew, Nix) that fails ``engines``.
    """
    existing = find_anika_node_executable("npm")
    if existing:
        return existing
    if sys.platform == "win32":
        ok = _heal_managed_node_windows()
    else:
        # ANIKA_NODE_SKIP_LINKS=1 keeps node/npm/npx out of ~/.local/bin: never shadow the user toolchain.
        ok = _run_node_bootstrap("_nb_install_bundled_node", timeout=600, ANIKA_NODE_SKIP_LINKS="1")
    if not ok:
        return None
    return _first_runnable_managed(_candidate_node_command_names("npm"))[0]


def heal_anika_managed_node() -> bool:
    """Redownload Anika-managed Node when the tree exists but is broken; at most once per process.
    ...
    if _managed_node_heal_attempted or not anika_managed_node_tree_present():
        return False
    if sys.platform == "win32":
...
def _managed_node_tree_outdated(home: Path | None = None) -> bool:
    ...
    for candidate in _iter_managed_node_candidates(_candidate_node_command_names("node"), home):
        ...
        if "-" in version:
            return True
        return major < _ANIKA_NODE_TARGET_MAJOR
    return False


def find_anika_node_executable(command: str) -> str | None:
    """Anika-managed Node/npm path, healing broken/outdated trees; heal failure still returns old Node."""
    ...
    if needs_heal and heal_anika_managed_node():
...
def find_node_executable(command: str) -> str | None:
    ...
    """
    managed = find_anika_node_executable(command)
    if managed:
        return managed
    if anika_managed_node_tree_present():
        return None
    return find_node_executable_on_path(command)


def with_anika_node_path(env: dict[str, str] | None = None) -> dict[str, str]:
    """Return *env* with Anika-managed Node directories prepended to PATH."""
    merged = dict(os.environ if env is None else env)
    parts = [p for p in merged.get("PATH", "").split(os.pathsep) if p]
    for entry in reversed([str(path) for path in iter_anika_node_dirs() if path.is_dir()]):
...
def agent_browser_runnable(path: str | None) -> bool:
    ...
    local ``node_modules/agent-browser/bin/...`` binary, which then disappears on the next ``anika update``
...
def display_anika_home() -> str:
    """User-facing ``~/`` display string for ANIKA_HOME (``~/.anika/profiles/coder``)."""
    home = get_anika_home()
    try:  # as_posix(): str() on Windows yields chimeras like ~/AppData\Local\anika/skills/
...
def secure_parent_dir(path: Path) -> None:
    """Chmod ``0o700`` on *path*'s parent, refusing ``/`` and top-level dirs (misresolved ANIKA_HOME)."""
    parent = path.parent.resolve()
    if parent == Path("/") or len(parent.parts) < 3:
        return
    # Refuse the install tree: chmod 0700 breaks anika-user traversal in Docker (UID 10000).
    # A credential file here means ANIKA_HOME misresolved; surface it (caused production lockouts).
    # See #25821, #93050.
    if parent == _INSTALL_ROOT or _INSTALL_ROOT in parent.parents:
        ...
        logging.getLogger(__name__).warning(
            ...
            "anika-agent install directory (%s). Credential files are "
            "normally stored under the anika home directory instead.", parent, _INSTALL_ROOT,
        )
        return
    with contextlib.suppress(OSError):
...
def _profile_home_path(env: dict[str, str] | None = None) -> str | None:
    """Return ``{ANIKA_HOME}/home`` when the profile-home directory exists."""
    anika_home = get_anika_home_override() or (env or {}).get("ANIKA_HOME") or _get_env_with_compat("ANIKA_HOME", "HERMES_HOME")
    if not anika_home:
        return None
    profile_home = os.path.join(anika_home, "home")
...
def _env_get(env: dict[str, str], key: str, default: str = "", legacy_name: str | None = None) -> str:
    """Stripped *key* from *env*, falling back to the process environment."""
    result = env.get(key) or os.getenv(key, default)
    if not str(result).strip() and legacy_name:
        result = os.getenv(legacy_name, default)
        if str(result).strip():
            _warn_deprecated_env(legacy_name, key)
...
def _iter_real_home_candidates(env: dict[str, str] | None = None) -> list[str]:
    ...
    env = env or {}
    candidates = [_env_get(env, "ANIKA_REAL_HOME", legacy_name="HERMES_REAL_HOME"), _env_get(env, "HOME")]
    with contextlib.suppress(Exception):
        import pwd
...
def get_real_home(env: dict[str, str] | None = None) -> str:
    """The OS user's real home, avoiding the Anika profile HOME.

    ``HOME`` belongs to the OS account and external CLIs keeping credentials under ``~``; a parent
    already running with ``HOME={ANIKA_HOME}/home`` is repaired back when possible.
    """
...
def get_subprocess_home(env: dict[str, str] | None = None) -> str | None:
    ...
    ``{ANIKA_HOME}/home``; ``real``: always real HOME; ``profile``: always the profile home.
    """
    env = env or {}
    profile_home = _profile_home_path(env)
...
def apply_subprocess_home_env(env: dict[str, str]) -> None:
    """Apply Anika' subprocess HOME contract to *env* in-place."""
    real_home = get_real_home(env)
    if real_home:
        env["ANIKA_REAL_HOME"] = real_home
...
def translate_cwd_for_wsl_backend(cwd: str) -> str:
    """Map a Windows-host cwd (drive path or ``\\\\wsl.localhost\\`` UNC) to POSIX when Anika runs in WSL.
...
def is_container() -> bool:
    ...
    See: __NEW_ORG__/anika-agent#47111
    """
...
def get_config_path() -> Path:
    """Return the path to ``config.yaml`` under ANIKA_HOME."""
    return get_anika_home() / "config.yaml"


def get_skills_dir() -> Path:
    """Return the path to the skills directory under ANIKA_HOME."""
    return get_anika_home() / "skills"


def get_env_path() -> Path:
    """Return the path to the ``.env`` file under ANIKA_HOME."""
...
def apply_ipv4_preference(force: bool = False) -> None:
    ...
    if getattr(socket.getaddrinfo, "_anika_ipv4_patched", False):
        return
    ...
    def _ipv4_getaddrinfo(host, port, family=0, type=0, proto=0, flags=0):
        ...
        return _original_getaddrinfo(host, port, family, type, proto, flags)
    _ipv4_getaddrinfo._anika_ipv4_patched = True  # type: ignore[attr-defined]
...
def venv_bin_dir(venv_dir, *, windows: bool | None = None) -> Path:
    ...
    Canonical helper for venv layout. This was open-coded in seven places across four ``anika_cli`` modules
    ...
    lived 2400 lines away in another function. A few sites outside ``anika_cli``
...
def is_first_party_module(name: str | None) -> bool:
    """True when *name* ships with Anika (exact first segment; ``startswith`` would claim ``agentops``)."""
    root = str(name).split(".")[0] if name else ""
    return bool(root) and (root in FIRST_PARTY_MODULE_ROOTS or root.startswith("anika_"))
...
def partial_update_hint(exc: BaseException) -> list[str]:
    ...
    return [
        ...
        "    anika update",
        "If that also fails, reinstall: https://anika-agent.__NEW_DOMAIN__",
    ]
