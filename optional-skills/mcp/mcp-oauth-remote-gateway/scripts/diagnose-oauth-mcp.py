...
Usage:
  ...
  <server_name> matches the files in $ANIKA_HOME/mcp-tokens/<server>.json etc.
...
def _anika_home():
    # Prefer Anika' own resolver (profile-safe); fall back to env then ~/.anika.
    try:
        from anika_constants import get_anika_home
        return str(get_anika_home())
    except Exception:
        return os.environ.get("ANIKA_HOME") or os.path.expanduser("~/.anika")


def _tokens_dir():
    return os.path.join(_anika_home(), "mcp-tokens")
    ...
    status, hdrs, body = _post(
        ...
        data={
            ...
            "params": {"protocolVersion": "2025-06-18", "capabilities": {},
                       "clientInfo": {"name": "anika-diag", "version": "1.0"}},
        },
        headers={
