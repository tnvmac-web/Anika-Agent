...
All per-subject data lives under PDD_DATA_DIR (default: $ANIKA_HOME/unbroker),
which is the same trust boundary Anika uses for .env and OAuth tokens.
"""
...
def anika_home() -> Path:
    return Path(os.environ.get("ANIKA_HOME") or (Path.home() / ".anika"))


def data_dir() -> Path:
    override = os.environ.get("PDD_DATA_DIR")
    return Path(override) if override else anika_home() / "unbroker"
