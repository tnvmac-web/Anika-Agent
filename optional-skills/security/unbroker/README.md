...
The design is **Anika-native**: a small deterministic Python CLI (`scripts/pdd.py`) owns the state
...
```bash
anika skills install official/security/unbroker
```

Then start a new Anika session and drive it (below). The skill works zero-config; a few optional
...
- **PII handling.** Dossiers live under the Anika home directory (`0600`, optionally
