...
## 5. GODMODE FAST — Anika 4 405B (Zero Refusal)

**Model:** `__NEW_ORG__/anika-4-405b`
**Strategy:** Anika models are already uncensored, so this combo is about speed — instant token streaming with zero refusal checking needed. Uses the same Pliny Love divider format as a formality.
...
## Using with Anika Agent

### As ephemeral system prompt (config.yaml)

Pick a template above and set it in `~/.anika/config.yaml`:
