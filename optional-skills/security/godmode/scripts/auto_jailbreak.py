...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("ANIKA_HOME", os.path.expanduser("~/.anika")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("ANIKA_HOME", Path.home() / ".anika")) / "skills" / "red-teaming" / "godmode"
...
ANIKA_HOME = Path(os.getenv("ANIKA_HOME", Path.home() / ".anika"))
CONFIG_PATH = ANIKA_HOME / "config.yaml"
PREFILL_PATH = ANIKA_HOME / "prefill.json"
...
MODEL_STRATEGIES = {
    ...
    # Nous/Anika models — already uncensored, just needs clean prompt
    "anika": {
        "order": ["prefill_only"],
        "system_templates": {},
    },
...
def _detect_model_family(model: str) -> str:
    ...
    if "grok" in model_lower or "x-ai" in model_lower:
        return "grok"
    if "anika" in model_lower or "nous" in model_lower:
        return "anika"
...
def _get_current_model() -> tuple:
    """Read current model and provider from Anika config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.anika/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart Anika for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart Anika for changes to take effect.")
