...
Usage (via cron with --no-agent):
    ...
    anika cron create api-events \\
      --schedule "*/1 * * * *" --no-agent \\
      --script "$ANIKA_HOME/skills/devops/watchers/scripts/watch_http_json.py" \\
...
def main() -> int:
    ...
    req = urllib.request.Request(args.url, headers={"User-Agent": "Anika-Watcher/1.0"})
