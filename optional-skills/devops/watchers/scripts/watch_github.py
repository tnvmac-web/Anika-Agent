...
Usage (via cron with --no-agent):
    ...
    anika cron create anika-issues \\
      --schedule "*/5 * * * *" --no-agent \\
      --script "$ANIKA_HOME/skills/devops/watchers/scripts/watch_github.py" \\
      --script-args "--name anika-issues --repo __NEW_ORG__/anika-agent --scope issues"

Set GITHUB_TOKEN (or GH_TOKEN) in the Anika .env file
(``${ANIKA_HOME:-~/.anika}/.env``) to avoid the 60 req/hr
