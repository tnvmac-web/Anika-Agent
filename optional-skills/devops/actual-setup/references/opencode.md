...
is typically only in `~/.anika/.env`, not a shell profile, so the env form
...
```
OpenCode DOES use the `provider/model` slash form on the CLI (unlike Anika,
...
`anika-custom-providers` skill, pitfall 2). Anika hits this because it forwards
