#!/usr/bin/env python3
"""Canvas LMS API CLI for Anika Agent.
...
def _check_config():
    ...
    if missing:
        anika_env = os.path.join(
            os.environ.get("ANIKA_HOME", os.path.expanduser("~/.anika")), ".env"
        )
        print(
            f"Missing required environment variables: {', '.join(missing)}\n"
            f"Set them in {anika_env} or export them in your shell.\n"
...
def main():
    parser = argparse.ArgumentParser(
        description="Canvas LMS API CLI for Anika Agent"
    )
