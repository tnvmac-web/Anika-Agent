...
ANIKA_HOME_DIR="${ANIKA_HOME:-$HOME/.anika}"
ANIKA_CFG="${ANIKA_HOME_DIR}/config.yaml"
MCP_PORT=40404
...
# ── 3. Ensure Anika config has twozero_td MCP entry ──
if [[ ! -f "$ANIKA_CFG" ]]; then
    echo -e " ${FAIL} Anika config not found at ${ANIKA_CFG}"
    manual_steps+=("Create ${ANIKA_CFG} with twozero_td MCP server entry")
elif grep -q 'twozero_td' "$ANIKA_CFG" 2>/dev/null; then
    echo -e " ${OK} twozero_td MCP entry exists in Anika config"
else
    echo -e " ${WARN} Adding twozero_td MCP entry to Anika config..."
    python3 -c "
...
" 2>/dev/null && echo -e " ${OK} twozero_td MCP entry added to config" \
              || { echo -e " ${FAIL} Could not update config (is PyYAML installed?)"; \
                   manual_steps+=("Add twozero_td MCP entry to ${ANIKA_CFG} manually"); }
    manual_steps+=("Restart Anika session to pick up config change")
fi
