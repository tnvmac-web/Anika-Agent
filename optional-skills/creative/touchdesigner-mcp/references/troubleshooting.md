...
## 5. Anika Configuration

### Config location

`$ANIKA_HOME/config.yaml` (defaults to `~/.anika/config.yaml` when `ANIKA_HOME` is unset)
...
Restart the Anika session for changes to take effect. The MCP connection is
