...
anika kanban watch --tenant <project-slug>

# Snapshot of the board
anika kanban list --tenant <project-slug>
anika kanban list --tenant <project-slug> --json     # machine-readable

# Per-status counts + oldest-ready age
anika kanban stats --tenant <project-slug>

# Visual dashboard (browser)
anika dashboard

# Inspect a specific task (includes comments + events)
anika kanban show <task-id>

# Follow a single task's event stream
anika kanban tail <task-id>
```

Verify available subcommands with `anika kanban --help` — the kanban CLI
...
|---------|--------------|--------|
| Task RUNNING but no heartbeat in 2+ min | Worker stuck, infinite loop, blocked on input | `anika kanban show <id>` — read the worker's last events. The dispatcher SIGTERMs tasks that exceed their `max-runtime`; if you need to stop one earlier, `anika kanban block <id>` then `anika kanban archive <id>`, and create a re-run task. |
| Same task retried 2+ times | Reproducible failure (missing key, bad spec, broken tool) | `anika kanban show <id>` to read failure events. Fix root cause before re-running. |
| RUNNING longer than max_runtime | Task is slow but progressing OR genuinely stuck | Check heartbeats with `anika kanban tail <id>`. If progressing, the dispatcher will SIGTERM eventually anyway — raise `max-runtime` on a re-created task. |
...
| Tasks created but never picked up | Profile not running, or tenant mismatch, or dispatcher not running | Check `anika profile list` (profile exists?), `anika status` (gateway/dispatcher up?), and verify tenant. |
...
anika kanban comment <renderer-task-id> "Scene 3 looks too sparse \
— increase visual density. Tighten color palette to brand spec."

# 2. Create a re-render task with the original as parent
anika kanban create "Scene 3 — re-render with feedback" \
...
NEW_TASK_ID=$(anika kanban create "Generate SRT captions from voiceover" \
    --assignee captioner \
    --workspace dir:"$HOME/projects/video-pipeline/<slug>" \
    --tenant <slug> \
...
anika kanban link "$NEW_TASK_ID" <editor-task-id>
```
...
anika kanban block <task-id>
anika kanban archive <task-id>

# Diagnose what happened
anika kanban show <task-id>      # task body, comments, recent events
anika kanban tail <task-id>      # follow the live event stream
anika kanban log <task-id>       # worker process log
```
...
3. Re-fire the initial `anika kanban create` for the director
...
while true; do
    clear
    anika kanban list --tenant <slug>
    echo "---"
    anika kanban stats --tenant <slug>
    sleep 30
done
```

For a live event feed, run `anika kanban watch --tenant <slug>` in a
...
- **Profile process not running.** Tasks queue indefinitely in READY if no
  worker for that profile is online. Check `anika profile list` and start
