...
the actual `setup.sh` that creates the project workspace, configures Anika
...
> [__NEW_ORG__/kanban-video-pipeline](https://github.com/__NEW_ORG__/kanban-video-pipeline).
...
2. **Create profiles** — `anika profile create <name> --clone`
3. **Configure profiles** — patch each profile's
   `~/.anika/profiles/<name>/config.yaml` to set toolsets, always_load skills,
   and `cwd`
4. **Write SOUL.md per profile** — the personality + role definition
5. **Copy any provided assets + write `brief.md`, `TEAM.md`, and `taste/`**
6. **Fire the initial kanban task** — `anika kanban create` assigned to the director
...
```bash
anika profile create director --clone 2>/dev/null || true
```
...
Each profile has a YAML config at `~/.anika/profiles/<name>/config.yaml`. The
...
p = os.path.expanduser(f"~/.anika/profiles/{profile}/config.yaml")
with open(p) as f:
...
PyYAML must be installed in the user's Python (it ships with most Anika
...
Each profile gets a `SOUL.md` at `~/.anika/profiles/<name>/SOUL.md` that
...
```bash
anika kanban create "Direct production of <video title>" \
    --assignee director \
...
the Anika `.env` (`${ANIKA_HOME:-$HOME/.anika}/.env`) and macOS Keychain
(if on macOS):
...
check_key() {
    ...
    local kc_account="$2"
    local kc_service="$3"
    local _anika_env="${ANIKA_HOME:-$HOME/.anika}/.env"
    if grep -q "^${var}=" "$_anika_env" 2>/dev/null && \
       [ -n "$(grep "^${var}=" "$_anika_env" | cut -d= -f2-)" ]; then
        return 0
    fi
    ...
    echo "ERROR: ${var} not set in ${_anika_env} or Keychain (${kc_account}/${kc_service})"
    return 1
}

check_key ELEVENLABS_API_KEY anika ELEVENLABS_API_KEY || exit 1
check_key OPENROUTER_API_KEY anika OPENROUTER_API_KEY || exit 1
# ...
```
