...
Maps each role archetype to the Anika skills it should `always_load` and the
toolsets it needs. Only references skills that ship in the public anika-agent
...
## Anika skills relevant to video production

### Visual / rendering skills (`anika-agent/skills/creative/`)
...
### Design / pre-production skills (`anika-agent/skills/creative/`)
...
### Audio / media skills (`anika-agent/skills/creative/` + `skills/media/`)
...
These are **not** Anika skills but external CLIs / APIs that profiles invoke.
...
## Built-in Anika tools for media review

These are native Anika tools — not invoked via terminal but through their own
...
key is present in `${ANIKA_HOME:-~/.anika}/.env` (or macOS Keychain) before firing the kanban.
...
| Anthropic | `ANTHROPIC_API_KEY` | every Anika profile (Claude) |

If a key is missing, prompt the user to add it. Storage methods, in order of
preference: macOS Keychain → `${ANIKA_HOME:-~/.anika}/.env` → environment variable.
...
When a new Anika-public video skill ships:
