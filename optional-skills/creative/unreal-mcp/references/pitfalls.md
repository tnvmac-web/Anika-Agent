...
### 1. Start order: editor first, Anika second

Anika probes MCP servers at session start. If the editor (and its server)
...
`LogModelContextProtocol` with the address), then open a NEW Anika session.
...
restart the editor, restart the Anika session.
...
server fails to bind (Output Log) or Anika' probe times out. Fix: change
Server Port Number in Editor Preferences > Model Context Protocol AND the
`url` in `~/.anika/config.yaml` (`mcp_servers.unreal-engine`), then restart
...
### 5. GenerateClientConfig is not for Anika

`ModelContextProtocol.GenerateClientConfig` writes config files for Claude
Code/Cursor/VSCode/Gemini/Codex into the project root. Anika' connection
lives in `~/.anika/config.yaml` via `anika mcp install unreal-engine`.
Running GenerateClientConfig neither helps nor harms Anika — just don't
mistake it for the Anika setup step.
...
explicitly warns against overlapping calls. Anika executes same-turn tool
...
### 9. Timeouts: Anika gives up before Unreal does

Anika' default per-call timeout is 120 s. Asset imports, first-shader
...
`<Project>/Saved/Screenshots/...`). If Anika runs elsewhere (SSH backend,
