...
Anika Agent (see website/docs/getting-started/platform-support.md). The
...
build sandbox. ``nix/python.nix`` sets ``ANIKA_NIX_BUILD=1`` on the
Anika package derivation, so only that build may create an artifact.
...
_IN_NIX_BUILD = os.environ.get("ANIKA_NIX_BUILD") == "1"

_BLOCK_MESSAGE = (
    "Building wheels or sdists for anika-agent is not supported.\n"
    "Anika is distributed via the shell installer, Docker image, or Nix.\n"
    "See: https://anika-agent.__NEW_DOMAIN__/docs/getting-started/installation\n"
    "\n"
    "If you are developing, use an editable install instead:\n"
    "  uv sync          # or: uv pip install -e .\n"
    "\n"
    "If you are building with Nix (uv2nix), this error should not fire —\n"
    "the Anika Nix derivation sets ANIKA_NIX_BUILD=1. If it does, file a bug."
)
...
# Root single-file modules (``run_agent``, ``anika_state``, ``toolsets``...)
