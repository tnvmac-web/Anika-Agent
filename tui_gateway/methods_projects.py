...
def _is_repo_junk(root: str) -> bool:
    ...
    ANIKA_HOME. User-created projects pointing there are still honored."""
    if not root:
        return True
    from anika_constants import get_anika_home
    real = os.path.realpath(root)
    anika_home = os.path.realpath(str(get_anika_home()))
    return (
        os.path.normcase(real) in _non_workspace_dirs()
        or real == anika_home
        or real.startswith(anika_home + os.sep))


def _is_session_cwd_junk(cwd: str) -> bool:
    """A non-git cwd that stays in flat Recents. A DESCENDANT of ANIKA_HOME may be an
    intentional prose/data workspace, so only ANIKA_HOME itself is excluded here."""
    if not cwd:
        return True
    from anika_constants import get_anika_home
    real = os.path.normcase(os.path.realpath(cwd))
    anika_home = os.path.normcase(os.path.realpath(str(get_anika_home())))
    return real in _non_workspace_dirs() or real == anika_home
...
def _scan_discovered_repos_remote(conn, policy: dict) -> bool:
    ...
    host must scan its own disk so repos with zero Anika sessions still appear in the sidebar (#81723).
