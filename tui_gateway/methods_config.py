...
from anika_constants import display_anika_home as _display_anika_home
...
def _cfg_get_provider(params):
    from anika_cli.models import list_available_providers, normalize_provider
...
_CONFIG_GETTERS = {
    ...
    "profile": lambda params: {"home": str(_anika_home), "display": _display_anika_home()},
...
def _readiness_check(rid, params, probe):
    ...
    optional ``profile`` param's ANIKA_HOME + ``.env`` secret scope (ContextVars: concurrent checks
    ...
    try:
        ...
        def probe(profile, scoped):
            ...
            if (not provider_configured and provider == "bedrock"
                ...
                return fail("No Anika provider is configured.", source)
...
def _(rid, params: dict) -> dict:
    ...
    force-redaction pipeline as ``anika debug share --nous``; redaction is NOT client-controllable
    ...
    try:
        from anika_cli.debug import _redact_log_text, build_nous_bundle, collect_share_bundle
