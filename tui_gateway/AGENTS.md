...
activate with `anika --tui` or `ANIKA_TUI=1`. `tui_gateway` is ALSO the backend the Desktop app
...
themselves come from `anika_cli/commands.py` (`anika_cli/AGENTS.md`).
...
npm run dev       # watch mode (rebuilds anika-ink + tsx --watch)
npm start         # production
npm run build     # full build (anika-ink + tsc)
