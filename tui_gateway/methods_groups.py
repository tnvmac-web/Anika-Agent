...
_WORKER_UNAVAILABLE = "Group Chat worker is unavailable. Restart the Anika gateway and try again."
...
def _profile_name() -> str:
    return (os.getenv("ANIKA_PROFILE") or "default").strip() or "default"
...
def _profile_execution_policy(profile: str) -> dict:
    ...
    if _bound_server is not None and profile not in {_current_profile(), _profile_name()}:
        token = set_anika_home_override(str(_foreign_profile_home(profile)))
    try:
