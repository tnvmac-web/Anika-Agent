"""Drive the real ``anika fallback add`` under a Linux PTY into an ordinary picker error.
...
def _persisted_model(root: Path, env: dict) -> dict:
    ...
    out = subprocess.run(
        [sys.executable, "-c", "import json; from anika_cli.config import load_config; "
...
def run(root: Path, output: Path) -> dict:
    with tempfile.TemporaryDirectory(prefix="anika_test_fallback_") as home:
        hh = Path(home) / ".anika"
        hh.mkdir()
        ...
        env = {"PATH": os.environ["PATH"], "HOME": home, "ANIKA_HOME": str(hh),
        ...
        proc = subprocess.Popen([sys.executable, "-m", "anika_cli.main", "fallback", "add"],
