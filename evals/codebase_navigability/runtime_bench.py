...
Each probe runs in a FRESH subprocess with an isolated ANIKA_HOME so nothing is cached across reps.
...
ENV = {**os.environ, "ANIKA_HOME": HOME, "PYTHONPATH": TREE, "PYTHONDONTWRITEBYTECODE": "0", "OPENROUTER_API_KEY": "sk-bench-placeholder",
       "ANIKA_SKIP_UPDATE_CHECK": "1", "NO_COLOR": "1", "TERM": "dumb", "COLUMNS": "120"}
for k in list(ENV):
    if k.startswith(("ANIKA_SESSION", "ANIKA_PROFILE")): ENV.pop(k)
...
IMPORT_TARGETS = ["run_agent", "cli", "anika_cli.main", "gateway.run", "tools.registry", "anika_state", "tui_gateway.server", "anika_cli.web_server", "model_tools", "agent.prompt_builder"]
probe = r'''
...
tree=os.getcwd(); foreign=[m for m,mod in list(sys.modules.items()) if getattr(mod,"__file__",None) and "anika-agent" in mod.__file__ and not mod.__file__.startswith(tree) and "site-packages" not in mod.__file__]
...
# 2. CLI end-to-end startup: `anika --version`, `anika --help`, `anika doctor --help`, `anika config get model` (no network)
CLI = {"version": ["anika_cli/main.py", "--version"], "help": ["anika_cli/main.py", "--help"], "config_get": ["anika_cli/main.py", "config", "get", "model"], "tools_list": ["anika_cli/main.py", "tools", "--help"], "skills_help": ["anika_cli/main.py", "skills", "--help"]}
cli = {}
...
try:
    ...
    T("resolve_toolset_anika_default_ms", lambda: resolve_toolset("anika-default"), 200)
...
try:
    import anika_state, tempfile, uuid
    from pathlib import Path
    db=anika_state.SessionDB(Path(tempfile.mkdtemp())/"s.db") if hasattr(anika_state,"SessionDB") else None
    if db:
        sid=str(uuid.uuid4())
