"""Run the read-tool eval through the REAL Anika AIAgent.

For each task: fresh temp ANIKA_HOME, fresh fixture workspace, real
...
def run_task(task, model: str, provider: str, timeout_mult: float,
    ...
    anika_home = Path(tempfile.mkdtemp(prefix="readtool-home-")) / ".anika"
    ...
    try:
        ...
        shutil.rmtree(anika_home.parent, ignore_errors=True)
    return result
...
def main() -> int:
    ...
    if not os.environ.get("OPENROUTER_API_KEY"):
        raise SystemExit(
            ...
            "source ~/.anika/.env; set +a  — then relaunch."
        )
