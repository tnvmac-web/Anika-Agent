...
The run gets a throwaway ANIKA_HOME so no local config leaks in, and the
...
if _or_key:
    ...
    # Resolved by the orchestrator BEFORE ANIKA_HOME is redirected to the
    # throwaway home (auth state lives in the real profile). Never printed.
    _tok = os.environ.get("BUBENCH_NOUS_TOKEN", "").strip()
    if not _tok:
        raise SystemExit("no OPENROUTER_API_KEY and no __NEW_ORG__ auth available")
    _agent_auth = dict(
        base_url=os.environ.get("BUBENCH_NOUS_BASE_URL", "https://inference-api.__NEW_DOMAIN__/v1"),
        api_key=_tok,
        provider="nous",
    )
