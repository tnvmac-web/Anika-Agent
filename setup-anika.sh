...
# 5. Symlinks the 'anika' CLI command into a user-facing bin dir
...
echo ""
echo -e "${CYAN}⚕ Anika Agent Setup${NC}"
echo ""
...
if is_termux; then
    ...
    if [ -n "$UV_CMD" ]; then
        ...
        _uv_log="$(mktemp 2>/dev/null || echo "/tmp/anika-uv-install.$$.log")"
...
if is_termux; then
    # Anika currently declares requires-python >=3.11,<3.14. Termux can expose
    ...
    if [ -z "${PYTHON_PATH:-}" ]; then
        if command -v python >/dev/null 2>&1; then
            ...
            echo -e "${RED}✗${NC} Termux Python $PYTHON_FOUND_VERSION is not supported; Anika requires Python >=3.11,<3.14"
...
# ============================================================================
# PATH setup — symlink anika into a user-facing bin dir
# ============================================================================

echo -e "${CYAN}→${NC} Setting up anika command..."

ANIKA_BIN="$SCRIPT_DIR/venv/bin/anika"
COMMAND_LINK_DIR="$(get_command_link_dir)"
COMMAND_LINK_DISPLAY_DIR="$(get_command_link_display_dir)"
mkdir -p "$COMMAND_LINK_DIR"
ln -sf "$ANIKA_BIN" "$COMMAND_LINK_DIR/anika"
echo -e "${GREEN}✓${NC} Symlinked anika → $COMMAND_LINK_DISPLAY_DIR/anika"
...
# ============================================================================
# Seed bundled skills into ~/.anika/skills/
# ============================================================================

ANIKA_SKILLS_DIR="${ANIKA_HOME:-$HOME/.anika}/skills"
mkdir -p "$ANIKA_SKILLS_DIR"

echo ""
echo "Syncing bundled skills to ~/.anika/skills/ ..."
if "$SCRIPT_DIR/venv/bin/python" "$SCRIPT_DIR/tools/skills_sync.py" 2>/dev/null; then
    echo -e "${GREEN}✓${NC} Skills synced"
else
    # Fallback: copy if sync script fails (missing deps, etc.)
    if [ -d "$SCRIPT_DIR/skills" ]; then
        cp -rn "$SCRIPT_DIR/skills/"* "$ANIKA_SKILLS_DIR/" 2>/dev/null || true
...
echo "Other commands:"
echo "  anika status        # Check configuration"
if is_termux; then
    echo "  anika gateway       # Run gateway in foreground"
else
    echo "  anika gateway install # Install gateway service (messaging + cron)"
fi
echo "  anika cron list     # View scheduled jobs"
echo "  anika doctor        # Diagnose issues"
echo ""
...
if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
    ...
    "$SCRIPT_DIR/venv/bin/python" -m anika_cli.main setup
fi
