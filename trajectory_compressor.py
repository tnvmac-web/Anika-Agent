...
from anika_constants import OPENROUTER_BASE_URL, get_anika_home
from agent.retry_utils import jittered_backoff
from anika_cli.env_loader import load_anika_dotenv

# Load .env from ANIKA_HOME first, then project root as a dev fallback.
load_anika_dotenv(anika_home=get_anika_home(), project_env=Path(__file__).parent / ".env")
...
_PROVIDER_HOSTS: Tuple[Tuple[str, str], ...] = (
    ("openrouter.ai", "openrouter"), ("__NEW_DOMAIN__", "nous"), ("z.ai", "zai"), ("moonshot.ai", "kimi-coding"),
...
class TrajectoryCompressor:
    ...
    def _init_summarizer(self):
        ...
        if provider:
            ...
            if client is None:
                raise RuntimeError(f"Provider '{provider}' is not configured. Check your API key or run: anika setup")
