...
failed approvals, chmod 0600 data files, codes never logged. Storage: ~/.anika/pairing/
"""
...
# path would freeze the ANIKA_HOME/profile context at gateway boot, ignoring later
# context-local overrides, so the gateway and ``anika pairing`` CLI wrote different
...
# computing the path eagerly freezes it to whatever ANIKA_HOME/profile context existed at that exact import
# moment for the rest of the process's lifetime -- even if a context-local override (see
# anika_constants.set_anika_home_override) is established afterward. A freshly-started, short-lived
# process (e.g. the ``anika pairing`` CLI) re-imports this module later with the final environment already
...
# (__NEW_ORG__/anika-agent#93449). ``_default_pairing_dir()`` below resolves this fresh on every call in
...
def _default_pairing_dir() -> Path:
    return PAIRING_DIR if PAIRING_DIR is not None else get_anika_dir("platforms/pairing", "pairing")
...
def _write_allowlist_env(env_var: str, ids: list) -> None:
    ...
    with contextlib.suppress(Exception):
        from anika_cli.config import save_env_value, remove_env_value
...
def _load_json_file(path: Path) -> dict:
    ...
    try:
        ...
        try:
            ...
            # exec` runs as root and writes the file, then the gateway process — running as `anika` after
        ...
        logger.warning(
            ...
            "If you ran `docker exec <container> anika pairing approve ...` as root, "
            "re-run with `docker exec -u anika <container> ...` and "
            "chown the existing file to the anika user, or restart the "
...
class PairingStore:
    ...
    profile's ANIKA_HOME exactly as ``anika -p <name> pairing ...`` does, so multiplex
    ...
    def __init__(self, profile: Optional[str] = None):
        ...
        self._dir = get_anika_dir("platforms/pairing", "pairing", home=profile_home) if profile else _default_pairing_dir()
