"""Relay/connector support package for the Anika gateway.
...
def relay_display_name() -> Optional[str]:
    ...
    if not value:
        try:
            from anika_cli.skin_engine import get_active_skin  # late import: boot-safe
        ...
        # prefix every reply "**Anika Agent:**" and shadow the connector's
        # linked-owner fallback, which actually disambiguates. Only a customized
        # name is forwarded.
        if value == "Anika Agent":
            value = ""
...
def _resolve_relay_identity_token() -> str:
    ...
    Canonical resolver shared by runtime self-provision and ``anika gateway enroll``.
    Modes, in precedence order:
      ...
      2.  __NEW_ORG__ Portal (default): ``resolve_nous_access_token()``.
...
def self_provision_relay() -> bool:
    ...
    ``~/.anika/.env``), so a hosted container re-provisions every boot; the
    ...
    try:
        ...
        host = ""
    gateway_id = os.environ.get("GATEWAY_RELAY_ID", "").strip() or f"gw-{host or 'anika'}"
