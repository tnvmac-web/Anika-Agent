"""Session-scoped context variables for the Anika gateway.

Replaces the old ``os.environ``-based ``ANIKA_SESSION_*`` state with task-local ``ContextVar``s
...
_SESSION_VARS = (
    ...
    "ANIKA_SESSION_PLATFORM", "ANIKA_SESSION_SOURCE", "ANIKA_SESSION_CHAT_ID",
    "ANIKA_SESSION_CHAT_TYPE", "ANIKA_SESSION_CHAT_NAME", "ANIKA_SESSION_THREAD_ID",
    "ANIKA_SESSION_USER_ID", "ANIKA_SESSION_USER_ID_ALT", "ANIKA_SESSION_USER_NAME",
    "ANIKA_SESSION_SCOPE_ID", "ANIKA_SESSION_KEY", "ANIKA_SESSION_ID",
    "ANIKA_UI_SESSION_ID", "ANIKA_SESSION_MESSAGE_ID", "ANIKA_SESSION_PROFILE",
    "ANIKA_BROWSER_CONTROL_PRINCIPAL", "ANIKA_BROWSER_CONTROL_TRANSPORT_FAMILY",
    "ANIKA_CRON_SESSION",
))
...
_SESSION_ASYNC_DELIVERY = ContextVar("ANIKA_SESSION_ASYNC_DELIVERY", default=_UNSET)

# Cron auto-delivery vars, set per-job in run_job() so concurrent jobs don't clobber.
_CRON_AUTO_DELIVER_PLATFORM = ContextVar("ANIKA_CRON_AUTO_DELIVER_PLATFORM", default=_UNSET)
_CRON_AUTO_DELIVER_CHAT_ID = ContextVar("ANIKA_CRON_AUTO_DELIVER_CHAT_ID", default=_UNSET)
_CRON_AUTO_DELIVER_THREAD_ID = ContextVar("ANIKA_CRON_AUTO_DELIVER_THREAD_ID", default=_UNSET)
...
def set_current_session_id(session_id: str) -> None:
    """Synchronize ``ANIKA_SESSION_ID`` across ContextVar and ``os.environ`` (tools read it
...
def get_session_env(name: str, default: str = "") -> str:
    """Read a session var by legacy ``ANIKA_SESSION_*`` name; drop-in for os.getenv.  The
...
# Surfaces that are not a human chat channel (gateway binds ANIKA_SESSION_PLATFORM, CLI/TUI/
# desktop bind ANIKA_SESSION_SOURCE, so both are consulted).  Default-deny: an unrecognized
...
def session_is_messaging_surface() -> bool:
    ...
    ``ANIKA_PLATFORM``, then the session platform, then the session source)."""
    platform = os.getenv("ANIKA_PLATFORM") or get_session_env("ANIKA_SESSION_PLATFORM", "")
    idents = (platform, get_session_env("ANIKA_SESSION_SOURCE", ""))
...
def declare_stateless_channel() -> None:
    ...
    See __NEW_ORG__/anika-agent#53027 and #63142.
    """
...
def async_delivery_supported() -> bool:
    ...
    (``ANIKA_KANBAN_TASK``: one-shot subprocesses whose parent disappears after the turn)."""
