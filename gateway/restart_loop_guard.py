...
Defenses 1-2 (``_ANIKA_GATEWAY`` guard on ``anika gateway stop|restart`` /
...
persisted to ``<ANIKA_HOME>/gateway/restart_loop.json`` and CHAIN while gaps stay
...
def _state_path():
    return get_anika_home() / "gateway" / "restart_loop.json"
