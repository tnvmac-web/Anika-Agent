...
SCALE_TO_ZERO_ENV = "ANIKA_SCALE_TO_ZERO"  # stamped by NAS when the Labs toggle is on
...
def dashboard_client_heartbeat_path(anika_home: Optional[os.PathLike | str] = None):
    """Path of the dashboard-client liveness marker under ANIKA_HOME."""
    if anika_home is None:
        from anika_constants import get_anika_home
        anika_home = get_anika_home()
    return Path(anika_home) / DASHBOARD_CLIENT_HEARTBEAT_REL
