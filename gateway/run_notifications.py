...
class GatewayNotificationsMixin:
    ...
    class _UpdatePaths:
        """Marker files ``anika update --gateway`` and its watcher exchange under ANIKA_HOME."""
    ...
    def _update_paths(cls) -> "GatewayNotificationsMixin._UpdatePaths":
        ...
        return cls._UpdatePaths(
            pending=_anika_home / ".update_pending.json",
            claimed=_anika_home / ".update_pending.claimed.json", output=_anika_home / ".update_output.txt",
            exit_code=_anika_home / ".update_exit_code",
            prompt=_anika_home / ".update_prompt.json", response=_anika_home / ".update_response",
        )
    ...
    async def _watch_update_progress(
        ...
        """Watch ``anika update --gateway``, streaming output + forwarding prompts.
        ...
        while loop.time() < deadline:
            if paths.exit_code.exists():
                ...
                with _log_suppressed(logging.WARNING, "Update final notification failed: %s"):
                    ...
                    await target.send(
                        "✅ Anika update finished." if exit_code == 0
                        else "❌ Anika update failed (exit code {}).".format(exit_code)
                    )
        ...
        if not paths.exit_code.exists():
            ...
            await _flush_buffer()
            with suppress(Exception):
                await target.send("❌ Anika update timed out after 30 minutes.")
            self._clear_update_markers(paths, session_key)
        ...
        try:
            ...
            if chat_id:
                ...
                if output:
                    if len(output) > 3500:
                        output = "…" + output[-3500:]
                    status = "✅ Anika update finished." if exit_code == 0 else "❌ Anika update failed."
                    msg = f"{status}\n\n```\n{output}\n```"
                else:
                    msg = (
                        "✅ Anika update finished successfully." if exit_code == 0 else
                        "❌ Anika update failed. Check the gateway logs or run `anika update` manually for details."
                    )
    ...
    async def _send_restart_notification(self) -> Optional[tuple[str, str, Optional[str]]]:
        ...
        from gateway.run import _anika_home, _non_conversational_metadata
        notify_path = _anika_home / ".restart_notify.json"
    ...
    async def _send_home_channel_startup_notifications(
        ...
        message = "♻️ Gateway online — Anika is back and ready."
    ...
    async def _send_session_db_warning_notifications(self) -> None:
        ...
        if not error:
            return
        from anika_state import _default_db_path, classify_persistence_error, format_session_db_unavailable
        if classify_persistence_error(error) == "corrupt":
            # Copy-pasteable, so name the real store (profiles / ANIKA_HOME do not live under ~/.anika).
            db_path = _default_db_path()
            message = (
                "⚠️ Session database corruption detected. Messages may not be "
                "persisted. Recovery options:\n"
                "1. Run `anika doctor --fix`\n"
                "2. Stop the gateway, then recover with:\n"
                f"   anika sessions recover --source {db_path} "
                "--inspect-only\n"
                "   (if it reports recoverable) anika sessions recover "
                ...
                "3. Restore from a backup in ~/.anika/backups/\n"
                "Run `anika doctor` for sanitized diagnostics."
            )
        else:
            message = (
                f"⚠️ Session database unavailable — messages may not be persisted. "
                f"{format_session_db_unavailable()}\nRun `anika doctor` for diagnostics."
            )
    ...
    async def _inject_watch_notification(self, synth_text: str, evt: dict) -> Optional[bool]:
        ...
        if not source:
            # API-server sessions bind the RAW X-Anika-Session-Id key, not a structured ``agent:...`` key.
