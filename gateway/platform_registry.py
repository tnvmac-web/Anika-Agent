...
class PlatformEntry:
    ...
    required_env: list = field(default_factory=list)  # ``anika setup`` display
    ...
    plugin_name: str = ""  # owning manifest so ``anika gateway setup`` can auto-enable it
...
class PlatformRegistry:
    ...
    def __init__(self) -> None:
        ...
        # Plugin adapters are isolated per resolved ANIKA_HOME and overlay the
        ...
        # `anika` invocation, so the import happens only when a lookup asks for it.
