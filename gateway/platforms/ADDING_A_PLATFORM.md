...
There are two ways to add a platform to the Anika gateway:

## Plugin Path (Recommended for Community/Third-Party)

Create a plugin directory in `~/.anika/plugins/` (or under `plugins/platforms/`
...
**zero changes to core Anika code**.
...
- `env_enablement_fn: () -> Optional[dict]` — seeds `PlatformConfig.extra`
  ...
  `anika gateway status` or `get_connected_platforms()` until the SDK
  instantiates.
...
- `plugin.yaml` `requires_env` / `optional_env` rich-dict entries —
  auto-populate `OPTIONAL_ENV_VARS` in `anika_cli/config.py` so the setup
...
Checklist for integrating a platform directly into the Anika core.
...
"anika-your-platform": {
    ...
    "tools": _ANIKA_CORE_TOOLS,
    "includes": []
},
```

And add it to the `anika-gateway` composite:

```python
"anika-gateway": {
    "includes": [..., "anika-your-platform"]
}
```
...
## 12. Status Display (`anika_cli/status.py`)
...
## 13. Gateway Setup Wizard (`anika_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ anika_cli/ toolsets.py \
  --include="*.py" -l | sort -u
