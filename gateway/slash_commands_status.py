...
class GatewayStatusCommandsMixin:
    ...
    async def _handle_topup_command(self, event: MessageEvent) -> str:
        """Handle /topup -- show the __NEW_ORG__ balance and hand off to the portal. Does NOT charge, confirm,
        ...
        lines = ["💳 **__NEW_ORG__ balance**"] + [ln for ln in view.balance_lines if not ln.lstrip().startswith("📈")]
    ...
    async def _handle_usage_command(self, event: MessageEvent) -> str:
        ...
        # __NEW_ORG__ credits + monthly-grant gauge (shared with CLI/TUI). Gates on "a __NEW_ORG__ account is
        # logged in" — NOT the inference provider — so a __NEW_ORG__ user inferring elsewhere still sees
    ...
    async def _handle_insights_command(self, event: MessageEvent) -> str:
        ...
        try:
            ...
            # Not a bare hop: ``SessionDB()`` resolves ``get_anika_home()`` at call time, a
