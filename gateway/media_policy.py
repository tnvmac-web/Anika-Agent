...
``validate_media_delivery_path`` reads ``ANIKA_MEDIA_DELIVERY_STRICT`` (gateway.strict),
``ANIKA_MEDIA_ALLOW_DIRS`` (gateway.media_delivery_allow_dirs) and
``ANIKA_MEDIA_TRUST_RECENT_FILES`` (gateway.trust_recent_files).  Every delivery
entrypoint (gateway startup, ``anika cron run``, ``anika send``) calls
...
_FLAG_ENVS = (("strict", "ANIKA_MEDIA_DELIVERY_STRICT"), ("trust_recent_files", "ANIKA_MEDIA_TRUST_RECENT_FILES"))
_ALLOW_DIRS_ENV = "ANIKA_MEDIA_ALLOW_DIRS"
