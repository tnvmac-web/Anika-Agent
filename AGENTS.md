# Anika Agent - Development Guide

Instructions for AI coding assistants and developers working on the anika-agent codebase.
...
Anika is a personal AI agent that runs the same agent core across a CLI, a messaging
...
Read the balance right: Anika ships a **lot**. Most merges are bug fixes to reported
...
- **Expand reach at the edges.** New adapters, channels, providers, models, desktop/TUI/
  ...
  the existing setup/config UX (`anika tools`, `anika setup`, auto-install) rather than
...
- **E2E validation, not just green unit mocks.** Anything touching resolution chains, config
  ...
  real path with real imports against a temp `ANIKA_HOME`. Mocks hide integration bugs.
...
- **New `ANIKA_*` env vars for non-secret config.** `.env` is for secrets only. Behavioral
...
- **Outbound telemetry / usage attribution without opt-in gating.** No analytics,
  ...
  (config gate + setup prompt + `anika tools` toggle) exists. Park behind a label.
...
- **Third-party products integrated into the core tree.** Observability backends, vendor
  ...
  backend we don't own. Ship as a **standalone plugin repo** (`~/.anika/plugins/` or pip
  entry point), promoted in the __NEW_ORG__ Discord `#plugins-skills-and-skins`. This is a
...
2. **CLI command + skill** — config/state/infra expressible as shell commands; the agent runs
   `anika <subcommand>` guided by a skill. Default for subscriptions, scheduled tasks,
   service setup (`anika webhook`, `anika cron`, `anika tools`).
...
4. **Plugin** — third-party/niche/user-specific; lives in `~/.anika/plugins/` or a pip
...
drive a locally spawned backend, one over SSH, one behind URL + token, or Anika Cloud, and
only the first two carry `ANIKA_DESKTOP=1`. An env-keyed gate is a silent no-op on the other
topologies — the tool is stripped from the schema while the platform hint tells the model it
is "inside the Anika desktop app". The pattern:

- **The toolset is the surface gate.** Keep such tools off `_ANIKA_CORE_TOOLS` and in a named
...
- **Ask which identity you mean.** `ANIKA_DESKTOP=1` legitimately means "this backend was
  spawned by the app" (cron ticker, web-dist handling). It does NOT mean "a GUI is watching";
  the embedded terminal pane (`anika --tui` against that backend) is the counterexample.
...
```
`scripts/run_tests.sh` probes `.venv`, then `venv`, then `$HOME/.anika/anika-agent/venv`
...
├── toolsets.py           # TOOLSETS dict, _ANIKA_CORE_TOOLS
├── cli.py                # AnikaCLI (REPL, slash dispatch) + anika_cli/cli_*_mixin.py
├── anika_state.py       # SessionDB facade; anika_state_*.py siblings
├── anika_constants.py   # get_anika_home(), display_anika_home() — profile-aware paths
├── anika_logging.py     # agent.log / errors.log / gateway.log (profile-aware)
├── batch_runner.py       # Parallel batch processing
├── agent/                # turn_*.py loop phases, providers, memory, compression, prompt builder
├── anika_cli/           # CLI subcommands, setup, config, plugins loader, skins, updater
...
├── ui-tui/               # Ink (React) terminal UI — `anika --tui`
...
**User state:** `~/.anika/config.yaml` (settings), `~/.anika/.env` (secrets only),
`~/.anika/logs/` (`agent.log` INFO+, `errors.log` WARNING+, `gateway.log`); all
profile-aware via `get_anika_home()`. Browse logs with `anika logs [--follow] [--level] [--session]`.
...
families: `anika_state.py` (21), `gateway/run.py` (15), `tools/mcp_tool.py` (15),
`anika_cli/kanban.py` (14), `anika_cli/web_server.py` (13 + 24 routers), `anika_cli/auth.py`
(12), `tools/browser_tool.py` (11), `cli.py` (12 `anika_cli/cli_*_mixin.py`), `run_agent.py`
...
- **Compat pointers are OFF LIMITS in-tree.** Old import paths kept alive for external plugins
  ...
  `-W error::anika_cli.plugin_compat.AnikaPluginCompatWarning` catches them in the suite.
...
- **Never infer process identity from argv substrings** (`"serve" in cmdline`) — the bug class
  ...
  `anika_cli.update_cmd._anika_holder_subcommand`; flag sets are DERIVED from the parser
  (`_holder_value_flags()`), never hand-written; match FULL cmdlines and truncate only for
  display. Details: `anika_cli/AGENTS.md`.
- **Never hardcode `~/.anika`.** `get_anika_home()` for code paths, `display_anika_home()`
  for user-facing text (both from `anika_constants`). Hardcoding breaks profiles (5 bugs in
  PR #3575). Module-level constants are fine — they cache after `_apply_profile_override()`
  sets `ANIKA_HOME`. Profile operations themselves are HOME-anchored
  (`_get_profiles_root()` = `Path.home()/.anika/profiles`) so `anika -p x profile list`
  sees all profiles — intentional, not a bug.
- **Argparse alias dispatch:** `add_parser("list", aliases=["ls"])` sets `dest` to the literal
  the user typed (`"ls"`). Dispatch must accept both (caught PTY-testing `anika webhook ls`).
- **Don't wire in dead code without E2E validation.** Unshipped code was dead for a reason;
  E2E the real resolution chain with real imports against a temp `ANIKA_HOME` first.
...
vars unset, `TZ=UTC`, `LANG=C.UTF-8`, `ANIKA_HOME` → temp dir, and per-file subprocess
...
- **Flake policy:** a failing FILE is retried once in a fresh subprocess (`--file-retries`;
  `ANIKA_TEST_FILE_RETRIES=0` disables). Pass-on-retry is green but printed under `⚠ FLAKY`
...
- **Tests must not write to `~/.anika/`.** The autouse `_isolate_anika_home` fixture in
  `tests/conftest.py` redirects `ANIKA_HOME`; never hardcode `~/.anika/` in tests. Profile
  tests also mock `Path.home()` so `_get_profiles_root()` / `_get_default_anika_home()` stay
  in the temp dir (pattern: `tests/anika_cli/test_profiles.py`):
  ```python
  @pytest.fixture
  def profile_env(tmp_path, monkeypatch):
      home = tmp_path / ".anika"; home.mkdir()
      monkeypatch.setattr(Path, "home", lambda: tmp_path)
      monkeypatch.setenv("ANIKA_HOME", str(home))
      return home
  ```
  Tests that `patch.object(Path, "home", ...)` must ALSO set `ANIKA_HOME` — code reads the
  env var, not `Path.home()/.anika`.
...
`tests/anika_cli/test_venv_holder_windows_live.py` on a real `windows-latest` runner (real
...
| `cli.py`, `anika_cli/`, `main.py` | `anika_cli/AGENTS.md` | CLI mixins, `_SLASH_DISPATCH`, slash registry, config system + loaders, skins, `anika update` pipeline, profiles / multiplex |
...
| `plugins/`, `anika_cli/plugins*.py` | `plugins/AGENTS.md` | Plugin kinds, native compat contract, in-tree policy, Sep-2026 compat window |
| `tui_gateway/`, `ui-tui/` | `tui_gateway/AGENTS.md` | Process model, JSON-RPC transport, key surfaces, slash flow, dev commands |
| `web/`, `anika_cli/web_routers/` | `web/AGENTS.md` | Dashboard embeds the real TUI; what React may and may not rebuild |
...
| `cron/`, kanban (`anika_cli/kanban*.py`, `tools/kanban_tools.py`, `plugins/kanban/`) | `cron/AGENTS.md` | Scheduler invariants, job fields, kanban board/dispatcher |
...
`anika-agent-dev` skill, not here.
