"""state.db file-level health helpers, split out of ``anika_state.py``.
...
holder-process classification.  Helpers that anika_state itself imports and
calls (``_connect_tracked_db`` & co) are looked up lazily from ``anika_state`` at
call time, so tests that monkeypatch ``anika_state.<name>`` keep intercepting.
"""
...
# shared lock (see anika_cli/sqlite_safe_read.py) so another process can treat this writer as
...
def _read_sqlite_application_id(db_path: Path) -> "Optional[int]":
    ...
    from anika_state_errors import _STATE_DB_APPLICATION_ID_OFFSET
...
def refuse_deleted_wal_generation(db_path) -> None:
    ...
    *before* ``sqlite3.connect`` so a second opener cannot mint a replacement WAL inode."""
    from anika_state import DeletedWalGenerationError
    from anika_state_errors import _DELETED_WAL_GENERATION_MSG
...
def _connect_tracked_db(path, tracking_path=None, **kwargs):
    ...
    without anika_cli); a real connection failure must propagate — a silent untracked retry
    would disable the guard for that connection."""
    try:
        from anika_cli.sqlite_safe_read import connect_tracked
    except ImportError:
        logger.debug("anika_cli.sqlite_safe_read unavailable; opening %s untracked "
                     "(byte-probe guard inactive in this install)", path)
        return sqlite3.connect(str(path), **kwargs)
    # Open through THIS module's sqlite3.connect so tests patching anika_state.sqlite3.connect keep control.
...
def is_zeroed_state_db(path: Path, *, probe_bytes: int = 100, force: bool = False) -> bool:
    ...
    snapshots).  Prefers ``anika_cli.backup.is_zeroed_sqlite_file``; this copy keeps SessionDB
    ...
    with quarantine_cross_process_lock(path) as acquired:
        if not acquired:
            logger.error("quarantine lock for %s not acquired within 5s — refusing to "
                         ...
                         "state-snapshots via `anika snapshot list` / `anika snapshot restore <id>`.",
                         path)
            return None
        return _do_quarantine()
...
def collect_state_db_stats(db_path: Path) -> Dict[str, Any]:
    ...
    from anika_state import _connect_tracked_db
